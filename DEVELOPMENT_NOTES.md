...
- Landed behavior:
  ...
  - and the helper returns structured failure codes so non-diagnostic callers
    can degrade safely without duplicating the semantics.
- Why this boundary matters:
...
- Why this is worth shipping:
  - it removes an awkward direct-binding footgun without inventing a new syntax family,
  - it keeps the widening honest and local to the direct actual path instead of silently changing bounded concat semantics too,
  - and it still preserves the sharp boundary that `=open` is meaningful only for realized child input formals, not for declared top outputs.
...
- Design correction:
  ...
  - and even though the current implementation still routes those roots through shared direct single-module machinery, strict mode and the live docs should not describe them as standalone-DT aliases.
...
- Why this is worth shipping:
  ...
  - and it narrows the next honest seam to the remaining lower-level coordination across the extracted live owners rather than more cleanup around an already-retired pre-stage wrapper.
...
- Why this is worth shipping:
  ...
  - and it narrows the next honest seam to the remaining coordination across planning, prepared-block projection, stage preparation, and final rendering rather than stage preparation itself.
...
- Why this is worth shipping:
  ...
  - and it sharpens the next seam honestly: the small remaining frontend facade surface or deeper cleanup under `FlattenedDT` / `EnableGraph`, not more direct helper trimming.
...
- Landed behavior:
  ...
  - `HDLGenerator` now consumes those helpers when deciding whether to reuse, append, or rebind structural instance port bindings,
  - and the current bounded signal-ref binding behavior stays unchanged while depending even less on pipeline-local list mutation rules.
- Why this is worth shipping:
  - it removes another low-level binding-ownership pocket from `HDLGenerator`,
  - it makes the structural helper module a more complete owner of the first bounded binding family,
  - and it leaves the next seam where it belongs: either widen `connection_expr` in one bounded portable form or keep peeling higher-level wiring consumers off compatibility mirrors.
...
- Landed behavior:
  ...
  - `HDLGenerator` now uses those helpers when building `C1` passthrough bindings, broader composition planned child bindings, and structural instance rebinding paths,
  - and the existing bounded `signal_ref` structural behavior remains unchanged while depending even less on pipeline-local field pairing.
- Why this is worth shipping:
  - it keeps the first actual-connection family together as one owned helper surface,
  - it removes another repeated `signal_name` / `connection_expr` pairing rule from the pipeline,
...
- Landed behavior:
  ...
  - the public-only sibling now emits one shared top-level register plus next-value logic in the generated top,
  - contributor outputs are rebound to private raw nets,
  - and preserved public top outputs are fanned back out from that lifted register instead of staying bound directly to separate child-owned carriers.
- Why this is worth shipping:
  - it widens the registered ownership/runtime lane in a real user-visible way instead of another planning-only slice,
  - it starts addressing public re-export/default-visibility policy beyond the earlier peer-read-only cases,
  - and it leaves the next seam where it belongs: which broader registered outputs should still lift automatically, and how far the combinational sibling lane should widen.
...
- Landed behavior:
  - `?mod:name` and `?module:name` now classify, parse, and generate through the same current direct single-module machinery as `?dt:name`,
...
- Later continuity corrected the semantic reading of that slice:
  - the shared implementation path should not be read as proving that `?mod:` / `?module:` are semantic aliases of `?dt:`.
- Why this is worth shipping:
...
- Landed behavior:
  - named `?fsmc` parser failures now have explicit summary coverage across both source-count and source-shape branches,
  - and named `?dtc` parser failures now have explicit summary coverage across both source-count and source-shape branches too.
- Why this is worth shipping:
  - the extractor already supported these siblings through the same named-child diagnostic patterns,
...
- Why this is worth shipping:
  - this is a real user-facing explicit-link failure path,
  - the summary behavior already existed,
  - and the new coverage proves that the CLI keeps the full child endpoint token visible instead of forcing users back into the raw exception text.

## 2026-03-19: blocked missing top-level explicit-link endpoint summaries are now regression-locked at CLI level
- Continued the active `R11` failed-run reporting lane by hardening another already-shipped explicit-link summary family instead of widening composition behavior.
- Landed behavior:
...
- Why this is worth shipping:
  - this is a real user-facing explicit-link failure path,
  - the summary behavior already existed,
  - and the new coverage proves that the CLI keeps the missing top endpoint visible instead of forcing users back into the raw exception text.
...
- Continued the active `R11` failed-run reporting lane by hardening one more already-shipped `C4` summary family instead of widening composition behavior.
- Landed behavior:
  - the existing failed-run summary path now has explicit CLI regression coverage for a reachable blocked width-mismatch `C4` declared connect-by-name failure,
  - so same-name width mismatches keep the conflicting child endpoint set in the concise `Reason:` line while still preserving `Lane: C4`, `Construct: =port`, and top-port context.
- Why this is worth shipping:
  - width mismatch is another main user-facing declared connect-by-name failure mode,
  - the summary behavior already existed,
  - and this proves that the short summary still carries the endpoint evidence needed to fix the mismatch without forcing users back into the raw exception text.
...
- Continued the active `R11` failed-run reporting lane by hardening one more already-shipped `C4` summary family instead of widening composition behavior.
- Landed behavior:
  - the existing failed-run summary path now has explicit CLI regression coverage for a reachable blocked ambiguous `C4` declared connect-by-name failure,
  - so same-name ambiguity failures keep the compatible child endpoint list in the concise `Reason:` line while still preserving `Lane: C4`, `Construct: =port`, and top-port context.
- Why this is worth shipping:
  - ambiguity is one of the main user-facing declared connect-by-name failure modes,
  - the summary behavior already existed,
  - and this proves that the short summary still carries the actionable candidate set instead of forcing users back into the raw exception text.
...
- Landed behavior:
  ...
  - and `C4` now also works for exactly one generated child (`?fsmc` or `?dtc`) plus multiple external `?rtl` children.
- Why this is worth shipping:
...
- The summary extractor already recognized known child headers across constructs, but the dotted-pair child-item contract had still not been explicitly locked on the `?ports` side.
- The active contract is now explicit for the `?ports` sibling too:
  - blocked `?ports` dotted-pair payloads keep `Construct: ?ports`
...
- The next widening step is now also shipped through composition tops themselves:
  ...
  - that same composition-top `intent_hir` slice now also carries the broader generated-child export instead of leaving it only as a separate top-level compatibility summary,
...
- The next structural-consumption step is now also live:
  - composition-top `module_info` and `statistics` now consume `structural_rtl_ir` for child, top-port, and internal-net counts,
  - so that bounded top-level accounting no longer rereads those fields directly from plan internals either.
- The next structural-consumption step is now also live through composition provenance:
  - `composition_report` now consumes `structural_rtl_ir` for top-port metadata and resolved-link endpoint lookup,
  - so that bounded provenance/reporting no longer rereads those explicit boundary/interface details directly from plan internals either.
...
- The next narrowing step is now also live through the generated-child export path:
