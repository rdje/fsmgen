...
- Mixed scalar/aggregate operands, mismatched aggregate shapes, and non-bitwise aggregate operators such as aggregate `+` now abort before generation with targeted diagnostics.
...
- Updated the user guide, mdBook parameter/composition chapters, roadmap status, memory, and development notes so the shipped boundary is documented as bitwise-only aggregate operator support, with richer aggregate operators still future typed work.
...
- Clarified that this is a scalar-expression slice only: parameters/generics remain scalar-or-aggregate semantic values, and richer aggregate-to-aggregate operator expressions beyond the now-shipped bitwise slice remain future typed work that must define legal operators by aggregate type/shape before generation.
