...
- Updated [perl/FSM/Pipeline/HDLGenerator.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Pipeline/HDLGenerator.pm) so the pipeline can derive a small composition failure report from blocked composition diagnostics, including the failing top name or external RTL module when available, the active `C1` / `C2` / `C3` / `C4` lane when the raised diagnostic names it, the active syntax construct when the blocked diagnostic points clearly at one such as `?ports`, `?toplink`, `?rtl`, `?fsmc`, `?dtc`, or `=port`, plus one concise context subject such as the offending child header, top port, explicit endpoint, metadata file, or token, alongside the blocked boundary label and a concise blocked-reason excerpt.
...
- Added [t/131-composition-failure-summary-reporting.t](/Users/richarddje/Documents/github/fsmgen/t/131-composition-failure-summary-reporting.t) to lock both the pipeline-side failure-report extraction and the non-quiet CLI summary for blocked composition failures, including concise reason extraction for parser-scoped and `.rtlif` failures, concise subject extraction for unsupported-child, top-port, and explicit-link endpoint failures, lane extraction for `C1`- and `C3`-scoped failures, and construct extraction for `?ports`, `?rtl`, and `?toplink` while still avoiding invented construct summaries for unsupported child headers.
