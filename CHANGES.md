...
- Added [t/291-composition-rtl-child-source-shape-diagnostics.t](/Users/richarddje/Documents/github/fsmgen/t/291-composition-rtl-child-source-shape-diagnostics.t) so nested `?rtl` payloads and multiple flat module references fail explicitly; parameter/generic override blocks are documented as the next semantic instantiation contract instead of being accepted as raw target-HDL text.
- Updated the user guide, composition scope, mdBook composition chapters, legacy mapping note, and roadmap docs so the alias form and the parameter/generic override boundary are visible in user-facing and implementation-steering references.
