...
- The first bounded `Structural RTL IR` extraction slice is now also shipped:
  ...
  - and explicit `?toplink` may now use `=open`, `=0`, `=1`, and exact-width `=N'b...` / `=N'd...` / `=N'h...` as source actuals, with `=open` still targeting realized child inputs only while fixed-width literal actuals may now also drive declared top outputs, and linked planning preserving those bindings directly instead of inventing helper nets or undeclared same-name top inputs,
...
- Forward-IR note: source-side top-port bit/slice and bounded concat
  ...
  bindings.
- Forward-IR note: direct fixed-width literal actuals such as `=0`, `=1`,
  `=N'b...`, `=N'd...`, and `=N'h...` now also reach declared top outputs
  through that same explicit top-output assignment path, while `=open`
  remains the bounded child-input-only sibling because “leave this output
  unconnected” is not honest top-boundary wiring.
