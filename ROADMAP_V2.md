...
`.705` recorded the missing source-reference artifact blocker for that import
prerequisite. `.706` resolves the local source availability blocker by
importing the user-approved Arm AMBA AHB Protocol Specification PDF under
...
verification-output, backend-language variants, and VHDL remain deferred.
`.745` now ships the matching bounded public AHB aggregate byte-lane `.ahb`
profile aliases. The shipped paths are `ppif/ahb_interconnect_byte_lane.ahb`
and `ppif/ahb_interconnect_two_subordinate_byte_lane.ahb`; support identities
are `intent.ahb_profile_alias_interconnect_byte_lane` and
`intent.ahb_profile_alias_interconnect_two_subordinate_byte_lane`; source kind
is `ial2_profile_alias`; coverage keys are
...
HDL module remains `ahb_tb`; and child counts remain 3 and 4. The aliases
preserve `composition.byte_lane_propagation`, child `narrow_transfer_policy`,
...
AHB byte-lane in-word `SEQ` subordinate `.ahb` profile alias.
`.754` now ships `ppif/ahb_lite_subordinate_byte_lane_seq.ahb`, mirroring the
shipped generic `.ppif` source, support-accounting as
...
recorded the alias support identities, propagation reports, and alias-only
child `.ahb alias exposure` cleanup expectations. No behavior changed in
`.759`.
...
report `busy_insertion.beats=single`; the pre-repair requester/paired tests
counted only transitions. Source-backed correction: fixed-length AHB permits BUSY-to-SEQ
...
semantics remain the selected `.4` contract. `.6` selected `.7`, which now
ships byte-identical `ppif/ahb_requester_busy_insert_two.ahb`. Existing `.ahb`
...
four-child `ahb_tb`, exact artifacts/windows/owner policy, numeric requester
...
parity pass. The generic checkpoint is 321/362/45 split 23 `.ppif`/22 `.ahb`. See
...
evenly split 23 `.ppif`/23 `.ahb`; focused t1529 will own alias parity without
...
selection after the audit commits cleanly. See
...
Contract `.4` now selects proposed data-only alias implementation `.5`. A
warning-free in-memory reserved-label probe proves byte-identical IAL1/IAL0,
width three, numeric `beats=4`, identical BUSY-support residue, alias-only
residue removal, and targeted wrong-profile/wrong-object diagnostics through
existing machinery. `.5` will add exact support identity, focused t1536 parity
...
through existing generators. It selects exact intent/object/anchor/support and
coverage identities, 3 IAL1/4 IAL0 artifacts, width-three
`4 -> 3 -> 2 -> 1 -> 0`, requester `before_beat=2`/`beats=4`, child/propagated
BUSY parking, one-hot fabric ownership, normalized semantic/read-only MCP
parity, and assertion-enabled t1537 5/4/1/4/1/`44332211` runtime. Projected
accounting is 329/370/53 split 27 `.ppif`/26 `.ahb`. Pending `.3` owns only
data/source/test/support/docs implementation after this contract commits
cleanly. No behavior changes in `.2`; aliases, two-subordinate exact-four,
...
Implementation `.2` now ships declaration-aware unary reduction lowering.
Scalars and static bits use identity/complement; declared vectors use only the
required backend-owned `std_logic` OR/AND/XOR fold helpers, with signed casts
and helper-name collision protection. Range slices, invalid selects,
unresolved/compound/malformed/residual forms fail closed. The first
preservation run proved blanket vector rejection would regress existing AMBA
`HRESP` and APB `wait_ctr`/`addr_q` direct paths, so explicit helpers reconcile
compiler independence with shipped scope. t1542/t1543 and real t1420/t386
...
`TASK-TREE-FROZEN-LEGACY-DOC-WORKFLOW-SYNC.1`. At selection time, active
task-tree guidance still required writes to or called canonical the four
decision-`0007`-frozen legacy blobs, despite `COMMIT.md`, bootstrap
instructions, enforcement, and current practice using the layered memory
model. The child was scoped to repair only workflow prose and preserve decision
`0019`'s node-list/frontier rule. Import-tree
...
continuity-only. The authoritative ISF spec still lists 327 of 332 current
...
Clean selector commit `28d3e777a` activates only
...
changes during activation. Completed `.8` now accepts decision `0039` and the
exact `fsmgen vial` capabilities/check/format/plan/run contract: normal/terse
...
Implementation audit `.7.1` found a real pre-binder contract gap: the checked
...
- Documentation-pressure note: decision `0040` closes the reusable README
  ...
  free. No predecessor ceiling rises. `.19` executes declared verifiers and
  ...
  frozen views; completed `.5` bounds the exact rationale ledger; `.8` partitions Chapter 14 exactly once into 13 direct topic pages; clean `dc1c64afb` activates `.9` alone and `.26` stays deferred.
