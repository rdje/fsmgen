...
- The first bounded `Structural RTL IR` extraction slice is now also shipped:
  ...
  - and that same shipped declarative-scope lane now also covers the first bounded `+types` slice: direct roots, composition tops, and `?pkg:name` packages accept scalar aliases for `bit`, `(bits N)`, and named scalar aliases without declaration-order dependence, direct-root `+size` and local composition `?ports` widths may use those aliases, explicit type cycles fail clearly, and forward `symbol_contract` / mirrored `module_info` now also preserve local type names/counts plus canonical scalar type specs,
