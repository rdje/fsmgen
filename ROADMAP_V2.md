...
- The first bounded `Structural RTL IR` extraction slice is now also shipped:
  ...
  - and the first semantic package/import slice now also ships on that same literal-actual path and the direct generated-root path: `?top` may use bounded `+import` blocks, shared `?pkg:name` roots may be embedded or resolved through the normal search roots, namespaced package symbols such as `=shared.RESET_BYTE` or `=shared.mode.BUSY` now resolve onto the same direct/concat structural literal path, and direct `?fsm` / `?dt` roots may now also use bounded `+import` blocks whose namespaced package symbols resolve in assignment RHS expressions and guard equality conditions without claiming aggregate package payloads are already live,
