...
- The first bounded `Structural RTL IR` extraction slice is now also shipped:
  ...
  - and explicit `?toplink` may now use `=open`, scalar `=0` / `=1`, and exact-width `=N'b...` / `=N'd...` / `=N'h...` as source actuals, with `=open` still targeting realized child inputs only while direct scalar `=0` / `=1` widen to the realized child-input or declared top-output target width and exact-width literal actuals may now also drive declared top outputs, and linked planning preserving those bindings directly instead of inventing helper nets or undeclared same-name top inputs,
...
- Forward-IR note: direct scalar `=0` / `=1` actuals plus exact-width
  `=N'b...`, `=N'd...`, and `=N'h...` literal actuals now also reach
