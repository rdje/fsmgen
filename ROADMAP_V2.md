...
- The first bounded `Structural RTL IR` extraction slice is now also shipped:
  ...
  - and the semantic package/import lane now also ships one bounded whole-list aggregate slice on that same literal-actual path and the direct generated-root path: `?top` may use bounded `+import` blocks, shared `?pkg:name` roots may be embedded or resolved through the normal search roots, namespaced package symbols such as `=shared.RESET_BYTE`, `=shared.mode.BUSY`, `=shared.BYTES[1]`, `=shared.FRAME.flag`, and whole list roots such as `=shared.HEADER` or `=shared.TAIL` now resolve onto the same direct/concat structural literal path, direct `?fsm` / `?dt` roots may now also use bounded `+import` blocks whose namespaced package scalar leaves and whole list-valued aggregate roots resolve in assignment RHS expressions and guard equality conditions, and hash-like whole-aggregate package flow/types remain intentionally future work,
  - and that same bounded aggregate-value contract now also covers local direct-root `+constants`, so direct `?fsm` / `?dt` sources may use local aggregate references such as `BYTES[1]`, `FRAME.flag`, and `NEST.header.nibble` plus whole list-valued aggregate roots such as `BYTES` or `TAIL` in assignment RHS expressions and guard equality conditions without moving those values into packages first, while unresolved whole hash-like aggregates such as `FRAME` still fail explicitly and hash-like whole-aggregate local-symbol flow/types remain future work,
  - and later bounded aggregate values may now reuse previously declared named scalar ingredients instead of forcing raw numerics at every leaf: local direct roots, local composition tops, and `?pkg:name` packages may now build later aggregate values from earlier constants, enum members, and earlier whole list-valued roots such as `(HEADER (mode.BUSY RESET_BYTE))` and `(PACKET (HEADER mode.IDLE))`, while forward references and hash-root packing remain intentionally future work,
  - but that declaration-order sensitivity is only the current shipped boundary, not the intended language model: the future symbol/type lane should resolve `+constants`, `+enums`, and future `+types` over one root-wide or package-wide declarative scope so normal non-cyclic references stop depending on declaration order, with one explicit semantic resolution pass and clear cycle diagnostics instead of parser-order semantics,
...
- Forward-IR note: the older direct pre-stage SystemVerilog generation-
  ...
  direct scaffold/internal-declaration assembly plus
  direct enable-condition generation plus
  `GenerationPrescanPreparationSupport` directly before consolidated
