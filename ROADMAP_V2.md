...
- Forward-IR note: the matching direct-root structural-builder counterpart is
  ...
  input-port generated-enable RHS target connectivity, or the selected direct
  top generated-enable condition reroute inline either. Direct output-port
  source summaries are now selected as the next active structural bridge under
  `R11-DIRECT-STRUCTURAL-OUTPUT-CONSUMERS`; full direct module rerouting and
