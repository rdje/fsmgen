...
- The first bounded normalized semantic JSON export now exists through `FSM::Support::NormalizedSemanticReport` and `bin/fsmgen --emit-semantic-json`, emits sanitized module/system/signal and forward-IR projections without HDL emission or private Perl object leakage, reuses stable diagnostics on failures, and is corpus-covered across every current `supported_smoke` and `strict_supported` entry by `t/302-normalized-semantic-json.t` plus `t/303-normalized-semantic-json-supported-corpus.t`.
Left:
