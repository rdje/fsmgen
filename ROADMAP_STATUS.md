...
- The nested failure `diagnostic` object used by bounded check JSON now also has its own explicit contract owner through `FSM::Support::CheckFailureDiagnosticContract`; it stabilizes the core diagnostic keys plus the matched-only, optional-artifact, and nested support-accounting key families, is advertised through the check-report contract, and is regression-locked by `t/331-check-failure-diagnostic-contract.t`.
