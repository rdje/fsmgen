...
- The first bounded `HDLGenerator->generate_hdl_from_file(...)` result contract now exists through `FSM::Support::HDLGeneratorResultContract`; it stabilizes top-level key presence for in-process embedders while explicitly keeping live/raw nested content out of the JSON-safe public interchange promise.
