...
- Current next decision point:
  ...
  - The next bounded aggregate top-expression inference slice is now shipped too: declared aggregate top-port paths such as `in_frame.tag` now contribute exact leaf widths during explicit-toplink concat inference, so a remaining omitted whole operand such as `payload` can be sized from the target remainder, while undeclared aggregate roots fail with a user-facing declared-root diagnostic instead of an internal exact-width assertion.
...
- The next bounded aggregate top-expression inference slice is now also shipped:
  - [perl/FSM/Composition/TopPortInferenceBuilder.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Composition/TopPortInferenceBuilder.pm) now treats declared aggregate top-port paths such as `in_frame.tag` as exact-width operands while sizing one remaining omitted whole top-port concat operand,
  ...
  - and [t/288-composition-aggregate-top-expression-inference.t](/Users/richarddje/Documents/github/fsmgen/t/288-composition-aggregate-top-expression-inference.t) locks both the accepted inference path and the blocked undeclared-root diagnostic.
