...
- Current next decision point:
  ...
  - A dedicated live entrypoint/import-tree architecture note now exists in [docs/BIN_FSMGEN_IMPORT_TREE.md](/Users/richarddje/Documents/github/fsmgen/docs/BIN_FSMGEN_IMPORT_TREE.md); refresh it at the start of a later session whenever the effective package spine or hotspot picture has materially changed. The current measured snapshot there is now `119` reachable project files and `118` reachable `.pm` packages, with the semantic `FSM::Package::*` family and the new composition actual-literal, source-expression spec, and parameter/generic value support owners now explicit in that live spine.
...
Left:
- Extend the now-shipped semantic parameter/generic override contract beyond external `?rtl` SystemVerilog instance emission: VHDL generic-map lowering, generated-child parameterization, and richer non-literal semantic values remain follow-up work.
