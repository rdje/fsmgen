...
- Current next decision point:
  ...
  - The next emitted-aggregate-lowering slice is now also shipped on the composition path: structural SystemVerilog emission now synthesizes backend-owned local packed typedefs for declared aggregate aliases, so typed top ports and typed structural nets no longer flatten all the way back to raw vector declarations at the final composition boundary.
  - The next honest seam is now the matching direct generated-module path, so direct `?fsm` / `?dt` aggregate aliases stop flattening at the final emitted HDL boundary too instead of only preserving type identity in metadata and validation.
