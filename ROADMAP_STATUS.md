...
- Current next decision point:
  ...
  - The first shipped “overridden” and “blocked” reporting slices are now both in place for successful composition runs, and the first bounded failure-path blocked-wording slice is now shipped for plain explicit top-port same-name convention, so the next honest diagnostics step is broader failure-path wording beyond that rather than more hidden inference.
