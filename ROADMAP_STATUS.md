...
- Current next decision point:
  ...
  - The direct-root language surface now treats canonical init/default metadata and width declarations as expression-bearing intent slots: `(:= (signal value))` is the strict-safe canonical init/reset form, its `value` may be a nested Lisp-ish expression over constants/enums/params/aggregate scalar leaves, legacy compact `(:= signal=value)` remains default-mode compatibility residue, and `+size` width entries may use positive integer constant expressions over the same semantic leaves before generation. The shared scalar literal path now also accepts common decimal, based SystemVerilog, `0x`, `0b`, and `0o` spellings consistently across direct/composition/package constants and positive integer scalar width symbols.
