...
- Current next decision point:
  ...
  - Broader external validation across all active `fsm/*.fsm` examples is not yet claimed: local reconnaissance still finds real combinational feedback loops in larger legacy/sample outputs such as `fsm/amba_requester.fsm`. The earlier AMBA modulo/division width-extension warning is fixed by preserving authored modulo/product grouping, but the feedback loops remain follow-up semantic hardening before widening `t/308` beyond the current focused `lte_dif_pmaster` plus MIPI smoke set.
