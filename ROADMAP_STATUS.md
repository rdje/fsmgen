...
- Current next decision point:
  ...
  - Broader external validation across all active `fsm/*.fsm` examples is not yet claimed: AMBA is now in the focused external smoke after the Q-named register-source fix, but the remaining historical/sample families still need audit before the project can claim every file under `fsm/` is warning-clean under Verilator/Yosys.
