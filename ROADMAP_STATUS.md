...
- Current next decision point:
  ...
  - A dedicated live entrypoint/import-tree architecture note now exists in [docs/BIN_FSMGEN_IMPORT_TREE.md](/Users/richarddje/Documents/github/fsmgen/docs/BIN_FSMGEN_IMPORT_TREE.md); refresh it at the start of a later session whenever the effective package spine or hotspot picture has materially changed. The current measured snapshot there is now `118` reachable project files and `117` reachable `.pm` packages, with the semantic `FSM::Package::*` family and the new composition actual-literal plus source-expression spec support owners now explicit in that live spine.
...
- Composition actual-literal lowering now has a dedicated owner:
  ...
  - and [docs/BIN_FSMGEN_IMPORT_TREE.md](/Users/richarddje/Documents/github/fsmgen/docs/BIN_FSMGEN_IMPORT_TREE.md) now records the updated `118` / `117` import-closure snapshot plus the new post-extraction hotspot shape.
