...
- Current next decision point:
  - Bounded check-only JSON diagnostics are now shipped: [bin/fsmgen](/Users/richarddje/Documents/github/fsmgen/bin/fsmgen) accepts `--check --json` and `--check-json`, runs the full pipeline without writing HDL, emits schema-versioned JSON to stdout, exits non-zero on failed checks, and classifies matched expected failures through [perl/FSM/Support/CheckDiagnostics.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/CheckDiagnostics.pm) using [perl/FSM/Support/RegressionCorpus.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/RegressionCorpus.pm) plus [perl/FSM/Support/DiagnosticCodes.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/DiagnosticCodes.pm). The next SPECFORGE-facing seam is check-JSON schema stabilization/widening only where fields are backed by support-accounting truth, not a broad normalized semantic export yet.
...
- The bounded check-only JSON diagnostic bridge is now shipped:
  ...
  - and [t/297-capability-manifest.t](/Users/richarddje/Documents/github/fsmgen/t/297-capability-manifest.t) locks the manifest's bounded check-JSON command metadata.
Left:
...
- Bounded check-only JSON diagnostics now exist through `FSM::Support::CheckDiagnostics` and `bin/fsmgen --check --json`, but the full diagnostic JSON schema is still intentionally narrow and should widen only from support-accounting truth.
Left:
...
- Stabilize/widen bounded check-only JSON diagnostics.
