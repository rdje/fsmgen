...
- Current next decision point:
  ...
  - The next honest aggregate-type seams are now broader inference-first aggregate access, aggregate target deconstruction/packing semantics, VHDL aggregate lowering, and public type/export surface stabilization rather than packed-width-only direct SystemVerilog declaration emission.
  - Future aggregate-assignment steering now records a possible intent-level pack/deconstruct lane: RHS pack should compose one LHS from static-width expressions, LHS deconstruct should split one RHS across legal static lvalues, authored left-to-right should map high-to-low, exact total width plus operand/type legality should be checked before generation, overlapping/duplicate LHS ranges should fail unless deliberately defined, and backends should receive normalized AST/IR assignments rather than renderer-side concat semantics.
