...
- The nested `semantic.explicit_system_contract` object used by successful bounded normalized semantic JSON now also has its own explicit contract owner through `FSM::Support::NormalizedSemanticExplicitSystemContract`; it stabilizes the explicit-system-contract key family when that authored contract is preserved, is advertised through both the semantic payload and normalized-semantic report contracts, and is regression-locked by `t/337-normalized-semantic-explicit-system-contract.t` plus the direct semantic-report regressions.
- The nested `semantic.system_contract` object used by successful bounded normalized semantic JSON now also has its own explicit contract owner through `FSM::Support::NormalizedSemanticSystemContract`; it stabilizes the system-contract key family, is advertised through both the semantic payload and normalized-semantic report contracts, and is regression-locked by `t/336-normalized-semantic-system-contract.t` plus the semantic-report regressions.
