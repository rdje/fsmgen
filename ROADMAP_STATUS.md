...
- Current next decision point:
  ...
  - The matching direct-root parameter cleanup is now shipped too: `(+params ...)` uses the neutral `FSM::ParameterValueSupport` owner, accepts the same bounded scalar integer formats and packed aggregate payloads as external RTL metadata, can now reuse resolved direct-root constants, enum members, aggregate constant roots, and already-parsed direct `+define` literals as parameter values, and records direct parameter metadata on the semantic module so Intent HIR exposes direct-root `parameter_names`.
