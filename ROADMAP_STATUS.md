...
- Current next decision point:
  ...
  - The first bounded failed-run composition-summary slice is now also shipped for non-quiet CLI runs: when a composition failure exposes a blocked boundary in the raised diagnostic, the CLI now prints a small composition-failure summary with the failing top or external RTL module context plus the blocked boundary label before re-raising the original error.
