...
- The first bounded `HDLGenerator->generate_hdl_from_file(...)` result contract now exists through `FSM::Support::HDLGeneratorResultContract`; it stabilizes top-level key presence plus the small nested identity slices `source_info.header` / `source_info.kind` and `module_info.module_name` / `module_info.source_root_kind` for in-process embedders while explicitly keeping broader live/raw nested content out of the JSON-safe public interchange promise.
