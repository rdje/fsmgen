...
- Current next decision point:
  ...
  - Broader external validation across all active `fsm/*.fsm` examples is not yet claimed: local reconnaissance still finds arithmetic operand-extension warnings around modulo/division and real combinational feedback loops in larger legacy/sample outputs such as `fsm/amba_requester.fsm`. Treat those as follow-up backend/semantic hardening before widening `t/308` beyond the current focused `lte_dif_pmaster` plus MIPI smoke set.
