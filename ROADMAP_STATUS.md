...
Left:
- Extend the now-shipped semantic parameter/generic contract beyond direct-root metadata, external `?rtl` SystemVerilog instance emission, generated `?fsmc` / `?dtc` SystemVerilog instance emission, and direct param-to-param default reuse: VHDL generic-map lowering and broader expression-valued semantic parameter domains remain follow-up work.
