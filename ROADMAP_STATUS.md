...
- The optional external generated-SystemVerilog validation lane now exists through `FSM::Support::HDLExternalValidation` and `bin/fsmgen --verify-hdl`; it runs Verilator lint and Yosys synthesis lowering after HDL emission when those tools are installed, and is regression-smoked by `t/308-systemverilog-external-validation.t`.
