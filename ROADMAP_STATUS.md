...
- Current next decision point:
  - The current `R12` slices promoted the recently hardened `.rtlif` output-direction system-role rejection, duplicate-port declaration rejection, unsupported port type rejection, invalid token-shape rejection, and non-positive width rejection into the regression corpus, each using isolated sidecar module names so the existing `uart_tx` missing-sidecar and focused diagnostic fixtures keep their exact meaning.
...
- That same composition-contract bucket now also accounts for token-scoped `.rtlif` metadata rejections:
  ...
  - [t/248-regression-corpus-accounting.t](/Users/richarddje/Documents/github/fsmgen/t/248-regression-corpus-accounting.t) now records `23` catalog entries and `13` explicit expected-failure entries,
