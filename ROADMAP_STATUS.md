...
- The typed extension/context contract now exists through `FSM::Support::ExtensionContract`; it stabilizes the current explicit object/module/config loading entrypoints, shipped hook names, context accessor names, and deliberate non-goals for embedders without reopening legacy plugin discovery. Its programmatic loading entry shapes are now also explicit: module entries are scalar `Module::Name` values, config-file entries are scalar non-empty config-file paths, direct extension objects must be blessed and provide at least one real supported hook method, and `FSM::Extension::Loader` / `FSM::Extension::Registry` reject malformed entries before `require`, file checks, opens, reference stringification, silent no-op extension state, or registry fallout can leak.
