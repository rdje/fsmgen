...
- The current saved SPECFORGE alignment guidance is tracked in [docs/SPECFORGE_FEEDBACK_RESPONSE.md](/Users/richarddje/Documents/github/fsmgen/docs/SPECFORGE_FEEDBACK_RESPONSE.md): FSMGen accepts the direction of becoming a precise `.fsm` contract authority for downstream tools, with strict mode as the generated-`.fsm` target, the first bounded `--capability-manifest` now shipped from support-accounting truth, stable diagnostic-code ownership now shipped before JSON diagnostic emission, check-only JSON diagnostics, normalized semantic JSON export, stronger clock/reset metadata, and later checked actor/channel/role/temporal/provenance metadata staged behind support accounting and `R13`.
...
- The expected-failure diagnostic-code bridge is now shipped:
  ...
  - and the capability manifest exposes both entry-level codes and the stable registry while keeping check-only JSON diagnostic emission explicitly not public yet.
Left:
...
- Stable diagnostic-code ownership now exists through `FSM::Support::DiagnosticCodes`; expected-failure corpus entries and the capability manifest expose those codes, while check-only JSON diagnostic emission remains a follow-up API step.
Left:
