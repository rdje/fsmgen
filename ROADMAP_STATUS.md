...
- Current next decision point:
  ...
  - The next `R11` slice ships the first external-RTL parameter/generic override contract: `.rtlif` may declare scalar or bounded aggregate `(params (NAME default_value) ...)` entries, `?rtl` instances may override those declared names through semantic `(params (NAME value) ...)` blocks, aggregate overrides are checked against the `.rtlif` default's inferred aggregate shape, and validated values now flow through `Instance`, `RealizedInstance`, structural RTL IR, and SystemVerilog `#(...)` instance-parameter emission. Generated-child parameterization and VHDL generic-map lowering remain follow-up work.
  ...
  - The matching direct-root parameter cleanup is now shipped too: `(+params ...)` uses the neutral `FSM::ParameterValueSupport` owner, accepts the same bounded scalar integer formats and packed aggregate payloads as external RTL metadata, can now reuse resolved direct-root constants, enum members, aggregate constant roots, and already-parsed direct `+define` literals as parameter values, records direct parameter metadata on the semantic module so Intent HIR exposes direct-root `parameter_names`, preserves direct parameter uses as `ParameterRef` AST leaves instead of literal-substituting their defaults, and emits direct SystemVerilog module `#(...)` parameter declarations. Generated-child parameterization remains the next meaningful follow-up because child internals now have real parameter references for future instance overrides to target.
...
Left:
- Extend the now-shipped semantic parameter/generic contract beyond direct-root parameter metadata and external `?rtl` SystemVerilog instance emission: VHDL generic-map lowering, generated-child parameterization, and richer non-literal semantic values remain follow-up work.
