...
Left:
- Extend the now-shipped semantic parameter/generic contract beyond direct-root metadata, external `?rtl` SystemVerilog instance emission, generated `?fsmc` / `?dtc` SystemVerilog instance emission, direct param-to-param default reuse, and the first bounded scalar operator-expression value slice: VHDL generic-map lowering plus richer expression-valued semantic parameter domains remain follow-up work, including typed aggregate-to-aggregate operator expressions for aggregate parameter/generic values where the operator is explicitly valid for the operand types/shapes.
