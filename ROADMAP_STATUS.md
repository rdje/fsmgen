...
- Current next decision point:
  ...
  - Broader external validation across all active `fsm/*.fsm` examples is not yet claimed: the direct APB requester/completer and AMBA requester are now in the focused corpus-backed external smoke, and the MIPI packet FIFO slice-width case is now covered too, but the latest all-`fsm/` audit still found known remaining blockers in the APB composition top, unsupported historical `?define`, malformed/legacy composition samples, one unsupported `!&` trial operator, and the remaining MIPI tester-control width cleanup before the project can claim every file under `fsm/` is warning-clean under Verilator/Yosys.
