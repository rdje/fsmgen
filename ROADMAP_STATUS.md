...
- Current next decision point:
  - Bounded check-only JSON diagnostics are now corpus-covered on both accepted and rejected support-accounting sides: [bin/fsmgen](/Users/richarddje/Documents/github/fsmgen/bin/fsmgen) accepts `--check --json` and `--check-json`, runs the full pipeline without writing HDL, emits schema-versioned JSON to stdout, exits non-zero on failed checks, and classifies matched expected failures through [perl/FSM/Support/CheckDiagnostics.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/CheckDiagnostics.pm) using [perl/FSM/Support/RegressionCorpus.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/RegressionCorpus.pm) plus [perl/FSM/Support/DiagnosticCodes.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/DiagnosticCodes.pm). Failed diagnostics now include a nested `support_accounting` object, the classifier prefers the most specific matching expected-error pattern, [t/300-check-json-regression-corpus.t](/Users/richarddje/Documents/github/fsmgen/t/300-check-json-regression-corpus.t) proves every current `expected_failure` entry emits its exact stable code and matched corpus contract through `--check-json`, and [t/301-check-json-supported-corpus.t](/Users/richarddje/Documents/github/fsmgen/t/301-check-json-supported-corpus.t) proves every current `supported_smoke` entry succeeds through `--check-json` while every current `strict_supported` entry succeeds through `--strict --check-json` without HDL emission. The next SPECFORGE-facing seam remains further check-JSON schema stabilization/widening only where fields are backed by support-accounting truth, not a broad normalized semantic export yet.
...
- The bounded check-only JSON diagnostic bridge is now shipped:
  ...
  - [t/301-check-json-supported-corpus.t](/Users/richarddje/Documents/github/fsmgen/t/301-check-json-supported-corpus.t) locks check-JSON acceptance across every current `supported_smoke` entry and strict check-JSON acceptance across every current `strict_supported` entry, including decodable success JSON, empty diagnostics, resolved source path, checked module/top identity, and no-HDL emission,
  - and [t/297-capability-manifest.t](/Users/richarddje/Documents/github/fsmgen/t/297-capability-manifest.t) locks the manifest's bounded check-JSON command metadata, support-accounting object flag, supported-smoke/strict-supported/expected-failure corpus-coverage flags, and most-specific-pattern classifier policy.
Left:
