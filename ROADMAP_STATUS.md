...
- The current saved SPECFORGE alignment guidance is tracked in [docs/SPECFORGE_FEEDBACK_RESPONSE.md](/Users/richarddje/Documents/github/fsmgen/docs/SPECFORGE_FEEDBACK_RESPONSE.md): FSMGen accepts the direction of becoming a precise `.fsm` contract authority for downstream tools, with strict mode as the generated-`.fsm` target, the first bounded `--capability-manifest` now shipped from support-accounting truth, stable diagnostic-code ownership and bounded check-only JSON diagnostics now shipped, normalized semantic JSON export, stronger clock/reset metadata, and later checked actor/channel/role/temporal/provenance metadata staged behind support accounting and `R13`.
...
- Define normalized semantic JSON export surfaces.
Exit criteria:
