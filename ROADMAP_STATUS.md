...
- The typed extension/context contract now exists through `FSM::Support::ExtensionContract`; it stabilizes the current explicit object/module/config loading entrypoints, shipped hook names, context accessor names, the direct registry/context constructor mechanics and payload shape, and deliberate non-goals for embedders without reopening legacy plugin discovery. Its programmatic loading entry shapes are now also explicit: module entries are scalar `Module::Name` values, config-file entries are scalar non-empty config-file paths, direct extension objects must be blessed and provide at least one real supported hook method, direct registry construction must use the exact class receiver plus unique supported scalar option names, direct registry dispatch contexts must be real matching-stage `FSM::Extension::Context` objects, direct context construction must use the exact class receiver plus unique supported scalar option names, context payloads must carry a supported stage, scalar source kind, and stage-specific raw/result data, the bounded source-kind family explicitly includes `fsm`, `dt`, and `composition`, and `FSM::Extension::Loader` / `FSM::Extension::Registry` / `FSM::Extension::Context` reject malformed entries before `require`, file checks, opens, reference stringification, silent no-op extension state, malformed-constructor fallout, malformed-context hook fallout, registry fallout, raw argument fallout, `bless` fallout, or payload-shape fallout can leak.
