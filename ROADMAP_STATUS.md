...
- Current next decision point:
  ...
  - A dedicated live entrypoint/import-tree architecture note now exists in [docs/BIN_FSMGEN_IMPORT_TREE.md](/Users/richarddje/Documents/github/fsmgen/docs/BIN_FSMGEN_IMPORT_TREE.md); refresh it at the start of a later session whenever the effective package spine or hotspot picture has materially changed. The current measured snapshot there is now `116` reachable project files and `115` reachable `.pm` packages, with the semantic `FSM::Package::*` family now explicit in that live spine.
