...
- Next decision point: the required README/bootstrap recovery pass was rerun before new implementation work. The live project-owned `FSM::...` transitive import closure reachable from [bin/fsmgen](/Users/richarddje/Documents/github/fsmgen/bin/fsmgen) remains topologically unchanged at `180` project files total, `179` `.pm` packages, no missing project modules, and unchanged family counts. [docs/BIN_FSMGEN_IMPORT_TREE.md](/Users/richarddje/Documents/github/fsmgen/docs/BIN_FSMGEN_IMPORT_TREE.md) now refreshes the measured public-surface line counts after the latest `R13` facade-boundary hardening: [perl/FSM/Pipeline/HDLGenerator.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Pipeline/HDLGenerator.pm) is `403` lines and [perl/FSM/Support/HDLGeneratorFacadeContract.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/HDLGeneratorFacadeContract.pm) is `222` lines. This is bootstrap/architecture-note maintenance only; lane status is unchanged, and the next honest implementation move remains another bounded `R13` runtime or negative-boundary audit around remaining embedder assumptions before widening any public API.
...
- The typed extension/context contract now exists through `FSM::Support::ExtensionContract`; it stabilizes the current explicit object/module/config loading entrypoints, shipped hook names, context accessor names, and deliberate non-goals for embedders without reopening legacy plugin discovery. Its programmatic loading entry shapes are now also explicit: module entries are scalar `Module::Name` values, config-file entries are scalar non-empty config-file paths, and `FSM::Extension::Loader` rejects undef/blank/reference entries before `require`, file checks, opens, or reference stringification can leak.
