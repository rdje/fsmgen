...
- Current next decision point:
  - Supported and expected-failure corpus metadata now have catalog-level shape gates in addition to behavior execution: [t/296-regression-corpus-supported-behavior.t](/Users/richarddje/Documents/github/fsmgen/t/296-regression-corpus-supported-behavior.t) runs every `supported_smoke` entry through default pipeline/CLI, then runs every `strict_supported` entry through strict pipeline/CLI regardless of fixture family; [t/248-regression-corpus-accounting.t](/Users/richarddje/Documents/github/fsmgen/t/248-regression-corpus-accounting.t) now locks strict-supported marker counts, requires every supported protocol and direct language-feature fixture to be strict-supported, requires every supported direct language-feature entry to carry non-empty compiled HDL-shape pattern metadata, requires every `expected_failure` entry to carry a compiled diagnostic regex, and requires every strict-rejection bucket to carry a compiled migration-hint regex.
...
- The first positive strict-mode acceptance markers are now support-accounted too:
  - [t/lib/FSM/Test/RegressionCorpus.pm](/Users/richarddje/Documents/github/fsmgen/t/lib/FSM/Test/RegressionCorpus.pm) tags all four supported protocol fixtures and all ten supported direct language-feature fixtures as `strict_supported`,
