...
- Current next decision point:
  ...
  - The first bounded failed-run composition-summary slice is now also shipped for non-quiet CLI runs: when a composition failure exposes a blocked boundary in the raised diagnostic, the CLI now prints a small composition-failure summary with the failing top or external RTL module context, a `Lane:` line when the blocked diagnostic names the active `C1` / `C2` / `C3` / `C4` lane, an additional concise child/top-port/metadata/token context line when that subject can be identified honestly, plus the blocked boundary label and a concise blocked-reason line before re-raising the original error.
