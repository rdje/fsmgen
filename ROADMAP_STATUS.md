...
- Current next decision point:
  ...
  - The bounded in-process `HDLGenerator` result contract is now manifest-backed and widened one step further: [perl/FSM/Support/HDLGeneratorResultContract.pm](/Users/richarddje/Documents/github/fsmgen/perl/FSM/Support/HDLGeneratorResultContract.pm) owns `embedding.hdl_generator_result`, [t/305-hdl-generator-result-contract.t](/Users/richarddje/Documents/github/fsmgen/t/305-hdl-generator-result-contract.t) proves direct-root and composition-root result top-level key presence plus the bounded nested identity slices `source_info.header` / `source_info.kind`, `module_info.module_name` / `module_info.source_root_kind`, and the bounded scalar `statistics` summary keys, and the same contract now also advertises that the top-level `intent_hir`, `lowered_rtl_ir`, and `structural_rtl_ir` hashes reuse the bounded normalized-semantic shell owners while the manifest still says the raw result hash is not JSON-safe as a whole.
