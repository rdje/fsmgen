...
- Current next decision point:
  ...
  - The matching bounded direct LHS deconstruct slice is now shipped: direct assignment targets may use `(concat ...)` or `(cat ...)` over static whole-signal, bit/slice, or aggregate-leaf lvalues; authored left-to-right targets map onto high-to-low RHS slices; exact operand/RHS widths, exact total-width agreement, and non-overlapping target ranges are validated before generation; and the enable-graph capture path lowers the deconstruct into ordinary normalized per-target assignments rather than leaving renderer-side concat semantics in the backend.
