...
- Current next decision point:
  ...
  - That same generated-child parser-summary path now also keeps unnamed-child context when the blocked diagnostic exposes it directly, so blocked unnamed `?fsmc` source-count failures keep `Construct: ?fsmc` plus `Child '?fsmc'`, while blocked unnamed `?dtc` source-shape failures keep `Construct: ?dtc` plus `Child '?dtc'` instead of dropping child context from the short summary.
