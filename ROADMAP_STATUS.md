...
- Current next decision point:
  ...
  - Broader external validation across all active `fsm/*.fsm` examples is not yet claimed: the direct APB requester/completer and AMBA requester are now in the focused corpus-backed external smoke, but the latest all-`fsm/` audit still found known remaining blockers in the APB composition top, unsupported historical `?define`, malformed/legacy composition samples, one unsupported `!&` trial operator, and a couple of MIPI width-cleanup cases before the project can claim every file under `fsm/` is warning-clean under Verilator/Yosys.
