...
Left:
- Extend the now-shipped semantic parameter/generic contract beyond direct-root metadata, external `?rtl` SystemVerilog instance emission, and generated `?fsmc` / `?dtc` SystemVerilog instance emission: VHDL generic-map lowering and richer non-literal semantic values remain follow-up work.
