...
(actor aggregate_rule_target_deferred
  ...
  (rule expose_mode ready
    (frame.mode mode_in)))
ISF
        qr/rule 'expose_mode' assignment target references aggregate storage path 'frame\.mode'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, rule assignment RHS scalar values or operands, or rule guard expression scalar operands/,
