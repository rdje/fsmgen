...
subtest 'rule aggregate diagnostics remain scalar-RHS-only' => sub {
...
ISF
    ...
    assert_parse_rejected(
        <<'ISF',
(actor aggregate_rule_expression_operand_deferred
  (types
    (type frame_t (record (mode (bits 2)) (flag bit))))
  (clock clk)
  (reset rst)
...
ISF
        qr/rule 'expose_mode' assignment RHS expression references aggregate storage path 'frame\.mode'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, or rule assignment RHS scalar leaf reads/,
        'aggregate rule RHS expression operands remain deferred',
    );
...
(actor aggregate_rule_target_deferred
  ...
  (rule expose_mode ready
    (frame.mode mode_in)))
ISF
        qr/rule 'expose_mode' assignment target references aggregate storage path 'frame\.mode'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, or rule assignment RHS scalar leaf reads/,
