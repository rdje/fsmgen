...
subtest 'HDLGenerator statistics contract full surface survives JSON round trip' => sub {
    my $contract = build_hdl_generator_statistics_contract();
    my $decoded = decode_json(encode_json($contract));
    is_deeply($decoded, $contract, 'decoded statistics contract matches owner contract');
};
