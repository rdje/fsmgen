...
subtest 'manifest semantic-exports contract identity metadata rebuilds cleanly after caller mutation' => sub {
    ...
    is($contract->{'status'}, $expected->{'status'}, 'fresh manifest semantic-exports contract rebuilds clean status');
    is($contract->{'contract_source'}, $expected->{'contract_source'}, 'fresh manifest semantic-exports contract rebuilds clean contract_source');
    is($contract->{'report_source'}, $expected->{'report_source'}, 'fresh manifest semantic-exports contract rebuilds clean report_source');
