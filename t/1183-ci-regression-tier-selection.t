...
subtest 'list mode advertises concrete quick and ISF test tiers' => sub {
    ...
    like($result->{stdout}, qr/t\/1234-isf-disjoint-rule-writes\.t/, 'ISF tier includes the latest ISF disjoint-rule write test');
...
subtest 'dry-run modes select the expected command families' => sub {
    ...
    like($isf->{stdout}, qr/t\/1234-isf-disjoint-rule-writes\.t/, 'ISF dry-run includes latest ISF disjoint-rule write test');
