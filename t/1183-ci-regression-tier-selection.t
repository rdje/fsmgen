...
subtest 'list mode advertises concrete quick and ISF test tiers' => sub {
    ...
    like($result->{stdout}, qr/t\/1245-isf-transaction-loop-lowering\.t/, 'ISF tier includes the latest ISF loop lowering test');
...
subtest 'dry-run modes select the expected command families' => sub {
    ...
    like($isf->{stdout}, qr/t\/1245-isf-transaction-loop-lowering\.t/, 'ISF dry-run includes latest ISF loop lowering test');
