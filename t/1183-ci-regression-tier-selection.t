...
subtest 'list mode advertises concrete quick and ISF test tiers' => sub {
    ...
    like($result->{stdout}, qr/t\/1244-isf-wait-clause-lowering\.t/, 'ISF tier includes the latest ISF wait lowering test');
...
subtest 'dry-run modes select the expected command families' => sub {
    ...
    like($isf->{stdout}, qr/t\/1244-isf-wait-clause-lowering\.t/, 'ISF dry-run includes latest ISF wait lowering test');
