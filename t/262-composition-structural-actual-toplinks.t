...
subtest 'linked plan builder rejects actual endpoints as explicit link targets' => sub {
    ...
    like(
        $exception,
        qr/uses actual endpoint '=open' as an explicit link target, .*only allows '=open' and binary literal actuals as link sources into realized child input ports/s,
...
subtest 'linked plan builder rejects unsupported actual literal forms' => sub {
        ...
        FSM::Composition::LinkedPlanBuilder->build_from_toplinks(
            ...
            toplinks => [
                FSM::Composition::TopLink->new(
                    name => 'wiring',
                    links => [
                        FSM::Composition::Link->new(source => "=8'hAA", target => 'uart_tx.data_in'),
                    ],
                ),
            ],
            realized_instances => [
    ...
    like(
        $exception,
        qr/uses actual endpoint '=8'hAA', .*currently accepts only '=open', '=0', '=1', or binary literal forms like '=8'b10100101'/s,
        'builder keeps the first structural-actual slice limited to open and binary literal sources',
    );
};
