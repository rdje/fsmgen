...
subtest 'manifest-embedded HDLGenerator semantic-layer presence key-family map rebuild cleanly after caller mutation' => sub {
    my $first = build_capability_manifest();
    my $mutated = $first->{embedding}{hdl_generator_result};

    mutate_structure($mutated->{'semantic_layer_presence_key_family_map'});
    ...
    is_deeply($contract->{'semantic_layer_presence_key_family_map'}, $expected->{'semantic_layer_presence_key_family_map'}, 'fresh manifest result contract rebuilds clean semantic_layer_presence_key_family_map');
