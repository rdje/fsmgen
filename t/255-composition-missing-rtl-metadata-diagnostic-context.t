...
subtest 'missing external rtl metadata keeps expected-artifact context' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $composition_path = File::Spec->catfile($tempdir, 'missing_rtlif_context_top.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'missing_rtlif_context_top.sv');

    write_file(
        $composition_path,
        <<'FSM'
(?top:missing_rtlif_context_top
  (?ports:public_io
    clk
    rstn
    serial_out>
...
FSM
    ...
    ok(!$success, 'CLI rejects unresolved external rtl metadata');
    ok(!-e $output_path, 'CLI does not emit HDL for unresolved external rtl metadata');
