...
sub capture {
    my (%args) = @_;
    return FSM::VIAL::Backend::Runner->_test_capture_process(
        $repo_root,
        [$^X, '-e', $args{program}],
        10,
        $args{limit},
...
subtest 'the exact aggregate capture limit remains accepted' => sub {
    ...
    ok(FSM::VIAL::Backend::Runner->_test_process_succeeded($result),
        'Runner accepts the exact-limit process outcome');
...
subtest 'one aggregate byte beyond the limit fails closed' => sub {
    ...
    ok(!FSM::VIAL::Backend::Runner->_test_process_succeeded($result),
        'Runner rejects the exhausted capture outcome');
...
subtest 'ordinary nonzero tool exit remains distinct from output exhaustion' => sub {
    ...
    ok(!FSM::VIAL::Backend::Runner->_test_process_succeeded($result),
        'Runner cannot accept a captured nonzero tool exit');
