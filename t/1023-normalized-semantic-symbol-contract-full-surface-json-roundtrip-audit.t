...
subtest 'normalized semantic symbol contract full surface survives JSON round trip' => sub {
    my $contract = build_normalized_semantic_symbol_contract();
    my $decoded = decode_json(encode_json($contract));
    is_deeply(
        $decoded,
        $contract,
        'decoded normalized semantic symbol contract matches direct owner contract'
    );
};
