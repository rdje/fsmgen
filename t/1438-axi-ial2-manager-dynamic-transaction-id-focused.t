...
sub assert_dynamic_behavior {
    ...
    if ($case->{behavior} eq 'mixed_dynamic_static_write_demux') {
        ...
        like($isf, qr/\(rule axi0_w1_static_busy_release \(& axi0_w1_complete axi0_w1_static_busy_q\)/, 'mixed write demux releases static busy state');
    ...
    if ($case->{behavior} eq 'mixed_dynamic_static_write_demux_multi_static') {
        ...
        like($isf, qr/\(input axi0_w1_request\)/, 'multi-static mixed write demux declares first static request input');
        like($isf, qr/\(input axi0_w2_request\)/, 'multi-static mixed write demux declares second static request input');
        ...
        like($isf, qr/\(output axi0_w1_complete\)/, 'multi-static mixed write demux exposes first static completion output');
        like($isf, qr/\(output axi0_w2_complete\)/, 'multi-static mixed write demux exposes second static completion output');
        ...
        like($isf, qr/\(var axi0_w1_static_busy_q \(width 1\)\)/, 'multi-static mixed write demux allocates first static busy storage');
        like($isf, qr/\(var axi0_w2_static_busy_q \(width 1\)\)/, 'multi-static mixed write demux allocates second static busy storage');
        like($isf, qr/\(rule axi0_w0_dynamic_id_capture\b/, 'multi-static mixed write demux emits dynamic ID capture rule');
        like($isf, qr/\(! \(& axi0_w1_request/, 'multi-static mixed write demux prevents dynamic capture during first static request');
        like($isf, qr/\(! \(& axi0_w2_request/, 'multi-static mixed write demux prevents dynamic capture during second static request');
        like($isf, qr/\(! \(== axi0_awid 4'd3\)\)/, 'multi-static mixed write demux prevents dynamic capture of first static concrete ID');
        like($isf, qr/\(! \(== axi0_awid 4'd5\)\)/, 'multi-static mixed write demux prevents dynamic capture of second static concrete ID');
        like($isf, qr/\(axi0_w0_dynamic_id_q axi0_awid\)/, 'multi-static mixed write demux captures dynamic AWID');
        like($isf, qr/\(rule axi0_w1_static_busy_capture \(& \(& axi0_w1_request/, 'multi-static mixed write demux captures first admitted static request busy state');
        like($isf, qr/\(rule axi0_w1_static_busy_capture[\s\S]*\(! \(& axi0_w2_request/, 'multi-static mixed write demux blocks first static capture during second static request');
        like($isf, qr/\(rule axi0_w2_static_busy_capture \(& \(& axi0_w2_request/, 'multi-static mixed write demux captures second admitted static request busy state');
        like($isf, qr/\(rule axi0_w2_static_busy_capture[\s\S]*\(! \(& axi0_w1_request/, 'multi-static mixed write demux blocks second static capture during first static request');
        ...
        like($isf, qr/\(rule axi0_w1_response_demux \(& axi0_write_complete axi0_w1_static_busy_q \(== axi0_bid 4'd3\)\)/, 'multi-static mixed write demux matches first static concrete BID');
        like($isf, qr/\(rule axi0_w2_response_demux \(& axi0_write_complete axi0_w2_static_busy_q \(== axi0_bid 4'd5\)\)/, 'multi-static mixed write demux matches second static concrete BID');
        like($isf, qr/\(rule axi0_w1_static_busy_release \(& axi0_w1_complete axi0_w1_static_busy_q\)/, 'multi-static mixed write demux releases first static busy state');
        like($isf, qr/\(rule axi0_w2_static_busy_release \(& axi0_w2_complete axi0_w2_static_busy_q\)/, 'multi-static mixed write demux releases second static busy state');
        ...
        like($isf, qr/axi0 w2 static completion releases active concrete ID/, 'multi-static mixed write demux emits second static completion-active assertion');
        assert_mixed_dynamic_static_write_multi_static_report($result->{report});
        ...
        like($fsm, qr/\(-axi0_w1_response_demux\s+<\(& axi0_write_complete axi0_w1_static_busy_q \(== axi0_bid 4'd3\)\)/, 'scheduled FSM lowers multi-static mixed first static BID match');
        like($fsm, qr/\(-axi0_w2_response_demux\s+<\(& axi0_write_complete axi0_w2_static_busy_q \(== axi0_bid 4'd5\)\)/, 'scheduled FSM lowers multi-static mixed second static BID match');
        like($fsm, qr/\(-axi0_w0_dynamic_id_capture\s+<\(& \(& axi0_w0_request[\s\S]*\(! \(== axi0_awid 4'd3\)\) \(! \(== axi0_awid 4'd5\)\)\)/, 'scheduled FSM lowers dynamic capture static-ID exclusions');
        my $hdl = hdl_for('axi0_capacity_status', $fsm);
        like($hdl, qr/\breg\s+\[3:0\]\s+axi0_w0_dynamic_id_q\b/, 'SystemVerilog declares multi-static mixed dynamic selected-ID state');
        like($hdl, qr/\breg\s+axi0_w1_static_busy_q\b/, 'SystemVerilog declares first mixed static busy state');
        like($hdl, qr/\breg\s+axi0_w2_static_busy_q\b/, 'SystemVerilog declares second mixed static busy state');
        ...
        like($hdl, qr/axi0_write_complete\s*&\s*axi0_w1_static_busy_q\s*&\s*\(axi0_bid\s*==\s*4'd3\)/, 'SystemVerilog lowers multi-static mixed first static response guard');
        like($hdl, qr/axi0_write_complete\s*&\s*axi0_w2_static_busy_q\s*&\s*\(axi0_bid\s*==\s*4'd5\)/, 'SystemVerilog lowers multi-static mixed second static response guard');
        return;
    }
    ...
    if ($case->{behavior} eq 'mixed_dynamic_static_read_demux_multi_static') {
        ...
        like($isf, qr/\(input axi0_r1_request\)/, 'multi-static mixed read demux declares first static request input');
        like($isf, qr/\(input axi0_r2_request\)/, 'multi-static mixed read demux declares second static request input');
        ...
        like($isf, qr/\(output axi0_r1_complete\)/, 'multi-static mixed read demux exposes first static completion output');
        like($isf, qr/\(output axi0_r2_complete\)/, 'multi-static mixed read demux exposes second static completion output');
        ...
        like($isf, qr/\(var axi0_r1_static_busy_q \(width 1\)\)/, 'multi-static mixed read demux allocates first static busy storage');
        like($isf, qr/\(var axi0_r2_static_busy_q \(width 1\)\)/, 'multi-static mixed read demux allocates second static busy storage');
        like($isf, qr/\(rule axi0_r0_dynamic_id_capture\b/, 'multi-static mixed read demux emits dynamic ID capture rule');
        like($isf, qr/\(! \(& axi0_r1_request/, 'multi-static mixed read demux prevents dynamic capture during first static request');
        like($isf, qr/\(! \(& axi0_r2_request/, 'multi-static mixed read demux prevents dynamic capture during second static request');
        like($isf, qr/\(! \(== axi0_arid 4'd3\)\)/, 'multi-static mixed read demux prevents dynamic capture of first static concrete ID');
        like($isf, qr/\(! \(== axi0_arid 4'd5\)\)/, 'multi-static mixed read demux prevents dynamic capture of second static concrete ID');
        like($isf, qr/\(axi0_r0_dynamic_id_q axi0_arid\)/, 'multi-static mixed read demux captures dynamic ARID');
        like($isf, qr/\(rule axi0_r1_static_busy_capture \(& \(& axi0_r1_request/, 'multi-static mixed read demux captures first admitted static request busy state');
        like($isf, qr/\(rule axi0_r1_static_busy_capture[\s\S]*\(! \(& axi0_r0_request/, 'multi-static mixed read demux blocks first static capture during dynamic request');
        like($isf, qr/\(rule axi0_r1_static_busy_capture[\s\S]*\(! \(& axi0_r2_request/, 'multi-static mixed read demux blocks first static capture during second static request');
        like($isf, qr/\(rule axi0_r2_static_busy_capture \(& \(& axi0_r2_request/, 'multi-static mixed read demux captures second admitted static request busy state');
        like($isf, qr/\(rule axi0_r2_static_busy_capture[\s\S]*\(! \(& axi0_r0_request/, 'multi-static mixed read demux blocks second static capture during dynamic request');
        like($isf, qr/\(rule axi0_r2_static_busy_capture[\s\S]*\(! \(& axi0_r1_request/, 'multi-static mixed read demux blocks second static capture during first static request');
        ...
        like($isf, qr/\(rule axi0_r1_response_demux \(& axi0_read_complete axi0_r1_static_busy_q \(== axi0_rid 4'd3\)\)/, 'multi-static mixed read demux matches first static concrete RID');
        like($isf, qr/\(rule axi0_r2_response_demux \(& axi0_read_complete axi0_r2_static_busy_q \(== axi0_rid 4'd5\)\)/, 'multi-static mixed read demux matches second static concrete RID');
        like($isf, qr/\(rule axi0_r1_static_busy_release \(& axi0_r1_complete axi0_r1_static_busy_q\)/, 'multi-static mixed read demux releases first static busy state');
        like($isf, qr/\(rule axi0_r2_static_busy_release \(& axi0_r2_complete axi0_r2_static_busy_q\)/, 'multi-static mixed read demux releases second static busy state');
        ...
        like($isf, qr/axi0 r2 static completion releases active concrete ID/, 'multi-static mixed read demux emits second static completion-active assertion');
        ...
        like($fsm, qr/\(-axi0_r1_response_demux\s+<\(& axi0_read_complete axi0_r1_static_busy_q \(== axi0_rid 4'd3\)\)/, 'scheduled FSM lowers multi-static mixed first static RID match');
        like($fsm, qr/\(-axi0_r2_response_demux\s+<\(& axi0_read_complete axi0_r2_static_busy_q \(== axi0_rid 4'd5\)\)/, 'scheduled FSM lowers multi-static mixed second static RID match');
        like($fsm, qr/\(-axi0_r0_dynamic_id_capture\s+<\(& \(& axi0_r0_request[\s\S]*\(! \(== axi0_arid 4'd3\)\) \(! \(== axi0_arid 4'd5\)\)\)/, 'scheduled FSM lowers dynamic read capture static-ID exclusions');
        my $hdl = hdl_for('axi0_capacity_status', $fsm);
        like($hdl, qr/\breg\s+\[3:0\]\s+axi0_r0_dynamic_id_q\b/, 'SystemVerilog declares multi-static mixed read dynamic selected-ID state');
        like($hdl, qr/\breg\s+axi0_r1_static_busy_q\b/, 'SystemVerilog declares first mixed read static busy state');
        like($hdl, qr/\breg\s+axi0_r2_static_busy_q\b/, 'SystemVerilog declares second mixed read static busy state');
        ...
        like($hdl, qr/axi0_read_complete\s*&\s*axi0_r1_static_busy_q\s*&\s*\(axi0_rid\s*==\s*4'd3\)/, 'SystemVerilog lowers multi-static mixed first static read response guard');
        like($hdl, qr/axi0_read_complete\s*&\s*axi0_r2_static_busy_q\s*&\s*\(axi0_rid\s*==\s*4'd5\)/, 'SystemVerilog lowers multi-static mixed second static read response guard');
        return;
    }
...
sub assert_mixed_dynamic_static_write_multi_static_report {
    ...
    is_deeply($write->{static_transactions}, [qw(w1 w2)], 'multi-static mixed write report names covered static transactions');
    is_deeply($write->{mixed_transactions}, { dynamic => [qw(w0)], static => [qw(w1 w2)] }, 'multi-static mixed write report names dynamic/static transaction roles as lists');
    is_deeply(
    ...
    is_deeply($write->{generated_rules}, [qw(axi0_w0_response_demux axi0_w1_response_demux axi0_w2_response_demux)], 'multi-static mixed write report names generated response-demux rules');
    is_deeply($write->{generated_completion_signals}, [qw(axi0_w0_complete axi0_w1_complete axi0_w2_complete)], 'multi-static mixed write report names generated completions');
    is_deeply(
        $write->{generated_assertions},
        [qw(
            axi0_w0_dynamic_request_not_busy
...
sub assert_mixed_dynamic_static_read_multi_static_report {
    ...
    is_deeply($read->{static_transactions}, [qw(r1 r2)], 'multi-static mixed read report names covered static transactions');
    is_deeply($read->{mixed_transactions}, { dynamic => [qw(r0)], static => [qw(r1 r2)] }, 'multi-static mixed read report names dynamic/static transaction roles as lists');
    is_deeply(
    ...
    is_deeply($read->{generated_rules}, [qw(axi0_r0_response_demux axi0_r1_response_demux axi0_r2_response_demux)], 'multi-static mixed read report names generated response-demux rules');
    is_deeply($read->{generated_completion_signals}, [qw(axi0_r0_complete axi0_r1_complete axi0_r2_complete)], 'multi-static mixed read report names generated completions');
    is_deeply(
        $read->{generated_assertions},
        [qw(
            axi0_r0_dynamic_request_not_busy
