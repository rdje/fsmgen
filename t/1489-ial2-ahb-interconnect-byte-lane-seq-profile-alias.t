...
subtest 'adapter accepts selected AHB aggregate byte-lane SEQ .ahb profile aliases' => sub {
    for my $case (alias_cases()) {
        ...
        for my $child_index (@{$case->{child_indices}}) {
            is(
                ...
                'in_word_progressive',
                "$case->{label} .ahb child $child_index report propagates child SEQ policy",
            );
            is(
...
subtest 'aggregate byte-lane SEQ .ahb diagnostics stay fail-closed for malformed aliases' => sub {
    ...
    ok(!$missing_ok, 'aggregate byte-lane SEQ .ahb without explicit profile is rejected');
    like(
        $@,
        qr/\.ahb source '.*missing_profile\.ahb' is missing required \(profile \.\.\.\) clause/,
        'aggregate byte-lane SEQ .ahb missing-profile diagnostic is targeted',
    );
    ...
    ok(!$mismatch_ok, 'aggregate byte-lane SEQ .ahb with a non-AHB profile is rejected');
    like(
        $@,
        qr/profile 'valid-ready' does not match \.ahb profile alias; expected ahb/,
        'aggregate byte-lane SEQ .ahb suffix/profile mismatch diagnostic is targeted',
    );
    ...
    ok(!$scalar_wiring_ok, 'two-subordinate aggregate byte-lane SEQ .ahb with scalar subordinate wiring is rejected');
    like(
        $@,
        qr/two-subordinate wiring must omit scalar bus.subordinate_select/,
        'two-subordinate aggregate byte-lane SEQ .ahb scalar wiring diagnostic is targeted',
    );
};

subtest 'CLI JSON surfaces aggregate byte-lane SEQ .ahb source identity and support accounting' => sub {
    for my $case (alias_cases()) {
...
subtest 'schedule JSON and outdir expose aggregate byte-lane SEQ .ahb review artifacts' => sub {
    for my $case (alias_cases()) {
        ...
        for my $child_index (@{$case->{child_indices}}) {
            is($schedule->{children}[$child_index]{transfer}{seq_policy}{mode}, 'in_word_progressive', "$case->{label} schedule JSON exposes child SEQ policy for child $child_index");
        ...
        like(slurp(File::Spec->catfile($outdir, $case->{byte_lane_fsm})), qr/seq_expected_addr_q/, "$case->{label} generated byte-lane SEQ subordinate keeps expected-address state");
        like(slurp(File::Spec->catfile($outdir, $case->{byte_lane_fsm})), qr/read_halfword_lane1_hit_start/, "$case->{label} generated byte-lane SEQ subordinate keeps halfword path");
    }
};
...
subtest 'existing AHB aggregate and endpoint boundaries stay unchanged' => sub {
    ...
    ok(!exists $word_only_alias->{report}{composition}{seq_policy_propagation}, 'word-only aggregate .ahb does not gain SEQ propagation report');

    my $byte_lane_alias = FSM::Adapter::IAL2::PPIF->new()->parse_file(one_subordinate_byte_lane_alias_path());
    ok($byte_lane_alias->{report}{composition}{byte_lane_propagation}{selected}, 'existing aggregate byte-lane .ahb keeps byte-lane propagation');
    ok(!exists $byte_lane_alias->{report}{composition}{seq_policy_propagation}, 'existing aggregate byte-lane .ahb does not gain SEQ propagation report');

    my $byte_lane_seq_ppif = FSM::Adapter::IAL2::PPIF->new()->parse_file(one_subordinate_seq_ppif_path());
    my %byte_lane_seq_ppif_residue = map { $_->{id} => 1 } @{$byte_lane_seq_ppif->{report}{unsupported_residue}};
    ok($byte_lane_seq_ppif_residue{ahb_aggregate_profile_alias_deferred}, 'generic aggregate byte-lane SEQ PPIF keeps alias-deferred residue');
    ok(residue_id_occurs($byte_lane_seq_ppif->{report}, 'ahb_profile_alias_deferred'), 'generic aggregate byte-lane SEQ PPIF keeps requester profile-alias residue');
    ok(residue_id_occurs($byte_lane_seq_ppif->{report}, 'ahb_subordinate_profile_alias_deferred'), 'generic aggregate byte-lane SEQ PPIF keeps subordinate profile-alias residue');
    ok(detail_pattern_occurs($byte_lane_seq_ppif->{report}, qr/\.ahb alias exposure/), 'generic aggregate byte-lane SEQ PPIF keeps child alias-exposure residue wording');

    my $endpoint_seq_alias = FSM::Adapter::IAL2::PPIF->new()->parse_file(endpoint_seq_alias_path());
    is($endpoint_seq_alias->{report}{transfer}{seq_policy}{mode}, 'in_word_progressive', 'endpoint byte-lane SEQ .ahb keeps endpoint SEQ policy');
    ok(!detail_pattern_occurs($endpoint_seq_alias->{report}, qr/\.ahb alias exposure/), 'endpoint byte-lane SEQ .ahb still removes endpoint alias-exposure residue wording');
    ...
    ok(!exists $two_subordinate_alias->{report}{composition}{seq_policy_propagation}, 'word-only two-subordinate .ahb does not gain SEQ propagation report');
};
...
sub alias_cases {
    return (
        {
            label => 'one-subordinate aggregate byte-lane SEQ',
            alias_path => one_subordinate_seq_alias_path(),
            ppif_path => one_subordinate_seq_ppif_path(),
            ...
            source_id => 'fsmgen-ahb-interconnect-byte-lane-seq',
            ...
            hdl_child_artifacts => [qw(amba_requester.fsm ahb_interconnect.fsm ahb_lite_subordinate_byte_lane_seq.fsm)],
            byte_lane_objects => [qw(ahb_lite_subordinate_byte_lane_seq)],
            local_address_signals => [qw(HADDR_REGS)],
            byte_lane_fsm => 'ahb_lite_subordinate_byte_lane_seq.fsm',
        },
        {
            label => 'two-subordinate aggregate byte-lane SEQ',
            alias_path => two_subordinate_seq_alias_path(),
            ppif_path => two_subordinate_seq_ppif_path(),
            ...
            source_id => 'fsmgen-ahb-interconnect-two-subordinate-byte-lane-seq',
            ...
            hdl_child_artifacts => [qw(amba_requester.fsm ahb_interconnect.fsm ahb_status_subordinate_byte_lane_seq.fsm ahb_control_subordinate_byte_lane_seq.fsm)],
            byte_lane_objects => [qw(ahb_status_subordinate_byte_lane_seq ahb_control_subordinate_byte_lane_seq)],
            local_address_signals => [qw(HADDR_STATUS HADDR_CONTROL)],
            byte_lane_fsm => 'ahb_control_subordinate_byte_lane_seq.fsm',
        },
    );
}
...
sub assert_seq_policy_propagation {
    ...
    for my $subordinate (@{$propagation->{subordinates}}) {
        is_deeply($subordinate->{supported_size}, [qw(byte halfword word)], "$label report carries byte-lane supported sizes");
        is_deeply($subordinate->{supported_seq_size}, [qw(byte halfword)], "$label report carries supported SEQ sizes");
        is($subordinate->{seq_policy}{mode}, 'in_word_progressive', "$label report carries child SEQ policy");
    }
}
...
sub one_subordinate_alias_source {
    return slurp(one_subordinate_seq_alias_path());
}

sub two_subordinate_alias_source {
    return slurp(two_subordinate_seq_alias_path());
}
