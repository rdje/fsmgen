...
subtest 'adapter composes exact-four requester BUSY insertion with subordinate BUSY parking' => sub {
    ...
    like($source, qr/\(ahb-requester amba_requester_busy_insert_four\b/, 'sample selects the exact-four BUSY-inserting requester');
    like($source, qr/\(busy-before-beat 2\)/, 'sample inserts BUSY before beat index two');
    like($source, qr/\(busy-beats 4\)/, 'sample selects four qualified BUSY events');
    ...
    is_deeply(
        ...
        'exact-four paired source exposes requester, subordinate, fabric, and top IAL0 artifacts',
    );
    is_deeply(
        ...
        'exact-four paired source exposes the exact generated IAL1 artifacts',
    );
    ...
    is($requester->{object_name}, 'amba_requester_busy_insert_four', 'requester child selects the exact-four BUSY-inserting object');
...
subtest 'CLI, semantic, MCP, and review surfaces agree on the exact-four paired aggregate' => sub {
    my $check = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', sample_path());
    ok($check->{success}, 'strict check succeeds for the exact-four paired BUSY source');
    ...
    is($mcp->{source_id}, sample_relpath(), 'MCP keeps repo-relative exact-four paired identity');
    ...
    ok($generate_ok, 'exact-four paired BUSY source emits HDL and review artifacts')
    ...
    like($requester_isf, qr/\(var ahb_busy_remaining_q \(width 3\) \(reset 0\)\)/, 'exact-four paired requester keeps its actor-owned counter');
    ...
    ok($verify_ok, 'public verify-hdl accepts the exact-four paired BUSY source')
        or diag(join('', @{$verify_stdout || []}), join('', @{$verify_stderr || []}));
};

subtest 'generated HDL pairs four qualified requester BUSY events with subordinate parking' => sub {
    my $tempdir = project_tempdir();
    my $hdl = File::Spec->catfile($tempdir, 'ahb_tb.sv');
    ...
    ok($generate_ok, 'public exact-four paired BUSY source emits generated HDL')
        or diag(join('', @{$generate_stdout || []}), join('', @{$generate_stderr || []}));
    ...
    my ($compile_ok, undef, undef, $compile_stdout, $compile_stderr) = run(
        command => [
            ...
            '-j', '1', '--top-module', 'ahb_exact_four_paired_busy_composition_tb',
            '--Mdir', $objdir, $hdl, testbench_path(),
        ],
    );
    ok($compile_ok, 'Verilator builds the assertion-enabled exact-four paired BUSY composition harness')
        or diag(join('', @{$compile_stdout || []}), join('', @{$compile_stderr || []}));
    return unless $compile_ok;

    my $binary = File::Spec->catfile($objdir, 'Vahb_exact_four_paired_busy_composition_tb');
    my ($run_ok, undef, undef, $run_stdout, $run_stderr) = run(command => [$binary]);
    ok($run_ok, 'generated-HDL exact-four paired BUSY composition passes with assertions enabled')
        or diag(join('', @{$run_stdout || []}), join('', @{$run_stderr || []}));
    like(
        join('', @{$run_stdout || []}),
        qr/PASS transfers=5 beats=4 busy=1 qualified_busy=4 resumed_seq=1 storage=44332211/,
        'runtime observes four qualified paired BUSY events, one resumed SEQ, parking, and four byte writes',
    );
};
...
sub sample_path {
    return File::Spec->catfile(
        $repo_root,
        'ppif',
        'ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif',
    );
}
...
sub testbench_path {
    return File::Spec->catfile($FindBin::Bin, 'data', 'ahb_exact_four_paired_busy_composition_tb.svt');
}

sub support_id {
    return 'intent.ppif_ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ppif_ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park_pipeline_cli';
}

sub project_tempdir {
    my $temp_root = File::Spec->catdir($repo_root, '.artifacts', 'tmp', 'tests');
    make_path($temp_root);
    return tempdir('t1537-XXXXXX', DIR => $temp_root, CLEANUP => 1);
}
