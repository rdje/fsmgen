...
subtest 'manifest producer section contract entrypoint metadata rebuilds cleanly after caller mutation' => sub {
    ...
    is_deeply($contract->{'entrypoints'}, $expected->{'entrypoints'}, 'fresh manifest producer contract rebuilds clean entrypoints');
};
done_testing();
