...
subtest 'PPIF adapter parses AXI manager dynamic same-ID issue-order queue policy metadata' => sub {
    my $sample_path = sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic same-ID issue-order queue policy sample exists');

    my $base = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_transaction_id_ppif(), sample_capacity_dynamic_transaction_id_ppif_path());
    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif(), $sample_path);

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic same-ID issue-order queue sample still uses the capacity/status generator');
    ...
    assert_dynamic_transaction_id_report($result->{report}{transactions}, 'adapter report');
    assert_dynamic_same_id_issue_order_queue_policy_report($result->{report}{same_id_ordering}, 'adapter report');
    my %residue = map { $_->{id} => 1 } @{$result->{report}{unsupported_residue}};
    ok($residue{dynamic_transaction_id_behavior}, 'dynamic behavior boundary remains explicit unsupported residue');
};

subtest 'PPIF adapter parses AXI manager dynamic read response-demux same-ID reject mapping' => sub {
    my $sample_path = sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read response-demux same-ID reject sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(
        sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif(),
        $sample_path,
    );

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read response-demux same-ID reject sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-response-demux-multi-same-id-reject', 'dynamic read response-demux same-ID reject source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_response_demux_multi_same_id_reject', 'dynamic read response-demux same-ID reject source intent name is preserved');
    assert_dynamic_read_response_demux_multi_report(
        $result->{report},
        'adapter mapped dynamic read report',
    ...
    assert_dynamic_same_id_reject_policy_generated_report(
        ...
        'adapter mapped dynamic read report',
        family => 'read',
        response_demux_mode => 'bounded_multi_dynamic_read_rid_demux_contract',
...
subtest 'PPIF adapter parses AXI manager single-active dynamic read response-demux same-ID reject mapping' => sub {
    my $base = FSM::Adapter::IAL2::PPIF->new()->parse_source(
        sample_capacity_dynamic_read_response_demux_ppif(),
        sample_capacity_dynamic_read_response_demux_ppif_path(),
    );
    my $sample_path = sample_capacity_dynamic_read_response_demux_same_id_reject_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status single-active dynamic read response-demux same-ID reject sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(
        sample_capacity_dynamic_read_response_demux_same_id_reject_ppif(),
        $sample_path,
    );
    ...
    is(
        $result->{generated_ial1}{text},
        $base->{generated_ial1}{text},
        'single-active dynamic read same-ID reject PPIF mapping does not alter generated IAL1 text',
    );
    is_deeply(
        $result->{generated_ial0}{files},
        $base->{generated_ial0}{files},
        'single-active dynamic read same-ID reject PPIF mapping does not alter generated IAL0 files',
    );
    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'single-active dynamic read response-demux same-ID reject sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-response-demux-same-id-reject', 'single-active dynamic read response-demux same-ID reject source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_response_demux_same_id_reject', 'single-active dynamic read response-demux same-ID reject source intent name is preserved');
    assert_dynamic_read_response_demux_report(
        $result->{report},
        'adapter mapped single-active dynamic read report',
        residue => [qw(read_data_interleaving bursts)],
    );
...
subtest 'PPIF adapter parses AXI manager mixed dynamic/static write same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_write_mixed_dynamic_static_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status mixed dynamic/static write same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_write_mixed_dynamic_static_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static write same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-write-mixed-dynamic-static-same-id-issue-order-queue', 'mixed dynamic/static write same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_write_mixed_dynamic_static_same_id_issue_order_queue', 'mixed dynamic/static write same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_awid \(width 4\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 declares AWID');
    like($isf, qr/\(input axi0_bid \(width 4\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 declares BID');
    like($isf, qr/\(var axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 allocates slot0 ID');
    like($isf, qr/\(var axi0_write_mixed_dynamic_static_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 allocates slot1 ID');
    like($isf, qr/\(rule axi0_write_mixed_dynamic_static_same_id_issue_order_empty_enqueue_w0[\s\S]*\(axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q axi0_awid\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 captures AWID into slot0');
    like($isf, qr/\(rule axi0_write_mixed_dynamic_static_same_id_issue_order_empty_enqueue_w1[\s\S]*\(axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd3\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 captures static literal into slot0');
    like($isf, qr/\(rule axi0_w1_response_demux [\s\S]*axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*\(pulse axi0_w1_complete\)\)/, 'mixed dynamic/static write same-ID issue-order queue generated IAL1 emits static BID demux');
...
subtest 'PPIF adapter parses AXI manager mixed dynamic/static read same-ID issue-order queue behavior' => sub {
    ...
    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static read same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-read-mixed-dynamic-static-same-id-issue-order-queue', 'mixed dynamic/static read same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue', 'mixed dynamic/static read same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_arid \(width 4\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 declares ARID');
    like($isf, qr/\(input axi0_rid \(width 4\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 declares RID');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 allocates slot0 ID');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 allocates slot1 ID');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r0[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q axi0_arid\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 captures ARID into slot0');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r1[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd3\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 captures static literal into slot0');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*\(pulse axi0_r1_complete\)\)/, 'mixed dynamic/static read same-ID issue-order queue generated IAL1 emits static RID demux');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager mixed dynamic/static read burst-last same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status mixed dynamic/static read burst-last same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static read burst-last same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-read-mixed-dynamic-static-burst-last-same-id-issue-order-queue', 'mixed dynamic/static read burst-last same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue', 'mixed dynamic/static read burst-last same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_arid \(width 4\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 declares ARID');
    like($isf, qr/\(input axi0_rid \(width 4\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 declares RID');
    like($isf, qr/\(input axi0_rlast\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 declares RLAST');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 allocates slot0 ID');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 allocates slot1 ID');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r0[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q axi0_arid\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 captures ARID into slot0');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r1[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd3\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 captures static literal into slot0');
    like($isf, qr/\(rule axi0_r0_response_demux [\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r0_complete\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 emits dynamic RID/RLAST demux');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r1_complete\)\)/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 emits static RID/RLAST demux');
    like($isf, qr/read dynamic same-ID non-last response beat does not dequeue/, 'mixed dynamic/static read burst-last same-ID issue-order queue generated IAL1 emits non-last no-dequeue assertion');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($result->{report}, 'adapter report', burst_last => 1);
};

subtest 'PPIF adapter parses AXI manager mixed dynamic/static read same-ID issue-order queue read-data behavior' => sub {
    my $sample_path = sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status mixed dynamic/static read same-ID issue-order queue read-data sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static read same-ID issue-order queue read-data sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-read-mixed-dynamic-static-same-id-issue-order-queue-read-data', 'mixed dynamic/static read same-ID issue-order queue read-data source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue_read_data', 'mixed dynamic/static read same-ID issue-order queue read-data source intent name is preserved');
    ...
    like($isf, qr/\(output axi0_r0_rdata \(width 32\)\)/, 'mixed dynamic/static read issue-order queue read-data declares dynamic scalar data output');
    like($isf, qr/\(output axi0_r1_rdata \(width 32\)\)/, 'mixed dynamic/static read issue-order queue read-data declares static scalar data output');
    ...
    like($fsm, qr/\(-axi0_r1_read_data_capture\s+<axi0_r1_complete\s+\(<- \(axi0_r1_rdata> axi0_rdata\)\)\s+\(<- \(axi0_r1_rresp> axi0_rresp\)\)/, 'mixed dynamic/static read issue-order queue read-data lowers static capture rule into generated .fsm');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_report(
        $result->{report}{read_data},
        'adapter mixed dynamic/static read same-ID issue-order queue read-data report',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_completion_pulse',
        transactions => [qw(r0 r1)],
    );
};

subtest 'PPIF adapter parses AXI manager mixed dynamic/static read burst-last same-ID issue-order queue read-data behavior' => sub {
    my $sample_path = sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-read-mixed-dynamic-static-burst-last-same-id-issue-order-queue-read-data', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data source intent name is preserved');
    ...
    like($isf, qr/\(output axi0_r0_last_rdata \(width 32\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data declares dynamic scalar last data output');
    like($isf, qr/\(output axi0_r1_last_rdata \(width 32\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data declares static scalar last data output');
    ...
    like($isf, qr/\(rule axi0_r1_read_data_capture axi0_r1_complete\s+\(axi0_r1_last_rdata axi0_rdata\)\s+\(axi0_r1_last_rresp axi0_rresp\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data captures static payload under queue last-beat completion');
    ...
    like($fsm, qr/\(-axi0_r1_read_data_capture\s+<axi0_r1_complete\s+\(<- \(axi0_r1_last_rdata> axi0_rdata\)\)\s+\(<- \(axi0_r1_last_rresp> axi0_rresp\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data lowers static last-beat capture rule into generated .fsm');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($result->{report}, 'adapter report', burst_last => 1);
    assert_read_data_last_beat_report(
        $result->{report}{read_data},
        'adapter mixed dynamic/static read burst-last same-ID issue-order queue read-data report',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
};

subtest 'PPIF adapter parses AXI manager mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat behavior' => sub {
    my $sample_path = sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-read-mixed-dynamic-static-burst-last-same-id-issue-order-queue-read-data-multi-beat', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat source intent name is preserved');
    ...
    assert_read_data_multi_beat_report(
        $result->{report}{read_data},
        'adapter mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat report',
        completion_validity => 'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
};

subtest 'PPIF adapter parses AXI manager dynamic write depth-3 same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_dynamic_write_depth3_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic write depth-3 same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_write_depth3_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic write depth-3 same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-write-depth3-same-id-issue-order-queue', 'dynamic write depth-3 same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_write_depth3_same_id_issue_order_queue', 'dynamic write depth-3 same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_w2_request\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 declares w2 request');
    like($isf, qr/\(output axi0_w2_complete\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 exposes w2 completion');
    like($isf, qr/\(var axi0_write_dynamic_same_id_issue_order_slot2_id_q \(width 4\)\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 allocates slot2 captured ID');
    like($isf, qr/\(rule axi0_write_dynamic_same_id_issue_order_w2_w1_w0_dequeue_enqueue_w0[\s\S]*\(axi0_write_dynamic_same_id_issue_order_slot2_id_q axi0_awid\)\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 refreshes tail-selected captured ID');
    like($isf, qr/\(rule axi0_write_dynamic_same_id_issue_order_w0_w1_dequeue_w0_enqueue_w2[\s\S]*\(axi0_write_dynamic_same_id_issue_order_slot0_id_q axi0_write_dynamic_same_id_issue_order_slot1_id_q\)[\s\S]*\(axi0_write_dynamic_same_id_issue_order_slot1_id_q axi0_awid\)\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 disambiguates cross-transaction enqueue');
    like($isf, qr/\(rule axi0_w2_response_demux [\s\S]*axi0_write_dynamic_same_id_issue_order_slot2_id_q[\s\S]*\(pulse axi0_w2_complete\)\)/, 'dynamic write depth-3 same-ID issue-order queue generated IAL1 emits w2 BID demux');
    assert_dynamic_write_depth3_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager dynamic read same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-same-id-issue-order-queue', 'dynamic read same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue', 'dynamic read same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_arid \(width 4\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 declares ARID');
    like($isf, qr/\(input axi0_rid \(width 4\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 declares RID');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 allocates slot0 captured ID');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 allocates slot1 captured ID');
    like($isf, qr/\(rule axi0_read_dynamic_same_id_issue_order_empty_enqueue_r0[\s\S]*\(axi0_read_dynamic_same_id_issue_order_slot0_id_q axi0_arid\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 captures ARID into slot0');
    like($isf, qr/\(rule axi0_r0_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*\(pulse axi0_r0_complete\)\)/, 'dynamic read same-ID issue-order queue generated IAL1 emits r0 RID demux');
    like($isf, qr/read dynamic same-ID response matches at least one captured runtime ID/, 'dynamic read same-ID issue-order queue generated IAL1 emits response selected-match assertion');
    assert_dynamic_read_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager dynamic read depth-3 same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read depth-3 same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read depth-3 same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-depth3-same-id-issue-order-queue', 'dynamic read depth-3 same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_depth3_same_id_issue_order_queue', 'dynamic read depth-3 same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_r2_request\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 declares r2 request');
    like($isf, qr/\(output axi0_r2_complete\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 exposes r2 completion');
    unlike($isf, qr/axi0_r2_dynamic_id_q/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 avoids legacy r2 selected-ID storage');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot2_id_q \(width 4\)\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 allocates slot2 captured ID');
    like($isf, qr/\(rule axi0_read_dynamic_same_id_issue_order_r2_r1_r0_dequeue_enqueue_r0[\s\S]*\(axi0_read_dynamic_same_id_issue_order_slot2_id_q axi0_arid\)\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 refreshes tail-selected captured ID');
    like($isf, qr/\(rule axi0_read_dynamic_same_id_issue_order_r0_r1_dequeue_r0_enqueue_r2[\s\S]*\(axi0_read_dynamic_same_id_issue_order_slot0_id_q axi0_read_dynamic_same_id_issue_order_slot1_id_q\)[\s\S]*\(axi0_read_dynamic_same_id_issue_order_slot1_id_q axi0_arid\)\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 disambiguates cross-transaction enqueue');
    like($isf, qr/\(rule axi0_r2_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_id_q[\s\S]*\(pulse axi0_r2_complete\)\)/, 'dynamic read depth-3 same-ID issue-order queue generated IAL1 emits r2 RID demux');
    assert_dynamic_read_depth3_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager dynamic read burst-last same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-same-id-issue-order-queue', 'dynamic read burst-last same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue', 'dynamic read burst-last same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_arid \(width 4\)\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 declares ARID');
    like($isf, qr/\(input axi0_rid \(width 4\)\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 declares RID');
    like($isf, qr/\(input axi0_rlast\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 declares RLAST');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 allocates slot0 captured ID');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 allocates slot1 captured ID');
    like($isf, qr/\(rule axi0_r0_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r0_complete\)\)/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 emits r0 RID/RLAST demux');
    like($isf, qr/read dynamic same-ID non-last response beat does not dequeue/, 'dynamic read burst-last same-ID issue-order queue generated IAL1 emits non-last no-dequeue assertion');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager dynamic read burst-last depth-3 same-ID issue-order queue behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last depth-3 same-ID issue-order queue sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last depth-3 same-ID issue-order queue sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue', 'dynamic read burst-last depth-3 same-ID issue-order queue source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue', 'dynamic read burst-last depth-3 same-ID issue-order queue source intent name is preserved');
    like($isf, qr/\(input axi0_r2_request\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 declares r2 request');
    like($isf, qr/\(input axi0_rlast\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 declares RLAST');
    like($isf, qr/\(output axi0_r2_complete\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 exposes r2 completion');
    unlike($isf, qr/axi0_r2_dynamic_id_q/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 avoids legacy r2 selected-ID storage');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot2_id_q \(width 4\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 allocates slot2 captured ID');
    like($isf, qr/\(rule axi0_r2_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r2_complete\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 emits r2 RID/RLAST demux');
    like($isf, qr/read dynamic same-ID non-last response beat does not dequeue/, 'dynamic read burst-last depth-3 same-ID issue-order queue generated IAL1 emits non-last no-dequeue assertion');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($result->{report}, 'adapter report');
};

subtest 'PPIF adapter parses AXI manager dynamic read same-ID issue-order queue read-data behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read same-ID issue-order queue read-data sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read same-ID issue-order queue read-data sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-same-id-issue-order-queue-read-data', 'dynamic read same-ID issue-order queue read-data source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue_read_data', 'dynamic read same-ID issue-order queue read-data source intent name is preserved');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_id_q[\s\S]*\(pulse axi0_r1_complete\)\)/, 'dynamic read same-ID issue-order queue read-data keeps queue-owned r1 RID demux');
    like($isf, qr/\(input axi0_rdata \(width 32\)\)/, 'dynamic read same-ID issue-order queue read-data declares RDATA input');
    like($isf, qr/\(input axi0_rresp \(width 2\)\)/, 'dynamic read same-ID issue-order queue read-data declares RRESP input');
    like($isf, qr/\(output axi0_r0_rdata \(width 32\)\)/, 'dynamic read same-ID issue-order queue read-data declares r0 scalar data output');
    like($isf, qr/\(output axi0_r1_rdata \(width 32\)\)/, 'dynamic read same-ID issue-order queue read-data declares r1 scalar data output');
    like($isf, qr/\(rule axi0_r1_read_data_capture axi0_r1_complete\s+\(axi0_r1_rdata axi0_rdata\)\s+\(axi0_r1_rresp axi0_rresp\)\)/, 'dynamic read same-ID issue-order queue read-data captures r1 payload under queue completion');
    unlike($isf, qr/\baxi0_rlast\b/, 'dynamic read same-ID issue-order queue read-data keeps RLAST absent');
    unlike($isf, qr/\baxi0_arlen\b/, 'dynamic read same-ID issue-order queue read-data keeps burst-length metadata absent');
    like($fsm, qr/\(-axi0_r1_read_data_capture\s+<axi0_r1_complete\s+\(<- \(axi0_r1_rdata> axi0_rdata\)\)\s+\(<- \(axi0_r1_rresp> axi0_rresp\)\)/, 'dynamic read same-ID issue-order queue read-data lowers r1 capture rule into generated .fsm');
    assert_dynamic_read_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_report(
        $result->{report}{read_data},
        'adapter dynamic read same-ID issue-order queue read-data report',
        'generated_dynamic_read_issue_order_queue_response_demux_completion_pulse',
        transactions => [qw(r0 r1)],
    );
};

subtest 'PPIF adapter parses AXI manager dynamic read burst-last same-ID issue-order queue read-data behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last same-ID issue-order queue read-data sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last same-ID issue-order queue read-data sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-same-id-issue-order-queue-read-data', 'dynamic read burst-last same-ID issue-order queue read-data source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data', 'dynamic read burst-last same-ID issue-order queue read-data source intent name is preserved');
    like($isf, qr/\(input axi0_rlast\)/, 'dynamic read burst-last same-ID issue-order queue read-data declares RLAST input');
    like($isf, qr/\(input axi0_rdata \(width 32\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data declares RDATA input');
    like($isf, qr/\(input axi0_rresp \(width 2\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data declares RRESP input');
    like($isf, qr/\(output axi0_r0_last_rdata \(width 32\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data declares r0 scalar last data output');
    like($isf, qr/\(output axi0_r1_last_rdata \(width 32\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data declares r1 scalar last data output');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r1_complete\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data keeps queue-owned r1 RID/RLAST demux');
    like($isf, qr/\(rule axi0_r1_read_data_capture axi0_r1_complete\s+\(axi0_r1_last_rdata axi0_rdata\)\s+\(axi0_r1_last_rresp axi0_rresp\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data captures r1 payload under queue last-beat completion');
    unlike($isf, qr/\baxi0_arlen\b/, 'dynamic read burst-last same-ID issue-order queue read-data keeps burst-length metadata absent');
    like($fsm, qr/\(-axi0_r1_read_data_capture\s+<axi0_r1_complete\s+\(<- \(axi0_r1_last_rdata> axi0_rdata\)\)\s+\(<- \(axi0_r1_last_rresp> axi0_rresp\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data lowers r1 capture rule into generated .fsm');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_last_beat_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last same-ID issue-order queue read-data report',
...
subtest 'PPIF adapter parses AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue-read-data-burst-length', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length source intent name is preserved');
    like($isf, qr/\(input axi0_r2_request\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares r2 request input');
    like($isf, qr/\(input axi0_rlast\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares RLAST input');
    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares ARLEN input');
    like($isf, qr/\(var axi0_r0_arlen_q \(width 8\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares r0 ARLEN storage');
    like($isf, qr/\(var axi0_r1_arlen_q \(width 8\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares r1 ARLEN storage');
    like($isf, qr/\(var axi0_r2_arlen_q \(width 8\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length declares r2 ARLEN storage');
    like($isf, qr/\(rule axi0_r2_burst_length_capture axi0_r2_request\s+\(axi0_r2_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length captures r2 ARLEN under request');
    like($isf, qr/\(rule axi0_r2_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r2_complete\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length keeps queue-owned r2 RID/RLAST demux');
    like($isf, qr/\(rule axi0_r2_read_data_capture axi0_r2_complete\s+\(axi0_r2_last_rdata axi0_rdata\)\s+\(axi0_r2_last_rresp axi0_rresp\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length captures r2 payload under queue last-beat completion');
    unlike($isf, qr/expected_beats|read_beat_count/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length keeps runtime beat-count state absent');
    like($fsm, qr/axi0_r2_burst_length_capture[\s\S]*\(<- \(axi0_r2_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length lowers r2 ARLEN capture rule into generated .fsm');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_burst_length_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length report',
        'report_only',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );
...
subtest 'PPIF adapter parses AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length validation behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue-read-data-burst-length-runtime-assertion', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length source intent name is preserved');
    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length declares ARLEN input');
    like($isf, qr/\(var axi0_r0_expected_beats_q \(width 5\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length declares r0 expected-beat storage');
    like($isf, qr/\(var axi0_r1_expected_beats_q \(width 5\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length declares r1 expected-beat storage');
    like($isf, qr/\(var axi0_r2_expected_beats_q \(width 5\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length declares r2 expected-beat storage');
    like($isf, qr/\(var axi0_r2_read_beat_count_q \(width 5\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length declares r2 read-beat counter');
    like($isf, qr/\(rule axi0_r2_burst_length_capture axi0_r2_request\s+\(axi0_r2_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length captures r2 ARLEN under request');
    like($isf, qr/\(rule axi0_r2_beat_count_init axi0_r2_request\s+\(axi0_r2_expected_beats_q \(\+ axi0_arlen\[4:0\] 5'd1\)\)\s+\(axi0_r2_read_beat_count_q 0\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length initializes r2 beat-count state');
    like($isf, qr/\(rule axi0_r2_read_beat_count \(& \(& axi0_read_complete [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_r2_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_r2_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_r2_q[\s\S]*\) \(! axi0_r2_request\)\)\s+\(axi0_r2_read_beat_count_q \(\+ axi0_r2_read_beat_count_q 5'd1\)\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length increments r2 on matched raw read beats');
    like($isf, qr/"axi0 r2 RLAST appears only on the expected final read beat"/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length emits r2 RLAST final-beat assertion');
    like($isf, qr/"axi0 r2 expected final read beat has RLAST"/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length emits r2 expected-final RLAST assertion');
    like($fsm, qr/axi0_r2_beat_count_init[\s\S]*\(<- \(axi0_r2_expected_beats_q \(\+ axi0_arlen\[4:0\] 5'd1\)\)\)[\s\S]*\(<- \(axi0_r2_read_beat_count_q 0\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length lowers r2 beat-count init into generated .fsm');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_burst_length_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );
...
subtest 'PPIF adapter parses AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue-read-data-multi-beat', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat source intent name is preserved');
    like($isf, qr/\(output axi0_r2_beat_rdata_15 \(width 32\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat exposes r2 final RDATA lane');
    like($isf, qr/\(output axi0_r2_beat_rresp_15 \(width 2\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat exposes r2 final RRESP lane');
    like($isf, qr/\(output axi0_r2_beat_valid \(width 16\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat exposes r2 valid mask');
    like($isf, qr/\(output axi0_r2_read_beats \(width 5\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat exposes r2 length output');
    like($isf, qr/\(output axi0_r2_rresp \(width 2\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat exposes r2 scalar RRESP aggregate');
    like($isf, qr/\(rule axi0_r2_read_beat_15_capture [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_r2_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_r2_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_r2_q[\s\S]*\(axi0_r2_beat_rdata_15 axi0_rdata\)[\s\S]*\(axi0_r2_beat_rresp_15 axi0_rresp\)[\s\S]*\(axi0_r2_beat_valid 16'b1111111111111111\)[\s\S]*\(axi0_r2_read_beats 5'd16\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat captures r2 final lane from matched raw read beats');
    like($fsm, qr/axi0_r2_read_beat_15_capture[\s\S]*\(<- \(axi0_r2_beat_rdata_15> axi0_rdata\)\)[\s\S]*\(<- \(axi0_r2_beat_rresp_15> axi0_rresp\)\)/, 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat lowers r2 final lane capture into generated .fsm');
    ...
    assert_read_data_multi_beat_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat report',
...
subtest 'PPIF adapter parses AXI manager dynamic read burst-last same-ID issue-order queue read-data burst-length behavior' => sub {
    ...
    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last same-ID issue-order queue read-data burst-length sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-same-id-issue-order-queue-read-data-burst-length', 'dynamic read burst-last same-ID issue-order queue read-data burst-length source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length', 'dynamic read burst-last same-ID issue-order queue read-data burst-length source intent name is preserved');
    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length declares ARLEN input');
    like($isf, qr/\(var axi0_r0_arlen_q \(width 8\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length declares r0 ARLEN storage');
    like($isf, qr/\(var axi0_r1_arlen_q \(width 8\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length declares r1 ARLEN storage');
    like($isf, qr/\(rule axi0_r1_burst_length_capture axi0_r1_request\s+\(axi0_r1_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length captures r1 ARLEN under request');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r1_complete\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length keeps queue-owned r1 RID/RLAST demux');
    like($isf, qr/\(rule axi0_r1_read_data_capture axi0_r1_complete\s+\(axi0_r1_last_rdata axi0_rdata\)\s+\(axi0_r1_last_rresp axi0_rresp\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length captures r1 payload under queue last-beat completion');
    like($fsm, qr/axi0_r1_burst_length_capture[\s\S]*\(<- \(axi0_r1_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data burst-length lowers r1 ARLEN capture rule into generated .fsm');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_burst_length_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last same-ID issue-order queue read-data burst-length report',
        'report_only',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
...
subtest 'PPIF adapter parses AXI manager dynamic read burst-last same-ID issue-order queue read-data runtime burst-length validation behavior' => sub {
    my $sample_path = sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path();
    ok(-f $sample_path, 'tracked runnable PPIF capacity/status dynamic read burst-last same-ID issue-order queue read-data runtime burst-length sample exists');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_source(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif(), $sample_path);
    my $isf = $result->{generated_ial1}{text};
    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};

    is($result->{kind}, 'protocol_intent.axi_manager_capacity_status', 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length sample still uses the capacity/status generator');
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-read-burst-last-same-id-issue-order-queue-read-data-burst-length-runtime-assertion', 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion', 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length source intent name is preserved');
    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length declares ARLEN input');
    like($isf, qr/\(var axi0_r0_expected_beats_q \(width 5\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length declares r0 expected-beat storage');
    like($isf, qr/\(var axi0_r1_expected_beats_q \(width 5\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length declares r1 expected-beat storage');
    like($isf, qr/\(var axi0_r1_read_beat_count_q \(width 5\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length declares r1 read-beat counter');
    like($isf, qr/\(rule axi0_r1_burst_length_capture axi0_r1_request\s+\(axi0_r1_arlen_q axi0_arlen\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length captures r1 ARLEN under request');
    like($isf, qr/\(rule axi0_r1_beat_count_init axi0_r1_request\s+\(axi0_r1_expected_beats_q \(\+ axi0_arlen\[4:0\] 5'd1\)\)\s+\(axi0_r1_read_beat_count_q 0\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length initializes r1 beat-count state');
    like($isf, qr/\(rule axi0_r1_read_beat_count \(& \(& axi0_read_complete [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_r1_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_r1_q[\s\S]*\) \(! axi0_r1_request\)\)\s+\(axi0_r1_read_beat_count_q \(\+ axi0_r1_read_beat_count_q 5'd1\)\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length increments r1 on matched raw read beats');
    like($isf, qr/"axi0 r1 RLAST appears only on the expected final read beat"/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length emits r1 RLAST final-beat assertion');
    like($isf, qr/"axi0 r1 expected final read beat has RLAST"/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length emits r1 expected-final RLAST assertion');
    like($fsm, qr/axi0_r1_beat_count_init[\s\S]*\(<- \(axi0_r1_expected_beats_q \(\+ axi0_arlen\[4:0\] 5'd1\)\)\)[\s\S]*\(<- \(axi0_r1_read_beat_count_q 0\)\)/, 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length lowers r1 beat-count init into generated .fsm');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($result->{report}, 'adapter report');
    assert_read_data_burst_length_report(
        $result->{report}{read_data},
        'adapter dynamic read burst-last same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
...
subtest 'PPIF adapter parses AXI manager multiple dynamic read burst-last response-demux behavior' => sub {
    ...
    like($isf, qr/\(rule axi0_r0_dynamic_id_release_recapture \(& \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r0_complete axi0_r0_dynamic_busy_q \(! \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\) \(! \(& axi0_r1_dynamic_busy_q \(== axi0_r1_dynamic_id_q axi0_arid\)\)\)\)/, 'multiple dynamic read burst-last response-demux generated IAL1 recaptures r0 on same-cycle final completion');
    like($isf, qr/\(rule axi0_r1_dynamic_id_release_recapture \(& \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r1_complete axi0_r1_dynamic_busy_q \(! \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\) \(! \(& axi0_r0_dynamic_busy_q \(== axi0_r0_dynamic_id_q axi0_arid\)\)\)\)/, 'multiple dynamic read burst-last response-demux generated IAL1 recaptures r1 on same-cycle final completion');
    like($isf, qr/\(rule axi0_r0_dynamic_id_release \(& axi0_r0_complete axi0_r0_dynamic_busy_q \(! axi0_r0_request\)\)/, 'multiple dynamic read burst-last response-demux release-only r0 excludes same-cycle r0 request');
    like($isf, qr/\(rule axi0_r1_dynamic_id_release \(& axi0_r1_complete axi0_r1_dynamic_busy_q \(! axi0_r1_request\)\)/, 'multiple dynamic read burst-last response-demux release-only r1 excludes same-cycle r1 request');
...
subtest 'PPIF adapter diagnostics fail closed before generation claims' => sub {
    my @cases = (
        ...
        ['manager dynamic same-ID issue-order queue policy requires transactions metadata',
            capacity_ppif_with_objects(manager_capacity_object_with_id_families_and_same_id_ordering(
                '(write (width 4) (request-id awid) (response-id bid)) (read (width 4) (request-id arid) (response-id rid))',
        ...
        ['manager dynamic same-ID issue-order queue policy requires same-family dynamic transaction',
            capacity_ppif_with_objects(manager_capacity_object_with_id_families_transactions_and_same_id_ordering(
                '(write (width 4) (request-id awid) (response-id bid)) (read (width 4) (request-id arid) (response-id rid))',
                default_manager_transactions(),
        ...
        ['manager dynamic transaction ID rejects concrete-only same-ID policy assumptions',
            ...
            qr/same_id_ordering_policy\.read concrete-id-reuse policy does not cover dynamic read transaction ID metadata; select dynamic-id-reuse reject or dynamic-id-reuse issue-order-queue for transaction\(s\): r0/],
...
subtest 'CLI emits IAL2 report JSON for AXI manager dynamic same-ID issue-order queue policy .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic same-ID issue-order queue policy .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic same-ID issue-order queue policy report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_same_id_issue_order_queue_policy', 'dynamic same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_transaction_id_report($report->{transactions}, 'CLI report');
    assert_dynamic_same_id_issue_order_queue_policy_report($report->{same_id_ordering}, 'CLI report');
    ok(!exists $report->{id_response_rule_engine}, 'CLI dynamic same-ID issue-order queue metadata report does not emit a concrete-ID assertion engine');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic same-ID issue-order queue metadata keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read response-demux same-ID reject .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read response-demux same-ID reject .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read response-demux same-ID reject report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_response_demux_multi_same_id_reject', 'dynamic read response-demux same-ID reject report carries the PPIF top-level intent name');
    assert_dynamic_read_response_demux_multi_report(
        $report,
        'CLI mapped dynamic read report',
        residue => [qw(read_data_interleaving bursts)],
    );
    assert_dynamic_same_id_reject_policy_generated_report(
    ...
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read response-demux same-ID reject keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager single-active dynamic read response-demux same-ID reject .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_response_demux_same_id_reject_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status single-active dynamic read response-demux same-ID reject .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status single-active dynamic read response-demux same-ID reject report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_response_demux_same_id_reject', 'single-active dynamic read response-demux same-ID reject report carries the PPIF top-level intent name');
    assert_dynamic_read_response_demux_report(
        $report,
        'CLI mapped single-active dynamic read report',
        residue => [qw(read_data_interleaving bursts)],
    );
    assert_dynamic_same_id_reject_policy_single_active_generated_report(
        $report->{same_id_ordering},
...
subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static write same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_write_mixed_dynamic_static_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static write same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static write same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_write_mixed_dynamic_static_same_id_issue_order_queue', 'mixed dynamic/static write same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_write_same_id_issue_order_queue_report($report, 'CLI report');
...
subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read same-ID issue-order queue .ppif' => sub {
    ...
    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue', 'mixed dynamic/static read same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'mixed dynamic/static read same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read burst-last same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read burst-last same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue', 'mixed dynamic/static read burst-last same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($report, 'CLI report', burst_last => 1);
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'mixed dynamic/static read burst-last same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read same-ID issue-order queue read-data .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read same-ID issue-order queue read-data .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read same-ID issue-order queue read-data report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue_read_data', 'mixed dynamic/static read same-ID issue-order queue read-data report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_report(
        $report->{read_data},
        'CLI mixed dynamic/static read same-ID issue-order queue read-data report',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'mixed dynamic/static read same-ID issue-order queue read-data keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read burst-last same-ID issue-order queue read-data .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($report, 'CLI report', burst_last => 1);
    assert_read_data_last_beat_report(
        $report->{read_data},
        'CLI mixed dynamic/static read burst-last same-ID issue-order queue read-data report',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
...
subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($report, 'CLI report', burst_last => 1);
    assert_read_data_burst_length_report(
        $report->{read_data},
        'CLI mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat', 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat report carries the PPIF top-level intent name');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report(
        $report,
        'CLI report',
        burst_last => 1,
    ...
    assert_read_data_multi_beat_report(
        $report->{read_data},
        'CLI mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat report',
        completion_validity => 'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat keeps the generated .fsm artifact name stable');
};
...
subtest 'CLI emits IAL2 report JSON for AXI manager dynamic write depth-3 same-ID issue-order queue .ppif' => sub {
    ...
    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic write depth-3 same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic write depth-3 same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_write_depth3_same_id_issue_order_queue', 'dynamic write depth-3 same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_write_depth3_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic write depth-3 same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue', 'dynamic read same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_read_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read depth-3 same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read depth-3 same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read depth-3 same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_depth3_same_id_issue_order_queue', 'dynamic read depth-3 same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_read_depth3_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read depth-3 same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue', 'dynamic read burst-last same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last depth-3 same-ID issue-order queue .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last depth-3 same-ID issue-order queue .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue', 'dynamic read burst-last depth-3 same-ID issue-order queue report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($report, 'CLI report');
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last depth-3 same-ID issue-order queue keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read same-ID issue-order queue read-data .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read same-ID issue-order queue read-data .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read same-ID issue-order queue read-data report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue_read_data', 'dynamic read same-ID issue-order queue read-data report carries the PPIF top-level intent name');
    assert_dynamic_read_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_report(
        $report->{read_data},
        'CLI dynamic read same-ID issue-order queue read-data report',
        'generated_dynamic_read_issue_order_queue_response_demux_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read same-ID issue-order queue read-data keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last same-ID issue-order queue read-data .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last same-ID issue-order queue read-data .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last same-ID issue-order queue read-data report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data', 'dynamic read burst-last same-ID issue-order queue read-data report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_last_beat_report(
        $report->{read_data},
        'CLI dynamic read burst-last same-ID issue-order queue read-data report',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
...
subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_burst_length_report(
        $report->{read_data},
        'CLI dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length report',
        'report_only',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_burst_length_report(
        $report->{read_data},
        'CLI dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat', 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report(
        $report,
        'CLI report',
        response_demux_residue => [],
    ...
    assert_read_data_multi_beat_report(
        $report->{read_data},
        'CLI dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat report',
        completion_validity => 'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat keeps the generated .fsm artifact name stable');
};
...
subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last same-ID issue-order queue read-data burst-length .ppif' => sub {
    ...
    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last same-ID issue-order queue read-data burst-length .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last same-ID issue-order queue read-data burst-length report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length', 'dynamic read burst-last same-ID issue-order queue read-data burst-length report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_burst_length_report(
        $report->{read_data},
        'CLI dynamic read burst-last same-ID issue-order queue read-data burst-length report',
        'report_only',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    is_deeply($report->{generated_artifacts}{ial0}{files}, ['axi0_capacity_status.fsm'], 'dynamic read burst-last same-ID issue-order queue read-data burst-length keeps the generated .fsm artifact name stable');
};

subtest 'CLI emits IAL2 report JSON for AXI manager dynamic read burst-last same-ID issue-order queue read-data runtime burst-length .ppif' => sub {
    my ($success, undef, undef, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--emit-schedule-json', sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path()],
    );

    ok($success, '--emit-schedule-json succeeds for capacity/status dynamic read burst-last same-ID issue-order queue read-data runtime burst-length .ppif');
    is(join('', @{$stderr_buf || []}), '', 'capacity/status dynamic read burst-last same-ID issue-order queue read-data runtime burst-length report keeps stderr clean');
    my $report = decode_json(join('', @{$stdout_buf || []}));
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_manager_capacity_status.v1', 'CLI keeps the capacity/status report schema');
    is($report->{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion', 'dynamic read burst-last same-ID issue-order queue read-data runtime burst-length report carries the PPIF top-level intent name');
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($report, 'CLI report');
    assert_read_data_burst_length_report(
        $report->{read_data},
        'CLI dynamic read burst-last same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
...
subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static write same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static write same-ID issue-order queue',
        path     => \&sample_capacity_write_mixed_dynamic_static_same_id_issue_order_queue_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        ...
        path     => \&sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read burst-last same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue',
        path     => \&sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read same-ID issue-order queue read-data .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static read same-ID issue-order queue read-data',
        path     => \&sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue_read_data',
    );
};

subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read burst-last same-ID issue-order queue read-data .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data',
        path     => \&sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data',
    );
};
...
subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length',
        path     => \&sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion',
    );
};

subtest 'CLI check JSON and semantic JSON support-account mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat',
        path     => \&sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat',
    );
};
...
subtest 'CLI check JSON and semantic JSON support-account dynamic write depth-3 same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        ...
        path     => \&sample_capacity_dynamic_write_depth3_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_write_depth3_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read same-ID issue-order queue',
        path     => \&sample_capacity_dynamic_read_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read depth-3 same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read depth-3 same-ID issue-order queue',
        path     => \&sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_read_depth3_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last same-ID issue-order queue',
        path     => \&sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last depth-3 same-ID issue-order queue .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue',
        path     => \&sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read same-ID issue-order queue read-data .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read same-ID issue-order queue read-data',
        path     => \&sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif_path,
        entry_id => 'intent.ppif_axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue_read_data',
    );
};

subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last same-ID issue-order queue read-data .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last same-ID issue-order queue read-data',
        path     => \&sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data burst-length',
        path     => \&sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data runtime burst-length',
        path     => \&sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat',
        path     => \&sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last same-ID issue-order queue read-data burst-length .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        ...
        path     => \&sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_ppif_path,
...
subtest 'CLI check JSON and semantic JSON support-account dynamic read burst-last same-ID issue-order queue read-data runtime burst-length .ppif separately' => sub {
    assert_ppif_strict_json_support_case(
        owner    => 'capacity/status dynamic read burst-last same-ID issue-order queue read-data runtime burst-length',
        path     => \&sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path,
...
sub sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_same_id_issue_order_queue_policy.ppif');
}
...
sub sample_capacity_write_mixed_dynamic_static_same_id_issue_order_queue_ppif_path {
...
sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue.ppif');
}

sub sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue_read_data.ppif');
}

sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_ppif_path {
...
sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion.ppif');
}

sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat.ppif');
}
...
sub sample_capacity_dynamic_read_same_id_issue_order_queue_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue.ppif');
}

sub sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_depth3_same_id_issue_order_queue.ppif');
}

sub sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue.ppif');
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue.ppif');
}

sub sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue_read_data.ppif');
}

sub sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_ppif_path {
...
sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length.ppif');
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion.ppif');
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat.ppif');
}
...
sub sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path {
...
sub sample_capacity_dynamic_read_response_demux_same_id_reject_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_response_demux_same_id_reject.ppif');
}
...
sub sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_manager_capacity_status_dynamic_read_response_demux_multi_same_id_reject.ppif');
}
...
sub sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif {
    return slurp(sample_capacity_dynamic_same_id_issue_order_queue_policy_ppif_path());
}
...
sub sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif {
    return slurp(sample_capacity_read_mixed_dynamic_static_same_id_issue_order_queue_read_data_ppif_path());
}
...
sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif {
    return slurp(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path());
}

sub sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif {
    return slurp(sample_capacity_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat_ppif_path());
}

sub sample_capacity_dynamic_write_depth3_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_dynamic_write_depth3_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_dynamic_read_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_dynamic_read_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_dynamic_read_depth3_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_ppif_path());
}

sub sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif {
    return slurp(sample_capacity_dynamic_read_same_id_issue_order_queue_read_data_ppif_path());
}
...
sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_ppif_path());
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion_ppif_path());
}

sub sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat_ppif_path());
}

sub sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_ppif {
    return slurp(sample_capacity_dynamic_read_burst_last_same_id_issue_order_queue_read_data_burst_length_ppif_path());
}
...
sub sample_capacity_dynamic_read_response_demux_same_id_reject_ppif {
    return slurp(sample_capacity_dynamic_read_response_demux_same_id_reject_ppif_path());
}
...
sub sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif {
    return slurp(sample_capacity_dynamic_read_response_demux_multi_same_id_reject_ppif_path());
}
...
sub assert_mixed_dynamic_static_write_same_id_issue_order_queue_report {
    ...
    is($demux->{mode}, 'bounded_mixed_dynamic_static_write_bid_issue_order_queue_demux_contract', "$owner marks mixed write issue-order queue demux mode");
    ok($demux->{generated_behavior}, "$owner marks mixed write issue-order queue demux generated");
    is_deeply($demux->{residue}, [qw(read_response_demux read_data_interleaving bursts)], "$owner removes same-ID residue from response-demux report");
    ...
    is($write->{transaction_completion_semantics}, 'earliest_matching_captured_or_static_runtime_id', "$owner reports mixed captured/static runtime-ID matching");
    is($write->{queue_state_representation}, 'compact_runtime_id_issue_order_slots', "$owner reports compact runtime-ID slot representation");
    ...
    is($policy->{enforcement}, 'generated_mixed_dynamic_static_issue_order_queue', "$owner reports generated mixed queue enforcement");
...
sub assert_mixed_dynamic_static_read_same_id_issue_order_queue_report {
    ...
    is($read->{response_scope}, $expected_response_scope, "$owner reports read response scope");
    if ($burst_last) {
        ...
        is($read->{last_signal_direction}, 'generated_input', "$owner reports generated RLAST input direction");
    ...
    ok($policy->{accepted_same_id_reuse}, "$owner accepts read same-ID reuse through queue");
    ...
    is_deeply($policy->{covered_dynamic_transactions}, [qw(r0)], "$owner reports covered dynamic read transaction");
    is_deeply($policy->{covered_static_transactions}, [qw(r1)], "$owner reports covered static read transaction");
    is($policy->{active_id_uniqueness_policy}, 'not_required_for_issue_order_queue', "$owner does not require active read ID uniqueness");
    is($policy->{static_id_conflict_policy}, 'ordered_overlap_allowed', "$owner reports ordered static-ID overlap");
    is($policy->{first_generated_scope}, $expected_first_scope, "$owner reports mixed read generated scope");

    ok($queue->{mixed_dynamic_static_issue_order_queue}, "$owner marks queue as mixed");
    ...
    is_deeply(
        [map { $_->{request_id_source} } @{$queue->{enqueue_pulses}}],
        ['axi0_arid', "4'd3"],
        "$owner reports dynamic read signal and static literal enqueue sources",
    );
    is_deeply(
        $queue->{slot_storage},
        [
    ...
    ok(grep { $_ eq 'axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r1' } @{$queue->{generated_update_rules}}, "$owner reports static read literal enqueue rule");
    ok(grep { $_ eq 'axi0_read_mixed_dynamic_static_same_id_issue_order_r0_r1_dequeue_enqueue_r1' } @{$queue->{generated_update_rules}}, "$owner reports mixed read selected dequeue plus static enqueue rule");
    ok(grep { $_ eq 'axi0_read_mixed_dynamic_static_same_id_issue_order_response_has_selected_match' } @{$queue->{generated_assertions}}, "$owner reports mixed read queue selected-match assertion");
    if ($burst_last) {
...
sub assert_dynamic_write_depth3_same_id_issue_order_queue_report {
    ...
    is_deeply(
        ...
        [qw(
            ...
            axi0_write_dynamic_same_id_issue_order_slot1_w0_q
            axi0_write_dynamic_same_id_issue_order_slot1_w1_q
            axi0_write_dynamic_same_id_issue_order_slot1_w2_q
            axi0_write_dynamic_same_id_issue_order_slot1_id_q
            axi0_write_dynamic_same_id_issue_order_slot2_w0_q
            axi0_write_dynamic_same_id_issue_order_slot2_w1_q
...
sub assert_dynamic_read_depth3_same_id_issue_order_queue_report {
    ...
    is_deeply(
        ...
        [qw(
            ...
            axi0_read_dynamic_same_id_issue_order_slot1_r0_q
            axi0_read_dynamic_same_id_issue_order_slot1_r1_q
            axi0_read_dynamic_same_id_issue_order_slot1_r2_q
            axi0_read_dynamic_same_id_issue_order_slot1_id_q
            axi0_read_dynamic_same_id_issue_order_slot2_r0_q
            axi0_read_dynamic_same_id_issue_order_slot2_r1_q
...
sub assert_dynamic_read_burst_last_same_id_issue_order_queue_report {
    ...
    my $expected_slot_storage_names = $args{slot_storage_names} // [qw(
        axi0_read_dynamic_same_id_issue_order_slot0_r0_q
        axi0_read_dynamic_same_id_issue_order_slot0_r1_q
        axi0_read_dynamic_same_id_issue_order_slot0_id_q
        axi0_read_dynamic_same_id_issue_order_slot1_r0_q
...
sub assert_rlast_report_prose_alignment {
    ...
    like(
        ...
        qr/[Ss]ingle-active dynamic write ID capture and BID response matching including same-cycle release-and-recapture, bounded multiple all-dynamic write BID response-demux matching including same-cycle release-and-recapture/,
    ...
    like(
        $id_residue->{detail},
        qr/generated scalar single-beat mixed dynamic\/static issue-order queue read-data RDATA\/RRESP capture/,
        "$owner reports mixed dynamic/static issue-order queue single-beat read-data capture as supported",
    );
    like(
        $id_residue->{detail},
        qr/generated scalar last-beat mixed dynamic\/static issue-order queue read-data RDATA\/RRESP capture/,
...
sub assert_dynamic_same_id_issue_order_queue_policy_report {
    ...
    is($ordering->{mode}, 'dynamic_id_reuse_policy', "$owner marks dynamic same-ID issue-order queue policy mode");
    ok(!$ordering->{generated_behavior}, "$owner marks dynamic same-ID issue-order queue generated behavior false");
    ok(!exists($ordering->{families}), "$owner does not report generated same-ID avoidance families");
    is_deeply(
        $ordering->{residue},
    ...
    ok(@{$ordering->{source_anchors}}, "$owner carries source anchors into dynamic same-ID issue-order queue metadata");
    is_deeply(
        [sort keys %{$ordering->{dynamic_id_reuse_policy}}],
        ['read'],
    ...
    is_deeply(
        ...
        "$owner reports selected-not-generated dynamic same-ID issue-order queue metadata",
    );
}
