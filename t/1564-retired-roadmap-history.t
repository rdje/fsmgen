...
my $verifier = File::Spec->catfile(
    $repo, 'scripts', 'check_retired_roadmap_history.pl');
...
subtest 'exact retired object passes with no live consumer' => sub {
    ...
    like($output,
        qr/exact .*:ROADMAP_STATUS\.md recovery and zero live consumers verified/,
...
subtest 'recreated path and planted active consumer fail independently' => sub {
    ...
    write_file($live->{root}, 'ROADMAP_STATUS.md', "recreated\n");
    my ($live_ok, $live_output) = run_verifier($live);
    ok(!$live_ok, 'recreated retired path fails');
    like($live_output,
        qr/ROADMAP_STATUS\.md is retired and must remain absent/,
        'live-path residue is named');

    my $consumer = make_fixture();
    write_file($consumer->{root}, 'AGENTS.md',
        "Read ROADMAP_STATUS.md for state.\n");
    ...
    like($consumer_output,
        qr/active policy consumer still names retired ROADMAP_STATUS\.md: AGENTS\.md/,
...
subtest 'retrieval identity failures fail closed' => sub {
    ...
    like($digest_output, qr/retrieved SHA-256 changed/,
    ...
    like($revision_output, qr/cannot retrieve/,
        'retrieval failure is explicit');
};

subtest 'whole-document token sweep detects planted orphan exactly' => sub {
    my $source = "# ROADMAP STATUS\n## R1\n- current `path/to/file.pm`\n";
    my $before = tokens($source);
    my $probe = 'fsmgen_orphan_probe_retired_roadmap_history';
...
sub make_fixture {
    my $root = create_project_tempdir(
        purpose => 'retired-roadmap-history-tests');
    run_git($root, 'init');
    run_git($root, 'config', 'user.name', 'Fixture');
    run_git($root, 'config', 'user.email', 'fixture@example.invalid');
    my $source = "# ROADMAP STATUS\n## R1\n- exact history\n";
    write_file($root, 'ROADMAP_STATUS.md', $source);
    ...
    unlink File::Spec->catfile($root, 'ROADMAP_STATUS.md')
        or die "cannot remove fixture ROADMAP_STATUS.md: $!";
    run_git($root, 'add', '-u');
    return {
        root => $root,
        revision => $revision,
