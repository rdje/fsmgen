...
(actor unknown_parameter_bank_width
  ...
  (storage
    (bank data (width DATA_W) (depth 2))))
ISF
        qr/\AError: actor 'unknown_parameter_bank_width' storage bank 'data' width token 'DATA_W' is not a declared actor scalar parameter/,
        'unknown symbolic width is rejected',
    );

    assert_parse_rejected(
        <<'ISF',
(actor constant_parameter_bank_width
  (clock clk)
  (constants
    (DATA_W 7))
  (interface
  ...
  (storage
    (bank data (width DATA_W) (depth 2))))
ISF
        qr/\AError: actor 'constant_parameter_bank_width' storage bank 'data' width token 'DATA_W' is an actor constant/,
        'actor constants remain outside this bank-width slice',
    );
...
ISF
        qr/\AError: actor 'expression_parameter_bank_width' storage 'data' width requires '\(width positive_integer_or_actor_scalar_parameter\)'/,
