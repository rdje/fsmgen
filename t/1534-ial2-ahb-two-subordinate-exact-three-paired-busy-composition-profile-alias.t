...
subtest 'two-subordinate exact-three paired .ahb alias mirrors generic source and lowering' => sub {
    ok(-f alias_path(), 'tracked two-subordinate exact-three paired .ahb alias exists');
    ...
    is($alias->{report}{source_object}{id}, 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-three-byte-lane-hburst-seq-busy-park', 'alias preserves source object');
    is($alias->{report}{source_object}{intent_name}, 'ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'alias preserves intent name');
...
subtest 'check, schedule, semantic JSON, and read-only MCP expose alias identity' => sub {
    ...
    is($schedule->{source_object}{id}, 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-three-byte-lane-hburst-seq-busy-park', 'schedule JSON keeps source object');
    is($schedule->{source_object}{intent_name}, 'ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'schedule JSON keeps intent name');
    ...
    is_deeply(
        ...
        [qw(ahb_control_subordinate_byte_lane_hburst_seq.isf ahb_interconnect.isf ahb_status_subordinate_byte_lane_hburst_seq.isf amba_requester_busy_insert_three.isf)],
...
subtest 'review artifacts, HDL, and verifier preserve two-subordinate exact-three paired identity' => sub {
    ...
    like(slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_three.isf')), qr/ahb_busy_remaining_q/, 'requester IAL1 keeps exact-three counter');
...
subtest 'profile boundaries and existing aggregate identities remain stable' => sub {
    ...
    is($generic->{support_accounting}{entry_id}, 'intent.ppif_ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'generic exact-three source keeps PPIF support identity');
    is($generic->{support_accounting}{source_kind}, 'ppif', 'generic exact-three source keeps PPIF source kind');

    my $two_window_exact_two = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', two_window_exact_two_alias_path());
    is($two_window_exact_two->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park', 'two-window exact-two paired alias keeps support identity');
    my $one_window_exact_three = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', one_window_exact_three_alias_path());
    is($one_window_exact_three->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'one-window exact-three paired alias keeps support identity');
...
sub alias_relpath {
    return 'ppif/ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ahb';
}

sub two_window_exact_two_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park.ahb');
}

sub one_window_exact_three_alias_path {
...
sub support_id {
    return 'intent.ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park_pipeline_cli';
}
