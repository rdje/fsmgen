...
my $isf_file = File::Spec->catfile($repo_root, 'isf', 'atl_data_route_pipeline.isf');

subtest 'ATL scalar data-route fixture lowers to one scheduled parent artifact' => sub {
    my ($files, $report) = lower_atl_fixture();

    is_deeply(
        sorted([keys %$files]),
        ['atl_data_route_pipeline.fsm'],
        'lowering emits only the selected parent scheduled FSM',
    );

    my $fsm = $files->{'atl_data_route_pipeline.fsm'};
    like($fsm, qr/\A\(\?fsm:atl_data_route_pipeline\b/, 'scheduled FSM names atl_data_route_pipeline');
    ...
    like($fsm, qr/\(= \(feed_consumer_start 1\)\)/, 'drive state requests the data route');
    like($fsm, qr/\(<- \(consumer_payload>\s+producer_payload\) <feed_consumer_start\)/,
        'drive body gates the actor-to-actor scalar route with the drive request');
    like($fsm, qr/\(<1 \(done> 1\)\)/, 'fixture completes with a one-cycle done pulse');

    assert_report_shape($report);
};

subtest 'ATL scalar data-route fixture strict schedule JSON matches the in-process report' => sub {
    ...
    ok($success, 'strict schedule JSON generation succeeds for the ATL data-route fixture');
...
subtest 'ATL scalar data-route fixture reaches plain and strict HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_data_route_pipeline_plain.sv');
    my $plain = generate_hdl($plain_hdl, [], 'plain HDL generation');

    like($plain, qr/\bmodule\s+atl_data_route_pipeline\b/, 'plain HDL contains the ATL parent module');
    ...
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_data_route_pipeline_strict.sv');
    my $strict = generate_hdl($strict_hdl, ['--strict'], 'strict HDL generation');

    like($strict, qr/\bmodule\s+atl_data_route_pipeline\b/, 'strict HDL contains the ATL parent module');
...
sub assert_report_shape {
    ...
    is($report->{source}, 'atl_data_route_pipeline.isf', 'schedule report names the ATL data-route fixture');
    is($report->{scheduled_fsm}, 'atl_data_route_pipeline.fsm', 'schedule report names the scheduled parent FSM');
    ...
    is($report->{port_count}, 4, 'schedule report port count includes generated data handoffs');
    ...
    is_deeply(
        ...
        'schedule report records the named data-route drive body',
    );
    is_deeply(
        $report->{transactions},
        [
        ...
        'schedule report records the data-route transaction state order',
    );
    ...
    is_deeply(
        ...
        'report records the producer and consumer static actor instances',
    );
    is_deeply($actor_network->{groups}, [], 'data-route fixture has no permanent static group');
    ...
    is_deeply(
        ...
        'report records the scalar actor-to-actor data-route evidence',
    );
}
...
sub generate_hdl {
    ...
    ok($success, "$label succeeds for the ATL data-route fixture");
