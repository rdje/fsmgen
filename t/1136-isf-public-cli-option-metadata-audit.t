...
subtest 'direct ISF public CLI option metadata is exact and unique' => sub {
    assert_cli_options(
        build_isf_public_interface_contract()->{cli_option_names},
        'direct ISF public-interface contract',
    );
};

subtest 'manifest ISF public CLI option metadata is exact and unique' => sub {
    my @views = (
        {
    ...
    for my $view (@views) {
        my $label = $view->{label};
        assert_cli_options(
            $view->{payload}{embedding}{isf_public_interface}{cli_option_names},
...
sub assert_cli_options {
    my ($options, $label) = @_;

    is_deeply(
        $options,
        isf_public_interface_cli_option_names(),
        "$label CLI option list is exact",
    );
    assert_unique_scalar_list($options, "$label CLI option list");
}
...
sub assert_unique_scalar_list {
    ...
    for my $value (@{$values || []}) {
        ...
        like($value, qr/\A--/, "$label entry '$value' is a long option name");
