...
subtest 'manifest producer section contract identity metadata rebuilds cleanly after caller mutation' => sub {
    ...
    is($contract->{'status'}, $expected->{'status'}, 'fresh manifest producer contract rebuilds clean status');
    is($contract->{'contract_source'}, $expected->{'contract_source'}, 'fresh manifest producer contract rebuilds clean contract_source');
    is($contract->{'report_source'}, $expected->{'report_source'}, 'fresh manifest producer contract rebuilds clean report_source');
