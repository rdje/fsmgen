...
subtest 'malformed contract objects fail closed and no direct lower-to-fsm entrypoint is exposed' => sub {
    ...
    my @cases = (
        ...
        ['dynamic last-beat read-data runtime burst length remains unsupported', sub { my $c = sample_contract_with_dynamic_read_data_last_beat(); $c->{read_data}{read}{burst_length} = sample_contract_with_read_data_burst_length_runtime_assertion()->{read_data}{read}{burst_length}; $c }, qr/no burst_length metadata or report-only burst_length metadata/],
        ['dynamic write response demux requires exactly one dynamic write transaction', sub {
            ...
            $c;
        }, qr/response_demux\.write dynamic ID matching supports exactly one dynamic write transaction/],
