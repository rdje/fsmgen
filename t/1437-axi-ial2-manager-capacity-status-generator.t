...
subtest 'dynamic same-ID reject policy reports selected-not-generated metadata without generated behavior' => sub {
    my $base = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_transaction_id());
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_same_id_reject_policy());
    ...
    is($result->{report}{source_object}{id}, 'axi-manager-capacity-status-dynamic-same-id-reject-policy', 'dynamic same-ID policy source object id is preserved');
    is($result->{report}{source_object}{intent_name}, 'axi_manager_capacity_status_dynamic_same_id_reject_policy', 'dynamic same-ID policy source intent name is preserved');
    assert_dynamic_transaction_id_report($result->{report}{transactions}, 'generator report');
    assert_dynamic_same_id_reject_policy_report($result->{report}{same_id_ordering}, 'generator report');
    my %residue = map { $_->{id} => 1 } @{$result->{report}{unsupported_residue}};
    ok($residue{dynamic_transaction_id_behavior}, 'dynamic behavior boundary remains explicit unsupported residue');
};
...
subtest 'dynamic same-ID reject policy coexists with concrete same-ID policy metadata' => sub {
    ...
    is($ordering->{mode}, 'id_reuse_policy', 'combined concrete and dynamic same-ID metadata uses combined reuse-policy mode');
    ok(!$ordering->{generated_behavior}, 'combined same-ID policy metadata remains selected-not-generated');
    is_deeply(
        $ordering->{residue},
        [qw(concrete_id_same_id_ordering per_id_issue_order_queues dynamic_id_same_id_ordering)],
        'combined same-ID policy residue keeps concrete queues and dynamic ordering explicit',
    );
...
subtest 'multiple dynamic write response-demux maps dynamic same-ID reject to generated assertions' => sub {
    ...
    is(
        $result->{generated_ial1}{text},
        $base->{generated_ial1}{text},
        'dynamic write same-ID reject mapping does not alter generated IAL1 text',
    );
    is_deeply(
        $result->{generated_ial0}{files},
        $base->{generated_ial0}{files},
        'dynamic write same-ID reject mapping does not alter generated IAL0 files',
    );
    assert_dynamic_write_response_demux_multi_report(
        $result->{report},
        'generator mapped dynamic write report',
        residue => [qw(read_response_demux read_data_interleaving bursts)],
    );
    assert_dynamic_same_id_reject_policy_generated_report(
        ...
        'generator mapped dynamic write report',
        family => 'write',
        response_demux_mode => 'bounded_multi_dynamic_write_bid_demux_contract',
        response_demux_transaction_completion_source => 'generated_dynamic_demux',
        covered_dynamic_transactions => [qw(w0 w1)],
...
subtest 'multiple dynamic read burst-last response-demux maps dynamic same-ID reject to generated assertions' => sub {
    my $base = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_response_demux_multi_burst_last());
    ...
    is(
        $result->{generated_ial1}{text},
        $base->{generated_ial1}{text},
        'dynamic read RLAST same-ID reject mapping does not alter generated IAL1 text',
    );
    is_deeply(
        $result->{generated_ial0}{files},
        $base->{generated_ial0}{files},
        'dynamic read RLAST same-ID reject mapping does not alter generated IAL0 files',
    );
    assert_dynamic_read_response_demux_multi_burst_last_report(
        $result->{report},
        'generator mapped dynamic read RLAST report',
        residue => [qw(read_data_interleaving bursts)],
    );
    assert_dynamic_same_id_reject_policy_generated_report(
        $result->{report}{same_id_ordering},
        'generator mapped dynamic read RLAST report',
        family => 'read',
        response_demux_mode => 'bounded_multi_dynamic_read_rid_rlast_demux_contract',
        response_demux_transaction_completion_source => 'generated_dynamic_demux_last_beat',
...
subtest 'dynamic write same-ID issue-order queue captures AWID slots and matches BID by issue order' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_write_same_id_issue_order_queue());
    my $isf = $result->{generated_ial1}{text};
    ...
    like($isf, qr/\(output axi0_w0_complete\)/, 'dynamic write issue-order queue exposes w0 completion');
    like($isf, qr/\(output axi0_w1_complete\)/, 'dynamic write issue-order queue exposes w1 completion');
    unlike($isf, qr/axi0_w0_dynamic_busy_q/, 'dynamic write issue-order queue does not allocate legacy dynamic busy state');
    ...
    like($isf, qr/write dynamic same-ID response matches at least one captured runtime ID/, 'dynamic write issue-order queue emits response match assertion');

    assert_dynamic_write_same_id_issue_order_queue_report($result->{report}, 'generator report');

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/\(-axi0_w0_response_demux\s+<\(\|/, 'scheduled .fsm lowers w0 dynamic queue response demux');
    like($fsm, qr/axi0_write_dynamic_same_id_issue_order_slot1_id_q/, 'scheduled .fsm carries slot1 ID state');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\breg\s+\[3:0\]\s+axi0_write_dynamic_same_id_issue_order_slot0_id_q\b/, 'SystemVerilog declares slot0 ID queue register');
    like($hdl, qr/\breg\s+\[3:0\]\s+axi0_write_dynamic_same_id_issue_order_slot1_id_q\b/, 'SystemVerilog declares slot1 ID queue register');
...
subtest 'mixed dynamic/static write multi-static same-ID issue-order queue captures AWID and static ID slots' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_mixed_dynamic_static_write_same_id_issue_order_queue_multi_static());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_awid \(width 4\)\)/, 'mixed dynamic/static write multi-static issue-order queue declares AWID input');
    like($isf, qr/\(input axi0_bid \(width 4\)\)/, 'mixed dynamic/static write multi-static issue-order queue declares BID input');
    like($isf, qr/\(output axi0_w0_complete\)/, 'mixed dynamic/static write multi-static issue-order queue exposes dynamic completion');
    like($isf, qr/\(output axi0_w1_complete\)/, 'mixed dynamic/static write multi-static issue-order queue exposes first static completion');
    like($isf, qr/\(output axi0_w2_complete\)/, 'mixed dynamic/static write multi-static issue-order queue exposes second static completion');
    unlike($isf, qr/axi0_w0_dynamic_busy_q/, 'mixed dynamic/static write multi-static issue-order queue does not allocate legacy dynamic busy state');
    unlike($isf, qr/axi0_w1_static_busy_q/, 'mixed dynamic/static write multi-static issue-order queue does not allocate legacy first static busy state');
    unlike($isf, qr/axi0_w2_static_busy_q/, 'mixed dynamic/static write multi-static issue-order queue does not allocate legacy second static busy state');
    like($isf, qr/\(var axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'mixed dynamic/static write multi-static issue-order queue allocates slot0 ID state');
    like($isf, qr/\(var axi0_write_mixed_dynamic_static_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'mixed dynamic/static write multi-static issue-order queue allocates slot1 ID state');
    like($isf, qr/\(var axi0_write_mixed_dynamic_static_same_id_issue_order_slot2_id_q \(width 4\)\)/, 'mixed dynamic/static write multi-static issue-order queue allocates slot2 ID state');
    like($isf, qr/\(rule axi0_write_mixed_dynamic_static_same_id_issue_order_empty_enqueue_w0[\s\S]*\(axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q axi0_awid\)\)/, 'mixed dynamic/static write multi-static issue-order queue captures AWID on dynamic enqueue');
    like($isf, qr/\(rule axi0_write_mixed_dynamic_static_same_id_issue_order_empty_enqueue_w1[\s\S]*\(axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd3\)\)/, 'mixed dynamic/static write multi-static issue-order queue captures first static literal on static enqueue');
    like($isf, qr/\(rule axi0_write_mixed_dynamic_static_same_id_issue_order_empty_enqueue_w2[\s\S]*\(axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd5\)\)/, 'mixed dynamic/static write multi-static issue-order queue captures second static literal on static enqueue');
    like($isf, qr/\(rule axi0_w2_response_demux [\s\S]*axi0_write_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*axi0_write_mixed_dynamic_static_same_id_issue_order_slot1_id_q[\s\S]*axi0_write_mixed_dynamic_static_same_id_issue_order_slot2_id_q[\s\S]*\(pulse axi0_w2_complete\)\)/, 'mixed dynamic/static write multi-static issue-order queue emits second static earliest matching BID completion');
    assert_mixed_dynamic_static_write_same_id_issue_order_queue_report(
        $result->{report},
        'generator multi-static report',
        static_transactions => [qw(w1 w2)],
        static_enqueue_sources => ["4'd3", "4'd5"],
    );

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/axi0_write_mixed_dynamic_static_same_id_issue_order_slot2_id_q/, 'scheduled .fsm carries mixed multi-static queue slot2 ID state');
    like($fsm, qr/4'd5/, 'scheduled .fsm carries second static literal queue enqueue');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\breg\s+\[3:0\]\s+axi0_write_mixed_dynamic_static_same_id_issue_order_slot2_id_q\b/, 'SystemVerilog declares mixed multi-static slot2 ID queue register');
    like($hdl, qr/axi0_write_mixed_dynamic_static_same_id_issue_order_slot\d_id_q_next\s*=\s*4'd5\s*;/, 'SystemVerilog captures second static literal into a queue slot');
};

subtest 'mixed dynamic/static read same-ID issue-order queue captures ARID and static ID slots' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_mixed_dynamic_static_read_same_id_issue_order_queue());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_arid \(width 4\)\)/, 'mixed dynamic/static read issue-order queue declares ARID input');
    like($isf, qr/\(input axi0_rid \(width 4\)\)/, 'mixed dynamic/static read issue-order queue declares RID input');
    like($isf, qr/\(output axi0_r0_complete\)/, 'mixed dynamic/static read issue-order queue exposes dynamic completion');
    like($isf, qr/\(output axi0_r1_complete\)/, 'mixed dynamic/static read issue-order queue exposes static completion');
    unlike($isf, qr/axi0_r0_dynamic_busy_q/, 'mixed dynamic/static read issue-order queue does not allocate legacy dynamic busy state');
    unlike($isf, qr/axi0_r1_static_busy_q/, 'mixed dynamic/static read issue-order queue does not allocate legacy static busy state');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q \(width 4\)\)/, 'mixed dynamic/static read issue-order queue allocates slot0 ID state');
    like($isf, qr/\(var axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q \(width 4\)\)/, 'mixed dynamic/static read issue-order queue allocates slot1 ID state');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r0[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q axi0_arid\)\)/, 'mixed dynamic/static read issue-order queue captures ARID on dynamic enqueue');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_empty_enqueue_r1[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q 4'd3\)\)/, 'mixed dynamic/static read issue-order queue captures static literal on static enqueue');
    like($isf, qr/\(rule axi0_read_mixed_dynamic_static_same_id_issue_order_r0_r1_dequeue_enqueue_r1[\s\S]*\(axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q 4'd3\)\)/, 'mixed dynamic/static read issue-order queue recaptures static literal after selected dequeue');
    like($isf, qr/\(rule axi0_r0_response_demux [\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q[\s\S]*\(pulse axi0_r0_complete\)\)/, 'mixed dynamic/static read issue-order queue emits dynamic earliest matching RID completion');
    like($isf, qr/\(rule axi0_r1_response_demux [\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q[\s\S]*\(pulse axi0_r1_complete\)\)/, 'mixed dynamic/static read issue-order queue emits static earliest matching RID completion');
    assert_mixed_dynamic_static_read_same_id_issue_order_queue_report($result->{report}, 'generator report');

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/axi0_read_mixed_dynamic_static_same_id_issue_order_slot1_id_q/, 'scheduled .fsm carries mixed read queue slot ID state');
...
subtest 'mixed dynamic/static read burst-last same-ID issue-order queue read-data validates runtime raw ARLEN' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data runtime burst-length declares ARLEN input');
    like($isf, qr/\(var axi0_r0_expected_beats_q \(width 5\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data runtime burst-length allocates dynamic expected-beat storage');
    like($isf, qr/\(var axi0_r1_expected_beats_q \(width 5\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data runtime burst-length allocates static expected-beat storage');
    like($isf, qr/\(var axi0_r0_read_beat_count_q \(width 5\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data runtime burst-length allocates dynamic read-beat counter');
    like($isf, qr/\(var axi0_r1_read_beat_count_q \(width 5\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data runtime burst-length allocates static read-beat counter');
    ...
    assert_read_data_burst_length_report(
        $result->{report}{read_data},
        'generator mixed dynamic/static read burst-last same-ID issue-order queue read-data runtime burst-length report',
        'runtime_assertion',
        'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );
    ...
    is(scalar(@beat_count_assertions), 8, 'scheduled .fsm lowers four mixed queue beat-count/RLAST assertions per covered transaction');
    like($fsm, qr/\(-axi0_r1_beat_count_init\s+<axi0_r1_request\s+\(<- \(axi0_r1_expected_beats_q \(\+ axi0_arlen\[4:0\] 5'd1\)\)\)\s+\(<- \(axi0_r1_read_beat_count_q 0\)\)/, 'scheduled .fsm lowers static mixed queue expected-beat initialization');
    like($fsm, qr/\(-axi0_r1_read_beat_count\s+<[\s\S]*axi0_read_mixed_dynamic_static_same_id_issue_order_slot0_id_q[\s\S]*\(<- \(axi0_r1_read_beat_count_q \(\+ axi0_r1_read_beat_count_q 5'd1\)\)\)/, 'scheduled .fsm lowers static mixed queue matched-beat increment');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\breg\s+\[4:0\]\s+axi0_r1_expected_beats_q\b/, 'SystemVerilog declares static mixed queue expected-beat storage');
    like($hdl, qr/\breg\s+\[4:0\]\s+axi0_r1_read_beat_count_q\b/, 'SystemVerilog declares static mixed queue beat-count storage');
    like($hdl, qr/axi0_r1_read_beat_count_q_next\s*=\s*axi0_r1_read_beat_count_q\s*\+\s*5'd1\s*;/, 'SystemVerilog increments static mixed queue beat count');
    like($hdl, qr/assign\s+axi0_r1_read_beat_count_en\s*=/, 'SystemVerilog emits static mixed queue beat-count increment enable');
};

subtest 'mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat emits output banks' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_multi_beat());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_rlast\)/, 'mixed dynamic/static read burst-last issue-order queue read-data multi-beat declares RLAST input');
    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data multi-beat declares ARLEN input');
    like($isf, qr/\(output axi0_r0_beat_rdata_0 \(width 32\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data multi-beat exposes dynamic first RDATA lane');
    like($isf, qr/\(output axi0_r1_beat_rdata_15 \(width 32\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data multi-beat exposes static final RDATA lane');
    like($isf, qr/\(output axi0_r1_beat_rresp_15 \(width 2\)\)/, 'mixed dynamic/static read burst-last issue-order queue read-data multi-beat exposes static final RRESP lane');
    ...
    assert_read_data_multi_beat_report(
        $result->{report}{read_data},
        'generator mixed dynamic/static read burst-last same-ID issue-order queue read-data multi-beat report',
        completion_validity => 'generated_mixed_dynamic_static_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1)],
    );

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/axi0_r1_read_beat_15_capture[\s\S]*\(<- \(axi0_r1_beat_rdata_15> axi0_rdata\)\)[\s\S]*\(<- \(axi0_r1_beat_rresp_15> axi0_rresp\)\)/, 'scheduled .fsm lowers static mixed queue final multi-beat lane capture');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\boutput\s+(?:reg\s+)?\[31:0\]\s+axi0_r1_beat_rdata_15\b/, 'SystemVerilog exposes static final RDATA lane output');
    like($hdl, qr/\boutput\s+(?:reg\s+)?\[1:0\]\s+axi0_r1_beat_rresp_15\b/, 'SystemVerilog exposes static final RRESP lane output');
    like($hdl, qr/\boutput\s+(?:reg\s+)?\[15:0\]\s+axi0_r1_beat_valid\b/, 'SystemVerilog exposes static valid-mask output');
    like($hdl, qr/\boutput\s+(?:reg\s+)?\[4:0\]\s+axi0_r1_read_beats\b/, 'SystemVerilog exposes static length output');
};
...
subtest 'dynamic write depth-3 same-ID issue-order queue captures third AWID slot and matches BID by issue order' => sub {
    ...
    like($isf, qr/\(output axi0_w2_complete\)/, 'dynamic write depth-3 issue-order queue exposes w2 completion');
    ...
    like($isf, qr/\(rule axi0_w2_response_demux [\s\S]*axi0_write_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_write_dynamic_same_id_issue_order_slot2_id_q[\s\S]*\(pulse axi0_w2_complete\)\)/, 'dynamic write depth-3 issue-order queue emits w2 earliest matching BID completion');
    ...
    assert_dynamic_write_depth3_same_id_issue_order_queue_report($result->{report}, 'generator report');
};

subtest 'dynamic read same-ID issue-order queue captures ARID slots and matches RID by issue order' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_same_id_issue_order_queue());
    my $isf = $result->{generated_ial1}{text};
    ...
    like($isf, qr/\(output axi0_r0_complete\)/, 'dynamic read issue-order queue exposes r0 completion');
    like($isf, qr/\(output axi0_r1_complete\)/, 'dynamic read issue-order queue exposes r1 completion');
    unlike($isf, qr/axi0_r0_dynamic_busy_q/, 'dynamic read issue-order queue does not allocate legacy r0 busy state');
    unlike($isf, qr/axi0_r1_dynamic_id_q/, 'dynamic read issue-order queue does not allocate legacy r1 selected ID state');
    ...
    like($isf, qr/read dynamic same-ID response matches at least one captured runtime ID/, 'dynamic read issue-order queue emits response match assertion');

    assert_dynamic_read_same_id_issue_order_queue_report($result->{report}, 'generator report');

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/\(-axi0_r0_response_demux\s+<\(\|/, 'scheduled .fsm lowers r0 dynamic queue response demux');
    like($fsm, qr/axi0_read_dynamic_same_id_issue_order_slot1_id_q/, 'scheduled .fsm carries slot1 ID state');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\breg\s+\[3:0\]\s+axi0_read_dynamic_same_id_issue_order_slot0_id_q\b/, 'SystemVerilog declares slot0 ID queue register');
    like($hdl, qr/\breg\s+\[3:0\]\s+axi0_read_dynamic_same_id_issue_order_slot1_id_q\b/, 'SystemVerilog declares slot1 ID queue register');
...
subtest 'dynamic read depth-3 same-ID issue-order queue captures third ARID slot and matches RID by issue order' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_depth3_same_id_issue_order_queue());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_r2_request\)/, 'dynamic read depth-3 issue-order queue declares r2 request input');
    like($isf, qr/\(output axi0_r2_complete\)/, 'dynamic read depth-3 issue-order queue exposes r2 completion');
    unlike($isf, qr/axi0_r2_dynamic_id_q/, 'dynamic read depth-3 issue-order queue does not allocate legacy r2 selected ID state');
    ...
    like($isf, qr/\(rule axi0_r2_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_id_q[\s\S]*\(pulse axi0_r2_complete\)\)/, 'dynamic read depth-3 issue-order queue emits r2 earliest matching RID completion');
    ...
    assert_dynamic_read_depth3_same_id_issue_order_queue_report($result->{report}, 'generator report');

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/\(-axi0_r2_response_demux\s+<\(\|/, 'scheduled .fsm lowers r2 dynamic queue response demux');
...
subtest 'dynamic read burst-last same-ID issue-order queue dequeues only on RLAST' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_burst_last_same_id_issue_order_queue());
    my $isf = $result->{generated_ial1}{text};
    ...
    like($isf, qr/\(output axi0_r0_complete\)/, 'dynamic read burst-last issue-order queue exposes r0 completion');
    like($isf, qr/\(output axi0_r1_complete\)/, 'dynamic read burst-last issue-order queue exposes r1 completion');
    ...
    assert_dynamic_read_burst_last_same_id_issue_order_queue_report($result->{report}, 'generator report');
...
subtest 'dynamic read burst-last depth-3 same-ID issue-order queue read-data captures r2 last payload' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_r2_request\)/, 'dynamic read burst-last depth-3 queue read-data declares r2 request input');
    like($isf, qr/\(input axi0_rlast\)/, 'dynamic read burst-last depth-3 queue read-data declares RLAST input');
    like($isf, qr/\(input axi0_rdata \(width 32\)\)/, 'dynamic read burst-last depth-3 queue read-data declares RDATA input');
    like($isf, qr/\(input axi0_rresp \(width 2\)\)/, 'dynamic read burst-last depth-3 queue read-data declares RRESP input');
    like($isf, qr/\(output axi0_r2_complete\)/, 'dynamic read burst-last depth-3 queue read-data exposes r2 completion');
    like($isf, qr/\(output axi0_r2_last_rdata \(width 32\)\)/, 'dynamic read burst-last depth-3 queue read-data exposes r2 scalar last RDATA');
    like($isf, qr/\(output axi0_r2_last_rresp \(width 2\)\)/, 'dynamic read burst-last depth-3 queue read-data exposes r2 scalar last RRESP');
    unlike($isf, qr/axi0_r2_dynamic_id_q/, 'dynamic read burst-last depth-3 queue read-data does not allocate legacy r2 selected ID state');
    like($isf, qr/\(var axi0_read_dynamic_same_id_issue_order_slot2_id_q \(width 4\)\)/, 'dynamic read burst-last depth-3 queue read-data allocates slot2 captured ARID');
    like($isf, qr/\(rule axi0_r2_response_demux [\s\S]*axi0_read_dynamic_same_id_issue_order_slot0_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot1_id_q[\s\S]*axi0_read_dynamic_same_id_issue_order_slot2_id_q[\s\S]*axi0_rlast[\s\S]*\(pulse axi0_r2_complete\)\)/, 'dynamic read burst-last depth-3 queue read-data keeps queue-owned r2 RID/RLAST demux');
    like($isf, qr/\(rule axi0_r2_read_data_capture axi0_r2_complete\s+\(axi0_r2_last_rdata axi0_rdata\)\s+\(axi0_r2_last_rresp axi0_rresp\)\)/, 'dynamic read burst-last depth-3 queue read-data captures r2 payload under queue last-beat completion');
    unlike($isf, qr/\baxi0_arlen\b/, 'dynamic read burst-last depth-3 queue read-data keeps burst-length metadata absent');

    assert_dynamic_read_burst_last_depth3_same_id_issue_order_queue_report($result->{report}, 'generator report');
    assert_read_data_last_beat_report(
        $result->{report}{read_data},
        'generator dynamic read burst-last depth-3 queue read-data report',
        'generated_dynamic_read_issue_order_queue_response_demux_last_beat_completion_pulse',
        transactions => [qw(r0 r1 r2)],
    );

    my $fsm = $result->{generated_ial0}{files}{'axi0_capacity_status.fsm'};
    like($fsm, qr/\(-axi0_r2_read_data_capture\s+<axi0_r2_complete\s+\(<- \(axi0_r2_last_rdata> axi0_rdata\)\)\s+\(<- \(axi0_r2_last_rresp> axi0_rresp\)\)/, 'scheduled .fsm lowers r2 depth-3 queue read-data capture');
    my $hdl = hdl_for('axi0_capacity_status', $fsm);
    like($hdl, qr/\binput\s+(?:wire\s+)?axi0_rlast\b/, 'SystemVerilog exposes RLAST for depth-3 queue read-data');
    like($hdl, qr/\boutput\s+reg\s+\[31:0\]\s+axi0_r2_last_rdata\b/, 'SystemVerilog exposes r2 depth-3 queue read-data output');
    like($hdl, qr/assign\s+axi0_r2_read_data_capture_en\s*=\s*axi0_r2_complete\s*;/, 'SystemVerilog guards r2 depth-3 queue read-data capture by completion');
    like($hdl, qr/axi0_r2_last_rdata_next\s*=\s*axi0_rdata\s*;/, 'SystemVerilog captures r2 depth-3 queue RDATA');
    like($hdl, qr/axi0_r2_last_rresp_next\s*=\s*axi0_rresp\s*;/, 'SystemVerilog captures r2 depth-3 queue RRESP');
...
subtest 'dynamic read burst-last depth-3 same-ID issue-order queue read-data multi-beat emits r2 output banks' => sub {
    my $result = FSM::IAL2::ProtocolIntent::AxiManagerCapacityStatus->new()->generate(sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat());
    my $isf = $result->{generated_ial1}{text};

    like($isf, qr/\(input axi0_arlen \(width 8\)\)/, 'dynamic read burst-last depth-3 queue read-data multi-beat declares ARLEN input');
    like($isf, qr/\(output axi0_r2_beat_rdata_15 \(width 32\)\)/, 'dynamic read burst-last depth-3 queue read-data multi-beat exposes r2 final RDATA lane');
    like($isf, qr/\(output axi0_r2_beat_rresp_15 \(width 2\)\)/, 'dynamic read burst-last depth-3 queue read-data multi-beat exposes r2 final RRESP lane');
    ...
    assert_read_data_multi_beat_report(
        $result->{report}{read_data},
        'generator dynamic read burst-last depth-3 queue read-data multi-beat report',
    ...
    like($hdl, qr/\boutput\s+(?:reg\s+)?\[4:0\]\s+axi0_r2_read_beats\b/, 'SystemVerilog exposes r2 length output');
};
...
subtest 'mixed dynamic/static read response-demux captures ARID and matches static RID' => sub {
    ...
    like(
        ...
        'mixed dynamic/static read demux dynamic release-only excludes same-cycle dynamic request',
    );
    like(
        $isf,
        qr/\(rule axi0_r1_static_busy_release \(& axi0_r1_complete axi0_r1_static_busy_q \(! axi0_r1_request\)\)\s+\(axi0_r1_static_busy_q 0\)\)/,
        'mixed dynamic/static read demux static release-only excludes same-cycle static request',
    );
    like(
        $isf,
        qr/\(rule axi0_r0_dynamic_id_release_recapture \(& \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r0_complete axi0_r0_dynamic_busy_q \(! \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\) \(! \(== axi0_arid 4'd3\)\)\)\s+\(axi0_r0_dynamic_id_q axi0_arid\)\s+\(axi0_r0_dynamic_busy_q 1\)\)/,
        'mixed dynamic/static read demux recaptures dynamic ID on same-cycle dynamic completion',
    );
    like(
        $isf,
        qr/\(rule axi0_r1_static_busy_release_recapture \(& \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r1_complete axi0_r1_static_busy_q \(! \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\)\)\s+\(axi0_r1_static_busy_q 1\)\)/,
        'mixed dynamic/static read demux recaptures static busy on same-cycle static completion',
    );
    ...
    like($isf, qr/"axi0 read dynamic request is idle or releasing active captured ID"/, 'mixed dynamic/static read demux emits dynamic idle-or-releasing assertion');
    like($isf, qr/"axi0 read static request is idle or releasing active concrete ID"/, 'mixed dynamic/static read demux emits static idle-or-releasing assertion');
...
subtest 'mixed dynamic/static read burst-last response-demux gates completion with RLAST' => sub {
    ...
    like(
        $isf,
        qr/\(rule axi0_r0_dynamic_id_release \(& axi0_r0_complete axi0_r0_dynamic_busy_q \(! axi0_r0_request\)\)\s+\(axi0_r0_dynamic_busy_q 0\)\)/,
        'mixed dynamic/static read RLAST demux dynamic release-only excludes same-cycle dynamic request',
    );
    like(
        $isf,
        qr/\(rule axi0_r1_static_busy_release \(& axi0_r1_complete axi0_r1_static_busy_q \(! axi0_r1_request\)\)\s+\(axi0_r1_static_busy_q 0\)\)/,
        'mixed dynamic/static read RLAST demux static release-only excludes same-cycle static request',
    );
    like(
        $isf,
        qr/\(rule axi0_r0_dynamic_id_release_recapture \(& \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r0_complete axi0_r0_dynamic_busy_q \(! \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\) \(! \(== axi0_arid 4'd3\)\)\)\s+\(axi0_r0_dynamic_id_q axi0_arid\)\s+\(axi0_r0_dynamic_busy_q 1\)\)/,
        'mixed dynamic/static read RLAST demux recaptures dynamic ID on same-cycle final dynamic completion',
    );
    like(
        $isf,
        qr/\(rule axi0_r1_static_busy_release_recapture \(& \(& axi0_r1_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\) axi0_r1_complete axi0_r1_static_busy_q \(! \(& axi0_r0_request \(\| \(< axi0_pending_reads_q 4\) \(\| axi0_r0_complete axi0_r1_complete\)\)\)\)\)\s+\(axi0_r1_static_busy_q 1\)\)/,
        'mixed dynamic/static read RLAST demux recaptures static busy on same-cycle final static completion',
    );
    like(
        $isf,
    ...
    like($isf, qr/"axi0 read dynamic request is idle or releasing active captured ID"/, 'mixed dynamic/static read RLAST demux emits dynamic idle-or-releasing assertion');
    like($isf, qr/"axi0 read static request is idle or releasing active concrete ID"/, 'mixed dynamic/static read RLAST demux emits static idle-or-releasing assertion');
    ...
    like($fsm, qr/\(-axi0_r0_dynamic_id_release_recapture\s+<\(& \(& axi0_r0_request/, 'scheduled .fsm lowers mixed dynamic read RLAST release-recapture');
    like($fsm, qr/\(-axi0_r1_static_busy_release_recapture\s+<\(& \(& axi0_r1_request/, 'scheduled .fsm lowers mixed static read RLAST release-recapture');
...
sub sample_contract_with_dynamic_same_id_reject_policy {
    my $contract = sample_contract_with_dynamic_transaction_id();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_same_id_reject_policy';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-same-id-reject-policy';
    $contract->{same_id_ordering_policy} = {
        read => {
            dynamic_id_reuse => 'reject',
        },
...
sub sample_contract_with_dynamic_write_same_id_issue_order_queue {
    my $contract = sample_contract_with_dynamic_write_response_demux_multi();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_write_same_id_issue_order_queue';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-write-same-id-issue-order-queue';
    $contract->{same_id_ordering_policy} = {
        write => {
            dynamic_id_reuse => 'issue-order-queue',
        },
...
sub sample_contract_with_mixed_dynamic_static_write_same_id_issue_order_queue_multi_static {
    my $contract = sample_contract_with_mixed_dynamic_static_write_same_id_issue_order_queue();
    $contract->{intent_name} = 'axi_manager_capacity_status_write_mixed_dynamic_static_same_id_issue_order_queue_multi_static';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-write-mixed-dynamic-static-same-id-issue-order-queue-multi-static';
...
sub sample_contract_with_mixed_dynamic_static_read_same_id_issue_order_queue {
    my $contract = sample_contract_with_mixed_dynamic_static_read_response_demux();
    $contract->{intent_name} = 'axi_manager_capacity_status_read_mixed_dynamic_static_same_id_issue_order_queue';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-read-mixed-dynamic-static-same-id-issue-order-queue';
    $contract->{read_max_pending} = 2;
    $contract->{same_id_ordering_policy} = {
        read => {
            dynamic_id_reuse => 'issue-order-queue',
        },
...
sub sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion {
    my $contract = sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_burst_length();
    $contract->{intent_name} = 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-read-mixed-dynamic-static-burst-last-same-id-issue-order-queue-read-data-burst-length-runtime-assertion';
    $contract->{read_data}{read}{burst_length}{validation} = 'runtime-assertion';
    return $contract;
}

sub sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_multi_beat {
    my $contract = sample_contract_with_mixed_dynamic_static_read_burst_last_same_id_issue_order_queue_read_data_burst_length_runtime_assertion();
    $contract->{intent_name} = 'axi_manager_capacity_status_read_mixed_dynamic_static_burst_last_same_id_issue_order_queue_read_data_multi_beat';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-read-mixed-dynamic-static-burst-last-same-id-issue-order-queue-read-data-multi-beat';
...
sub sample_contract_with_dynamic_write_depth3_same_id_issue_order_queue {
    ...
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-write-depth3-same-id-issue-order-queue';
    $contract->{write_max_pending} = 3;
    push @{$contract->{transactions}}, {
        kind             => 'write',
        name             => 'w2',
...
sub sample_contract_with_dynamic_read_same_id_issue_order_queue {
    my $contract = sample_contract_with_dynamic_read_response_demux_multi();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_read_same_id_issue_order_queue';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-read-same-id-issue-order-queue';
    $contract->{same_id_ordering_policy} = {
        read => {
            dynamic_id_reuse => 'issue-order-queue',
        },
...
sub sample_contract_with_dynamic_read_depth3_same_id_issue_order_queue {
    my $contract = sample_contract_with_dynamic_read_same_id_issue_order_queue();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_read_depth3_same_id_issue_order_queue';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-read-depth3-same-id-issue-order-queue';
...
sub sample_contract_with_dynamic_read_burst_last_same_id_issue_order_queue {
    my $contract = sample_contract_with_dynamic_read_same_id_issue_order_queue();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_read_burst_last_same_id_issue_order_queue';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-read-burst-last-same-id-issue-order-queue';
    $contract->{response_demux}{read}{response_scope} = 'burst-last';
    $contract->{response_demux}{read}{last_signal} = 'axi0_rlast';
    $contract->{response_demux}{read}{last_signal_width} = 1;
    return $contract;
...
sub sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data {
    my $contract = sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue-read-data';
    $contract->{read_data} = {
        read => {
            capture_scope     => 'last-beat',
            completion_source => 'response-demux',
...
sub sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat {
    my $contract = sample_contract_with_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_burst_length_runtime_assertion();
    $contract->{intent_name} = 'axi_manager_capacity_status_dynamic_read_burst_last_depth3_same_id_issue_order_queue_read_data_multi_beat';
    $contract->{source}{object_id} = 'axi-manager-capacity-status-dynamic-read-burst-last-depth3-same-id-issue-order-queue-read-data-multi-beat';
    $contract->{read_data} = sample_contract_with_read_data_multi_beat()->{read_data};
...
sub assert_mixed_dynamic_static_write_same_id_issue_order_queue_report {
    ...
    for my $slot (0 .. $#transactions) {
        push @slot_storage,
            (map { { name => "axi0_write_mixed_dynamic_static_same_id_issue_order_slot${slot}_${_}_q", width => 1 } } @transactions),
            { name => "axi0_write_mixed_dynamic_static_same_id_issue_order_slot${slot}_id_q", width => 4 };
    }
    my $expected_first_generated_scope = @$static_transactions == 2
    ...
    is_deeply($write->{generated_completion_signals}, [map { "axi0_${_}_complete" } @transactions], "$owner reports generated completion signals");
    ...
    is($policy->{first_generated_scope}, $expected_first_generated_scope, "$owner reports mixed generated scope");

    is($queue->{depth}, scalar(@transactions), "$owner reports mixed queue depth");
    ...
    is_deeply($queue->{static_transactions}, $static_transactions, "$owner reports mixed queue static transaction");
...
sub assert_mixed_dynamic_static_read_same_id_issue_order_queue_report {
    ...
    my $response_demux_residue = $args{response_demux_residue} // [qw(read_data_interleaving bursts)];
    ...
    is_deeply($demux->{residue}, $response_demux_residue, "$owner removes covered residue from response-demux report");
    ...
    is_deeply($read->{dynamic_transactions}, [qw(r0)], "$owner reports covered dynamic read");
    is_deeply($read->{static_transactions}, [qw(r1)], "$owner reports covered static read");
    is_deeply($read->{mixed_transactions}, { dynamic => 'r0', static => 'r1' }, "$owner reports mixed read transaction roles");
    is($read->{static_id_overlap_policy}, 'allowed_by_issue_order_queue', "$owner allows static/dynamic read overlap by queue order");
    is_deeply($read->{generated_rules}, [qw(axi0_r0_response_demux axi0_r1_response_demux)], "$owner reports generated read response-demux rules");
    is_deeply($read->{generated_completion_signals}, [qw(axi0_r0_complete axi0_r1_complete)], "$owner reports generated read completion signals");
    ...
    ok(!$policy->{generated_scoreboard_behavior}, "$owner does not mark scoreboard behavior");
    ok($policy->{mixed_dynamic_static_issue_order_queue_covered}, "$owner marks mixed read queue coverage");
    is_deeply($policy->{covered_dynamic_transactions}, [qw(r0)], "$owner reports covered dynamic transaction");
    ...
    is_deeply(
        ...
        "$owner reports mixed read queue slot storage",
    );
...
sub assert_dynamic_write_depth3_same_id_issue_order_queue_report {
    ...
    is($demux->{mode}, 'bounded_dynamic_write_bid_issue_order_queue_demux_contract', "$owner marks dynamic write depth-3 issue-order queue demux mode");
    ok($demux->{generated_behavior}, "$owner marks dynamic write depth-3 issue-order queue demux generated");
    is_deeply($demux->{residue}, [qw(read_response_demux read_data_interleaving bursts)], "$owner removes same-ID residue from response-demux report");
    is($write->{transaction_completion_source}, 'generated_dynamic_issue_order_queue_demux', "$owner reports dynamic issue-order queue completion source");
    is_deeply($write->{dynamic_transactions}, [qw(w0 w1 w2)], "$owner reports three covered dynamic writes");
    is_deeply($write->{generated_rules}, [qw(axi0_w0_response_demux axi0_w1_response_demux axi0_w2_response_demux)], "$owner reports generated response-demux rules");
    is_deeply($write->{generated_completion_signals}, [qw(axi0_w0_complete axi0_w1_complete axi0_w2_complete)], "$owner reports generated completion signals");
    is_deeply(
        [map { $_->{id}{implementation_status} } @{$report->{transactions}}],
    ...
    is_deeply($report->{same_id_ordering}{residue}, [], "$owner clears same-ID ordering residue");
    is($policy->{implementation_status}, 'generated_dynamic_write_bid_issue_order_queue', "$owner reports generated dynamic queue policy");
    is($policy->{first_generated_scope}, 'write_bid_three_dynamic_transactions', "$owner reports first generated depth-3 write queue scope");
    is_deeply($policy->{covered_dynamic_transactions}, [qw(w0 w1 w2)], "$owner reports covered dynamic queue transactions");

    is($queue->{depth}, 3, "$owner reports queue depth 3");
    is_deeply($queue->{transactions}, [qw(w0 w1 w2)], "$owner reports queue transaction order");
    is($queue->{request_id_source}, 'axi0_awid', "$owner reports queue request ID capture source");
    assert_dynamic_queue_identity_recapture_report(
        $queue,
        'axi0_awid',
        "$owner dynamic write depth-3 issue-order queue",
    );
    is_deeply(
        $queue->{slot_storage},
        [
    ...
    ok(grep { $_ eq 'axi0_write_dynamic_same_id_issue_order_w0_dequeue_enqueue_w0' } @{$queue->{generated_update_rules}}, "$owner reports one-entry same-transaction ID-refresh rule");
    ok(grep { $_ eq 'axi0_write_dynamic_same_id_issue_order_w2_w1_w0_dequeue_enqueue_w0' } @{$queue->{generated_update_rules}}, "$owner reports depth-3 tail-selected same-transaction ID-refresh rule");
    ok(grep { $_ eq 'axi0_write_dynamic_same_id_issue_order_w0_w1_dequeue_w0_enqueue_w2' } @{$queue->{generated_update_rules}}, "$owner reports disambiguated cross-transaction enqueue rule");
    ok(grep { $_ eq 'axi0_write_dynamic_same_id_issue_order_slot2_onehot0' } @{$queue->{generated_assertions}}, "$owner reports slot2 onehot assertion");
    ok(grep { $_ eq 'axi0_write_dynamic_same_id_issue_order_w2_completion_selected_match' } @{$queue->{generated_assertions}}, "$owner reports w2 completion selected-match assertion");
}
...
sub assert_dynamic_read_same_id_issue_order_queue_report {
    ...
    is($demux->{mode}, 'bounded_dynamic_read_rid_issue_order_queue_demux_contract', "$owner marks dynamic read issue-order queue demux mode");
    ok($demux->{generated_behavior}, "$owner marks dynamic read issue-order queue demux generated");
    ...
    is($read->{transaction_completion_source}, 'generated_dynamic_issue_order_queue_demux', "$owner reports dynamic issue-order queue completion source");
    is($read->{transaction_completion_semantics}, 'earliest_matching_captured_runtime_id', "$owner reports earliest captured runtime-ID matching");
    is($read->{queue_state_representation}, 'compact_runtime_id_issue_order_slots', "$owner reports compact runtime-ID slot representation");
    ...
    ok($report->{same_id_ordering}{generated_behavior}, "$owner marks same-ID ordering generated");
    is_deeply($report->{same_id_ordering}{residue}, [], "$owner clears same-ID ordering residue");
    is($policy->{implementation_status}, 'generated_dynamic_read_rid_issue_order_queue', "$owner reports generated dynamic read queue policy");
    is($policy->{enforcement}, 'generated_dynamic_issue_order_queue', "$owner reports generated dynamic queue enforcement");
    ok($policy->{accepted_same_id_reuse}, "$owner accepts dynamic same-ID reuse through queue");
    ok($policy->{generated_queue_behavior}, "$owner marks generated dynamic queue behavior");
    ...
    is($policy->{first_generated_scope}, 'read_rid_two_dynamic_transactions', "$owner reports first generated read queue scope");
    is($policy->{active_id_uniqueness_policy}, 'not_required_for_issue_order_queue', "$owner does not require active dynamic ID uniqueness");
...
sub assert_dynamic_read_depth3_same_id_issue_order_queue_report {
    ...
    is($demux->{mode}, 'bounded_dynamic_read_rid_issue_order_queue_demux_contract', "$owner marks dynamic read depth-3 issue-order queue demux mode");
    ok($demux->{generated_behavior}, "$owner marks dynamic read depth-3 issue-order queue demux generated");
    is_deeply($demux->{residue}, [qw(read_data_interleaving bursts)], "$owner removes same-ID residue from response-demux report");
    is($read->{response_scope}, 'single_beat', "$owner reports single-beat response scope");
    is($read->{transaction_completion_source}, 'generated_dynamic_issue_order_queue_demux', "$owner reports dynamic issue-order queue completion source");
    is($read->{transaction_completion_semantics}, 'earliest_matching_captured_runtime_id', "$owner reports earliest captured runtime-ID matching");
    ...
    is($read->{runtime_id_queue_key}, 'captured_request_id', "$owner reports captured request ID queue key");
    is($read->{response_demux_strategy}, 'dynamic_issue_order_earliest_matching_slot', "$owner reports earliest matching slot strategy");
    is_deeply($read->{dynamic_transactions}, [qw(r0 r1 r2)], "$owner reports three covered dynamic reads");
    is_deeply($read->{generated_rules}, [qw(axi0_r0_response_demux axi0_r1_response_demux axi0_r2_response_demux)], "$owner reports generated response-demux rules");
    is_deeply($read->{generated_completion_signals}, [qw(axi0_r0_complete axi0_r1_complete axi0_r2_complete)], "$owner reports generated completion signals");
    is_deeply(
        [map { $_->{id}{implementation_status} } @{$report->{transactions}}],
        [qw(generated_issue_order_queue_matching generated_issue_order_queue_matching generated_issue_order_queue_matching)],
        "$owner reports generated dynamic queue matching ownership",
    ...
    ok($policy->{generated_queue_behavior}, "$owner marks generated dynamic queue behavior");
    ok($policy->{dynamic_issue_order_queue_covered}, "$owner marks dynamic issue-order queue coverage");
    is_deeply($policy->{covered_dynamic_transactions}, [qw(r0 r1 r2)], "$owner reports covered dynamic queue transactions");
    is($policy->{first_generated_scope}, 'read_rid_three_dynamic_transactions', "$owner reports first generated depth-3 read queue scope");
    is($policy->{active_id_uniqueness_policy}, 'not_required_for_issue_order_queue', "$owner does not require active dynamic ID uniqueness");

    is($queue->{depth}, 3, "$owner reports queue depth 3");
    is($queue->{queue_state_representation}, 'compact_runtime_id_issue_order_slots', "$owner reports queue representation");
    is_deeply($queue->{transactions}, [qw(r0 r1 r2)], "$owner reports queue transaction order");
    is($queue->{request_id_source}, 'axi0_arid', "$owner reports queue request ID capture source");
    ...
    assert_dynamic_queue_identity_recapture_report(
        $queue,
        'axi0_arid',
        "$owner dynamic read depth-3 issue-order queue",
    );
    is_deeply(
        $queue->{slot_storage},
        [
...
sub assert_dynamic_read_burst_last_same_id_issue_order_queue_report {
    ...
    is($read->{last_signal_width}, 1, "$owner reports one-bit RLAST");
    is($read->{last_signal_direction}, 'generated_input', "$owner reports RLAST as generated input");
    is($read->{transaction_completion_source}, 'generated_dynamic_issue_order_queue_demux_last_beat', "$owner reports dynamic issue-order queue last-beat completion source");
    is($read->{transaction_completion_semantics}, 'earliest_matching_captured_runtime_id_and_last_signal', "$owner reports earliest captured runtime-ID plus RLAST semantics");
    is($read->{beat_valid_output}, 'none', "$owner reports no beat-valid output");
    is($read->{burst_length_source}, 'rlast_only', "$owner reports RLAST-only burst length source");
    ...
    ok($report->{same_id_ordering}{generated_behavior}, "$owner marks same-ID ordering generated");
    is_deeply($report->{same_id_ordering}{residue}, [], "$owner clears same-ID ordering residue");
    is($policy->{implementation_status}, 'generated_dynamic_read_rid_rlast_issue_order_queue', "$owner reports generated dynamic read RID/RLAST queue policy");
    is($policy->{enforcement}, 'generated_dynamic_issue_order_queue', "$owner reports generated dynamic queue enforcement");
    ok($policy->{accepted_same_id_reuse}, "$owner accepts dynamic same-ID reuse through queue");
    ok($policy->{generated_queue_behavior}, "$owner marks generated dynamic queue behavior");
    ok($policy->{dynamic_issue_order_queue_covered}, "$owner marks dynamic issue-order queue coverage");
...
sub assert_mixed_dynamic_static_read_response_demux_report {
    ...
    is_deeply(
        ...
        "$owner reports static read capture and recapture state",
    );
    is_deeply(
        $demux->{residue},
        $expected_residue,
...
sub assert_mixed_dynamic_static_read_rlast_response_demux_report {
    ...
    is_deeply(
        ...
        "$owner reports static read RLAST capture and recapture state",
    );
    is_deeply(
        $demux->{residue},
        $expected_residue,
