...
(actor aggregate_condition_still_deferred
  ...
  (transaction main
    ...
    (when frame.flag
      (set done 1))))
ISF
        qr/when condition references aggregate storage path 'frame\.flag'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, or rule assignment RHS scalar values or operands/,
