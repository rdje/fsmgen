...
my $standalone_dtc_list_generic_map_path = File::Spec->catfile($tempdir, 'vhdl_standalone_dtc_list_generic_map_top.fsm');
...
my $apb_c4_aggregate_generic_map_path = File::Spec->catfile($apb_c4_aggregate_generic_map_dir, 'apb_c4_aggregate_generic_map_top.fsm');
...
write_file(
    $standalone_dtc_list_generic_map_path,
    <<'FSM'
(?top:vhdl_standalone_dtc_list_generic_map_top
  (?ports:public_io
    clk
    rst_n
    data_in<16
...
write_file(
    $standalone_dtc_map_generic_map_path,
    <<'FSM'
(?top:vhdl_standalone_dtc_map_generic_map_top
  (?ports:public_io
    clk
    rst_n
    data_in<16
...
my $standalone_dtc_list_generic_map_result = $pipeline->generate_hdl_from_file($standalone_dtc_list_generic_map_path);
like(
    $standalone_dtc_list_generic_map_result->{hdl_code},
    ...
    'pipeline emits the standalone-DT child packed-list generic declaration',
);
like(
    $standalone_dtc_list_generic_map_result->{hdl_code},
    qr/\brouter\s+:\s+entity\s+work\.standalone_route_src\s+generic\s+map\s*\(\s*LANES\s+=>\s+"1010010100111100"\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits standalone-DT packed-list generic maps before the child port map',
);
unlike(
    $standalone_dtc_list_generic_map_result->{hdl_code},
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.LANES\s*\(|16'b|8'h/s,
    'pipeline standalone-DT packed-list generic-map VHDL output does not leak SystemVerilog generic syntax or packed literals',
);

my $standalone_dtc_map_generic_map_result = $pipeline->generate_hdl_from_file($standalone_dtc_map_generic_map_path);
like(
    ...
    'pipeline emits the standalone-DT child packed-map generic declaration',
);
like(
    $standalone_dtc_map_generic_map_result->{hdl_code},
    qr/\brouter\s+:\s+entity\s+work\.standalone_route_src\s+generic\s+map\s*\(\s*FRAME\s+=>\s+"101"\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits standalone-DT packed-map generic maps before the child port map',
);
unlike(
    $standalone_dtc_map_generic_map_result->{hdl_code},
...
my $apb_c4_scalar_generic_map_result = $pipeline->generate_hdl_from_file($apb_c4_scalar_generic_map_path);
like(
    $apb_c4_scalar_generic_map_result->{hdl_code},
    qr/\bentity\s+apb_requester\s+is\s+generic\s*\(\s*TIMEOUT_CYCLES\s+:\s+integer\s*:=\s*4\s*\);\s+port\s*\(/s,
    'pipeline emits the APB requester scalar generic declaration',
);
like(
    $apb_c4_scalar_generic_map_result->{hdl_code},
    qr/\bentity\s+apb_completer\s+is\s+generic\s*\(\s*TIMEOUT_CYCLES\s+:\s+integer\s*:=\s*4\s*\);\s+port\s*\(/s,
    'pipeline emits the APB completer scalar generic declaration',
);
like(
    $apb_c4_scalar_generic_map_result->{hdl_code},
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+8\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits APB requester scalar generic maps before the port map',
);
like(
    $apb_c4_scalar_generic_map_result->{hdl_code},
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+6\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits APB completer scalar generic maps before the port map',
);
unlike(
    $apb_c4_scalar_generic_map_result->{hdl_code},
...
my $apb_c4_one_bit_generic_map_result = $pipeline->generate_hdl_from_file($apb_c4_one_bit_generic_map_path);
like(
    $apb_c4_one_bit_generic_map_result->{hdl_code},
    qr/\bentity\s+apb_requester\s+is\s+generic\s*\(\s*ENABLE_DEFAULT\s+:\s+std_logic\s*:=\s*'0'\s*\);\s+port\s*\(/s,
    'pipeline emits the APB requester one-bit generic declaration',
);
like(
    $apb_c4_one_bit_generic_map_result->{hdl_code},
    qr/\bentity\s+apb_completer\s+is\s+generic\s*\(\s*ENABLE_DEFAULT\s+:\s+std_logic\s*:=\s*'0'\s*\);\s+port\s*\(/s,
    'pipeline emits the APB completer one-bit generic declaration',
);
like(
    $apb_c4_one_bit_generic_map_result->{hdl_code},
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*ENABLE_DEFAULT\s+=>\s+'1'\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits APB requester one-bit generic maps before the port map',
);
like(
    $apb_c4_one_bit_generic_map_result->{hdl_code},
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*ENABLE_DEFAULT\s+=>\s+'1'\s*\)\s+port\s+map\s*\(/s,
    'pipeline emits APB completer one-bit generic maps before the port map',
);
unlike(
    $apb_c4_one_bit_generic_map_result->{hdl_code},
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.ENABLE_DEFAULT\s*\(|1'b/s,
...
my $apb_c4_aggregate_generic_map_result = $pipeline->generate_hdl_from_file($apb_c4_aggregate_generic_map_path);
like(
    $apb_c4_aggregate_generic_map_result->{hdl_code},
    qr/\bLANES\s+:\s+std_logic_vector\(15\s+downto\s+0\)\s*:=\s*"1010010100111100"/s,
    'pipeline emits the APB packed-list generic declaration',
);
like(
    $apb_c4_aggregate_generic_map_result->{hdl_code},
    qr/\bFRAME\s+:\s+std_logic_vector\(2\s+downto\s+0\)\s*:=\s*"000"/s,
    'pipeline emits the APB packed-map generic declaration',
);
like(
    $apb_c4_aggregate_generic_map_result->{hdl_code},
    ...
    'pipeline emits APB requester aggregate generic maps before the port map',
);
like(
    $apb_c4_aggregate_generic_map_result->{hdl_code},
    ...
    'pipeline emits APB completer aggregate generic maps before the port map',
);
unlike(
    $apb_c4_aggregate_generic_map_result->{hdl_code},
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.LANES\s*\(|\.FRAME\s*\(|16'b|3'b|8'h|2'b/s,
    'pipeline APB/C4 aggregate generic-map VHDL output does not leak SystemVerilog generic syntax or packed literals',
);

my $apb_c4_package_generic_map_result = $pipeline->generate_hdl_from_file($apb_c4_package_generic_map_path);
like(
    $apb_c4_package_generic_map_result->{hdl_code},
    qr/\bTIMEOUT_CYCLES\s+:\s+integer\s*:=\s*4\b/s,
    'pipeline emits the APB package-backed scalar generic declaration',
);
like(
    $apb_c4_package_generic_map_result->{hdl_code},
    qr/\bRESET_VALUE\s+:\s+std_logic_vector\(7\s+downto\s+0\)\s*:=\s*"00000000"/s,
    'pipeline emits the APB package-backed multi-bit generic declaration',
);
like(
    $apb_c4_package_generic_map_result->{hdl_code},
    ...
    'pipeline emits APB requester package-backed generic maps before the port map',
);
like(
    $apb_c4_package_generic_map_result->{hdl_code},
    ...
    'pipeline emits APB completer package-backed generic maps before the port map',
);
unlike(
    $apb_c4_package_generic_map_result->{hdl_code},
...
my ($standalone_dtc_list_generic_map_success, $standalone_dtc_list_generic_map_error_message, $standalone_dtc_list_generic_map_full_buf, $standalone_dtc_list_generic_map_stdout_buf, $standalone_dtc_list_generic_map_stderr_buf) = run(
    command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $standalone_dtc_list_generic_map_output_path, $standalone_dtc_list_generic_map_path],
);

my $standalone_dtc_list_generic_map_combined_output = join(
    '',
    @{ $standalone_dtc_list_generic_map_stdout_buf || [] },
    @{ $standalone_dtc_list_generic_map_stderr_buf || [] },
    ($standalone_dtc_list_generic_map_error_message || ''),
);

ok($standalone_dtc_list_generic_map_success, 'CLI accepts standalone-DT packed-list generic maps for VHDL')
    or diag($standalone_dtc_list_generic_map_combined_output);
ok(-e $standalone_dtc_list_generic_map_output_path, 'CLI writes standalone-DT packed-list generic-map VHDL output');

my $standalone_dtc_list_generic_map_cli_hdl = read_file($standalone_dtc_list_generic_map_output_path);
like(
    $standalone_dtc_list_generic_map_cli_hdl,
    qr/\bLANES\s+:\s+std_logic_vector\(15\s+downto\s+0\)\s*:=\s*"0000000000000000"/s,
    'CLI standalone-DT packed-list generic-map output includes the child generic declaration',
);
like(
    $standalone_dtc_list_generic_map_cli_hdl,
    qr/\brouter\s+:\s+entity\s+work\.standalone_route_src\s+generic\s+map\s*\(\s*LANES\s+=>\s+"1010010100111100"\s*\)\s+port\s+map\s*\(/s,
    'CLI standalone-DT packed-list generic-map output includes the child generic map',
);
unlike(
    $standalone_dtc_list_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.LANES\s*\(|16'b|8'h/s,
    'CLI standalone-DT packed-list generic-map VHDL output does not leak SystemVerilog generic syntax or packed literals',
);

my ($standalone_dtc_map_generic_map_success, $standalone_dtc_map_generic_map_error_message, $standalone_dtc_map_generic_map_full_buf, $standalone_dtc_map_generic_map_stdout_buf, $standalone_dtc_map_generic_map_stderr_buf) = run(
    command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $standalone_dtc_map_generic_map_output_path, $standalone_dtc_map_generic_map_path],
);

my $standalone_dtc_map_generic_map_combined_output = join(
    '',
    @{ $standalone_dtc_map_generic_map_stdout_buf || [] },
    @{ $standalone_dtc_map_generic_map_stderr_buf || [] },
    ($standalone_dtc_map_generic_map_error_message || ''),
);

ok($standalone_dtc_map_generic_map_success, 'CLI accepts standalone-DT packed-map generic maps for VHDL')
    or diag($standalone_dtc_map_generic_map_combined_output);
ok(-e $standalone_dtc_map_generic_map_output_path, 'CLI writes standalone-DT packed-map generic-map VHDL output');

my $standalone_dtc_map_generic_map_cli_hdl = read_file($standalone_dtc_map_generic_map_output_path);
like(
    $standalone_dtc_map_generic_map_cli_hdl,
    qr/\bFRAME\s+:\s+std_logic_vector\(2\s+downto\s+0\)\s*:=\s*"000"/s,
    'CLI standalone-DT packed-map generic-map output includes the child generic declaration',
);
like(
    $standalone_dtc_map_generic_map_cli_hdl,
    qr/\brouter\s+:\s+entity\s+work\.standalone_route_src\s+generic\s+map\s*\(\s*FRAME\s+=>\s+"101"\s*\)\s+port\s+map\s*\(/s,
    'CLI standalone-DT packed-map generic-map output includes the child generic map',
);
unlike(
    $standalone_dtc_map_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.FRAME\s*\(|3'b|2'b/s,
...
my ($apb_c4_scalar_generic_map_success, $apb_c4_scalar_generic_map_error_message, $apb_c4_scalar_generic_map_full_buf, $apb_c4_scalar_generic_map_stdout_buf, $apb_c4_scalar_generic_map_stderr_buf) = run(
    command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $apb_c4_scalar_generic_map_output_path, $apb_c4_scalar_generic_map_path],
);

my $apb_c4_scalar_generic_map_combined_output = join(
    '',
    @{ $apb_c4_scalar_generic_map_stdout_buf || [] },
    @{ $apb_c4_scalar_generic_map_stderr_buf || [] },
    ($apb_c4_scalar_generic_map_error_message || ''),
);

ok($apb_c4_scalar_generic_map_success, 'CLI accepts bounded APB/C4 scalar generic maps for VHDL')
    or diag($apb_c4_scalar_generic_map_combined_output);
ok(-e $apb_c4_scalar_generic_map_output_path, 'CLI writes APB/C4 scalar generic-map VHDL output');

my $apb_c4_scalar_generic_map_cli_hdl = read_file($apb_c4_scalar_generic_map_output_path);
like(
    $apb_c4_scalar_generic_map_cli_hdl,
    qr/\bTIMEOUT_CYCLES\s+:\s+integer\s*:=\s*4/s,
    'CLI APB/C4 scalar generic-map output includes child generic declarations',
);
like(
    $apb_c4_scalar_generic_map_cli_hdl,
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+8\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 scalar generic-map output includes the requester generic map',
);
like(
    $apb_c4_scalar_generic_map_cli_hdl,
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+6\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 scalar generic-map output includes the completer generic map',
);
unlike(
    $apb_c4_scalar_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.TIMEOUT_CYCLES\s*\(/s,
...
my ($apb_c4_one_bit_generic_map_success, $apb_c4_one_bit_generic_map_error_message, $apb_c4_one_bit_generic_map_full_buf, $apb_c4_one_bit_generic_map_stdout_buf, $apb_c4_one_bit_generic_map_stderr_buf) = run(
    command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $apb_c4_one_bit_generic_map_output_path, $apb_c4_one_bit_generic_map_path],
);

my $apb_c4_one_bit_generic_map_combined_output = join(
    '',
    @{ $apb_c4_one_bit_generic_map_stdout_buf || [] },
    @{ $apb_c4_one_bit_generic_map_stderr_buf || [] },
    ($apb_c4_one_bit_generic_map_error_message || ''),
);

ok($apb_c4_one_bit_generic_map_success, 'CLI accepts bounded APB/C4 one-bit generic maps for VHDL')
    or diag($apb_c4_one_bit_generic_map_combined_output);
ok(-e $apb_c4_one_bit_generic_map_output_path, 'CLI writes APB/C4 one-bit generic-map VHDL output');

my $apb_c4_one_bit_generic_map_cli_hdl = read_file($apb_c4_one_bit_generic_map_output_path);
like(
    $apb_c4_one_bit_generic_map_cli_hdl,
    qr/\bENABLE_DEFAULT\s+:\s+std_logic\s*:=\s*'0'/s,
    'CLI APB/C4 one-bit generic-map output includes child generic declarations',
);
like(
    $apb_c4_one_bit_generic_map_cli_hdl,
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*ENABLE_DEFAULT\s+=>\s+'1'\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 one-bit generic-map output includes the requester generic map',
);
like(
    $apb_c4_one_bit_generic_map_cli_hdl,
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*ENABLE_DEFAULT\s+=>\s+'1'\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 one-bit generic-map output includes the completer generic map',
);
unlike(
    $apb_c4_one_bit_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.ENABLE_DEFAULT\s*\(|1'b/s,
...
my ($apb_c4_aggregate_generic_map_success, $apb_c4_aggregate_generic_map_error_message, $apb_c4_aggregate_generic_map_full_buf, $apb_c4_aggregate_generic_map_stdout_buf, $apb_c4_aggregate_generic_map_stderr_buf) = run(
    command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $apb_c4_aggregate_generic_map_output_path, $apb_c4_aggregate_generic_map_path],
);

my $apb_c4_aggregate_generic_map_combined_output = join(
    '',
    @{ $apb_c4_aggregate_generic_map_stdout_buf || [] },
    @{ $apb_c4_aggregate_generic_map_stderr_buf || [] },
    ($apb_c4_aggregate_generic_map_error_message || ''),
);

ok($apb_c4_aggregate_generic_map_success, 'CLI accepts bounded APB/C4 aggregate generic maps for VHDL')
    or diag($apb_c4_aggregate_generic_map_combined_output);
ok(-e $apb_c4_aggregate_generic_map_output_path, 'CLI writes APB/C4 aggregate generic-map VHDL output');

my $apb_c4_aggregate_generic_map_cli_hdl = read_file($apb_c4_aggregate_generic_map_output_path);
like(
    $apb_c4_aggregate_generic_map_cli_hdl,
    qr/\bLANES\s+:\s+std_logic_vector\(15\s+downto\s+0\)\s*:=\s*"1010010100111100"/s,
    'CLI APB/C4 aggregate generic-map output includes the packed-list generic declaration',
);
like(
    $apb_c4_aggregate_generic_map_cli_hdl,
    qr/\bFRAME\s+:\s+std_logic_vector\(2\s+downto\s+0\)\s*:=\s*"000"/s,
    'CLI APB/C4 aggregate generic-map output includes the packed-map generic declaration',
);
like(
    $apb_c4_aggregate_generic_map_cli_hdl,
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*LANES\s+=>\s+"0011110010100101",\s*FRAME\s+=>\s+"101"\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 aggregate generic-map output includes the requester generic map',
);
like(
    $apb_c4_aggregate_generic_map_cli_hdl,
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*LANES\s+=>\s+"0011110010100101",\s*FRAME\s+=>\s+"101"\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 aggregate generic-map output includes the completer generic map',
);
unlike(
    $apb_c4_aggregate_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.LANES\s*\(|\.FRAME\s*\(|16'b|3'b|8'h|2'b/s,
...
my $apb_c4_package_generic_map_combined_output = join(
    '',
    @{ $apb_c4_package_generic_map_stdout_buf || [] },
    @{ $apb_c4_package_generic_map_stderr_buf || [] },
    ($apb_c4_package_generic_map_error_message || ''),
);

ok($apb_c4_package_generic_map_success, 'CLI accepts bounded APB/C4 package-backed generic maps for VHDL')
    or diag($apb_c4_package_generic_map_combined_output);
ok(-e $apb_c4_package_generic_map_output_path, 'CLI writes APB/C4 package-backed generic-map VHDL output');

my $apb_c4_package_generic_map_cli_hdl = read_file($apb_c4_package_generic_map_output_path);
like(
    $apb_c4_package_generic_map_cli_hdl,
    qr/\bTIMEOUT_CYCLES\s+:\s+integer\s*:=\s*4\b/s,
    'CLI APB/C4 package-backed generic-map output includes the scalar generic declaration',
);
like(
    $apb_c4_package_generic_map_cli_hdl,
    qr/\bRESET_VALUE\s+:\s+std_logic_vector\(7\s+downto\s+0\)\s*:=\s*"00000000"/s,
    'CLI APB/C4 package-backed generic-map output includes the multi-bit generic declaration',
);
like(
    $apb_c4_package_generic_map_cli_hdl,
    qr/\brequester\s+:\s+entity\s+work\.apb_requester\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+8,\s*RESET_VALUE\s+=>\s+"10100101"\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 package-backed generic-map output includes the requester generic map',
);
like(
    $apb_c4_package_generic_map_cli_hdl,
    qr/\bcompleter\s+:\s+entity\s+work\.apb_completer\s+generic\s+map\s*\(\s*TIMEOUT_CYCLES\s+=>\s+8,\s*RESET_VALUE\s+=>\s+"10100101"\s*\)\s+port\s+map\s*\(/s,
    'CLI APB/C4 package-backed generic-map output includes the completer generic map',
);
unlike(
    $apb_c4_package_generic_map_cli_hdl,
    qr/\bmodule\b|\bassign\b|\bendmodule\b|\balways_(?:ff|comb)\b|\#\s*\(|\.TIMEOUT_CYCLES\s*\(|\.RESET_VALUE\s*\(|8'hA5|\bparam_pkg\b/s,
    'CLI APB/C4 package-backed generic-map VHDL output does not leak SystemVerilog generic syntax, raw literals, or package tokens',
);
