...
my $verifier = File::Spec->catfile(
    $repo, 'scripts', 'check_retired_achievement_history.pl');
...
subtest 'exact retired object passes with no live consumer' => sub {
    ...
    like($output,
        qr/exact .*:LIVE_ACHIEVEMENT_STATUS\.md recovery and zero live consumers verified/,
...
subtest 'recreated path and planted active consumer fail independently' => sub {
    ...
    like($consumer_output,
        qr/active policy consumer still names retired LIVE_ACHIEVEMENT_STATUS\.md: AGENTS\.md/,
...
subtest 'whole-document token sweep detects planted orphan exactly' => sub {
    my $source = "# LIVE ACHIEVEMENTS\n## UNIT.1\n- completed `path/to/file.pm`\n";
    my $before = tokens($source);
    my $probe = 'fsmgen_orphan_probe_retired_achievement_history';
...
sub make_fixture {
    my $root = create_project_tempdir(
        purpose => 'retired-achievement-history-tests');
    run_git($root, 'init');
    run_git($root, 'config', 'user.name', 'Fixture');
    run_git($root, 'config', 'user.email', 'fixture@example.invalid');
    my $source = "# LIVE ACHIEVEMENTS\n## UNIT.1\n- exact history\n";
    write_file($root, 'LIVE_ACHIEVEMENT_STATUS.md', $source);
    ...
    unlink File::Spec->catfile($root, 'LIVE_ACHIEVEMENT_STATUS.md')
        or die "cannot remove fixture LIVE_ACHIEVEMENT_STATUS.md: $!";
    run_git($root, 'add', '-u');
    return {
        root => $root,
        revision => $revision,
        ...
        lines => 3,
        bytes => length($source),
        longest => length('# LIVE ACHIEVEMENTS'),
    };
}
