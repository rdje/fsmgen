...
subtest 'source is the exact selected two-subordinate exact-four paired BUSY transform' => sub {
    ...
    is(slurp(sample_path()), frozen_expected_source(), 'sample differs from the shipped two-subordinate exact-three paired source only by the frozen identity/requester/cardinality delta');
    ...
    is_deeply(
        ...
        [qw(amba_requester_busy_insert_four.isf ahb_status_subordinate_byte_lane_hburst_seq.isf ahb_control_subordinate_byte_lane_hburst_seq.isf ahb_interconnect.isf)],
    ...
    is($requester->{object_name}, 'amba_requester_busy_insert_four', 'requester child selects shipped exact-four BUSY requester');
    ...
    is($requester->{transfer}{busy_beats}, 4, 'requester child transfer reports exact-four BUSY cardinality');
    is($requester->{busy_insertion}{beats}, 4, 'requester child reports numeric exact-four BUSY cardinality');
...
subtest 'unsupported neighboring source identity stays explicit and unmatched' => sub {
    my $tempdir = project_tempdir();
    my $probe = File::Spec->catfile($tempdir, 'two_subordinate_exact_four_unmatched.ppif');
    write_file($probe, slurp(sample_path()));
    my $check = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', $probe);
    ok($check->{success}, 'unsupported neighboring source still syntax-checks and lowers');
    ok(!$check->{support_accounting}{matched}, 'unsupported neighboring source is explicitly unmatched');
    is(scalar(@{$check->{diagnostics} || []}), 0, 'unmatched successful source emits no false diagnostic');
};

subtest 'strict CLI, semantic, MCP, review-artifact, and HDL surfaces agree' => sub {
    ...
    is($semantic->{support_accounting}{source_kind}, 'ppif', 'semantic JSON reports generic PPIF source kind');
    undef $semantic;
    ...
    is($mcp->{source_id}, sample_relpath(), 'MCP keeps repo-relative selected source identity');
    ...
    is($mcp->{report}{generation_result_snapshot}{summary}{module_name}, 'ahb_tb', 'MCP semantic report exposes ahb_tb');
    is($mcp->{report}{generation_result_snapshot}{summary}{source_root_kind}, 'top', 'MCP semantic report exposes top root');
    is($mcp->{report}{generation_result_snapshot}{summary}{composition_child_count}, 4, 'MCP semantic report exposes four children');
    ...
    my $requester_isf = slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_four.isf'));
    like($requester_isf, qr/\(drive transfer_busy\b/, 'requester IAL1 keeps BUSY drive');
    like($requester_isf, qr/\(var ahb_busy_remaining_q \(width 3\) \(reset 0\)\)/, 'requester IAL1 keeps exact-four actor-owned counter');
...
subtest 'generated HDL pairs exact-four requester BUSY with both subordinate windows' => sub {
    my $tempdir = project_tempdir();
    my $hdl = File::Spec->catfile($tempdir, 'ahb_tb.sv');
    ...
    my ($compile_ok, undef, undef, $compile_stdout, $compile_stderr) = run(
        command => [
            ...
            '-j', '1', '--top-module', 'ahb_two_subordinate_exact_four_paired_busy_composition_tb',
            '--Mdir', $objdir, $hdl, testbench_path(),
        ],
    );
    ok($compile_ok, 'Verilator builds the assertion-enabled two-subordinate exact-four paired BUSY harness')
        or diag(join('', @{$compile_stdout || []}), join('', @{$compile_stderr || []}));
    return unless $compile_ok;

    my $binary = File::Spec->catfile($objdir, 'Vahb_two_subordinate_exact_four_paired_busy_composition_tb');
    my ($run_ok, undef, undef, $run_stdout, $run_stderr) = run(command => [$binary]);
    ok($run_ok, 'generated-HDL two-window exact-four paired BUSY proof passes with assertions enabled')
        or diag(join('', @{$run_stdout || []}), join('', @{$run_stderr || []}));
    like(
        join('', @{$run_stdout || []}),
        qr/PASS commands=2 transfers=10 beats=8 busy=2 qualified_busy=8 resumed_seq=2 status=44332211 control=88776655/,
        'runtime proves four qualified BUSY events and one resumed SEQ per command across both windows',
    );
};
...
sub sample_path {
    return File::Spec->catfile($repo_root, split m{/}, sample_relpath());
}
...
sub predecessor_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ppif');
}

sub testbench_path {
    return File::Spec->catfile($FindBin::Bin, 'data', 'ahb_two_subordinate_exact_four_paired_busy_composition_tb.svt');
}

sub source_object_id {
    return 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-four-byte-lane-hburst-seq-busy-park';
}

sub source_intent {
    return 'ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park';
}

sub support_id {
    return 'intent.ppif_ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ppif_ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park_pipeline_cli';
}

sub frozen_expected_source {
    my $source = slurp(predecessor_path());
    replace_once(\$source, '(protocol-platform-intent ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', '(protocol-platform-intent ' . source_intent());
    replace_once(\$source, '(object fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-three-byte-lane-hburst-seq-busy-park)', '(object ' . source_object_id() . ')');
    replace_once(\$source, '(section bounded-ahb-interconnect-two-subordinate-requester-busy-insert-three-byte-lane-hburst-seq-busy-park)', '(section bounded-ahb-interconnect-two-subordinate-requester-busy-insert-four-byte-lane-hburst-seq-busy-park)');
    replace_once(\$source, '(ahb-requester amba_requester_busy_insert_three', '(ahb-requester amba_requester_busy_insert_four');
    replace_once(\$source, '(busy-beats 3)', '(busy-beats 4)');
    replace_once(\$source, '(requester requester amba_requester_busy_insert_three)', '(requester requester amba_requester_busy_insert_four)');
    return $source;
}
...
sub project_tempdir {
    ...
    make_path($temp_root);
    return tempdir('t1539-XXXXXX', DIR => $temp_root, CLEANUP => 1);
}
