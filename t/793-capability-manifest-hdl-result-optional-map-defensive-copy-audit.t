...
subtest 'manifest-embedded HDLGenerator optional composition key-family map rebuild cleanly after caller mutation' => sub {
    my $first = build_capability_manifest();
    my $mutated = $first->{embedding}{hdl_generator_result};

    mutate_structure($mutated->{'optional_composition_key_family_map'});
    ...
    is_deeply($contract->{'optional_composition_key_family_map'}, $expected->{'optional_composition_key_family_map'}, 'fresh manifest result contract rebuilds clean optional_composition_key_family_map');
