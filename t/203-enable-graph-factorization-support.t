...
subtest 'enable-graph factorization support rebuilds the factorization-analysis contract from a prepared backend context' => sub {
...
FSM
    ...
    is(ref($counts), 'HASH', 'factorization support returns a logical-operation count map for the prepared backend context');

    my @ast_expressions = $support->collect_all_wen_en_ast_expressions();
    cmp_ok(scalar(@ast_expressions), '>', 0, 'factorization support collects the prepared enable and assignment AST expressions');

    my ($intermediate_backed_expression) = grep {
        my $sv = eval { $_->{ast}->to_systemverilog() } || '';
        $sv =~ /intermediate_or_A_B_/;
    } @ast_expressions;
    ok($intermediate_backed_expression, 'factorization support exposes the prepared AST set through its current intermediate-backed owner expressions');
    ok($support->ast_contains_intermediate_signals($intermediate_backed_expression->{ast}), 'factorization support recognizes intermediate-backed expressions in the prepared AST set');
    ...
    cmp_ok($fed_count, '>', 0, 'factorization support feeds the prepared AST set into a generic factorizer');
    ...
    cmp_ok($second_pass_fed_count, '>', 0, 'factorization support exposes post-substitution ASTs that still depend on intermediates to the second-pass factorizer');
};
