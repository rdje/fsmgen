...
subtest 'malformed shift clauses fail before scheduled emission' => sub {
    assert_lower_rejected(<<'ISF', 'missing shift_left bit', qr/\ATransaction 'main': shift_left requires '\(shift_left reg bit \[\(width N\|TX_PARAM\|PARAM\|CONST\|PACKAGE\.CONSTANT\)\]\)' in transaction body/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'nested shift_left register', qr/\ATransaction 'main': shift_left requires '\(shift_left reg bit \[\(width N\|TX_PARAM\|PARAM\|CONST\|PACKAGE\.CONSTANT\)\]\)' in transaction body/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'malformed shift_left width option', qr/\Ashift_left optional arguments must be '\(width N\|TX_PARAM\|PARAM\|CONST\|PACKAGE\.CONSTANT\)'/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'missing shift_right bit', qr/\ATransaction 'main': shift_right requires '\(shift_right reg bit \[\(width N\|TX_PARAM\|PARAM\|CONST\|PACKAGE\.CONSTANT\)\]\)' in transaction body/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'nested shift_right bit', qr/\ATransaction 'main': shift_right requires '\(shift_right reg bit \[\(width N\|TX_PARAM\|PARAM\|CONST\|PACKAGE\.CONSTANT\)\]\)' in transaction body/);
