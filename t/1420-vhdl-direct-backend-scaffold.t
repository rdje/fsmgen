...
subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal literal-first multiplication RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_mul.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_mul.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_first_mul
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s, 'vector positive literal-first multiplication fixture lowers non-signed input port A');
    like($hdl, qr/\bsignal\s+PROD\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal-first multiplication fixture lowers non-signed output signal');
    like($hdl, qr/\bPROD\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\*\s+unsigned\(A\),\s+8\)\);/s, 'non-signed vector positive literal-first multiplication lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\(2,\s+8\)|arithmetic expression '2 \* A'/s, 'non-signed vector positive literal-first multiplication avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal-first multiplication')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal-first multiplication VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bPROD\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\*\s+unsigned\(A\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal-first multiplication assignment');
...
subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal literal-literal division RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_pair_div.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_pair_div.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_pair_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bsignal\s+QUOT\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal-literal division fixture lowers non-signed output signal');
    like($hdl, qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+to_unsigned\(3,\s+8\),\s+8\)\);/s, 'non-signed vector positive literal-literal division lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\([23],\s+8\)|arithmetic expression '2 \/ 3'/s, 'non-signed vector positive literal-literal division avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal-literal division')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal-literal division VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+to_unsigned\(3,\s+8\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal-literal division assignment');
    unlike($cli_hdl, qr/to_signed\([23],\s+8\)|arithmetic expression '2 \/ 3'/s, 'CLI non-signed vector positive literal-literal division avoids signed casts and the prior arithmetic guard');
};

subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal literal-literal modulo RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_pair_mod.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_pair_mod.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_pair_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bsignal\s+REM\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal-literal modulo fixture lowers non-signed output signal');
    like($hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+to_unsigned\(3,\s+8\),\s+8\)\);/s, 'non-signed vector positive literal-literal modulo lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\([23],\s+8\)|arithmetic expression '2 % 3'/s, 'non-signed vector positive literal-literal modulo avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal-literal modulo')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal-literal modulo VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+to_unsigned\(3,\s+8\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal-literal modulo assignment');
    unlike($cli_hdl, qr/to_signed\([23],\s+8\)|arithmetic expression '2 % 3'/s, 'CLI non-signed vector positive literal-literal modulo avoids signed casts and the prior arithmetic guard');
};
...
subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal literal-first division RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_div.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_div.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_first_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s, 'vector positive literal-first division fixture lowers non-signed input port A');
    like($hdl, qr/\bsignal\s+QUOT\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal-first division fixture lowers non-signed output signal');
    like($hdl, qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+unsigned\(A\),\s+8\)\);/s, 'non-signed vector positive literal-first division lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\(2,\s+8\)|arithmetic expression '2 \/ A'/s, 'non-signed vector positive literal-first division avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal-first division')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal-first division VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+unsigned\(A\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal-first division assignment');
    unlike($cli_hdl, qr/to_signed\(2,\s+8\)|arithmetic expression '2 \/ A'/s, 'CLI non-signed vector positive literal-first division avoids signed casts and the prior arithmetic guard');
};
...
subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal modulo RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_mod.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_mod.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s, 'vector positive literal modulo fixture lowers non-signed input port A');
    like($hdl, qr/\bsignal\s+REM\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal modulo fixture lowers non-signed output signal');
    like($hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+mod\s+to_unsigned\(2,\s+8\),\s+8\)\);/s, 'non-signed vector positive literal modulo lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\(2,\s+8\)|arithmetic expression 'A % 2'/s, 'non-signed vector positive literal modulo avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal modulo')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal modulo VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+mod\s+to_unsigned\(2,\s+8\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal modulo assignment');
    unlike($cli_hdl, qr/to_signed\(2,\s+8\)|arithmetic expression 'A % 2'/s, 'CLI non-signed vector positive literal modulo avoids signed casts and the prior arithmetic guard');
};

subtest 'direct VHDL scaffold lowers non-signed vector positive numeric-literal literal-first modulo RHS shape' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_mod.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'direct_vhdl_vector_positive_literal_first_mod.vhd');
    write_file(
        $fsm_path,
        <<'FSM'
(?fsm:direct_vhdl_vector_positive_literal_first_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    my $hdl = generate_vhdl($fsm_path);
    like($hdl, qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s, 'vector positive literal-first modulo fixture lowers non-signed input port A');
    like($hdl, qr/\bsignal\s+REM\s+:\s+std_logic_vector\(7\s+downto\s+0\);/s, 'vector positive literal-first modulo fixture lowers non-signed output signal');
    like($hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+unsigned\(A\),\s+8\)\);/s, 'non-signed vector positive literal-first modulo lowers through resized to_unsigned arithmetic');
    unlike($hdl, qr/to_signed\(2,\s+8\)|arithmetic expression '2 % A'/s, 'non-signed vector positive literal-first modulo avoids signed casts and the prior arithmetic guard');

    my ($success, $error_message, $full_buf, $stdout_buf, $stderr_buf) = run(
        command => ['./bin/fsmgen', '--language', 'vhdl', '--quiet', '-o', $output_path, $fsm_path],
    );
    my $combined_output = join('', @{ $stdout_buf || [] }, @{ $stderr_buf || [] }, ($error_message || ''));
    ok($success, 'CLI accepts direct --language vhdl for non-signed vector positive literal-first modulo')
        or diag($combined_output);
    ok(-e $output_path, 'CLI writes non-signed vector positive literal-first modulo VHDL output file');

    my $cli_hdl = read_file($output_path);
    like($cli_hdl, qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+unsigned\(A\),\s+8\)\);/s, 'CLI VHDL output includes non-signed vector positive literal-first modulo assignment');
    unlike($cli_hdl, qr/to_signed\(2,\s+8\)|arithmetic expression '2 % A'/s, 'CLI non-signed vector positive literal-first modulo avoids signed casts and the prior arithmetic guard');
};
