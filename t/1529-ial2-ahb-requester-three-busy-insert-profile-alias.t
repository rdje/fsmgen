...
subtest 'exact-three requester .ahb alias mirrors generic source, lowering, and report' => sub {
    ...
    is($alias->{generated_ial1}{name}, 'amba_requester_busy_insert_three.isf', 'alias exposes exact-three IAL1 artifact');
    ...
    is_deeply(
        ...
        {'amba_requester_busy_insert_three.fsm' => $ppif->{generated_ial0}{files}{'amba_requester_busy_insert_three.fsm'}},
        'alias keeps exact-three IAL0 artifact text',
    );

    is($alias->{report}{schema}, 'fsmgen.ial2.protocol_intent.ahb_requester.v1', 'alias keeps requester report schema');
    is($alias->{report}{source_object}{id}, 'fsmgen-ahb-requester-busy-insert-three', 'alias preserves source object');
    is($alias->{report}{source_object}{intent_name}, 'ahb_requester_busy_insert_three', 'alias preserves intent name');
    ...
    is($alias->{report}{busy_insertion}{beats}, 3, 'alias keeps numeric exact-three event count');
    ...
    is(
        ...
        'alias and generic source keep identical exact-three BUSY support residue',
    );
    like($alias->{generated_ial1}{text}, qr/\(var ahb_busy_remaining_q \(width 2\) \(reset 0\)\)/, 'alias keeps exact-three width-two actor counter');
};
...
subtest 'check, semantic JSON, and read-only MCP expose alias support identity' => sub {
    ...
    is($check->{result}{module_name}, 'amba_requester_busy_insert_three', 'check JSON reports exact-three module');
    is($check->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_requester_busy_insert_three', 'check JSON reports alias support identity');
    is($check->{support_accounting}{coverage}, 'ial2_ahb_profile_alias_requester_busy_insert_three_pipeline_cli', 'check JSON reports alias coverage');
    ...
    my $mcp = decode_json(
        $adapter->call_tool(
            ...
            { source_path => 'ppif/ahb_requester_busy_insert_three.ahb' },
        )->{content}[0]{text},
    );
    is($mcp->{query_kind}, 'semantic', 'MCP dispatches through bounded semantic query');
    is($mcp->{source_id}, 'ppif/ahb_requester_busy_insert_three.ahb', 'MCP keeps repo-relative alias identity');
...
subtest 'schedule, review artifacts, HDL, and verifier preserve exact-three identity' => sub {
    ...
    is($schedule->{busy_insertion}{beats}, 3, 'schedule JSON keeps numeric exact-three count');
    is($schedule->{generated_artifacts}{ial1}{name}, 'amba_requester_busy_insert_three.isf', 'schedule JSON names IAL1 artifact');
    is_deeply($schedule->{generated_artifacts}{ial0}{files}, ['amba_requester_busy_insert_three.fsm'], 'schedule JSON names IAL0 artifact');
    ...
    ok(-f File::Spec->catfile($outdir, 'amba_requester_busy_insert_three.isf'), 'outdir contains exact-three IAL1 artifact');
    ok(-f File::Spec->catfile($outdir, 'amba_requester_busy_insert_three.fsm'), 'outdir contains exact-three IAL0 artifact');
    ok(-f $hdl, 'outdir command emits HDL');
    like(slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_three.isf')), qr/ahb_busy_remaining_q/, 'generated IAL1 keeps exact-three counter');
    like(slurp($hdl), qr/\bmodule\s+amba_requester_busy_insert_three\b/, 'generated HDL keeps exact-three module');
    run_command_ok(
...
subtest 'profile boundaries and existing requester identities remain stable' => sub {
    ...
    is($generic->{support_accounting}{entry_id}, 'intent.ppif_ahb_requester_busy_insert_three', 'generic exact-three source keeps PPIF support identity');
...
sub assert_semantic_payload {
    ...
    is($semantic->{generation_result_snapshot}{summary}{module_name}, 'amba_requester_busy_insert_three', "$label reports exact-three module");
    is($semantic->{semantic}{module}{source_root_kind}, 'fsm', "$label reports generated FSM root");
    is($semantic->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_requester_busy_insert_three', "$label reports alias support identity");
    is($semantic->{support_accounting}{source_kind}, 'ial2_profile_alias', "$label reports profile-alias source kind");
}

sub ppif_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'ahb_requester_busy_insert_three.ppif');
}
