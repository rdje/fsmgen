...
subtest 'two-subordinate exact-four paired .ahb alias mirrors generic source and lowering' => sub {
    ...
    is($alias->{report}{source_object}{id}, 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-four-byte-lane-hburst-seq-busy-park', 'alias preserves source object');
    is($alias->{report}{source_object}{intent_name}, 'ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park', 'alias preserves intent name');
    is($alias->{report}{composition}{child_instance_count}, 4, 'alias preserves the four-child aggregate');
    ...
    for my $child_index (2, 3) {
        is_deeply($alias->{report}{children}[$child_index]{transfer}{seq_policy}{parks_on}, [qw(busy)], "alias child $child_index preserves BUSY parking");
    }
    for my $propagated (@{$alias->{report}{composition}{seq_policy_propagation}{subordinates}}) {
        is_deeply($propagated->{seq_policy}{parks_on}, [qw(busy)], 'alias preserves each propagated BUSY park');
    }
    is($alias->{report}{composition}{response_mux}{data_phase_owner}{mode}, 'one_hot_accepted_subordinate', 'alias preserves retained one-hot response ownership');
...
subtest 'check, schedule, semantic JSON, and read-only MCP expose alias identity' => sub {
    ...
    is($check->{result}{composition_child_count}, 4, 'check JSON reports child count');
    ...
    is($schedule->{source_object}{id}, 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-four-byte-lane-hburst-seq-busy-park', 'schedule JSON keeps source object');
    is($schedule->{source_object}{intent_name}, 'ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park', 'schedule JSON keeps intent name');
    is($schedule->{composition}{child_instance_count}, 4, 'schedule JSON keeps child count');
    is_deeply(
        [sort map { $_->{name} } @{$schedule->{generated_artifacts}{ial1}{items}}],
        [qw(ahb_control_subordinate_byte_lane_hburst_seq.isf ahb_interconnect.isf ahb_status_subordinate_byte_lane_hburst_seq.isf amba_requester_busy_insert_four.isf)],
        'schedule JSON keeps exact IAL1 artifacts',
    );
    is_deeply(
        [sort @{$schedule->{generated_artifacts}{ial0}{files}}],
        [qw(ahb_control_subordinate_byte_lane_hburst_seq.fsm ahb_interconnect.fsm ahb_status_subordinate_byte_lane_hburst_seq.fsm ahb_tb.fsm amba_requester_busy_insert_four.fsm)],
    ...
    for my $child_index (2, 3) {
        is_deeply($schedule->{children}[$child_index]{transfer}{seq_policy}{parks_on}, [qw(busy)], "schedule JSON keeps child $child_index parking");
    }
    for my $propagated (@{$schedule->{composition}{seq_policy_propagation}{subordinates}}) {
        is_deeply($propagated->{seq_policy}{parks_on}, [qw(busy)], 'schedule JSON keeps each propagated park');
    }
    is_deeply(
        [map { [$_->{name}, $_->{base}{default}, $_->{limit}] } @{$schedule->{composition}{address_map}{windows}}],
        [[status => 0, 4], [control => 4, 8]],
        'schedule JSON preserves status and control windows',
    );
    is($schedule->{composition}{response_mux}{data_phase_owner}{mode}, 'one_hot_accepted_subordinate', 'schedule JSON keeps one-hot response ownership');
...
subtest 'review artifacts, HDL, and verifier preserve two-subordinate exact-four paired identity' => sub {
    ...
    like(
        slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_four.isf')),
        qr/\(var ahb_busy_remaining_q \(width 3\) \(reset 0\)\)/,
        'requester IAL1 keeps the width-three exact-four counter',
    );
    like(slurp(File::Spec->catfile($outdir, 'ahb_status_subordinate_byte_lane_hburst_seq.isf')), qr/ahb_phase_pending_q/, 'status subordinate IAL1 keeps accepted-phase storage');
    like(slurp(File::Spec->catfile($outdir, 'ahb_control_subordinate_byte_lane_hburst_seq.isf')), qr/ahb_phase_pending_q/, 'control subordinate IAL1 keeps accepted-phase storage');
...
subtest 'profile boundaries and existing aggregate identities remain stable' => sub {
    ...
    is($generic->{support_accounting}{entry_id}, 'intent.ppif_ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park', 'generic exact-four source keeps PPIF support identity');
    is($generic->{support_accounting}{source_kind}, 'ppif', 'generic exact-four source keeps PPIF source kind');

    my $two_window_exact_three = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', two_window_exact_three_alias_path());
    is($two_window_exact_three->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'two-window exact-three paired alias keeps support identity');
    my $one_window_exact_four = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', one_window_exact_four_alias_path());
    is($one_window_exact_four->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park', 'one-window exact-four paired alias keeps support identity');
    my $base = run_json_command('./bin/fsmgen', '--quiet', '--emit-schedule-json', base_alias_path());
    ok(!exists $base->{children}[0]{busy_insertion}, 'base aggregate requester remains BUSY-insertion free');
};
...
sub assert_semantic_payload {
    ...
    is($semantic->{generation_result_snapshot}{summary}{composition_child_count}, 4, "$label reports four children");
...
sub ppif_relpath {
    return 'ppif/ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif';
}

sub alias_relpath {
    return 'ppif/ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ahb';
}

sub two_window_exact_three_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_two_subordinate_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ahb');
}

sub one_window_exact_four_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ahb');
}

sub base_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_two_subordinate_byte_lane_hburst_seq_busy_park.ahb');
}
...
sub support_id {
    return 'intent.ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ahb_profile_alias_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park_pipeline_cli';
}

sub project_tempdir {
    my $temp_root = File::Spec->catdir($repo_root, '.artifacts', 'tmp', 'tests');
    make_path($temp_root);
    return tempdir('t1540-XXXXXX', DIR => $temp_root, CLEANUP => 1);
}
