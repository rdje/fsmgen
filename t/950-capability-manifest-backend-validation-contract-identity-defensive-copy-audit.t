...
subtest 'manifest backend-validation contract identity metadata rebuilds cleanly after caller mutation' => sub {
    ...
    is($contract->{'status'}, $expected->{'status'}, 'fresh manifest backend-validation contract rebuilds clean status');
    is($contract->{'contract_source'}, $expected->{'contract_source'}, 'fresh manifest backend-validation contract rebuilds clean contract_source');
    is($contract->{'report_source'}, $expected->{'report_source'}, 'fresh manifest backend-validation contract rebuilds clean report_source');
