...
subtest 'actor scalar parameters specialize generated activation parameter overrides' => sub {
    my $source = <<'ISF';
(actor activation_parameter_actor_params
  (clock clk)
  (params
    (TOP_WIDTH 16)
    (LANE_A 8'hA5)
...
ISF
    ...
    is_deeply(
        ...
        'spawn, generated do, and rule-trigger overrides resolve actor scalar parameters before IR publication',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $top_fsm = $lowered->{files}{'activation_parameter_actor_params_top.fsm'};
    ok(defined($top_fsm), 'generated top is emitted for actor-parameter overrides');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\(LANES \(8'hA5 8'h3C\)\)\s+\)\s+\)/s,
        'spawn generated top applies resolved actor-parameter overrides');
    like($top_fsm, qr/\(\?fsmc:parent_worker_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\(LANES \(8'hA5 8'h3C\)\)\s+\)\s+\)/s,
        'generated do top applies resolved actor-parameter overrides');
    like($top_fsm, qr/\(\?fsmc:launch_worker_trigger_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'rule-trigger generated top applies resolved actor-parameter override');
    ...
    is_deeply(
        ...
        [
            ...
            [
                { name => 'WIDTH', source => 'override', value => '16' },
                { name => 'LANES', source => 'default', value => "(8'h00 8'h00)" },
            ],
        ],
        'schedule report exposes resolved actor-parameter literal override values',
    );
};
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'non-scalar actor parameter is not a parameter value source', qr/actor parameter 'TOP_WIDTH' must resolve to a scalar numeric or exact-width literal/);
(actor non_scalar_actor_parameter_value
  (clock clk)
  (params
    (TOP_WIDTH (8 16)))
  (interface
