...
subtest 'adapter composes exact-three requester BUSY insertion with subordinate BUSY parking' => sub {
    ok(-f sample_path(), 'tracked exact-three paired BUSY aggregate PPIF sample exists');
    my $source = slurp(sample_path());
    like($source, qr/\(ahb-requester amba_requester_busy_insert_three\b/, 'sample selects the exact-three BUSY-inserting requester');
    like($source, qr/\(busy-before-beat 2\)/, 'sample inserts BUSY before beat index two');
    like($source, qr/\(busy-beats 3\)/, 'sample selects three qualified BUSY events');
    ...
    is_deeply(
        ...
        'exact-three paired source exposes requester, subordinate, fabric, and top IAL0 artifacts',
    );
    is_deeply(
        ...
        'exact-three paired source exposes the exact generated IAL1 artifacts',
    );
    ...
    is($requester->{object_name}, 'amba_requester_busy_insert_three', 'requester child selects the exact-three BUSY-inserting object');
...
subtest 'CLI, semantic, MCP, and review surfaces agree on the exact-three paired aggregate' => sub {
    my $check = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', sample_path());
    ok($check->{success}, 'strict check succeeds for the exact-three paired BUSY source');
    ...
    is($mcp->{source_id}, sample_relpath(), 'MCP keeps repo-relative exact-three paired identity');
    ...
    ok($generate_ok, 'exact-three paired BUSY source emits HDL and review artifacts')
    ...
    like($requester_isf, qr/\(var ahb_busy_remaining_q \(width 2\) \(reset 0\)\)/, 'exact-three paired requester keeps its actor-owned counter');
    ...
    ok($verify_ok, 'public verify-hdl accepts the exact-three paired BUSY source')
        or diag(join('', @{$verify_stdout || []}), join('', @{$verify_stderr || []}));
};

subtest 'generated HDL pairs three qualified requester BUSY events with subordinate parking' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $hdl = File::Spec->catfile($tempdir, 'ahb_tb.sv');
    ...
    ok($generate_ok, 'public exact-three paired BUSY source emits generated HDL')
        or diag(join('', @{$generate_stdout || []}), join('', @{$generate_stderr || []}));
    ...
    my ($compile_ok, undef, undef, $compile_stdout, $compile_stderr) = run(
        command => [
            ...
            '-j', '1', '--top-module', 'ahb_exact_three_paired_busy_composition_tb',
            '--Mdir', $objdir, $hdl, testbench_path(),
        ],
    );
    ok($compile_ok, 'Verilator builds the assertion-enabled exact-three paired BUSY composition harness')
        or diag(join('', @{$compile_stdout || []}), join('', @{$compile_stderr || []}));
    return unless $compile_ok;

    my $binary = File::Spec->catfile($objdir, 'Vahb_exact_three_paired_busy_composition_tb');
    my ($run_ok, undef, undef, $run_stdout, $run_stderr) = run(command => [$binary]);
    ok($run_ok, 'generated-HDL exact-three paired BUSY composition passes with assertions enabled')
        or diag(join('', @{$run_stdout || []}), join('', @{$run_stderr || []}));
    like(
        join('', @{$run_stdout || []}),
        qr/PASS transfers=5 beats=4 busy=1 qualified_busy=3 resumed_seq=1 storage=44332211/,
        'runtime observes three qualified paired BUSY events, one resumed SEQ, parking, and four byte writes',
    );
};
...
sub sample_path {
    return File::Spec->catfile(
        $repo_root,
        'ppif',
        'ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ppif',
    );
}
...
sub testbench_path {
    return File::Spec->catfile($FindBin::Bin, 'data', 'ahb_exact_three_paired_busy_composition_tb.svt');
}

sub support_id {
    return 'intent.ppif_ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ppif_ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park_pipeline_cli';
}
