...
subtest 'source is the exact selected two-subordinate exact-two paired BUSY transform' => sub {
    ok(-f sample_path(), 'tracked two-subordinate exact-two paired BUSY PPIF sample exists');
    is(slurp(sample_path()), frozen_expected_source(), 'sample differs from the shipped two-subordinate exact-one paired source only by the frozen identity/requester/cardinality delta');
    ...
    is_deeply(
        ...
        [qw(amba_requester_busy_insert_two.isf ahb_status_subordinate_byte_lane_hburst_seq.isf ahb_control_subordinate_byte_lane_hburst_seq.isf ahb_interconnect.isf)],
    ...
    is($requester->{object_name}, 'amba_requester_busy_insert_two', 'requester child selects shipped exact-two BUSY requester');
    ...
    is($requester->{transfer}{busy_beats}, 2, 'requester child transfer reports exact-two BUSY cardinality');
    is($requester->{busy_insertion}{beats}, 2, 'requester child reports numeric exact-two BUSY cardinality');
    ...
    like(residue_detail($requester, 'ahb_requester_busy_insert_support'), qr/exactly 2 qualified requester HTRANS BUSY events/, 'requester child keeps numeric exact-two BUSY support residue');
...
subtest 'strict CLI, semantic, MCP, review-artifact, and HDL surfaces agree' => sub {
    ...
    like($requester_isf, qr/\(var ahb_busy_remaining_q \(width 2\) \(reset 0\)\)/, 'requester IAL1 keeps exact-two actor-owned counter');
...
subtest 'generated HDL pairs exact-two requester BUSY with both subordinate windows' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $hdl = File::Spec->catfile($tempdir, 'ahb_tb.sv');
    ...
    my ($compile_ok, undef, undef, $compile_stdout, $compile_stderr) = run(
        command => [
            ...
            '-j', '1', '--top-module', 'ahb_two_subordinate_exact_two_paired_busy_composition_tb',
            '--Mdir', $objdir, $hdl, testbench_path(),
        ],
    );
    ok($compile_ok, 'Verilator builds the assertion-enabled two-subordinate exact-two paired BUSY harness')
        or diag(join('', @{$compile_stdout || []}), join('', @{$compile_stderr || []}));
    return unless $compile_ok;

    my $binary = File::Spec->catfile($objdir, 'Vahb_two_subordinate_exact_two_paired_busy_composition_tb');
    my ($run_ok, undef, undef, $run_stdout, $run_stderr) = run(command => [$binary]);
    ok($run_ok, 'generated-HDL two-window exact-two paired BUSY proof passes with assertions enabled')
        or diag(join('', @{$run_stdout || []}), join('', @{$run_stderr || []}));
    like(
        join('', @{$run_stdout || []}),
        qr/PASS commands=2 transfers=10 beats=8 busy=2 qualified_busy=4 resumed_seq=2 status=44332211 control=88776655/,
        'runtime proves two qualified BUSY events and one resumed SEQ per command across both windows',
    );
};
...
sub predecessor_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park.ppif');
}

sub testbench_path {
    return File::Spec->catfile($FindBin::Bin, 'data', 'ahb_two_subordinate_exact_two_paired_busy_composition_tb.svt');
}

sub source_object_id {
    return 'fsmgen-ahb-interconnect-two-subordinate-requester-busy-insert-two-byte-lane-hburst-seq-busy-park';
}

sub source_intent {
    return 'ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park';
}

sub support_id {
    return 'intent.ppif_ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ppif_ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park_pipeline_cli';
}

sub frozen_expected_source {
    my $source = slurp(predecessor_path());
    replace_once(\$source, '(protocol-platform-intent ahb_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park', '(protocol-platform-intent ' . source_intent());
    replace_once(\$source, '(object fsmgen-ahb-interconnect-requester-busy-insert-two-subordinate-byte-lane-hburst-seq-busy-park)', '(object ' . source_object_id() . ')');
    replace_once(\$source, '(section bounded-ahb-interconnect-requester-busy-insert-two-subordinate-byte-lane-hburst-seq-busy-park)', '(section bounded-ahb-interconnect-two-subordinate-requester-busy-insert-two-byte-lane-hburst-seq-busy-park)');
    replace_once(\$source, '(ahb-requester amba_requester_busy_insert', '(ahb-requester amba_requester_busy_insert_two');
    replace_once(\$source, "      (busy-before-beat 2))\n", "      (busy-before-beat 2)\n      (busy-beats 2))\n");
    replace_once(\$source, '(requester requester amba_requester_busy_insert)', '(requester requester amba_requester_busy_insert_two)');
    return $source;
}
