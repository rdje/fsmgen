...
subtest 'direct generation raw_ast result branch is a caller-owned snapshot' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $fsm_path = File::Spec->catfile($tempdir, 'direct_raw_ast_alias_root.fsm');
    write_file(
        $fsm_path,
        <<'FSM'
(?dt:direct_raw_ast_alias_root
  (-route
    (serial_out> = trigger)
  )
  (+size
...
FSM
    ...
    $result->{raw_ast}[0][0] = '?dt:mutated_by_result';
    is(
        $raw_ast->[0][0],
        '?dt:direct_raw_ast_alias_root',
    ...
    $raw_ast->[0][0] = '?dt:mutated_by_caller';
    is(
        $result->{raw_ast}[0][0],
        '?dt:mutated_by_result',
