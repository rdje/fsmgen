...
(actor unknown_parameter_transaction_port_width
  ...
  (transaction child
    (ports
      (input addr (width DATA_W)))))
ISF
        qr/\AError: actor 'unknown_parameter_transaction_port_width' transaction 'child' port 'addr' width token 'DATA_W' is not a declared actor scalar parameter/,
        'unknown symbolic width is rejected',
    );

    assert_parse_rejected(
        <<'ISF',
(actor constant_parameter_transaction_port_width
  (clock clk)
  (constants
    (DATA_W 8))
  (interface
  ...
  (transaction child
    (ports
      (input addr (width DATA_W)))))
ISF
        qr/\AError: actor 'constant_parameter_transaction_port_width' transaction 'child' port 'addr' width token 'DATA_W' is an actor constant/,
        'actor constants remain outside this port-width slice',
    );
...
ISF
        qr/\AError: transaction 'child' port 'addr' width requires '\(width positive_integer_or_actor_scalar_parameter\)'/,
