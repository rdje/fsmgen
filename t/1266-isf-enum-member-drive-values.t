...
(actor enum_drive_target_still_deferred
  ...
  (transaction main
    ...
    (drive mark_busy)))
ISF
        qr/drive 'mark_busy' target references enum member 'mode\.BUSY'; this ISF surface accepts enum member references only as actor constants, transaction set RHS scalar values or operands, transaction switch branch values, drive body RHS scalar values, and drive-call actual scalar values or operands/,
        'enum members in drive targets remain deferred',
    );

    assert_parse_rejected(
        <<'ISF',
(actor enum_rule_action_still_deferred_after_drive
  (enums
    (mode (IDLE 0) (BUSY 1)))
  (clock clk)
  (reset rst)
