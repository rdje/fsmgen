...
my @required_examples = (
    ...
    'top-level when-body nested repeat local do then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    'top-level when-body nested repeat local do after multi-pending await_any then generated spawn before same-body await_all drain',
    ...
    'top-level when-body nested repeat generated do with static params after multi-pending await_any then generated spawn before same-body await_all drain',
    'top-level when-body nested repeat generated do with static params after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain',
    ...
    'top-level when-body nested repeat generated do with static params then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    ...
    'top-level when-body nested repeat generated do with static params and bind handoffs then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    'top-level when-body nested repeat generated do with static params, optional bind handoffs, and same-domain metadata before post-do multi-pending await_any while generated nested spawn pending before same-body await_all drain',
    ...
    'top-level when-body nested repeat generated do with static params, optional bind handoffs, and same-domain metadata then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    ...
    'top-level switch-branch nested repeat local do then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    'top-level switch-branch nested repeat local do after multi-pending await_any then generated spawn before same-body await_all drain',
    ...
    'top-level switch-branch nested repeat generated do with static params after multi-pending await_any then generated spawn before same-body await_all drain',
    'top-level switch-branch nested repeat generated do with static params after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain',
    ...
    'top-level switch-branch nested repeat generated do with static params then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    ...
    'top-level switch-branch nested repeat generated do with static params and bind handoffs then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    'top-level switch-branch nested repeat generated do with static params, optional bind handoffs, and same-domain metadata before post-do multi-pending await_any while generated nested spawn pending before same-body await_all drain',
    ...
    'top-level switch-branch nested repeat generated do with static params, optional bind handoffs, and same-domain metadata then generated spawn before post-spawn multi-pending await_any and same-body await_all drain',
    ...
    'top-level when-body nested repeat local do then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    'top-level when-body nested repeat local do after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level when-body nested repeat local do after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level when-body nested repeat generated do with static params after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level when-body nested repeat generated do with static params after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level when-body nested repeat generated do with static params then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level when-body nested repeat generated do with static params and bind handoffs after multi-pending await_any while generated nested spawn pending before same-body await_all drain subset',
    'top-level when-body nested repeat generated do with static params and bind handoffs after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level when-body nested repeat generated do with static params and bind handoffs after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level when-body nested repeat generated do with static params and bind handoffs then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level when-body nested repeat generated do with static params, optional bind handoffs, and same-domain metadata then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat local do then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    'top-level switch-branch nested repeat local do after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level switch-branch nested repeat local do after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat generated do with static params after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level switch-branch nested repeat generated do with static params after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat generated do with static params then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat generated do with static params and bind handoffs after multi-pending await_any while generated nested spawn pending before same-body await_all drain subset',
    'top-level switch-branch nested repeat generated do with static params and bind handoffs after multi-pending await_any then generated spawn before same-body await_all drain subset',
    'top-level switch-branch nested repeat generated do with static params and bind handoffs after multi-pending await_any then generated spawn before second multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat generated do with static params and bind handoffs then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
    ...
    'top-level switch-branch nested repeat generated do with static params, optional bind handoffs, and same-domain metadata then generated spawn before post-spawn multi-pending await_any and same-body await_all drain subset',
...
my @required_non_claims = (
    ...
    'prior-active-await_any local-do, plain generated-child, static-parameter generated-do, bound generated-do, and same-domain generated-do spawn-after-do with a second post-spawn await_any are shipped',
    'prior-active-await_any same-domain generated-do spawn-after-do is shipped',
    'missing same-body await_all drain, cross-domain activation, deeper branch/loop nesting, CDC behavior, and broader outstanding-child lifetime semantics remain fail-closed',
