...
ISF
    ...
    assert_parse_rejected(
        <<'ISF',
(actor enum_drive_call_expression_still_deferred
  (enums
    (mode (IDLE 0) (BUSY 1)))
  (clock clk)
  (reset rst)
...
ISF
        qr/transaction 'main' drive 'mark_mode' actual expression references enum member 'mode\.BUSY'; this ISF surface accepts enum member references only as actor constants, transaction set RHS scalar values or operands, transaction switch branch values, drive body RHS scalar values, and drive-call scalar actual values/,
        'enum members in drive call actual expressions remain deferred',
    );
...
(actor enum_inline_drive_still_deferred
  ...
  (transaction main
    ...
    (drive inline_mode
      (mode_out mode.BUSY))))
ISF
        qr/transaction 'main' references enum member 'mode\.BUSY'; this ISF surface accepts enum member references only as actor constants, transaction set RHS scalar values or operands, transaction switch branch values, drive body RHS scalar values, and drive-call scalar actual values/,
