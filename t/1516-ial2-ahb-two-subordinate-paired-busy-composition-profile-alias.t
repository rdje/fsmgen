...
subtest 'two-subordinate paired BUSY .ahb mirrors the generic source' => sub {
    ok(-f alias_path(), 'tracked two-subordinate paired BUSY .ahb alias exists');
    is(slurp(alias_path()), slurp(ppif_path()), '.ahb source is byte-identical to generic .ppif');
};

subtest 'public CLI and report surfaces select the alias contract' => sub {
    ...
    is(
        $schedule->{source_object}{id},
        'fsmgen-ahb-interconnect-requester-busy-insert-two-subordinate-byte-lane-hburst-seq-busy-park',
        'schedule JSON preserves source object',
    );
    is(
        $schedule->{source_object}{intent_name},
        'ahb_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park',
        'schedule JSON preserves intent name',
    );
    is($schedule->{target_protocol}{profile}, 'ahb', 'schedule JSON preserves explicit AHB profile');
    is($schedule->{composition}{child_instance_count}, 4, 'schedule JSON preserves four-child top');
    is_deeply(
        [map { $_->{name} } @{$schedule->{generated_artifacts}{ial1}{items}}],
        [qw(
        ...
        'schedule JSON exposes exact shared IAL1 artifact order',
    );
    is_deeply(
        ...
        'schedule JSON exposes exact shared IAL0 artifacts',
    );
    is($schedule->{children}[0]{busy_insertion}{htrans_busy_encoding}, "2'b01", 'requester child keeps BUSY encoding');
    is($schedule->{children}[0]{busy_insertion}{before_beat}, 2, 'requester child keeps BUSY insertion index');
    is($schedule->{children}[0]{busy_insertion}{beats}, 'single', 'requester child keeps one-presentation bound');
    is_deeply($schedule->{children}[2]{transfer}{seq_policy}{parks_on}, [qw(busy)], 'status child keeps BUSY parking');
    is_deeply($schedule->{children}[3]{transfer}{seq_policy}{parks_on}, [qw(busy)], 'control child keeps BUSY parking');
    is_deeply(
        [map { $_->{seq_policy}{parks_on} } @{$schedule->{composition}{seq_policy_propagation}{subordinates}}],
        [[qw(busy)], [qw(busy)]],
        'aggregate propagation keeps both BUSY parks',
    );
    my $windows = $schedule->{composition}{address_map}{windows};
    is_deeply([map { $_->{name} } @{$windows}], [qw(status control)], 'address map preserves status/control order');
    ...
    ok(!residue_id_occurs($schedule, 'ahb_aggregate_profile_alias_deferred'), 'alias removes aggregate alias residue');
    ok(!residue_id_occurs($schedule, 'ahb_profile_alias_deferred'), 'alias removes requester alias residue');
    ok(!residue_id_occurs($schedule, 'ahb_subordinate_profile_alias_deferred'), 'alias removes both subordinate alias residue occurrences');
    ok(!detail_pattern_occurs($schedule, qr/\.ahb alias exposure/), 'alias removes alias-exposure wording');
    ok(residue_id_occurs($schedule, 'ahb_requester_busy_insert_support'), 'alias keeps requester BUSY support residue');
    ok(residue_id_occurs($schedule, 'ahb_burst_seq_support_deferred'), 'alias keeps aggregate burst support residue');
    ok(!detail_pattern_occurs($schedule, qr/BUSY-in-burst continuation remains deferred/), 'parked alias keeps .799 BUSY report truthfulness');
...
subtest 'alias emits shared review artifacts and clean HDL' => sub {
    ...
    like(slurp(File::Spec->catfile($outdir, 'ahb_status_subordinate_byte_lane_hburst_seq.isf')), qr/ahb_phase_pending_q/, 'status IAL1 keeps one accepted next phase');
    like(slurp(File::Spec->catfile($outdir, 'ahb_control_subordinate_byte_lane_hburst_seq.isf')), qr/ahb_phase_pending_q/, 'control IAL1 keeps one accepted next phase');
...
subtest 'alias generated HDL retires one qualified BUSY event per command' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $hdl = File::Spec->catfile($tempdir, 'ahb_tb.sv');
    ...
    ok($generate_ok, 'two-subordinate BUSY alias emits generated HDL for runtime proof')
    ...
    ok($compile_ok, 'Verilator builds the assertion-enabled two-subordinate BUSY alias harness')
    ...
    ok($run_ok, 'generated-HDL two-subordinate BUSY alias proof passes with assertions enabled')
    ...
    like(
        ...
        'alias runtime proves one qualified BUSY event per command and both final storage values',
    );
};

subtest 'malformed two-subordinate profile aliases fail closed' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
...
sub alias_path {
    return File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'ahb_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park.ahb');
}
...
sub support_id {
    return 'intent.ahb_profile_alias_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ahb_profile_alias_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park_pipeline_cli';
}
