...
subtest 'adapter parses and lowers the exact-four requester source' => sub {
    ...
    like(sample_source(), qr/\(section bounded-requester-four-busy-insertion\)/, 'sample keeps its selected source anchor');
    like(sample_source(), qr/\(busy-before-beat 2\)/, 'sample selects insertion before beat index two');
    like(sample_source(), qr/\(busy-beats 4\)/, 'sample selects exactly four qualified BUSY events');

    my $result = FSM::Adapter::IAL2::PPIF->new()->parse_file(sample_path());
    is($result->{layer}, 'IAL2', 'adapter result stays IAL2');
    is($result->{kind}, 'protocol_intent.ahb_requester', 'exact-four insertion is an AHB requester option');
    is($result->{report}{source_object}{id}, 'fsmgen-ahb-requester-busy-insert-four', 'source object id is selected');
    is($result->{report}{source_object}{intent_name}, 'ahb_requester_busy_insert_four', 'source intent name is selected');
    is($result->{generated_ial1}{name}, 'amba_requester_busy_insert_four.isf', 'generated IAL1 artifact is distinct');
    is_deeply(
    ...
    like($isf, qr/\(var ahb_busy_remaining_q \(width 3\) \(reset 0\)\)/, 'actor-owned counter widens minimally for literal four');
    ...
    like(
        $isf,
        qr/\(when \(& \(== beat_index_q 2\) \(== busy_inserted_q 0\)\)\s+\(set ahb_busy_remaining_q 4\)\s+\(drive transfer_busy\)\s+\(set busy_inserted_q 1\)/s,
        'insertion initializes literal four before driving BUSY',
    );
...
subtest 'CLI, semantic, MCP, schedule, review, and verify surfaces agree' => sub {
    ...
    ok($check->{success}, 'strict check succeeds for exact-four source');
    ...
    is($schedule->{busy_insertion}{beats}, 4, 'schedule JSON exposes numeric exact-four');
    ...
    run_command_ok(
        ...
        'exact-four source emits HDL and review artifacts',
    );
    ok(-f File::Spec->catfile($outdir, 'amba_requester_busy_insert_four.isf'), 'outdir contains exact-four IAL1');
    ok(-f File::Spec->catfile($outdir, 'amba_requester_busy_insert_four.fsm'), 'outdir contains exact-four IAL0');
    like(slurp($hdl), qr/\bmodule\s+amba_requester_busy_insert_four\b/, 'generated HDL contains the selected module');
    run_command_ok(
        ['./bin/fsmgen', '--quiet', '--strict', '--verify-hdl', sample_path()],
        'public verifier accepts the exact-four source',
    );
};
...
subtest 'generated HDL retires exactly four qualified BUSY events and resumes SEQ' => sub {
    ...
    run_command_ok(
        ...
        'public exact-four requester emits generated HDL',
    );
    return unless -f $hdl;
    ...
    my ($compile_ok, undef, undef, $compile_stdout, $compile_stderr) = run(
        command => [
            ...
            '-j', '1', '--top-module', 'ahb_requester_four_busy_insert_tb',
            '--Mdir', $objdir, $hdl, testbench_path(),
        ],
    );
    ok($compile_ok, 'Verilator builds the exact-four requester harness')
        or diag(join('', @{$compile_stdout || []}), join('', @{$compile_stderr || []}));
    return unless $compile_ok;

    my $binary = File::Spec->catfile($objdir, 'Vahb_requester_four_busy_insert_tb');
    my @scenarios = (
    ...
    for my $scenario (@scenarios) {
        ...
        ok($run_ok, "$label exact-four generated-HDL insertion passes")
            or diag(join('', @{$run_stdout || []}), join('', @{$run_stderr || []}));
        like(
            join('', @{$run_stdout || []}),
            qr/PASS transfers=5 beats=4 busy=1 qualified_busy=4 stall_mode=$mode stall_clocks=$stall_clocks busy_remaining=0/,
            "$label directly observes 4-to-3-to-2-to-1-to-0 retirement and completes four data beats",
        );
    }
};

subtest 'exact-one, exact-two, exact-three, and base requester behavior remain distinct' => sub {
    my $three = FSM::Adapter::IAL2::PPIF->new()->parse_file(exact_three_path());
    is($three->{report}{busy_insertion}{beats}, 3, 'exact-three source keeps numeric report value three');
...
sub assert_semantic_payload {
    ...
    is($semantic->{generation_result_snapshot}{summary}{module_name}, module_name(), "$label reports exact-four module");
    is($semantic->{semantic}{module}{source_root_kind}, 'fsm', "$label reports generated FSM root");
    is($semantic->{support_accounting}{entry_id}, support_id(), "$label reports exact-four support identity");
    is($semantic->{support_accounting}{source_kind}, 'ppif', "$label reports PPIF source kind");
}

sub sample_relpath { return 'ppif/ahb_requester_busy_insert_four.ppif' }
...
sub exact_three_path { return File::Spec->catfile($repo_root, 'ppif', 'ahb_requester_busy_insert_three.ppif') }
...
sub testbench_path { return File::Spec->catfile($repo_root, 't', 'data', 'ahb_requester_four_busy_insert_tb.svt') }
