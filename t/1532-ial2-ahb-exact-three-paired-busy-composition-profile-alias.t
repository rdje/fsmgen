...
subtest 'exact-three paired .ahb alias mirrors generic source and lowering' => sub {
    ok(-f alias_path(), 'tracked exact-three paired .ahb alias exists');
    ...
    is($alias->{report}{source_object}{id}, 'fsmgen-ahb-interconnect-requester-busy-insert-three-byte-lane-hburst-seq-busy-park', 'alias preserves source object');
    is($alias->{report}{source_object}{intent_name}, 'ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'alias preserves intent name');
    ...
    is($alias->{report}{children}[0]{busy_insertion}{beats}, 3, 'alias preserves numeric exact-three cardinality');
...
subtest 'check, schedule, semantic JSON, and read-only MCP expose alias identity' => sub {
    ...
    is($schedule->{source_object}{id}, 'fsmgen-ahb-interconnect-requester-busy-insert-three-byte-lane-hburst-seq-busy-park', 'schedule JSON keeps source object');
    is($schedule->{source_object}{intent_name}, 'ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'schedule JSON keeps intent name');
    ...
    is_deeply(
        ...
        [qw(ahb_interconnect.isf ahb_lite_subordinate_byte_lane_hburst_seq.isf amba_requester_busy_insert_three.isf)],
        'schedule JSON keeps exact IAL1 artifacts',
    );
    is_deeply(
        [sort @{$schedule->{generated_artifacts}{ial0}{files}}],
        [qw(ahb_interconnect.fsm ahb_lite_subordinate_byte_lane_hburst_seq.fsm ahb_tb.fsm amba_requester_busy_insert_three.fsm)],
    ...
    is($schedule->{children}[0]{busy_insertion}{beats}, 3, 'schedule JSON keeps numeric exact-three count');
...
subtest 'review artifacts, HDL, and verifier preserve exact-three paired identity' => sub {
    my $tempdir = tempdir('t1532-XXXXXX', DIR => File::Spec->catdir($repo_root, '.artifacts'), CLEANUP => 1);
    ...
    like(slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_three.isf')), qr/\(var ahb_busy_remaining_q \(width 2\) \(reset 0\)\)/, 'requester IAL1 keeps the width-two exact-three counter');
...
subtest 'profile boundaries and existing aggregate identities remain stable' => sub {
    ...
    is($generic->{support_accounting}{entry_id}, 'intent.ppif_ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park', 'generic exact-three source keeps PPIF support identity');
    is($generic->{support_accounting}{source_kind}, 'ppif', 'generic exact-three source keeps PPIF source kind');

    my $exact_two = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', exact_two_alias_path());
    is($exact_two->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park', 'exact-two paired alias keeps support identity');
    my $requester = run_json_command('./bin/fsmgen', '--quiet', '--strict', '--check', '--json', requester_alias_path());
    is($requester->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_requester_busy_insert_three', 'standalone exact-three requester alias keeps support identity');
};
...
sub ppif_relpath {
    return 'ppif/ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ppif';
}

sub alias_relpath {
    return 'ppif/ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ahb';
}
...
sub requester_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_requester_busy_insert_three.ahb');
}
...
sub support_id {
    return 'intent.ahb_profile_alias_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ahb_profile_alias_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park_pipeline_cli';
}
