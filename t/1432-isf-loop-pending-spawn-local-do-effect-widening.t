...
subtest 'multi-pending and until-contained variants remain outside the exact slice' => sub {
...
ISF
    ...
    my $until = parse_actor(actor_for_body('until_pending_spawn_local_do', <<'ISF'), 'until-pending-spawn-local-do');
(until cond
  (repeat loops
    (spawn worker as w0)
    (do helper)
...
ISF
    ...
    my ($until_lowered, $until_err) = lower_actor($until);
    ok(!$until_lowered, 'until-contained analogue remains outside the while-only selection');
    like($until_err, qr/repeat-body do cannot appear while repeat-body spawn clauses are pending/,
        'until-contained analogue keeps the existing pending-spawn do gate');
};
