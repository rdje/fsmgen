...
subtest 'adapter report and flat three-child composition preserve the selected contract' => sub {
    ...
    is($result->{kind}, 'protocol_intent.axi_read_burst4_transaction_composition', 'composition kind is exact');
    is($result->{mode}, 'read-burst4-transaction-composition', 'composition mode is exact');
    is($report->{schema}, 'fsmgen.ial2.protocol_intent.axi_read_burst4_transaction_composition.v1', 'report schema is exact');
    is($report->{source_object}{id}, 'axi-read-burst4-transaction-composition', 'source object identity is preserved');
    is($report->{source_object}{intent_name}, 'axi_read_burst4_transaction_composition', 'root intent name is preserved');
    is_deeply(
    ...
    is_deeply(
        ...
        { profile => 'axi4', object => 'axi-read-burst4-transaction-composition', role => 'manager' },
    ...
    is($report->{bindings}{status}{beat_done}, 'read_beat_done', 'beat completion binding is distinct');
    is($report->{bindings}{status}{transaction_done}, 'read_transaction_done', 'transaction completion binding is distinct');
    is_deeply($report->{bindings}{status}{response_beat_index}, { name => 'response_beat_index', width => 2 }, 'response beat index binding is exact');
    is_deeply(
        $report->{fixed_burst_policy},
        {
            address_width => 32, address_alignment_bytes => 4, span_bytes => 16,
            four_kib_contained => JSON::PP::true, id_width => 4,
            data_width => 32, arlen => 3, arsize => 2, arburst => 1,
            arburst_name => 'INCR', beat_count => 4,
        ...
        'fixed four-beat AR R policy is complete and exact',
    );
    ...
    is($report->{transaction_coordinator}{r_arm_policy}, 'first_after_ar_completion_then_once_after_each_nonfinal_beat', 'one R acceptor is explicitly re-armed four times');
    is($report->{transaction_coordinator}{busy_policy}, 'admission_through_fourth_r_beat_retirement', 'busy spans the four-beat read transaction');
    is($report->{transaction_coordinator}{mismatch_policy}, 'assert_and_drain_to_authoritative_fourth_accepted_beat', 'RID/RLAST mismatch drains to the authoritative count');
    is($report->{transaction_coordinator}{response_status_policy}, 'raw_per_beat_rresp_not_success_and_not_aggregated', 'RRESP remains raw per-beat status');
    is_deeply(
    ...
    my ($coordinator_item) = grep { $_->{object_name} eq 'axi_read_burst4_transaction_coordinator' } @{$result->{generated_ial1}{items}};
    my $coordinator_isf = $coordinator_item->{text};
    like($coordinator_isf, qr/\(rule arm_first_response \(& active_q \(! response_armed_q\) \(== beat_index_q 0\)/, 'coordinator gates first R arm on AR completion');
    like($coordinator_isf, qr/\(rule arm_next_response \(& active_q \(! response_armed_q\) \(! \(== beat_index_q 0\)\)/, 'coordinator explicitly re-arms the unchanged R child');
    like($coordinator_isf, qr/\(set response_id_match \(& response_id_match \(== captured_rid_i expected_arid_q\)\)\)/, 'coordinator accumulates RID match');
    like($coordinator_isf, qr/AXI RLAST must be low before and high on fixed-four beat index 3/, 'coordinator carries the count/RLAST assertion');
    my ($schedule) = grep { $_->{object_name} eq 'axi_read_burst4_transaction_coordinator' } @{$report->{generated_schedules}{items}};
    is($schedule->{report}{port_count}, 20, 'coordinator has twenty ports');
    is($schedule->{report}{state_count}, 0, 'coordinator remains rule-only');
    is_deeply(
        [map { [$_->{name}, $_->{assignments}] } @{$schedule->{report}{dt_blocks}}],
        [
            ['admit', 10], ['clear_ar_start', 1], ['arm_first_response', 3],
            ['arm_next_response', 2], ['clear_r_arm', 1], ['clear_request_done', 1],
            ['advance_beat', 6], ['finish_burst', 8],
            ['clear_beat_done', 1], ['clear_transaction_done', 1],
        ],
        'ten coordinator rule DTs retain exact assignment counts',
    );
    is_deeply(
        ...
        'eight realized coordinator priorities are exact',
    );
    is_deeply(
        $report->{generated_artifacts}{ial0}{files},
        [qw(axi_ar_driver.fsm axi_r_beat_acceptor.fsm axi_read_burst4_transaction_composition.fsm axi_read_burst4_transaction_coordinator.fsm)],
        'three leaf FSMs plus one selected top are exact',
    );
    is($report->{generated_artifacts}{hdl_entry}{entry_artifact}, 'axi_read_burst4_transaction_composition.fsm', 'structural top artifact is selected');
    is_deeply(
        ...
        'fifteen enforced static rules are ordered and exact',
    );
    is_deeply(
        ...
        'fourteen unsupported residue entries are ordered and exact',
    );

    my $top = $result->{generated_ial0}{files}{'axi_read_burst4_transaction_composition.fsm'};
    like($top, qr/\A\(\?top:axi_read_burst4_transaction_composition\b/, 'generated IAL0 entry is a structural top');
    ...
    like($top, qr/\(\?fsmc:transaction_coordinator axi_read_burst4_transaction_coordinator\)/, 'top directly instantiates coordinator');
    like($top, qr/\(=8'd3 ar_driver\.cmd_arlen\)/, 'top fixes ARLEN to four beats');
...
subtest 'malformed and expanded full-read contracts fail closed' => sub {
    my @cases = (
        ...
        ['wrong profile family', sub { replace_once(sample_text(), '(profile axi4)', '(profile ahb)') }, qr/profile 'ahb' does not match \(axi-read-burst4-transaction-composition/, 'bad-profile.ppif'],
        ...
        ['missing beat done', sub { my $s = sample_text(); $s =~ s/\n      \(beat-done read_beat_done\)// or die; $s }, qr/status \.\.\.\) is missing required \(beat-done \.\.\.\) clause/, 'missing-beat-done.ppif'],
        ...
        ['missing response beat index', sub { my $s = sample_text(); $s =~ s/\n      \(response-beat-index response_beat_index width 2\)// or die; $s }, qr/status \.\.\.\) is missing required \(response-beat-index \.\.\.\) clause/, 'missing-beat-index.ppif'],
        ...
        ['mixed standalone AR', sub { append_root_clause(sample_text(), standalone_ar_clause()) }, qr/cannot mix \(axi-read-burst4-transaction-composition \.\.\.\) with other intent objects/, 'mixed.ppif'],
        ['multiple aggregate objects', sub { append_root_clause(sample_text(), second_composition_clause()) }, qr/supports exactly one \(axi-read-burst4-transaction-composition \.\.\.\) object/, 'duplicate-object.ppif'],
        ...
        ['generated collision', sub { replace_once(sample_text(), '(axi-read-burst4-transaction-composition axi_read_burst4_transaction_composition', '(axi-read-burst4-transaction-composition axi_ar_driver') }, qr/generated duplicate \.fsm artifact 'axi_ar_driver\.fsm'/, 'collision.ppif'],
        ['profile alias rejection', sub { sample_text() }, qr/\(axi-read-burst4-transaction-composition \.\.\.\) remains unsupported for the first profile-alias implementation/, 'composition.axi'],
    );
    ...
    my @width_cases = (
        ...
        ['status.response_beat_index', '(response-beat-index response_beat_index width 2)', '(response-beat-index response_beat_index width 3)', qr/status\.response_beat_index\.width must be 2/],
    );
...
subtest 'CLI support accounting and external HDL use the public full-read path' => sub {
    ...
    is($check->{result}{module_name}, 'axi_read_burst4_transaction_composition', 'check JSON reports the top module');
    ...
    is($check->{support_accounting}{entry_id}, 'intent.ppif_axi_read_burst4_transaction_composition', 'check JSON matches support accounting');

    my $schedule = run_json_command('./bin/fsmgen', '--quiet', '--emit-schedule-json', sample_path());
    is($schedule->{schema}, 'fsmgen.ial2.protocol_intent.axi_read_burst4_transaction_composition.v1', 'schedule JSON exposes aggregate schema');
    ...
    is($semantic->{semantic}{module}{composition_net_count}, 48, 'semantic JSON reports forty-eight nets');
    is($semantic->{semantic}{module}{composition_resolved_link_count}, 46, 'semantic JSON reports forty-six links');
    is($semantic->{support_accounting}{entry_id}, 'intent.ppif_axi_read_burst4_transaction_composition', 'semantic JSON matches support accounting');
    ...
    for my $artifact (qw(axi_ar_driver.isf axi_r_beat_acceptor.isf axi_read_burst4_transaction_coordinator.isf axi_ar_driver.fsm axi_r_beat_acceptor.fsm axi_read_burst4_transaction_coordinator.fsm axi_read_burst4_transaction_composition.fsm)) {
        ok(-f File::Spec->catfile($outdir, $artifact), "outdir contains $artifact");
    }
    like(slurp($hdl), qr/\bmodule\s+axi_read_burst4_transaction_composition\b/, 'generated HDL contains the structural top');
    like(slurp($hdl), qr/each accepted AXI RID must match the admitted ARID/, 'generated HDL retains RID assertion');
    like(slurp($hdl), qr/AXI RLAST must be low before and high on fixed-four beat index 3/, 'generated HDL retains RLAST assertion');
...
subtest 'generated structural top joins AR then retires exact raw R outcomes' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $hdl = File::Spec->catfile($tempdir, 'axi_read_burst4_transaction_composition.sv');
    my $testbench = File::Spec->catfile($tempdir, 'axi_read_burst4_transaction_composition_tb.sv');
...
sub sample_path { File::Spec->catfile($FindBin::Bin, '..', 'ppif', 'axi_read_burst4_transaction_composition.ppif') }
...
sub second_composition_clause {
    ...
    my $start = index($source, '  (axi-read-burst4-transaction-composition');
    die "failed to locate aggregate object\n" if $start < 0;
    my $object = substr($source, $start);
    $object =~ s/\)\s*\z// or die "failed to remove root close\n";
    $object =~ s/axi_read_burst4_transaction_composition/axi_read_burst4_transaction_composition_2/g;
    $object =~ s/\b(clk|rst_n|read_cmd_valid|cmd_read_addr|cmd_read_id|arready|arvalid|araddr|arid|arlen|arsize|arburst|rvalid|rready|rid|rdata|rresp|rlast|response_rid|response_rdata|response_rresp|response_rlast|read_busy|read_request_done|read_beat_done|read_transaction_done|response_beat_index|response_id_match|response_last_match)\b/${1}_2/g;
    return $object;
}
