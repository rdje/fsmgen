...
subtest 'exact-two paired .ahb alias mirrors generic source and lowering' => sub {
    ok(-f alias_path(), 'tracked exact-two paired .ahb alias exists');
    ...
    is($alias->{report}{source_object}{id}, 'fsmgen-ahb-interconnect-requester-busy-insert-two-byte-lane-hburst-seq-busy-park', 'alias preserves source object');
    is($alias->{report}{source_object}{intent_name}, 'ahb_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park', 'alias preserves intent name');
    ...
    is($alias->{report}{children}[0]{busy_insertion}{beats}, 2, 'alias preserves numeric exact-two cardinality');
...
subtest 'check, schedule, semantic JSON, and read-only MCP expose alias identity' => sub {
    ...
    is($schedule->{source_object}{id}, 'fsmgen-ahb-interconnect-requester-busy-insert-two-byte-lane-hburst-seq-busy-park', 'schedule JSON keeps source object');
    is($schedule->{source_object}{intent_name}, 'ahb_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park', 'schedule JSON keeps intent name');
    ...
    is($schedule->{children}[0]{busy_insertion}{beats}, 2, 'schedule JSON keeps numeric exact-two count');
...
subtest 'review artifacts, HDL, and verifier preserve exact-two paired identity' => sub {
    ...
    like(slurp(File::Spec->catfile($outdir, 'amba_requester_busy_insert_two.isf')), qr/ahb_busy_remaining_q/, 'requester IAL1 keeps exact-two counter');
...
subtest 'profile boundaries and existing aggregate identities remain stable' => sub {
    ...
    is($generic->{support_accounting}{entry_id}, 'intent.ppif_ahb_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park', 'generic exact-two source keeps PPIF support identity');
    ...
    is($exact_one->{support_accounting}{entry_id}, 'intent.ahb_profile_alias_interconnect_requester_busy_insert_byte_lane_hburst_seq_busy_park', 'exact-one paired alias keeps support identity');
...
sub alias_relpath {
    return 'ppif/ahb_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park.ahb';
}

sub exact_one_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_requester_busy_insert_byte_lane_hburst_seq_busy_park.ahb');
}

sub base_alias_path {
    return File::Spec->catfile($repo_root, 'ppif', 'ahb_interconnect_byte_lane_hburst_seq_busy_park.ahb');
}
...
sub support_id {
    return 'intent.ahb_profile_alias_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park';
}

sub coverage_key {
    return 'ial2_ahb_profile_alias_interconnect_requester_busy_insert_two_byte_lane_hburst_seq_busy_park_pipeline_cli';
}
