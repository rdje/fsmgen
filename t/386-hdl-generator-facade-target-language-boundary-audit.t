...
subtest 'facade target_language option routes direct VHDL non-signed negative numeric-literal addition behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_negative_literal_add.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_negative_literal_add
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_negative_literal_add\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed negative literal addition direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed negative literal addition input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bSUM\s+<=\s+std_logic_vector\(unsigned\(A\)\s+\+\s+unsigned\(to_signed\(-1,\s+8\)\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector plus negative literal through two-complement numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_unsigned\(-1,\s+8\)|arithmetic expression 'A \+ -1'/s,
        'explicit VHDL non-signed negative literal addition facade generation avoids SystemVerilog syntax, invalid to_unsigned, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed negative numeric-literal subtraction behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_negative_literal_sub.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_negative_literal_sub
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_negative_literal_sub\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed negative literal subtraction direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed negative literal subtraction input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bDIFF\s+<=\s+std_logic_vector\(unsigned\(A\)\s+-\s+unsigned\(to_signed\(-1,\s+8\)\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector minus negative literal through two-complement numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_unsigned\(-1,\s+8\)|arithmetic expression 'A - -1'/s,
        'explicit VHDL non-signed negative literal subtraction facade generation avoids SystemVerilog syntax, invalid to_unsigned, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed negative numeric-literal multiplication behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_negative_literal_mul.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_negative_literal_mul
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_negative_literal_mul\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed negative literal multiplication direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed negative literal multiplication input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bPROD\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+\*\s+unsigned\(to_signed\(-2,\s+8\)\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector times negative literal through resized two-complement numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_unsigned\(-2,\s+8\)|arithmetic expression 'A \* -2'/s,
        'explicit VHDL non-signed negative literal multiplication facade generation avoids SystemVerilog syntax, invalid to_unsigned, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal multiplication behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_mul.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_mul
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_mul\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal multiplication direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed positive literal multiplication input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bPROD\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+\*\s+to_unsigned\(2,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector times positive literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\(2,\s+8\)|arithmetic expression 'A \* 2'/s,
        'explicit VHDL non-signed positive literal multiplication facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};
...
subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal literal-literal multiplication behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_pair_mul.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_pair_mul
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_pair_mul\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal-literal multiplication direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bPROD\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\*\s+to_unsigned\(3,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector literal-literal multiplication through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\([23],\s+8\)|arithmetic expression '2 \* 3'/s,
        'explicit VHDL non-signed positive literal-literal multiplication facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal literal-literal division behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_pair_div.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_pair_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_pair_div\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal-literal division direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+to_unsigned\(3,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector literal-literal division through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\([23],\s+8\)|arithmetic expression '2 \/ 3'/s,
        'explicit VHDL non-signed positive literal-literal division facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal literal-literal modulo behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_pair_mod.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_pair_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_pair_mod\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal-literal modulo direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+to_unsigned\(3,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector literal-literal modulo through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\([23],\s+8\)|arithmetic expression '2 % 3'/s,
        'explicit VHDL non-signed positive literal-literal modulo facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};
...
subtest 'facade target_language option routes direct VHDL non-signed negative numeric-literal division behavior' => sub {
    ...
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_negative_literal_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_negative_literal_div\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed negative literal division direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed negative literal division input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+\/\s+unsigned\(to_signed\(-2,\s+8\)\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector divided by negative literal through resized two-complement numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_unsigned\(-2,\s+8\)|arithmetic expression 'A \/ -2'/s,
        'explicit VHDL non-signed negative literal division facade generation avoids SystemVerilog syntax, invalid to_unsigned, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal division behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_div.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_div\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal division direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed positive literal division input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+\/\s+to_unsigned\(2,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector divided by positive literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\(2,\s+8\)|arithmetic expression 'A \/ 2'/s,
        'explicit VHDL non-signed positive literal division facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal literal-first division behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_first_div.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_first_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_first_div\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal-first division direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed positive literal-first division input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bQUOT\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+\/\s+unsigned\(A\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector literal-first division through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\(2,\s+8\)|arithmetic expression '2 \/ A'/s,
        'explicit VHDL non-signed positive literal-first division facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed negative numeric-literal modulo behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_negative_literal_mod.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_negative_literal_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_negative_literal_mod\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed negative literal modulo direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed negative literal modulo input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bREM\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+mod\s+unsigned\(to_signed\(-2,\s+8\)\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector modulo negative literal through resized two-complement numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_unsigned\(-2,\s+8\)|arithmetic expression 'A % -2'/s,
        'explicit VHDL non-signed negative literal modulo facade generation avoids SystemVerilog syntax, invalid to_unsigned, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal modulo behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_mod.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_mod\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal modulo direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed positive literal modulo input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bREM\s+<=\s+std_logic_vector\(resize\(unsigned\(A\)\s+mod\s+to_unsigned\(2,\s+8\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector modulo positive literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\(2,\s+8\)|arithmetic expression 'A % 2'/s,
        'explicit VHDL non-signed positive literal modulo facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL non-signed positive numeric-literal literal-first modulo behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_vector_positive_literal_first_mod.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_vector_positive_literal_first_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_vector_positive_literal_first_mod\s+is\b/s,
        'explicit VHDL facade generation emits the non-signed positive literal-first modulo direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+std_logic_vector\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers non-signed positive literal-first modulo input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bREM\s+<=\s+std_logic_vector\(resize\(to_unsigned\(2,\s+8\)\s+mod\s+unsigned\(A\),\s+8\)\);/s,
        'explicit VHDL facade generation lowers non-signed vector literal-first modulo through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|to_signed\(2,\s+8\)|arithmetic expression '2 % A'/s,
        'explicit VHDL non-signed positive literal-first modulo facade generation avoids SystemVerilog syntax, signed casts, and the prior guard',
    );
};
...
subtest 'facade target_language option routes direct VHDL signed negative numeric-literal multiplication behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_signed_negative_literal_mul.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_signed_negative_literal_mul
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_signed_negative_literal_mul\s+is\b/s,
        'explicit VHDL facade generation emits the signed negative literal multiplication direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+signed\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers signed negative literal multiplication input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bPROD\s+<=\s+resize\(A\s+\*\s+to_signed\(-2,\s+8\),\s+8\);/s,
        'explicit VHDL facade generation lowers signed vector times negative literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|std_logic_vector\(resize\(unsigned\(A\)|unsigned\(A\)|to_unsigned\(-2,\s+8\)|arithmetic expression 'A \* -2'/s,
        'explicit VHDL signed negative literal multiplication facade generation avoids SystemVerilog syntax, unsigned casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL signed negative numeric-literal division behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_signed_negative_literal_div.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_signed_negative_literal_div
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_signed_negative_literal_div\s+is\b/s,
        'explicit VHDL facade generation emits the signed negative literal division direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+signed\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers signed negative literal division input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bQUOT\s+<=\s+resize\(A\s+\/\s+to_signed\(-2,\s+8\),\s+8\);/s,
        'explicit VHDL facade generation lowers signed vector divided by negative literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|std_logic_vector\(resize\(unsigned\(A\)|unsigned\(A\)|to_unsigned\(-2,\s+8\)|arithmetic expression 'A \/ -2'/s,
        'explicit VHDL signed negative literal division facade generation avoids SystemVerilog syntax, unsigned casts, and the prior guard',
    );
};

subtest 'facade target_language option routes direct VHDL signed negative numeric-literal modulo behavior' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $direct_path = File::Spec->catfile($tempdir, 'facade_direct_vhdl_signed_negative_literal_mod.fsm');
    write_file(
        $direct_path,
        <<'FSM'
(?fsm:facade_direct_vhdl_signed_negative_literal_mod
  (+system
    (clock clk)
    (sreset reset)
  )
...
FSM
    ...
    like(
        $vhdl_result->{hdl_code},
        qr/\bentity\s+facade_direct_vhdl_signed_negative_literal_mod\s+is\b/s,
        'explicit VHDL facade generation emits the signed negative literal modulo direct entity',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bA\s+:\s+in\s+signed\(7\s+downto\s+0\);?/s,
        'explicit VHDL facade generation lowers signed negative literal modulo input port',
    );
    like(
        $vhdl_result->{hdl_code},
        qr/\bREM\s+<=\s+resize\(A\s+mod\s+to_signed\(-2,\s+8\),\s+8\);/s,
        'explicit VHDL facade generation lowers signed vector modulo negative literal through resized numeric_std',
    );
    unlike(
        $vhdl_result->{hdl_code},
        qr/\bmodule\b|\balways_(?:ff|comb)\b|std_logic_vector\(resize\(unsigned\(A\)|unsigned\(A\)|to_unsigned\(-2,\s+8\)|arithmetic expression 'A % -2'/s,
        'explicit VHDL signed negative literal modulo facade generation avoids SystemVerilog syntax, unsigned casts, and the prior guard',
    );
};
