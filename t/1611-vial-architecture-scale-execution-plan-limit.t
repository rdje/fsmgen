...
subtest 'sixteen-MiB limit rejects mutation and unowned reference level' => sub {
    ...
    ok(!$unowned, 'reference plan cannot enter the generated byte slice');
    like($@, qr/execution-graph gate slice does not own the requested shape/,
        'unowned-level rejection names the bounded frontier');
};
