...
(actor unknown_parameter_bank_depth
  ...
  (storage
    (bank data (width 7) (depth DEPTH))))
ISF
        qr/\AError: actor 'unknown_parameter_bank_depth' storage bank 'data' depth token 'DEPTH' is not a declared actor scalar parameter/,
        'unknown symbolic depth is rejected',
    );

    assert_parse_rejected(
        <<'ISF',
(actor constant_parameter_bank_depth
  (clock clk)
  (constants
    (DEPTH 3))
  (interface
  ...
  (storage
    (bank data (width 7) (depth DEPTH))))
ISF
        qr/\AError: actor 'constant_parameter_bank_depth' storage bank 'data' depth token 'DEPTH' is an actor constant/,
        'actor constants remain outside this bank-depth slice',
    );
...
ISF
        qr/\AError: actor 'expression_parameter_bank_depth' storage 'data' depth requires '\(depth positive_integer_or_actor_scalar_parameter\)'/,
