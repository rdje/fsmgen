...
like(
    $exception,
        qr/Composition top 'unsupported_child_top' contains child '\?bogus:child', .*composition child kind support is blocked because the active composition parser currently accepts only '\?fsmc', '\?dtc', '\?rtl', '\?ports', '\?wiring', '\+constants', '\+enums', '\+types', and '\+import'/s,
...
my $combined_output = join(
    ...
    like(
        $combined_output,
        qr/composition child kind support is blocked because the active composition parser currently accepts only '\?fsmc', '\?dtc', '\?rtl', '\?ports', '\?wiring', '\+constants', '\+enums', '\+types', and '\+import'/s,
