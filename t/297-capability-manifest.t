...
subtest 'manifest captures the first downstream tool contract surface' => sub {
    ...
    like(
        $file_surface_by_suffix{'.ppif'}{current_boundary},
        qr/APB address widths other than 32, wait-count widths other than 4, data widths beyond the selected sideband-aware 16\/32-bit boundary, additional APB PPROT policy families, APB back-to-back variants beyond the selected fixed\/status, selected sideband-aware fixed\/status, selected sideband-aware protected fixed multi-register status, selected sideband-aware data16 fixed multi-register status, selected sideband-aware data16-protection fixed multi-register status, no-sideband multi-peripheral status, and sideband-aware multi-peripheral status families/,
