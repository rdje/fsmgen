...
my $pin_ingress_vector_multi_isf_file = File::Spec->catfile(
    $repo_root,
    'isf',
    'atl_resolved_child_pin_ingress_vector_multi_pipeline.isf',
);
my $pin_ingress_mixed_isf_file = File::Spec->catfile(
    $repo_root,
    'isf',
    'atl_resolved_child_pin_ingress_mixed_pipeline.isf',
);
...
my $pin_egress_vector_isf_file = File::Spec->catfile(
    $repo_root,
    'isf',
    'atl_resolved_child_pin_egress_vector_pipeline.isf',
);
my $pin_egress_vector_multi_isf_file = File::Spec->catfile(
    $repo_root,
    'isf',
...
my $two_child_trigger_batch_isf_file = File::Spec->catfile(
    $repo_root,
    'isf',
    'atl_two_child_trigger_batch_pipeline.isf',
);
...
subtest 'ATL pin-ingress vector multi-route fixture lowers exact-width top inputs through parent into one resolved child' => sub {
    my ($files, $report) = lower_atl_fixture($pin_ingress_vector_multi_isf_file);

    is_deeply(
        sorted([keys %$files]),
        [
            'atl_resolved_child_pin_ingress_vector_multi_pipeline.fsm',
            'atl_resolved_child_pin_ingress_vector_multi_pipeline__worker.fsm',
            'atl_resolved_child_pin_ingress_vector_multi_pipeline_top.fsm',
        ],
        'pin-ingress vector multi-route lowering emits exactly the parent, resolved child, and generated ATL top FSM artifacts',
    );

    my $parent = $files->{'atl_resolved_child_pin_ingress_vector_multi_pipeline.fsm'};
    like($parent, qr/\A\(\?fsm:atl_resolved_child_pin_ingress_vector_multi_pipeline\b/,
        'pin-ingress vector multi-route parent uses the fixture module name');
    like($parent, qr/\(payload 8\)/, 'pin-ingress vector multi-route parent exposes the top payload input width');
    like($parent, qr/\(sideband 4\)/, 'pin-ingress vector multi-route parent exposes the top sideband input width');
    like($parent, qr/\(worker_payload 8\)/,
        'pin-ingress vector multi-route parent exposes the generated worker payload handoff width');
    like($parent, qr/\(worker_sideband 4\)/,
        'pin-ingress vector multi-route parent exposes the generated worker sideband handoff width');
    like($parent, qr/\brun_drive_1\b/, 'pin-ingress vector multi-route parent contains the first drive-call state');
    like($parent, qr/\brun_drive_2\b/, 'pin-ingress vector multi-route parent contains the second drive-call state');
    like($parent, qr/\brun_atl_trigger_3\b/,
        'pin-ingress vector multi-route parent triggers the child after both vector drive calls');
    like($parent, qr/\brun_await_4\b/,
        'pin-ingress vector multi-route parent waits for the child after the trigger');
    like($parent, qr/\(= \(feed_payload_start 1\)\)/,
        'pin-ingress vector multi-route first drive-call state pulses the payload route enable');
    like($parent, qr/\(= \(feed_sideband_start 1\)\)/,
        'pin-ingress vector multi-route second drive-call state pulses the sideband route enable');
    like($parent, qr/\(-feed_payload\s+\(<- \(worker_payload> payload\) <feed_payload_start\)\s+\)/s,
        'pin-ingress vector multi-route payload drive transfers the top payload into the worker handoff');
    like($parent, qr/\(-feed_sideband\s+\(<- \(worker_sideband> sideband\) <feed_sideband_start\)\s+\)/s,
        'pin-ingress vector multi-route sideband drive transfers the top sideband into the worker handoff');

    my $child = $files->{'atl_resolved_child_pin_ingress_vector_multi_pipeline__worker.fsm'};
    like($child, qr/\A\(\?fsm:atl_resolved_child_pin_ingress_vector_multi_pipeline__worker\b/,
        'pin-ingress vector multi-route child uses the resolved child module name');
    like($child, qr/\(\+interface\s+\(input payload\)\s+\(input sideband\)\s+\)/s,
        'pin-ingress vector multi-route child preserves both routed inputs as explicit generated interface roles');
    like($child, qr/\(payload 8\)/, 'pin-ingress vector multi-route child keeps the payload size declaration');
    like($child, qr/\(sideband 4\)/, 'pin-ingress vector multi-route child keeps the sideband size declaration');
    like($child, qr/\(process_start 1\)/, 'pin-ingress vector multi-route child keeps its authored process_start input');
    like($child, qr/\(done 1\)/, 'pin-ingress vector multi-route child keeps its authored done output');

    my $top = $files->{'atl_resolved_child_pin_ingress_vector_multi_pipeline_top.fsm'};
    like($top, qr/\A\(\?top:atl_resolved_child_pin_ingress_vector_multi_pipeline_top\b/,
        'pin-ingress vector multi-route generated top uses the fixture top module name');
    like($top, qr/\(\?ports:public_io\s+clk\s+rst_n\s+start\s+payload<8\s+sideband<4\s+done>\s+\)/s,
        'pin-ingress vector multi-route generated top exposes only parent public pins plus clock/reset');
    like($top, qr/\(payload atl_resolved_child_pin_ingress_vector_multi_pipeline\.payload\)/,
        'pin-ingress vector multi-route generated top wires top payload into the parent');
    like($top, qr/\(sideband atl_resolved_child_pin_ingress_vector_multi_pipeline\.sideband\)/,
        'pin-ingress vector multi-route generated top wires top sideband into the parent');
    like($top, qr/\(atl_resolved_child_pin_ingress_vector_multi_pipeline\.worker_payload worker\.payload\)/,
        'pin-ingress vector multi-route generated top wires the parent payload handoff into the child');
    like($top, qr/\(atl_resolved_child_pin_ingress_vector_multi_pipeline\.worker_sideband worker\.sideband\)/,
        'pin-ingress vector multi-route generated top wires the parent sideband handoff into the child');
    like($top, qr/\(atl_resolved_child_pin_ingress_vector_multi_pipeline\.worker_process_start worker\.process_start\)/,
        'pin-ingress vector multi-route generated top wires parent trigger handoff to the child transaction start input');
    like($top, qr/\(worker\.done atl_resolved_child_pin_ingress_vector_multi_pipeline\.worker_done\)/,
        'pin-ingress vector multi-route generated top wires child event pulse to the parent event handoff input');

    assert_pin_ingress_vector_multi_report_shape($report);
};

subtest 'ATL pin-ingress mixed route fixture lowers scalar and vector top inputs through parent into one resolved child' => sub {
    my ($files, $report) = lower_atl_fixture($pin_ingress_mixed_isf_file);

    is_deeply(
        sorted([keys %$files]),
        [
            'atl_resolved_child_pin_ingress_mixed_pipeline.fsm',
            'atl_resolved_child_pin_ingress_mixed_pipeline__worker.fsm',
            'atl_resolved_child_pin_ingress_mixed_pipeline_top.fsm',
        ],
        'pin-ingress mixed-route lowering emits exactly the parent, resolved child, and generated ATL top FSM artifacts',
    );

    my $parent = $files->{'atl_resolved_child_pin_ingress_mixed_pipeline.fsm'};
    like($parent, qr/\A\(\?fsm:atl_resolved_child_pin_ingress_mixed_pipeline\b/,
        'pin-ingress mixed-route parent uses the fixture module name');
    like($parent, qr/\(payload 8\)/,
        'pin-ingress mixed-route parent preserves the top payload input width');
    like($parent, qr/\(valid 1\)/,
        'pin-ingress mixed-route parent preserves the top valid input width');
    like($parent, qr/\(worker_payload 8\)/,
        'pin-ingress mixed-route parent exposes the generated worker payload handoff width');
    like($parent, qr/\(worker_valid 1\)/,
        'pin-ingress mixed-route parent exposes the generated worker valid handoff width');
    like($parent, qr/\brun_drive_1\b/,
        'pin-ingress mixed-route parent contains the vector drive-call state');
    like($parent, qr/\brun_drive_2\b/,
        'pin-ingress mixed-route parent contains the scalar drive-call state');
    like($parent, qr/\brun_atl_trigger_3\b/,
        'pin-ingress mixed-route parent triggers the child after both drive calls');
    like($parent, qr/\(-feed_payload\s+\(<- \(worker_payload> payload\) <feed_payload_start\)\s+\)/s,
        'pin-ingress mixed-route payload drive transfers the top payload into the worker handoff');
    like($parent, qr/\(-feed_valid\s+\(<- \(worker_valid> valid\) <feed_valid_start\)\s+\)/s,
        'pin-ingress mixed-route valid drive transfers the top valid bit into the worker handoff');

    my $child = $files->{'atl_resolved_child_pin_ingress_mixed_pipeline__worker.fsm'};
    like($child, qr/\A\(\?fsm:atl_resolved_child_pin_ingress_mixed_pipeline__worker\b/,
        'pin-ingress mixed-route child uses the resolved child module name');
    like($child, qr/\(\+interface\s+\(input payload\)\s+\(input valid\)\s+\)/s,
        'pin-ingress mixed-route child preserves both routed inputs as explicit generated interface roles');
    like($child, qr/\(payload 8\)/,
        'pin-ingress mixed-route child keeps the payload size declaration');
    like($child, qr/\(valid 1\)/,
        'pin-ingress mixed-route child keeps the valid size declaration');

    my $top = $files->{'atl_resolved_child_pin_ingress_mixed_pipeline_top.fsm'};
    like($top, qr/\(\?ports:public_io\s+clk\s+rst_n\s+start\s+payload<8\s+valid\s+done>\s+\)/s,
        'pin-ingress mixed-route generated top exposes only parent public pins plus clock/reset');
    like($top, qr/\(payload atl_resolved_child_pin_ingress_mixed_pipeline\.payload\)/,
        'pin-ingress mixed-route generated top wires top payload into the parent');
    like($top, qr/\(valid atl_resolved_child_pin_ingress_mixed_pipeline\.valid\)/,
        'pin-ingress mixed-route generated top wires top valid into the parent');
    like($top, qr/\(atl_resolved_child_pin_ingress_mixed_pipeline\.worker_payload worker\.payload\)/,
        'pin-ingress mixed-route generated top wires the parent payload handoff into the child');
    like($top, qr/\(atl_resolved_child_pin_ingress_mixed_pipeline\.worker_valid worker\.valid\)/,
        'pin-ingress mixed-route generated top wires the parent valid handoff into the child');

    assert_pin_ingress_mixed_report_shape($report);
};
...
subtest 'ATL pin-egress vector fixture lowers exact-width child output through parent to top output' => sub {
    my ($files, $report) = lower_atl_fixture($pin_egress_vector_isf_file);

    is_deeply(
        sorted([keys %$files]),
        [
            'atl_resolved_child_pin_egress_vector_pipeline.fsm',
            'atl_resolved_child_pin_egress_vector_pipeline__worker.fsm',
            'atl_resolved_child_pin_egress_vector_pipeline_top.fsm',
        ],
        'pin-egress vector lowering emits exactly the parent, resolved child, and generated ATL top FSM artifacts',
    );

    my $parent = $files->{'atl_resolved_child_pin_egress_vector_pipeline.fsm'};
    like($parent, qr/\A\(\?fsm:atl_resolved_child_pin_egress_vector_pipeline\b/,
        'pin-egress vector parent uses the fixture module name');
    like($parent, qr/\(result 8\)/, 'pin-egress vector parent preserves the top result output width');
    like($parent, qr/\(worker_payload 8\)/, 'pin-egress vector parent exposes the generated worker payload source handoff width');
    like($parent, qr/\brun_atl_trigger_1\b/, 'pin-egress vector parent triggers the child before waiting');
    like($parent, qr/\brun_await_2\b/, 'pin-egress vector parent waits for the child event before publishing');
    like($parent, qr/\brun_drive_3\b/, 'pin-egress vector parent contains the post-event drive-call state');
    like($parent, qr/\(= \(publish_result_start 1\)\)/,
        'pin-egress vector drive-call state pulses the named drive enable');
    like($parent, qr/\(-publish_result\s+\(<- \(result> worker_payload\) <publish_result_start\)\s+\)/s,
        'pin-egress vector named drive transfers the worker handoff into the top result');

    my $child = $files->{'atl_resolved_child_pin_egress_vector_pipeline__worker.fsm'};
    like($child, qr/\A\(\?fsm:atl_resolved_child_pin_egress_vector_pipeline__worker\b/,
        'pin-egress vector child uses the resolved child module name');
    like($child, qr/\(\+interface\s+\(output payload\)\s+\)/s,
        'pin-egress vector child preserves the declared payload output as an explicit generated interface role');
    like($child, qr/\(payload 8\)/, 'pin-egress vector child keeps the payload size declaration');
    like($child, qr/\(process_start 1\)/, 'pin-egress vector child keeps its authored process_start input');
    like($child, qr/\(done 1\)/, 'pin-egress vector child keeps its authored done output');

    my $top = $files->{'atl_resolved_child_pin_egress_vector_pipeline_top.fsm'};
    like($top, qr/\A\(\?top:atl_resolved_child_pin_egress_vector_pipeline_top\b/,
        'pin-egress vector generated top uses the fixture top module name');
    like($top, qr/\(\?ports:public_io\s+clk\s+rst_n\s+start\s+result>8\s+done>\s+\)/s,
        'pin-egress vector generated top exposes only parent public pins plus clock/reset');
    like($top, qr/\(atl_resolved_child_pin_egress_vector_pipeline\.result result\)/,
        'pin-egress vector generated top wires parent result to the top result output');
    like($top, qr/\(worker\.payload atl_resolved_child_pin_egress_vector_pipeline\.worker_payload\)/,
        'pin-egress vector generated top wires the child payload output into the parent handoff');
    like($top, qr/\(atl_resolved_child_pin_egress_vector_pipeline\.worker_process_start worker\.process_start\)/,
        'pin-egress vector generated top wires parent trigger handoff to the child transaction start input');
    like($top, qr/\(worker\.done atl_resolved_child_pin_egress_vector_pipeline\.worker_done\)/,
        'pin-egress vector generated top wires child event pulse to the parent event handoff input');

    assert_pin_egress_vector_report_shape($report);
};

subtest 'ATL pin-egress vector multi-route fixture lowers multiple exact-width child outputs through parent to top outputs' => sub {
    my ($files, $report) = lower_atl_fixture($pin_egress_vector_multi_isf_file);

    is_deeply(
        sorted([keys %$files]),
        [
            'atl_resolved_child_pin_egress_vector_multi_pipeline.fsm',
            'atl_resolved_child_pin_egress_vector_multi_pipeline__worker.fsm',
            'atl_resolved_child_pin_egress_vector_multi_pipeline_top.fsm',
        ],
        'pin-egress vector multi-route lowering emits exactly the parent, resolved child, and generated ATL top FSM artifacts',
    );

    my $parent = $files->{'atl_resolved_child_pin_egress_vector_multi_pipeline.fsm'};
    like($parent, qr/\A\(\?fsm:atl_resolved_child_pin_egress_vector_multi_pipeline\b/,
        'pin-egress vector multi-route parent uses the fixture module name');
    like($parent, qr/\(result 8\)/,
        'pin-egress vector multi-route parent preserves the top result output width');
    like($parent, qr/\(status 4\)/,
        'pin-egress vector multi-route parent preserves the top status output width');
    like($parent, qr/\(worker_payload 8\)/,
        'pin-egress vector multi-route parent exposes the generated worker payload source handoff width');
    like($parent, qr/\(worker_status 4\)/,
        'pin-egress vector multi-route parent exposes the generated worker status source handoff width');
    like($parent, qr/\brun_atl_trigger_1\b/,
        'pin-egress vector multi-route parent triggers the child before waiting');
    like($parent, qr/\brun_await_2\b/,
        'pin-egress vector multi-route parent waits for the child event before publishing');
    like($parent, qr/\brun_drive_3\b/,
        'pin-egress vector multi-route parent contains the first post-event drive-call state');
    like($parent, qr/\brun_drive_4\b/,
        'pin-egress vector multi-route parent contains the second post-event drive-call state');
    like($parent, qr/\(= \(publish_result_start 1\)\)/,
        'pin-egress vector multi-route first drive-call state pulses the result route enable');
    like($parent, qr/\(= \(publish_status_start 1\)\)/,
        'pin-egress vector multi-route second drive-call state pulses the status route enable');
    like($parent, qr/\(-publish_result\s+\(<- \(result> worker_payload\) <publish_result_start\)\s+\)/s,
        'pin-egress vector multi-route result drive transfers the worker payload handoff into the top result');
    like($parent, qr/\(-publish_status\s+\(<- \(status> worker_status\) <publish_status_start\)\s+\)/s,
        'pin-egress vector multi-route status drive transfers the worker status handoff into the top status');

    my $child = $files->{'atl_resolved_child_pin_egress_vector_multi_pipeline__worker.fsm'};
    like($child, qr/\A\(\?fsm:atl_resolved_child_pin_egress_vector_multi_pipeline__worker\b/,
        'pin-egress vector multi-route child uses the resolved child module name');
    like($child, qr/\(\+interface\s+\(output payload\)\s+\(output status\)\s+\)/s,
        'pin-egress vector multi-route child preserves both routed outputs as explicit generated interface roles');
    like($child, qr/\(payload 8\)/,
        'pin-egress vector multi-route child keeps the payload size declaration');
    like($child, qr/\(status 4\)/,
        'pin-egress vector multi-route child keeps the status size declaration');
    like($child, qr/\(process_start 1\)/,
        'pin-egress vector multi-route child keeps its authored process_start input');
    like($child, qr/\(done 1\)/,
        'pin-egress vector multi-route child keeps its authored done output');

    my $top = $files->{'atl_resolved_child_pin_egress_vector_multi_pipeline_top.fsm'};
    like($top, qr/\A\(\?top:atl_resolved_child_pin_egress_vector_multi_pipeline_top\b/,
        'pin-egress vector multi-route generated top uses the fixture top module name');
    like($top, qr/\(\?ports:public_io\s+clk\s+rst_n\s+start\s+result>8\s+status>4\s+done>\s+\)/s,
        'pin-egress vector multi-route generated top exposes only parent public pins plus clock/reset');
    like($top, qr/\(atl_resolved_child_pin_egress_vector_multi_pipeline\.result result\)/,
        'pin-egress vector multi-route generated top wires parent result to the top result output');
    like($top, qr/\(atl_resolved_child_pin_egress_vector_multi_pipeline\.status status\)/,
        'pin-egress vector multi-route generated top wires parent status to the top status output');
    like($top, qr/\(worker\.payload atl_resolved_child_pin_egress_vector_multi_pipeline\.worker_payload\)/,
        'pin-egress vector multi-route generated top wires the child payload output into the parent handoff');
    like($top, qr/\(worker\.status atl_resolved_child_pin_egress_vector_multi_pipeline\.worker_status\)/,
        'pin-egress vector multi-route generated top wires the child status output into the parent handoff');
    like($top, qr/\(atl_resolved_child_pin_egress_vector_multi_pipeline\.worker_process_start worker\.process_start\)/,
        'pin-egress vector multi-route generated top wires parent trigger handoff to the child transaction start input');
    like($top, qr/\(worker\.done atl_resolved_child_pin_egress_vector_multi_pipeline\.worker_done\)/,
...
subtest 'ATL two-child trigger-batch fixture lowers same-cycle trigger handoffs through one generated top' => sub {
    my ($files, $report) = lower_atl_fixture($two_child_trigger_batch_isf_file);

    is_deeply(
        sorted([keys %$files]),
        [
            'atl_two_child_trigger_batch_pipeline.fsm',
            'atl_two_child_trigger_batch_pipeline__reader.fsm',
            'atl_two_child_trigger_batch_pipeline__writer.fsm',
            'atl_two_child_trigger_batch_pipeline_top.fsm',
        ],
        'two-child trigger-batch lowering emits exactly the parent, two resolved children, and generated ATL top FSM artifacts',
    );

    my $parent = $files->{'atl_two_child_trigger_batch_pipeline.fsm'};
    like($parent, qr/\A\(\?fsm:atl_two_child_trigger_batch_pipeline\b/,
        'two-child trigger-batch parent uses the fixture module name');
    like($parent, qr/\(reader_capture_start 1\)/,
        'two-child trigger-batch parent exposes the reader trigger handoff output');
    like($parent, qr/\(reader_done 1\)/,
        'two-child trigger-batch parent exposes the reader event handoff input');
    like($parent, qr/\(writer_emit_start 1\)/,
        'two-child trigger-batch parent exposes the writer trigger handoff output');
    like($parent, qr/\(writer_done 1\)/,
        'two-child trigger-batch parent exposes the writer event handoff input');
    like($parent, qr/\brun_atl_trigger_batch_1\b/,
        'two-child trigger-batch parent contains one same-cycle trigger batch state');
    like($parent, qr/\brun_await_2\b/,
        'two-child trigger-batch parent contains the reader await state');
    like($parent, qr/\brun_await_3\b/,
        'two-child trigger-batch parent contains the writer await state');
    unlike($parent, qr/\brun_atl_trigger_3\b/,
        'two-child trigger-batch parent does not split the writer handoff into a later trigger state');
    like($parent, qr/\(run_atl_trigger_batch_1\s+\(<1 \(reader_capture_start> 1\)\)\s+\(<1 \(writer_emit_start> 1\)\)\s+\(-> run_await_2\)\s+\)/s,
        'trigger-batch state pulses both child starts before the first wait');
    like($parent, qr/\(<reader_done\s+\(-> run_await_3\)\s+\)/,
        'reader await state advances to the writer wait');
    like($parent, qr/\(<writer_done\s+\(-> run_done_4\)\s+\)/,
        'writer await state advances to completion');

    my $reader = $files->{'atl_two_child_trigger_batch_pipeline__reader.fsm'};
    like($reader, qr/\A\(\?fsm:atl_two_child_trigger_batch_pipeline__reader\b/,
        'trigger-batch reader child uses the resolved reader module name');
    like($reader, qr/\(capture_start 1\)/,
        'trigger-batch reader child keeps its authored capture_start input');
    like($reader, qr/\(done 1\)/, 'trigger-batch reader child keeps its authored done output');
    like($reader, qr/\bcapture_idle_0\b/,
        'trigger-batch reader child keeps its capture transaction entry state');

    my $writer = $files->{'atl_two_child_trigger_batch_pipeline__writer.fsm'};
    like($writer, qr/\A\(\?fsm:atl_two_child_trigger_batch_pipeline__writer\b/,
        'trigger-batch writer child uses the resolved writer module name');
    like($writer, qr/\(emit_start 1\)/,
        'trigger-batch writer child keeps its authored emit_start input');
    like($writer, qr/\(done 1\)/, 'trigger-batch writer child keeps its authored done output');
    like($writer, qr/\bemit_idle_0\b/,
        'trigger-batch writer child keeps its emit transaction entry state');

    my $top = $files->{'atl_two_child_trigger_batch_pipeline_top.fsm'};
    like($top, qr/\A\(\?top:atl_two_child_trigger_batch_pipeline_top\b/,
        'two-child trigger-batch generated top uses the fixture top module name');
    like($top, qr/\(\?ports:public_io\s+clk\s+rst_n\s+start\s+done>\s+\)/s,
        'two-child trigger-batch generated top exposes only parent public pins plus clock/reset');
    like($top, qr/\(\?fsmc:atl_two_child_trigger_batch_pipeline atl_two_child_trigger_batch_pipeline\)/,
        'two-child trigger-batch generated top instantiates the scheduled parent');
    like($top, qr/\(\?fsmc:reader atl_two_child_trigger_batch_pipeline__reader\)/,
        'two-child trigger-batch generated top instantiates the resolved reader child');
    like($top, qr/\(\?fsmc:writer atl_two_child_trigger_batch_pipeline__writer\)/,
        'two-child trigger-batch generated top instantiates the resolved writer child');
    like($top, qr/\(start atl_two_child_trigger_batch_pipeline\.start\)/,
        'two-child trigger-batch generated top wires top start into the parent');
    like($top, qr/\(atl_two_child_trigger_batch_pipeline\.done done\)/,
        'two-child trigger-batch generated top wires parent done to the top output');
    like($top, qr/\(atl_two_child_trigger_batch_pipeline\.reader_capture_start reader\.capture_start\)/,
        'two-child trigger-batch generated top wires parent reader trigger handoff to the reader transaction start input');
    like($top, qr/\(reader\.done atl_two_child_trigger_batch_pipeline\.reader_done\)/,
        'two-child trigger-batch generated top wires reader event pulse to the parent event handoff input');
    like($top, qr/\(atl_two_child_trigger_batch_pipeline\.writer_emit_start writer\.emit_start\)/,
        'two-child trigger-batch generated top wires parent writer trigger handoff to the writer transaction start input');
    like($top, qr/\(writer\.done atl_two_child_trigger_batch_pipeline\.writer_done\)/,
        'two-child trigger-batch generated top wires writer event pulse to the parent event handoff input');

    assert_two_child_trigger_batch_report_shape($report);
};
...
subtest 'ATL pin-ingress vector multi-route fixture strict schedule JSON matches the in-process report' => sub {
    my (undef, $in_process_report) = lower_atl_fixture($pin_ingress_vector_multi_isf_file);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
            '--strict',
            '--quiet',
            '--emit-schedule-json',
            $pin_ingress_vector_multi_isf_file,
        ],
        'pin-ingress vector multi-route strict schedule JSON generation',
    );

    ok($success, 'strict schedule JSON generation succeeds for the ATL pin-ingress vector multi-route fixture');
    is($stderr, '', 'pin-ingress vector multi-route strict schedule JSON generation keeps stderr clean');
    is_deeply(
        decode_json($stdout),
        $in_process_report,
        'pin-ingress vector multi-route strict schedule JSON generation matches the in-process report',
    );
};

subtest 'ATL pin-ingress mixed route fixture strict schedule JSON matches the in-process report' => sub {
    my (undef, $in_process_report) = lower_atl_fixture($pin_ingress_mixed_isf_file);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
            '--strict',
            '--quiet',
            '--emit-schedule-json',
            $pin_ingress_mixed_isf_file,
        ],
        'pin-ingress mixed route strict schedule JSON generation',
    );

    ok($success, 'strict schedule JSON generation succeeds for the ATL pin-ingress mixed route fixture');
    is($stderr, '', 'pin-ingress mixed route strict schedule JSON generation keeps stderr clean');
    is_deeply(
        decode_json($stdout),
        $in_process_report,
        'pin-ingress mixed route strict schedule JSON generation matches the in-process report',
    );
};
...
subtest 'ATL pin-egress vector fixture strict schedule JSON matches the in-process report' => sub {
    my (undef, $in_process_report) = lower_atl_fixture($pin_egress_vector_isf_file);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
            '--strict',
            '--quiet',
            '--emit-schedule-json',
            $pin_egress_vector_isf_file,
        ],
        'pin-egress vector strict schedule JSON generation',
    );

    ok($success, 'strict schedule JSON generation succeeds for the ATL pin-egress vector fixture');
    is($stderr, '', 'pin-egress vector strict schedule JSON generation keeps stderr clean');
    is_deeply(
        decode_json($stdout),
        $in_process_report,
        'pin-egress vector strict schedule JSON generation matches the in-process report',
    );
};

subtest 'ATL pin-egress vector multi-route fixture strict schedule JSON matches the in-process report' => sub {
    my (undef, $in_process_report) = lower_atl_fixture($pin_egress_vector_multi_isf_file);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
            '--strict',
            '--quiet',
            '--emit-schedule-json',
            $pin_egress_vector_multi_isf_file,
        ],
        'pin-egress vector multi-route strict schedule JSON generation',
    );

    ok($success, 'strict schedule JSON generation succeeds for the ATL pin-egress vector multi-route fixture');
    is($stderr, '', 'pin-egress vector multi-route strict schedule JSON generation keeps stderr clean');
    is_deeply(
        decode_json($stdout),
        $in_process_report,
        'pin-egress vector multi-route strict schedule JSON generation matches the in-process report',
    );
};
...
subtest 'ATL two-child trigger-batch fixture strict schedule JSON matches the in-process report' => sub {
    my (undef, $in_process_report) = lower_atl_fixture($two_child_trigger_batch_isf_file);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
            '--strict',
            '--quiet',
            '--emit-schedule-json',
            $two_child_trigger_batch_isf_file,
        ],
        'two-child trigger-batch strict schedule JSON generation',
    );

    ok($success, 'strict schedule JSON generation succeeds for the ATL two-child trigger-batch fixture');
    is($stderr, '', 'two-child trigger-batch strict schedule JSON generation keeps stderr clean');
    is_deeply(
        decode_json($stdout),
        $in_process_report,
        'two-child trigger-batch strict schedule JSON generation matches the in-process report',
    );
};
...
subtest 'ATL pin-ingress vector multi-route fixture strict outdir lowering writes the generated top' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
        ...
        'pin-ingress vector multi-route strict outdir lowering',
    );

    ok($success, 'strict outdir lowering succeeds for the ATL pin-ingress vector multi-route fixture');
    like($stdout, qr/Wrote: .*atl_resolved_child_pin_ingress_vector_multi_pipeline_top\.fsm/,
        'pin-ingress vector multi-route strict outdir lowering reports the written generated top');
    is($stderr, '', 'pin-ingress vector multi-route strict outdir lowering keeps stderr clean');
    is_deeply(
        sorted([fsm_basenames_in($dir)]),
        expected_fsm_basenames_for_source($pin_ingress_vector_multi_isf_file),
        'pin-ingress vector multi-route strict outdir lowering writes the parent, resolved child, and generated top files',
    );
};

subtest 'ATL pin-ingress mixed route fixture strict outdir lowering writes the generated top' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
        ...
        'pin-ingress mixed route strict outdir lowering',
    );

    ok($success, 'strict outdir lowering succeeds for the ATL pin-ingress mixed route fixture');
    like($stdout, qr/Wrote: .*atl_resolved_child_pin_ingress_mixed_pipeline_top\.fsm/,
        'pin-ingress mixed route strict outdir lowering reports the written generated top');
    is($stderr, '', 'pin-ingress mixed route strict outdir lowering keeps stderr clean');
    is_deeply(
        sorted([fsm_basenames_in($dir)]),
        expected_fsm_basenames_for_source($pin_ingress_mixed_isf_file),
        'pin-ingress mixed route strict outdir lowering writes the parent, resolved child, and generated top files',
    );
};
...
subtest 'ATL pin-egress vector fixture strict outdir lowering writes the generated top' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
        ...
        'pin-egress vector strict outdir lowering',
    );

    ok($success, 'strict outdir lowering succeeds for the ATL pin-egress vector fixture');
    like($stdout, qr/Wrote: .*atl_resolved_child_pin_egress_vector_pipeline_top\.fsm/,
        'pin-egress vector strict outdir lowering reports the written generated top');
    is($stderr, '', 'pin-egress vector strict outdir lowering keeps stderr clean');
    is_deeply(
        sorted([fsm_basenames_in($dir)]),
        expected_fsm_basenames_for_source($pin_egress_vector_isf_file),
        'pin-egress vector strict outdir lowering writes the parent, resolved child, and generated top files',
    );
};

subtest 'ATL pin-egress vector multi-route fixture strict outdir lowering writes the generated top' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
        ...
        'pin-egress vector multi-route strict outdir lowering',
    );

    ok($success, 'strict outdir lowering succeeds for the ATL pin-egress vector multi-route fixture');
    like($stdout, qr/Wrote: .*atl_resolved_child_pin_egress_vector_multi_pipeline_top\.fsm/,
        'pin-egress vector multi-route strict outdir lowering reports the written generated top');
    is($stderr, '', 'pin-egress vector multi-route strict outdir lowering keeps stderr clean');
    is_deeply(
        sorted([fsm_basenames_in($dir)]),
        expected_fsm_basenames_for_source($pin_egress_vector_multi_isf_file),
        'pin-egress vector multi-route strict outdir lowering writes the parent, resolved child, and generated top files',
    );
};
...
subtest 'ATL two-child trigger-batch fixture strict outdir lowering writes the generated top' => sub {
    my $dir = tempdir(CLEANUP => 1);
    my ($success, $stdout, $stderr) = run_cli(
        [
            './bin/fsmgen',
        ...
        'two-child trigger-batch strict outdir lowering',
    );

    ok($success, 'strict outdir lowering succeeds for the ATL two-child trigger-batch fixture');
    like($stdout, qr/Wrote: .*atl_two_child_trigger_batch_pipeline_top\.fsm/,
        'two-child trigger-batch strict outdir lowering reports the written generated top');
    is($stderr, '', 'two-child trigger-batch strict outdir lowering keeps stderr clean');
    is_deeply(
        sorted([fsm_basenames_in($dir)]),
        expected_fsm_basenames_for_source($two_child_trigger_batch_isf_file),
        'two-child trigger-batch strict outdir lowering writes the parent, resolved children, and generated top files',
    );
};
...
subtest 'ATL pin-ingress vector multi-route fixture reaches generated-top HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_resolved_child_pin_ingress_vector_multi_pipeline_plain.sv');
    my $plain = generate_hdl(
        $plain_hdl,
        [],
        'pin-ingress vector multi-route plain HDL generation',
        $pin_ingress_vector_multi_isf_file,
    );

    assert_pin_ingress_vector_multi_generated_top_hdl($plain, 'pin-ingress vector multi-route plain HDL');

    my $strict_dir = tempdir(CLEANUP => 1);
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_resolved_child_pin_ingress_vector_multi_pipeline_strict.sv');
    my $strict = generate_hdl(
        $strict_hdl,
        ['--strict'],
        'pin-ingress vector multi-route strict HDL generation',
        $pin_ingress_vector_multi_isf_file,
    );

    assert_pin_ingress_vector_multi_generated_top_hdl($strict, 'pin-ingress vector multi-route strict HDL');
};

subtest 'ATL pin-ingress mixed route fixture reaches generated-top HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_resolved_child_pin_ingress_mixed_pipeline_plain.sv');
    my $plain = generate_hdl(
        $plain_hdl,
        [],
        'pin-ingress mixed route plain HDL generation',
        $pin_ingress_mixed_isf_file,
    );

    assert_pin_ingress_mixed_generated_top_hdl($plain, 'pin-ingress mixed route plain HDL');

    my $strict_dir = tempdir(CLEANUP => 1);
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_resolved_child_pin_ingress_mixed_pipeline_strict.sv');
    my $strict = generate_hdl(
        $strict_hdl,
        ['--strict'],
        'pin-ingress mixed route strict HDL generation',
        $pin_ingress_mixed_isf_file,
    );

    assert_pin_ingress_mixed_generated_top_hdl($strict, 'pin-ingress mixed route strict HDL');
};
...
subtest 'ATL pin-egress vector fixture reaches generated-top HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_resolved_child_pin_egress_vector_pipeline_plain.sv');
    my $plain = generate_hdl(
        $plain_hdl,
        [],
        'pin-egress vector plain HDL generation',
        $pin_egress_vector_isf_file,
    );

    assert_pin_egress_vector_generated_top_hdl($plain, 'pin-egress vector plain HDL');

    my $strict_dir = tempdir(CLEANUP => 1);
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_resolved_child_pin_egress_vector_pipeline_strict.sv');
    my $strict = generate_hdl(
        $strict_hdl,
        ['--strict'],
        'pin-egress vector strict HDL generation',
        $pin_egress_vector_isf_file,
    );

    assert_pin_egress_vector_generated_top_hdl($strict, 'pin-egress vector strict HDL');
};

subtest 'ATL pin-egress vector multi-route fixture reaches generated-top HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_resolved_child_pin_egress_vector_multi_pipeline_plain.sv');
    my $plain = generate_hdl(
        $plain_hdl,
        [],
        'pin-egress vector multi-route plain HDL generation',
        $pin_egress_vector_multi_isf_file,
    );

    assert_pin_egress_vector_multi_generated_top_hdl($plain, 'pin-egress vector multi-route plain HDL');

    my $strict_dir = tempdir(CLEANUP => 1);
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_resolved_child_pin_egress_vector_multi_pipeline_strict.sv');
    my $strict = generate_hdl(
        $strict_hdl,
        ['--strict'],
        'pin-egress vector multi-route strict HDL generation',
        $pin_egress_vector_multi_isf_file,
    );
...
subtest 'ATL two-child trigger-batch fixture reaches generated-top HDL generation' => sub {
    my $plain_dir = tempdir(CLEANUP => 1);
    my $plain_hdl = File::Spec->catfile($plain_dir, 'atl_two_child_trigger_batch_pipeline_plain.sv');
    my $plain = generate_hdl(
        $plain_hdl,
        [],
        'two-child trigger-batch plain HDL generation',
        $two_child_trigger_batch_isf_file,
    );

    assert_two_child_trigger_batch_generated_top_hdl($plain, 'two-child trigger-batch plain HDL');

    my $strict_dir = tempdir(CLEANUP => 1);
    my $strict_hdl = File::Spec->catfile($strict_dir, 'atl_two_child_trigger_batch_pipeline_strict.sv');
    my $strict = generate_hdl(
        $strict_hdl,
        ['--strict'],
        'two-child trigger-batch strict HDL generation',
        $two_child_trigger_batch_isf_file,
    );

    assert_two_child_trigger_batch_generated_top_hdl($strict, 'two-child trigger-batch strict HDL');
};
...
LIBRARY
    ...
    lower_source_fails_like(
        pin_ingress_vector_multi_atl_fixture_variant(3),
        qr/ATL generated-child pin-ingress data movement 'feed_sideband' top-level input pin 'pins\.sideband' width 4 does not match sink endpoint 'worker\.sideband' width 3; width adaptation remains deferred/,
        'pin-ingress vector multi-route data route fails closed when one top input and child input width differ',
    );

    lower_source_fails_like(
        pin_ingress_mixed_atl_fixture_variant(3),
        qr/ATL generated-child pin-ingress data movement 'feed_payload' top-level input pin 'pins\.payload' width 8 does not match sink endpoint 'worker\.payload' width 3; width adaptation remains deferred/,
        'pin-ingress mixed route data route fails closed when the vector top input and child input widths differ',
    );
...
LIBRARY
    ...
    lower_source_fails_like(
        pin_egress_vector_atl_fixture_variant(4),
        qr/ATL generated-child pin-egress data movement 'publish_result' source endpoint 'worker\.payload' width 4 does not match top-level output pin 'pins\.result' width 8; width adaptation remains deferred/,
        'pin-egress vector data route fails closed when child output and top output widths differ',
    );
...
sub assert_pin_ingress_vector_multi_report_shape {
    my ($report) = @_;

    is($report->{source}, 'atl_resolved_child_pin_ingress_vector_multi_pipeline.isf',
        'pin-ingress vector multi-route schedule report names the fixture');
    is($report->{scheduled_fsm}, 'atl_resolved_child_pin_ingress_vector_multi_pipeline.fsm',
        'pin-ingress vector multi-route schedule report names the scheduled parent FSM');
    is($report->{inputs}, 4, 'pin-ingress vector multi-route report input count includes payload, sideband, and worker event handoff');
    is($report->{outputs}, 4, 'pin-ingress vector multi-route report output count includes done, trigger, and data handoffs');
    is($report->{port_count}, 8, 'pin-ingress vector multi-route report port count includes public and generated handoff ports');
    is($report->{state_count}, 7, 'pin-ingress vector multi-route report state count includes two drives, trigger, await, done, and timeout states');
    is_deeply($report->{compile_issues}, [], 'pin-ingress vector multi-route schedule report has no compile issues');
    is_deeply(
        $report->{dt_blocks},
        [
            {
        ...
        'pin-ingress vector multi-route schedule report records both vector transfer drive bodies',
    );
    is_deeply(
        $report->{transactions},
        [
        ...
        'pin-ingress vector multi-route schedule report records both drives before trigger and await',
    );

    my $actor_network = $report->{actor_network};
    is($actor_network->{kind}, 'static_declaration', 'pin-ingress vector multi-route actor network kind');
    is_deeply(
        $actor_network->{generated_tops},
        [
            {
                kind                 => 'resolved_child_trigger_event_handoff',
                top_module           => 'atl_resolved_child_pin_ingress_vector_multi_pipeline_top',
                top_fsm              => 'atl_resolved_child_pin_ingress_vector_multi_pipeline_top.fsm',
                parent_module        => 'atl_resolved_child_pin_ingress_vector_multi_pipeline',
                parent_scheduled_fsm => 'atl_resolved_child_pin_ingress_vector_multi_pipeline.fsm',
                instance             => 'worker',
                child_module         => 'atl_resolved_child_pin_ingress_vector_multi_pipeline__worker',
                child_scheduled_fsm  => 'atl_resolved_child_pin_ingress_vector_multi_pipeline__worker.fsm',
                target_transaction   => 'process',
                trigger_parent_port  => 'worker_process_start',
                trigger_child_port   => 'process_start',
                event                => 'done',
        ...
        'pin-ingress vector multi-route report records the generated ATL top without private data-link internals',
    );
    is_deeply(
        $actor_network->{data_movements},
        [
        ...
        'pin-ingress vector multi-route report records both public exact-width pin-to-child data movements',
    );
    is_deeply(
        $actor_network->{transaction_triggers},
        [
        ...
        'pin-ingress vector multi-route report records the worker process trigger handoff',
    );
    is_deeply(
        $actor_network->{event_waits},
        [
        ...
        'pin-ingress vector multi-route report records the worker done event handoff',
    );
}

sub assert_pin_ingress_mixed_report_shape {
    my ($report) = @_;

    is($report->{source}, 'atl_resolved_child_pin_ingress_mixed_pipeline.isf',
        'pin-ingress mixed route schedule report names the fixture');
    is($report->{scheduled_fsm}, 'atl_resolved_child_pin_ingress_mixed_pipeline.fsm',
        'pin-ingress mixed route schedule report names the scheduled parent FSM');
    is($report->{inputs}, 4, 'pin-ingress mixed route report input count includes payload, valid, and worker event handoff');
    is($report->{outputs}, 4, 'pin-ingress mixed route report output count includes done, trigger, and data handoffs');
    is($report->{port_count}, 8, 'pin-ingress mixed route report port count includes public and generated handoff ports');
    is($report->{state_count}, 7, 'pin-ingress mixed route report state count includes two drives, trigger, await, done, and timeout states');
    is_deeply($report->{compile_issues}, [], 'pin-ingress mixed route schedule report has no compile issues');
    is_deeply(
        $report->{dt_blocks},
        [
            {
        ...
        'pin-ingress mixed route schedule report records the vector and scalar transfer drive bodies',
    );
    is_deeply(
        $report->{transactions},
        [
        ...
        'pin-ingress mixed route schedule report records both drives before trigger and await',
    );

    my $actor_network = $report->{actor_network};
    is($actor_network->{kind}, 'static_declaration', 'pin-ingress mixed route actor network kind');
    is_deeply(
        $actor_network->{generated_tops},
        [
            {
                kind                 => 'resolved_child_trigger_event_handoff',
                top_module           => 'atl_resolved_child_pin_ingress_mixed_pipeline_top',
                top_fsm              => 'atl_resolved_child_pin_ingress_mixed_pipeline_top.fsm',
                parent_module        => 'atl_resolved_child_pin_ingress_mixed_pipeline',
                parent_scheduled_fsm => 'atl_resolved_child_pin_ingress_mixed_pipeline.fsm',
                instance             => 'worker',
                child_module         => 'atl_resolved_child_pin_ingress_mixed_pipeline__worker',
                child_scheduled_fsm  => 'atl_resolved_child_pin_ingress_mixed_pipeline__worker.fsm',
                target_transaction   => 'process',
                trigger_parent_port  => 'worker_process_start',
                trigger_child_port   => 'process_start',
                event                => 'done',
        ...
        'pin-ingress mixed route report records the generated ATL top without private data-link internals',
    );
    is_deeply(
        $actor_network->{data_movements},
        [
        ...
        'pin-ingress mixed route report records route-local scalar and exact-width pin-to-child data movements',
    );
    is_deeply(
        $actor_network->{transaction_triggers},
        [
        ...
        'pin-ingress mixed route report records the worker process trigger handoff',
    );
    is_deeply(
        $actor_network->{event_waits},
        [
        ...
        'pin-ingress mixed route report records the worker done event handoff',
    );
}
...
sub assert_pin_egress_vector_report_shape {
    my ($report) = @_;

    is($report->{source}, 'atl_resolved_child_pin_egress_vector_pipeline.isf',
        'pin-egress vector schedule report names the fixture');
    is($report->{scheduled_fsm}, 'atl_resolved_child_pin_egress_vector_pipeline.fsm',
        'pin-egress vector schedule report names the scheduled parent FSM');
    is($report->{inputs}, 3, 'pin-egress vector report input count includes start, worker event, and worker payload handoff');
    is($report->{outputs}, 3, 'pin-egress vector report output count includes result, done, and trigger handoff');
    is($report->{port_count}, 6, 'pin-egress vector report port count includes public and generated handoff ports');
    is($report->{state_count}, 6, 'pin-egress vector report state count includes trigger, await, drive, done, and timeout states');
    is_deeply($report->{compile_issues}, [], 'pin-egress vector schedule report has no compile issues');
    is_deeply(
        $report->{dt_blocks},
        [
            {
        ...
        'pin-egress vector schedule report records the vector transfer drive body',
    );
    is_deeply(
        $report->{transactions},
        [
        ...
        'pin-egress vector schedule report records the trigger, await, then drive state order',
    );

    my $actor_network = $report->{actor_network};
    is($actor_network->{kind}, 'static_declaration', 'pin-egress vector actor network kind');
    is_deeply(
        $actor_network->{generated_tops},
        [
            {
                kind                 => 'resolved_child_trigger_event_handoff',
                top_module           => 'atl_resolved_child_pin_egress_vector_pipeline_top',
                top_fsm              => 'atl_resolved_child_pin_egress_vector_pipeline_top.fsm',
                parent_module        => 'atl_resolved_child_pin_egress_vector_pipeline',
                parent_scheduled_fsm => 'atl_resolved_child_pin_egress_vector_pipeline.fsm',
                instance             => 'worker',
                child_module         => 'atl_resolved_child_pin_egress_vector_pipeline__worker',
                child_scheduled_fsm  => 'atl_resolved_child_pin_egress_vector_pipeline__worker.fsm',
                target_transaction   => 'process',
                trigger_parent_port  => 'worker_process_start',
                trigger_child_port   => 'process_start',
                event                => 'done',
        ...
        'pin-egress vector report records the generated ATL top without private data-link internals',
    );
    is_deeply(
        $actor_network->{data_movements},
        [
        ...
        'pin-egress vector report records the public exact-width child-to-pin data movement',
    );
    is_deeply(
        $actor_network->{transaction_triggers},
        [
        ...
        'pin-egress vector report records the worker process trigger handoff',
    );
    is_deeply(
        $actor_network->{event_waits},
        [
        ...
        'pin-egress vector report records the worker done event handoff',
    );
}

sub assert_pin_egress_vector_multi_report_shape {
    my ($report) = @_;

    is($report->{source}, 'atl_resolved_child_pin_egress_vector_multi_pipeline.isf',
        'pin-egress vector multi-route schedule report names the fixture');
    is($report->{scheduled_fsm}, 'atl_resolved_child_pin_egress_vector_multi_pipeline.fsm',
        'pin-egress vector multi-route schedule report names the scheduled parent FSM');
    is($report->{inputs}, 4, 'pin-egress vector multi-route report input count includes start, worker event, and two worker data handoffs');
    is($report->{outputs}, 4, 'pin-egress vector multi-route report output count includes result, status, done, and trigger handoff');
    is($report->{port_count}, 8, 'pin-egress vector multi-route report port count includes public and generated handoff ports');
    is($report->{state_count}, 7, 'pin-egress vector multi-route report state count includes trigger, await, two drives, done, and timeout states');
    is_deeply($report->{compile_issues}, [], 'pin-egress vector multi-route schedule report has no compile issues');
    is_deeply(
        $report->{dt_blocks},
        [
            {
        ...
        'pin-egress vector multi-route schedule report records both vector transfer drive bodies',
    );
    is_deeply(
        $report->{transactions},
        [
        ...
        'pin-egress vector multi-route schedule report records trigger, await, then both drives',
    );

    my $actor_network = $report->{actor_network};
    is($actor_network->{kind}, 'static_declaration', 'pin-egress vector multi-route actor network kind');
    is_deeply(
        $actor_network->{generated_tops},
        [
            {
                kind                 => 'resolved_child_trigger_event_handoff',
                top_module           => 'atl_resolved_child_pin_egress_vector_multi_pipeline_top',
                top_fsm              => 'atl_resolved_child_pin_egress_vector_multi_pipeline_top.fsm',
                parent_module        => 'atl_resolved_child_pin_egress_vector_multi_pipeline',
                parent_scheduled_fsm => 'atl_resolved_child_pin_egress_vector_multi_pipeline.fsm',
                instance             => 'worker',
                child_module         => 'atl_resolved_child_pin_egress_vector_multi_pipeline__worker',
                child_scheduled_fsm  => 'atl_resolved_child_pin_egress_vector_multi_pipeline__worker.fsm',
                target_transaction   => 'process',
                trigger_parent_port  => 'worker_process_start',
                trigger_child_port   => 'process_start',
                event                => 'done',
        ...
        'pin-egress vector multi-route report records the generated ATL top without private data-link internals',
    );
    is_deeply(
        $actor_network->{data_movements},
        [
        ...
        'pin-egress vector multi-route report records both public exact-width child-to-pin data movements',
    );
    is_deeply(
        $actor_network->{transaction_triggers},
        [
        ...
        'pin-egress vector multi-route report records the worker process trigger handoff',
    );
    is_deeply(
        $actor_network->{event_waits},
        [
        ...
        'pin-egress vector multi-route report records the worker done event handoff',
    );
}
...
sub assert_two_child_trigger_batch_report_shape {
    my ($report) = @_;

    is($report->{source}, 'atl_two_child_trigger_batch_pipeline.isf',
        'two-child trigger-batch schedule report names the fixture');
    is($report->{scheduled_fsm}, 'atl_two_child_trigger_batch_pipeline.fsm',
        'two-child trigger-batch schedule report names the scheduled parent FSM');
    is($report->{inputs}, 3, 'two-child trigger-batch report input count includes start and both event handoffs');
    is($report->{outputs}, 3, 'two-child trigger-batch report output count includes done and both trigger handoffs');
    is($report->{port_count}, 6,
        'two-child trigger-batch report port count includes public and generated handoff ports');
    is($report->{state_count}, 6,
        'two-child trigger-batch report state count includes one batch state, two waits, done, idle, and timeout');
    is_deeply($report->{compile_issues}, [], 'two-child trigger-batch schedule report has no compile issues');
    is_deeply($report->{dt_blocks}, [], 'two-child trigger-batch schedule report has no drive bodies');
    is_deeply(
        $report->{transactions},
        [
            {
        ...
        'two-child trigger-batch schedule report records same-cycle trigger batch then source-ordered waits',
    );

    my $actor_network = $report->{actor_network};
    is($actor_network->{kind}, 'static_declaration', 'two-child trigger-batch actor network kind');
    is_deeply(
        $actor_network->{instances},
        [
            {
                ...
                alias           => 'pkt_lib',
                export          => 'packet_reader',
                module          => 'atl_two_child_trigger_batch_pipeline__reader',
                scheduled_fsm   => 'atl_two_child_trigger_batch_pipeline__reader.fsm',
            },
            {
                name            => 'writer',
                actor_type      => 'pkt_lib.packet_writer',
                ...
                alias           => 'pkt_lib',
                export          => 'packet_writer',
                module          => 'atl_two_child_trigger_batch_pipeline__writer',
                scheduled_fsm   => 'atl_two_child_trigger_batch_pipeline__writer.fsm',
            },
        ],
        'two-child trigger-batch report records both resolved child actor metadata entries',
    );
    is_deeply($actor_network->{groups}, [], 'two-child trigger-batch fixture has no permanent static group');
    is_deeply(
        $actor_network->{generated_tops},
        [
            {
                kind                 => 'resolved_children_trigger_batch_event_sequence',
                top_module           => 'atl_two_child_trigger_batch_pipeline_top',
                top_fsm              => 'atl_two_child_trigger_batch_pipeline_top.fsm',
                parent_module        => 'atl_two_child_trigger_batch_pipeline',
                parent_scheduled_fsm => 'atl_two_child_trigger_batch_pipeline.fsm',
                clock                => 'clk',
                reset                => 'rst_n',
                children             => [
                    {
                        instance             => 'reader',
                        child_module         => 'atl_two_child_trigger_batch_pipeline__reader',
                        child_scheduled_fsm  => 'atl_two_child_trigger_batch_pipeline__reader.fsm',
                        target_transaction   => 'capture',
                        trigger_parent_port  => 'reader_capture_start',
                        trigger_child_port   => 'capture_start',
                        event                => 'done',
                    ...
                    {
                        instance             => 'writer',
                        child_module         => 'atl_two_child_trigger_batch_pipeline__writer',
                        child_scheduled_fsm  => 'atl_two_child_trigger_batch_pipeline__writer.fsm',
                        target_transaction   => 'emit',
                        trigger_parent_port  => 'writer_emit_start',
                        trigger_child_port   => 'emit_start',
                        event                => 'done',
        ...
        'two-child trigger-batch report records one generated ATL top with per-child wiring metadata',
    );
    is_deeply($actor_network->{data_movements}, [],
        'two-child trigger-batch fixture does not use ATL data movement');
    is_deeply(
        ...
        'two-child trigger-batch report records the canonical temporary association schedule',
    );
    is_deeply(
        ...
        'two-child trigger-batch report preserves compatibility group schedule evidence',
    );
    is_deeply(
        $actor_network->{transaction_triggers},
        [
        ...
        'two-child trigger-batch report records both transaction trigger handoffs',
    );
    is_deeply(
        $actor_network->{event_waits},
        [
        ...
        'two-child trigger-batch report records source-ordered event wait handoffs',
    );
}
...
sub assert_pin_ingress_vector_multi_generated_top_hdl {
    my ($hdl, $label) = @_;

    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_vector_multi_pipeline_top\b/,
        "$label contains the generated ATL top module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_vector_multi_pipeline\b/,
        "$label contains the scheduled parent module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_vector_multi_pipeline__worker\b/,
        "$label contains the resolved child module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_vector_multi_pipeline__worker\s*\([^;]*\binput\s+wire\s+\[7:0\]\s+payload\b/s,
        "$label preserves the child payload input as an 8-bit module port");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_vector_multi_pipeline__worker\s*\([^;]*\binput\s+wire\s+\[3:0\]\s+sideband\b/s,
        "$label preserves the child sideband input as a 4-bit module port");
    like($hdl, qr/\bwire\s+\[7:0\]\s+comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_payload\b/,
        "$label declares the 8-bit parent-to-child payload link");
    like($hdl, qr/\bwire\s+\[3:0\]\s+comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_sideband\b/,
        "$label declares the 4-bit parent-to-child sideband link");
    like($hdl, qr/\bwire\s+comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_process_start\b/,
        "$label declares the parent-to-child trigger link");
    like($hdl, qr/\bwire\s+comp_link_worker_done\b/,
        "$label declares the child-to-parent event link");
    like($hdl, qr/\.payload\(payload\)/,
    ...
    like($hdl, qr/\.worker_payload\(comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_payload\)/,
        "$label connects the parent payload handoff to the internal payload link");
    like($hdl, qr/\.worker_sideband\(comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_sideband\)/,
        "$label connects the parent sideband handoff to the internal sideband link");
    like($hdl, qr/\.payload\(comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_payload\)/,
        "$label connects the internal payload link to the child payload input");
    like($hdl, qr/\.sideband\(comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_sideband\)/,
        "$label connects the internal sideband link to the child sideband input");
    like($hdl, qr/\.process_start\(comp_link_atl_resolved_child_pin_ingress_vector_multi_pipeline_worker_process_start\)/,
        "$label connects the child process start input to the internal trigger link");
    like($hdl, qr/\.done\(comp_link_worker_done\)/,
        "$label connects the child done output to the internal event link");
    like($hdl, qr/\.worker_done\(comp_link_worker_done\)/,
...
sub assert_pin_ingress_mixed_generated_top_hdl {
    my ($hdl, $label) = @_;

    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_mixed_pipeline_top\b/,
        "$label contains the generated ATL top module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_mixed_pipeline\b/,
        "$label contains the scheduled parent module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_mixed_pipeline__worker\b/,
        "$label contains the resolved child module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_mixed_pipeline__worker\s*\([^;]*\binput\s+wire\s+\[7:0\]\s+payload\b/s,
        "$label preserves the child payload input as an 8-bit module port");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_ingress_mixed_pipeline__worker\s*\([^;]*\binput\s+wire\s+valid\b/s,
        "$label preserves the child valid input as a scalar module port");
    like($hdl, qr/\bwire\s+\[7:0\]\s+comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_payload\b/,
        "$label declares the 8-bit parent-to-child payload link");
    like($hdl, qr/\bwire\s+comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_valid\b/,
        "$label declares the scalar parent-to-child valid link");
    like($hdl, qr/\bwire\s+comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_process_start\b/,
        "$label declares the parent-to-child trigger link");
    like($hdl, qr/\bwire\s+comp_link_worker_done\b/,
        "$label declares the child-to-parent event link");
    like($hdl, qr/\.payload\(payload\)/,
    ...
    like($hdl, qr/\.valid\(valid\)/,
        "$label connects the public top valid input to the parent valid input");
    like($hdl, qr/\.worker_payload\(comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_payload\)/,
        "$label connects the parent payload handoff to the internal payload link");
    like($hdl, qr/\.worker_valid\(comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_valid\)/,
        "$label connects the parent valid handoff to the internal valid link");
    like($hdl, qr/\.payload\(comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_payload\)/,
        "$label connects the internal payload link to the child payload input");
    like($hdl, qr/\.valid\(comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_valid\)/,
        "$label connects the internal valid link to the child valid input");
    like($hdl, qr/\.process_start\(comp_link_atl_resolved_child_pin_ingress_mixed_pipeline_worker_process_start\)/,
        "$label connects the child process start input to the internal trigger link");
    like($hdl, qr/\.done\(comp_link_worker_done\)/,
        "$label connects the child done output to the internal event link");
    like($hdl, qr/\.worker_done\(comp_link_worker_done\)/,
...
sub assert_pin_egress_vector_generated_top_hdl {
    my ($hdl, $label) = @_;

    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_pipeline_top\b/,
        "$label contains the generated ATL top module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_pipeline\b/,
        "$label contains the scheduled parent module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_pipeline__worker\b/,
        "$label contains the resolved child module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_pipeline__worker\s*\([^;]*\boutput\s+reg\s+\[7:0\]\s+payload\b/s,
    ...
    like($hdl, qr/\bwire\s+comp_link_atl_resolved_child_pin_egress_vector_pipeline_worker_process_start\b/,
        "$label declares the parent-to-child trigger link");
    like($hdl, qr/\bwire\s+comp_link_worker_done\b/,
        "$label declares the child-to-parent event link");
    like($hdl, qr/\.result\(result\)/,
    ...
    like($hdl, qr/\.worker_payload\(comp_link_worker_payload\)/,
        "$label connects the internal payload link to the parent payload handoff");
    like($hdl, qr/\.process_start\(comp_link_atl_resolved_child_pin_egress_vector_pipeline_worker_process_start\)/,
        "$label connects the child process start input to the internal trigger link");
    like($hdl, qr/\.done\(comp_link_worker_done\)/,
        "$label connects the child done output to the internal event link");
    like($hdl, qr/\.worker_done\(comp_link_worker_done\)/,
...
sub assert_pin_egress_vector_multi_generated_top_hdl {
    my ($hdl, $label) = @_;

    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_multi_pipeline_top\b/,
        "$label contains the generated ATL top module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_multi_pipeline\b/,
        "$label contains the scheduled parent module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_multi_pipeline__worker\b/,
        "$label contains the resolved child module");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_multi_pipeline__worker\s*\([^;]*\boutput\s+reg\s+\[7:0\]\s+payload\b/s,
        "$label preserves the child payload output as an 8-bit module port");
    like($hdl, qr/\bmodule\s+atl_resolved_child_pin_egress_vector_multi_pipeline__worker\s*\([^;]*\boutput\s+reg\s+\[3:0\]\s+status\b/s,
        "$label preserves the child status output as a 4-bit module port");
    like($hdl, qr/\bwire\s+\[7:0\]\s+comp_link_worker_payload\b/,
        "$label declares the 8-bit child-to-parent payload link");
    like($hdl, qr/\bwire\s+\[3:0\]\s+comp_link_worker_status\b/,
        "$label declares the 4-bit child-to-parent status link");
    like($hdl, qr/\bwire\s+comp_link_atl_resolved_child_pin_egress_vector_multi_pipeline_worker_process_start\b/,
        "$label declares the parent-to-child trigger link");
    like($hdl, qr/\bwire\s+comp_link_worker_done\b/,
        "$label declares the child-to-parent event link");
    like($hdl, qr/\.result\(result\)/,
    ...
    like($hdl, qr/\.process_start\(comp_link_atl_resolved_child_pin_egress_vector_multi_pipeline_worker_process_start\)/,
        "$label connects the child process start input to the internal trigger link");
    like($hdl, qr/\.done\(comp_link_worker_done\)/,
        "$label connects the child done output to the internal event link");
    like($hdl, qr/\.worker_done\(comp_link_worker_done\)/,
...
sub assert_two_child_trigger_batch_generated_top_hdl {
    my ($hdl, $label) = @_;

    like($hdl, qr/\bmodule\s+atl_two_child_trigger_batch_pipeline_top\b/,
        "$label contains the generated ATL top module");
    like($hdl, qr/\bmodule\s+atl_two_child_trigger_batch_pipeline\b/,
        "$label contains the scheduled parent module");
    like($hdl, qr/\bmodule\s+atl_two_child_trigger_batch_pipeline__reader\b/,
        "$label contains the resolved reader child module");
    like($hdl, qr/\bmodule\s+atl_two_child_trigger_batch_pipeline__writer\b/,
        "$label contains the resolved writer child module");
    like($hdl, qr/\bwire\s+comp_link_atl_two_child_trigger_batch_pipeline_reader_capture_start\b/,
        "$label declares the parent-to-reader trigger link");
    like($hdl, qr/\bwire\s+comp_link_reader_done\b/,
        "$label declares the reader-to-parent event link");
    like($hdl, qr/\bwire\s+comp_link_atl_two_child_trigger_batch_pipeline_writer_emit_start\b/,
        "$label declares the parent-to-writer trigger link");
    like($hdl, qr/\bwire\s+comp_link_writer_done\b/,
        "$label declares the writer-to-parent event link");
    like($hdl, qr/\.reader_capture_start\(comp_link_atl_two_child_trigger_batch_pipeline_reader_capture_start\)/,
        "$label connects the parent reader trigger handoff to the internal reader trigger link");
    like($hdl, qr/\.capture_start\(comp_link_atl_two_child_trigger_batch_pipeline_reader_capture_start\)/,
        "$label connects the reader capture start input to the internal reader trigger link");
    like($hdl, qr/\.done\(comp_link_reader_done\)/,
        "$label connects the reader done output to the internal reader event link");
    like($hdl, qr/\.reader_done\(comp_link_reader_done\)/,
        "$label connects the parent reader event handoff input to the internal reader event link");
    like($hdl, qr/\.writer_emit_start\(comp_link_atl_two_child_trigger_batch_pipeline_writer_emit_start\)/,
        "$label connects the parent writer trigger handoff to the internal writer trigger link");
    like($hdl, qr/\.emit_start\(comp_link_atl_two_child_trigger_batch_pipeline_writer_emit_start\)/,
        "$label connects the writer emit start input to the internal writer trigger link");
    like($hdl, qr/\.done\(comp_link_writer_done\)/,
        "$label connects the writer done output to the internal writer event link");
    like($hdl, qr/\.writer_done\(comp_link_writer_done\)/,
...
sub pin_ingress_vector_multi_atl_fixture_variant {
    my ($child_sideband_width) = @_;
    $child_sideband_width //= 4;
    return <<"ISF";
(actor atl_resolved_child_pin_ingress_vector_multi_pipeline
  (clock clk)
  (reset (rst_n async active_low))
  (interface
    (input start)
    (input payload (width 8))
    (input sideband (width 4))
    (output done))
  (imports
    (library common.packet as pkt_lib))
  (instance worker of pkt_lib.packet_worker)
...
(library common.packet
  ...
  (actor packet_worker
    ...
    (interface
      (input process_start)
      (input payload (width 8))
      (input sideband (width $child_sideband_width))
      (output done))
    (transaction process
      (on process_start)
      (complete done))))
...
sub pin_ingress_mixed_atl_fixture_variant {
    my ($child_payload_width) = @_;
    $child_payload_width //= 8;
    return <<"ISF";
...
sub pin_egress_vector_atl_fixture_variant {
    my ($child_payload_width) = @_;
    $child_payload_width //= 8;
    return <<"ISF";
(actor atl_resolved_child_pin_egress_vector_pipeline
  (clock clk)
  (reset (rst_n async active_low))
  (interface
    (input start)
...
sub pin_egress_vector_multi_atl_fixture_variant {
    my ($child_status_width) = @_;
    $child_status_width //= 4;
    return <<"ISF";
(actor atl_resolved_child_pin_egress_vector_multi_pipeline
  (clock clk)
  (reset (rst_n async active_low))
  (interface
    (input start)
    (output result (width 8))
    (output status (width 4))
    (output done))
  (imports
    (library common.packet as pkt_lib))
  (instance worker of pkt_lib.packet_worker)
...
sub expected_fsm_basenames_for_source {
    ...
    if ($filename eq 'atl_two_child_pipeline'
        || $filename eq 'atl_two_child_trigger_batch_pipeline'
