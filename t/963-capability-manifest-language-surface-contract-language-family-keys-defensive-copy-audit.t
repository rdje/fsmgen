...
subtest 'manifest language-surface contract language family key lists rebuilds cleanly after caller mutation' => sub {
    ...
    mutate_structure($mutated->{'declarations_presence_keys'});
    mutate_structure($mutated->{'composition_presence_keys'});
    ...
    is_deeply($contract->{'declarations_presence_keys'}, $expected->{'declarations_presence_keys'}, 'fresh manifest language-surface contract rebuilds clean declarations_presence_keys');
    is_deeply($contract->{'composition_presence_keys'}, $expected->{'composition_presence_keys'}, 'fresh manifest language-surface contract rebuilds clean composition_presence_keys');
};
done_testing();
