...
subtest 'drive body aggregate diagnostics stay direct-RHS-only' => sub {
...
ISF
    ...
    assert_parse_rejected(
        <<'ISF',
(actor aggregate_drive_expression_operand_still_deferred
  (types
    (type frame_t (record (mode (bits 2)) (flag bit))))
  (clock clk)
  (reset rst)
  ...
  (transaction main
    ...
    (drive publish)))
ISF
        qr/drive 'publish' RHS expression references aggregate storage path 'frame\.mode'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, transaction condition expression scalar operands, rule assignment RHS scalar values or operands, rule guard expression scalar operands, or drive body RHS scalar values/,
        'aggregate drive RHS expression operands remain deferred',
    );
