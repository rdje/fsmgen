...
subtest 'pipeline derives child context from blocked ?fsmc source-count failures' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $composition_path = File::Spec->catfile($tempdir, 'missing_fsm_source_failure_summary_top.fsm');

    write_file(
        $composition_path,
        <<'FSM'
(?top:missing_fsm_source_failure_summary_top
  (?fsmc:child)
)
FSM
    );
    ...
    ok($report, 'pipeline derives a composition failure report from blocked ?fsmc source-count failures');
    is($report->{top_name}, 'missing_fsm_source_failure_summary_top', 'failure report preserves the top name for blocked ?fsmc source-count failures');
    is($report->{construct}, '?fsmc', 'failure report preserves the ?fsmc construct for blocked source-count failures');
    is($report->{context_label}, 'Child', 'failure report classifies blocked ?fsmc source-count failures as child context');
    ...
    is($report->{blocked_boundary}, 'composition child source count', 'failure report preserves the blocked ?fsmc source-count boundary');
    is($report->{blocked_boundary_label}, 'child source count', 'failure report exposes a CLI-friendly blocked-boundary label for ?fsmc source-count failures');
    is(
        $report->{blocked_reason},
        "the active composition parser currently requires exactly one FSM source name per '?fsmc'",
        'failure report preserves the concise ?fsmc source-count reason',
    );
};
...
subtest 'CLI prints child context for blocked ?fsmc source-count failures' => sub {
    my $tempdir = tempdir(CLEANUP => 1);
    my $composition_path = File::Spec->catfile($tempdir, 'missing_fsm_source_failure_summary_cli_top.fsm');
    my $output_path = File::Spec->catfile($tempdir, 'missing_fsm_source_failure_summary_cli_top.sv');

    write_file(
        $composition_path,
        <<'FSM'
(?top:missing_fsm_source_failure_summary_cli_top
  (?fsmc:child)
)
FSM
    );
    ...
    ok(!$success, 'CLI fails for blocked ?fsmc source-count fixture');
    ok(!-e $output_path, 'CLI does not emit HDL output for blocked ?fsmc source-count fixture');
    ...
    like($combined_output, qr/=== Composition Failure Summary ===/s, 'CLI prints the composition failure summary section for blocked ?fsmc source-count failures');
    like($combined_output, qr/Construct:\s+\?fsmc/s, 'CLI reports the ?fsmc construct for blocked source-count failures');
