...
(actor aggregate_rule_guard_scalar_path
  ...
  (rule fire_when_flag frame.flag
    (set fire 1)))
ISF
        qr/rule 'fire_when_flag' guard references aggregate storage path 'frame\.flag'; this ISF slice accepts aggregate storage paths only as direct transaction set RHS scalar leaf reads, direct transaction set target scalar leaf writes, rule assignment RHS scalar values or operands, or rule guard expression scalar operands/,
