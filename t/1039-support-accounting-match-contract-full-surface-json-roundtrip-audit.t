...
subtest 'support accounting match contract full surface survives JSON round trip' => sub {
    my $contract = build_support_accounting_match_contract();
    my $decoded = decode_json(encode_json($contract));
    is_deeply($decoded, $contract, 'decoded match contract matches direct owner contract');
};
