...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat do after multi-pending await_any before drain', qr/switch-branch nested repeat local do while generated spawns are pending is supported only before a later same-body '\(await_all done\)' drain, with no prior multi-pending await_any observation/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat do after multi-pending await_any before drain', qr/when-body nested repeat local do while generated spawns are pending is supported only before a later same-body '\(await_all done\)' drain, with no prior multi-pending await_any observation/);
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat parameterized generated do after multi-pending await_any before drain', qr/when-body nested repeat generated do with static params while generated spawns are pending is supported only before a later same-body '\(await_all done\)' drain, with no prior multi-pending await_any observation/);
(actor when_nested_repeat_parameterized_generated_do_after_multi_pending_await_any
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat bound generated do while spawn pending', qr/when-body nested repeat do while generated spawns are pending supports only local plain '\(do child\)', plain generated-child '\(do child\)', or static generated '\(do child \(params \.\.\.\)\)' in the current subset/);
(actor when_nested_repeat_bound_generated_do_while_spawn_pending
  (clock clk)
  (interface
    (input start)
    (input cond)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat domain generated do while spawn pending', qr/when-body nested repeat do while generated spawns are pending supports only local plain '\(do child\)', plain generated-child '\(do child\)', or static generated '\(do child \(params \.\.\.\)\)' in the current subset/);
(actor when_nested_repeat_domain_generated_do_while_spawn_pending
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat parameterized generated do after multi-pending await_any before drain', qr/switch-branch nested repeat generated do with static params while generated spawns are pending is supported only before a later same-body '\(await_all done\)' drain, with no prior multi-pending await_any observation/);
(actor switch_nested_repeat_parameterized_generated_do_after_multi_pending_await_any
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat bound generated do while spawn pending', qr/switch-branch nested repeat do while generated spawns are pending supports only local plain '\(do child\)', plain generated-child '\(do child\)', or static generated '\(do child \(params \.\.\.\)\)' in the current subset/);
(actor switch_nested_repeat_bound_generated_do_while_spawn_pending
  (clock clk)
  (interface
    (input start)
    (input mode)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat domain generated do while spawn pending', qr/switch-branch nested repeat do while generated spawns are pending supports only local plain '\(do child\)', plain generated-child '\(do child\)', or static generated '\(do child \(params \.\.\.\)\)' in the current subset/);
(actor switch_nested_repeat_domain_generated_do_while_spawn_pending
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
