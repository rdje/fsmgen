...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat bound generated do after multi-pending await_any before drain', qr/when-body nested repeat generated do with static params and bindings while generated spawns are pending is supported only before a later same-body '\(await_all done\)' drain, with no prior multi-pending await_any observation/);
(actor when_nested_repeat_bound_generated_do_after_multi_pending_await_any
  (clock clk)
  (interface
    (input start)
    (input cond)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat domain generated do while spawn pending', qr/when-body nested repeat do while generated spawns are pending supports only local plain '\(do child\)', plain generated-child '\(do child\)', static generated '\(do child \(params \.\.\.\)\)', or static bound generated '\(do child \(params \.\.\.\) \(bind \.\.\.\)\)' in the current subset/);
