...
subtest 'when body nested repeat local do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_local_do_after_await_any_then_spawn
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1 w2)],
        'when-body prior-await_any local-do-then-spawn subset records all generated spawn instances');
    is($ir->{counters}{local_worker_start}, 1,
        'when-body prior-await_any local-do-then-spawn subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'when-body prior-await_any local-do-then-spawn subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_local_do_after_await_any_then_spawn.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_local_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any local-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body prior-await_any local-do-then-spawn generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'prior-await_any local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any local-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after prior await_any materializes before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'local do after prior await_any waits for a fresh local child done pulse');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after local do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'post-do generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($parent_fsm, qr/\(local_worker_idle_0[\s\S]*<local_worker_start[\s\S]*\(-> local_worker_done_1\)/,
        'local child entry is rewired to the prior-await_any local do start handoff');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the first pre-do spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the second pre-do spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w2 worker\s+\(params\s+\(WIDTH 48\)\s+\)\s+\)/s,
        'generated top keeps the post-do spawn parameter override');
    ...
    is_deeply(
        ...
        'report exposes prior-await_any local-do-then-spawn generated spawn port-binding provenance',
    );
    is_deeply(
        $report->{clock_domains}[0]{child_instances},
        [
            { kind => 'spawn', owner => 'parent', child => 'worker', instance => 'w0' },
            { kind => 'spawn', owner => 'parent', child => 'worker', instance => 'w1' },
            { kind => 'spawn', owner => 'parent', child => 'worker', instance => 'w2' },
        ],
        'clock-domain report metadata groups pre-do and post-do generated spawns after prior await_any',
    );
};

subtest 'when body nested repeat local do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_local_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (interface
    (input start)
    (input cond)
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1 w2)],
        'when-body prior-await_any local-do second-await_any subset records all generated spawn instances');
    is($ir->{counters}{local_worker_start}, 1,
        'when-body prior-await_any local-do second-await_any subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'when-body prior-await_any local-do second-await_any subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_local_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_local_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any local-do second-await_any parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body prior-await_any local-do second-await_any generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'second-await_any local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation in the second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'local do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'post-do generated spawn advances to the second await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($parent_fsm, qr/\(local_worker_idle_0[\s\S]*<local_worker_start[\s\S]*\(-> local_worker_done_1\)/,
        'local child entry is rewired to the prior-await_any second-await_any local do start handoff');
    like($top_fsm, qr/\(\?fsmc:w0 worker\)/,
        'generated top keeps the first pre-do spawn instance for the second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\)/,
        'generated top keeps the second pre-do spawn instance for the second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w2 worker\)/,
        'generated top keeps the post-do spawn instance for the second-await_any subset');
...
subtest 'when body nested repeat local do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_local_do_then_spawn_before_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1)],
        'when-body local-do-then-spawn subset records both generated spawn instances',
    );
    is($ir->{counters}{local_worker_start}, 1,
        'when-body local-do-then-spawn subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'when-body local-do-then-spawn subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_local_do_then_spawn_before_drain.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_local_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'when-body local-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body local-do-then-spawn generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'when-body local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the local-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before local-do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'local do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after local do materializes before the later generated spawn');
    ...
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the pre-do generated spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the post-do generated spawn parameter override');
    ...
    is_deeply(
        ...
        'report exposes both local-do-then-spawn generated spawn port-binding sets',
    );
    is_deeply(
        $report->{clock_domains}[0]{child_instances},
        [
        ...
        'clock-domain report metadata keeps both generated spawns around the local do',
    );
};
...
subtest 'when body nested repeat generated-child do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_generated_child_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body generated-child do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body generated-child do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body generated-child do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'when-body generated-child do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body generated-child do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'when-body generated-child do-then-spawn post-await_any keeps the do site plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_generated_child_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_generated_child_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body generated-child do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for post-await_any do and spawns');
    ok(defined($top_fsm), 'when-body generated-child do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the generated-child do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before generated-child do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the generated-child do in the post-await_any subset');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'generated-child do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after generated-child do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later generated spawn advances to the post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the pre-do spawn instance for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top keeps the generated do instance for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for post-await_any subset');
};
...
subtest 'when body nested repeat generated-child do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_generated_child_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any generated-child-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any generated-child subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any generated-child subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any generated-child subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any generated-child subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any generated-child subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'when-body prior-await_any generated-child subset keeps the do instance plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_generated_child_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_generated_child_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any generated-child-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for prior-await_any generated-child-do-then-spawn');
    ok(defined($top_fsm), 'when-body prior-await_any generated-child-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any generated-child-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after prior await_any materializes before the generated-child do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'generated-child do after prior await_any waits for its generated instance done');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after generated-child do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'post-do generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first pre-do spawn instance');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second pre-do spawn instance');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top instantiates the generated-child do instance separately before the later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance');
};

subtest 'when body nested repeat generated-child do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_generated_child_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any generated-child second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any generated-child second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any generated-child second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any generated-child second-await_any subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any generated-child second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any generated-child second-await_any subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'when-body prior-await_any generated-child second-await_any subset keeps the do instance plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_generated_child_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_generated_child_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any generated-child second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for generated-child second-await_any');
    ok(defined($top_fsm), 'when-body prior-await_any generated-child second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any generated-child second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn in the generated-child second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation in the generated-child second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before the generated-child do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'generated-child do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'post-do generated spawn advances to the second await_any observation in the generated-child subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first pre-do spawn instance for the generated-child second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second pre-do spawn instance for the generated-child second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top instantiates the generated-child do instance separately before the later spawn and second await_any');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for the generated-child second-await_any subset');
};
...
subtest 'when body nested repeat parameterized generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_parameterized_do_then_spawn_before_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body parameterized do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body parameterized do-then-spawn preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body parameterized do-then-spawn records the generated do instance');
    ok($instances{w1}, 'when-body parameterized do-then-spawn preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body parameterized do-then-spawn preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'when-body parameterized do-then-spawn targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'when-body parameterized do-then-spawn preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_parameterized_do_then_spawn_before_drain.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_parameterized_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'when-body parameterized do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for parameterized do and spawns');
    ok(defined($top_fsm), 'when-body parameterized do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the parameterized do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before parameterized do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'parameterized generated do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after parameterized generated do materializes before the later generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the pre-do spawn instance for parameterized do');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the parameterized do instance override');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for parameterized do');
};

subtest 'when body nested repeat parameterized generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_parameterized_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body parameterized do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body parameterized do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body parameterized do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'when-body parameterized do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body parameterized do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'when-body parameterized do-then-spawn post-await_any preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_parameterized_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_parameterized_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body parameterized do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for parameterized post-await_any do and spawns');
    ok(defined($top_fsm), 'when-body parameterized do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the parameterized do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before parameterized do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the parameterized generated do in the post-await_any subset');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'parameterized generated do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after parameterized generated do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later generated spawn advances to the parameterized post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'parameterized post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after parameterized post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after parameterized post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the pre-do spawn instance for parameterized post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the parameterized do instance override for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for parameterized post-await_any subset');
};
...
subtest 'when body nested repeat parameterized generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_parameterized_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any parameterized-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any parameterized subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any parameterized subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any parameterized subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any parameterized subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any parameterized subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'when-body prior-await_any parameterized subset targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'when-body prior-await_any parameterized subset preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_parameterized_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_parameterized_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any parameterized-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for prior-await_any parameterized-do-then-spawn');
    ok(defined($top_fsm), 'when-body prior-await_any parameterized-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any parameterized-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn for parameterized do');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation for parameterized do');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before parameterized do');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after prior await_any materializes before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'parameterized generated do after prior await_any waits for its generated instance done');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after parameterized generated do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'post-do generated spawn advances directly to the mandatory drain for parameterized do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains pre-do and post-do generated spawns for parameterized do');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first pre-do spawn instance for parameterized prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second pre-do spawn instance for parameterized prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the generated do instance before later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for parameterized prior-await_any subset');
};

subtest 'when body nested repeat parameterized generated do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_parameterized_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any parameterized second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any parameterized second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any parameterized second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any parameterized second-await_any subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any parameterized second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any parameterized second-await_any subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'when-body prior-await_any parameterized second-await_any subset targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'when-body prior-await_any parameterized second-await_any subset preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_parameterized_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_parameterized_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any parameterized second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for parameterized second-await_any');
    ok(defined($top_fsm), 'when-body prior-await_any parameterized second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any parameterized second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second pre-do spawn in the parameterized second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation in the parameterized second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'parameterized generated do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'post-do generated spawn advances to the second await_any observation in the parameterized subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first pre-do spawn instance for the parameterized second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second pre-do spawn instance for the parameterized second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the generated do instance before the later spawn and second await_any');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the post-do spawn instance for the parameterized second-await_any subset');
};

subtest 'when body nested repeat parameterized generated do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_parameterized_do_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body parameterized do before post-do await_any records both spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body parameterized do before post-do await_any preserves the first pending spawn');
    ok($instances{w1}, 'when-body parameterized do before post-do await_any preserves the second pending spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body parameterized do before post-do await_any records the generated do instance');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body parameterized do before post-do await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'when-body parameterized do before post-do await_any targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'when-body parameterized do before post-do await_any preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_parameterized_do_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_parameterized_do_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body parameterized do before post-do await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for when pre-await_any parameterized do');
    ok(defined($top_fsm), 'when-body parameterized do before post-do await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the parameterized-do-before-post-do-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before parameterized do before post-do await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first generated spawn starts before the second spawn in parameterized post-do await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second generated spawn advances to the parameterized generated do before post-do await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'parameterized generated do completes before the post-do await_any observation can run');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after parameterized generated do materializes before the post-do await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'post-do await_any observes either generated child without draining the generated-spawn set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after parameterized post-do await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after parameterized post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first pending spawn instance before parameterized post-do await_any');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second pending spawn instance before parameterized post-do await_any');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the generated do instance before post-do await_any');
};
...
subtest 'when body nested repeat bound generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_bound_do_then_spawn_before_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body bound do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body bound do-then-spawn preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body bound do-then-spawn records the generated do instance');
    ok($instances{w1}, 'when-body bound do-then-spawn preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body bound do-then-spawn preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body bound do-then-spawn preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body bound do-then-spawn exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_bound_do_then_spawn_before_drain.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_bound_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'when-body bound do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for bound do and spawns');
    ok(defined($top_fsm), 'when-body bound do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the bound do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before bound do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the bound generated do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'bound generated do keeps input and output binding handoffs reviewable before the later spawn');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'bound generated do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after bound generated do materializes before the later generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the pre-do spawn parameter override for bound do');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the bound do instance override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the post-do spawn parameter override for bound do');
    like($top_fsm, qr/\(when_repeat_bound_do_then_spawn_before_drain\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires bound do input handoff separately from both spawns');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data when_repeat_bound_do_then_spawn_before_drain\.parent_worker_repeat_do_0_data\)/,
        'generated top wires bound do output handoff separately from both spawns');
    ...
    is_deeply(
        ...
        'report exposes pre-do spawn, bound do, and post-do spawn port-binding provenance',
    );
};

subtest 'when body nested repeat bound generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_bound_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body bound do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body bound do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body bound do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'when-body bound do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body bound do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body bound do-then-spawn post-await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body bound do-then-spawn post-await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_bound_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_bound_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body bound do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for bound post-await_any do and spawns');
    ok(defined($top_fsm), 'when-body bound do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the bound do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before bound do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first generated spawn starts before the bound generated do in the post-await_any subset');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'bound generated do keeps input and output binding handoffs reviewable before the post-spawn await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'bound generated do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after bound generated do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later generated spawn advances to the bound post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'bound post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after bound post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after bound post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the pre-do spawn parameter override for bound post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the bound do instance override for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the post-do spawn parameter override for bound post-await_any subset');
    like($top_fsm, qr/\(when_repeat_bound_do_then_spawn_before_await_any\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires bound do input handoff separately from both spawns in post-await_any subset');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data when_repeat_bound_do_then_spawn_before_await_any\.parent_worker_repeat_do_0_data\)/,
        'generated top wires bound do output handoff separately from both spawns in post-await_any subset');
    ...
    is_deeply(
        ...
        'report exposes pre-do spawn, bound do, and post-do spawn port-binding provenance before post-spawn await_any',
    );
};
...
subtest 'when body nested repeat bound generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_bound_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
    ...
    (input payload0 (width 8))
    (input payload1 (width 8))
    (input payload2 (width 8))
    (input req_addr (width 8))
    (input status)
    (output done)
    (output spawn_resp0 (width 8))
    (output spawn_resp1 (width 8))
    (output spawn_resp2 (width 8))
    (output resp (width 8)))
  (transaction parent
    (on start)
    (when cond
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any bound-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any bound subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any bound subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any bound subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any bound subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any bound subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body prior-await_any bound subset preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body prior-await_any bound subset exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_bound_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_bound_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any bound-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for prior-await_any bound-do-then-spawn');
    ok(defined($top_fsm), 'when-body prior-await_any bound-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any bound-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first bound pre-do generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second bound pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before bound do');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after prior await_any materializes before the bound generated do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'bound generated do after prior await_any keeps input and output binding handoffs reviewable');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'bound generated do after prior await_any completes before the later generated spawn');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after bound generated do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'post-do generated spawn advances directly to the mandatory drain for bound do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains pre-do and post-do generated spawns for bound do');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the first pre-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the second pre-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the bound generated do instance before later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker\s+\(params\s+\(WIDTH 40\)\s+\)\s+\)/s,
        'generated top keeps the post-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(when_repeat_bound_do_after_await_any_then_spawn\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires bound do input handoff after prior await_any');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data when_repeat_bound_do_after_await_any_then_spawn\.parent_worker_repeat_do_0_data\)/,
        'generated top wires bound do output handoff after prior await_any');
    ...
    is_deeply(
        ...
        'report exposes pre-do spawns, bound do, and post-do spawn port-binding provenance after prior await_any',
    );
};

subtest 'when body nested repeat bound generated do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_bound_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any bound second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'when-body prior-await_any bound second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'when-body prior-await_any bound second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'when-body prior-await_any bound second-await_any subset records generated do instance');
    ok($instances{w2}, 'when-body prior-await_any bound second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any bound second-await_any subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body prior-await_any bound second-await_any subset preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body prior-await_any bound second-await_any subset exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'when_repeat_bound_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_bound_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'when-body prior-await_any bound second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for bound second-await_any');
    ok(defined($top_fsm), 'when-body prior-await_any bound second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the prior-await_any bound second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second pre-do generated spawn advances to the prior await_any observation for bound second-await_any');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either pre-do generated child before bound do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'bound generated do after prior await_any keeps input and output binding handoffs reviewable before second await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'bound generated do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'post-do generated spawn advances to the second await_any observation for bound do');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'second await_any observes any pre-do or post-do generated child without draining the set for bound do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the bound generated do instance before later spawn and second await_any');
    like($top_fsm, qr/\(when_repeat_bound_do_after_await_any_then_spawn_second_await_any\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires bound do input handoff before second await_any');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data when_repeat_bound_do_after_await_any_then_spawn_second_await_any\.parent_worker_repeat_do_0_data\)/,
        'generated top wires bound do output handoff before second await_any');
    ...
    is_deeply(
        ...
        'report exposes pre-do spawns, bound do, and post-do spawn port-binding provenance for second await_any',
    );
};
...
subtest 'when body nested repeat domain generated do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_domain_do_before_await_any
  (clock-domains
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body domain do before post-do await_any records both pending spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'when-body domain do before post-do await_any preserves the first pending spawn domain metadata');
    is($instances{w1}{domain}, 'core',
        'when-body domain do before post-do await_any preserves the second pending spawn domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body domain do before post-do await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'when-body domain do before post-do await_any preserves same-domain metadata on the do instance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body domain do before post-do await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body domain do before post-do await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'when_repeat_domain_do_before_await_any__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_domain_do_before_await_any_top.fsm'};

    ok(defined($core_fsm), 'when-body domain do before post-do await_any core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body domain do before post-do await_any domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the domain-do-before-post-do-await_any nested repeat region');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before domain do before post-do await_any materializes before the first spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first same-domain generated spawn starts before the second spawn in domain post-do await_any subset');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second same-domain generated spawn advances to the domain generated do before post-do await_any');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'same-domain generated do before post-do await_any keeps input and output binding handoffs reviewable');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'same-domain generated do completes before the post-do await_any observation can run');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after same-domain generated do materializes before the post-do await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'post-do await_any observes either same-domain generated child without draining the generated-spawn set');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after same-domain post-do await_any materializes before the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after same-domain post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core when_repeat_domain_do_before_await_any__domain_core\)/,
        'domain top instantiates the when pre-await_any core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the when pre-await_any core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups both pending spawns and the pre-await_any same-domain do with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of pre-await_any same-domain do metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups both pending spawns and the same-domain do before post-do await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive pre-await_any same-domain do metadata');
};
...
subtest 'when body nested repeat domain generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_domain_do_after_await_any_then_spawn
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'when-body prior-await_any domain-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'when-body prior-await_any domain subset preserves first pre-do spawn domain metadata');
    is($instances{w1}{domain}, 'core',
        'when-body prior-await_any domain subset preserves second pre-do spawn domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body prior-await_any domain subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'when-body prior-await_any domain subset preserves same-domain metadata on the do instance');
    is($instances{w2}{domain}, 'core',
        'when-body prior-await_any domain subset preserves post-do spawn domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body prior-await_any domain subset preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body prior-await_any domain subset exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'when_repeat_domain_do_after_await_any_then_spawn__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_domain_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($core_fsm), 'when-body prior-await_any domain-do-then-spawn core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body prior-await_any domain-do-then-spawn domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first same-domain pre-do spawn starts before the second pre-do spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second same-domain pre-do spawn advances to the prior await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'prior await_any observes either same-domain pre-do generated child before domain do');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after prior await_any materializes before the same-domain generated do');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'same-domain generated do after prior await_any keeps input and output binding handoffs reviewable');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'same-domain generated do after prior await_any completes before the later generated spawn');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after same-domain generated do materializes before the post-do generated spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'post-do same-domain generated spawn advances directly to the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains pre-do and post-do same-domain generated spawns');
    like($top_fsm, qr/\(\?fsmc:core when_repeat_domain_do_after_await_any_then_spawn__domain_core\)/,
        'domain top instantiates the core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups pre-do spawns, same-domain do, and post-do spawn with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of prior-await_any same-domain do-then-spawn metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups pre-do spawns, same-domain do, and post-do spawn after prior await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
...
subtest 'when body nested repeat domain generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_domain_do_then_spawn_before_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body domain do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'when-body domain do-then-spawn preserves the pre-do spawn same-domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body domain do-then-spawn preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'when-body domain do-then-spawn preserves same-domain metadata on the do instance');
    is($instances{w1}{domain}, 'core',
        'when-body domain do-then-spawn preserves the post-do spawn same-domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body domain do-then-spawn preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body domain do-then-spawn exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'when_repeat_domain_do_then_spawn_before_drain__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_domain_do_then_spawn_before_drain_top.fsm'};

    ok(defined($core_fsm), 'when-body domain do-then-spawn core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body domain do-then-spawn domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the same-domain do-then-spawn nested repeat region');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first same-domain generated spawn starts before the generated do');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'same-domain generated do keeps input and output binding handoffs reviewable before the later spawn');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'same-domain generated do completes before the later generated spawn can start');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after same-domain generated do materializes before the later generated spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later same-domain generated spawn advances directly to the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both same-domain generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core when_repeat_domain_do_then_spawn_before_drain__domain_core\)/,
        'domain top instantiates the core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups pre-do spawn, same-domain do, and post-do spawn with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of same-domain do-then-spawn metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups pre-do spawn, same-domain do, and post-do spawn',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive same-domain do-then-spawn metadata');
};

subtest 'when body nested repeat domain generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor when_repeat_domain_do_then_spawn_before_await_any
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'when-body domain do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'when-body domain do-then-spawn post-await_any preserves the pre-do spawn same-domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'when-body domain do-then-spawn post-await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'when-body domain do-then-spawn post-await_any preserves same-domain metadata on the do instance');
    is($instances{w1}{domain}, 'core',
        'when-body domain do-then-spawn post-await_any preserves the post-do spawn same-domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'when-body domain do-then-spawn post-await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'when-body domain do-then-spawn post-await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'when_repeat_domain_do_then_spawn_before_await_any__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'when_repeat_domain_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($core_fsm), 'when-body domain do-then-spawn post-await_any core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'when-body domain do-then-spawn post-await_any domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_when_\d+[\s\S]*\(=1 \(-> parent_repeat_init_\d+\)\)/,
        'when true path enters the same-domain do-then-spawn post-await_any nested repeat region');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first same-domain generated spawn starts before the generated do in the post-await_any subset');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'same-domain generated do keeps input and output binding handoffs reviewable before the post-spawn await_any');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'same-domain generated do completes before the later generated spawn in the post-await_any subset');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after same-domain generated do materializes before the later generated spawn in the post-await_any subset');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later same-domain generated spawn advances to the post-spawn await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'same-domain post-spawn await_any observes either generated child without draining the set');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after same-domain post-spawn await_any materializes before the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after same-domain post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core when_repeat_domain_do_then_spawn_before_await_any__domain_core\)/,
        'domain top instantiates the post-await_any core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the post-await_any core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups pre-do spawn, same-domain do, and post-do spawn before post-spawn await_any',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of same-domain do-then-spawn post-await_any metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups pre-do spawn, same-domain do, and post-do spawn before post-spawn await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive same-domain do-then-spawn post-await_any metadata');
};
...
subtest 'switch branch nested repeat local do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_local_do_after_await_any_then_spawn
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1 w2)],
        'switch-branch prior-await_any local-do-then-spawn subset records all generated spawn instances');
    is($ir->{counters}{local_worker_start}, 1,
        'switch-branch prior-await_any local-do-then-spawn subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'switch-branch prior-await_any local-do-then-spawn subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_local_do_after_await_any_then_spawn.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_local_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any local-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch prior-await_any local-do-then-spawn generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'switch prior-await_any local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any local-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after switch prior await_any materializes before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'switch local do after prior await_any waits for a fresh local child done pulse');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch local do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch post-do generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($parent_fsm, qr/\(local_worker_idle_0[\s\S]*<local_worker_start[\s\S]*\(-> local_worker_done_1\)/,
        'local child entry is rewired to the switch prior-await_any local do start handoff');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the first switch pre-do spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the second switch pre-do spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w2 worker\s+\(params\s+\(WIDTH 48\)\s+\)\s+\)/s,
        'generated top keeps the switch post-do spawn parameter override');
    ...
    is_deeply(
        ...
        'report exposes switch prior-await_any local-do-then-spawn generated spawn port-binding provenance',
    );
    is_deeply(
        $report->{clock_domains}[0]{child_instances},
        [
        ...
        'clock-domain report metadata groups switch pre-do and post-do generated spawns after prior await_any',
    );
};

subtest 'switch branch nested repeat local do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_local_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (interface
    (input start)
    (input mode (width 2))
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1 w2)],
        'switch-branch prior-await_any local-do second-await_any subset records all generated spawn instances');
    is($ir->{counters}{local_worker_start}, 1,
        'switch-branch prior-await_any local-do second-await_any subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'switch-branch prior-await_any local-do second-await_any subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_local_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_local_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any local-do second-await_any parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch prior-await_any local-do second-await_any generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'switch second-await_any local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any local-do second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation in the second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'switch local do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'switch post-do generated spawn advances to the second await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($parent_fsm, qr/\(local_worker_idle_0[\s\S]*<local_worker_start[\s\S]*\(-> local_worker_done_1\)/,
        'local child entry is rewired to the switch prior-await_any second-await_any local do start handoff');
    like($top_fsm, qr/\(\?fsmc:w0 worker\)/,
        'generated top keeps the first switch pre-do spawn instance for the second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\)/,
        'generated top keeps the second switch pre-do spawn instance for the second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w2 worker\)/,
        'generated top keeps the switch post-do spawn instance for the second-await_any subset');
...
subtest 'switch branch nested repeat local do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_local_do_then_spawn_before_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    is_deeply(
        [ map { $_->{instance} } @{$ir->{spawn_instances}} ],
        [qw(w0 w1)],
        'switch-branch local-do-then-spawn subset records both generated spawn instances',
    );
    is($ir->{counters}{local_worker_start}, 1,
        'switch-branch local-do-then-spawn subset registers the local child start handoff');
    is($ir->{counters}{local_worker_done}, 1,
        'switch-branch local-do-then-spawn subset registers the local child done handoff');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_local_do_then_spawn_before_drain.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_local_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch local-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch local-do-then-spawn generated top .fsm is emitted');
    ok(!exists($lowered->{files}{'local_worker.fsm'}), 'switch-branch local do target remains in the parent scheduled module');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the local-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch local-do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the local do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(local_worker_start 1\)\)[\s\S]*<local_worker_done\s+\(-> parent_sample_\d+\)/,
        'switch local do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch local do materializes before the later generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later switch generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'switch await_all drains both pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the switch pre-do generated spawn parameter override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the switch post-do generated spawn parameter override');
    ...
    is_deeply(
        ...
        'report exposes both switch local-do-then-spawn generated spawn port-binding sets',
    );
    is_deeply(
        $report->{clock_domains}[0]{child_instances},
        [
...
subtest 'switch branch nested repeat generated-child do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_generated_child_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch generated-child do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch generated-child do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch generated-child do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'switch-branch generated-child do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch generated-child do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'switch-branch generated-child do-then-spawn post-await_any keeps the do site plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_generated_child_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_generated_child_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch generated-child do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch post-await_any do and spawns');
    ok(defined($top_fsm), 'switch-branch generated-child do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the generated-child do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch generated-child do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the generated-child do in the post-await_any subset');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch generated-child do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch generated-child do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later switch generated spawn advances to the post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the switch pre-do spawn instance for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top keeps the switch generated do instance for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for post-await_any subset');
};
...
subtest 'switch branch nested repeat generated-child do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_generated_child_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any generated-child-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any generated-child subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any generated-child subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any generated-child subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any generated-child subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any generated-child subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'switch-branch prior-await_any generated-child subset keeps the do instance plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_generated_child_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_generated_child_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any generated-child-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch prior-await_any generated-child-do-then-spawn');
    ok(defined($top_fsm), 'switch-branch prior-await_any generated-child-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any generated-child-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after switch prior await_any materializes before the generated-child do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch generated-child do after prior await_any waits for its generated instance done');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch generated-child do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch post-do generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pre-do spawn instance');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pre-do spawn instance');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top instantiates the switch generated-child do instance separately before the later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance');
};

subtest 'switch branch nested repeat generated-child do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_generated_child_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any generated-child second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any generated-child second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any generated-child second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any generated-child second-await_any subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any generated-child second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any generated-child second-await_any subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'switch-branch prior-await_any generated-child second-await_any subset keeps the do instance plain');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_generated_child_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_generated_child_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any generated-child second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch generated-child second-await_any');
    ok(defined($top_fsm), 'switch-branch prior-await_any generated-child second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any generated-child second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn in the generated-child second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation in the generated-child second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before the generated-child do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch generated-child do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'switch post-do generated spawn advances to the second await_any observation in the generated-child subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pre-do spawn instance for the generated-child second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pre-do spawn instance for the generated-child second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top instantiates the switch generated-child do instance separately before the later spawn and second await_any');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for the generated-child second-await_any subset');
};

subtest 'switch branch nested repeat generated-child do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_generated_child_do_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch generated-child do before post-do await_any records both spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch generated-child do before post-do await_any preserves the first pending spawn');
    ok($instances{w1}, 'switch-branch generated-child do before post-do await_any preserves the second pending spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch generated-child do before post-do await_any records the generated do instance');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch generated-child do before post-do await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [],
        'switch-branch generated-child do before post-do await_any stays plain generated-child do');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_generated_child_do_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_generated_child_do_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch generated-child do before post-do await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch pre-await_any generated-child do');
    ok(defined($top_fsm), 'switch-branch generated-child do before post-do await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the generated-child-do-before-post-do-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch generated-child do before post-do await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second spawn in generated-child post-do await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second switch generated spawn advances to the generated-child do before post-do await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch generated-child do completes before the post-do await_any observation can run');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after switch generated-child do materializes before the post-do await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch post-do await_any observes either generated child without draining the generated-spawn set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch generated-child post-do await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch generated-child post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pending spawn instance before generated-child post-do await_any');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pending spawn instance before generated-child post-do await_any');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker(?:\s|\))/,
        'generated top instantiates the switch generated-child do instance separately before post-do await_any');
};
...
subtest 'switch branch nested repeat parameterized generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_parameterized_do_then_spawn_before_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch parameterized do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch parameterized do-then-spawn preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch parameterized do-then-spawn records the generated do instance');
    ok($instances{w1}, 'switch-branch parameterized do-then-spawn preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch parameterized do-then-spawn preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'switch-branch parameterized do-then-spawn targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'switch-branch parameterized do-then-spawn preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_parameterized_do_then_spawn_before_drain.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_parameterized_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch parameterized do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch parameterized do and spawns');
    ok(defined($top_fsm), 'switch-branch parameterized do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the parameterized do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch parameterized do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch parameterized generated do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch parameterized generated do materializes before the later generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later switch generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both switch generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the switch pre-do spawn instance for parameterized do');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the switch parameterized do instance override');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for parameterized do');
};

subtest 'switch branch nested repeat parameterized generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_parameterized_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch parameterized do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch parameterized do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch parameterized do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'switch-branch parameterized do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch parameterized do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'switch-branch parameterized do-then-spawn post-await_any preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_parameterized_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_parameterized_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch parameterized do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch parameterized post-await_any do and spawns');
    ok(defined($top_fsm), 'switch-branch parameterized do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the parameterized do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch parameterized do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the parameterized generated do in the post-await_any subset');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch parameterized generated do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch parameterized generated do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later switch generated spawn advances to the parameterized post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch parameterized post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch parameterized post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch parameterized post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the switch pre-do spawn instance for parameterized post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the switch parameterized do instance override for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for parameterized post-await_any subset');
};
...
subtest 'switch branch nested repeat parameterized generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_parameterized_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any parameterized-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any parameterized subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any parameterized subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any parameterized subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any parameterized subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any parameterized subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'switch-branch prior-await_any parameterized subset targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'switch-branch prior-await_any parameterized subset preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_parameterized_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_parameterized_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any parameterized-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch prior-await_any parameterized-do-then-spawn');
    ok(defined($top_fsm), 'switch-branch prior-await_any parameterized-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any parameterized-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn for parameterized do');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation for parameterized do');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before parameterized do');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after switch prior await_any materializes before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch parameterized generated do after prior await_any waits for its generated instance done');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch parameterized generated do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch post-do generated spawn advances directly to the mandatory drain for parameterized do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all drains pre-do and post-do generated spawns for parameterized do');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pre-do spawn instance for parameterized prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pre-do spawn instance for parameterized prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch generated do instance before later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for parameterized prior-await_any subset');
};

subtest 'switch branch nested repeat parameterized generated do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_parameterized_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any parameterized second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any parameterized second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any parameterized second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any parameterized second-await_any subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any parameterized second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any parameterized second-await_any subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'switch-branch prior-await_any parameterized second-await_any subset targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'switch-branch prior-await_any parameterized second-await_any subset preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_parameterized_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_parameterized_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any parameterized second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch parameterized second-await_any');
    ok(defined($top_fsm), 'switch-branch prior-await_any parameterized second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any parameterized second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second pre-do spawn in the parameterized second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation in the parameterized second-await_any subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before the parameterized generated do');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch parameterized generated do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'switch post-do generated spawn advances to the second await_any observation in the parameterized subset');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch second await_any observes any pre-do or post-do generated child without draining the set');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pre-do spawn instance for the parameterized second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pre-do spawn instance for the parameterized second-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch generated do instance before the later spawn and second await_any');
    like($top_fsm, qr/\(\?fsmc:w2 worker(?:\s|\))/,
        'generated top keeps the switch post-do spawn instance for the parameterized second-await_any subset');
};

subtest 'switch branch nested repeat parameterized generated do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_parameterized_do_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch parameterized do before post-do await_any records both spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch parameterized do before post-do await_any preserves the first pending spawn');
    ok($instances{w1}, 'switch-branch parameterized do before post-do await_any preserves the second pending spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch parameterized do before post-do await_any records the generated do instance');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch parameterized do before post-do await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'switch-branch parameterized do before post-do await_any targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '16' }],
        'switch-branch parameterized do before post-do await_any preserves static parameter overrides');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_parameterized_do_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_parameterized_do_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch parameterized do before post-do await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch pre-await_any parameterized do');
    ok(defined($top_fsm), 'switch-branch parameterized do before post-do await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the parameterized-do-before-post-do-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch parameterized do before post-do await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second spawn in parameterized post-do await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second switch generated spawn advances to the parameterized generated do before post-do await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch parameterized generated do completes before the post-do await_any observation can run');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after switch parameterized generated do materializes before the post-do await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch post-do await_any observes either generated child without draining the generated-spawn set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch parameterized post-do await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch parameterized post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker(?:\s|\))/,
        'generated top keeps the first switch pending spawn instance before parameterized post-do await_any');
    like($top_fsm, qr/\(\?fsmc:w1 worker(?:\s|\))/,
        'generated top keeps the second switch pending spawn instance before parameterized post-do await_any');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch generated do instance before post-do await_any');
};
...
subtest 'switch branch nested repeat bound generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_bound_do_then_spawn_before_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch bound do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch bound do-then-spawn preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch bound do-then-spawn records the generated do instance');
    ok($instances{w1}, 'switch-branch bound do-then-spawn preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch bound do-then-spawn preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch bound do-then-spawn preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch bound do-then-spawn exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_bound_do_then_spawn_before_drain.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_bound_do_then_spawn_before_drain_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch bound do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch bound do and spawns');
    ok(defined($top_fsm), 'switch-branch bound do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the bound do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch bound do-then-spawn materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the bound generated do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch bound generated do keeps input and output binding handoffs reviewable before the later spawn');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch bound generated do completes before the later generated spawn can start');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch bound generated do materializes before the later generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later switch generated spawn advances directly to the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both switch generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the switch pre-do spawn parameter override for bound do');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the switch bound do instance override');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the switch post-do spawn parameter override for bound do');
    like($top_fsm, qr/\(switch_repeat_bound_do_then_spawn_before_drain\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires switch bound do input handoff separately from both spawns');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data switch_repeat_bound_do_then_spawn_before_drain\.parent_worker_repeat_do_0_data\)/,
        'generated top wires switch bound do output handoff separately from both spawns');
    ...
    is_deeply(
        ...
        'report exposes switch pre-do spawn, bound do, and post-do spawn port-binding provenance',
    );
};

subtest 'switch branch nested repeat bound generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_bound_do_then_spawn_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch bound do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch bound do-then-spawn post-await_any preserves the pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch bound do-then-spawn post-await_any records the generated do instance');
    ok($instances{w1}, 'switch-branch bound do-then-spawn post-await_any preserves the post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch bound do-then-spawn post-await_any preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch bound do-then-spawn post-await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch bound do-then-spawn post-await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_bound_do_then_spawn_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_bound_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch bound do-then-spawn post-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch bound post-await_any do and spawns');
    ok(defined($top_fsm), 'switch-branch bound do-then-spawn post-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the bound do-then-spawn post-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch bound do-then-spawn post-await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch generated spawn starts before the bound generated do in the post-await_any subset');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch bound generated do keeps input and output binding handoffs reviewable before the post-spawn await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch bound generated do completes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch bound generated do materializes before the later generated spawn in the post-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later switch generated spawn advances to the bound post-spawn await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch bound post-spawn await_any observes either generated child without draining the set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch bound post-spawn await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch bound post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the switch pre-do spawn parameter override for bound post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top keeps the switch bound do instance override for post-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the switch post-do spawn parameter override for bound post-await_any subset');
    like($top_fsm, qr/\(switch_repeat_bound_do_then_spawn_before_await_any\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires switch bound do input handoff separately from both spawns in post-await_any subset');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data switch_repeat_bound_do_then_spawn_before_await_any\.parent_worker_repeat_do_0_data\)/,
        'generated top wires switch bound do output handoff separately from both spawns in post-await_any subset');
    ...
    is_deeply(
        ...
        'report exposes switch pre-do spawn, bound do, and post-do spawn port-binding provenance before post-spawn await_any',
    );
};
...
subtest 'switch branch nested repeat bound generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_bound_do_after_await_any_then_spawn
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any bound-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any bound subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any bound subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any bound subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any bound subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any bound subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch prior-await_any bound subset preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch prior-await_any bound subset exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_bound_do_after_await_any_then_spawn.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_bound_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any bound-do-then-spawn parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch prior-await_any bound-do-then-spawn');
    ok(defined($top_fsm), 'switch-branch prior-await_any bound-do-then-spawn generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any bound-do-then-spawn nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch bound pre-do generated spawn starts before the second pre-do spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch bound pre-do generated spawn advances to the prior await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before bound do');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'sample after switch prior await_any materializes before the bound generated do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch bound generated do after prior await_any keeps input and output binding handoffs reviewable');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch bound generated do after prior await_any completes before the later generated spawn');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch bound generated do materializes before the post-do generated spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch post-do generated spawn advances directly to the mandatory drain for bound do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains switch pre-do and post-do generated spawns for bound do');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the first switch pre-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the second switch pre-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch bound generated do instance before later spawn');
    like($top_fsm, qr/\(\?fsmc:w2 worker\s+\(params\s+\(WIDTH 40\)\s+\)\s+\)/s,
        'generated top keeps the switch post-do spawn parameter override for bound prior-await_any subset');
    like($top_fsm, qr/\(switch_repeat_bound_do_after_await_any_then_spawn\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires switch bound do input handoff after prior await_any');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data switch_repeat_bound_do_after_await_any_then_spawn\.parent_worker_repeat_do_0_data\)/,
        'generated top wires switch bound do output handoff after prior await_any');
    ...
    is_deeply(
        ...
        'report exposes switch pre-do spawns, bound do, and post-do spawn port-binding provenance after prior await_any',
    );
};

subtest 'switch branch nested repeat bound generated do after multi-pending await_any can run before later generated spawn, second await_any, and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_bound_do_after_await_any_then_spawn_second_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any bound second-await_any subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch prior-await_any bound second-await_any subset preserves first pre-do spawn');
    ok($instances{w1}, 'switch-branch prior-await_any bound second-await_any subset preserves second pre-do spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch prior-await_any bound second-await_any subset records generated do instance');
    ok($instances{w2}, 'switch-branch prior-await_any bound second-await_any subset preserves post-do spawn');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any bound second-await_any subset preserves do activation provenance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch prior-await_any bound second-await_any subset preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch prior-await_any bound second-await_any subset exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_bound_do_after_await_any_then_spawn_second_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_bound_do_after_await_any_then_spawn_second_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch prior-await_any bound second-await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch bound second-await_any');
    ok(defined($top_fsm), 'switch-branch prior-await_any bound second-await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any bound second-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch pre-do generated spawn advances to the prior await_any observation for bound second-await_any');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either pre-do generated child before bound do');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch bound generated do after prior await_any keeps input and output binding handoffs reviewable before second await_any');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch bound generated do completes before the later generated spawn in the second-await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'switch post-do generated spawn advances to the second await_any observation for bound do');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_await_all_\d+\)[\s\S]*<w2_done[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch second await_any observes any pre-do or post-do generated child without draining the set for bound do');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'switch await_all after the second await_any drains pre-do and post-do generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch bound generated do instance before later spawn and second await_any');
    like($top_fsm, qr/\(switch_repeat_bound_do_after_await_any_then_spawn_second_await_any\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires switch bound do input handoff before second await_any');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data switch_repeat_bound_do_after_await_any_then_spawn_second_await_any\.parent_worker_repeat_do_0_data\)/,
        'generated top wires switch bound do output handoff before second await_any');
    ...
    is_deeply(
        ...
        'report exposes switch pre-do spawns, bound do, and post-do spawn port-binding provenance for second await_any',
    );
};

subtest 'switch branch nested repeat bound generated do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_bound_do_before_await_any
  (clock clk)
  (reset rst_n)
  (interface
    (input start)
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch bound do before post-do await_any records both pending spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    ok($instances{w0}, 'switch-branch bound do before post-do await_any preserves the first pending spawn');
    ok($instances{w1}, 'switch-branch bound do before post-do await_any preserves the second pending spawn');
    ok($instances{parent_worker_repeat_do_0}, 'switch-branch bound do before post-do await_any records the generated do instance');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch bound do before post-do await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{child}, 'worker',
        'switch-branch bound do before post-do await_any targets the generated child transaction');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch bound do before post-do await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch bound do before post-do await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $parent_fsm = $lowered->{files}{'switch_repeat_bound_do_before_await_any.fsm'};
    my $child_fsm = $lowered->{files}{'worker.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_bound_do_before_await_any_top.fsm'};

    ok(defined($parent_fsm), 'switch-branch bound do before post-do await_any parent scheduled .fsm is emitted');
    ok(defined($child_fsm), 'generated child scheduled .fsm is emitted once for switch pre-await_any bound do');
    ok(defined($top_fsm), 'switch-branch bound do before post-do await_any generated top .fsm is emitted');
    like($parent_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the bound-do-before-post-do-await_any nested repeat region');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch bound do before post-do await_any materializes before the first spawn');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch generated spawn starts before the second spawn in bound post-do await_any subset');
    like($parent_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second switch generated spawn advances to the bound generated do before post-do await_any');
    like($parent_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch bound generated do before post-do await_any keeps input and output binding handoffs reviewable');
    like($parent_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch bound generated do completes before the post-do await_any observation can run');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after switch bound generated do materializes before the post-do await_any observation');
    like($parent_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch post-do await_any observes either generated child without draining the generated-spawn set');
    like($parent_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch bound post-do await_any materializes before the mandatory drain');
    like($parent_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch bound post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:w0 worker\s+\(params\s+\(WIDTH 16\)\s+\)\s+\)/s,
        'generated top keeps the first pending spawn instance before switch bound post-do await_any');
    like($top_fsm, qr/\(\?fsmc:w1 worker\s+\(params\s+\(WIDTH 24\)\s+\)\s+\)/s,
        'generated top keeps the second pending spawn instance before switch bound post-do await_any');
    like($top_fsm, qr/\(\?fsmc:parent_worker_repeat_do_0 worker\s+\(params\s+\(WIDTH 32\)\s+\)\s+\)/s,
        'generated top applies static parameter override to the switch bound generated do instance before post-do await_any');
    like($top_fsm, qr/\(switch_repeat_bound_do_before_await_any\.parent_worker_repeat_do_0_addr parent_worker_repeat_do_0\.addr\)/,
        'generated top wires switch bound do input handoff before post-do await_any');
    like($top_fsm, qr/\(parent_worker_repeat_do_0\.data switch_repeat_bound_do_before_await_any\.parent_worker_repeat_do_0_data\)/,
        'generated top wires switch bound do output handoff before post-do await_any');
    ...
    is_deeply(
        ...
        'report exposes both switch pending spawns and bound do-before-post-do-await_any transaction port-binding provenance',
    );
};

subtest 'switch branch nested repeat domain generated do can run before post-do multi-pending await_any before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_domain_do_before_await_any
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch domain do before post-do await_any records both pending spawns and the generated do instance');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'switch-branch domain do before post-do await_any preserves the first pending spawn domain metadata');
    is($instances{w1}{domain}, 'core',
        'switch-branch domain do before post-do await_any preserves the second pending spawn domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch domain do before post-do await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'switch-branch domain do before post-do await_any preserves same-domain metadata on the do instance');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch domain do before post-do await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch domain do before post-do await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'switch_repeat_domain_do_before_await_any__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_domain_do_before_await_any_top.fsm'};

    ok(defined($core_fsm), 'switch-branch domain do before post-do await_any core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch domain do before post-do await_any domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the domain-do-before-post-do-await_any nested repeat region');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(before status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample before switch domain do before post-do await_any materializes before the first spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch same-domain generated spawn starts before the second spawn in domain post-do await_any subset');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'second switch same-domain generated spawn advances to the domain generated do before post-do await_any');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch same-domain generated do before post-do await_any keeps input and output binding handoffs reviewable');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch same-domain generated do completes before the post-do await_any observation can run');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'sample after switch same-domain generated do materializes before the post-do await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch post-do await_any observes either same-domain generated child without draining the generated-spawn set');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch same-domain post-do await_any materializes before the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch same-domain post-do await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core switch_repeat_domain_do_before_await_any__domain_core\)/,
        'domain top instantiates the switch pre-await_any core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the switch pre-await_any core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups both switch pending spawns and the pre-await_any same-domain do with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of switch pre-await_any same-domain do metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups both switch pending spawns and the same-domain do before post-do await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive switch pre-await_any same-domain do metadata');
};
...
subtest 'switch branch nested repeat domain generated do after multi-pending await_any can run before later generated spawn and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_domain_do_after_await_any_then_spawn
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 4,
        'switch-branch prior-await_any domain-do-then-spawn subset records all generated instances');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'switch-branch prior-await_any domain subset preserves first pre-do spawn domain metadata');
    is($instances{w1}{domain}, 'core',
        'switch-branch prior-await_any domain subset preserves second pre-do spawn domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch prior-await_any domain subset preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'switch-branch prior-await_any domain subset preserves same-domain metadata on the do instance');
    is($instances{w2}{domain}, 'core',
        'switch-branch prior-await_any domain subset preserves post-do spawn domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch prior-await_any domain subset preserves static parameter overrides on the do instance');

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'switch_repeat_domain_do_after_await_any_then_spawn__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_domain_do_after_await_any_then_spawn_top.fsm'};

    ok(defined($core_fsm), 'switch-branch prior-await_any domain-do-then-spawn core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch prior-await_any domain-do-then-spawn domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the prior-await_any domain-do-then-spawn nested repeat region');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'first switch same-domain pre-do spawn starts before the second pre-do spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'second switch same-domain pre-do spawn advances to the prior await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch prior await_any observes either same-domain pre-do generated child before domain do');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch same-domain generated do after prior await_any keeps input and output binding handoffs reviewable');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch same-domain generated do after prior await_any completes before the later generated spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w2_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'switch post-do same-domain generated spawn advances directly to the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done w2_done\)\)/,
        'await_all drains switch pre-do and post-do same-domain generated spawns');
    like($top_fsm, qr/\(\?fsmc:core switch_repeat_domain_do_after_await_any_then_spawn__domain_core\)/,
        'domain top instantiates the switch core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the switch core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups switch pre-do spawns, same-domain do, and post-do spawn with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of switch prior-await_any same-domain do-then-spawn metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups switch pre-do spawns, same-domain do, and post-do spawn after prior await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
...
subtest 'switch branch nested repeat domain generated do can run before later generated spawn before await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_domain_do_then_spawn_before_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch domain do-then-spawn subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'switch-branch domain do-then-spawn preserves the pre-do spawn same-domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch domain do-then-spawn preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'switch-branch domain do-then-spawn preserves same-domain metadata on the do instance');
    is($instances{w1}{domain}, 'core',
        'switch-branch domain do-then-spawn preserves the post-do spawn same-domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch domain do-then-spawn preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch domain do-then-spawn exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'switch_repeat_domain_do_then_spawn_before_drain__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_domain_do_then_spawn_before_drain_top.fsm'};

    ok(defined($core_fsm), 'switch-branch domain do-then-spawn core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch domain do-then-spawn domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the same-domain do-then-spawn nested repeat region');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch same-domain generated spawn starts before the generated do');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch same-domain generated do keeps input and output binding handoffs reviewable before the later spawn');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch same-domain generated do completes before the later generated spawn can start');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch same-domain generated do materializes before the later generated spawn');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'later switch same-domain generated spawn advances directly to the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all drains both switch same-domain generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core switch_repeat_domain_do_then_spawn_before_drain__domain_core\)/,
        'domain top instantiates the switch core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the switch core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups switch pre-do spawn, same-domain do, and post-do spawn with their owner domain',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of switch same-domain do-then-spawn metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups switch pre-do spawn, same-domain do, and post-do spawn',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive switch same-domain do-then-spawn metadata');
};

subtest 'switch branch nested repeat domain generated do then spawn can run before post-spawn await_any and await_all' => sub {
    my $source = <<'ISF';
(actor switch_repeat_domain_do_then_spawn_before_await_any
  (clock-domains
    (domain core (clock clk) (reset rst_n) :default)
    (domain aux  (clock aux_clk) (reset aux_rst_n)))
  (interface
...
ISF
    ...
    my $ir = FSM::Scheduler::ISF::LoweringIR->new()->build_module($actor);
    is(scalar(@{$ir->{spawn_instances}}), 3,
        'switch-branch domain do-then-spawn post-await_any subset records both spawns and generated do');
    my %instances = map { $_->{instance} => $_ } @{$ir->{spawn_instances}};
    is($instances{w0}{domain}, 'core',
        'switch-branch domain do-then-spawn post-await_any preserves the pre-do spawn same-domain metadata');
    is($instances{parent_worker_repeat_do_0}{activation_kind}, 'do',
        'switch-branch domain do-then-spawn post-await_any preserves do activation provenance');
    is($instances{parent_worker_repeat_do_0}{domain}, 'core',
        'switch-branch domain do-then-spawn post-await_any preserves same-domain metadata on the do instance');
    is($instances{w1}{domain}, 'core',
        'switch-branch domain do-then-spawn post-await_any preserves the post-do spawn same-domain metadata');
    is_deeply($instances{parent_worker_repeat_do_0}{parameter_overrides}, [{ name => 'WIDTH', value => '32' }],
        'switch-branch domain do-then-spawn post-await_any preserves static parameter overrides on the do instance');
    is_deeply(
        $instances{parent_worker_repeat_do_0}{port_bindings},
        [
            {
        ...
        'switch-branch domain do-then-spawn post-await_any exposes do-site binding handoffs',
    );

    my $lowered = FSM::Scheduler::ISF->new()->lower($actor);
    my $core_fsm = $lowered->{files}{'switch_repeat_domain_do_then_spawn_before_await_any__domain_core.fsm'};
    my $top_fsm = $lowered->{files}{'switch_repeat_domain_do_then_spawn_before_await_any_top.fsm'};

    ok(defined($core_fsm), 'switch-branch domain do-then-spawn post-await_any core-domain scheduled .fsm is emitted');
    ok(defined($top_fsm), 'switch-branch domain do-then-spawn post-await_any domain top .fsm is emitted');
    like($core_fsm, qr/\(parent_switch_\d+[\s\S]*\(=0 \(-> parent_repeat_init_\d+\)\)/,
        'matching switch branch enters the same-domain do-then-spawn post-await_any nested repeat region');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w0_start> 1\)\)[\s\S]*\(-> parent_do_\d+\)/,
        'first switch same-domain generated spawn starts before the generated do in the post-await_any subset');
    like($core_fsm, qr/\(-parent_worker_repeat_do_0_port_bindings\s+\(= \(parent_worker_repeat_do_0_addr> req_addr\)\)\s+\(= \(resp> parent_worker_repeat_do_0_data\) <parent_worker_repeat_do_0_done\)\s+\)/s,
        'switch same-domain generated do keeps input and output binding handoffs reviewable before the post-spawn await_any');
    like($core_fsm, qr/\(parent_do_\d+[\s\S]*\(= \(parent_worker_repeat_do_0_start> 1\)\)[\s\S]*<parent_worker_repeat_do_0_done\s+\(-> parent_sample_\d+\)/,
        'switch same-domain generated do completes before the later generated spawn in the post-await_any subset');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_do status\)\)[\s\S]*\(-> parent_spawn_\d+\)/,
        'sample after switch same-domain generated do materializes before the later generated spawn in the post-await_any subset');
    like($core_fsm, qr/\(parent_spawn_\d+[\s\S]*\(= \(w1_start> 1\)\)[\s\S]*\(-> parent_await_any_\d+\)/,
        'later switch same-domain generated spawn advances to the post-spawn await_any observation');
    like($core_fsm, qr/\(parent_await_any_\d+[\s\S]*<w0_done[\s\S]*\(-> parent_sample_\d+\)[\s\S]*<w1_done[\s\S]*\(-> parent_sample_\d+\)/,
        'switch same-domain post-spawn await_any observes either generated child without draining the set');
    like($core_fsm, qr/\(parent_sample_\d+[\s\S]*\(<= \(after_any status\)\)[\s\S]*\(-> parent_await_all_\d+\)/,
        'sample after switch same-domain post-spawn await_any materializes before the mandatory drain');
    like($core_fsm, qr/\(parent_await_all_\d+[\s\S]*\(-> parent_repeat_check_\d+ <\(& w0_done w1_done\)\)/,
        'await_all after switch same-domain post-spawn await_any drains both generated spawns before nested repeat re-entry');
    like($top_fsm, qr/\(\?fsmc:core switch_repeat_domain_do_then_spawn_before_await_any__domain_core\)/,
        'domain top instantiates the switch post-await_any core-domain scheduled artifact');
    like($top_fsm, qr{/core\.resp/resp/},
        'domain top exposes the switch post-await_any core-domain do output handoff result');
    ...
    is_deeply(
        ...
        'domain partition groups switch pre-do spawn, same-domain do, and post-do spawn before post-spawn await_any',
    );
    is_deeply($ir_domain{aux}{child_instances}, [],
        'domain partition keeps unrelated domains free of switch same-domain do-then-spawn post-await_any metadata');
    ...
    is_deeply(
        ...
        'clock-domain report metadata groups switch pre-do spawn, same-domain do, and post-do spawn before post-spawn await_any',
    );
    is_deeply($reported_domain{aux}{child_instances}, [],
        'unrelated domains do not receive switch same-domain do-then-spawn post-await_any metadata');
};
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat local do after multi-pending await_any then spawn without drain', qr/switch-branch nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_local_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat local do after multi-pending await_any then spawn with second await_any without drain', qr/switch-branch nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_local_do_after_multi_pending_await_any_then_spawn_with_second_await_any_without_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat local do after multi-pending await_any then spawn without drain', qr/when-body nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_local_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat local do after multi-pending await_any then spawn with second await_any without drain', qr/when-body nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_local_do_after_multi_pending_await_any_then_spawn_with_second_await_any_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat parameterized generated do after multi-pending await_any then spawn without drain', qr/when-body nested repeat generated do with static params while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_parameterized_generated_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat generated-child do after multi-pending await_any then spawn without drain', qr/when-body nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_generated_child_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat generated-child do after multi-pending await_any then spawn with second await_any without drain', qr/when-body nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_generated_child_do_after_multi_pending_await_any_then_spawn_with_second_await_any_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat bound generated do after multi-pending await_any then spawn with second await_any without drain', qr/when-body nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_bound_generated_do_after_multi_pending_await_any_then_spawn_with_second_await_any_without_drain
  (clock clk)
  (interface
    (input start)
    (input cond)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat domain generated do after multi-pending await_any then spawn without drain', qr/when-body nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_domain_generated_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat spawn after domain generated do with post-spawn await_any without final drain', qr/when-body nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_spawn_after_domain_generated_do_with_post_await_any_without_final_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat spawn after parameterized generated do with post-spawn await_any without final drain', qr/when-body nested repeat generated do with static params while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_spawn_after_parameterized_generated_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat spawn after bound generated do with post-spawn await_any without final drain', qr/when-body nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_spawn_after_bound_generated_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface
    (input start)
    (input cond)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat spawn after generated-child do with post-spawn await_any without final drain', qr/when-body nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_spawn_after_generated_child_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat local do before post-do multi-pending await_any without drain', qr/when-body nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_local_do_before_post_do_multi_pending_await_any_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat generated-child do before post-do multi-pending await_any without drain', qr/when-body nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_generated_child_do_before_post_do_multi_pending_await_any_without_drain
  (clock clk)
  (interface (input start) (input cond) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'when nested repeat domain generated do before post-do multi-pending await_any without drain', qr/when-body nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor when_nested_repeat_domain_generated_do_before_post_do_multi_pending_await_any_without_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat parameterized generated do after multi-pending await_any then spawn without drain', qr/switch-branch nested repeat generated do with static params while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_parameterized_generated_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat bound generated do after multi-pending await_any then spawn with second await_any without drain', qr/switch-branch nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_bound_generated_do_after_multi_pending_await_any_then_spawn_with_second_await_any_without_drain
  (clock clk)
  (interface
    (input start)
    (input mode)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat bound generated do after multi-pending await_any then spawn without drain', qr/switch-branch nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_bound_generated_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock clk)
  (interface
    (input start)
    (input mode)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat domain generated do after multi-pending await_any then spawn without drain', qr/switch-branch nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_domain_generated_do_after_multi_pending_await_any_then_spawn_without_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat spawn after domain generated do with post-spawn await_any without final drain', qr/switch-branch nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_spawn_after_domain_generated_do_with_post_await_any_without_final_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat bound generated do before post-do multi-pending await_any without drain', qr/switch-branch nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_bound_generated_do_before_post_do_await_any_without_drain
  (clock clk)
  (interface
    (input start)
    (input mode)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat spawn after bound generated do with post-spawn await_any without final drain', qr/switch-branch nested repeat generated do with static params and bindings while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_spawn_after_bound_generated_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface
    (input start)
    (input mode)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat spawn after parameterized generated do with post-spawn await_any without final drain', qr/switch-branch nested repeat generated do with static params while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_spawn_after_parameterized_generated_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat spawn after generated-child do with post-spawn await_any without final drain', qr/switch-branch nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_spawn_after_generated_child_do_with_post_await_any_without_final_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat local do before post-do multi-pending await_any without drain', qr/switch-branch nested repeat local do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_local_do_before_post_do_multi_pending_await_any_without_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat generated-child do before post-do multi-pending await_any without drain', qr/switch-branch nested repeat generated-child do while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_generated_child_do_before_post_do_multi_pending_await_any_without_drain
  (clock clk)
  (interface (input start) (input mode) (input loops (width 3)) (output done))
  (transaction parent
    (on start)
...
ISF
    ...
    assert_lower_rejected(<<'ISF', 'switch nested repeat domain generated do before post-do multi-pending await_any without drain', qr/switch-branch nested repeat generated do with static params and same-domain metadata while generated spawns are pending requires later same-body '\(await_all done\)' before the nested repeat check can loop/);
(actor switch_nested_repeat_domain_generated_do_before_post_do_multi_pending_await_any_without_drain
  (clock-domains
    (domain core (clock clk) (reset rst_n)))
  (interface
    (input start (domain core))
