...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.663: select APB protected generalized timing`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.664` is active after `.663`; select the public contract for bounded APB sideband-aware 32-bit protected generalized register-set timing before any behavior change.
- recently_done: `.663` selected `.664`, public contract selection for bounded APB sideband-aware 32-bit protected generalized `reg0..regN` register-set multi-peripheral back-to-back timing, without behavior changes. The selection follows `.662` because no-policy generalized timing is shipped for both selected 32-bit and data16 widths, while protected generalized register sets still need a public access-policy matrix for `reg2..regN` before any timing guard widens.
...
- next_action: Execute `.664`: settle the exact APB protected generalized public contract, including source names, register cardinality, local-address rules, policy matrix, reports, diagnostics, validation, rollback, docs, Knowledge Map, and next owner.
