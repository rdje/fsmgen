...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.647: audit APB data16-protection readiness`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.648` is active after `.647`; select the public APB data16-protection multi-peripheral multi-register contract.
- recently_done: `.647` selected `.648`, public contract selection for bounded APB sideband-aware data16-protection multi-peripheral multi-register back-to-back timing, without behavior changes. Fixed data16-protection multi-register timing and selected multi-peripheral data16-protection status/control timing are shipped, but no explicit public multi-peripheral multi-register data16-protection source family exists yet.
...
- next_action: Execute `.648`: select exact public `.ppif`/`.apb` source names, source object, anchor, storage/policy shape, requester/completer/interconnect timing requirements, report/residue movement, support-accounting identities, diagnostics, validation gates, rollback, docs, and Knowledge Map for bounded APB data16-protection multi-peripheral multi-register timing before any parser/generator/sample/test/HDL behavior change.
