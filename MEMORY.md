...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.650: select APB status/control protection audit`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.651` is active after `.650`; audit APB status/control protected-storage generalization readiness before any behavior change.
- recently_done: `.650` selected `.651`, readiness audit for APB status/control protected-storage generalization, after `.649` shipped selected data16-protection `reg0`/`reg1` multi-peripheral multi-register timing. No parser, generator, sample, support-accounting, validation, generated artifact, JSON, HDL/runtime, suffix, direct backend, verification-output, backend-language, APB, AXI, AHB, or VHDL behavior changed.
...
- next_action: Execute `.651`: read the `.650` selector and shipped APB status/control/protected timing records, then decide whether the next exact owner is contract selection, direct implementation, a smaller prerequisite, or explicit deferral.
