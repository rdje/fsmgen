...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.37: select the fixed-four AXI read contract`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.37` freezes additive `(axi-read-burst4-transaction-composition ...)` / AxiReadBurst4TransactionComposition/schema-v1 with exact thirteen anchors, LEN3/SIZE2/INCR, aligned 16-byte 4-KiB guard, unchanged AR/R reuse, raw beat event/index, sticky RID/RLAST sequence, count-authoritative drain, 20-port ten-rule coordinator, 29-port three-child C4 at 48 nets/46 links, support target 306/347/347, and t/1507 exact 4/13/4/13/3 proof. `.38` is active atomic implementation. Fact: `docs/knowledge/ial2-axi-read-burst4-transaction-composition-contract-selection.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.37` (done, this slice): froze exact additive syntax/identities/source, parser/generator normalization, legality predicate, child reuse, coordinator IAL1, C4 wiring, lifecycle/drain/reset, report/artifacts, fifteen static strings, fourteen residue entries, diagnostics, support/manifest/test owners, proof, validation, rollback, and atomic `.38`. No behavior changed.
...
- next_action: Implement `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.38` atomically from the `.37` contract: generator, additive PPIF dispatch/source, support/manifest 306/347/347, exact t/1507 plus `.svt` 4/13/4/13/3 proof, shipped mdBook/fact/continuity, cleanup, and full gates while preserving every existing surface. (AHB `.788` BUSY-insertion impl stays deferred.)
