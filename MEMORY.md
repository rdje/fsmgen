...
- latest_commit: `ISF-TRIGGER-ANCHOR.5a` (this commit) — **Ref** trigger: `(point NAME)` body marker + `(at NAME)` → `(state_active <state>)` (module-wide bindings, post-lowering resolution, fail-closed); `t/1416`; `--verify-hdl` clean. `git log -1` for the hash. (`TRACE-SEVERITY-NEVER-GATED` is DONE — `.1`–`.4`, decision `0010`, guard `t/1415`.)
- active_work_unit: `ISF-TRIGGER-ANCHOR` (`.1`–`.4`,`.6`,`.5a` done). Trigger vocabulary nearly complete: event + inline/monitor + window parity + Ref-via-point all ship; `(contract …)` removed. Decision `0010` (trace severity), `0011` (relative paths) also shipped this session.
