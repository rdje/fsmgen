...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.699: select AHB profile alias contract`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.700` is pending after `.699`; implement the bounded AHB `.ahb` profile-alias suffix selected by the contract.
- recently_done: `.699` selected `.700` bounded AHB `.ahb` profile-alias implementation, mirroring `ppif/ahb_requester.ppif` at future `ppif/ahb_requester.ahb` with explicit `(profile ahb)`, generated `amba_requester.isf` before `amba_requester.fsm`, and support identity `intent.ahb_profile_alias_requester/source_kind ial2_profile_alias`; `.698` selected the contract-selection owner; `.697` shipped the bounded AHB requester `.ppif`.
...
- next_action: Execute `.700`: implement only the selected bounded AHB `.ahb` profile-alias suffix, preserving `.ppif` behavior and all broader AHB/AXI/APB/VHDL boundaries.
