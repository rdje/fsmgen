...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.39: select the fixed-four AXI W burst driver`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.39` selects a standalone fixed-four full-width W burst driver because shipped AxiWDriver hard-wires WLAST=1 and cannot be re-armed honestly under AWLEN3. Leading shape captures four data32/strobe4 tuples, drives WLAST 0/0/0/1 with stable payload under stalls, exposes beat event/index plus final done, and preserves all current sources. `.40` is the active readiness audit for exact explicit/packed/streaming payload, generator/schedule/report/proof/owner boundaries. Fact: `docs/knowledge/ial2-post-axi-burst4-read-next-increment-selection.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.39` (done, this slice): compared fixed-four W, immediate AW+W/AW+W+B, dynamic read, capacity adapter, response banks, outstanding queues, malformed-subordinate recovery, and decision-0020 activation; selected standalone fixed-four W as the smallest honest missing primitive and assigned `.40`. No behavior changed.
...
- next_action: Execute `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.40`: audit the fixed-four W driver against exact Issue L anchors and shipped W/write/read schedules; choose explicit fields versus packed bank versus streaming producer; prove the exact WLAST/index/hold/cardinality/reset schedule and freeze every later contract/implementation owner without changing behavior. (AHB `.788` BUSY-insertion impl stays deferred.)
