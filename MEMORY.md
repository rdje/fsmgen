...
- latest_commit: this task-scoped commit, `README-ENTRYPOINT-APPEND-LOG-DRIFT.2: trim objective section to a real entry point`; predecessor is `26409f17f`.
- active_work_unit: `README-ENTRYPOINT-APPEND-LOG-DRIFT` (director-activated `2026-07-25`: README must not be a per-leaf append log; shape 1 trim + shape 3 enforce). `.1` done (decision `0021`, extends `0007`); `.2` done — README.md 9,911 -> 2,803 lines / 928 -> 440 KiB, objective section 7,191 -> 83 lines, zero leaf refs, 5/5 focused tests pass. Next leaf `.3`: de-narrate `## Documentation index` entries (~1,287 doc rows carrying leaf chronology, ~99 corpus rows duplicating `docs/REGRESSION_CORPUS.md`); then `.4` registers the mechanical README doctrine check. `IAL2-FEATURE-COMPLETENESS-FRONTIER` stays `active` but complete through `.811` at 320/361/44 split 22 `.ppif`/22 `.ahb`; its PNT stays paused by director request.
...
- next_action: Continue `README-ENTRYPOINT-APPEND-LOG-DRIFT.3` (de-narrate the `## Documentation index` entries), then `.4` (register the mechanical README doctrine check that makes the append-log regrowth fail the build). Do not resume IAL2 PNT or activate decision 0020 without director instruction. Reuse `ARTIFACT-CLEANUP-JUL25`'s classification table for the next 12-24h sweep.
