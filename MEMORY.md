...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.42: ship the fixed-four AXI W burst driver`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.42` ships additive `(axi-w-burst4-driver ...)` / AxiWBurst4Driver/schema-v1 with seven anchors; four atomic explicit data32/strobe4 tuples; 18 ports/30 signals/zero states; seven rules at 13/6/6/6/6/1/1; seven declared stores/five priorities; continuous WVALID; WLAST 0/0/0/1; accepted-beat event/index2 and final done; exact report/static/residue/diagnostics; one ISF/FSM; t/1508 exact 14/14/3; support 307/348/348. `.43` actively selects the next increment from fixed-four AW+W request/full-write, dynamic/general payload, capacity, queueing, recovery, alias, and decision-0020 candidates. Fact: `docs/knowledge/ial2-axi-w-burst4-driver-behavior.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.42` (done, this slice): shipped the additive fixed-four W parser/generator/public source/support/manifest/book/fact/test path, preserving the single-beat driver and compositions; exact public CLI/HDL proof is 14 handshakes/14 beat events/3 completions with reset abort and recovery, and accounting is 307/348/348.
...
- next_action: Execute `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.43`: audit the shipped W burst4 primitive with reusable AW/B/composition/capacity seams, compare the required bounded next-direction candidates, select exactly one smallest coherent increment plus readiness/contract owner, record a behavior-neutral selector note, synchronize durable surfaces, run gates, and commit. (AHB `.788` BUSY-insertion impl stays deferred.)
