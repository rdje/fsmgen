...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.690: scaffold IAL2 protocol intent chapter`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.691` is pending after `.690`; add AXI tri-mode mdBook coverage with runnable checked-in examples.
- recently_done: `.690` scaffolded the user-facing IAL2 protocol/platform intent mdBook chapter and tri-mode navigation map; `.689` selected documentation-only follow-on leaves `.690` through `.693` for IAL2 AXI/APB/AHB tri-mode mdBook coverage; `.688` shipped APB data16 no-policy six-register timing in commit `7d189d964`.
...
- next_action: Execute `.691`: add AXI tri-mode mdBook coverage using runnable checked-in AXI examples, while keeping AXI framed as the first IAL2 profile/example rather than the IAL2 language boundary.
