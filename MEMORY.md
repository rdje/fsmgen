...
- latest_commit: this task-scoped commit, `README-ENTRYPOINT-APPEND-LOG-DRIFT.1: record bounded-entrypoint decision`; predecessor is `15b28a674`.
- active_work_unit: `README-ENTRYPOINT-APPEND-LOG-DRIFT` (director-activated `2026-07-25`: README must not be a per-leaf append log; shape 1 trim + shape 3 enforce). `.1` done (decision `0021`, extends `0007`). Next leaf `.2`: trim `## Project objective` (README.md lines 74-7265, 7,191 lines / 73% / 1,827 leaf refs) to a real objective section; then `.3` de-narrate the documentation index, `.4` register the mechanical README doctrine check. `IAL2-FEATURE-COMPLETENESS-FRONTIER` stays `active` but complete through `.811` at 320/361/44 split 22 `.ppif`/22 `.ahb`; its PNT stays paused by director request. `ARTIFACT-CLEANUP-JUL25.1` done (sweep + reusable keep/delete table).
