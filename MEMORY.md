...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.35: select bounded AXI multi-beat read composition`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.35` selects a one-outstanding full-width INCR multi-beat read transaction composition as the next functional direction, with fixed four beats (`ARLEN=3`, `ARSIZE=2`, INCR), retained ID4, repeated raw R capture plus per-beat event, exact count/`RLAST` retirement, and a 4-KiB guard as the leading boundary. `.36` is the active behavior-neutral readiness audit. Fact: `docs/knowledge/ial2-post-axi-full-read-next-increment-selection.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.35` (done, this slice): compared multi-beat read/write, physical-to-capacity integration, queueing/outstanding, legality/attributes, aliases, and decision 0020 after full read/write shipped; selected bounded multi-beat INCR read plus `.36` readiness. Capacity-owned storage/demux is not duplicated; aliases add no capability; decision 0020 remains director-gated. No behavior changed.
...
- next_action: Execute behavior-neutral audit `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.36`: choose fixed-four versus bounded authored length, prove repeated R-arm timing and full-width INCR/4-KiB legality, select per-beat event/count/`RLAST`/RID/RRESP/error/reset/drain semantics, and fix topology/report/diagnostic/CLI/proof/owner contracts without changing behavior. (AHB `.788` BUSY-insertion impl stays deferred.)
