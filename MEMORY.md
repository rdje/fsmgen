...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.685: ship APB six-register timing`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.686` is active after `.685`; select the next APB timing/register-set residue owner after 32-bit no-policy six-register generalized timing shipped.
- recently_done: `.685` shipped the bounded APB sideband-aware 32-bit no-policy six-register generalized `reg0..regN` two-peripheral timing family through the byte-identical `.ppif`/`.apb` public source pair.
...
- next_action: Execute `.686`: read `.685` behavior plus the current APB generalized cardinality/residue surfaces, then select the next APB timing/register-set residue owner before any parser/generator/sample/test/HDL behavior change.
