...
- Saved shipped behavior:
  ...
  - and the CLI now prints a bounded composition-failure summary during non-quiet failed composition runs when the raised diagnostic exposes a blocked composition boundary, including a `Lane:` line when that diagnostic already names the active `C1` / `C2` / `C3` / `C4` lane, a concise context line for the offending child/top-port/metadata/token when that context can be separated honestly from longer follow-up detail, plus a concise blocked-reason line.
