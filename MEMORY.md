...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.664: select APB protected generalized contract`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.665` is active after `.664`; implement the selected bounded APB sideband-aware 32-bit protected generalized register-set timing behavior.
- recently_done: `.664` selected `.665`, direct implementation of exactly `ppif/apb_composition_multi_peripheral_multi_register_sideband_protection_generalized_status_back_to_back.ppif` and its `.apb` alias, without behavior changes. The selected policy matrix keeps `reg0` read-allow, requires privileged `PPROT[0] == 1` for `reg0` writes, and requires privileged `PPROT[0] == 1` for every `reg1..regN` read and write.
...
- next_action: Execute `.665`: add the selected APB protected generalized public sources, support identities, capability buckets, focused tests, report/residue movement, diagnostics, behavior docs, mdBook, task-tree, Memory, Knowledge Map, and bounded timing guard widening only for the selected 32-bit `reg0..regN` protected family.
