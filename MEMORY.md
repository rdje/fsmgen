...
- latest_commit: clean `f6c7c1d44 IAL2-AHB-EXACT-TWO-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.7: activate public contract selection`; its message file is cleared.
- active_work_unit: `IAL2-AHB-EXACT-TWO-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.7` is complete pending its task-scoped commit; proposed `.8` owns direct generic implementation after clean `.7`.
...
- recently_done: `.7` selects topology-first generic source `ppif/ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park.ppif`, exact support/coverage/t1525/four-child report-artifact-window-owner/runtime/normalized-semantic/read-only MCP contract, and proposed `.8` implementation. Reserved-name strict/schedule/real MCP probes pass with truthful unmatched support; projected accounting is 319/360/43 split 22 `.ppif`/21 `.ahb`; no behavior changed.
...
- deferred_not_abandoned: proposed `.8` owns only the selected generic source/support/t1525/docs; a later matching alias remains separate, while counts beyond two, decision 0020 and its transaction-layer horizon, runtime BUSY policy/status, larger bursts, queues, and optional signals remain inactive/later.
- in_flight_uncommitted: completed `.7` contract record/fact/task/current-doc/mdBook/Memory/Knowledge Map synchronization awaiting the task-scoped commit; no source/support/test/parser/generator/semantic-MCP/HDL/runtime behavior change.
...
- next_action: Complete final doctrine/commit workflow for `.7`, then from the clean commit activate proposed `.8` before adding the selected source/support/t1525 behavior.
