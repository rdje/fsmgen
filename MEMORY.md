...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.673: select APB data16 five-register contract`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.674` is active after `.673`; select the bounded APB sideband-aware data16 no-policy five-register generalized register-set public contract before behavior changes.
- recently_done: `.673` selected `.674`, public contract selection for bounded APB sideband-aware data16 no-policy five-register generalized `reg0..regN` multi-peripheral timing; no parser, generator, source, support-accounting, report, generated artifact, HDL/runtime, APB, AXI, AHB, or VHDL behavior changed.
...
- next_action: Execute `.674`: select exact data16 no-policy five-register `.ppif`/`.apb` public source names, metadata, report/support expectations, diagnostics, validation probes, rollback, docs, Knowledge Map, and next owner before implementation.
