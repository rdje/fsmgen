...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.682: select APB broader cardinality audit`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.683` is active after `.682`; audit broader APB generalized register-set cardinality before any behavior changes.
- recently_done: `.682` selected `.683`, a no-behavior readiness audit for broader APB generalized register-set cardinality after all four five-register siblings shipped.
...
- next_action: Execute `.683`: read `.682`, the four shipped APB five-register behavior records, current ApbCompleter/ApbComposition guard/residue surfaces, support/tests/docs/KM, then decide whether the next exact owner is more-than-five registers, more-than-two peripheral completers, a smaller report/static diagnostic/address-map/public-fixture prerequisite, explicit deferral, or another bounded APB timing/register-set owner.
