...
- latest_commit: this closeout commit is `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.8: repair direct completion capture`; it closes the child tree after durable predecessor `2738733ea` and its message file must be cleared.
- active_work_unit: `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT` is done through `.8`; no implementation leaf is active. The active parent `IAL2-FEATURE-COMPLETENESS-FRONTIER` needs its next no-behavior selector after the clean `.8` commit.
...
- recently_done: `.8` repairs the distinct direct `fsm/ahb_lite_subordinate.fsm` with Q-named `<-` persistent loads and completion-edge NONSEQ/SEQ dispatch through its existing four states. t1520 proves exact success/final-ERROR/SEQ/IDLE outcomes with no pending bank/relaunch; t1519 preserves the separately repaired generated family; t248+t297 preserve 6,815 support/capability assertions. New current fact: `ial2-ahb-direct-subordinate-register-output-completion-repair`.
...
- deferred_not_abandoned: decision 0020 and its transaction-layer horizon remain proposed/inactive; policy/multiple BUSY, distinct bus-BUSY status, larger bursts, general queues/outstanding transfers, and optional AHB signals remain for later selectors. The bounded generated and direct completion-edge phase repairs are now closed.
- in_flight_uncommitted: none after the `.8` closeout commit; source, tests, docs, fact cards, task status, and resume state are one verified slice.
...
- next_action: From the clean `.8` closeout, add and activate the next no-behavior selector under `IAL2-FEATURE-COMPLETENESS-FRONTIER` before any new implementation; choose from the remaining roadmap-aligned frontier while keeping decision 0020 inactive.
