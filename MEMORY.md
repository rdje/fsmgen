...
- latest_commit: clean `512e65b7e IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.1: prove current single-BUSY cardinality`; its message file is cleared.
- active_work_unit: `IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.2` completed exact current single-BUSY ready/grant-qualified event-cardinality repair contract selection without behavior changes; it awaits validation/commit before `.3` activation.
...
- recently_done: `.2` freezes `single` as exactly one rising `HGRANT && HREADY && HTRANS==BUSY` event, stable BUSY through either qualifier stall, then the same pending `SEQ` through existing address-pending ownership. It selects a conditional accept rule plus ready/BUSY gate with no new public syntax/report/storage/counter. Assertion-enabled public 32-clock ready-low and grant-low probes pass one qualified BUSY/four data beats; `.3` owns implementation. `.1` proved the current ten-edge mismatch and feasible exact-one/exact-two candidates.
- SURFACED (pre-existing, nonblocking to AHB): a declared concurrent-rule-over-transaction priority does not mask different-value registered-output selectors; the candidate hits generated `selector multi-value conflict: HTRANS`. Proposed inactive owner `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT`, fact `isf-rule-transaction-output-priority-gap`; the selected AHB candidate avoids this route.
...
- deferred_not_abandoned: multiple-BUSY syntax/count behavior waits for the current single-event repair; decision 0020 and its transaction-layer horizon remain proposed/inactive; runtime-selected BUSY policy, distinct bus-BUSY status, larger bursts, general queues/outstanding transfers, and optional AHB signals remain later.
- in_flight_uncommitted: `.2` contract record/fact and current task/index/Memory/roadmap/mdBook truth sync; no source, test, generator, artifact, or shipped behavior change.
...
- next_action: Validate and commit `.2`, verify a clean handoff, then activate `.3` and implement the selected exact-one requester BUSY repair.
