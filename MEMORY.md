...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.655: select APB 32-bit protection multi-register sources`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.656` is active after `.655`; implement selected APB sideband-aware 32-bit protected `reg0`/`reg1` multi-peripheral multi-register back-to-back timing behavior.
- recently_done: `.655` selected `.656`, direct implementation of exactly `ppif/apb_composition_multi_peripheral_multi_register_sideband_protection_status_back_to_back.ppif` and `.apb`, without behavior changes. The contract uses one requester, two peripheral completers, 32-bit APB/register data, `PPROT width 3`, `PSTRB width 4`, windows at `0` and `256`, adjacent setup, and protected `reg0`/`reg1` at local addresses `0` and `4`; a temporary exact-name candidate still fails closed at the current multi-peripheral timing guard. No parser, generator, public source, support-accounting identity, capability bucket, schedule/check/semantic JSON contract, generated artifact, HDL behavior, APB transaction behavior, AXI, AHB, or VHDL behavior changed.
...
- next_action: Execute `.656`: add the selected public source pair, exact guard/report/support/test/docs updates, and behavior record for APB 32-bit protected `reg0`/`reg1` multi-peripheral multi-register timing.
