...
- latest_commit: current HEAD after this slice is `IAL2-FEATURE-COMPLETENESS-FRONTIER.790: ship AHB requester BUSY-insertion alias`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.791` is active and unblocked; select the next exact AHB feature-completeness slice after requester BUSY-insertion `.ppif`/`.ahb` parity shipped.
...
- recently_done: `.790` ships `ppif/ahb_requester_busy_insert.ahb` as a byte-identical, data-only mirror of the generic source. Existing suffix handling preserves the generated module/artifacts, BUSY report and held-BUSY/resumed-SEQ behavior while removing only alias residue; no parser/generator change. t1512 proves source/artifact/report/CLI parity, t1498 preserves generated-HDL behavior, `--verify-hdl` passes, and t248/t297 pass at 310 protocol / 351 supported-smoke / 351 strict entries. Facts: `docs/knowledge/ial2-ahb-requester-busy-insertion-profile-alias-behavior.md` and its contract-selection predecessor.
...
- deferred_not_abandoned: decision 0020 and its transaction-layer horizon remain proposed/inactive; `.791` continues only the already-active AHB feature-completeness tree until ongoing work dries out.
...
- blockers: none for `.791`; the pre-existing public-sync failures and latent WRAP risk are separately proposed/inactive. The macOS RAM-guard metric caveat remains tracked by `AGENT-RUNTIME-RAM-GUARD-MACOS-METRIC-REFINEMENT`.
- next_action: Execute `.791` as a no-behavior selector: consult the Knowledge Map and current AHB residue, select exactly one smallest evidence-backed follow-on owner, synchronize docs/task tree/Memory, validate, and commit; keep decision 0020 proposed/inactive.
