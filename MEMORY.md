...
- latest_commit: current HEAD is clean `3e1dcc930 IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.3: repair generated AHB phase pipeline`; its message file is cleared.
- active_work_unit: `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.4` is complete and verified in the working tree, pending its required closeout commit; `.5` must not activate before the repository is clean.
...
- in_flight_uncommitted: completed/verified `.4` task/Memory/KM routing correction, no-behavior t1520 direct-seed audit, audit record/fact, README/ROADMAP/mdBook/current seed-doc sync await commit. The seed and all runtime behavior remain unchanged.
...
- next_action: Run final memory/path/diff/doctrine checks, commit `.4`, clear/verify `git_message_brief.txt`, then activate `.5` from the clean repository for direct-seed contract selection.
