...
- latest_commit: this task-scoped commit, `IAL2-FEATURE-COMPLETENESS-FRONTIER.810: select two-subordinate exact-two alias`; clean predecessor is `a5b7fa236`.
- active_work_unit: `.810` is complete and selects proposed `.811` byte-identical matching `.ahb` alias implementation; `.811` remains pending until this selector commits cleanly.
...
- recently_done: `.810` selects proposed `.811` alias `ppif/ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park.ahb`. Reserved-suffix strict/schedule/normalized-semantic/real read-only MCP probes pass with truthful unmatched support and alias-only residue cleanup; projected accounting is 320/361/44 split 22/22, with t1526 parity and shared t1525 runtime. No behavior changed.
...
- in_flight_uncommitted: `.810` selector docs/fact/task/index/Memory only; no parser/generator/source/support/test/semantic-MCP/HDL/runtime behavior change.
...
- next_action: Commit `.810` cleanly, then activate proposed `.811` before adding the byte-identical alias/support/t1526/docs; do not add a second runtime or activate decision 0020.
