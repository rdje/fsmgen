...
- latest_commit: current HEAD is `28682669f IAL2-FEATURE-COMPLETENESS-FRONTIER.797: select two-subordinate paired AHB BUSY readiness`; `.798` closeout commit is being prepared.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.799` is active; `.798` proved paired generation readiness and selected the report-only prerequisite. `.800` is pending paired contract selection.
...
- recently_done: `.798` passed temporary paired candidate check/schedule/semantic/outdir/SystemVerilog/Yosys probes (ahb_tb, 4 children, 29 signals, 21,656 HDL lines, exact artifacts/windows/busy/parking); no behavior changed. Fact: `ial2-ahb-two-subordinate-paired-busy-readiness-audit`.
- SURFACED (pre-existing two-subordinate BUSY report contradiction): broader residue says continuation is deferred while burst residue says parking ships; `.799` owns the exact report-only repair, preserving non-parking deferral, before `.800` paired contract selection.
...
- deferred_not_abandoned: decision 0020 and its transaction-layer horizon remain proposed/inactive; `.799` continues the current AHB activity without pivoting to that horizon.
- in_flight_uncommitted: `.798` audit/docs/task/Memory/Knowledge Map sync is being committed; temporary `/tmp/fsmgen-ial2-798.CgDp8z` is removed before commit; ignored local-only mirrors remain unchanged.
- blockers: none for `.799`; the macOS RAM-guard host metric remains a known proposed infra repair, so heavyweight runs use the documented equivalent direct pressure/RSS monitor until approved and fixed.
- next_action: Remove the `.798` temporary probe directory, commit `.798`, then implement `.799` report-only wording repair and activate `.800` without activating decision 0020 or proposed audits.
