...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.7: restore exactly-one AW acceptance per command`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.7` (done) shipped the exact existing-ISF correction for the `.5` AW double-handshake in `AxiAwDriver.pm`: transaction samples -> inline `launch_aw_start` -> `launch_aw` rule registers `active_q`/busy/valid/payload -> priority-winning `accept_aw` rule clears active/busy/valid on the `AWVALID && AWREADY` edge -> wait on latched `active_q`. Generated schedule: 6 states, no compile issues, three expected priority resolutions. `t/1499` now has 4 subtests including generated-HDL simulation: continuous READY + stalled/four-cycle stable payload/one-cycle READY produce `PASS handshakes=2 done_pulses=2`, final valid/busy low; `--verify-hdl` lint/Yosys PASS. Guarded t/248+t/297+t/1473: 6649 PASS. The old double-handshake fact is superseded; current fact: `docs/knowledge/ial2-axi-aw-single-transfer-correction-shape.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.7` (done, this slice): implemented and executably regressed exactly-one AW acceptance per command, replaced the mdBook warning with the shipped guarantee, and superseded the old double-handshake fact. `.6` selected/proved the correction; `.5` root-caused the bug; `.1`-.4 selected, audited, contracted, and shipped the original bounded AW driver.
...
- next_action: Execute `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.8`: write the bounded single-beat AXI W-driver readiness audit (WVALID/WDATA32/WSTRB4/WLAST=1/WREADY + busy/done), carrying forward `.7`'s exactly-one acceptance and stalled-payload invariant, then select the exact contract owner. (AHB `.788` BUSY-insertion impl stays deferred.)
