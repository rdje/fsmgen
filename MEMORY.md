...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.697: implement AHB requester PPIF`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.698` is pending after `.697`; perform a no-behavior AHB `.ahb` profile-alias readiness audit.
- recently_done: `.697` shipped the bounded AHB requester generic `.ppif` source at `ppif/ahb_requester.ppif`, generated `amba_requester.isf` and `amba_requester.fsm`, support-accounted `intent.ppif_ahb_requester`, updated mdBook/docs/facts/tests, and kept `.ahb` unsupported; `.696` selected the contract; `.695` audited readiness.
...
- next_action: Execute `.698`: audit readiness for an AHB `.ahb` profile alias after generic AHB requester `.ppif` shipped, without accepting `.ahb` or changing parser/generator behavior.
