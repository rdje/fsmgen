...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.475: select queue recapture report contract`.
...
- recently_done: `IAL2-FEATURE-COMPLETENESS-FRONTIER.475` selected `.476`, readiness audit for identity-preserving same-transaction queue recapture ID refresh. It does not add a positive same-cycle queue recapture report field yet: current queue reports keep `generated_update_rules` as literal emitted-rule lists under `same_id_ordering.dynamic_id_reuse_policy.*.generated_queues[]`, and classic `same_cycle_release_recapture_policy` / `release_recapture_*` fields remain exclusive to dynamic response-demux capture state.
