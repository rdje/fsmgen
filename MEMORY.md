...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.3: select the AW address-channel driver contract`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.4` is pending — the **implementation leaf** (first behavior-landing slice) for the bounded AW address-channel driver. `.1` selected it, `.2` audited it, `.3` (done) fixed the contract in `docs/IAL2_AXI_MANAGER_INITIATOR_AW_DRIVER_CONTRACT_SELECTION.md`: clause `(axi-aw-driver …)`; contract kind `axi_aw_driver`; module `FSM::IAL2::ProtocolIntent::AxiAwDriver`; result kind `protocol_intent.axi_aw_driver`; schema `fsmgen.ial2.protocol_intent.axi_aw_driver.v1`; source `ppif/axi_aw_driver.ppif` (profile axi4). Distinct command inputs (`aw_cmd_valid` + `cmd_awaddr`(32)/`cmd_awid`(4)/`cmd_awlen`(8)/`cmd_awsize`(3)/`cmd_awburst`(2) + `awready`) vs driven AW outputs (`awvalid` + `awaddr`/`awid`/`awlen`/`awsize`/`awburst` + `aw_busy`/`aw_done`), AHB-consistent. First slice drives the FULL burst-describing set; AWID pinned 4; fail-closed axi4 width pins. Target ISF = `accept_command`/`assert_aw`/`finish` drive blocks + an `aw_issue` transaction that samples the command into `*_q` and drives the channel, `(when awready (drive finish))`. `.4` builds `AxiAwDriver.pm` + wires `PPIF.pm` (use import; `axi-aw-driver` clause + `_parse_axi_aw_driver` + accumulator + missing-intent enum in `_contract_from_root`; `_is_axi_aw_driver_contract` + dispatch arm) + `ppif/axi_aw_driver.ppif` + `RegressionCorpus.pm`(t/248) + `LanguageSurfaceSection.pm`(t/297) + `t/14xx` modeled on `t/1473` + a mdBook initiator section in `docs/book/src/16a-ial2-axi.md`.
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.3` (done, this slice): fixed the AW-driver contract (names, distinct command-input/driven-output signal sets, full burst-describing payload, AWID pin 4, target ISF + report shape + residue) resolving the audit §8 questions; spawned `.4` (implementation). Earlier this session (all committed): `.1` selected the AW driver, `.2` wrote the readiness audit; filed active owner `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` (+ KM card ial2-axi-manager-initiator-pivot); `.785`/`.786`/`.787` AHB BUSY-insertion select/audit/contract; `IAL2-MDBOOK-COHERENCE-AXI-COVERAGE` (proposed) for the AXI mdBook coverage gap. No behavior change in any; doc/continuity gates pass.
...
- next_action: Run `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.4`, the implementation leaf — build `FSM::IAL2::ProtocolIntent::AxiAwDriver` per `docs/IAL2_AXI_MANAGER_INITIATOR_AW_DRIVER_CONTRACT_SELECTION.md`, wire `PPIF.pm`, add `ppif/axi_aw_driver.ppif`, the `RegressionCorpus.pm` entry (t/248), the `LanguageSurfaceSection.pm` boundary prose (t/297), `t/14xx-ial2-axi-aw-driver.t` (modeled on t/1473), and a mdBook initiator section; strict-check + lower + `--verify-hdl` + gates. This is the first behavior-landing slice of the thread. (AHB `.788` BUSY-insertion impl stays a deferred pending leaf.)
