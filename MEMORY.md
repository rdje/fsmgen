...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.683: audit APB broader cardinality readiness`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.684` is active after `.683`; select the bounded APB 32-bit no-policy six-register generalized register-set public contract before any behavior changes.
- recently_done: `.683` audited broader APB generalized register-set cardinality and selected `.684`, public contract selection for the first bounded 32-bit no-policy six-register generalized two-peripheral timing family.
...
- next_action: Execute `.684`: select exact `.ppif`/`.apb` source names, object id, anchor, six-register representative `reg0..reg5` local addresses `0/4/8/12/16/20`, 32-bit no-policy family bound `maximum_count = 6`, support identities, reports, diagnostics, validation, rollback, docs, Knowledge Map, and implementation owner before behavior changes.
