...
- latest_commit: this task-scoped commit, `IAL2-FEATURE-COMPLETENESS-FRONTIER.811: ship two-subordinate exact-two alias`; clean activation predecessor is `4c63e45cc`.
- active_work_unit: `.811` is complete at 320/361/44 split 22 `.ppif`/22 `.ahb`; PNT is paused by director request after this commit, with no next leaf selected or activated.
...
- next_action: Wait for director instruction. On resume from the clean tree, select the next exact parent-owned leaf before changes; do not activate decision 0020 implicitly.
