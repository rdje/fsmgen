...
- latest_commit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.652: select APB status/control residue cleanup`.
- active_work_unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.653` is active after `.652`; retire stale APB status/control protected-storage wording from narrowed timing residue.
- recently_done: `.652` selected `.653`, report/static residue cleanup for the already-shipped bounded APB status/control protected-storage generalization. The existing `.638` 32-bit status/control protected source pair and `.634` data16 status/control protected source pair are the public contract; no new public source, support-accounting identity, capability bucket, parser branch, timing branch, generated artifact, HDL behavior, APB transaction behavior, AXI, AHB, or VHDL behavior was selected.
...
- next_action: Execute `.653`: update the narrowed APB timing residue/static prose so selected status/control protected storage is no longer named as live residue, then run focused residue/static checks and doctrine gates.
