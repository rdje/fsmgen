...
- latest_commit: current HEAD closes `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.6: audit direct capture lowering`; its message file is cleared by the required post-commit step.
- active_work_unit: `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.6` is complete, verified, and durably closed; `.7` corrected contract selection is the next frontier and `.8` implementation remains pending behind it.
...
- in_flight_uncommitted: none after the `.6` closeout commit. The failed dispatcher was restored exactly; no behavior repair remains in the tree.
...
- next_action: Activate `.7` from the clean repository and select the exact lowering-safe separated-current/next-bank plus relaunch contract before `.8` implementation.
