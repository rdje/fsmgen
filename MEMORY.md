...
- latest_commit: current HEAD after this slice is `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.5: select the single-transfer correctness prerequisite before W drive`; use `git log -1 --oneline` for the exact hash.
- active_work_unit: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` is active; `.5` (done) keeps a bounded single-beat W write-data driver as the next functional direction under decision `0020`, but selects `.6` as the immediate behavior-neutral correctness audit. A generated ISF/FSM/HDL trace plus a temporary Verilator acceptance-count harness proved a foundational correction to the `.4` note: one `aw_cmd_valid` command produces **two** rising-edge `AWVALID && AWREADY` acceptances when `AWREADY` remains asserted because the registered `AWVALID` clears one edge after entry to the deassert state. `--verify-hdl` proves lint/synthesis cleanliness, not transaction cardinality. Evidence: `docs/IAL2_AXI_MANAGER_INITIATOR_POST_AW_NEXT_INCREMENT_SELECTION.md`; fact card: `docs/knowledge/ial2-axi-aw-driver-double-handshake.md`.
...
- recently_done: `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.5` (done, this slice): selected the driven Valid-Ready single-transfer correctness prerequisite before W drive, reproduced/root-caused the AW double-handshake, synchronized the task tree/index/book, and added a Knowledge Map fact card. `.1`-.4 earlier: selected, audited, contracted, and shipped the bounded AW driver. Earlier: filed active owner (+ KM card ial2-axi-manager-initiator-pivot); `.785`/`.786`/`.787` AHB BUSY-insertion select/audit/contract; `IAL2-MDBOOK-COHERENCE-AXI-COVERAGE` (proposed).
...
- next_action: Execute `IAL2-AXI-MANAGER-INITIATOR-FRONTIER.6`: audit the shipped AW driver's driven Valid-Ready schedule, prove bounded correction candidates against an executable acceptance-count harness, and select the exact behavior owner that restores one accepted transfer per command before W implementation. (AHB `.788` BUSY-insertion impl stays a deferred pending leaf.)
