...
- latest_commit: `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.2: select one-slot phase recapture` is this committed checkpoint; its parent is clean `.1` commit `5cbed61cc`.
- active_work_unit: `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.2` is complete; `.3` is the pending shared-generator/report/runtime implementation leaf and may activate only from this clean checkpoint.
...
- recently_done: `.2` reconciles the second bus acceptance at cycle 37 with delayed ahb_access_done_q at cycle 48 and selects one accepted next address/control bank captured on HSEL && HREADY && active HTRANS. It captures HADDR/HTRANS/optional HBURST/HWRITE/HSIZE/wait_cycles, never HWDATA; final ERROR+IDLE cancels while final ERROR+active captures. `.3` owns shared-generator/report/runtime implementation; `.4` separately audits the direct `.fsm` seed. Facts: `ial2-ahb-pipelined-active-transfer-runtime-audit`, `ial2-ahb-pipelined-active-transfer-contract-selection`.
...
- in_flight_uncommitted: none at this checkpoint; `.3` has not activated and no behavior edit has begun.
...
- next_action: Clear/verify the `.2` commit message file and clean tree, then activate `.3` and implement the selected one-slot generated-subordinate phase-recapture contract.
