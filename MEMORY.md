...
- latest_commit: `ISF-TRIGGER-ANCHOR.5c` (this commit) — **Ref boundary fix**: `(at NAME)` now resolves to a generated `*_active` SIGNAL the named `(point …)`/`(on … as …)` state drives (lazily, like the monitor's arm), NOT `(state_active <state>)` — so ISF-originated asserts never reference `current_state` (ISF↔FSM boundary; user-flagged). `t/1416`; `--verify-hdl` clean (assertion references the signal, current_state absent). `git log -1` for the hash.
