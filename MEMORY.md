...
- DIRECTOR NORTH STAR (pending durable capture — do this next, before/with `.5`): the director described (thinking-aloud future route, explicitly NO pivot) a layered transactor architecture — protocol-agnostic transaction interface (write/read, single or burst) upward; primitive per-(protocol,role) IAL2 role blocks as bus adapters; roles compose into higher-order IAL2 entities that present the transaction interface up the stack; sibling sub-blocks interact via that interface (bridges/converters). Seed already exists: the AHB requester's `local-command`/`local-status` is an embryonic transaction port. TO CAPTURE at the clean checkpoint: a decision record (`docs/decisions/0020-…`) + a **proposed** (not PNT-eligible) horizon owner `IAL2-TRANSACTION-LAYERED-ROLE-COMPOSITION-HORIZON`, and reframe this thread's goal under it. The `0020` draft was written then removed to keep `.4` unmixed; recreate it.
...
- next_action: (1) At the clean checkpoint, capture the DIRECTOR NORTH STAR above — decision record `0020` + proposed horizon owner `IAL2-TRANSACTION-LAYERED-ROLE-COMPOSITION-HORIZON` + reframe this thread's goal under it — and give the director the promised comment on the direction. (2) Then continue the AXI initiator thread: file a `.5` selector for the next increment (W write-data drive toward a write transaction). (AHB `.788` BUSY-insertion impl stays a deferred pending leaf.)
