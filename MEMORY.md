...
- latest_commit: clean `e7f3d7ca9 IAL2-AHB-EXACT-TWO-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.3: ship exact-two paired BUSY composition`; its message file is cleared.
- active_work_unit: `IAL2-AHB-EXACT-TWO-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.4` is active for no-behavior matching `.ahb` profile-alias contract selection.
...
- in_flight_uncommitted: activation-only task/index/Memory state for `.4`; no alias source/support/test/parser/generator/API/HDL/runtime behavior change.
...
- next_action: Commit activation-only `.4`, then reconcile the shipped generic/exact-one aliases and select or reject the exact matching alias contract without behavior changes.
