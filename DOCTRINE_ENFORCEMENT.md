...
registry-driven adapter runner, enforces explicit derive-on-read and
verified-copy field contracts, and invokes the separate cross-revision
...
| `LIVE-DOCUMENT-SIZE` | `scripts/check_live_document_size.sh` | Bounded JSONL control data; lifecycle/locality/pressure/route/index/evidence/retention rules; exact maintained-reference authority; derived-on-read accessors; executed verified-copy, freshness, retrieval, and currency oracles; frozen/archive truth; resulting-tree agreement; complete Markdown coverage. |
...
| `CLAIM-INVENTORY` | `scripts/check_claim_verification_inventory.pl` | Selected high-traffic live-document surfaces and current doctrine constants re-derive to the exact bounded inventory; a separate Git census agrees on every numeric source line; an exact versioned source fixes original-constant membership independently of later fields; incidental partitions and conservative actionable debt remain explicit; every open record names its migration owner. |
