...
my %compatible = (
    ...
    generated_projection  => { generated_file => 1, collection => 1, query => 1 },
...
for my $row (@{$surface_rows}) {
    ...
    if ($record{locator} eq 'collection') {
        ...
        if ($record{lifecycle} eq 'generated_projection') {
            problem("surface $id generated collection needs canonical_inputs") if !@{$canonical};
        } else {
...
for my $id (@surface_order) {
    ...
    } elsif ($record->{lifecycle} eq 'generated_projection'
            && ($record->{locator} eq 'generated_file'
                || $record->{locator} eq 'collection')) {
...
for my $id (@surface_order) {
    ...
    problem("surface $id generated index verifier $generated_id is not a generated projection")
        if $generated->{lifecycle} ne 'generated_projection'
            || ($generated->{locator} ne 'generated_file'
                && $generated->{locator} ne 'collection');
    if ($generated->{locator} eq 'generated_file') {
        ...
        problem("surface $id generated index does not match $generated_id collection index")
            if !defined($generated->{index}) || $generated->{index} ne $record->{index};
    }
