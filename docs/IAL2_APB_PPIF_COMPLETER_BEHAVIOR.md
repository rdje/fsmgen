...
The later `.594` slice adds the sideband-aware 16-bit data multi-register
completer samples:
...
Those samples keep `PADDR` and decoded address width at 32 bits, keep
`wait_cycles` at 4 bits, use 16-bit `PWDATA`/`PRDATA`/register data, derive
2-bit `PSTRB`, and align decoded register addresses to 2-byte boundaries. The
generated byte-lane writes map `PSTRB[0]` to `[7:0]` and `PSTRB[1]` to
`[15:8]`. See
