...
| `ISF-REGISTER-RESET-VALUES` | `active` | `R14` | `ISF-REGISTER-RESET-VALUES.4` (`.1`–`.3` done; `.fsm (reset V)` carrier + ISF `(local … (reset V))` surface → register powers up at V; `.4` = register-map field reset values, `.5` = docs) | [docs/tasks/ISF-REGISTER-RESET-VALUES.md](docs/tasks/ISF-REGISTER-RESET-VALUES.md) |
