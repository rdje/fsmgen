...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.832` selects the nested-bitwise concurrent-assertion tree; child audit `.1` now freezes grouping at inline-intermediate substitution as the general repair and awaits a clean `.2` activation | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `active` | `R14 / generated verification correctness` | `.1` complete: an all-CoreAST trace isolates lost child-parent context at inline `SignalRef` substitution and selects explicit grouping of every rendered replacement; tracked t1544 freezes the current defect, and `.2` awaits clean continuity activation for the renderer/t1410-t1412/t1544/assertion-enabled-t1507 repair | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
...
| `FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY` | `proposed` | `performance/scalability / end-to-end design capacity` | `.832` selects the smaller shipped assertion-correctness audit first; `.1` still owns measurable big/really-big workload, correctness-oracle, resource-profile, budget, graceful-failure, and regression-gate selection before any capacity claim | [docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md](docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md) |
