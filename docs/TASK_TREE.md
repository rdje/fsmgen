...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.829` complete in `a2750d8a6`; generalized AHB BUSY audit `.1` and contract `.2` select active lowerer/test-only implementation `.3` at unchanged 332/373/56 split 28/28 | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
| `IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY insertion` | `.1` proves 2..16 readiness; `.2` freezes canonical admission/residue/t1541/no-per-count-fixture/same-volume boundaries; `.3` active from clean contract commit `7e2b436cf` to implement only that contract | [docs/tasks/IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT.md) |
