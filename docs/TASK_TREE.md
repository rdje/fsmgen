...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.4 selected/audited/contracted/shipped the bounded AW driver. `.5` found its held-READY double-acceptance; `.6` selected a clean existing-ISF correction. `.7` done: shipped the 6-state inline-launch + priority-resolved `launch_aw`/`accept_aw` + latched-`active_q` schedule and executable regression. Schedule has zero compile issues/three priority resolutions; `t/1499` proves exactly one transfer/done per command across continuous READY and stalled one-cycle READY with stable payload; lint/synthesis and guarded t/248+t/297+t/1473 (6649 tests) pass. `.8` pending: readiness-audit the bounded single-beat W driver (WVALID/WDATA32/WSTRB4/WLAST=1/WREADY + busy/done) using the same exactly-once invariant. Deferred: W implementation, AW+W composition, multi-beat sequencing, B completion, AR, burst/address generation, capacity-core integration | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
