...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.775` active; select the public contract for the endpoint AHB subordinate BUSY-in-burst parking source (`.774` audit found the burst machinery ready and the delta bounded, but the contract surface — in-place vs. new stem, the "busy parks" keyword, fail-closed policy — is open) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
