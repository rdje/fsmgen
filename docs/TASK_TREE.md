...
| `FSMGEN-HIR-ROADMAP-FRONTIER` | `active` | `architecture / high-level frontend IR` | `.6` selects private semantic SourceHIR v2, byte-identical `isf/phase_test.isf`, existing-adapter/scheduler re-entry, and exact IAL0 oracle; proposed `.7` implementation is next, then `.8` re-audits promotion | [docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md](docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md) |
