...
| `ISF-NESTED-CROSS-DOMAIN-ACTIVATION` | `active` | `R14` | `ISF-NESTED-CROSS-DOMAIN-ACTIVATION.7` (`.1`–`.6` done; `.6` shipped top-level `when`/`switch` branch-contained `repeat` cross-domain `(do)`) | [docs/tasks/ISF-NESTED-CROSS-DOMAIN-ACTIVATION.md](docs/tasks/ISF-NESTED-CROSS-DOMAIN-ACTIVATION.md) |
| `ISF-LOOP-EARLY-EXIT` | `active` | `R14` | `ISF-LOOP-EARLY-EXIT.5` (optional docs consolidation; `.1`–`.4` done) | [docs/tasks/ISF-LOOP-EARLY-EXIT.md](docs/tasks/ISF-LOOP-EARLY-EXIT.md) |
| `ISF-PROPERTY-SAMPLED-VALUE` | `active` | `R14` | `.1`–`.2` done — `(stable/changed/rose/fell SIG)` → `$stable/$changed/$rose/$fell(SIG)` in `assert/assume/cover` (verilator-simulable, `t/1417`); `.3` value-returning `(past …)` deferred | [docs/tasks/ISF-PROPERTY-SAMPLED-VALUE.md](docs/tasks/ISF-PROPERTY-SAMPLED-VALUE.md) |
