...
| `IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY policy` | `.1` passes with internal `3 -> 2 -> 1 -> 0`, stall stability, semantic/MCP parity, and no lower-layer repair; `.2` is active to freeze literal set, identities, report/residue, support, validation, and generic-before-alias contract without behavior | [docs/tasks/IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT.md) |
