...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.785` pending; select the next bounded AHB feature-completeness increment after the aggregate BUSY-park family (`.784` shipped both aggregate BUSY-park `.ahb` aliases `ahb_interconnect_byte_lane_hburst_seq_busy_park.ahb` + the two-subordinate sibling as byte-identical mirrors, support identities `intent.ahb_profile_alias_interconnect_byte_lane_hburst_seq_busy_park` child count 3 / two-subordinate child count 4, data-only via the existing suffix-keyed suppression; focused `t/1497`, `t/248` 297/338, behavior doc + KM + mdBook synced) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
