...
| --- | --- | --- | --- | --- |
| `ISF-TRIGGER-ANCHOR` | `in-progress` | `R14` | `ISF-TRIGGER-ANCHOR.2` (`.1` design done; decision `0009`) — trigger vocabulary (event/inline/ref) + synthesizable-monitor output-mode so `(contract …)` dissolves into the property engine and is removed (closes `0008`) | [docs/tasks/ISF-TRIGGER-ANCHOR.md](docs/tasks/ISF-TRIGGER-ANCHOR.md) |
