...
| `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT` | `active` | `IAL2 / AHB endpoint phase correctness` | `.6` is complete: it failed the `.5` existing-register contract closed, proved the register-input mux/current-write conflict, restored behavior, and added a structural lock. Next frontier `.7` selects the exact separated-bank/relaunch contract before `.8` implementation | [docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md](docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md) |
