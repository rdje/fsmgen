...
| `ISF-REGISTER-RESET-VALUES` | `active` | `R14` | `ISF-REGISTER-RESET-VALUES.5` (`.1`–`.4` done; `(reset V)` on `(local …)` and `(storage (var …))` → register/CSR powers up at V; verilator-clean; `.5` = a runnable register-map book example + doc-truth sync) | [docs/tasks/ISF-REGISTER-RESET-VALUES.md](docs/tasks/ISF-REGISTER-RESET-VALUES.md) |
