...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1` done (selected the **bounded AW address-channel driver** — issue one `AWVALID` + AW payload `AWADDR`/`AWID`/`AWLEN`/`AWSIZE`/`AWBURST` handshake against `AWREADY` — as the smallest safe first increment, reusing the existing AW valid-ready authoring shape + the `AhbRequester.pm` drive-block model, driven instead of observed; deferred as larger: W write-data drive, AR read-address drive, burst/address generation, capacity-core integration, `.axi` alias; evidence + owner map in `docs/IAL2_AXI_MANAGER_INITIATOR_FIRST_INCREMENT_SELECTION.md`). `.2` pending (readiness audit): map the exact `PPIF.pm`/generator/`RegressionCorpus.pm`/`LanguageSurfaceSection.pm`/`t/14xx`/mdBook owners + fix the AW driver signal/port list and fail-closed rules before contract selection | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
