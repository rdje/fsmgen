...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 shipped/corrected AW; `.8`-.10 shipped single-beat W; `.11`-.14 shipped explicitly armed B acceptance. `.15`-.18 ship bounded request-only write composition; `.19`-.22 ship bounded full-write composition. `.23`-.26 ship `(axi-ar-driver ...)` with t/1504 exact 3/3 proof. `.27`-.30 select, audit, contract, and ship `(axi-r-beat-acceptor ...)` at support 304/345/345 with t/1505 exact raw RID4/RDATA32/RRESP2/RLAST1 3/3 proof. `.31` selects fixed-single-beat AR+R full-read composition; `.32` proves thirteen-anchor/27-port/three-child C4 readiness; `.33` selects exact `(axi-read-transaction-composition ...)` / AxiReadTransactionComposition/schema-v1 contract with unchanged AR/R, zero-state seven-rule coordinator, fixed LEN0/SIZE2/INCR, raw RID4/RDATA32/RRESP2/RLAST1 plus ID/last status, t/1506 exact 5/4/5/4 proof, and 305/346/346 accounting. `.34` active atomic implementation. Deferred: dynamic/multi-beat reads, ARLEN/RLAST counters, response aggregation, capacity integration, decision-0020 transaction interface, back-to-back/outstanding/queues/demux/interleaving, aliases, verification output, backend variants/VHDL/direct, AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
