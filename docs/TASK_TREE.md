...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | requester BUSY `.5` ships additive exact-two generic behavior; completed `.6` selects pending `.7` byte-identical `.ahb` alias implementation with semantic/MCP parity and no second runtime; decision 0020 remains proposed/inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY policy` | `.5` ships optional `(busy-beats 2)` and t1521; completed `.6` selects pending `.7` byte-identical exact-two `.ahb` alias, projected 316/357/40, focused t1522 semantic/MCP parity, and shared t1521 runtime | [docs/tasks/IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.md) |
