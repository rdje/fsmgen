...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.798` proved the four-child paired candidate across CLI/artifact/HDL surfaces and selected `.799`, the active report-only repair for contradictory two-subordinate BUSY wording; `.800` returns to paired contract selection. Decision 0020 remains proposed/inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| --- | --- | --- | --- | --- |
| `IAL2-AHB-REQUESTER-WRAP-PROGRESSION-AUDIT` | `proposed` | `IAL2 / AHB requester correctness` | `IAL2-AHB-REQUESTER-WRAP-PROGRESSION-AUDIT.1` runtime-prove or disprove a latent sequential-clause risk: at the wrap boundary the first clause writes `wrap_base_q`, then the following negated-equality state may re-evaluate the mutated address and overwrite it with base-plus-step; discovered during terminal-count repair, not yet runtime-proven, inactive until ongoing AHB work dries out | [docs/tasks/IAL2-AHB-REQUESTER-WRAP-PROGRESSION-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-WRAP-PROGRESSION-AUDIT.md) |
| `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT` | `proposed` | `IAL2 / AHB endpoint phase correctness` | `.1` runtime-prove consecutive active NONSEQ/SEQ address/data phases without an unselected, IDLE, or BUSY boundary; `.794` now proves the shipped boundary-producing requester and one-transfer subordinate latch only, so broader pipelining remains inactive until ongoing work dries out | [docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md](docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md) |
