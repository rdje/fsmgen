...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.813` selected the now-complete AHB interconnect output-arbitration tree; `.814` is active from clean child commit `6eeac974c` to select the next exact owner; the separate proposed subordinate arbitration audit and decision 0020 remain inactive candidates | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION` | `proposed` | `IAL2 / AHB subordinate correctness` | inactive `.1` must audit the full endpoint selector set after a fabric-suppressed paired probe exposed simultaneous idle-state and `ahb_phase_capture` `HRDATA_REGS <- 0`; activate only from a clean roadmap-selected boundary | [docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md) |
