...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.815` selected the direct IAL0 AHB arbitration tree; its `.1` contract selects proposed four-write implementation `.2` to retire t1520's sole remaining audited AHB `--no-assert` boundary; decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION` | `active` | `IAL0 / AHB direct-seed correctness` | completed `.1` selects proposed `.2`: remove exactly access HREADYOUT/HRESP/HRDATA zero writes plus unsupported HRESP zero, retain exclusive unsupported HREADYOUT/HRDATA, then remove only t1520 `--no-assert`; implementation awaits the clean contract commit | [docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md](docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md) |
