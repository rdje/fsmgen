...
| `FSMGEN-HIR-ROADMAP-FRONTIER` | `active` | `architecture / high-level frontend IR` | `.7` implements and proves the exact private SourceHIR concrete-control-to-IAL1 route; after its clean commit, activate proposed `.8` continuity-only for the two-route promotion audit | [docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md](docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md) |
