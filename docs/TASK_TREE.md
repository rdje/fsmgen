...
| `ISF-PROPERTY-IMPLICATION` | `active` | `R14` | `ISF-PROPERTY-IMPLICATION.3` (`.1`–`.2` done — overlapping implication `(=> A B)` → SVA `(A) \|-> (B)`, verified fires-on-violation; `.3` = next-cycle `\|=>` + `(within S N)` → `##[1:N]`) — step 2 of decision 0008 (enables removing `(contract …)`) | [docs/tasks/ISF-PROPERTY-IMPLICATION.md](docs/tasks/ISF-PROPERTY-IMPLICATION.md) |
