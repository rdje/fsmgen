...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.4 selected/audited/contracted/shipped the bounded AW driver. `.5` found its held-READY double-acceptance; `.6` selected a clean existing-ISF correction; `.7` shipped the 6-state priority-resolved exactly-once schedule and executable regression. `.8` done: audited the separate bounded single-beat W driver (WVALID/WDATA32/WSTRB4/WLAST=1/WREADY + busy/done), pinned legal zero-strobe handling, exactly-once/stall-stability invariants, the .7-derived rule-pair target, all implementation owners, and explicit transaction-level residue. `.9` pending: select the exact W-driver public contract before behavior. Deferred: W implementation, AW+W composition, multi-beat sequencing, B completion, outstanding transactions, AR, burst/address coupling, capacity-core integration, decision-0020 transaction-interface activation, aliases, verification-output, backend variants/VHDL, and AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
