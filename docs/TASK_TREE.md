...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.829` complete in `a2750d8a6`; generalized AHB BUSY audit `.1` proves 2..16 readiness and contract `.2` selects proposed lowerer/test-only implementation `.3` at unchanged 332/373/56 split 28/28 | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
| `IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY insertion` | `.1` complete after 46 structural/report/MCP/diagnostic assertions and seven all-assertion 5/8/16 runtimes; `.2` freezes canonical decimal 2..16 admission, numeric residue, no per-count fixtures, focused t1541, same-volume t1535 repair, unchanged accounting, and proposed `.3` implementation | [docs/tasks/IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-GENERALIZED-BUSY-COUNT-RANGE-READINESS-AUDIT.md) |
...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `proposed` | `ISF / scheduler and output-selector correctness` | inactive; assertion-enabled AHB audit candidate proved declared rule-over-transaction priority does not mask conflicting different-value output selectors; activate only from a clean tree after current AHB work dries out | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
