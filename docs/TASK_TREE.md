...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1` done (selected the **bounded AW address-channel driver** — issue one `AWVALID` + AW payload `AWADDR`/`AWID`/`AWLEN`/`AWSIZE`/`AWBURST` handshake against `AWREADY` — as the smallest safe first increment, reusing the existing AW valid-ready authoring shape + the `AhbRequester.pm` drive-block model, driven instead of observed; deferred: W drive, AR drive, burst/address gen, capacity-core integration, `.axi` alias; `docs/IAL2_AXI_MANAGER_INITIATOR_FIRST_INCREMENT_SELECTION.md`). `.2` done (readiness audit `docs/IAL2_AXI_MANAGER_INITIATOR_AW_DRIVER_READINESS_AUDIT.md`: fixed the safe first-slice AW interface — inputs `aw_cmd_valid`/`awready`+payload, outputs `awvalid`+driven payload+`aw_busy`/`aw_done` — the target ISF shape, fail-closed rules, and the exact owner map with `PPIF.pm` anchors). `.3` done (contract selected `docs/IAL2_AXI_MANAGER_INITIATOR_AW_DRIVER_CONTRACT_SELECTION.md`: clause `(axi-aw-driver …)`, module `AxiAwDriver`, kind `axi_aw_driver`, schema `…axi_aw_driver.v1`, source `ppif/axi_aw_driver.ppif`; distinct command inputs `aw_cmd_valid`+`cmd_aw*` vs driven outputs `awvalid`+`aw*`+`aw_busy`/`aw_done`; full burst-describing payload set; AWID pinned 4; fail-closed axi4 width pins). `.4` done — **shipped the AXI AW address-channel driver generator** (the first AXI initiator behavior-landing slice): `perl/FSM/IAL2/ProtocolIntent/AxiAwDriver.pm`, wired into `PPIF.pm` (new `(axi-aw-driver …)` clause + dispatch), public `ppif/axi_aw_driver.ppif`, `RegressionCorpus.pm`/t248, `LanguageSurfaceSection.pm`/t297, `t/1499` (all subtests incl `--verify-hdl` PASS: verilator+yosys), mdBook initiator section; module `axi_aw_driver`, 7 states, strict-check/lower/`--verify-hdl` clean. Next: a `.5` selector for the W write-data drive toward a write transaction (coordinate with the director's transaction-layered/composable-role North Star, to be captured) | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
