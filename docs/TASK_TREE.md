...
| `LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION` | `active` | `infra/continuity / project-wide live-document lifecycle` | `.2` ships neutral JSONL common enforcement over 20 surfaces and every tracked Markdown path without changing family topology, hard limits, frozen identities, README landing content, or product behavior; next clean selection is independent schema leaf `.6` because `.3` remains blocked by the four-file review | [docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md](docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md) |
