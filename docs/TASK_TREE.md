...
| --- | --- | --- | --- | --- |
| `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE` | `active` | `R14` | current frontier (`.8.29`; select the next effect-proven behavior-widening combination after the `while`/`until` three-spawn post-`do` multi-pending `await_any` plus `await_all` drain slices) | [docs/tasks/ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.md](docs/tasks/ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.md) |
