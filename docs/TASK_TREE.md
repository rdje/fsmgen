...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.13.1` ship bounded VIAL through portable AHB parity and the complete selected simulator-neutral Accellera-UVM emission matrix/review workflow. Active `.13.2` owns exact open-source tool/library inspection and reusable experimental feasibility-probe implementation; `.13.3` records NEXSIM's planned semantic API/MCP qualification plane while exact provider releases remain unavailable. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
