...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.11` ship bounded VIAL semantics through portable AHB parity. Completed `.12` selects simulator-neutral Accellera UVM 2020-3.1 under decision `0050`; active `.13.1` decomposes full reviewable emission without a simulator gate, `.13.1.1` is next for clean activation and the first gallery, `.13.2` then probes exact open-tool compile/elaboration early, and `.13.3` waits on capability-ready PGEN+NEXSIM releases. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
...
| --- | --- | --- | --- | --- |
| `IASIM-EXECUTABLE-REFERENCE-SEMANTICS` | `proposed` | `HIAL/VIAL / executable intent reference semantics / differential validation` | `.1` audits whether the current HIAL semantic projections form a complete executable boundary, selects IASIM's independence and proof claims, and defines one canonical HIAL/VIAL co-simulation model before any engine implementation | [docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md](docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md) |
