...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.13.2` ship bounded VIAL through portable AHB parity, complete selected simulator-neutral Accellera-UVM emission, and an exact reusable Verilator/UVM experimental probe whose partial tool-limited evidence is not product support. `.13.3` records NEXSIM's planned semantic API/MCP qualification plane while exact provider releases remain unavailable; `.14` is the next unblocked architecture leaf. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
