...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.4 selected/audited/contracted/shipped the bounded AW driver. `.5` proved that continuously-high `AWREADY` can accept twice from one command and kept W as the next functional direction. `.6` done: selected and executably proved a clean existing-ISF correction — inline launch handoff + priority-resolved `launch_aw`/`accept_aw` rules + latched `active_q` loop; temporary schedule has 6 states, zero compile issues, lint/synthesis PASS, and two commands across continuous/stalled-one-cycle READY yield exactly two handshakes/two done pulses with stable payload. `.7` pending: implement that correction in `AxiAwDriver.pm` and add the executable cardinality regression before W implementation. Deferred: W implementation, AW+W composition, multi-beat sequencing, B completion, AR, burst/address generation, capacity-core integration | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
