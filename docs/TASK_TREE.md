...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.827` active from clean child behavior commit `a62ddb705`; compare the matching two-window exact-four alias, broader AHB, HIAL/VIAL, verification, VHDL/portability, priority, scale, other protocols/backends, and decision `0020` before selecting exactly one | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `proposed` | `Verification code generation / intent architecture` | remains inactive because `.824` selects the smaller proven exact-four paired alias; retain typed bridge, portable/native semantics, SV/UVM and VHDL lowerings, simulator capability profiles, parity, scalability, and inert-skeleton migration | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
