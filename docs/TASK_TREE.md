...
| `BIN-FSMGEN-IMPORT-TREE-JUL30-IDENTIFIER-REFRESH` | `done` | `bootstrap architecture maintenance` | complete `.1`: canonical note/fact match the 229-file / 228-package / 19-IAL2 closure, Support 71, portable identifier-policy reachability, and current selected line counts | [docs/tasks/BIN-FSMGEN-IMPORT-TREE-JUL30-IDENTIFIER-REFRESH.md](docs/tasks/BIN-FSMGEN-IMPORT-TREE-JUL30-IDENTIFIER-REFRESH.md) |
...
| `TASK-ACCEPTANCE-PORTABLE-DOCTRINE` | `done` | `infra/continuity / doctrine enforcement` | complete `.1`-.3: neutral TSV contract, Git-index checker, FSMGen declarations, 9-group probes, seventh-doctrine registration, and bootstrap/commit/toolbox/template/README/mdBook/decision/fact alignment all ship | [docs/tasks/TASK-ACCEPTANCE-PORTABLE-DOCTRINE.md](docs/tasks/TASK-ACCEPTANCE-PORTABLE-DOCTRINE.md) |
...
| `README-STATIC-LANDING-PAGE` | `done` | `infra/continuity / entry-point documentation` | complete `.1`-.3: README.md became a verified stable landing page; decisions 0038 and 0040 refine decision 0024 with locally authoritative harness-neutral caps plus pressure-closed routed destinations | [docs/tasks/README-STATIC-LANDING-PAGE.md](docs/tasks/README-STATIC-LANDING-PAGE.md) |
...
| `README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE` | `active` | `infra/continuity / entry-point documentation` | `.1` closes the policy/enforcement hole under decision 0040 with a 15-route registry; clean commit `45fc6631e` activates `.2` alone to select a project-/harness-neutral live-document size-containment doctrine plus local remediation owners for the 4.66 MB task file, 1.15 MB book chapter, and multi-megabyte append ledgers without rewriting history | [docs/tasks/README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE.md](docs/tasks/README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE.md) |
...
| `MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC` | `done` | `roadmap/documentation alignment / VHDL` | complete `.1`: Chapters 00/10 now match Chapter 14's bounded shipped direct plus exact C1/C2/C3/APB-C4 VHDL subset and preserve full-backend/GHDL/external-compiler deferrals | [docs/tasks/MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.md](docs/tasks/MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.md) |
| `MDBOOK-AHB-BUSY-COUNT-RESIDUE-SYNC` | `done` | `roadmap/documentation alignment / IAL2 AHB requester` | complete `.1`: Chapter 16c now distinguishes exact-one-through-four catalog fixtures from shipped generic literal counts `5..16` and preserves every above-16/runtime residue boundary | [docs/tasks/MDBOOK-AHB-BUSY-COUNT-RESIDUE-SYNC.md](docs/tasks/MDBOOK-AHB-BUSY-COUNT-RESIDUE-SYNC.md) |
| `FSMGEN-HIR-ROADMAP-FRONTIER` | `done` | `architecture / high-level frontend IR` | complete `.1`-.8: distinct private SourceHIR, exact v1 PPIF/IAL2 and v2 ISF/IAL1 proofs, and decision 0031 retain the validated seam without a third architecture-only route; public producer/projection remains separately proposed | [docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md](docs/tasks/FSMGEN-HIR-ROADMAP-FRONTIER.md) |
...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | `.3` ships bounded VIAL semantics; `.5` the review-routed bridge; `.7.3` the private deterministic plan; decision `0039` and completed `.8` select `fsmgen vial`, semantic-equivalent normal/terse forms, separate VIAL/HIAL inputs, portable API, atomic repository-local artifacts, and manifest compatibility without implementation; clean `.8` commit `d34da3254` activates `.9` alone for plain-SV/Verilator backend-contract selection; decision `0034` keeps VIAL non-synthesis-bounded | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
...
| `PUBLIC-SYNC-TEST-DRIFT-REPAIR` | `done` | `public-contract, specification, and focused-test synchronization` | `.1`-.4 complete: public presence, exact 332/332 ISF test index, t1474 aggregate diagnostic, and four named-drive-priority IAL0 expectations synchronized without product changes | [docs/tasks/PUBLIC-SYNC-TEST-DRIFT-REPAIR.md](docs/tasks/PUBLIC-SYNC-TEST-DRIFT-REPAIR.md) |
| `MDBOOK-RUSTDOC-NON-RUST-FENCE-REPAIR` | `done` | `documentation validation / mdBook` | complete `.1`: exactly four plain-text openings are classified as `text`, diagram bytes are preserved, and all 36 mdBook chapters pass rustdoc plus HTML build | [docs/tasks/MDBOOK-RUSTDOC-NON-RUST-FENCE-REPAIR.md](docs/tasks/MDBOOK-RUSTDOC-NON-RUST-FENCE-REPAIR.md) |
| `PROTOCOL-COMPOSITION-HDL-INSTANCE-IDENTIFIER-AUDIT` | `done` | `HDL quality / protocol composition identifiers` | complete `.1`-.2: decision 0027's portable child-label registry/allocator, authored source guards, emitter defenses, APB/AHB integration, public APB `interconnect_instance` delta, and focused target/tool regressions ship | [docs/tasks/PROTOCOL-COMPOSITION-HDL-INSTANCE-IDENTIFIER-AUDIT.md](docs/tasks/PROTOCOL-COMPOSITION-HDL-INSTANCE-IDENTIFIER-AUDIT.md) |
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `done` | `R14 / generated verification correctness` | complete `.1`-.3: general grouped inline-intermediate rendering, assertion-enabled AXI read proof, and t1502's exact grouped AXI write-request expectation all align without product-output changes in `.3` | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `done` | `ISF / scheduler and output-selector correctness` | `.3` ships exact-one-local-caller bidirectional target-local priority, logical actor reports, and ambiguous-caller fail-closed behavior with executable SV/Verilog proof; separate proposed VHDL reduction task still owns invalid `(|drive_start)` residue | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
...
| `DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING` | `done` | `Backends And Validation / direct VHDL correctness` | `.2` ships scalar/static-bit identity, required-only vector folds, signed casts, condition context, targeted unsupported-shape rejection, and token-free named-drive/AMBA/APB output | [docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md](docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md) |
