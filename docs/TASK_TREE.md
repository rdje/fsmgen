...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `verification architecture` | `.17.3.4.1` caller-sealed structural measurement is complete; proposed `.17.3.4.2` is the next activation frontier for exact resumable matrix publication | [task](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
