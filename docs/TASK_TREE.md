...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 shipped/corrected the bounded AW driver. `.8`-.10 audited, contracted, and shipped the additive single-beat W driver with executable exactly-once/stall-stability/zero-strobe proof. `.11` selected a bounded manager-side B write-response acceptor as the smallest next direction: it fills the missing physical `BVALID && BREADY` plus response-payload seam toward the shipped abstract `write_complete`/`BID` response core. `.12` active: audit arm-vs-bounded-continuously-ready policy, widths/capture/cardinality, generated schedule, fail-closed rules, owners, and exact contract-selection leaf. Later AW+W composition must reuse generated AW/W children plus a distinct coordinator. Deferred: behavior implementation, AW+W/full transactor, multi-beat/burst, outstanding writes, AR/R, capacity integration, decision-0020 transaction-interface activation, aliases, verification-output, backend variants/VHDL, direct backend, and AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
