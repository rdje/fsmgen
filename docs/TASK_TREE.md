...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.13.1.1` now ship bounded VIAL semantics through portable AHB parity plus the deterministic simulator-neutral Accellera UVM 2020-3.1 emitter foundation and first checked gallery. Proposed `.13.1.2` is next for separate clean activation of complete topology/lifecycle/notification structures; `.13.2` is dependency-ready for separately owned experimental open-tool probes, and `.13.3` waits on capability-ready PGEN+NEXSIM releases. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
