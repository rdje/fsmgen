...
| `ISF-REMAINING-BROAD-FRONTIER` | `active` | `R14` | `ISF-REMAINING-BROAD-FRONTIER.9.1` (`.9` selected value-returning `(past SIG [N])` inside assertion/property expressions only; next step is implementation without widening synthesizable expression positions) | [docs/tasks/ISF-REMAINING-BROAD-FRONTIER.md](docs/tasks/ISF-REMAINING-BROAD-FRONTIER.md) |
