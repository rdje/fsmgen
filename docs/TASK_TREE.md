...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | exact-three audit `.1` proves guarded continuous/ready-low/grant-low `3 -> 2 -> 1 -> 0` retirement through the shipped width-two counter with semantic/MCP parity and selects proposed `.2` public contract selection; support remains 320/361/44 and decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY policy` | `.1` passes: one guarded assertion-enabled binary proves internal `3 -> 2 -> 1 -> 0`, stall stability, one resumed SEQ, four data beats, strict/schedule/semantic/MCP parity, and preservation with no lower-layer repair; proposed `.2` owns public contract selection after `.1` commits | [docs/tasks/IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-EXACT-THREE-BUSY-INSERTION-READINESS-AUDIT.md) |
