...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.815` selects proposed direct IAL0 AHB subordinate output-arbitration contract `.1` as the next exact correctness owner because t1520 is the sole remaining audited AHB `--no-assert` boundary; activation waits for the clean selector commit and decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION` | `proposed` | `IAL0 / AHB direct-seed correctness` | `.815` selects pending `.1` because assertion-enabled diagnostics expose separate access/read/ERROR conditional overrides in `fsm/ahb_lite_subordinate.fsm` and t1520 is the sole remaining audited AHB `--no-assert`; activate only after the selector commits cleanly | [docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md](docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md) |
