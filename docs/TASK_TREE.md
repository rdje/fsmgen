...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.13.1.2` ship bounded VIAL semantics through portable AHB parity plus deterministic simulator-neutral Accellera UVM 2020-3.1 foundation, complete selected passive topology/lifecycle, and typed ordered notification/interception emission in the six-source gallery. Proposed `.13.1.3` is next for stimulus/TLM/factory/configuration/RAL/constrained-decision structures; `.13.2` is separately dependency-ready for experimental open-tool probes, and `.13.3` waits on capability-ready PGEN+NEXSIM releases. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
...
| --- | --- | --- | --- | --- |
| `MDBOOK-PARAGRAPH-SPACING` | `proposed` | `project documentation / rendered readability` | `.5` performs a non-urgent in-app-browser audit of the actual rendered book, including ordinary paragraphs, list-contained prose, DOM structure, and effective theme/CSS, before selecting any formatting-only repair | [docs/tasks/MDBOOK-PARAGRAPH-SPACING.md](docs/tasks/MDBOOK-PARAGRAPH-SPACING.md) |
| `XIAL-NATIVE-DEVELOPMENT-FRAMEWORK` | `proposed` | `HIAL/VIAL / complete native design-and-verification ecosystem` | `.1` defines the complete HDL-independent xIAL work loop, product boundaries, IASIM-kernel relationship, reusable IP/VIP model, interactive and batch workflows, and native signoff claim before any framework implementation | [docs/tasks/XIAL-NATIVE-DEVELOPMENT-FRAMEWORK.md](docs/tasks/XIAL-NATIVE-DEVELOPMENT-FRAMEWORK.md) |
| `IASIM-EXECUTABLE-REFERENCE-SEMANTICS` | `proposed` | `HIAL/VIAL / native executable Intent Abstraction semantics / signoff` | `.1` defines IASIM's HDL-independent semantic authority, audits the current HIAL/VIAL executable boundaries, selects Perl 5 for the definition-oriented reference kernel with profiling-gated Rust/C-ABI accelerators only, and selects the independent conformance/signoff proof stack before implementation | [docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md](docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md) |
...
| `IAL2-TRANSACTION-LAYERED-ROLE-COMPOSITION-HORIZON` | `proposed` | `IAL2 / architecture horizon / transaction layer + role composition` | Future North Star, explicitly **no pivot** under decision `0020`: layer protocol-agnostic single/burst `write` and `read` transactions above primitive `(protocol, role)` bus adapters, then compose higher-order IAL2 bridges/converters through that interface. The AHB requester `local-command`/`local-status` is the embryonic transaction port; AXI AW driver `.4` is the first bus-side primitive. Not PNT-eligible until director-activated. | [docs/tasks/IAL2-TRANSACTION-LAYERED-ROLE-COMPOSITION-HORIZON.md](docs/tasks/IAL2-TRANSACTION-LAYERED-ROLE-COMPOSITION-HORIZON.md) |
...
terminal subtree is retrieved and proved by revision, digest, retention
contract, goal, count, closure, verification, and commit evidence. Migration
manifests separately prove full-source identity, semantic closure, working-set
dimensions, and unretained residue without false partition arithmetic. The full schema and examples live
