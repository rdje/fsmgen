...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.831` done: selects proposed no-behavior `DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.1` to audit the foreign unary-reduction token before implementation; pending clean activation commit | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING` | `selected` | `Backends And Validation / direct VHDL correctness` | `.831` selects pending `.1` as the exact next no-behavior audit after direct VHDL leaked SystemVerilog reduction syntax `(|drive_start)`; activate only after the clean selector commit | [docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md](docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md) |
