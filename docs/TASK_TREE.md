...
| `README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE` | `active` | `infra/continuity / entry-point documentation` | `.1` closes the policy/enforcement hole under decision 0040 with a 15-route registry; proposed `.2` next audits current high-water debt, including a 4.66 MB task file, 1.15 MB book chapter, and multi-megabyte append ledgers, then selects exact remediation owners without rewriting history | [docs/tasks/README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE.md](docs/tasks/README-POLICY-ROUTED-DESTINATION-PRESSURE-CLOSURE.md) |
