...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | `.3` ships bounded VIAL semantics; `.5` ships the review-routed bridge; `.6` selects deterministic target-neutral execution; `.7.1` confirms an enum/two-state versus HIAL four-state transaction binding mismatch, `.7.2` is blocked pending the director's exact-identity versus directional-adapter choice, and `.7.3` owns implementation afterward; decision `0034` keeps VIAL non-synthesis-bounded | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
