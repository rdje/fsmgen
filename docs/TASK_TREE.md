...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.786` pending; audit bounded requester-side single BUSY-beat insertion readiness (`.785` selected requester-side BUSY insertion — driving `HTRANS=BUSY` for one held beat inside a `WRAP4`/`INCR4` burst — as the smallest next AHB increment after the subordinate/aggregate BUSY-park `.ppif`/`.ahb` family completed: the shipped requester emits only `idle`/`nonseq`/`seq` with no `busy=2'b01` encoding or `transfer_busy` drive block, so nothing yet drives the BUSY beat the subordinate is built to park across; halfword/word burst SEQ, wider/indefinite bursts, and multi-word progression cross the single-word window / need wider counters, and optional/property-gated signals are an unanchored fresh family — all deferred; selection doc + KM card, no behavior change) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
