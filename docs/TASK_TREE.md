...
| --- | --- | --- | --- | --- |
| `LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION` | `active` | `infra/continuity / project-wide live-document lifecycle` | `.11` is complete: decisions `0048`/`0049` independently retire the former achievement and roadmap-status views with exact Git recovery and no replacement projections. The next clean selection is `.5`, the conditional rationale-ledger migration; pending `.26` retains the broader agent-era inventory | [docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md](docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md) |
