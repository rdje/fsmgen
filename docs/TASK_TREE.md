...
| `PUBLIC-SYNC-TEST-DRIFT-REPAIR` | `active` | `public-contract, specification, and focused-test synchronization` | `.1` complete: exact three-key verification-observation presence family now restores t1131; `.2` remains pending until the clean `.1` implementation commit, and `.3` stays behind `.2` | [docs/tasks/PUBLIC-SYNC-TEST-DRIFT-REPAIR.md](docs/tasks/PUBLIC-SYNC-TEST-DRIFT-REPAIR.md) |
