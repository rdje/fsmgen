...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.813` selected the AHB interconnect output-arbitration tree; child audit `.1` maps five one-window/seven two-window conflicts and completed `.2` selects proposed interconnect implementation `.3`; a separate proposed subordinate arbitration audit owns the feasibility-discovered idle/phase-capture overlap; decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION` | `active` | `IAL2 / AHB interconnect correctness` | `.1` is complete at `c32255645`; `.2` selects complementary per-window and exclusive global response modes; active `.3` implements them with direct-fabric assertion gate t1530 while retaining the paired `--no-assert` boundary | [docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md) |
