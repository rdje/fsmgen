...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.813` selects proposed AHB interconnect default/decode output-arbitration audit `.1` before more paired expansion because shipped aggregate tests retain `--no-assert`; activate the child only after clean selector commit; decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION` | `proposed` | `IAL2 / AHB interconnect correctness` | selected by completed parent `.813`; after its clean selector commit, activate `.1` to reproduce and map the shipped default/mapped selector overlap before choosing any repair | [docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md) |
