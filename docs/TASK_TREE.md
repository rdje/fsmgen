...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.830` done: selects proposed no-behavior `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.1` after proving the unresolved seam is transaction-invoked named-drive selector priority, not the already-working direct rule/transaction assignment path; pending clean activation commit | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `proposed` | `ISF / scheduler and output-selector correctness` | selected by completed `.830`, pending clean activation of no-behavior audit `.1`; direct assignment priority already works, while a transaction-invoked named drive retains owner kind `drive`, remains outside that pass, and can conflict with a rule selector | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
