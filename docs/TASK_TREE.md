...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | `.3` ships bounded VIAL semantics; `.5` the review-routed bridge; `.7.3` the private deterministic plan; `.8` selects public tooling; decision `0043` and completed `.9` select static plain-SV lowering, inactive-edge scheduling, declared-probe adapters, closed trace/results, and exact Verilator 5.046 known-value qualification; completed `.10.1` ships public capabilities/check/canonical normal-terse formatting, `.10.2` all three canonical HIAL plan routes plus virtual/atomic repository-local artifacts, and `.10.3` private deterministic portable-SV emission plus pure trace validation; active `.10.4` owns public publication/exact tool/runtime/results after clean `.10.3` commit `201590d84`; decision `0034` keeps VIAL non-synthesis-bounded | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
