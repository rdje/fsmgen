...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `active` | `ISF / scheduler and output-selector correctness` | `.1` done through clean audit commit `e715a34c7`; `.2` is active to freeze unique-caller target-local priority plus ambiguous-caller fail-closed semantics without changing behavior | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
