...
| --- | --- | --- | --- | --- |
| `LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION` | `active` | `infra/continuity / project-wide live-document lifecycle` | `.10` alone is active from clean `.9` commit `a20d38afc`: split only oversized canonical cards at stable fact boundaries, then replace the monolithic generated map with bounded deterministic shards and query-first discovery; pending `.26` retains the broader agent-era inventory | [docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md](docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md) |
