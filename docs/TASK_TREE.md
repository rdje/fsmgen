...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.817` active as the no-behavior post-exact-three paired selector from clean commit `00d71114d`; it must choose exactly one next roadmap owner | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `proposed` | `Verification code generation / intent architecture` | inactive; define current synthesizable IAL0/1/2 as HIAL and audit peer pure-verification VIAL, typed bridge, portable semantics, native SV/UVM and VHDL lowerings, explicit portable-Verilator versus full-language/UVM simulator profiles, VHDL/mixed-language claim profiles, parity, scalability, and inert-skeleton migration; `.817` evaluates priority without an implicit pivot | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
