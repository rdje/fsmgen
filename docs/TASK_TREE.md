...
| `IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION` | `active` | `IAL2 / AHB subordinate correctness` | active `.1` audits the full endpoint selector set after a fabric-suppressed paired probe exposed simultaneous idle-state and `ahb_phase_capture` `HRDATA_REGS <- 0`; activation is documentation-only from clean selector/scalability-capture boundaries | [docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md) |
