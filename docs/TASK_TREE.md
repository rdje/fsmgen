...
| --- | --- | --- | --- | --- |
| `PROJECT-DATA-LOCALITY-SAME-VOLUME-ADOPTION` | `active` | `infra/continuity / project data locality` | `.1` complete: decision 0022, root contract, exact debt signatures, doctrine registration, Knowledge Map fact, and three adjacent startup-drift owners. `.2` is active for CLI/in-process lowering, Knowledge Map scratch, test harness, and legacy-config storage migration; legacy blobs remain frozen | [docs/tasks/PROJECT-DATA-LOCALITY-SAME-VOLUME-ADOPTION.md](docs/tasks/PROJECT-DATA-LOCALITY-SAME-VOLUME-ADOPTION.md) |
...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.811` ships the byte-identical two-subordinate exact-two `.ahb` alias at 320/361/44 split 22/22, with t1526 normalized-semantic/read-only MCP parity and shared t1525 runtime; PNT is paused by director request after the clean `.811` commit, with no next leaf selected and decision 0020 inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
