...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.832` selects the nested-bitwise concurrent-assertion precedence audit; clean selector commit `1be57f7bd` activates only child `.1` through continuity changes | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `active` | `R14 / generated verification correctness` | `.1` active continuity-only after clean selector `1be57f7bd`: audit concurrent-property intermediate inlining that emits `high & bit3 | bit2` for parsed `high & (bit3 | bit2)`, then freeze a general AST-preserving contract plus t1410-t1412 and assertion-enabled legal-`0x00000004` t1507 coverage | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
