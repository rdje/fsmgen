...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | child tree completes through `.8`: topology-first generic two-subordinate exact-two source/support/t1525 ship at 319/360/43 with normalized semantic/read-only MCP parity; after the clean `.8` commit, select the next exact parent-owned leaf, while decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
