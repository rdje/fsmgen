...
| `DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING` | `active` | `Backends And Validation / direct VHDL correctness` | `.1` done selects scalar identity/complement plus deterministic rejection of unsupported reductions; proposed `.2` awaits a clean audit commit before activation | [docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md](docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md) |
