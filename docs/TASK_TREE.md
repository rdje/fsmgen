...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 selected/audited/contracted/shipped/corrected the bounded AW driver with a six-state exactly-once schedule and executable regression. `.8` audited the separate single-beat W driver. `.9` done: selected additive `(axi-w-driver ...)`, `AxiWDriver`, `axi_w_driver.v1`, exact five-anchor command/channel source, one-beat WLAST=1/zero-strobe-legal semantics, six-state accept-over-launch schedule, report/residue/diagnostics, support owner, and t/1500 generated-HDL contract. `.10` pending: implement it end to end. Deferred: AW+W composition, multi-beat sequencing, B completion, outstanding transactions, AR/R, burst/address coupling, capacity-core integration, decision-0020 transaction-interface activation, aliases, verification-output, backend variants/VHDL, and AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
