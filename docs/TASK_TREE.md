...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.811` ships the byte-identical two-subordinate exact-two `.ahb` alias at 320/361/44 split 22/22. After clean same-volume adoption, `.812` is active to select the next smallest exact owner, starting with literal requester BUSY counts beyond two versus larger policy/status/burst/signal alternatives; decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
