...
| --- | --- | --- | --- | --- |
| `README-ENTRYPOINT-APPEND-LOG-DRIFT` | `proposed` | `infra/continuity / entry-point documentation` | inactive, director-activated only; ramp-up measured the entry-point `README.md` at 9,911 lines / 928 KiB with `## Project objective` alone spanning 7,191 lines (73%) and 1,827 per-leaf references, which is the `MEMORY_ARCHITECTURE.md` §12 "re-narrating git history into prose docs" anti-pattern the memory adoption removed from `MEMORY.md`; four candidate shapes recorded, none selected | [docs/tasks/README-ENTRYPOINT-APPEND-LOG-DRIFT.md](docs/tasks/README-ENTRYPOINT-APPEND-LOG-DRIFT.md) |
