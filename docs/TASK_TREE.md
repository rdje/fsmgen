...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | completed `.815` selected the now-active direct IAL0 AHB subordinate output-arbitration contract `.1` because t1520 is the sole remaining audited AHB `--no-assert` boundary; decision 0020 remains inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION` | `active` | `IAL0 / AHB direct-seed correctness` | active `.1` selects the exclusive direct-seed output-mode contract after assertion-enabled diagnostics exposed separate access/read/ERROR conditional overrides and t1520 remained the sole audited AHB `--no-assert`; activation is documentation-only from clean selector commit `8cae38a73` | [docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md](docs/tasks/IAL0-AHB-DIRECT-SUBORDINATE-OUTPUT-ARBITRATION.md) |
