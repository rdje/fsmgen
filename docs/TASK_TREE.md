...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.780` active; no-behavior readiness audit for bounded aggregate AHB BUSY-parking propagation (`.779` selected it — the endpoint BUSY-park is shipped and its residue defers `aggregate propagation`, while the aggregate residue still lists `BUSY-in-burst handling`; the interconnect already clones the child `seq_policy` verbatim so a `(parked-transfer busy)` child auto-forwards `parks_on=[busy]`) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
