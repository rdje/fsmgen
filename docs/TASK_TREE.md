...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 shipped/corrected AW; `.8`-.10 shipped single-beat W. `.11`-.13 selected/audited/fixed the B response acceptor contract. `.14` ships `(axi-b-response-acceptor ...)` end to end: AxiBResponseAcceptor, schema/source/module/support identities, explicit arm, eager BREADY, BID4/BRESP2 capture, stable outputs, later done, six-state accept-over-arm schedule, and t/1501 generated-HDL proof (unarmed/already-high/delayed BVALID; exactly two handshakes/two done pulses). Counts are 300/341/341. `.15` active: select the next bounded increment across AW+W child-reusing coordination, capacity integration, complete write transactor, AR/R, burst/data coupling, and the decision-0020 transaction interface. Deferred until selected: those directions plus back-to-back/outstanding/extended responses, aliases, verification output, backend variants/VHDL/direct, AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
