...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.781` active; no-behavior public contract selection for the aggregate AHB BUSY-park source(s) (`.780` audit found the aggregate ready — the interconnect composes child subordinate FSMs via `AhbSubordinate->generate` and clones the child `seq_policy` verbatim, so a `(parked-transfer busy)` child parks BUSY with no interconnect generator/parser change; only the stem name(s)/support identity/residue-scope contract is open) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
