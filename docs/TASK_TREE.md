...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.14` selects provider-free VHDL-2008 plus an OSVVM 2026.05 advanced tier and exact unexecuted GHDL 6.0.0 profiles while preserving the inert legacy package. `.13.3` remains provider-blocked; proposed `.15` is next for clean activation/decomposition so emission can precede unavailable runtime qualification. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
