...
| `ISF-PROPERTY-WINDOW-RANGE` | `active` | `R14` | `ISF-PROPERTY-WINDOW-RANGE.2` (`.1` select done) — `(within B MIN MAX)` → `##[MIN:MAX]` bounded window (`min > 1`; SPECFORGE-confirmed `1 <= MIN <= MAX`) | [docs/tasks/ISF-PROPERTY-WINDOW-RANGE.md](docs/tasks/ISF-PROPERTY-WINDOW-RANGE.md) |
