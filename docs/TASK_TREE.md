...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.783` pending; select the matching aggregate AHB BUSY-park `.ahb` alias contract (`.782` shipped both aggregate BUSY-park `.ppif` stems `ahb_interconnect_byte_lane_hburst_seq_busy_park.ppif` child count 3 + the two-subordinate sibling child count 4, source data + `AhbInterconnect.pm:1401` residue narrowing only via `_all_subordinates_park_busy`; focused `t/1496`, `t/248` 295/336, behavior doc + KM + mdBook synced) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
