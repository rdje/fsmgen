...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.3 selected, audited, and fixed the bounded AW-driver contract. `.4` shipped the first AXI initiator behavior (`AxiAwDriver`, public `ppif/axi_aw_driver.ppif`, full tests/accounting/book surface). `.5` done: W single-beat write-data drive remains the next functional direction under decision `0020`, but a generated-artifact trace and executable cardinality probe proved that the shipped registered-output schedule can accept **two** AW transfers from one command when `AWREADY` remains high. `.6` pending: audit the driven Valid-Ready schedule and select the narrow lowering-clean correction plus an exact accepted-transfer-count regression before any W implementation copies the pattern. Deferred: W implementation, AW+W composition, multi-beat sequencing, B completion, AR, burst/address generation, capacity-core integration | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
