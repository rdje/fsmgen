...
| --- | --- | --- | --- | --- |
| `README-ENTRYPOINT-APPEND-LOG-DRIFT` | `active` | `infra/continuity / entry-point documentation` | director-activated `2026-07-25` ("README.md shall NOT be used as a per-leaf append log"), selecting shape 1 (trim to a real entry point) + shape 3 (cap and enforce). `.1` records decision `0021`; next leaf is `.2`, trimming `## Project objective` from 7,191 lines of per-leaf chronology, then `.3` de-narrates the documentation index and `.4` registers the mechanical README doctrine check | [docs/tasks/README-ENTRYPOINT-APPEND-LOG-DRIFT.md](docs/tasks/README-ENTRYPOINT-APPEND-LOG-DRIFT.md) |
