...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.832` selects the proposed no-behavior nested-bitwise concurrent-assertion precedence audit; clean selector commit and separate continuity activation are required before the child changes | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `proposed` | `R14 / generated verification correctness` | selected by parent `.832`; `.1` must audit concurrent-property intermediate inlining that emits `high & bit3 | bit2` for parsed `high & (bit3 | bit2)`, freeze a general AST-preserving contract plus t1410-t1412 and assertion-enabled legal-`0x00000004` t1507 coverage, and remain inactive until clean continuity activation | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
