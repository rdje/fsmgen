...
| --- | --- | --- | --- | --- |
| `LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION` | `active` | `infra/continuity / project-wide live-document lifecycle` | active `.11` has independently audited and now retires the former achievement journal under decision `0048`, with exact version recovery and absence/consumer enforcement. Next, `.11` must audit the still-frozen roadmap-status value independently and ask the director before changing it; pending `.26` retains the broader agent-era inventory | [docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md](docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md) |
