...
| `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT` | `active` | `IAL2 / AHB endpoint phase correctness` | `.2` selects a depth-one accepted address/control bank at the ready/completion edge, not delayed internal done; `.3` is the pending shared-generator/report/runtime repair after the clean `.2` commit, and `.4` later audits the distinct direct seed | [docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md](docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md) |
