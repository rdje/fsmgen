...
| `Aggregate Types And Data` | `active RICHER-AGGREGATE-OPERATORS tree for aggregate operator widening selection; completed AGGREGATE-AUTOGROWTH-FROM-USAGE tree for bounded automatic aggregate growth; completed BACKEND-OWNED-STRUCT-RECORD-DEFAULT-LOWERING tree for exact-contract Verilog-family structured-lowering audit; future task tree required for broader aggregate lowering, VHDL aggregate support, or public type/export APIs outside those trees` |
