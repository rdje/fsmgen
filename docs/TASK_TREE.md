...
| `BACKEND-API-VALIDATION-FRONTIER` | `active` | Backends And Validation / Embedding And Public APIs | `BACKEND-API-VALIDATION-FRONTIER.21` (`.20.1` shipped direct VHDL delayed-pulse clock-branch lowering while arithmetic remains fail-closed; next select the following exact backend/API edge) | [docs/tasks/BACKEND-API-VALIDATION-FRONTIER.md](docs/tasks/BACKEND-API-VALIDATION-FRONTIER.md) |
