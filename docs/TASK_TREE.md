...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.782` active; implement both aggregate AHB BUSY-park `.ppif` stems (`.781` selected the contract — `ahb_interconnect_byte_lane_hburst_seq_busy_park.ppif` child count 3 + the two-subordinate sibling child count 4, each a copy with the child transfer `(ignored-transfer busy)`->`(parked-transfer busy)`; source data + `AhbInterconnect.pm:1401` residue narrowing only, no interconnect code change; focused `t/1496`, `t/248` 295/336) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
