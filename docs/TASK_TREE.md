...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1` pending; audit the AXI manager initiator (bus-driving) surface and select the smallest safe first increment + owner. Director-directed pivot from the AXI response/bookkeeping side: the capacity/status core (`AxiManagerCapacityStatus.pm`, ~9,773 lines) drives **0** AW/AR/W channel signals — FSMGen can monitor a handshake and route responses but cannot yet **issue** an AXI transaction. Model on the AHB requester (`AhbRequester.pm`, the bus-driving analog). Leaning: start from a bounded AW address-channel driver | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
