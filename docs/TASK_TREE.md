...
| `IASIM-EXECUTABLE-REFERENCE-SEMANTICS` | `proposed` | `HIAL/VIAL / native executable Intent Abstraction semantics / signoff` | `.1` selects IASIM's HDL-independent Perl-first semantic/signoff authority; `.8` now owns future NEXSIM API/MCP common checkpoints, replayable first-divergence localization, and layer-specific mismatch classification without activating either runtime | [docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md](docs/tasks/IASIM-EXECUTABLE-REFERENCE-SEMANTICS.md) |
