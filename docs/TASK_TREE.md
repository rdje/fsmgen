...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.831` done through clean selector commit `5f904d2d2`; selected direct-VHDL unary-reduction audit `.1` is now active without a product behavior change | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING` | `active` | `Backends And Validation / direct VHDL correctness` | `.2` active after clean audit commit `16f6140c4`; implement scalar identity/complement, declaration-context threading, and unsupported-reduction fail-closed behavior | [docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md](docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md) |
