...
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `active` | `R14 / generated verification correctness` | `.1`-.2 complete the semantic repair; `.3` active from clean `299db4cae` to synchronize only t1502's stale pre-grouping exact AXI assertion-text expectation | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
