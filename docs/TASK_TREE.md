...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.819` active from clean behavior commit `d94f303d8` to select exactly one next owner after the exact-three paired generic/profile pair reaches 324/365/48 split 24/24; no behavior changes in activation | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `proposed` | `Verification code generation / intent architecture` | inactive after `.817`; define current synthesizable IAL0/1/2 as HIAL and audit peer VIAL, typed bridge, portable/native semantics, SV/UVM and VHDL lowerings, portable event-capable compiled Verilator versus full-language/UVM simulator profiles, VHDL/mixed-language claims, parity, scalability, and inert-skeleton migration | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
