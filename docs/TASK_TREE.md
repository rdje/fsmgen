...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 shipped/corrected the bounded AW driver. `.8`-.10 audited, contracted, and shipped the additive single-beat W driver: `axi_w_driver`, WDATA32/WSTRB4/WLAST=1, legal zero strobe, six-state accept-over-launch exactly-once schedule. `t/1500` generated HDL proves continuous READY plus stable stalled/one-cycle READY produce exactly two handshakes/two done pulses; Verilator/Yosys and support/manifest/AW regressions pass. `.11` pending: compare coordinated AW+W, B completion, complete write transactor, AR, multi-beat/burst coupling, and capacity integration, then select one post-W direction. Deferred until selected: AW+W composition, multi-beat sequencing, B completion, outstanding transactions, AR/R, burst/address coupling, capacity-core integration, decision-0020 transaction-interface activation, aliases, verification-output, backend variants/VHDL, and AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
