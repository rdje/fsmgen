...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.776` active; implement the bounded endpoint AHB subordinate BUSY-in-burst parking source `ppif/ahb_lite_subordinate_byte_lane_hburst_seq_busy_park.ppif` (new `(parked-transfer busy)` vocabulary; IDLE-only `ahb_seq_idle_clear`; `parks_on` report; residue narrowing; focused `t/1494`) — first behavior-bearing slice of the BUSY-park feature | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
