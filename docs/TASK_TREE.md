...
| `BACKEND-API-VALIDATION-FRONTIER` | `active` | Backends And Validation / Embedding And Public APIs | `BACKEND-API-VALIDATION-FRONTIER.4.6.1` (`.4.6` selected `fsm/generic_fifo.fsm`; next resolve or explicitly defer the legacy `?define` / `?&` template boundary) | [docs/tasks/BACKEND-API-VALIDATION-FRONTIER.md](docs/tasks/BACKEND-API-VALIDATION-FRONTIER.md) |
