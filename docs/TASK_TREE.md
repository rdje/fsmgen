...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.777` active; select the matching endpoint AHB subordinate BUSY-park `.ahb` profile alias contract (`.776` shipped the endpoint `.ppif` BUSY-park source with `(parked-transfer busy)`, IDLE-only clear, `parks_on` report, `t/1494`) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
