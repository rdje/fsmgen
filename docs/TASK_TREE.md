...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.830` done through clean selector commit `f67705356`; selected protocol-neutral named-drive priority audit `.1` is now active without an IAL2 behavior change | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `active` | `ISF / scheduler and output-selector correctness` | `.1` done: t1542 proves the direct control, named-drive assertion failure, and shared-caller ambiguity; selects pending no-behavior `.2` to freeze unique-caller target-local masking plus ambiguous-caller fail-closed semantics after a clean audit commit | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
