...
| --- | --- | --- | --- | --- |
| `TRACE-SEVERITY-NEVER-GATED` | `in-progress` | infra/observability | `.2` (`.1` done — ungated `fsm_warn`/`fsm_error`/`fsm_fatal` in `FSM::Debug`, decision `0010`); reroute the 36 severity-bearing gated trace calls (18 files) so warnings/errors/fatals are never masked | [docs/tasks/TRACE-SEVERITY-NEVER-GATED.md](docs/tasks/TRACE-SEVERITY-NEVER-GATED.md) |
| `ISF-TRIGGER-ANCHOR` | `in-progress` | `R14` | `.1`–`.4`,`.6` done — event + inline/monitor triggers + window parity shipped; **`(contract …)` removed** (closes `0008`). Remaining: `.5` Ref `(at NAME)` (open semantic fork), `.6c` residual "contract" wording | [docs/tasks/ISF-TRIGGER-ANCHOR.md](docs/tasks/ISF-TRIGGER-ANCHOR.md) |
...
| `ISF-COMPOUND-ASSIGN` | `active` | `R14` | `ISF-COMPOUND-ASSIGN.2` (`.1` done; `(incr/decr NAME [by N])` compound-assignment sugar — parser desugar to `(set NAME (± NAME N))`) | [docs/tasks/ISF-COMPOUND-ASSIGN.md](docs/tasks/ISF-COMPOUND-ASSIGN.md) |
