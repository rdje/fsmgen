...
| `IAL2-AHB-REQUESTER-EXACT-FOUR-BUSY-INSERTION-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY policy` | `.1` proves assertion-enabled exact-four `4->3->2->1->0` runtime on a 3-bit counter and selects proposed no-behavior `.2` to freeze literal-2..4 plus minimum-width derivation while preserving width two for exact two/three | [docs/tasks/IAL2-AHB-REQUESTER-EXACT-FOUR-BUSY-INSERTION-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-EXACT-FOUR-BUSY-INSERTION-READINESS-AUDIT.md) |
