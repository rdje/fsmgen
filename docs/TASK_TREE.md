...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `active` | `ISF / scheduler and output-selector correctness` | `.1` active after clean selector `f67705356`; audit the working direct assignment control and failing transaction-invoked named-drive selector path, then select exact masking or the smallest fail-closed prerequisite without behavior changes | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
