...
| `ISF-FOR-LOOP` | `active` | `R14` | `ISF-FOR-LOOP.7` (`.1`–`.6` done; indexed counted loop, explicit-width/variable-count, range/strided, and nested `(for (i M) (for (j N)…))` via index hoisting; simulated; `.7` = embedded for in control-flow bodies) | [docs/tasks/ISF-FOR-LOOP.md](docs/tasks/ISF-FOR-LOOP.md) |
