...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.784` pending; implement the matching aggregate AHB BUSY-park `.ahb` aliases (`.783` selected the contract — `ahb_interconnect_byte_lane_hburst_seq_busy_park.ahb` + the two-subordinate sibling, byte-identical mirrors, data-only via the existing suffix-keyed suppression preserving `parks_on=[busy]`; support identities `intent.ahb_profile_alias_interconnect_byte_lane_hburst_seq_busy_park` child count 3 / two-subordinate child count 4; focused `t/1497`, `t/248` 297/338) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
