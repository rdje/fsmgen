...
| `IAL2-MDBOOK-COHERENCE-AXI-COVERAGE` | `active` | `IAL2 mdBook alignment` | `.4` same-ID ordering, mixed populations, and response demultiplexing | [task](docs/tasks/IAL2-MDBOOK-COHERENCE-AXI-COVERAGE.md) |
...
| `IAL2-T1436-PREEXISTING-FAILURES` | `proposed` | `infra / test + HDL-quality hygiene` | `.1` aligns the stale APB multi-peripheral diagnostic expectation; `.2` prevents provisional one-bit intermediate metadata from collapsing multi-bit `== 0`/`== 1` comparisons before authoritative width inference | [docs/tasks/IAL2-T1436-PREEXISTING-FAILURES.md](docs/tasks/IAL2-T1436-PREEXISTING-FAILURES.md) |
