...
| `IAL2-AXI-MANAGER-INITIATOR-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness / AXI initiator` | `.1`-.7 shipped/corrected the bounded AW driver; `.8`-.10 shipped the additive single-beat W driver with executable invariants. `.11` selected a bounded manager-side B write-response acceptor. `.12` audited and proved the safe boundary: explicit one-response arm, eager post-arm `BREADY`, handshake-edge capture of fixed `BID[3:0]`/`BRESP[1:0]`, stable captured outputs, later one-cycle done; temporary six-state ISF/Verilator proof passed two arms/two handshakes/two done pulses. `.13` active: select exact public/generator/report/source/support/test contract and behavior leaf. Later AW+W composition must reuse generated AW/W children plus a distinct coordinator. Deferred: implementation, AW+W/full transactor, back-to-back/outstanding responses, width/sideband extensions, multi-beat/burst, AR/R, capacity integration, decision-0020 transaction-interface activation, aliases, verification-output, backend variants/VHDL, direct backend, and AHB/APB | [docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md](docs/tasks/IAL2-AXI-MANAGER-INITIATOR-FRONTIER.md) |
