...
| --- | --- | --- | --- | --- |
| `LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION` | `active` | `infra/continuity / project-wide live-document lifecycle` | active `.11` has retired the achievement journal and independently audited the still-frozen roadmap-status record. The audit recommends retirement after finding only historical direct-browse value; `.11` now awaits the director's retain/retire/re-form selection before changing that lifecycle. Pending `.26` retains the broader agent-era inventory | [docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md](docs/tasks/LIVE-DOCUMENT-SIZE-CONTAINMENT-ADOPTION.md) |
