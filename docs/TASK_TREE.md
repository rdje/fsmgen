...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.788` pending (deferred by the AXI-initiator pivot; not abandoned); implement the bounded requester-side AHB single BUSY-beat insertion source (`.787` selected the contract: a new additive stem `ppif/ahb_requester_busy_insert.ppif` — actor `amba_requester_busy_insert` — adding a `(busy 2'b01)` HTRANS encoding + a `(busy-before-beat N)` clause that drives one held `HTRANS=BUSY` beat before `SEQ` beat `beat_index N` holding address/control and not advancing counters, via a `transfer_busy` drive block + a `busy_inserted_q` one-shot; fail-closed for N outside 1..15/non-literal/missing busy; report `busy_insertion` block + residue; no new bus-BUSY port; support `intent.ppif_ahb_requester_busy_insert`, `t/1498`, `t/248` 298/339; contract doc + KM card, no behavior change) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
