...
| `ISF-MULTIBIT-LOOP-PREDICATE-TRUTHINESS-REPAIR` | `active` | `R14 / ISF lowering correctness` | `.1` done: bare known-width multi-bit `while`/`until` predicates now branch on nonzero at entry/retest while preserving one-bit lowering, with generated-HDL proof over every three-bit value and the former AHB `4`-beat stall. `.2` active: repair the independently exposed AHB terminal-beat `1 → 0 → 31` sequential-clause defect and prove generated-HDL `INCR4` completion | [docs/tasks/ISF-MULTIBIT-LOOP-PREDICATE-TRUTHINESS-REPAIR.md](docs/tasks/ISF-MULTIBIT-LOOP-PREDICATE-TRUTHINESS-REPAIR.md) |
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.788` blocked on `ISF-MULTIBIT-LOOP-PREDICATE-TRUTHINESS-REPAIR.1`; the generated requester currently stalls when its five-bit bare loop predicate holds `4`, so the bounded requester-side AHB single BUSY-beat insertion source cannot be runtime-proven until shared loop truthiness is repaired; `.787` contract remains selected and unchanged | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
