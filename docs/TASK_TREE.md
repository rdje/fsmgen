...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.787` pending; select the public contract for the requester-side AHB BUSY-beat insertion source (`.786` audited requester BUSY insertion — driving `HTRANS=BUSY` for one held beat, holding addr/control and not advancing counters — and found the burst machinery ready (beat_index_q/beats_remaining_q width 5, all eight burst encodings) but the contract surface open: `busy=2'b01` encoding + insertion-clause syntax, new stem vs in-place widen of `ppif/ahb_requester.ppif`, the `transfer_busy` drive block + beat-loop insertion point/SEQ re-entry, fail-closed, `local_status` reporting; receiving side already parks; readiness-audit doc + KM card, no behavior change) | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
