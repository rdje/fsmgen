...
| `IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT` | `active` | `IAL2 / AHB endpoint phase correctness` | `.7` is complete: warning-clean four-scenario proof selects Q-named `<-` loads plus the existing four-state dispatcher and rejects D-input bank/relaunch `UNOPTFLAT`/latency. Next frontier `.8` implements that exact contract | [docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md](docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md) |
