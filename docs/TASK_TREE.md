...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.833` selects proposed no-behavior `MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.1`; next clean action is selector commit, then child activation | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC` | `proposed` | `roadmap/documentation alignment / VHDL` | selected by parent `.833`; `.1` aligns Chapter 00 and Chapter 10 wording with Chapter 14's bounded shipped direct/composition VHDL subset while preserving full-backend/GHDL deferrals; activation waits for the clean selector commit | [docs/tasks/MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.md](docs/tasks/MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.md) |
...
| `FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY` | `proposed` | `performance/scalability / end-to-end design capacity` | `.833` selects the smaller mdBook VHDL truth repair first; `.1` still owns measurable big/really-big workload, correctness-oracle, resource-profile, budget, graceful-failure, and regression-gate selection before any capacity claim | [docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md](docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md) |
