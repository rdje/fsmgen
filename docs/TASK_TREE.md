...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.830` done through clean selector `f67705356`; selected named-drive child is through audit `.1` and contract `.2`, and implementation `.3` is active while IAL2 behavior/accounting remain unchanged | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT` | `active` | `ISF / scheduler and output-selector correctness` | `.3` active after clean contract commit `b44afcc51`: implement exact-one-local-caller bidirectional target-local priority and ambiguous-caller fail-closed behavior; SV/Verilog are executable gates, while separate proposed VHDL reduction task owns invalid `(|drive_start)` residue | [docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md](docs/tasks/ISF-RULE-TRANSACTION-OUTPUT-PRIORITY-ENFORCEMENT.md) |
