...
| `IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION` | `active` | `IAL2 / AHB interconnect correctness` | `.1` is complete: fresh address-zero/nonzero runs reproduce `HADDR_REGS`, metadata maps five one-window/seven two-window conflicting outputs, and proposed `.2` owns generator-local mutually exclusive arbitration contract selection after clean audit commit | [docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-INTERCONNECT-DEFAULT-DECODE-OUTPUT-ARBITRATION.md) |
