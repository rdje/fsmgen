...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `active` | `Verification code generation / intent architecture` | Completed `.3`–`.12` ship bounded VIAL semantics through portable AHB parity and select simulator-neutral Accellera UVM 2020-3.1. Active `.13.1.1` now owns the native emitter substrate, deterministic artifact graph, static validators, and first gallery without a simulator gate; `.13.2` then probes exact open-tool compile/elaboration early, and `.13.3` waits on capability-ready PGEN+NEXSIM releases. | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
