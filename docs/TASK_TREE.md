...
| `IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION` | `proposed` | `IAL2 / AHB subordinate correctness` | `.814` selects pending `.1` to audit the full endpoint selector set after a fabric-suppressed paired probe exposed simultaneous idle-state and `ahb_phase_capture` `HRDATA_REGS <- 0`; activate only after the selector commits cleanly | [docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md](docs/tasks/IAL2-AHB-SUBORDINATE-DEFAULT-PHASE-OUTPUT-ARBITRATION.md) |
