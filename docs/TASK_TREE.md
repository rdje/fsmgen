...
| `PROJECT-STATUS-AND-CHANGELOG-POLICY-REVIEW` | `proposed` | `infra/continuity / project-document lifecycle` | proposed `.1`: later audit the purpose, audience, overlap, consumers, costs, and retain/redefine/derive/merge/archive/retire options for all four legacy project documents; current policy remains unchanged until an evidence-backed replacement is approved | [docs/tasks/PROJECT-STATUS-AND-CHANGELOG-POLICY-REVIEW.md](docs/tasks/PROJECT-STATUS-AND-CHANGELOG-POLICY-REVIEW.md) |
...
| `README-STATIC-LANDING-PAGE` | `done` | `infra/continuity / entry-point documentation` | complete `.1`-.3: README.md is a verified 244-line / 9,679-byte stable landing page; README_POLICY.md is the 61-line project-neutral shareable standard; decision 0024 and a 300-line / 16,384-byte doctrine gate prevent regrowth; roadmap/bootstrap/mdBook navigation routes live state to Memory plus task trees | [docs/tasks/README-STATIC-LANDING-PAGE.md](docs/tasks/README-STATIC-LANDING-PAGE.md) |
| `README-POLICY-STORAGE-LOCATION` | `active` | `infra/continuity / entry-point documentation` | active `.1`: clarify that adopting projects keep the tracked README_POLICY.md at repository root beside README.md, then synchronize the mdBook pointer and close the leaf | [docs/tasks/README-POLICY-STORAGE-LOCATION.md](docs/tasks/README-POLICY-STORAGE-LOCATION.md) |
...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | `.835`, its import-map child, and the director-requested `README-STATIC-LANDING-PAGE` continuity tree are complete; next clean PNT action is to select the smallest exact roadmap-aligned owner after `.835` | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE` | `proposed` | `Verification code generation / intent architecture` | `.834` selects the smaller frozen-workflow truth repair first; typed bridge, portable/native semantics, SV/UVM and VHDL lowerings, simulator profiles, parity, scalability, and inert-skeleton migration remain intact for a later clean selector | [docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md](docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md) |
...
| `ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR` | `done` | `R14 / generated verification correctness` | complete `.1`-.2: all-CoreAST audit plus grouped inline-intermediate substitution, direct/temporal/property regressions, unchanged AXI admission, expanded `5/17/5/17/4` behavior, and separate assertion-enabled legal-`0x00000004` `1/4/1/4/1` runtime | [docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md](docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md) |
...
| `FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY` | `proposed` | `performance/scalability / end-to-end design capacity` | `.834` selects the smaller frozen-workflow truth repair first; `.1` still owns measurable big/really-big workload, correctness-oracle, resource-profile, budget, graceful-failure, and regression-gate selection before any capacity claim | [docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md](docs/tasks/FSMGEN-END-TO-END-LARGE-DESIGN-SCALABILITY.md) |
