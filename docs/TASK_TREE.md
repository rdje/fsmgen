...
| `IAL2-FEATURE-COMPLETENESS-FRONTIER` | `active` | `IAL2 / SV-backed feature completeness` | requester multiple-BUSY audit `.1` is complete; after its clean commit, `.2` owns exact current single-BUSY event-cardinality repair contract selection; decision 0020 remains proposed/inactive | [docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md](docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md) |
...
| `IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT` | `active` | `IAL2 / AHB requester BUSY policy` | `.1` found the shipped `beats=single` requester emits ten ready-qualified BUSY edges, proved one-/two-event candidates, and hands the clean post-commit frontier to `.2` exact single-BUSY event-cardinality repair contract selection | [docs/tasks/IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.md](docs/tasks/IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT.md) |
