...
- Work unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.687`
- Date: `2026-06-28`
- Status: selected
- Scope: public contract selection only for the bounded APB sideband-aware
  data16 no-policy six-register generalized register-set cardinality widening

## Decision

`IAL2-FEATURE-COMPLETENESS-FRONTIER.687` selects
`IAL2-FEATURE-COMPLETENESS-FRONTIER.688` to directly implement exactly these
public sources:
...
This selection read the `.686` selector, `.685` 32-bit no-policy
six-register behavior, `.684` 32-bit no-policy six-register contract, `.683`
broader-cardinality audit, `.675` data16 no-policy five-register behavior,
`.681` data16 protected five-register behavior, `.678` 32-bit protected
five-register behavior, current `ApbCompleter` and `ApbComposition` data16
generalized guards and residue, `RegressionCorpus`, `LanguageSurfaceSection`,
focused APB/profile-alias/support/capability tests, README, ROADMAP_V2,
mdBook, Memory, Knowledge Map, and relevant decisions.

The selected implementation stays on the data16 no-policy path because it is
the direct 16-bit/2-byte-stride counterpart of the `.685` six-register
32-bit no-policy widening and keeps register-local protection policy out of
scope.

## Selected Public Source Contract

The `.688` implementation shall keep the existing selected data16
...
The admitted data16 no-policy generalized family after `.688` shall be:
...
The public representative selected for `.688` shall use six registers per
peripheral:
...
Implementation may widen only the current shared data16 no-policy helper
bound for the selected sideband-aware no-policy generalized two-peripheral
family from `maximum_count = 5` to `maximum_count = 6`. It must keep
protected data16 six-register register sets and more than six registers
fail-closed until future task-tree owners select those boundaries.
...
- protected data16 six-register generalized register sets;
- protected 32-bit six-register generalized register sets;
...
`.688` must keep diagnostics fail-closed. The selected widening may accept
...
`.688` closeout validation must cover:
...
- generated artifact probes for `reg5` storage, read hits, write hits,
  byte-lane writes, and read-data drives;
- status/control windows at `0` and `258`;
- both peripheral completers carrying `reg0/reg1/reg2/reg3/reg4/reg5` at
  local addresses `0/2/4/6/8/10`;
...
- malformed storage diagnostics for wrong `reg5` address, wrong register
  ...
  register-local `access-policy`, protected data16 six-register,
  protected 32-bit six-register, more-than-six-register, and
  more-than-two-peripheral variants;
...
This selection does not implement behavior for:
