...
- allow bounded scalar, aggregate, and resolved composition-top/package-symbol per-instance parameter/generic overrides through declared `.rtlif` `(params (NAME default_value) ...)` metadata plus semantic `?rtl` `(params (NAME value) ...)` override blocks that are name-validated and aggregate-shape-validated before target lowering,
