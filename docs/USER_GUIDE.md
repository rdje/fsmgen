...
- When a deconstruct RHS is a whole aggregate constant such as `FRAME`, exact nested subaggregate fragments keep their own type-shape contracts. For example, splitting `FRAME` into `tag` and `payload` fragments checks a typed `payload_t` target against the `FRAME.payload` contract rather than against the entire `FRAME` record.
