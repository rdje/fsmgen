...
- When a deconstruct RHS is itself a `(concat ...)` / `(cat ...)`, any deconstruct fragment that exactly matches whole RHS concat operand boundaries keeps that operand instead of becoming a part-select of the whole concat expression. If the aligned RHS operand is a typed aggregate signal or typed aggregate leaf/root, its aggregate type-shape contract is preserved for the pre-generation assignment validator.
