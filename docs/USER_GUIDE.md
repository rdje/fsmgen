...
- Composition tops themselves now also surface top-level forward IR summaries through direct-result `intent_hir` and `lowered_rtl_ir`, and `module_info` mirrors those same serialized layers with bounded top-port, child-count, lane, internal-net, instance, and auxiliary-assignment summaries.
