...
- Explicit `?toplink` source actuals `=open`, scalar `=0` / `=1`, and exact-width binary/decimal/hex literals such as `=8'b10100101`, `=8'd165`, or `=8'hA5`, with `=open` still targeting realized child inputs only while direct scalar `=0` / `=1` widen to the realized child-input or declared top-output target width and exact-width literal actuals may now also target declared top outputs
...
- scalar `=0` and `=1` direct actuals may now target either a realized child input port or a declared top output by widening to that direct binding target width as numeric zero/one,
