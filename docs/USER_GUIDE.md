...
- Direct-root note:
  ...
  - bounded aggregate type aliases now cover packed `(list ...)` and `(record ...)` forms; on the current direct generated-module backend, those named aggregate types still lower to one packed vector width instead of frontend-shaped typedefs.
