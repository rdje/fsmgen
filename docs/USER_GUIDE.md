...
- Bounded top-root symbol sections `(+constants ...)` and `(+enums ...)`
  ...
  - composition-top aggregate references must resolve all the way to a scalar leaf such as `BYTES[1]`, `FRAME.flag`, or `NEST.header.nibble`
...
- Local composition-top symbol note:
  ...
  - local composition-top references must resolve all the way to a scalar leaf such as `BYTES[1]`, `FRAME.flag`, or `NEST.header.nibble`.
  - unresolved whole-aggregate references such as `FRAME` are currently outside the bounded literal-actual path.
