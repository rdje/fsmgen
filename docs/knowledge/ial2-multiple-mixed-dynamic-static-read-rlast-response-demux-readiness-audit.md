...
At the time of `.301`, the public boundary was fail-closed. A temporary probe
combining the `.299` multi-static read shape with `.280`
`response-scope burst-last` syntax failed under strict check JSON with:
...
`.302` selected the public/report contract before behavior changes,
...
The direct generated behavior later shipped in `.303`.
