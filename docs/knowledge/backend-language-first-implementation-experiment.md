...
`.fsm` check smoke for `feature.direct_sreset_active_high`. The active next
slice is `.3.3`, the first Perl-oracle parity smoke for that Rust check result.
