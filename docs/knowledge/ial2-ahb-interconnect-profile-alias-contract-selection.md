...
The selected future alias path is `ppif/ahb_interconnect.ahb`, mirroring the
...
`.726` owns implementation. Multi-subordinate decode, multiple managers, bus
matrices, optional signals, burst `SEQ`, byte-lane/narrow-transfer behavior,
direct backend, verification-output generation, AXI/APB behavior, and VHDL
remain future task-tree-owned work.
