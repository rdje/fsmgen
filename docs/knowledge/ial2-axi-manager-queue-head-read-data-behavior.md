...
Read burst-last last-beat queue-head read-data is shipped separately by
`IAL2-FEATURE-COMPLETENESS-FRONTIER.115` and reports
`generated_queue_head_response_demux_last_beat_completion_pulse`. Multi-beat
queue-head read-data, queue-head `burst-length` metadata, deeper or multiple
groups, mixed same-family auto-ID plus concrete queue-head demux, direct
