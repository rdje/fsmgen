...
answers:
  ...
  - "where does (=> A B) / next / within / after / sampled-value / past get parsed and rendered to SVA?"
...
- **Parse** — `FSM::Adapter::FSMGenFull::Parser::parse_check_property`: recognizes the
  ...
  expression builder in property-value mode, where value-returning `(past SIG [N])`
  is accepted only for check expressions. **Add a new combinator/leaf here, or add a
  property-only value function to the expression builder and call it from this path.**
...
- **Keep-alive** — `FSM::Adapter::FSMGenFull::SignalAnalyzer::_analyze_check_condition_references`
  walks the same fields plus CoreAST function-call arguments, so a signal used only in
  a check, including through `$past`, stays a port.
