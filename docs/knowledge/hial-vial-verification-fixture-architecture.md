...
fixtures are supported without inventing transaction truth. `.10.3` is the
active backend/trace owner after clean `.10.2` commit `045629c97`; activation
changes no product behavior, and `.11` retains runtime parity.
