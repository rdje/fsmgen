...
title: APB .apb profile-alias readiness is the next owner after APB PPIF requester-transfer
answers:
...
`.apb` is not accepted yet. `.552` must audit whether APB is ready for a
public `.apb` alias contract or whether another APB `.ppif` behavior,
manifest/support-accounting/report prerequisite, cleanup prerequisite, or
deferral should come first. `.552` must not change parser/generator behavior
or accept any new suffix.
