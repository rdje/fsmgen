...
Follow-on `.705` is blocked because no approved/provided AHB/AHB-Lite source
artifact exists under `docs/vendor/` or `.cache/local-references/`. The seed
contract remains blocked until the user provides or explicitly approves an
official source artifact suitable for `docs/vendor/arm/amba/ahb/`.
