...
answers:
  ...
  - "are structural_rtl_ir instance parameter overrides or port bindings entry keys advertised?"
date: 2026-06-05
status: current
...
Nested instance `parameter_overrides[]` and `port_bindings[]` entries are still
not separately advertised; later exact task-tree owners must widen those nested
