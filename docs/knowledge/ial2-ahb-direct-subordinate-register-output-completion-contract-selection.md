...
title: Direct AHB completion repair selected Q-named register-output assignment
answers:
...
`.7` selected the existing four-state direct dispatcher with every persistent
...
or report is added. A warning-clean disposable candidate passed active NONSEQ
...
`UNOPTFLAT` and avoidable latency. `.8` now ships the selected implementation;
current behavior routes to fact
