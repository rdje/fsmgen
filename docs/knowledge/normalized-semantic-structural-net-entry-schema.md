...
internal declaration plan plus top-level state and standalone-DT enable wires
projected from the already-prepared direct backend enable registries. Those
direct entries preserve width, signedness, state-model, and declared-type
metadata where available, but they do not claim DT-specific/LHS WEN/EN wires,
direct instances, links, auxiliary assignments, or assignment connectivity.
