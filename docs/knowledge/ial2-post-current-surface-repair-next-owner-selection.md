...
answers:
  ...
  - "did the pre-repair AHB subordinate accept consecutive active address phases at .807?"
...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER\.807|ahb_access_active_q|IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT' docs/IAL2_POST_CURRENT_SURFACE_REPAIR_NEXT_OWNER_SELECTION.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/tasks/IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.md
---

After `.806` repaired then-current AHB alias truth, `.807` selected the existing
`IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT`. At that pre-repair point,
subordinate ownership admitted only when `ahb_access_active_q` was clear and
released only on unselected/IDLE/BUSY boundaries. The paired requester then
supplied such a boundary, so t/1513/t1515 did not prove direct active-to-active
replacement.
...
Current resolution: `.3` replaced this generated-family behavior with the
depth-one phase pipeline in fact `ial2-ahb-pipelined-active-transfer-repair`.
This card is historical selection evidence and no longer answers a
present-tense current-behavior question.
