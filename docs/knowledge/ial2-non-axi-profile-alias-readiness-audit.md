...
are not ready for implementation or public contract selection. The CLI rejects
`.chi`, `.ace`, `.ahb`, `.apb`, `.atb`, `.smbus`, and `.i2s` before PPIF
parsing; PPIF profile-alias validation is `.axi`-specific; and there is no
non-AXI profile-alias support-accounted fixture.
