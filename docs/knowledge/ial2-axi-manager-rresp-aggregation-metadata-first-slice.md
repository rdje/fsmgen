...
`.77` did not generate scalar aggregate outputs or update rules. Existing
generated multi-beat output-bank `.isf`, `.fsm`, and SystemVerilog behavior
remained unchanged and continued to expose per-beat `RRESP` lanes, valid
masks, and length outputs.
...
audit is complete, and `.79` now ships generated scalar aggregate outputs,
request-time `2'd0` initialization, matched-beat update rules, generated
aggregate report fields, and removal of `generated_rresp_aggregation` from
read-data residue.
