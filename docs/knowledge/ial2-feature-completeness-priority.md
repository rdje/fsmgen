...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER|IAL2 feature completeness|issue-order-queue|selected_not_generated|generated_same_id_queue_head_demux|generated AXI same-ID read burst-last queue state|IAL1/IAL0/SV prerequisites|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_SAME_ID_QUEUE_BEHAVIOR_FIRST_SLICE_SELECTION.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---

The current feature-completeness priority is IAL2 on the
SystemVerilog-backed path.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.106` owns the next PNT frontier:
implementing generated AXI same-ID read burst-last queue state and queue-head
demux behavior for the selected two-transaction depth-2 sample shape. Write
queue-head behavior, read `single-beat`, deeper/multiple groups, read-data
consumption, direct backend lowering, and VHDL remain deferred.
