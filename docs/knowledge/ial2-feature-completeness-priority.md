...
`.170` selected `.171` because existing write depth-2 one-group and
multi-group queue-head response-demux samples are generated/support-accounted,
read depth-3 siblings prove the shared queue-state machinery, and a temporary
write depth-3 candidate passes strict check with no diagnostics while
remaining selected-not-generated only with
