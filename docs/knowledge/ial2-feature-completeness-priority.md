...
The current PNT frontier is `.130`. Report-only raw-`ARLEN` and runtime
beat-count/`RLAST` multi-group variants, deeper queues, same-family mixed
auto-ID, write or read single-beat multi-group queue-head behavior, packed
burst-vector outputs, alternate payload assembly, direct backend lowering,
