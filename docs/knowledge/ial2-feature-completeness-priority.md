...
group of two transactions at computed depth 2. The current PNT frontier is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.111`, the post-read-single-beat same-ID
queue behavior expansion audit/selector. Read-data consumption,
deeper/multiple groups, same-family mixed auto-ID, direct backend lowering,
