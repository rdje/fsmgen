...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER|IAL2 feature completeness|issue-order-queue|selected_not_generated|generated_same_id_queue_head_demux|same-ID queue state and queue-head behavior readiness|IAL1/IAL0/SV prerequisites|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_SAME_ID_QUEUE_HEAD_RESPONSE_DEMUX_METADATA_FIRST_SLICE.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---

The current feature-completeness priority is IAL2 on the
SystemVerilog-backed path.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.104` owns the next PNT frontier:
auditing generated AXI same-ID queue state and queue-head demux behavior
readiness, while accepted concrete same-ID reuse, generated queue behavior,
queue-head response-demux rules, direct backend lowering, and VHDL remain
deferred.
...
false; it advanced to `.104`, generated queue state and queue-head behavior
readiness.
