...
The active IAL2 frontier is `IAL2-FEATURE-COMPLETENESS-FRONTIER.150`, the
next AXI manager feature-completeness selector. Read-data over depth-3 queues,
write or burst-last depth-3 response-demux, multiple or mixed depth-3 groups,
same-family mixed auto-ID, group-local enqueue widening, packed burst-vector
outputs, alternate payload assembly, direct backend lowering, and VHDL remain
deferred.
