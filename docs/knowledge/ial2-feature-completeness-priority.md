...
admitted-request onehot boundary is preserved. The active frontier is
`.141` selected `IAL2-FEATURE-COMPLETENESS-FRONTIER.142`, readiness audit for
generated read single-beat multi-group queue-head response-demux, because a
temporary two-group single-beat read probe reports selected metadata with two
duplicate concrete read-ID groups but remains generated-false with
`generated_same_id_queue_head_demux` residue. The active frontier is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.142`. Deeper queues, same-family mixed
auto-ID, read single-beat multi-group behavior implementation, packed
burst-vector outputs, alternate payload assembly, direct backend lowering, and
