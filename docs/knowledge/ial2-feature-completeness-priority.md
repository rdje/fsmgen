...
answers:
  ...
  - "what did IAL2-FEATURE-COMPLETENESS-FRONTIER.185 select?"
  - "what did IAL2-FEATURE-COMPLETENESS-FRONTIER.186 ship?"
  ...
  - "what is the next IAL2 frontier after multiple/mixed depth-3 runtime residue cleanup?"
  - "what did IAL2-FEATURE-COMPLETENESS-FRONTIER.189 select?"
  - "what is the next IAL2 frontier after multiple/mixed depth-3 runtime multi-beat selection?"
  - "what did IAL2-FEATURE-COMPLETENESS-FRONTIER.190 select?"
  - "what is the next IAL2 frontier after multiple/mixed depth-3 multi-beat readiness?"
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.233?"
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.234?"
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.235?"
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.522?"
  ...
  - "what follows broader mixed write BID queue readiness?"
...
semantic/MCP proof, and current 321/362/45 accounting. Completed `.4` selects
proposed `.5`, the matching byte-identical `.ahb` alias implementation,
projected at 322/363/46 split 23/23 with focused t1529 parity and t1528 retained
as the sole shared runtime. No alias or behavior changes in `.4`.
...
request-set fit guards and concrete-ID group assertion scope, while the
remaining local same-ID ordering residue is `per_id_issue_order_queues`.
...
generated over static concrete ID values and finite transaction inventory,
while PPIF transactions currently accept only `auto` or concrete `(value N)`
IDs. Dynamic user-ID arbitration remains unsupported until a public contract
is selected.
...
request point; metadata-first parser/report readiness is next before any
dynamic capture, response matching, same-ID policy, queue, scoreboard, sample,
support-accounting, validation, test, generated-artifact, or HDL behavior.
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.219` shipped that metadata boundary and
advanced the frontier to `.220`. The public sample is
...
generated dynamic transaction-ID capture and response matching. `.221` must
audit capture timing, outstanding-state lifetime, response-match/completion
semantics, first bounded read/write shape, generated artifact boundaries,
diagnostics, validation, rollback, and explicit residue before any behavior
changes.
...
matching. `.222` must define admitted-request capture timing, single-active
...
validation, rollback, and explicit residue before any behavior changes.
...
The selected contract reuses existing `response-demux.write` with one
transaction-local dynamic write ID; no new dynamic-ID lifecycle clause or
metadata-only generated-completion acceptance is selected. `.223` must
capture the write request-ID source at the admitted request point, enforce
single-active selected-ID/busy ownership, match `BID` against the captured ID,
generate the transaction completion pulse, release busy, report the dynamic
write demux artifacts, and preserve explicit residue.
...
`RID` response matching. The audit kept dynamic read burst-last/`RLAST`,
read-data routing, burst-length/runtime validation, interleaving, multiple
dynamic reads, mixed dynamic/static read demux, same-cycle recapture, same-ID
ordering, queues, scoreboards, direct backend behavior, HDL shapes outside
the selected SystemVerilog path, and VHDL deferred.
...
single-beat dynamic read behavior, added the support-accounted
`ppif/axi_manager_capacity_status_dynamic_read_response_demux.ppif` sample,
and advanced the frontier to `.228`, the next exact-owner selector after
bounded single-active dynamic read/write demux. `.228` selected `.229`,
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.232` selected `.233`, readiness audit for
dynamic read-data routing over the generated single-active dynamic read
response-demux family. The audit must decide whether single-beat dynamic
read-data, last-beat dynamic read-data, both, or a smaller prerequisite should
be selected next, while preserving fail-closed burst-length/runtime,
multi-beat, multiple/mixed dynamic demux, same-cycle recapture, same-ID
ordering, queues, scoreboards, direct backend, and VHDL boundaries.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.233` selected `.234`, direct bounded
implementation of scalar dynamic read-data capture over generated
single-active dynamic read response-demux. `.234` reuses existing
`read-data.read` syntax, covers exactly one dynamic read transaction, supports
scalar single-beat and scalar last-beat capture from generated dynamic
completion pulses, and keeps dynamic burst-length/runtime, multi-beat,
multiple/mixed dynamic demux, same-cycle recapture, same-ID ordering, queues,
scoreboards, direct backend, and VHDL boundaries fail-closed.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.234` shipped that behavior with
support-accounted single-beat and last-beat PPIF samples, generated
`RDATA`/`RRESP` capture from dynamic response-demux completion pulses, and
completion-validity vocabulary for dynamic single-beat and dynamic last-beat
read-data. `IAL2-FEATURE-COMPLETENESS-FRONTIER.235` selected `.236`, AXI
manager focused-suite cost cleanup, before further dynamic behavior expansion.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.236` added
`t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t` as a bounded
routine validation target for that dynamic family and selected `.237`, dynamic
burst-length readiness audit. `IAL2-FEATURE-COMPLETENESS-FRONTIER.237`
selected `.238`, direct bounded implementation of report-only dynamic
...
local coverage diagnostic for concrete queue-head transactions. Adjacent mixed
response-demux-only, auto-ID read-data, and queue-head read-data samples
strict-check cleanly.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.195` selected `.196` after `.194`
shipped response-demux-only mixed behavior. Live mixed reports expose combined
auto-ID and concrete queue-head completion coverage, while read-data coverage
still has separate queue-head and auto-ID transaction paths that need an audit
...
read single-beat, read burst-last, and write response-demux-only fixtures.
The generated behavior combines auto-ID and queue-head completion outputs,
response-demux states, reports, request-ID ownership, and assertions without
new syntax.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.193` selected `.194` after auditing
temporary read single-beat, read burst-last, and write probes that combined
same-family auto-ID lifecycle metadata with concrete same-ID queue-head
demux. All probes failed closed at the same local planner diagnostic,
adjacent auto-ID and queue-head samples strict-checked cleanly, and no parser
or lower-layer prerequisite was needed before the bounded implementation.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.192` selected `.193` after `.191`
shipped generated multiple/mixed depth-3 runtime-validation multi-beat
output-bank behavior. The selected audit covers the explicit current
fail-closed same-family mixed boundary between generated auto-ID response-demux
and concrete queue-head response-demux, before any parser, generator, PPIF
sample, support-accounting catalog, validation, generated-artifact, test, or
HDL behavior change.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.191` shipped generated multi-beat
output-bank behavior over the two `.186` multiple/mixed depth-3 read
burst-last queue-head runtime-validation shapes. The new public PPIF samples
cover queue-depth sets `3,3` and `3,2`, report per-beat output banks,
runtime-assertion `ARLEN` validation, empty `read_data` residue, and empty
`response_demux` residue, and are support-accounted.
...
with mixed depth-3/depth-2 groups. The implementation widened only the local
no-`burst_length` last-beat read-data coverage gate and preserved
burst-length, runtime-validation, multi-beat payload, write-family read-data,
mixed auto-ID, direct backend, VHDL, and backend-language boundaries.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.181` selected `.182`, readiness audit for
report-only raw-`ARLEN` burst-length over that multiple/mixed depth-3 scalar
last-beat shape, before runtime validation, multi-beat payload,
write-family read-data, mixed auto-ID, direct backend, VHDL, or
backend-language variants.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.182` selected `.183`, direct bounded
implementation of report-only raw-`ARLEN` burst-length over the same
multiple/mixed depth-3 scalar last-beat shape. Temporary in-memory candidates
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.184` selected `.185`, readiness audit for
generated runtime beat-count/`RLAST` validation over the same
multiple/mixed depth-3 scalar last-beat shape.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.185` selected `.186`, direct bounded
implementation of generated runtime validation. `.186` shipped two
support-accounted runtime-assertion samples covering depth `3,3` and mixed
depth `3,2` queue sets. They generate request-captured raw `ARLEN`,
expected-beat storage, read-beat counters, request/response update rules, and
beat-count/`RLAST` assertions, remove `generated_beat_count_validation`
residue, and preserve multi-beat output, write-family read-data, mixed
auto-ID, direct backend, VHDL, and backend-language boundaries.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.187` selected `.188`, report/static
support-residue cleanup. The live `.186` behavior is generated, but the AXI
ID/order unsupported-residue detail still classifies multiple/mixed depth-3
runtime validation with multi-beat payload as outside the shell. The
runtime-validation half is stale; multi-beat payload remains deferred.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.188` cleaned that support/residue wording.
Selected multiple/mixed depth-3 runtime-validation scalar last-beat shapes are
now supported in the static detail, while read burst-last multi-beat payload
over those groups remains explicit residue.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.189` selected `.190`, readiness audit for
generated multi-beat output-bank behavior over those multiple/mixed depth-3
runtime-validation groups. Live probes showed the `.186` samples keep only
multi-beat payload/output/aggregation residue, while the one-depth-3 and
depth-2 multi-group multi-beat precedents are residue-clean. The target group
sets still fail closed at the local multi-beat admission gate.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.190` selected `.191`, direct bounded
...
queue-head shapes. No smaller prerequisite is needed: the runtime-validation,
one-depth-3 multi-beat, and depth-2 multi-group multi-beat helpers are
transaction-list driven once the local multi-beat coverage gate admits the
shape.
...
support-accounted.
`.174` shipped that generated multiple/mixed depth-3 response-demux-only
behavior for read single-beat, read burst-last, and write families. The six
public samples cover two-depth-3 and mixed depth-3/depth-2 queue sets for
each family, are support-accounted through `intent.ppif_*` entries, and
preserve read-data, burst-length, runtime-validation, multi-beat, mixed
auto-ID, group-local enqueue widening, direct backend, VHDL, and
backend-language variant boundaries.
`.175` selected `IAL2-FEATURE-COMPLETENESS-FRONTIER.176`, readiness audit for
generated read-data over multiple or mixed depth-3 concrete same-ID
queue-head groups. Temporary read single-beat and read burst-last read-data
probes over two depth-3 queue groups fail closed at the local read-data
coverage gate; the selector made no behavior changes.
`.176` selected `IAL2-FEATURE-COMPLETENESS-FRONTIER.177`, direct bounded
implementation of read single-beat scalar `RDATA`/`RRESP` over generated
multiple/mixed depth-3 queue-head groups. Burst-last read-data, burst-length,
runtime-validation, multi-beat payload, mixed auto-ID, direct backend, VHDL,
and backend-language variants remain deferred.
`.177` shipped generated read single-beat scalar `RDATA`/`RRESP` over
multiple/mixed depth-3 queue-head groups and advanced the active frontier to
`.178`, the next IAL2 feature-completeness selector. `.178` selected `.179`,
readiness audit for generated read burst-last scalar last-beat read-data over
multiple/mixed depth-3 queue-head groups. Burst-length, runtime-validation,
