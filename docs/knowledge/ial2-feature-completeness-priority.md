...
next feature-completeness selector. Report-only raw-`ARLEN` and runtime
beat-count/`RLAST` multi-group scalar variants, deeper queues, same-family
mixed auto-ID, write or read single-beat multi-group queue-head behavior,
