...
`response_demux` residue for the covered sample. The active frontier is now
`IAL2-FEATURE-COMPLETENESS-FRONTIER.170`, readiness audit for generated
write-family depth-3 concrete same-ID queue-head response-demux. `.169`
selected `.170` because existing write depth-2 one-group and multi-group
queue-head response-demux samples are generated, while a temporary write
depth-3 candidate passes strict check with no diagnostics and remains
