...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER|IAL2 feature completeness|issue-order-queue|generated_read_burst_last_queue_head_demux|next same-ID queue behavior expansion|IAL1/IAL0/SV prerequisites|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_SAME_ID_QUEUE_BEHAVIOR_FIRST_SLICE.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---

The current feature-completeness priority is IAL2 on the
SystemVerilog-backed path.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.107` owns the next PNT frontier: auditing
and selecting the next AXI same-ID queue behavior expansion after `.106`
shipped generated read burst-last depth-2 queue state and queue-head demux for
the public sample. Write queue-head behavior, read `single-beat`,
deeper/multiple groups, read-data consumption, direct backend lowering, and
