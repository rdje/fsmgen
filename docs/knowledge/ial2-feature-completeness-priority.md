...
multiple generated read burst-last concrete same-ID queue-head groups. The
current PNT frontier is `.129`. Report-only/runtime-only multi-group
variants, deeper queues, same-family mixed auto-ID, write or read single-beat
multi-group queue-head behavior, packed burst-vector outputs, alternate
