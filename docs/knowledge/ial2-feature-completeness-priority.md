...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER\\.150|IAL2-FEATURE-COMPLETENESS-FRONTIER\\.151|POST_READ_SINGLE_BEAT_DEPTH3_QUEUE_HEAD_RESPONSE_DEMUX_NEXT_SLICE_SELECTION|support-detail expectation|depth-2 queue-head|read single-beat depth-3 queue-head|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_POST_READ_SINGLE_BEAT_DEPTH3_QUEUE_HEAD_RESPONSE_DEMUX_NEXT_SLICE_SELECTION.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---
...
expectation alignment, because focused PPIF validation still expects older
support-detail wording while the production report now explicitly names
independent `depth-2` queue-head groups. The active IAL2 frontier is `.151`.
Read-data over depth-3 queues, write or burst-last depth-3 response-demux,
multiple or mixed depth-3 groups, same-family mixed auto-ID, group-local
enqueue widening, packed burst-vector outputs, alternate payload assembly,
