...
response-demux. That audit is read-family only, burst-last scope, one-bit
`RLAST`, exactly one duplicate concrete `RID` group of three read
transactions, and generated queue-head response-demux only. Write depth-3,
