...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.64` owns the next PNT frontier: select the
next exact owner after report-only AXI burst read-data ARLEN beat-count/depth
metadata.
...
frontier to `.64`, the next exact-owner selector.
