...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER|IAL2 feature completeness|issue-order-queue|generated_write_bid_queue_head_demux|generated_read_burst_last_queue_head_demux|IAL1/IAL0/SV prerequisites|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_POST_SAME_ID_QUEUE_BEHAVIOR_NEXT_SLICE_SELECTION.md docs/AXI_IAL2_MANAGER_SAME_ID_QUEUE_BEHAVIOR_FIRST_SLICE.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---

The current feature-completeness priority is IAL2 on the
SystemVerilog-backed path.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.108` owns the next PNT frontier:
generated AXI same-ID write queue-head behavior for one duplicate concrete
write-ID group of two transactions at computed depth 2. Selector `.107`
selected that boundary after `.106` shipped generated read burst-last depth-2
queue state and queue-head demux for the public sample. Read `single-beat`,
