...
live reports remain preserved. The active frontier is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.139`, readiness audit for generated
write-family multi-group queue-head response-demux. `.138` selected `.139`
because the one-group write queue-head sample is generated, read burst-last
multi-group queue-head behavior is generated, and a temporary two-group write
probe reports two duplicate concrete write-ID groups while still remaining
metadata-only with `generated_same_id_queue_head_demux` residue. Deeper
queues, same-family mixed auto-ID, read single-beat multi-group queue-head
