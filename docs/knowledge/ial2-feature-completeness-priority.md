...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.109` selected the current PNT frontier:
`IAL2-FEATURE-COMPLETENESS-FRONTIER.110`, generated AXI read `single-beat`
same-ID queue-head behavior for one duplicate concrete read-ID group of two
transactions at computed depth 2. Read-data consumption, deeper/multiple
groups, same-family mixed auto-ID, direct backend lowering, and VHDL remain
deferred.
