...
reverify: rg -n 'IAL2-FEATURE-COMPLETENESS-FRONTIER|IAL2 feature completeness|issue-order-queue|queue-head response-demux metadata|selected_not_generated|IAL1/IAL0/SV prerequisites|VHDL backend/reroute' docs/TASK_TREE.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/AXI_IAL2_MANAGER_SAME_ID_QUEUE_HEAD_RESPONSE_DEMUX_CONTRACT_SELECTION.md README.md ROADMAP_V2.md docs/book/src/14-feature-backlog.md
---

The current feature-completeness priority is IAL2 on the
SystemVerilog-backed path.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.103` owns the next PNT frontier:
shipping parser/report metadata and static validation for the AXI same-ID
queue-head response-demux contract, while duplicated concrete same-ID reuse,
generated queue behavior, queue-head response demux, direct backend lowering,
and VHDL remain fail-closed/deferred.
