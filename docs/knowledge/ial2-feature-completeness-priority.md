...
queue-head groups deeper than two slots. The active IAL2 frontier is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.148`. Same-family mixed auto-ID,
group-local enqueue widening, packed burst-vector outputs, alternate payload
assembly, direct backend lowering, and VHDL remain deferred.
