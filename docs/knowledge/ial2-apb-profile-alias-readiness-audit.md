...
FSMGen still does not accept `.apb`. The next owner must first select the
public alias contract for explicit profile policy, authored `.apb` source-path
identity, support-accounting identity/source kind, manifest wording,
diagnostics, and the mandatory generated `.isf` review step before generated
`.fsm`.
