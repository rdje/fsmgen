...
title: IAL2 feature completeness next slice is post-burst-length owner selection
answers:
...
burst-length metadata first slice, the next active leaf is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.64`.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.64` owns the selector after report-only
ARLEN burst-length metadata. It must choose the next exact owner before
generated ARLEN capture, beat-count validation, storage, reassembly,
per-beat outputs, `RRESP` aggregation, per-ID queues, or backend behavior.
