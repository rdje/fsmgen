...
title: IAL2 feature completeness next slice is post multiple dynamic read RLAST selection
answers:
...
`.256` must choose the next exact owner from the remaining dynamic read-side
residue, starting with read-data, burst-length/runtime validation, and
multi-beat output-bank widening over multiple dynamic read demux.
