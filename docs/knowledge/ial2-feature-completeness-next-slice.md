...
title: IAL2 feature completeness next slice is mixed dynamic/static read single-beat behavior
answers:
...
`.274`. `.275` selected `.276`, direct generated behavior through future public
sample
`ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux.ppif`.
No behavior changed in `.275`.
