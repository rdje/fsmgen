...
title: IAL2 feature completeness next slice is AXI burst read-data contract selection
answers:
...
selector, the next active leaf is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.57`.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.57` owns public AXI burst read-data
contract selection after generated `RLAST` completion and report alignment.
It must choose the additive source/report boundary for burst read-data
metadata or record a smaller prerequisite first before parser/report metadata
or generated behavior changes.
