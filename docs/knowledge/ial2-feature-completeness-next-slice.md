...
title: IAL2 feature completeness next slice is multi-beat read-data output-bank behavior
answers:
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.67?"
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.73?"
...
selector, burst read-data contract selector, last-beat read-data metadata
...
burst-length metadata first slice, post-burst-length metadata selector,
ARLEN capture readiness audit, generated raw-ARLEN capture behavior first
slice, beat-count/RLAST validation readiness audit, public
runtime-validation contract selector, and generated runtime-validation first
slice, post beat-count/RLAST validation selector, multi-beat read-data
contract selector, multi-beat parser/report metadata first slice, and
multi-beat output readiness audit, the next active leaf is
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.65`, a readiness audit before generated
ARLEN burst-length capture. The audit must confirm request-event binding,
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.66` shipped generated raw-ARLEN capture:
width-8 `axi0_arlen` input, per-transaction raw-ARLEN storage, request-event
guarded capture rules, `.fsm`/SystemVerilog lowering, generated
burst-length artifact report fields, and residue without
`generated_burst_length_capture`. It selected
...
payload reassembly, per-beat outputs, `RRESP` aggregation, direct backend
lowering, or VHDL work.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.67` found no lower-layer substrate
prerequisite for generated validation after a public validation mode exists:
generated storage, expression assignments, response-demux match expressions,
and clocked runtime assertions are available. It did not select direct
behavior because `validation report-only` must remain no-runtime-check
behavior. `IAL2-FEATURE-COMPLETENESS-FRONTIER.68` selected
`(validation runtime-assertion)` / `runtime_assertion`, preserved
`validation report-only` / `report_only`, and required parser support plus
generated runtime assertions to ship together.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.69` shipped that implementation: parser
support, generated expected-count storage, matched-beat counters, runtime
validation rules/assertions, support-accounted fixture, and report fields.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.70` selected public AXI multi-beat
read-data reassembly/output contract selection. `IAL2-FEATURE-COMPLETENESS-FRONTIER.71`
selected `capture-scope multi-beat` with mandatory ARLEN runtime assertions,
per-beat status, `multi-beat-by-rid` interleaving, per-transaction
data/status output prefixes, valid-mask outputs, and length outputs. The next
leaf, `IAL2-FEATURE-COMPLETENESS-FRONTIER.72`, shipped parser/report
metadata and static validation for that syntax, including the support-accounted
`ppif/axi_manager_capacity_status_read_data_multi_beat.ppif` sample. Generated
payload reassembly and output lanes remain deferred. The follow-up
`IAL2-FEATURE-COMPLETENESS-FRONTIER.73` audit found no lower-layer
prerequisite for first generated output-bank behavior when implemented with
scalar lane outputs, public output registers as beat storage, lane-specific
guards, constant prefix valid masks, and the existing matched-read-beat plus
`!request_event` boundary. The active leaf is
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.56` selected `.57`, public AXI burst
read-data contract selection, because direct behavior would require unselected
capture scope, output binding, beat-count/depth policy, `RRESP` aggregation,
interleaving diagnostics, and report residue movement.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.57` selected `.58`, parser/report
metadata and static validation for explicit last-beat read-data capture. The
selected contract pairs `(capture-scope last-beat)` with generated
`response_scope burst_last`, uses the response-demux last-beat completion
pulse as validity, requires `(status-policy last-beat)`, and leaves full
...
read-data behavior remain unchanged. It handed off to `.59`, the readiness
audit.

`IAL2-FEATURE-COMPLETENESS-FRONTIER.59` audited generated last-beat read-data
capture readiness and found no new IAL1/IAL0/SystemVerilog prerequisite. The
existing read-data input/output/capture-rule/report helpers can use the
generated burst-last response-demux completion pulses already bound by `.58`.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.60` shipped generated last-beat
`RDATA`/`RRESP` capture behavior: inputs, per-transaction last-beat
data/status outputs, guarded capture rules, generated artifact reports, HDL
reachability, and read-data residue without
`generated_last_beat_read_data_capture`. `IAL2-FEATURE-COMPLETENESS-FRONTIER.61`
selected `.62`, public AXI burst read-data beat-count/depth contract
selection, as the active frontier.
