...
title: IAL2 feature completeness next slice is read-data reassembly contract selection
answers:
...
slice, and post beat-count/RLAST validation selector, the next active leaf is
...
read-data reassembly/output contract selection. The next active leaf is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.71`, which must select the public
contract for beat storage, output shape, length/valid behavior, all-beat
`RRESP` aggregation, and interleaving/per-ID queue assumptions before parser,
