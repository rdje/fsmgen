...
title: IAL2 feature completeness next slice is post mixed dynamic/static write demux selection
answers:
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.252?"
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.256?"
  ...
  - "what is IAL2-FEATURE-COMPLETENESS-FRONTIER.269?"
...
response-demux.
`IAL2-FEATURE-COMPLETENESS-FRONTIER.266` selected `.267` because dynamic
multi-beat admission and report-residue recognition are still single-active
while the lower output-bank helpers are already transaction-list shaped after
coverage admission. `.267` selected `.268`, and `.268` now ships generated
bounded multiple dynamic multi-beat output-bank behavior through
`ppif/axi_manager_capacity_status_dynamic_read_data_multi_transaction_multi_beat.ppif`.
`.269` selected `.270` because the all-dynamic multiple dynamic path now
covers write response-demux, read single-beat response-demux, read
burst-last/`RLAST` response-demux, scalar read-data, report-only raw-`ARLEN`
capture, runtime beat-count/`RLAST` validation, and multi-beat output banks.
`.270` selected `.271` after confirming that current dynamic response-demux
intentionally fails closed unless every selected family transaction is
dynamic. The remaining local response-ownership gap is mixed dynamic/static
response-demux, where one raw response could match both static concrete-ID
state and active dynamic captured-ID state unless the public
ownership/assertion contract is selected first. The first safe family is
write `BID`, because it avoids read `RLAST`, burst-length/runtime, read-data,
and multi-beat output-bank coupling. `.271` selected `.272`, direct generated
behavior for exactly one dynamic write transaction plus exactly one concrete
static write transaction using existing `response-demux.write` syntax. `.272`
now ships that behavior through
...
raw `BID` cannot legally match both owners. `.273` must select the next exact
owner among the remaining mixed/read/multiple/static, same-cycle widening,
release-and-recapture, queues, scoreboards, direct backend, backend-language,
VHDL, or cleanup candidates before further behavior changes.
...
sample `ppif/axi_manager_capacity_status_dynamic_read_response_demux_multi.ppif`.
The generated contract uses per-transaction selected-ID/busy state, admitted
`ARID` capture, matched `RID` completion pulses, request onehot0, active
dynamic-ID uniqueness, active-match, unique-match, and completion-active
assertions. `.252` selected `.253`, readiness audit for multiple dynamic read
burst-last/`RLAST` response-demux. `.253` selected `.254`, contract selection,
because multiple-read burst-last behavior must pin all-dynamic family shape,
one-bit `last-signal` ownership, raw matched-`RID` beat assertions,
selected-ID/busy lifetime across non-last beats, final `RID && RLAST`
completion semantics, report vocabulary, sample/support expectations,
validation, and read-data/runtime/multi-beat residue before implementation.
`.254` selected `.255`, direct implementation. The selected contract uses
all-dynamic multi-active read capture, a one-bit `last-signal`, raw `RID` beat
active/unique assertions without `RLAST`, final `RID && RLAST`
completion/release, and `bounded_multi_dynamic_read_rid_rlast_demux_contract`
report vocabulary while keeping read-data, burst-length/runtime validation,
and multi-beat output banks over multiple dynamic read demux deferred.
`.255` ships that generated response-demux behavior with the support-accounted
sample
`ppif/axi_manager_capacity_status_dynamic_read_response_demux_multi_burst_last.ppif`.
`.256` selected `.257`, read-data readiness, because the dynamic read-data
coverage helper still required exactly one dynamic read transaction at that
point. `.257` selected `.258`, public contract selection. `.258` selected
`.259`, direct implementation, and `.259` now ships that behavior. The
shipped samples are
...
with read-data bindings required to exactly cover all generated dynamic read
demux transactions. `.260` selected `.261`, readiness audit for generated
burst-length and runtime beat-count/`RLAST` validation over generated multiple
dynamic read response-demux, because multi-beat output-bank widening depends
on per-transaction raw-`ARLEN` capture and runtime counter/assertion semantics
across multiple active dynamic reads. `.261` selected `.262`, public contract
selection, because the lower helpers are close after coverage admission but
sample names, report-only/runtime split, report vocabulary, diagnostics,
...
multiple dynamic read response-demux, and reserved `.264` at that point for
the runtime beat-count/`RLAST` assertion sibling. `.263` ships the report-only sample
`ppif/axi_manager_capacity_status_dynamic_read_data_multi_burst_length.ppif`.
`.264` ships the runtime sample
`ppif/axi_manager_capacity_status_dynamic_read_data_multi_burst_length_runtime_assertion.ppif`.
`.265` selected `.266`, readiness audit for generated multiple dynamic
multi-beat output-bank behavior before any further multiple dynamic read-data
behavior is widened. `.266` selected `.267`, public contract selection for
that bounded multiple dynamic multi-beat output-bank shape. `.267` selected
`.268`, and `.268` ships the selected all-dynamic multi-transaction
multi-beat output-bank contract.
