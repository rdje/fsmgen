...
title: IAL2 feature completeness next slice is multiple dynamic read-data readiness
answers:
...
must audit how scalar single-beat and last-beat read-data can consume generated
completion pulses from the `.251` and `.255` multiple dynamic read demux
shapes before burst-length/runtime validation or multi-beat output-bank
widening over multiple dynamic read demux.
