...
exposes arrays such as `generated_ial1.items[]`, `generated_ial0.items[]`, and
report-level `channels[]`; the current single-object `.ppif` result shape
remains stable until an explicit
...
arbitrary "first channel wins" HDL or semantic selection.
`IAL2-PPIF-BUNDLE-HDL-ENTRY-FIRST-SLICE.1` ships default HDL generation and
`--verify-hdl` through the selected aggregate wrapper/top entry for the tracked
AW/W bundle sample.
