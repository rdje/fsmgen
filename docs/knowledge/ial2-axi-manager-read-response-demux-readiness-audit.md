...
`.39` is also complete: it shipped read response-demux parser/report metadata
with `response_demux.read.generated_behavior: false` and advanced the frontier
to `.40`, a readiness audit before generated read `RID` demux behavior.
