...
The first additive generic source will be
...
A width-two `ahb_busy_remaining_q` is initialized to two before BUSY becomes
visible. Each `HGRANT && HREADY && HTRANS==BUSY` event decrements it; the final
event clears it and hands the same transfer to existing address-pending `SEQ`
ownership. Ready/grant stalls do not consume count. `.5` owns implementation;
aliases, paired exact-two sources, generalized counts, policy/runtime behavior,
and decision 0020 remain deferred.
