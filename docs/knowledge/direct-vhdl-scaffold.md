...
scaffold also lowers the first arithmetic RHS shape: binary scalar addition
lowers to one-bit truncated `xor` semantics, while same-width vector
`NAME + NAME` assignments and same-width multi-operand
