...
Both `.467` shapes keep the response-demux queue-owned and generate scalar
`RDATA`/`RRESP` capture. The `.469` sibling adds report-only raw-`ARLEN`
burst-length capture for the scalar last-beat queue path. Runtime validation,
multi-beat output banks, broader queues, mixed dynamic/static queues,
