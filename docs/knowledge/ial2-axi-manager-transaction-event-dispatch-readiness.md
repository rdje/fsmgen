...
reverify: rg -n 'Readiness Conclusion|Selected Implementation Boundary|IAL2-FEATURE-COMPLETENESS-FRONTIER\\.15|OR fan-in|transaction_event_dispatch|combined enable' docs/AXI_IAL2_MANAGER_TRANSACTION_EVENT_DISPATCH_READINESS_AUDIT.md docs/tasks/IAL2-FEATURE-COMPLETENESS-FRONTIER.md docs/TASK_TREE.md docs/book/src/14-feature-backlog.md README.md ROADMAP_V2.md
---
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.15` is the next active leaf. It should
implement unique per-transaction request/completion event inputs, scalar
one-event compatibility, OR fan-in guards for multi-event direction groups,
and additive `transaction_event_dispatch` report metadata.
