...
evidence: docs/TASK_TREE.md; docs/tasks/TASK-TREE-STALE-STATUS-DRIFT-REPAIR.md
reverify: perl -ne 'BEGIN { %skip = map { $_ => 1 } qw(docs/tasks/TEMPLATE.md docs/tasks/TASK-TREE-STALE-STATUS-DRIFT-REPAIR.md) } next if $skip{$ARGV}; if (/^- Status: .(active|proposed|in_progress|pending)./ || /^  Status: .(active|proposed|in_progress|pending)./) { print "$ARGV:$.:$_"; $bad = 1 } END { exit($bad ? 1 : 0) }' docs/tasks/*.md
---
...
`active`/`in_progress`/`proposed`/PNT-eligible `pending` statuses outside the
current repair leaf while the repair is in flight.
