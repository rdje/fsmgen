...
answers:
  ...
  - "what same-ID queue behavior will FSMGen implement next?"
date: 2026-06-14
status: current
...
implements generated read burst-last queue state plus queue-head response demux
for the existing public same-ID queue-head sample shape: one selected read
family, one duplicate concrete read-ID group, exactly two read transactions,
computed queue depth `2`, one-bit `RLAST`, no same-family auto-ID lifecycle,
and no read-data consumption.

The implementation must generate compact one-hot queue state and queue-head
completion demux together. Covered transaction completions become generated
pulse outputs only for that shape. Write queue-head behavior, read
`single-beat`, deeper or multiple groups, mixed same-family auto-ID, read-data
consumption, direct backend, and VHDL remain deferred.
