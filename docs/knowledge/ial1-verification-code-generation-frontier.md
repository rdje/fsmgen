...
support. The next eligible verification frontier leaf is `.6`, the direct
IAL2 route audit.
...
The next eligible verification-code-generation leaf is `.6`. Direct
IAL2-to-verification generation remains an audit question. It may later be
selected as a direct route, as an IAL2-to-IAL1 verification annotation handoff,
or as unnecessary for the first implementation. No implementation may assume
that answer before the audit completes.
