# AXI IAL2 Manager Three-Static Mixed Dynamic/Static Read-Data Contract Selection

Task-tree owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.329`

Date: 2026-06-23

## Decision

Select `IAL2-FEATURE-COMPLETENESS-FRONTIER.330`, direct generated behavior
...
The selected contract composes the shipped `.322` three-static mixed
dynamic/static single-beat `RID` response-demux and `.326` three-static
mixed dynamic/static burst-last `RID && RLAST` response-demux with the
existing scalar `read-data.read` surface. The implementation should admit the
three-static scalar no-`burst_length` boundary only, then reuse the existing
normalized scalar read-data capture rules. It must not add a second raw
`RID` or `RID && RLAST` matcher in the read-data path.
...
The `.330` implementation should add two support-accounted public PPIF
samples:
...
The single-beat sample should compose the `.322` public response-demux shape
...
The last-beat sample should compose the `.326` public burst-last
...
`r1`, `r2`, and `r3` remain concrete static read transactions with static IDs
`3`, `5`, and `7`.

## Public Boundary

`.330` should implement only scalar read-data over already generated
one-dynamic plus three-static mixed dynamic/static read response-demux:

- exactly one read transaction uses `(id dynamic)`;
- exactly three read transactions use pairwise-distinct concrete static IDs;
...
- `capture-scope single-beat` is accepted only with `response-scope
  single-beat`, no `last-signal`, no `status-policy`, and no `burst_length`;
- `capture-scope last-beat` is accepted only with `response-scope
  ...
  `burst_length`;
- `read-data.read.transactions` must bind the generated three-static mixed
  dynamic/static read demux transaction set exactly once; and
- three-static `capture-scope multi-beat`, `burst_length`, runtime
  beat-count/`RLAST` validation, and multi-beat output banks remain
  fail-closed.

The existing one-dynamic plus two-static scalar/burst-length/runtime/multi-beat
read-data behavior must remain unchanged. `.330` must not widen the
two-static burst-length/runtime/multi-beat owners to three static
...
already emitted by `.322` and `.326`:

```text
dynamic_transactions: [r0]
static_transactions: [r1, r2, r3]
generated_completion_signals: [axi0_r0_complete, axi0_r1_complete, axi0_r2_complete, axi0_r3_complete]
```
...
diagnosed by the existing normalization layer once the multiple mixed
coverage branch admits the correct covered set.
...
axi0_r3_complete -> capture axi0_rdata/axi0_rresp into r3 outputs
```
...
read_data:
  ...
  read:
    ...
    transactions:
      ...
      - transaction: r3
        completion_signal: axi0_r3_complete
        data_output: axi0_r3_rdata
        status_output: axi0_r3_rresp
```

The single-beat read-data residue remains:
...
The last-beat read-data residue remains:
...
multi-beat output-bank coverage.
...
The focused dynamic-test behavior labels should be:
...
The `.330` implementation should widen only the scalar no-`burst_length`
portion of generated multiple mixed dynamic/static read-data coverage:

- accept exactly one dynamic plus two concrete static reads for all existing
  `.307`, `.310`, `.312`, and `.314` boundaries;
- accept exactly one dynamic plus three concrete static reads only for
  scalar single-beat and scalar last-beat read-data with no `burst_length`
  metadata;
- reject generated three-static read-data coverage when generated completion
  signal count does not match the dynamic-plus-static transaction count;
- reject missing `read-data.read` bindings for any generated three-static
  mixed read demux transaction;
- reject extra `read-data.read` transaction bindings outside the generated
  three-static mixed read demux transaction set;
- preserve duplicate binding diagnostics; and
- keep three-static `burst_length`, runtime beat-count/`RLAST` validation,
  and multi-beat output banks fail-closed.

Existing `.284` one-static mixed read-data samples, `.307` two-static
read-data samples, `.310/.312/.314` two-static burst/runtime/multi-beat
samples, `.322` three-static single-beat response-demux sample, and `.326`
three-static burst-last response-demux sample must preserve their current
schedule/check/semantic reports.

## Validation Gates

The `.330` implementation should cover:
...
env FSMGEN_DYNAMIC_CASE_FILTER=mixed_dynamic_static_read_data_multi_static3 FSMGEN_DYNAMIC_SKIP_CLI_JSON=1 prove -Iperl t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t
env FSMGEN_DYNAMIC_CASE_FILTER=mixed_dynamic_static_read_data_multi_static3_last_beat FSMGEN_DYNAMIC_SKIP_CLI_JSON=1 prove -Iperl t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t
env FSMGEN_DYNAMIC_CASE_FILTER=mixed_dynamic_static_read_data_multi_static FSMGEN_DYNAMIC_SKIP_CLI_JSON=1 prove -Iperl t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t
env FSMGEN_DYNAMIC_CASE_FILTER=mixed_dynamic_static_read_data_multi_static_last_beat FSMGEN_DYNAMIC_SKIP_CLI_JSON=1 prove -Iperl t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t
prove -Iperl t/248-regression-corpus-accounting.t
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --emit-schedule-json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --strict --check --json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --strict --emit-semantic-json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --verify-hdl ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --emit-schedule-json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_burst_last_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --strict --check --json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_burst_last_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --strict --emit-semantic-json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_burst_last_read_data.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- env -u PERL5LIB ./bin/fsmgen --quiet --verify-hdl ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static3_burst_last_read_data.ppif
```

If guarded direct probes trip host-memory cutoffs before assertion output,
the implementation should record that explicitly and use guarded lightweight
...
Rollback is the `.329` selector commit. Reverting it restores `.329` as the
active contract-selection frontier and removes the `.330` implementation
owner.
...
- generated scalar read-data implementation over three-static mixed read
  demux until `.330` ships it;
