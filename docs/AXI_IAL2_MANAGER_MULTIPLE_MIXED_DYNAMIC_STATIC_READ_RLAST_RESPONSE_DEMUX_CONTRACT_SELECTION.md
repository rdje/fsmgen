# AXI IAL2 Manager Multiple Mixed Dynamic/Static Read RLAST Response-Demux Contract Selection

Task-tree owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.302`

Date: 2026-06-23

## Decision

Select `IAL2-FEATURE-COMPLETENESS-FRONTIER.303`, direct generated behavior
for bounded multiple mixed dynamic/static read burst-last `RID && RLAST`
response-demux.

The selected public contract composes the shipped `.299` one-dynamic plus
two-static single-beat `RID` ownership/report contract with the shipped `.280`
one-dynamic plus one-static burst-last final-completion contract.
...
The `.303` implementation should add one support-accounted public PPIF sample:
...
`.303` should implement only this first multiple mixed read burst-last
contract:
...
a write-family behavior clause explicitly consumes them. The `.303` public
...
three selected read transactions. A later owner may widen same-cycle
...
releases only from that transaction's generated final-beat completion pulse. A
request in the same cycle as that transaction's completion remains invalid in
`.303`, matching existing dynamic and mixed no-release-and-recapture
behavior.
...
not complete or release any selected transaction. This contract does not
validate beat counts or enforce AXI burst length; it uses `RLAST` only to
choose the generated completion/release beat.

## Report Vocabulary

`.303` should report a new multiple mixed read burst-last mode:
...
The existing `.276` one-dynamic plus one-static single-beat read mode,
`.280` one-dynamic plus one-static burst-last read mode, and `.299`
one-dynamic plus two-static single-beat read mode must remain unchanged.
...
must stay explicit until later owners consume the new multiple mixed
burst-last completion or raw matched-beat boundary.
...
`.303` should keep fail-closed behavior outside the selected contract and
sharpen diagnostics for:

- missing dynamic read transaction in the selected multiple mixed burst-last
  family;
- more than one dynamic read transaction in this first multiple mixed
  burst-last family;
- fewer or more than two concrete static read transactions in this first
  multiple mixed burst-last family;
...
- read-data over multiple mixed read burst-last response-demux before that
  contract is selected;
- burst-length/runtime validation over multiple mixed read burst-last
  response-demux before that contract is selected;
- multi-beat output banks over multiple mixed read burst-last response-demux
  before that contract is selected;
...
The `.303` implementation should run focused gates:
...
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- ./bin/fsmgen --strict --check --json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static_burst_last.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- ./bin/fsmgen --strict --emit-semantic-json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static_burst_last.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- ./bin/fsmgen --strict --verify-hdl --output /tmp/fsmgen_multi_mixed_read_rlast_demux.sv ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_multi_static_burst_last.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- ./bin/fsmgen --strict --check --json ppif/axi_manager_capacity_status_read_mixed_dynamic_static_response_demux_burst_last.ppif
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- prove -l t/248-regression-corpus-accounting.t
scripts/run_with_ram_guard.sh --host-max-pct 88 --process-max-rss-mb 4096 -- prove -l t/1438-axi-ial2-manager-dynamic-transaction-id-focused.t
...
For `.302`, documentation-only validation is Knowledge Map generation/check,
...
unchanged and the fail-closed boundary was already probed in `.301`.

## Explicit Residue

The following remain future owners:

- direct implementation of the selected `.303` multiple mixed dynamic/static
  read burst-last `RID && RLAST` response-demux contract;
...
Rollback is the `.302` contract-selection commit. Reverting it restores `.302`
as the active contract selector after `.301`; reverting the future `.303`
implementation should remove only generated multiple mixed read burst-last
behavior, its public sample, support-accounting entry, tests, docs, and fact
card.
