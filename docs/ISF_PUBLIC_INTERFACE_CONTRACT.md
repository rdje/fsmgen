...
states plus independent shift states that neither read nor overwrite a pending
sample alias. Top-level zero-count successors that cannot yet carry pending
samples fail closed.
...
independent scalar setter and shift successors. Setters or shifts that read or
overwrite a pending sample alias remain fail-closed.
