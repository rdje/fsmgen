...
The next selected sink-event-wait ordering hardening keeps the same public
surface unchanged: the sink child event wait must occur after the sink child
trigger, and sink-wait-before-trigger clauses stay fail-closed until a later
contract explicitly adds sticky event, replay, or pre-trigger acknowledgement
metadata.
