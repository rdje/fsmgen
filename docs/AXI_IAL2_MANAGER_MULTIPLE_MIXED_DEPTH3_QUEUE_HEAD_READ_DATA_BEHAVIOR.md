# AXI IAL2 Manager Multiple/Mixed Depth-3 Queue-Head Read-Data Behavior

Task-tree owner:
`IAL2-FEATURE-COMPLETENESS-FRONTIER.177`.

Date: `2026-06-18`.

## Purpose

This slice implements the bounded read-data behavior selected by `.176`:
generated scalar single-beat `RDATA`/`RRESP` capture over generated read
single-beat concrete same-ID queue-head response-demux groups where every
...
- scalar per-transaction data/status outputs such as `axi0_r5_rdata` and
  `axi0_r5_rresp`;
...
- no `axi0_rlast`, `axi0_arlen`, beat-count state, per-beat output bank, packed
  payload vector, or aggregate-only status output.

Representative generated single-beat capture rule for the last transaction in
the two-depth-3 sample:

```lisp
(rule axi0_r5_read_data_capture axi0_r5_complete
  (axi0_r5_rdata axi0_rdata)
  (axi0_r5_rresp axi0_rresp))
```

The matching queue-head response-demux guard remains single-beat and `RLAST`
free:
...
completion signals and one response-demux rule per transaction.
...
- generated read single-beat queue-head response-demux completion pulses;
- scalar `RDATA`/`RRESP` outputs for every covered transaction;
...
This slice widens only the local single-beat read-data coverage admission
predicate. Existing read-data normalization, generated artifact enumeration,
report projection, and HDL lowering remain transaction-list driven.
...
- read single-beat depth-2 one-group queue-head read-data;
- read single-beat depth-2 multi-group queue-head read-data;
- read single-beat one-depth-3 queue-head read-data;
...
- read burst-last read-data over multiple/mixed depth-3 queue-head groups;
- burst-length, runtime-validation, or multi-beat payload behavior over
  multiple/mixed depth-3 queue-head groups;
...
```bash
./bin/fsmgen --emit-schedule-json ppif/axi_manager_capacity_status_read_single_beat_multi_depth3_same_id_queue_head_read_data.ppif
./bin/fsmgen --strict --check --json ppif/axi_manager_capacity_status_read_single_beat_multi_depth3_same_id_queue_head_read_data.ppif
./bin/fsmgen --strict --emit-semantic-json ppif/axi_manager_capacity_status_read_single_beat_multi_depth3_same_id_queue_head_read_data.ppif
./bin/fsmgen --quiet --verify-hdl --output /tmp/fsmgen_axi_read_single_beat_multi_depth3_same_id_queue_head_read_data.sv ppif/axi_manager_capacity_status_read_single_beat_multi_depth3_same_id_queue_head_read_data.ppif
```
