# IAL2 APB Protection Back-To-Back Contract Selection

- Work unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.627`
- Date: `2026-06-28`
- Status: selected
- Scope: public contract selection only for bounded APB sideband-aware
  protection back-to-back timing-policy behavior

## Decision

`IAL2-FEATURE-COMPLETENESS-FRONTIER.627` selects
`IAL2-FEATURE-COMPLETENESS-FRONTIER.628` to directly implement the bounded APB
sideband-aware protection back-to-back timing-policy contract for exactly four
public sources:

- `ppif/apb_completer_multi_register_sideband_protection_back_to_back.ppif`
- `ppif/apb_completer_multi_register_sideband_protection_back_to_back.apb`
- `ppif/apb_composition_multi_register_sideband_protection_status_back_to_back.ppif`
- `ppif/apb_composition_multi_register_sideband_protection_status_back_to_back.apb`

The first protected timing owner deliberately excludes data16-protection and
multi-peripheral multi-register timing. It combines the already shipped
32-bit sideband requester queue with the already shipped 32-bit register-local
`PPROT[0]` access-policy behavior, then adds adjacent setup only for the exact
selected protected two-register standalone completer and compatible fixed
one-requester/one-completer composition.

This selector changes no parser behavior, generator behavior, sample files,
support-accounting catalog entries, validation behavior, generated artifacts,
schedule/check/semantic JSON behavior, HDL/runtime behavior, suffix
acceptance, direct backend lowering, verification-output generation,
backend-language variants, APB behavior, AXI behavior, AHB behavior, or VHDL
behavior.

## Evidence Read

The selector read:

- `.626` post-data16-back-to-back next-slice selector;
- `.625` APB sideband-aware data16 back-to-back behavior;
- `.622/.621` APB sideband-aware multi-register back-to-back behavior and
  contract;
- `.612` APB sideband requester queued `PPROT/PSTRB` behavior;
- `.603/.597` APB data16 and 32-bit `PPROT` access-policy behavior;
- `.594/.589` APB data16 and sideband/strobe behavior;
- current protected standalone, fixed-composition, and multi-peripheral APB
  sources and reports;
- ApbRequesterTransfer, ApbCompleter, ApbComposition, RegressionCorpus,
  LanguageSurfaceSection, focused APB/profile-alias/support tests, README,
  ROADMAP_V2, mdBook, task tree, Memory, Knowledge Map, and relevant
  decisions.

Live `.626` probes confirmed:

- 32-bit protected standalone, fixed, and multi-peripheral samples expose
  `protection_policy`, have no `back_to_back_policy`, and keep broad
  `apb_back_to_back_policy_deferred`;
- data16-protection samples expose `protection_policy`, have no
  `back_to_back_policy`, and keep broad `apb_back_to_back_policy_deferred`;
- a temporary protected adjacent-setup standalone completer candidate fails
  at the current no-policy timing guard.

## Selected Completer Contract

The selected standalone completer is the protected 32-bit sideband-aware
two-register extension of the `.622` adjacent setup family:

- one `apb-completer apb_completer`;
- 32-bit `PADDR`, `PWDATA`, `PRDATA`, and register data;
- bus `PPROT width 3`;
- bus `PSTRB width 4`;
- `wait_cycles width 4`;
- exactly two source-ordered storage registers:
  - `reg0` at address `0`, data width `32`, reset `0`,
    `(read allow)`, `(write require (privileged 1))`;
  - `reg1` at address `4`, data width `32`, reset `0`,
    `(read require (privileged 1))`,
    `(write require (privileged 1))`;
- `(setup-detect (select 1) (enable 0))`;
- `(timing-policy (setup-admission adjacent))`.

`.628` may widen adjacent setup only to this selected protected two-register
shape. It must keep the no-policy one-register, no-policy multi-register,
data16 no-policy, and earlier sideband timing behavior unchanged.

## Protected Adjacent Behavior

The selected timing policy must not weaken protection semantics:

- `PPROT` and `PSTRB` are sampled on every admitted setup phase, including an
  adjacent setup that follows the prior access response;
- allowed reads and writes keep the `.597` protected behavior;
- allowed mapped writes update only selected `PSTRB` byte lanes;
- `PSTRB=0` remains a successful no-byte write when the mapped write is
  allowed;
- denied reads complete with `PREADY=1`, `PSLVERR=1`, and `PRDATA=0`;
- denied writes complete with `PREADY=1`, `PSLVERR=1`, and no storage update,
  including when `PSTRB=0`;
- unmapped accesses keep unmapped-address error behavior and do not evaluate
  register-local policy;
- adjacent setup admission must not create more than one active APB transfer.

## Selected Fixed Composition Contract

The selected fixed composition combines:

- the `.612` 32-bit sideband requester status timing policy:
  `(timing-policy (back-to-back queued) (queue-depth 1) (overflow reject))`;
- requester response `accepted`, `busy`, and `status width 2`;
- request `req_prot width 3` and `req_wstrb width 4`;
- the selected protected two-register adjacent setup completer above;
- one requester child and one completer child;
- 32-bit sideband-aware fixed wiring with `PPROT width 3` and `PSTRB width 4`;
- no top-level `apb-composition` timing-policy clause.

The generated fixed composition must only propagate `PPROT/PSTRB/PWDATA` and
mux the selected protected completer response. Protection enforcement remains
owned by the completer. The top must expose requester `accepted`, `busy`,
`status`, `done`, `last_error`, and `last_read_data` outputs.

## Report And Support Movement

Selected standalone and fixed-composition reports must add the existing
endpoint `timing_policy` shapes and fixed aggregate `back_to_back_policy`
metadata while preserving `protection_policy` metadata.

Selected reports must remove broad `apb_back_to_back_policy_deferred` only for
the four selected protected timing surfaces. They must retain:

- `apb_additional_back_to_back_policies_deferred` for unselected APB timing
  families;
- `apb_additional_protection_policies_deferred` for unsupported `PPROT`
  predicates, global/window/interconnect-owned policy, runtime policy, and
  broader protection-policy semantics.

Selected support-accounting identities:

- `intent.ppif_apb_completer_multi_register_sideband_protection_back_to_back`
- `intent.apb_profile_alias_completer_multi_register_sideband_protection_back_to_back`
- `intent.ppif_apb_composition_multi_register_sideband_protection_status_back_to_back`
- `intent.apb_profile_alias_composition_multi_register_sideband_protection_status_back_to_back`

Selected coverage buckets:

- `ial2_ppif_apb_completer_multi_register_sideband_protection_back_to_back_pipeline_cli`
- `ial2_apb_profile_alias_completer_multi_register_sideband_protection_back_to_back_pipeline_cli`
- `ial2_ppif_apb_composition_multi_register_sideband_protection_status_back_to_back_pipeline_cli`
- `ial2_apb_profile_alias_composition_multi_register_sideband_protection_status_back_to_back_pipeline_cli`

## Diagnostics

`.628` must reject unsupported protected timing shapes, including:

- protected timing without complete `PPROT/PSTRB` sidebands;
- data16-protection timing;
- protected storage with more or fewer than the selected two registers;
- protected storage with register addresses other than `0` and `4`;
- protected storage with data width other than `32`;
- access-policy clauses outside the exact selected register-local policy
  shape;
- fixed compositions with missing requester `accepted/busy/status` timing
  response;
- multi-peripheral protected timing propagation;
- queue depths other than `1`;
- overflow policies other than `reject`;
- accepted-less requesters;
- multiple active APB transfers.

## Validation Target For `.628`

The implementation owner should cover:

- syntax checks for touched APB modules/tests;
- direct schedule JSON, strict check JSON, semantic JSON, generated review
  artifact, and HDL-shape probes for all four selected public sources;
- standalone completer evidence for adjacent setup plus protected read/write
  allow/deny behavior;
- fixed-composition evidence for queued `PPROT/PSTRB` propagation into the
  protected completer and top-level `accepted` exposure;
- malformed protected timing diagnostics for wrong address, wrong policy, and
  data16-protection attempts;
- focused APB profile-alias, completer, composition, support-accounting, and
  capability-manifest tests;
- README, ROADMAP_V2, mdBook, task tree, Memory, Knowledge Map, docs path,
  diff, and doctrine gates.

## Deferred Work

This selection does not include data16-protection timing, multi-peripheral
multi-register timing propagation, combined data16-protection timing, queue
depths greater than `1`, overflow policies other than `reject`, accepted-less
requesters, multiple active APB transfers, additional `PPROT` predicates,
global/window/interconnect-owned protection policies, direct backend lowering,
verification-output generation, backend-language variants, AXI, AHB, or VHDL
behavior.

## Rollback

Rollback of the future `.628` implementation removes only the four selected
public samples, selected protected timing-policy guard widening,
support-accounting entries, focused tests, behavior docs, Knowledge Map fact
card, README, ROADMAP_V2, mdBook, task tree, Memory, and generated Knowledge
Map updates. Existing 32-bit no-sideband timing, sideband requester timing,
sideband no-policy multi-register timing, data16 timing, APB protection
behavior without timing, AXI, AHB, and VHDL behavior remain owned by earlier or
future slices.
