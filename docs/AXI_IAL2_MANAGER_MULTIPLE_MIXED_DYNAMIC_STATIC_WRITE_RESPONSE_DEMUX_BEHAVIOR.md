...
Two-dynamic plus one-static mixed writes, broader mixed cardinalities beyond
the selected two-static recapture sibling, read multiple mixed demux,
read-data, burst-length/runtime validation, multi-beat output banks,
same-cycle widening beyond generated release-and-recapture, dynamic same-ID
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.400` extends this same public sample with
...
`axi0_w2_static_busy_release_recapture`. Dynamic recapture requires an
admitted `w0` request, generated `w0` completion, active dynamic busy state,
no admitted `w1`/`w2` request, and an `axi0_awid` value different from both
static IDs. Each static recapture requires its own admitted static request,
own generated completion, own active busy state, no admitted dynamic request,
and no sibling static request.

The report adds dynamic recapture fields under
`response_demux.write.dynamic_capture.transactions[0]` and list-shaped
`response_demux.write.static_capture[]` entries for `w1` and `w2`.
