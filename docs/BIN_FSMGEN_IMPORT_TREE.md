...
- Static trace from [bin/fsmgen](bin/fsmgen) currently reaches `254` project files total, `253` `.pm` packages.
- The R14 `.isf` front door is reachable from [bin/fsmgen](bin/fsmgen)
  ...
  pipeline.
- The IAL2 `.ppif` front door is reachable from [bin/fsmgen](bin/fsmgen)
  through a conditional runtime require of
  [perl/FSM/Adapter/IAL2/PPIF.pm](perl/FSM/Adapter/IAL2/PPIF.pm), which uses
  ...
  for shipped AXI manager capacity/status source objects, the bounded AXI
  initiator primitive/composition owners under
  [perl/FSM/IAL2/ProtocolIntent/](perl/FSM/IAL2/ProtocolIntent/), and the AHB
  requester, subordinate, and interconnect owners. The same PPIF front door
  reaches
  ...
  register-set composition, `.apb` alias, and busy-capable source objects.
  Every shipped IAL2 path then reuses the IAL1 `.isf` lowerer before handing
  generated `.fsm` artifacts to the ordinary HDL pipeline.
- Project-owned output, temporary, and cache destinations are now reachable
  through [perl/FSM/ProjectDataLocality.pm](perl/FSM/ProjectDataLocality.pm),
  which keeps CLI and lowerer-created data repository-derived and on the same
  filesystem volume.
- Composition parsers, protocol-intent composition owners, and both structural
  emitters now reach the shared portable child-instance-label contract in
  [perl/FSM/Support/HDLInstanceIdentifierPolicy.pm](perl/FSM/Support/HDLInstanceIdentifierPolicy.pm).
  Authored labels fail closed against the target-case-aware portable keyword
  union, while generated role labels use deterministic suffix allocation.
...
- The explicit `fsmgen vial` dispatch now reaches the private span-aware VIAL
  ...
  normalizer, plus the defensive tool/API and CLI adapter. Public `plan` now
  reaches the canonical direct-IAL0, direct-IAL1, and IAL2-via-generated/
  reparsed-IAL1 routes through private bridge/ExecutionIR builders and the
...
- The top-level manifest owner
  ...
  in [perl/FSM/Support/ProducerSection.pm](perl/FSM/Support/ProducerSection.pm),
...
- The machine-readable support/API surface is now reachable through
  ...
  bounded manifest-facing `producer` section contract in
  [perl/FSM/Support/ProducerContract.pm](perl/FSM/Support/ProducerContract.pm), the bounded
...
- The bounded in-process `HDLGenerator` result contract now also makes both halves of the
  ...
  shells/artifacts for in-process Perl embedders, `source_info`, `module_info`, and
  `statistics` advertise bounded stable subsurfaces rather than whole-hash promises,
  `source_info`, `module_info`, and `statistics` now each advertise their own nested-object
  contract owner, `composition_plan`, `composition_spec`, `fsm_module`, `raw_ast`, and
  `resolved_package_imports` now also each advertise their own shell-only contract owner, and
  the top-level `intent_hir` / `lowered_rtl_ir` / `structural_rtl_ir` hashes are explicitly
  treated as reused normalized-semantic shell contracts rather than separately stabilized full
  trees; structured downstream consumers should stay on those advertised semantic/summary paths
  or the sanitized
...
- The latest support/API surface also reaches the serializable plan/report
  ...
  pipeline objects.
- The latest first-class semantic-introspection support surface also reaches
  [perl/FSM/Support/SemanticIntrospectionSection.pm](perl/FSM/Support/SemanticIntrospectionSection.pm)
  and
  [perl/FSM/Support/SemanticIntrospectionContract.pm](perl/FSM/Support/SemanticIntrospectionContract.pm),
  so manifest consumers can discover query domains, query families, MCP
  resource/tool mappings, safety policy, and public surface ownership without
  parsing ad hoc prose.

## Executive read
[bin/fsmgen](bin/fsmgen) is a thin CLI/reporting shell plus the R14 `.isf`
and IAL2 `.ppif` pre-lowering front doors, repository-local artifact/temp
routing, and explicit verification-output dispatch.
...
transaction compositions plus the AHB requester, subordinate, and interconnect
owners alongside the existing Valid-Ready, AXI manager capacity/status, and APB
families. The semantic package/type family beneath
[perl/FSM/Package/](perl/FSM/Package) remains intentionally explicit: reusable
named values, package imports, scalar/list/record type aliases, aggregate paths,
and aggregate expression contracts have owners instead of parser-local string
rules.
...
as the scalar/aggregate literal, bounded scalar-expression, matching-shape
leafwise aggregate-expression, and unary aggregate complement owner for direct `+params`, `.rtlif` defaults, external
...
The downstream-tool Support family now owns schema-versioned capability,
diagnostic, support-accounting, semantic-report, embedding, documentation,
language-surface, verification-output, and external-validation contracts. The
complete reachable package list below is canonical; duplicating it here would
make this executive view another inventory.

Those contracts keep public JSON defensive and corpus-accounted, distinguish
matched, unmatched, and expected-failure evidence, and avoid leaking private
Perl/AST objects or HDL text. The raw in-process `HDLGenerator` result remains
explicitly non-JSON-safe. Optional post-emission validation is SystemVerilog-
only through Verilator lint and ABC-free Yosys structural synthesis; GHDL and
ABC-specific validation remain separately qualified lanes.
...
- [perl/FSM/HDL/FlattenedDT/Backend/VHDL.pm](perl/FSM/HDL/FlattenedDT/Backend/VHDL.pm)
...
- total reachable project files: `254`
- reachable `.pm` packages: `253`
- `Support`: `76`
- `Composition`: `36`
- `HDL`: `33`
- `Package`: `14`
...
- [perl/FSM/Scheduler/ISF/Emitter/JSON.pm](perl/FSM/Scheduler/ISF/Emitter/JSON.pm): `1105`
- [perl/FSM/Support/CapabilityManifest.pm](perl/FSM/Support/CapabilityManifest.pm): `38`
- [perl/FSM/Support/CapabilityManifestContract.pm](perl/FSM/Support/CapabilityManifestContract.pm): `347`
- [perl/FSM/Support/ProducerSection.pm](perl/FSM/Support/ProducerSection.pm): `62`
- [perl/FSM/Support/SupportAccountingSection.pm](perl/FSM/Support/SupportAccountingSection.pm): `98`
...
- [perl/FSM/Support/SemanticIntrospectionSection.pm](perl/FSM/Support/SemanticIntrospectionSection.pm): `68`
...
- [perl/FSM/Support/LanguageSurfaceSection.pm](perl/FSM/Support/LanguageSurfaceSection.pm): `309`
...
- [perl/FSM/Support/NormalizedSemanticReport.pm](perl/FSM/Support/NormalizedSemanticReport.pm): `390`
...
- [perl/FSM/Support/HDLInstanceIdentifierPolicy.pm](perl/FSM/Support/HDLInstanceIdentifierPolicy.pm): `173`
- [perl/FSM/VIAL/Tool.pm](perl/FSM/VIAL/Tool.pm): `1262`
- [perl/FSM/VIAL/ToolCLI.pm](perl/FSM/VIAL/ToolCLI.pm): `403`
- [perl/FSM/VIAL/PlanBuilder.pm](perl/FSM/VIAL/PlanBuilder.pm): `535`
- [perl/FSM/VIAL/ArtifactTransaction.pm](perl/FSM/VIAL/ArtifactTransaction.pm): `334`
- [perl/FSM/VIAL/Backend/SVPortableVerilator.pm](perl/FSM/VIAL/Backend/SVPortableVerilator.pm): `1893`
- [perl/FSM/VIAL/Backend/Runner.pm](perl/FSM/VIAL/Backend/Runner.pm): `644`
...
- the AXI manager capacity/status PPIF owner
  [perl/FSM/IAL2/ProtocolIntent/AxiManagerCapacityStatus.pm](perl/FSM/IAL2/ProtocolIntent/AxiManagerCapacityStatus.pm)
...
- [perl/FSM/Composition/LinkedPlanBuilder.pm](perl/FSM/Composition/LinkedPlanBuilder.pm)
  remains a major `R11` planning hotspot because source-expression resolution, aggregate shape
  checks, carrier allocation, and binding-type preservation still meet there, but open/numeric
...
- [perl/FSM/Pipeline/HDLGenerator.pm](perl/FSM/Pipeline/HDLGenerator.pm)
- [perl/FSM/ProjectDataLocality.pm](perl/FSM/ProjectDataLocality.pm)
...
- [perl/FSM/Support/NormalizedSemanticReport.pm](perl/FSM/Support/NormalizedSemanticReport.pm)
- [perl/FSM/VerificationOutput/UVM/PassiveMonitorSkeleton.pm](perl/FSM/VerificationOutput/UVM/PassiveMonitorSkeleton.pm)
- [perl/FSM/VerificationOutput/VHDL/ObservationPackageSkeleton.pm](perl/FSM/VerificationOutput/VHDL/ObservationPackageSkeleton.pm)
...
For `.ppif` inputs, [bin/fsmgen](bin/fsmgen) also conditionally requires:
- [perl/FSM/Adapter/IAL2/PPIF.pm](perl/FSM/Adapter/IAL2/PPIF.pm)

For `fsmgen vial`, [bin/fsmgen](bin/fsmgen) conditionally requires:
...
- debug/trace routing
- output-file writing
- repository-local output, trace, temporary-workspace, and cache routing
- VIAL subcommand dispatch and public result presentation
- generated verification-output target dispatch and artifact-manifest writing
...
Its only non-trivial local logic is presentation/reporting and input-routing glue.

## Runtime spine
Normal execution includes one distinct VIAL tool/runtime spine plus optional
`.ppif` / `.isf` pre-spines feeding two sibling `.fsm` spines under the same
CLI and top-level facade.
...
bin/fsmgen
  -> FSM::SourcePathResolver
     ...
     -> FSM::IAL2::ProtocolIntent::AxiAwDriver / AxiWDriver / AxiWBurst4Driver
     -> FSM::IAL2::ProtocolIntent::AxiArDriver / AxiRBeatAcceptor
     -> FSM::IAL2::ProtocolIntent::AxiBResponseAcceptor
     -> FSM::IAL2::ProtocolIntent::Axi*Composition owners
     -> FSM::IAL2::ProtocolIntent::ApbRequesterTransfer
     -> FSM::IAL2::ProtocolIntent::ApbCompleter
     -> FSM::IAL2::ProtocolIntent::ApbComposition
     -> FSM::IAL2::ProtocolIntent::AhbRequester
     -> FSM::IAL2::ProtocolIntent::AhbSubordinate
     -> FSM::IAL2::ProtocolIntent::AhbInterconnect
     -> FSM::Adapter::ISF
     -> FSM::Scheduler::ISF
  -> generated .isf review artifact(s)
  -> generated .fsm review artifact(s), including bundle wrapper/top entries
  -> Direct-root spine below, unless --emit-schedule-json exits earlier

VIAL tooling/runtime spine
bin/fsmgen
  -> FSM::VIAL::ToolCLI
  -> FSM::VIAL::Tool
     -> SourceProjection / Parser / SemanticBuilder
     -> PlanBuilder / HIAL VIALBridge / ExecutionBuilder
     -> Backend::SVPortableVerilator
     -> Backend::Runner
        -> Backend::TraceValidator
        -> Backend::ResultProducer
  -> FSM::VIAL::ArtifactTransaction
  -> virtual or atomic repository-local artifact graph

Verification-output spine
bin/fsmgen
  -> FSM::Adapter::ISF
  -> FSM::Scheduler::ISF
  -> FSM::VerificationOutput::UVM::PassiveMonitorSkeleton, or
     FSM::VerificationOutput::VHDL::ObservationPackageSkeleton
  -> repository-local artifact tree plus artifact manifest
...
### Public VIAL source, planning, and qualified runtime tooling

- [perl/FSM/VIAL/ToolCLI.pm](perl/FSM/VIAL/ToolCLI.pm)
- [perl/FSM/VIAL/Tool.pm](perl/FSM/VIAL/Tool.pm)
- [perl/FSM/VIAL/SourceProjection.pm](perl/FSM/VIAL/SourceProjection.pm)
- [perl/FSM/VIAL/PlanBuilder.pm](perl/FSM/VIAL/PlanBuilder.pm)
- [perl/FSM/VIAL/ArtifactTransaction.pm](perl/FSM/VIAL/ArtifactTransaction.pm)
...
- [perl/FSM/VIAL/ExecutionBuilder.pm](perl/FSM/VIAL/ExecutionBuilder.pm)
- [perl/FSM/VIAL/ExecutionIR.pm](perl/FSM/VIAL/ExecutionIR.pm)
- [perl/FSM/VIAL/ExecutionRandom.pm](perl/FSM/VIAL/ExecutionRandom.pm)
- [perl/FSM/VIAL/ExecutionReport.pm](perl/FSM/VIAL/ExecutionReport.pm)
- [perl/FSM/VIAL/Backend/SVPortableVerilator.pm](perl/FSM/VIAL/Backend/SVPortableVerilator.pm)
- [perl/FSM/VIAL/Backend/Runner.pm](perl/FSM/VIAL/Backend/Runner.pm)
- [perl/FSM/VIAL/Backend/TraceValidator.pm](perl/FSM/VIAL/Backend/TraceValidator.pm)
- [perl/FSM/VIAL/Backend/ResultProducer.pm](perl/FSM/VIAL/Backend/ResultProducer.pm)
- [perl/FSM/HIAL/VIALBridge/Builder.pm](perl/FSM/HIAL/VIALBridge/Builder.pm)
- [perl/FSM/HIAL/VIALBridge/Manifest.pm](perl/FSM/HIAL/VIALBridge/Manifest.pm)
- [perl/FSM/HIAL/VIALBridge/Report.pm](perl/FSM/HIAL/VIALBridge/Report.pm)

Both authored projections normalize before the single semantic builder.
Source actions remain no-write. `plan` routes HIAL through canonical review
artifacts and exposes only sanitized JSON-safe projections. `run` additionally
selects the exact qualified Verilator 5.046 profile, emits portable
...
- [perl/FSM/IAL2/ProtocolIntent/ValidReadyChannel.pm](perl/FSM/IAL2/ProtocolIntent/ValidReadyChannel.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiManagerCapacityStatus.pm](perl/FSM/IAL2/ProtocolIntent/AxiManagerCapacityStatus.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiAwDriver.pm](perl/FSM/IAL2/ProtocolIntent/AxiAwDriver.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiWDriver.pm](perl/FSM/IAL2/ProtocolIntent/AxiWDriver.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiWBurst4Driver.pm](perl/FSM/IAL2/ProtocolIntent/AxiWBurst4Driver.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiBResponseAcceptor.pm](perl/FSM/IAL2/ProtocolIntent/AxiBResponseAcceptor.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiWriteRequestComposition.pm](perl/FSM/IAL2/ProtocolIntent/AxiWriteRequestComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiWriteBurst4RequestComposition.pm](perl/FSM/IAL2/ProtocolIntent/AxiWriteBurst4RequestComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiWriteTransactionComposition.pm](perl/FSM/IAL2/ProtocolIntent/AxiWriteTransactionComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiArDriver.pm](perl/FSM/IAL2/ProtocolIntent/AxiArDriver.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiRBeatAcceptor.pm](perl/FSM/IAL2/ProtocolIntent/AxiRBeatAcceptor.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiReadTransactionComposition.pm](perl/FSM/IAL2/ProtocolIntent/AxiReadTransactionComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/AxiReadBurst4TransactionComposition.pm](perl/FSM/IAL2/ProtocolIntent/AxiReadBurst4TransactionComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/ApbRequesterTransfer.pm](perl/FSM/IAL2/ProtocolIntent/ApbRequesterTransfer.pm)
- [perl/FSM/IAL2/ProtocolIntent/ApbCompleter.pm](perl/FSM/IAL2/ProtocolIntent/ApbCompleter.pm)
- [perl/FSM/IAL2/ProtocolIntent/ApbComposition.pm](perl/FSM/IAL2/ProtocolIntent/ApbComposition.pm)
- [perl/FSM/IAL2/ProtocolIntent/AhbRequester.pm](perl/FSM/IAL2/ProtocolIntent/AhbRequester.pm)
- [perl/FSM/IAL2/ProtocolIntent/AhbSubordinate.pm](perl/FSM/IAL2/ProtocolIntent/AhbSubordinate.pm)
- [perl/FSM/IAL2/ProtocolIntent/AhbInterconnect.pm](perl/FSM/IAL2/ProtocolIntent/AhbInterconnect.pm)
- [perl/FSM/Adapter/ISF.pm](perl/FSM/Adapter/ISF.pm)
- [perl/FSM/Adapter/ISF/Parser.pm](perl/FSM/Adapter/ISF/Parser.pm)
...
This R14 / IAL2 pre-lowering layer is intentionally before the main
`HDLGenerator` facade: it turns protocol/platform `.ppif` sources into
reviewable `.isf` and `.fsm` artifacts, and turns intent-scheduled `.isf`
sources into explicit `.fsm` artifacts or a schedule JSON report. The generated
`.fsm` then uses the ordinary direct-root pipeline, so `.ppif` and `.isf` do not
create a second HDL backend path.
...
- [perl/FSM/Support/VIALNativeUVMEmissionContract.pm](perl/FSM/Support/VIALNativeUVMEmissionContract.pm)
- [perl/FSM/Support/VIALVHDLEmissionContract.pm](perl/FSM/Support/VIALVHDLEmissionContract.pm)
...
- [perl/FSM/Support/ISFResourceCatalog.pm](perl/FSM/Support/ISFResourceCatalog.pm)
...
These owners emit the bounded inert UVM passive-monitor and VHDL observation
package skeleton targets selected by the CLI. They consume ISF observation
metadata and write repository-local artifact trees plus manifests; they do not
claim UVM compilation, VHDL compilation, PSL, or simulation support.
...
- [perl/FSM/Composition/ISFEventCDCModuleEmitter.pm](perl/FSM/Composition/ISFEventCDCModuleEmitter.pm)
...
- [perl/FSM/Composition/LinkedPlanBuilder.pm](perl/FSM/Composition/LinkedPlanBuilder.pm) is no
...
- The remaining `EnableGraph`-family gravity is now narrower than before:
  ...
  support, and factorization policy all have dedicated owners now, and the direct first-pass
  AST-factorization pipeline plus the iterative fixpoint loop-state/pass helper/pass execution
  families do too. So the next honest seams are no longer “who owns the first factorization
  pass,” “who owns the fixpoint pass helpers,” “who owns the fixpoint aggregate
  loop-state contract,” “who owns the consolidated intermediate dependency graph
  mechanics,” “who owns the old direct consolidated block shell,” “who owns the old
  direct consolidated emitter shell,” “who owns the live consolidated intermediate
  stage-6/prescan handoff,” “who owns the whole post-flattening SystemVerilog assembly
  sequence,” or “who owns the pre-stage scaffold/declaration/enable/prescan sequence”;
  they are the remaining lower-level direct-backend planning/stage coordination and broader
  direct-backend convergence rather than the old direct planning, capture, assembly, or WEN/EN
  clusters.
