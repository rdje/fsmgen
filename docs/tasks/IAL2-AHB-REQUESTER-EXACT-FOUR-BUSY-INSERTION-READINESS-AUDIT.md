...
- ID: `IAL2-AHB-REQUESTER-EXACT-FOUR-BUSY-INSERTION-READINESS-AUDIT.3`
  ...
  Verification: `Activated only after clean .2 contract commit 58efc8aff. Activation changes task/index/Memory/roadmap/mdBook/HIAL-VIAL/contract-fact continuity state only. The normalizer remains literal 2..3, exact-two/three generated counters remain width two, and no exact-four source/support/test exists until implementation. All shipped parser/generator/report/residue/semantic-MCP API/HDL/runtime behavior is unchanged. Focused t1518+t1256+t1414 pass 3 files/22 tests; Knowledge Map remains current at 1,034 facts/5,278 keys; mdBook builds exactly 72 files/16,232,700 bytes and its repository-local generated tree is removed without residue; Memory is 60 lines; diff and all six doctrine gates pass. Canonical Stats-compatible RAM is 73.4% (17.628/24.000 GiB), kernel pressure is separately 1 (normal), and guard occupancy is excluded as capacity truth. Implementation evidence remains pending.`
  Commit: `IAL2-AHB-REQUESTER-EXACT-FOUR-BUSY-INSERTION-READINESS-AUDIT.3: activate generic exact-four BUSY`
