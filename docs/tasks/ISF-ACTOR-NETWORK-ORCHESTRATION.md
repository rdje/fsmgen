...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.58` | `active` | `.9.57` selected source-event-wait ordering hardening before pre-trigger acknowledgement or sticky event sampling behavior. |
...
- No blocker for active `.9.58`. It must add only the selected
  source-event-wait ordering hardening coverage around the shipped
  generated-child actor-to-actor route and must not widen pre-trigger
  acknowledgement or sticky event sampling behavior.
