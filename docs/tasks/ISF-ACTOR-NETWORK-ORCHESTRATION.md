...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.50` | `active` | `.9.49` selected repeated route-child event-wait hardening before event fan-in/fan-out or repeated wait sequencing behavior. |
...
- No blocker for active `.9.50`. It must add only the selected repeated
  route-child event-wait hardening coverage around the shipped
  generated-child actor-to-actor route and must not widen event
  fan-in/fan-out or repeated wait sequencing behavior.
