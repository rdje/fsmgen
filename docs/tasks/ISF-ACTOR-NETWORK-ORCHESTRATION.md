...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.62` | `active` | `.9.61` selected parent-work isolation around the contiguous route before pre/post route side effects are claimed. |
...
- No blocker for active `.9.62`. It must add only the selected
  parent-work-isolation hardening before claiming pre-route setup,
  post-route sampling, local side effects, cleanup work, route continuation,
  storage, muxing, ready/backpressure, or payload behavior.
