...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.65` | `active` | `.9.64` hardened route-boundary cardinality; the next leaf must select one bounded ATL widening or hardening slice before code. |
...
- No blocker for active `.9.65`. It must select exactly one next bounded ATL
  widening or hardening slice after the shipped route-boundary cardinality
  hardening.
