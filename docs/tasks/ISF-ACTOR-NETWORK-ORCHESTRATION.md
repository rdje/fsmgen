...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.59` | `active` | `.9.58` hardened source-event-wait ordering; the next leaf must select one bounded ATL widening or hardening slice before code. |
...
- No blocker for active `.9.59`. It must select exactly one next bounded ATL
  widening or hardening slice after the shipped source-event-wait ordering
  boundary.
