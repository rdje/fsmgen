...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.57` | `active` | `.9.56` hardened sink-event-wait ordering; the next leaf must select one bounded ATL widening or hardening slice before code. |
...
- No blocker for active `.9.57`. It must select exactly one next bounded ATL
  widening or hardening slice after the shipped sink-event-wait ordering
  boundary.
