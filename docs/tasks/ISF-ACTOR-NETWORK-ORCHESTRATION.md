...
| --- | --- | --- | --- |
| 1 | `ISF-ACTOR-NETWORK-ORCHESTRATION.9.47` | `active` | `.9.46` hardened same-child source/sink coverage for the generated-child actor-to-actor route; the next leaf must select the next bounded ATL widening or hardening slice before code. |
...
- No blocker for active `.9.47`. It is a selection leaf and must pick exactly
