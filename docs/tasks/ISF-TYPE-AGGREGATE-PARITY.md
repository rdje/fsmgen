...
- ID: `ISF-TYPE-AGGREGATE-PARITY.46`
  Status: `done`
  Goal: `support enum member values as standalone transaction conditions`
  Acceptance: `transaction when/while/until scalar conditions accept local/package enum members with computed .fsm selector review artifacts, strict CLI HDL generation, and fail-closed diagnostics for unknown members and expression operator-position enum members`
  Verification: `perl -Iperl -c perl/FSM/Adapter/ISF/Parser.pm`; `perl -Iperl -c perl/FSM/Scheduler/ISF/Emitter/FSM.pm`; `perl -Iperl -c perl/FSM/Support/ISFPublicInterfaceContract.pm`; `bash -n bin/ci-regression`; `prove -Iperl t/1300-isf-enum-member-standalone-condition-values.t t/1275-isf-enum-member-condition-values.t t/1295-isf-enum-member-switch-selector-values.t t/1265-isf-enum-member-switch-branch-values.t t/1263-isf-enum-member-set-values.t t/1272-isf-enum-member-rule-values.t t/1274-isf-enum-member-rule-guard-values.t t/1266-isf-enum-member-drive-values.t t/1279-isf-enum-member-inline-drive-values.t t/1144-isf-public-tested-by-metadata-audit.t t/1250-isf-spec-focused-test-index-audit.t t/1112-isf-public-interface-contract.t t/1115-isf-public-interface-cli-manifest-audit.t t/1183-ci-regression-tier-selection.t`; `./bin/ci-regression --list`; `./bin/ci-regression isf --no-book`; `mdbook build docs/book`; `git diff --check`
  Commit: `ISF-TYPE-AGGREGATE-PARITY.46: support enum standalone conditions`

- ID: `ISF-TYPE-AGGREGATE-PARITY.47`
  Status: `done`
  Goal: `support enum member values as standalone rule guards`
  Acceptance: `rule guard scalar values accept local/package enum members with guarded non-state DT .fsm review artifacts, strict CLI HDL generation, public when normalization, and fail-closed diagnostics for unknown members while enum targets and expression operator-position enum members remain deferred`
  Verification: `perl -Iperl -c perl/FSM/Adapter/ISF/Parser.pm`; `perl -Iperl -c perl/FSM/Support/ISFPublicInterfaceContract.pm`; `prove -Iperl t/1301-isf-enum-member-rule-standalone-guard-values.t t/1274-isf-enum-member-rule-guard-values.t t/1263-isf-enum-member-set-values.t t/1272-isf-enum-member-rule-values.t t/1266-isf-enum-member-drive-values.t t/1279-isf-enum-member-inline-drive-values.t t/1144-isf-public-tested-by-metadata-audit.t t/1250-isf-spec-focused-test-index-audit.t t/1112-isf-public-interface-contract.t t/1115-isf-public-interface-cli-manifest-audit.t`; `./bin/ci-regression --list`; `./bin/ci-regression isf --no-book`; `mdbook build docs/book`; `git diff --check`
...
- ID: `ISF-TYPE-AGGREGATE-PARITY.49`
  Status: `done`
  Goal: `close the enum/type/aggregate parity tree with an explicit shipped-surface book matrix and remaining-deferral boundary`
  Acceptance: `the tree records exhaustion/closure, the mdBook has a self-contained type/enum/aggregate shipped-surface page with examples, review-artifact guidance, diagnostics, and explicit deferrals, and the spec, downstream handoff, task index, roadmap status, and live docs agree that remaining enum target/operator and aggregate carrier/subaggregate work requires future task-tree ownership`
  Verification: `mdbook build docs/book`; `prove -Iperl t/1120-isf-public-live-document-path-audit.t t/1250-isf-spec-focused-test-index-audit.t t/1112-isf-public-interface-contract.t t/1115-isf-public-interface-cli-manifest-audit.t t/1144-isf-public-tested-by-metadata-audit.t`; `git diff --check`
  Commit: `ISF-TYPE-AGGREGATE-PARITY.49: close type aggregate parity tree`

## Current Frontier

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `none` | `done` | This tree is exhausted and closed; remaining enum target/operator and aggregate carrier/subaggregate work is explicitly deferred to future task-tree ownership. |
...
- `2026-05-16`: `ISF-TYPE-AGGREGATE-PARITY.46` selects standalone enum
  transaction conditions because enum condition expression operands and enum
  switch selectors already proved the computed-selector path. Local and
  package enum members such as `mode.BUSY` and `shared.mode.BUSY` now resolve
  as direct `when`/`while`/`until` conditions and lower through computed
  `.fsm` selectors, while expression operator-position enum members and
  non-condition enum targets remain closed.
- `2026-05-16`: `ISF-TYPE-AGGREGATE-PARITY.47` selects standalone enum rule
  guards because rule guard expression operands already resolve through the
  existing rule-DT guard path. Local and package enum members such as
  `mode.BUSY` and `shared.mode.BUSY` now resolve as shorthand and long-form
...
- `2026-05-16`: `ISF-TYPE-AGGREGATE-PARITY.49` closes this task tree because
  the remaining enum and aggregate items are semantic-contract families rather
  than adjacent parser widenings. Enum target/lvalue and operator-position
  behavior, subaggregate operands/updates, additional aggregate carriers,
  broader field/slice/update lowering, and shape inference now remain explicit
  future work outside this closed tree. The mdBook owns a self-contained
  shipped-surface matrix for the closed subset.
...
- Which enum member expression/value contexts beyond actor constants, actor
  ...
  transaction `switch` selector/branch values, rule guard scalar values or
  expression operands,
  rule assignment RHS scalar values/expression operands, drive body RHS scalar
  ...
  operands should ship next? Remaining enum work is now mainly target and
  operator-position contexts.
...
- Which enum or aggregate value/update context should ship after aggregate
  standalone transaction conditions? `ISF-TYPE-AGGREGATE-PARITY.46` shipped
  standalone enum transaction conditions.
...
- Which enum or aggregate value/update context should ship after aggregate
  standalone rule guards? `ISF-TYPE-AGGREGATE-PARITY.49` closed this tree
  after documenting the shipped surface and remaining deferrals.
- Which enum/type/aggregate items remain after this closure? Enum
  target/lvalue and operator-position behavior, non-static rule-action enum
  contexts outside shipped trigger overrides, subaggregate operands/updates,
  additional aggregate carriers, broader aggregate field/slice/update
  lowering, and shape inference require future task-tree ownership.
...
| `2026-05-16` | `ISF-TYPE-AGGREGATE-PARITY.49` | `mdbook build docs/book`; `prove -Iperl t/1120-isf-public-live-document-path-audit.t t/1250-isf-spec-focused-test-index-audit.t t/1112-isf-public-interface-contract.t t/1115-isf-public-interface-cli-manifest-audit.t t/1144-isf-public-tested-by-metadata-audit.t`; `git diff --check` | `passed` |
...
| `ISF-TYPE-AGGREGATE-PARITY.49` | `ISF-TYPE-AGGREGATE-PARITY.49: close type aggregate parity tree` | `Tree closure and shipped-surface mdBook matrix slice.` |
...
- `2026-05-16`: Shipped standalone transaction condition enum member values
  for `ISF-TYPE-AGGREGATE-PARITY.46` and advanced the frontier to
...
- `2026-05-16`: Closed the enum/type/aggregate parity tree with
  `ISF-TYPE-AGGREGATE-PARITY.49`, adding a self-contained mdBook shipped
  surface and recording remaining deferrals as future task-tree work.
