...
| `temporal_contracts[]` | `transaction`, `name`, `kind`, `trigger`, `signal`, `within_cycles`, `pending_signal`, `counter_signal`, `fail_signal`, `overlap_policy`, `reset_policy`, `assertion_projection` | Current `kind` value: `bounded_eventually`; `overlap_policy` value: `fail`; `assertion_projection` value: `systemverilog_sticky_fail`; `reset_policy` uses the bounded reset shape or null. |
