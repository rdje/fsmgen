...
| 80 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.80` | `pending` | Implements the selected switch-contained await_any-before-local-do subset. |
...
- None for tracking. Leaf `.80` must implement only the selected
  switch-contained await_any-before-local-do subset.
