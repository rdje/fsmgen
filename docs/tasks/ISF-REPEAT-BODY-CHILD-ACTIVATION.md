...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.103`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select exactly one remaining repeat-body child activation frontier before any further behavior-bearing implementation; candidate frontiers include switch-contained generated-child post-do await_any, static-parameter generated-do post-do await_any, bind/domain post-do variants, new spawn after the do before drain, cross-domain activation, deeper branch/loop nesting, or broader outstanding-child semantics, and the selected leaf records exclusions and focused validation.`
...
| 103 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.103` | `pending` | Selects exactly one remaining repeat-body child activation frontier before further implementation. |
...
- None for tracking. Leaf `.103` must select exactly one bounded remaining
  repeat-body child activation frontier before any additional implementation.
