...
| 98 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.98` | `pending` | Implements only the selected when-contained local-do-before-post-do-await_any subset. |
...
- None for tracking. Leaf `.98` must implement only the selected
  when-contained local-do-before-post-do-`await_any` subset and keep every
