...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.63`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select the next bounded repeat-body child activation subset before any code changes; candidate frontiers include switch-contained generated-child do while generated nested spawn is pending, parameterized/bound/domain-qualified generated do while spawn pending, await_any around pending-spawn do, new spawn after pending-spawn do before drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics.`
...
| 63 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.63` | `pending` | Selects the next bounded repeat-body child activation subset before more code. |
...
- None for tracking. Leaf `.63` owns the next selection before any additional
  repeat-body child activation implementation.
