...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.93`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select exactly one remaining fail-closed boundary before implementation; candidate frontiers include domain metadata after prior multi-pending await_any, await_any after do, new spawn after do before drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics.`
...
| 93 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.93` | `pending` | Selects the next remaining fail-closed repeat-body child activation subset before any further implementation. |
...
- None for tracking. Leaf `.93` must select exactly one remaining fail-closed
