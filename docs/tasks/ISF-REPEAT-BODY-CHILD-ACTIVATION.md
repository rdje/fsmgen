...
| 110 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.110` | `pending` | Implements only the selected when-contained bound generated-do-before-post-do-await_any subset. |
...
- None for tracking. Leaf `.110` must implement only the selected
  when-contained bound generated-do-before-post-do-`await_any` subset and
  keep every other remaining boundary fail-closed.
