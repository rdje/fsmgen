...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.107`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select the next bounded repeat-body child-activation subset before implementation; candidate backlog remains switch-contained static-parameter generated do before post-do multi-pending await_any, bind handoffs or domain metadata before post-do await_any, new spawn after do before drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics.`
...
| 107 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.107` | `pending` | Selects the next bounded repeat-body child-activation subset before implementation. |
...
- None for tracking. Leaf `.107` must select exactly one next bounded subset
