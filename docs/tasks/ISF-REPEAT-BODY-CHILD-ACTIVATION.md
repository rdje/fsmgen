...
| 100 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.100` | `pending` | Implements only the selected switch-contained local-do-before-post-do-await_any subset. |
...
- None for tracking. Leaf `.100` must implement only the selected
  switch-contained local-do-before-post-do-`await_any` subset and keep every
  other remaining boundary fail-closed.
