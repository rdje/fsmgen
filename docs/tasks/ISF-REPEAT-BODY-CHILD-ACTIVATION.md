...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.83`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select the next exact bounded subset before any further behavior-bearing implementation; candidate deferred surfaces include parameterized/bound/domain generated do after prior multi-pending await_any, the switch-contained generated-child analogue, await_any after do, spawn-after-do before drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics.`
...
| 83 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.83` | `pending` | Selects the next bounded repeat-body child activation subset. |
...
- None for tracking. Leaf `.83` must select the next bounded repeat-body child
  activation subset before implementation.
