...
| 96 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.96` | `pending` | Implements only the selected switch-contained domain generated-do await_any-before-do subset. |
...
- None for tracking. Leaf `.96` must implement only the selected
  switch-contained same-domain generated-do `await_any`-before-do subset and
