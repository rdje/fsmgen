...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.105`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select the next bounded implementation subset before any further behavior-bearing code change; candidate frontiers include static-parameter generated do before post-do multi-pending await_any, bind handoffs, domain metadata, new spawn after the do before the mandatory drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics, with every unselected frontier remaining deferred.`
...
| 105 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.105` | `pending` | Selects the next bounded repeat-body child activation subset before more implementation. |
...
- None for tracking. Leaf `.105` must select one bounded remaining
  repeat-body child activation subset before any additional behavior-bearing
  implementation and keep every unselected boundary deferred.
