...
| 94 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.94` | `pending` | Implements only the selected when-contained domain generated-do await_any-before-do subset. |
...
- None for tracking. Leaf `.94` must implement only the selected
  when-contained same-domain generated-do `await_any`-before-do subset and
