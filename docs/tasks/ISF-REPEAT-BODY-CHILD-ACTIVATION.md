...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.89`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select the next exact bounded repeat-body child activation subset before any behavior-bearing implementation begins; the selection must define the source shape, generated-top ownership, re-entry/drain contract, fail-closed exclusions, documentation impact, and focused validation scope.`
...
| 89 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.89` | `pending` | Selects the next bounded repeat-body child activation subset before any further implementation. |
...
- None for tracking. Leaf `.89` must select the next bounded repeat-body
  child activation subset before any further behavior-bearing implementation.
