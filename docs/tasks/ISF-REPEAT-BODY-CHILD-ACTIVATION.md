...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.99`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select exactly one remaining fail-closed boundary before implementation; candidate frontiers include switch-contained post-do await_any, generated-do post-do await_any, new spawn after do before drain, cross-domain activation, deeper branch/loop nesting, and broader outstanding-child semantics.`
...
| 99 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.99` | `pending` | Selects the next remaining fail-closed repeat-body child activation subset before any further implementation. |
...
- None for tracking. Leaf `.99` must select exactly one remaining fail-closed
  repeat-body child activation boundary before any further implementation.
