...
- ID: `ISF-REPEAT-BODY-CHILD-ACTIVATION.101`
  Status: `pending`
  Goal: `Select the next repeat-body child activation subset.`
  Acceptance: `Task tree, roadmap, and book backlog select exactly one remaining repeat-body child activation frontier before any further behavior-bearing implementation; candidate frontiers include generated-do post-do await_any, new spawn after the do before drain, cross-domain activation, deeper branch/loop nesting, or broader outstanding-child semantics, and the selected leaf records exclusions and focused validation.`
...
| 101 | `ISF-REPEAT-BODY-CHILD-ACTIVATION.101` | `pending` | Selects exactly one remaining repeat-body child activation frontier before further implementation. |
...
- None for tracking. Leaf `.101` must select exactly one bounded remaining
