...
| Legacy/backend expression AST | [FSM::AST::Node](../../perl/FSM/AST/Node.pm), in-file `FSM::AST::Utils` package, [FSM::ExpressionNamer](../../perl/FSM/ExpressionNamer.pm), [FSM::GlobalASTManager](../../perl/FSM/GlobalASTManager.pm) | Backend capture, expression naming, and factorization paths | `ASTFactorization`, `EnableGraph::*`, SystemVerilog backend support owners | No public report contract; some statistics expose counts or rendered expressions | Backend-local expression representation. It overlaps with `CoreAST` expression nodes but is not a source-level canonical IR. The former standalone `perl/FSM/AST/Utils.pm` duplicate was removed by `EXPR-AST-UTILS-OWNER-CONSOLIDATION.2`. |
