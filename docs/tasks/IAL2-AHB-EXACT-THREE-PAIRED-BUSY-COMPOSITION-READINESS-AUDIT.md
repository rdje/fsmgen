...
- ID: `IAL2-AHB-EXACT-THREE-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.3`
  ...
  Verification: `Activated only after clean .2 contract commit 547d8102f and clean activation commit 24b302647. Shipped ppif/ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park.ppif as the exact data-only extension of the byte-stable exact-two source: selected intent/object/anchor, amba_requester_busy_insert_three, before_beat=2/beats=3, existing subordinate/fabric/top, ports/window/storage/wiring unchanged. RegressionCorpus now records exact support intent.ppif_ahb_interconnect_requester_busy_insert_three_byte_lane_hburst_seq_busy_park and selected coverage as ppif/supported_smoke/strict/module ahb_tb/root top/3 children; t248+t297 pass 2 files/6,923 tests and actual inventory is 47 paths split 24 .ppif/23 .ahb at 323 protocol and 364 supported+strict. Focused t1531 passes 4 top-level/72 nested assertions in 334 seconds: strict/check/schedule, exact 3 IAL1/4 IAL0 artifacts, repository-local outdir, --verify-hdl, normalized semantic JSON, real fsmgen_semantic_introspect read_only=true/shell_access=false, and assertion-enabled Verilator runtime transfers=5/beats=4/BUSY episodes=1/qualified BUSY=3/resumed SEQ=1/storage=0x44332211 with direct 3 -> 2 -> 1 -> 0 and stable requester/subordinate/fabric ownership. Representative unchanged t1523+t1528+t1530 pass 3 files/11 tests in 417 seconds; all named t1520/t1523-t1526/t1528-t1530 owners are syntax-clean, exact-two one-/two-window and standalone exact-three aliases remain byte-identical, and no existing source/test bytes changed. Current-truth t1518 passes 5 top-level tests; corrected t1256+t1414 pass 2 files/17 after one obsolete t1256 filename caused a harness-path-only exit, not a test failure. Behavior/current docs, README, roadmap, support docs, mdBook, facts, task/index, and Memory synchronize; Knowledge Map is 1,024 facts/5,211 keys. mdBook builds and its exact 72-file/16,143,920-byte output was removed without residue; all six doctrine gates pass. Final Stats-compatible capacity is 64.7% (15.52/24.00 GiB), kernel pressure is 1 (normal), and guard occupancies up to 88.5% are excluded as RAM-capacity truth. No parser/generator algorithm, existing source bytes, report/semantic-MCP API, .ahb alias, two-subordinate exact-three topology, broader BUSY/protocol/backend, HIAL/VIAL activation, VHDL, verification generation, or decision-0020 behavior changed.`
  Commit: `IAL2-AHB-EXACT-THREE-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.3: ship exact-three paired AHB composition`
...
- `2026-07-29`: Clean contract commit `547d8102f` satisfies the `.3` activation
  ...
  activation.
- `2026-07-29`: `.3` ships the selected generic exact-three paired source at
  323/364/47 split 24 `.ppif`/23 `.ahb`; the child tree is complete and hands
  selection back to the parent frontier. The matching alias, two-window
  exact-three form, and HIAL/VIAL simulator-profile work remain separate.
