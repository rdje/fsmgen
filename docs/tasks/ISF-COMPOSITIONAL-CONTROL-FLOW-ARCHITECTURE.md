...
- Selected the `while`-contained three-spawn post-`do` multi-pending
  `await_any` observation plus later `await_all` drain as the next
  implementation slice:
  `(while cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (do helper) (await_any done) (await_all done)))`.
- The source/backlog anchor is the still-public deferred wider post-`do`
  multi-pending `await_any` boundary in the mdBook, specs, backlog, and
  Knowledge Map after `.8.24`.
- A read-only effect-checker probe proves the selected `while` shape has clean
  `while` and `repeat` backedges, static generated-spawn instances
  `w0`/`w1`/`w2`, generated-top start/done handoff requirements for
  `w0_done`/`w1_done`/`w2_done`, same-domain activation targets for all
  spawns and `helper`, a local `helper` drain,
  `await_any_observes_without_full_drain` over `w0_done,w1_done,w2_done`,
  `await_any_multi_pending_requires_later_drain`, and a later `await_all`
  drain for `w0_done,w1_done,w2_done`; public lowering still rejects the
  source at the post-`do` multi-pending `await_any` gate before the
  implementation leaf.
- The matching body-first `until` three-spawn post-`do` multi-pending
  `await_any`, generated `do`, fan-outs beyond four, and missing later
  `await_all` remain outside the selected next implementation leaf.

#### ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.26 — While-Contained Three-Spawn Post-Do Multi-Pending AwaitAny Then AwaitAll

Status: done

Goal: Accept the selected `while` repeat-body sequence where three generated
`spawn` clauses remain pending across a local blocking `do`, a post-`do`
multi-pending `await_any` observes one of the pending done pulses, and a later
same-body `await_all` drains the full outstanding set before repeat and
`while` re-entry.
...
- The public validator permits only the selected same-domain `while`
  three-spawn shape when `ControlFlowEffects` proves clean repeat/`while`
  backedges, deterministic generated-top handoffs and static instances for all
  pending generated spawns, a local blocking-`do` done drain, a post-`do`
  multi-pending `await_any` observation with a later-drain obligation, and an
  `await_all` drain over the exact outstanding spawned done-port set.
- Existing accepted one-, two-, three-, and four-spawn local-`do` `await_all`
  fixtures plus single-pending post-`do` `await_any` fixtures and the shipped
  `while`/`until` two-spawn post-`do` multi-pending `await_any` fixtures
  remain accepted.
- The matching `until` three-spawn post-`do` multi-pending `await_any`,
  generated `do`, cross-domain activation, missing final `await_all`, wider
  fan-outs, and unrelated deeper nesting remain fail-closed.
- The mdBook, downstream integration spec, live ISF spec/indexes, task tree,
  and Knowledge Map are updated for the new public behavior.
...
- Public lowering now accepts the exact
  ...
  `await_all` drain, and clean `repeat`/`while` backedges.
- Focused tests now cover the accepted `while` three-spawn post-`do`
  multi-pending `await_any` lowering path, keep the previously shipped
  two-spawn `while`/`until` paths accepted, and lock a four-spawn `while`
  post-`do` multi-pending `await_any` shape plus the matching three-spawn
  `until` shape behind the public gate.
- Existing one-, two-, three-, and four-spawn local-`do` `await_all` fixtures
  and single-pending post-`do` `await_any` fixtures remain accepted.
- User-facing docs, downstream handoff specs, the support matrix, backlog, and
  the Knowledge Map fact card now state that the exact `while` two- and
  three-spawn post-`do` multi-pending `await_any` plus later `await_all`
  shapes and exact body-first `until` two-spawn shape are shipped; body-first
  `until` three-spawn, generated `do`, four-or-wider post-`do`
  multi-pending `await_any`, fan-outs beyond four, and missing later
  `await_all` remain fail-closed.

#### ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.27 — Next Effect-Proven Combination Selection

Status: done

Goal: Select the next narrow behavior-widening combination that can be accepted
by construction through the migrated region/effect checker after the
`while` three-spawn post-`do` multi-pending `await_any` plus later `await_all`
slice.
...
- Selected the matching body-first `until` three-spawn post-`do`
  multi-pending `await_any` observation plus later `await_all` drain as the
...
- The source/backlog anchor is the still-public deferred body-first `until`
  three-spawn post-`do` multi-pending `await_any` boundary in the mdBook,
  specs, backlog, and Knowledge Map after `.8.26`.
- A read-only effect-checker probe proves the selected `until` shape has clean
  `until` and `repeat` backedges, static generated-spawn instances
  `w0`/`w1`/`w2`, generated-top start/done handoff requirements for
  `w0_done`/`w1_done`/`w2_done`, same-domain activation targets for all
  ...
  source at the post-`do` multi-pending `await_any` gate before the
  implementation leaf.
- Four-or-wider post-`do` multi-pending `await_any`, generated `do`, fan-outs
  beyond four, and missing later `await_all` remain outside the selected next
  implementation leaf.
...
- The public validator permits only the selected same-domain `until`
  three-spawn shape when `ControlFlowEffects` proves clean repeat/`until`
  backedges, deterministic generated-top handoffs and static instances for all
  pending generated spawns, a local blocking-`do` done drain, a post-`do`
  multi-pending `await_any` observation with a later-drain obligation, and an
  `await_all` drain over the exact outstanding spawned done-port set.
- Existing accepted one-, two-, three-, and four-spawn local-`do` `await_all`
  fixtures plus single-pending post-`do` `await_any` fixtures, the shipped
  `while` two-/three-spawn post-`do` multi-pending `await_any` fixtures, and
  the shipped `until` two-spawn post-`do` multi-pending `await_any` fixture
  remain accepted.
- Generated `do`, cross-domain activation, missing final `await_all`,
  four-or-wider post-`do` multi-pending `await_any`, and unrelated deeper
  nesting remain fail-closed.
- The mdBook, downstream integration spec, live ISF spec/indexes, task tree,
  and Knowledge Map are updated for the new public behavior.
...
- Selected the exact four-spawn `while` and body-first `until` local blocking
  `do` plus post-`do` multi-pending `await_any` observation and later same-body
  `await_all` drain combination for the next implementation leaf:
  `(while cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (spawn worker as w3) (do helper) (await_any done) (await_all done)))`
  and
  `(until cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (spawn worker as w3) (do helper) (await_any done) (await_all done)))`.
- The selection is grounded in the mdBook control-flow chapter, support
  matrix, backlog, live ISF spec, downstream integration spec, Knowledge Map
  fact card, and the focused effect-widening tests for the current
  three-spawn public boundary.
- The private effect checker already proves clean `while`/`until` and
  `repeat` backedges, static `w0`/`w1`/`w2`/`w3` identities, generated-top
  start/done handoffs for all four children, same-domain activation targets,
  a local `helper` drain, a post-`do` `await_any` observation over the exact
  pending set with a later-drain obligation, and a later `await_all` drain for
  `w0_done,w1_done,w2_done,w3_done`.
- Public lowering still rejects this four-spawn post-`do` multi-pending
  `await_any` shape at the existing public gate. This selection slice does not
  change behavior, so the mdBook, live specs, downstream spec, and Knowledge
  Map fact card do not need behavior wording changes here.
- `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.30` now owns the
  implementation before any source, test, generated artifact, or public
  behavior documentation change.

#### ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.30 — Four-Spawn Post-Do AwaitAny Fanout

Status: done

Goal: Implement the selected four-spawn `while` and body-first `until` local
blocking `do` plus post-`do` multi-pending `await_any` observation and later
same-body `await_all` drain combination by construction through the migrated
region/effect checker.

Acceptance:

- The public validator admits only the exact same-domain `while` and
  body-first `until` shape with four generated `spawn` children, a local
  blocking `do`, a post-`do` `await_any` over the exact pending set, and a
  later same-body `await_all` drain over that same set.
- The public widening consumes the migrated effect-checker proof before
  accepting the shape.
- Generated `do`, cross-domain activation, binding/CDC inference, missing
  later `await_all`, deeper unrelated nesting, and five-or-wider post-`do`
  multi-pending `await_any` fanout remain fail-closed.
- Focused coverage converts the current four-spawn `while`/`until` post-`do`
  multi-pending `await_any` negatives in
  `t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t` to
  positive lowering checks and adds five-spawn post-`do` `await_any` negative
  coverage.
- Emitted lowering evidence shows four child spawn states, the local blocking
  `do`, an `await_any` observation over `w0_done,w1_done,w2_done,w3_done`,
  and a later `await_all` drain over those four done ports before any repeat or
  loop backedge.
- The mdBook control-flow chapter, support matrix, backlog, live ISF spec,
  downstream integration spec, Knowledge Map fact card, task tree, and
  `MEMORY.md` are synced to the new public boundary.

Result:

- Public lowering now accepts the exact `while` and body-first `until`
  four-spawn local blocking `do` plus post-`do` multi-pending `await_any`
  observation and later same-body `await_all` drain shape when the migrated
  effect checker proves all four generated-spawn handoffs, the local
  `helper` drain, the observation-only `await_any` over
  `w0_done,w1_done,w2_done,w3_done`, the later exact `await_all` drain, and
  clean repeat/loop backedges.
- The public validator widening is limited to the proof-gated post-`do`
  `await_any` selector for exactly two, three, or four pending spawns in exact
  one-loop `while` or body-first `until` bodies.
- Focused coverage converts the former four-spawn `while` and `until`
  negatives in
  `t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t` to
  positive lowering checks and adds five-spawn `while`/`until` negative
  coverage. Five-or-wider post-`do` multi-pending `await_any`, generated `do`,
  missing later `await_all`, cross-domain activation, and unrelated deeper
  nesting remain fail-closed.
- The mdBook control-flow chapter now includes runnable four-spawn `while` and
  body-first `until` examples for post-`do` `await_any` plus later
  `await_all`; the support matrix, backlog, live ISF spec, downstream
  integration spec, and Knowledge Map fact card now state the shipped
  two-, three-, and four-spawn boundary and the five-or-wider deferral.
...
Goal: Select the next narrow behavior-widening combination that can be accepted
by construction through the migrated region/effect checker after the
four-spawn post-`do` multi-pending `await_any` plus later `await_all` slices.
...
- 2026-06-11 (`.8.25`): selected the `while` three-spawn local blocking `do`
  plus post-`do` multi-pending `await_any` observation and later same-body
  `await_all` drain combination for the next implementation leaf:
  `(while cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (do helper) (await_any done) (await_all done)))`.
  A read-only `perl -Iperl` probe confirms the private effect checker proves
  clean `while`/`repeat` backedges, static `w0`/`w1`/`w2` identities,
  generated-top start/done handoff requirements for all three instances,
  same-domain activation targets, a local `helper` drain,
  `await_any_observes_without_full_drain` over `w0_done,w1_done,w2_done`,
  `await_any_multi_pending_requires_later_drain`, and a later `await_all`
  drain for `w0_done,w1_done,w2_done`; public lowering still rejects it at the
  post-`do` multi-pending `await_any` gate before the implementation leaf.
  `prove -Iperl
  t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t`,
  `scripts/check_memory_architecture.sh`,
  `knowledge-map/scripts/check_knowledge_map.sh`, `prove -Iperl
  t/1414-docs-relative-paths-audit.t`, and `git diff --check` pass.
- 2026-06-11 (`.8.26`): accepted the selected `while` three-spawn local
  blocking `do` plus post-`do` multi-pending `await_any` observation and later
  same-body `await_all` drain shape by widening only the `while` one-loop
  post-`do` `await_any` selector from two to two-or-three pending spawns.
  Added positive three-spawn `while` coverage, added a four-spawn `while`
  negative while preserving the three-spawn `until` negative, and synced the
  mdBook control-flow chapter, support matrix, backlog, live ISF spec,
  downstream integration spec, and Knowledge Map fact card to the new
  boundary. `perl -Iperl -c perl/FSM/Scheduler/ISF/LoweringIR.pm`; `perl
  -Iperl -c
  t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t`; `prove
  -Iperl t/1432-isf-loop-pending-spawn-local-do-effect-widening.t
  t/1433-isf-until-pending-spawn-local-do-effect-widening.t
  ...
  `scripts/check_memory_architecture.sh`; `git diff --check`; and
  `./bin/ci-regression isf --no-book` (Files=294, Tests=2133) pass.
- 2026-06-11 (`.8.27`): selected the matching body-first `until` three-spawn
  local blocking `do` plus post-`do` multi-pending `await_any` observation and
  later same-body `await_all` drain combination for the next implementation
  leaf:
  `(until cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (do helper) (await_any done) (await_all done)))`.
  A read-only `perl -Iperl` probe confirms the private effect checker proves
  clean `until`/`repeat` backedges, static `w0`/`w1`/`w2` identities,
  generated-top start/done handoff requirements for all three instances,
  same-domain activation targets, a local `helper` drain,
  `await_any_observes_without_full_drain` over `w0_done,w1_done,w2_done`,
  `await_any_multi_pending_requires_later_drain`, and a later `await_all`
...
- 2026-06-11 (`.8.28`): accepted the selected body-first `until` three-spawn
  ...
  `scripts/check_memory_architecture.sh`; `git diff --check`; and
  `./bin/ci-regression isf --no-book` (Files=294, Tests=2133) pass.
- 2026-06-11 (`.8.29`): selected the exact four-spawn `while` and
  body-first `until` local blocking `do` plus post-`do` multi-pending
  `await_any` observation and later same-body `await_all` drain combination for
  the next implementation leaf:
  `(while cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (spawn worker as w3) (do helper) (await_any done) (await_all done)))`
  and
  `(until cond (repeat n (spawn worker as w0) (spawn worker as w1) (spawn worker as w2) (spawn worker as w3) (do helper) (await_any done) (await_all done)))`.
  The selection is grounded in the mdBook/spec/downstream/Knowledge Map
  boundary and in the current focused tests: the private effect checker proves
  clean `while`/`until` and `repeat` backedges, static `w0`/`w1`/`w2`/`w3`
  identities, generated-top handoffs, same-domain activation targets, the
  local `helper` drain, the post-`do` multi-pending `await_any` observation
  with a later-drain obligation, and the later exact `await_all` drain; public
  lowering still rejects the shape at the four-or-wider post-`do` multi-pending
  `await_any` gate before `.8.30`. `perl -Iperl -c
  perl/FSM/Scheduler/ISF/ControlFlowEffects.pm`; `perl -Iperl -c
  perl/FSM/Scheduler/ISF/LoweringIR.pm`; `prove -Iperl
  t/1432-isf-loop-pending-spawn-local-do-effect-widening.t
  t/1433-isf-until-pending-spawn-local-do-effect-widening.t
  t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t`;
  `knowledge-map/scripts/check_knowledge_map.sh`;
  `scripts/check_memory_architecture.sh`; and `git diff --check` pass.
- 2026-06-11 (`.8.30`): accepted the selected four-spawn `while` and
  body-first `until` local blocking `do` plus post-`do` multi-pending
  `await_any` observation and later same-body `await_all` drain shape by
  widening only the proof-gated one-loop post-`do` `await_any` selector to
  exactly two, three, or four pending spawns for both `while` and `until`.
  Converted the former four-spawn `while`/`until` negatives to positive
  lowering checks, added five-spawn `while`/`until` negatives, and synced the
  mdBook control-flow chapter, support matrix, backlog, live ISF spec,
  downstream integration spec, and Knowledge Map fact card to the new boundary.
  `perl -Iperl -c perl/FSM/Scheduler/ISF/LoweringIR.pm`; `perl -Iperl -c
  t/1434-isf-while-pending-spawn-local-do-awaitany-effect-widening.t`;
  `prove -Iperl t/1432-isf-loop-pending-spawn-local-do-effect-widening.t
  t/1433-isf-until-pending-spawn-local-do-effect-widening.t
...
- `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.25`: this commit,
  `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.25: select wider post-do awaitany local-do fanout`.
- `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.26`: this commit,
...
- `ISF-COMPOSITIONAL-CONTROL-FLOW-ARCHITECTURE.8.29`: this commit,
