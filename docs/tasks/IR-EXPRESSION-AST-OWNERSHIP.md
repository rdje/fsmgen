...
| Backend AST utility package | [perl/FSM/AST/Node.pm](../../perl/FSM/AST/Node.pm) | Convenience factory surface for backend AST nodes. | Callers load `FSM::AST::Node` and use the in-file `FSM::AST::Utils` package. | `CaptureSupport`, `EnableSupport`, and `AssignmentSupport`. | The former standalone `perl/FSM/AST/Utils.pm` duplicate was removed by `EXPR-AST-UTILS-OWNER-CONSOLIDATION.2`; the in-file package in `Node.pm` is the sole live owner. |
...
| Tracked ExpressionNamer copy | former `perl/FSM/ExpressionNamer.pm.new` | Tracked source residue declaring the same `FSM::ExpressionNamer` package name as the live module. | None found in `bin`, `perl`, `t`, or `docs` references. | None found by static search. | This was selected for cleanup and later removed by `EXPR-NAMER-TRACKED-COPY-CLEANUP.1`. |
...
| `GlobalASTManager` | Actionable legacy ownership concern | Static search found no runtime import/use outside its own module and compatibility audit tests. Its `run_global_factorization` path parses strings through `ExpressionNamer`, while `collect_ast` drops unblessed values; that conflicted with its former "single authority" header and with the current live SystemVerilog `GlobalFactorizationSupport` owner. | `.3` created a cleanup leaf; `GLOBAL-AST-MANAGER-BOUNDARY.1` classified it as compatibility-only and `.2` corrected stale ownership wording. |
| Former standalone `perl/FSM/AST/Utils.pm` versus in-file `FSM::AST::Utils` in `Node.pm` | Actionable duplicate owner | Callers use the in-file package loaded by `FSM::AST::Node`; the former standalone file defined the same package and referenced `FSM::AST::Node::Literal` / `BinaryOp` / `UnaryOp`, which did not match the actual classes. | `.3` created a cleanup leaf; `EXPR-AST-UTILS-OWNER-CONSOLIDATION.2` removed the standalone duplicate. |
| Former tracked `perl/FSM/ExpressionNamer.pm.new` | Actionable tracked residue | It declared the same package as the live module, was tracked, and no references loaded it. It created source-of-truth ambiguity for review even if runtime could not load it through normal `use FSM::ExpressionNamer`. | `.3` created a cleanup leaf; `EXPR-NAMER-TRACKED-COPY-CLEANUP.1` removed it. |
