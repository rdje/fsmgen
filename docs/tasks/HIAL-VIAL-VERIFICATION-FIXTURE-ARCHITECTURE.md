...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.5.3.1`
  ...
  Verification: `Clean lifecycle implementation 037c962fd and activation 1cd7310e9 provide the exact authority and boundary. Decision 0084 selects a Darwin-only explicit-guard qualification watcher plus a default bounded evidence watcher, not a backend workaround. On macOS 26.5.2 build 25F84/Mac16,11/M4 Pro/APFS with Verilator 5.046 and Apple clang 21, the primary generated binary passes with first output at 391.195 ms while one compiler orchestrator is active and syspolicyd is 49.0% CPU; two natural zero-compiler observations also pass at 311.167/345.101 ms. Generated 1,847,672-byte 0755 ARM64 binaries, their byte-identical different-path controls, and fresh minimal C++ controls share com.apple.provenance and valid linker-created ad-hoc signatures; all execute, as does /usr/bin/true. The exact quiet-primary four-second policy window has zero syspolicyd log events. Final quiet t1558 passes Files=1/Tests=7 in 167 seconds including repeated API/CLI/phase/direct-drive byte determinism and exact cleanup. These controls refute compiler load, high policy-daemon CPU, provenance/signature presence, generated bytes, and generated path as individually sufficient deterministic causes; they do not erase the three historical 30-second zero-output stalls or the 2,389/2,389 _dyld_start sample. The guarded watcher keeps a primary timeout failed, runs no retry, samples read-only after two seconds, writes closed host-path-free new-file-only JSON below a same-device non-symlink root, and changes no unrelated process, security/signing, deadline, backend, public result, support, performance, capacity, or API contract. Bounded evidence t1665+t1666 passes Files=2/Tests=50 by default; the hardened quiet guard passes Files=1/Tests=27.`
