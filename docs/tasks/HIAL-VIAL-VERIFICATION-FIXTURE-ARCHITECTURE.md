...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.2.1`
  ...
  Verification: `Clean failure evidence at 363165d94 and the enforced acceptance checklist below retain the exact controller diagnostic, component timings, falsification probe, implementation mechanism, focused regressions, scale proofs, documentation, claim, and doctrine closure. Implementation commit 514761a4e preserves rejected/not-run evidence, uses bounded proven-ASCII lexing with the Unicode cursor path intact, and normalizes wide four-state masks linearly without changing the fixture, timeout, API, or claims. After that commit, the exact old-revision incomplete publication was re-censused at 54 files/4,511,776 bytes with no family/full manifest or staging residue, then removed as revision-incompatible ignored evidence. A clean-revision guarded source_bytes_combined/limit_v1 validation plus independent report regeneration completes in 56.893675 seconds total: accepted_validation/accepted, parse_validate validated_unmeasured, one 4,514-byte stage output, eighteen fully censused foundation artifacts, and exact staging cleanup. The publication and staging namespaces are empty, the Git tree is clean, and the full exact 108-profile capture is ready under parent .17.3.2. The deleted incomplete evidence is not Git-recoverable as files; its exact census, failure facts, and digest remain durable in this task and commits f350553e1/514761a4e.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3`
  ...
  Verification: `Clean predecessor 9980fe5c1 closes the exact semantic/bridge matrix before this pivot. The caller-sealed adapter and isolated publisher reconstruct only canonical producer shapes, delegate guarded validation/sampling/lifecycle to the common foundation, bind source-free envelopes without invented controller evidence, and seal families only after every immutable child shares one clean revision, host, tool, and guard identity. Children .1 and .2 repair interruption recovery, per-profile isolation, early total-operation and serialized-plan enforcement, redundant random evaluation, and accepted-plan transcript reuse without weakening guards, timeouts, caps, authority, or nonclaims. At clean revision 9c0209c22, the exact capture publishes all 40 execution and 32 checking profiles; an 88% host-memory cutoff during immutable execution-family revalidation leaves zero staging and is recovered by exact same-identity resume. Both families and complete identity execution-checking-matrix/0548629720a9f4c8636c912396cfaefff523da92fc800594618db68b788b91d0 seal accepted: 41 accepted, 19 expected-rejection, five inapplicable-measurement, two preflight-dominated, 31 applicable-measurement, 62 controller-applicable, ten controller-inapplicable, 119 raw records, ten source-free profiles, and zero exclusions. The active namespace contains exactly 72 profiles plus two family manifests and one complete manifest, 75 files/7,312,415 bytes, with zero raw staging. A separate guarded `--validate` canonically reloads the same identity and partition with zero diagnostics. Backend emission, provider/tool access, compile/run/trace/result work, promoted budgets, remaining cap-policy work, support, capacity, and reached-boundary claims remain separately owned.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3.1`
  ...
  Verification: `Clean commit c7493e3de gives each exact relative stage identity a stable repository-volume advisory lock, rejects live contention, and reclaims only a recursively validated real-directory/regular-single-link same-volume orphan while ownership is held; symlink, FIFO, hard-link, lock-identity, and cleanup ambiguity fail closed. The retained execution bindings gate/03 tree was recovered by the real adapter—not manually deleted—and its replacement report accepted three samples, zero exclusions, and zero residue. Exact/default foundation, adjacent adapter/publisher, locality, CI-inventory, task acceptance, all doctrines, and bounded warning-debt declaration pass. Two post-commit captures cross the former raw-stage crash point and leave zero staging residue; this leaf is complete. The second run resumes nineteen immutable profiles but exposes separately owned coordinator heap retention before profile twenty.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3.2.1`
  ...
  Verification: `ExecutionBuilder now calls _preflight_expanded_operations_total immediately after exact scenario selection. The helper independently walks compact ordinary, repeat, and parallel action trees; validates nonempty canonical structure and positive repeat counts; saturates addition and multiplication at the relevant bound; requires each result to equal SemanticBuilder's validated per-scenario action_count; and applies the unchanged expanded_operations_total cap before semantic identity hashing, bridge indexing, binding, type registration, graph/source-map construction, or plan serialization. The existing terminal check remains. Synthetic compact tests accept exactly 1,000,000, reject 1,000,001 with the byte-equal historical diagnostic, and reject forged counts as internal consistency failures. The real exact over-limit evaluation passes under the unchanged 4,096-MiB guard at Files=1/Tests=9 in one second, records expected_rejection, and proves _index_bridge has zero entries. Accepted ExecutionIR, topology, adjacent total-operation qualification, and measurement-adapter regressions pass at Files=4/Tests=19 in 80 seconds; active execution publications and raw staging remain absent. The task, decision, mdBook, fact card, Memory, claims, containment, and doctrine evidence are synchronized. Parent .17.3.3.2 is the active fresh-matrix frontier; broader .17.4 cap-policy work remains proposed.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3.2.3`
  ...
  Verification: `ExecutionBuilder performs a general canonical plan projection before random-decision-list, random-source-map, ExecutionIR, or terminal-report materialization. The unchanged 16,777,216-byte cap runs on that exact projection; accepted plans match terminal canonical bytes and still pass the original terminal check. Child .1 replaces duplicate accepted-plan random search with one private sealed bounded transcript while independently reconstructing final expectations and records. RAM-guarded evidence keeps 8,440 accepted at 16,775,415 bytes, rejects 8,441 and 32,768 with the byte-equal historical /plan diagnostic, and preserves 65,536 preflight dominance plus 65,537 occurrence-cap dominance. The former random-occurrences qualification route now publishes expected_rejection/validated_not_measured, and the full clean-revision matrix seals all 40 execution and 32 checking profiles plus both families and the complete manifest. Separate guarded reload accepts the same matrix identity with zero diagnostics or raw exclusions. Frozen random/replay hashes, identities, attempts, diagnostics, ordinary ExecutionIR behavior, private report/transcript boundaries, guards, timeouts, caps, and explicit nonclaims remain unchanged.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3.2.3.1`
  ...
  Verification: `The revision-037e56f09 diagnosis/archive remain exact: 22 reports/2,609,009 bytes reconstruct at digest 6dbfa2d02f7eee0332881074628d50d2b454fe94ac241137a11d7c651c162d90, while the focused RED records two ExecutionRandom::generate entries for one accepted occurrence. Decision 0079 is implemented generally: projection generates or replay-validates once into a private SHA-256-sealed, u32-framed canonical UTF-8 scalar bounded to 17,039,360 bytes; accepted materialization independently rewalks and validates every expectation/record before consuming and deleting the transcript. Corruption, over-bound, authority-precedence, exact-limit, exhaustion, and guarded profile-23 suites pass without cap, timeout, guard, rerun, identity, or nonclaim changes. At clean revision 9c0209c22cd035fdc4dd55e3938842b79b36b622, the exact guarded capture publishes 40 execution and 32 checking profiles. Its initial process seals execution, then the unchanged 88% host cutoff stops immutable family revalidation with no raw staging or partial checking publication; exact resume revalidates that seal, publishes checking, and seals accepted complete identity execution-checking-matrix/0548629720a9f4c8636c912396cfaefff523da92fc800594618db68b788b91d0. The 75 active files occupy 7,312,415 bytes and contain 41 accepted, 19 expected-rejection, five inapplicable-measurement, two preflight-dominated, 31 applicable-measurement, 62 controller-applicable, ten controller-inapplicable, 119 raw-measurement, ten source-free, and zero excluded records. A separate guarded `--validate` returns the same identity/partition with zero diagnostics; final staging and process censuses are empty. Explicit support, performance-budget, capacity, reached-boundary, backend, external-tool, compile/run/trace/result, partial/mixed matrix, discarded-sample, mutable-publication, and public-API nonclaims remain intact.`
