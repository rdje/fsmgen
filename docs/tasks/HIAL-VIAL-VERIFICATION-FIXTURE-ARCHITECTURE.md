...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.8`
  ...
  Verification: `Clean .17.6 implementation commit 5db70fa08 aligns the support contract and passes exact Runner/capability/book/doctrine gates. Post-commit rereading of normative inputs finds decisions 0055/0056 still use present-tense 4-MiB/4-MiB and proposed-.17.6 language. The decision-store convention requires preserving selection history while recording later execution status rather than letting automation consume stale current truth.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.8: activate scale-decision execution reconciliation`
