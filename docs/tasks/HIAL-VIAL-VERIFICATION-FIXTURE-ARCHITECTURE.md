...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2`
  ...
  Verification: `ArchitectureScaleExecutionGraph now owns the binding, topology, fiber, type, and map gates, the complete random, serialized-plan, scenario, operations-per-scenario, live-fiber, total-fiber, and total-operation ladders, and the execution-type qualification level. Each level is frozen by its own focused test - t/1604 through t/1627 - and each slice's exact sources, SemanticIR, bridge, workload and plan identities, measurements, diagnostics, deterministic reruns, mutation rejection, unowned-level closure, public capability isolation, and named regression gates are recorded in this file's per-slice `Acceptance Checklist (enforced)` sections; the mdBook chapter and the `vial-execution-scale-reachability` fact card carry the same numbers for readers. Four axes reach their own nominal cap: simultaneously_live_fibers at 16,384 and scenarios at 4,096 with accepted plans, fibers_total at 65,536 structurally before the 16-MiB serialized-plan cap rejects its plan, and operations_total one operation past 1,000,000. The operations-per-scenario pair and the total-operation qualification and limit levels never reach theirs, and each records one VIAL_SCALE_LIMIT_INTERACTION routed to .17.4. Two constructions are in use because the 1,048,576-byte VIAL source cap forbids one literal record per unit above 74,824 single-scenario resets, 74,656 fanned-out resets, and 22,536 fibers: gate and qualification levels are literal, the two highest fiber and total-operation levels use the ordinary repeat form. Proof method is now also selected per level: decision 0061 clause 8 makes the 1,000,000-operation limit preflight_dominated/not_materialized against its measured 65,536-operation witness rather than spending roughly 32 GiB to reach the same answer, while 1,000,001 is proved by a real run behind FSMGEN_VIAL_SCALE_EXACT at a raised 6,144-MiB descendant cutoff. The owned frontier is one published %OWNED_LEVELS declaration - 32 of the catalog's 65 shapes - that construct gates on and every sibling test derives its still-unowned boundary from, so owning a level no longer staleness-breaks its siblings. Stage order is now uniform: decision 0061 clause 4 evaluates VIAL source and SemanticIR before the canonical bridge on the direct-IAL1 route too, which is what makes the 4,096-declaration parser cap - not a bridge cap - the authority for 8,192 types, over a route whose own measured boundary is 1,043 types at the 16-MiB serialized-manifest cap. The leaf remains active for the binding and source-map levels, the two highest execution-type levels, final qualification, and cleanup. No public capability, support, performance, or capacity claim changes, and a reached structural cap is reported as reached, never as supported capacity.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2: reach the execution-type qualification authority`
  Blocked by: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.3 - every level this leaf has left is a bindings, execution_types, or source_map_records level whose decision-0061 authority measurement contradicts: each is decided by the workload's own 1,114,112-byte construction envelope before any product cap, and events, types, and endpoint declarations have no compact repeat form to reach the product cap the way fibers and operations did. Authoring them now would freeze a fixture bound as a product limit. Unblocks when .17.2.4.3 selects the treatment; run .17.2.4.3 instead.`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.3`
  ...
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.3: own the mispredicted execution-scale authorities`
  Blocked by: `none`
...
That is where implementation stops for now. Every level `.17.2.4.2` has left is
a `bindings`, `execution_types`, or `source_map_records` level whose selected
authority measurement contradicts, always the same way: the generated source
...
16-MiB serialized-manifest cap, and 65,553 source maps at the 65,536
expanded-action semantic cap. `.17.2.4.2` is therefore blocked and new
`.17.2.4.3` is active to select whether those axes are re-levelled onto their
measured product boundaries, recorded as construction-envelope rejections, or
given a different envelope; a fixture bound must never be reported as a product
limit. Implementation, final qualification, and cleanup follow that selection,
after which proposed `.17.2.5` through `.17.2.7` and measurement `.17.3` come
next.
...
- `.17.2.4.2` measured the million-record execution cost while closing the
  total-operation ladder: an operation graph costs about 5.0 KiB of resident
  state per operation (436 MiB at 65,536, 1,442 at 262,144, 2,692 at 524,288,
  3,977 at 786,432), and serializing a plan costs several times that again. Two
  selected levels that are still unowned ask for the same million records -
  `source_map_records` `limit_v1`/`over_limit_v1`, whose maps are emitted one
  per operation - and so do `.17.2.5`'s `checking_state_v1`
  `scoreboard_capacity` and `bins_and_cross_tuples` levels. Decide for each
  whether it is preflight-dominated by a smaller witness under decision `0061`
  clause 8 or proved by an opt-in run at a raised descendant cutoff, before
  authoring it; do not put a multi-GiB construction into a default gate.
- Decision `0061`'s selected authorities for the still-unowned `bindings`,
  `execution_types`, and `source_map_records` levels are contradicted by
  measurement, and the contradiction is uniform: the generated source exceeds
  the workload's own 1,114,112-byte construction envelope before any product cap
  is consulted. `bindings` needs 1,933,429 / 3,866,741 / 3,866,800 HIAL bytes at
  32,768 / 65,536 / 65,537; `execution_types` needs 2,413,815 HIAL bytes at
  65,536; `source_map_records` needs 3,670,808 / 14,000,792 VIAL bytes at
  262,144 / 1,000,000. Unlike fibers and operations, events, types, and endpoint
  declarations have no `repeat` form to compact them. Each axis's genuine
  product boundary is measured and much lower: 2,054 bindings (bridge event cap
  2,048, rejecting at `/events`), 1,043 execution types (16-MiB serialized
  manifest, rejecting at `/`), and 65,553 source maps (the 65,536 expanded-action
  semantic cap). Select, before authoring these levels, whether to re-level
  these three axes onto their measured product boundaries, to record the nominal
  levels as construction-envelope rejections, or to change the envelope - and do
  not let a fixture bound be reported as a product limit.
