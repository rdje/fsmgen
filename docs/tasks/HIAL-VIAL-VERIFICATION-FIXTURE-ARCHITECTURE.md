...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.2`
  ...
  Verification: `Clean predecessor 3946bf101 completes the common measurement foundation and activation commit f82645cf8 owns this leaf. The honest watcher RED failed only because FSM::VIAL::ArchitectureScaleSemanticBridgeMeasurement did not exist. The implemented exact-class adapter exposes only canonical construct, validate, measure, and independent report-validation entrypoints for semantic_catalog_v1 and bridge_fanout_v1. It reconstructs every input through the completed family producer, regenerates each stage twice, embeds complete projected family evaluation evidence, retains the byte-exact unprojected canonical evaluation as a censused artifact, and admits no caller construction, IR, report, worker, stage plan, sample, or count. Semantic costs stop at construct/parse_validate; bridge costs add bridge; all are FSMGen-owned, and later stages remain not_run. Validation retains no performance counters; gate and qualification measurement require the active repository guard and exactly three/five ordinals after accepted correctness. A collision-checked recursive boundary renames canonical diagnostic path keys to semantic_path only in the measurement evidence, so semantic pointers stay meaningful without weakening the foundation's machine-local-path rejection; independent validation rederives that projection and the original artifact identity. Exact authority, applicability, raw-record, exclusion, diagnostic, cleanup, nonclaim, and aggregate identities fail closed. Focused exact adapter measurement passes under the guard at Files=1/Tests=5; common measurement/guard regression passes Files=2/Tests=8; adjacent bridge/adapter regression passes Files=2/Tests=11; syntax and diff checks pass. Clean adapter commit ae6f346ec precedes a new exact 108-profile matrix inventory and repository-root-derived operator. The runner requires a clean revision and the real RAM guard, publishes each complete profile set by same-volume atomic rename, resumes only revalidated same-revision/common-host/tool/guard evidence, rejects collisions and ambiguous crash staging, withholds family manifests until their exact child census passes, and withholds the complete seal until both families share one identity. The focused default runner watcher plus adjacent adapter pass Files=2/Tests=8; an isolated same-volume probe proves publication, reload, idempotence, collision rejection, and cleanup. First clean capture at b3a03d285 failed closed before publication because the inventory used gate/qualification shorthand while the adapter sealed gate_measurement/qualification_measurement; the focused fix now reuses that exact adapter vocabulary and a guarded first profile admits all three gate samples. A second clean capture at 363165d94 published 54 exact semantic profile sets, then profile 55 source_bytes_combined/limit_v1 failed closed without publication: the foundation truthfully recorded VIAL_SCALE_MEASUREMENT_TIMEOUT at parse_validate after the fixed 120-second ceiling, but adapter revalidation masked it as an output-count mismatch. Guarded component timing isolates 7.91 seconds of construction, 60.16 seconds for check_source, 60.33 seconds for the second parse, 0.72 seconds for report/integrity/projection work, and 44.16 seconds for two formatter passes; the parser lexer re-encodes each decoded character to advance byte spans. Child .17.3.2.1 owns truthful failure preservation and throughput repair without changing the fixture or timeout. The incomplete old-revision publication census is exactly 54 files/4,511,776 bytes with combined sorted-manifest SHA-256 d3628a55e4501331959be02fd14b4905403d9f98dad8923c815222a9cdc1fece, no family/full manifest, and no staging residue. No provider, compiler, simulator, external verification tool, public API, performance budget, support, capacity, or reached-boundary claim is added.`
