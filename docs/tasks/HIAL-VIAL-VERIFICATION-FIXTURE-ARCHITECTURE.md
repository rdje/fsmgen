...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2`
  ...
  Verification: `ArchitectureScaleExecutionGraph now owns 40 selected shapes: every selected level across the ten implemented execution axes, distinct from the catalog's 25 deliberately non-selected fail-closed shapes. Each level is frozen by focused t/1604 through t/1628 evidence with exact source, SemanticIR, bridge, workload, plan, diagnostic, mutation, rerun, public-isolation, and earliest-authority oracles; the mdBook and the partitioned reachability/axis-outcome Knowledge Map cards carry the same reader-facing facts. Four axes reach their nominal cap, while earlier semantic, bridge, serialized-plan, construction-envelope, or selected preflight authorities are reported without being promoted to support or capacity. Decision 0072 governs eight unconstructible levels: three binding, two execution-type, and three source-map levels. Source-map qualification/limit/over generate 3,670,808/14,000,792/14,000,806 VIAL bytes for 262,144/1,000,000/1,000,001 requested maps against the 1,114,112-byte input-1 envelope; all return envelope_unconstructible/not_constructed with no retained source or stage identity, refused builds, and paired fixture-interaction plus measured 46,294/46,295 whole-route boundary records routed to .17.4. Canonical validation reconstructs the exact source-free generic failure rather than retaining checked-AHB bytes. The exact pre-partition 383-line/26,208-byte card remains commit-retrievable and reconstructs as an 11-question route/gate card plus a 36-question axis-outcome card with answer-set equality. The leaf remains active only for final family qualification and cleanup.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2: close source-map limits`
  Blocked by: `none`
...
every prior answer key exactly once. Final family qualification and cleanup
follow, after which proposed `.17.2.5` through `.17.2.7` and measurement
`.17.3` come next.
...
- `.17.2.4.2` measured the million-record execution cost while closing the
  ...
  3,977 at 786,432), and serializing a plan costs several times that again.
  Decision `0072` now keeps the source-map million-record levels out of that
  multi-GiB route by reporting their earlier construction-envelope result.
  `.17.2.5` must still select the proof method for its `checking_state_v1`
  `scoreboard_capacity` and `bins_and_cross_tuples` million-record levels before
  authoring them; do not put a multi-GiB construction into a default gate.
