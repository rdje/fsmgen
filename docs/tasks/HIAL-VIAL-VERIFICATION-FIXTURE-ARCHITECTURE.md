...
profile-local structural oracles, and four diagnosed behavior defects. Proposed
repair parent `.17.2.6.2` is the next separate activation; it will decompose
the five ordered slices and select linear portable-VHDL validation plus
accepted-boundary proof first. Generator `.17.2.6.3`, runtime/balanced
