...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2`
  ...
  Verification: `ArchitectureScaleExecutionGraph now owns the binding, topology, fiber, type, and map gates, the complete random, serialized-plan, scenario, operations-per-scenario, live-fiber, and total-fiber ladders, and the total-operation qualification level. Each level is frozen by its own focused test - t/1604 through t/1625 - and each slice's exact sources, SemanticIR, bridge, workload and plan identities, measurements, diagnostics, deterministic reruns, mutation rejection, unowned-level closure, public capability isolation, and named regression gates are recorded in this file's per-slice `Acceptance Checklist (enforced)` sections; the mdBook chapter and the `vial-execution-scale-reachability` fact card carry the same numbers for readers. Three axes reach their own nominal cap: simultaneously_live_fibers at 16,384 and scenarios at 4,096 with accepted plans, and fibers_total at 65,536 structurally before the 16-MiB serialized-plan cap rejects its plan. The operations-per-scenario pair and the total-operation qualification level never reach theirs, and each records one VIAL_SCALE_LIMIT_INTERACTION routed to .17.4. Two constructions are in use because the 1,048,576-byte VIAL source cap forbids one literal record per unit above 74,900 resets and 22,536 fibers: gate and qualification levels are literal, the two highest fiber levels use the ordinary repeat form. The leaf remains active for the total-operation limit/over-limit levels - see Open Questions for their two measured costs - the binding, type, and source-map qualification/limit/over-limit levels, final qualification, and cleanup. No public capability, support, performance, or capacity claim changes, and a reached structural cap is reported as reached, never as supported capacity.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2: close the total-fiber ladder`
...
serialized, with none. Its remaining slices are the higher total-operation,
binding, type, and map levels, final qualification, and cleanup, after which
proposed `.17.2.5` through `.17.2.7` and measurement `.17.3` follow.
