...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.3.2.1`
  ...
  Verification: `Activation evidence is complete; implementation is pending. The clean b9463c663 exact matrix publishes nineteen profiles, including operations_total/limit_v1, before the isolated operations_total/over_limit_v1 worker reaches 4,869.5 MiB and the real guard terminates it with exit 137 at the unchanged 4,096-MiB cutoff. Source inspection locates the cause at ExecutionBuilder::_build_operations: it calls _limit('expanded_operations_total', ...) only after _emit_sequence has built and source-mapped every selected operation. The existing exact t/1626 contract requires a separate 6,144-MiB guard for that outcome, while decisions 0055/0056 require product limit enforcement to fail gracefully inside the selected fixed safety envelope. Re-derivation is the real guard diagnostic plus the exact nineteen-file archive census; falsification is successful per-profile isolation together with the same profile-twenty stop and the historical 6,144-MiB test prerequisite, which rejects coordinator retention as the remaining explanation; durability is decision 0078, this leaf, the mdBook/card update, the revision-keyed 19-report/1,739,213-byte archive, and Git. Task-tree integrity passes at three trees/982 nodes; Knowledge Map parity passes at 1,150 facts/6,076 questions/6,243 occurrences/133 shards; all 54 mdBook chapters test; documentation-relative paths pass Files=1/Tests=2; syntax and diff checks pass. No product code or test has changed in activation; bounded claim and containment governance records are refreshed without an enforcement-ceiling increase.`
  Commit: `pending implementation after activation`
