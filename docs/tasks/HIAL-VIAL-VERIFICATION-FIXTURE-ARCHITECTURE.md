...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4`
  ...
  Verification: `Clean predecessor 040ab0e55 seals the execution/checking matrix, and clean child-.1 commit 814e0f37e completes the caller-sealed structural adapter with exact guarded regeneration and cleanup. Decisions 0056/0075 freeze the common record, profile order, outcomes, provider boundary, and nonclaims. Active child .2 now owns one isolated-profile, content-addressed, clean-revision/host/tool/guard-identity publisher derived from the proven execution/checking matrix architecture, followed by a guarded complete capture and separate canonical reload. Provider verification remains explicit, read-only emit evidence—not external-tool, compile/runtime, support, budget, capacity, or reached-boundary evidence.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4: activate structural backend-emission measurement; HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.1: measure structural backend emission`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2`
  ...
  Verification: `Clean child-.1 commit 814e0f37e supplies the complete validated report boundary. Producer owned_shapes is the sole ordered inventory authority: four backend profiles by five levels, with gate/qualification routed to measurement and reference/limit/excess routed to validation. The proven execution/checking publisher is the selected base because it already isolates capture, resume, and immutable validation per profile; bounds child envelopes, errors, and retained coordinator state; binds nested reports to one clean revision/host/tool/enforced-guard identity; publishes atomically on the repository volume; and withholds family/complete seals across any failure or identity drift. This leaf must derive rather than copy the twenty-member partition, measure the largest canonical envelope before freezing its ceiling, preserve OSVVM provider inclusion and exact dominance/nonclaims, and prove hostile process/publication/staging cases before guarded clean-revision capture plus separate reload. No product code, measurement, artifact, or claim changes in activation.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2: activate structural measurement matrix publication`
...
| --- | --- | --- | --- |
| `2026-08-24` | `.17.3.4.2` structural measurement matrix activation | clean child-.1 predecessor; decisions 0056/0075; producer-owned profile order; semantic/bridge and execution/checking publisher audit; isolated worker, envelope, identity, atomic publication, resume/reload, dominance, and nonclaim boundaries; task/index/Memory continuity | `passed activation`; active `.17.3.4.2` derives the exact four-profile/five-level inventory from producer `owned_shapes`, routes only gate/qualification to sampling, and adopts the latest isolated-profile publisher architecture rather than retaining full reports in a coordinator. Implementation must calibrate a closed result ceiling from canonical reports, prove hostile process/publication/staging controls, then commit cleanly before guarded capture and separate reload. Product code/tests/artifacts remain unchanged and no external-tool, runtime, budget, support, capacity, reached-boundary, parity, or public-API claim is admitted. |
...
## Acceptance Checklist (enforced) — `.17.3.4.2` structural measurement matrix activation

- [x] **ROOT CAUSE (WHY + WHERE)** — Clean commit `814e0f37e` closes canonical per-profile measurement, but no owner yet freezes the full producer partition or makes raw reports resumable at one clean revision/host/tool/guard identity. `git log -S'_run_profile_process' --oneline -- perl/FSM/VIAL/ArchitectureScaleExecutionCheckingMeasurementMatrix.pm` identifies the proven isolated-profile repair that prevents a long-lived publisher from accumulating full-report allocator state.
- [x] **ADDRESSED (verified)** — Active leaf `.17.3.4.2` now exclusively owns a producer-derived four-profile/five-level inventory, per-profile child capture/validation, measured result-envelope ceiling, immutable content-addressed publication, same-identity resume, family/complete seals, dominance/provider/nonclaim accounting, hostile process/filesystem evidence, clean-revision guarded capture, and separate reload. The count is rederived from `owned_shapes`, challenged by exact-order/missing/extra watcher controls, and retained in this task and future test/publication evidence.
- [x] **NO REGRESSION** — The tree was clean at `814e0f37e` before activation; product modules, tests, measurement evidence, qualification artifacts, mdBook behavior, claims, guards, and ceilings are unchanged. Task integrity, Memory/index alignment, relative paths, containment, claim inventory, Knowledge Map, and all doctrines must pass before commit. External-tool, compile/run/trace/result, runtime, support, budget, capacity, reached-boundary, parity, partial/mixed matrix, and public-API nonclaims remain exact.
