...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2`
  ...
  Verification: `ArchitectureScaleExecutionGraph now owns binding, topology, fiber, type, map, the complete 8,192-/262,144-/1,000,000-/1,000,001-attempt random ladder, the one-/four-/sixteen-MiB/over-limit plan ladder, the 512-/4,096-/4,097-scenario ladder, and the complete 256-/8,192-/65,536-/65,537-operations-per-scenario ladder. The operation ladder is the first selected pair whose own nominal execution cap is never the authority. The 918,533-byte limit source parses into one complete 65,536-action scenario, and the unchanged public binder returns only serialized_plan_bytes exceeds the limit 16777216 at /plan; one further 14-byte reset record makes the ordinary parser return only scenario exceeds 65536 expanded actions at /packages/0/fixtures/0/scenarios/0. Neither leaves a partial ExecutionIR or plan, the semantic-owned rejection is now evaluated before any canonical bridge construction, and both evaluations record one VIAL_SCALE_LIMIT_INTERACTION discrepancy routed to .17.4. Exact source/SemanticIR/bridge/workload/plan identities, the exact one-record source delta, deterministic reruns, mutation rejection, unowned-level closure, and public capability isolation are frozen. Focused t1620/t1621 pass at 1 file/4 tests each in 89 and 44 seconds; the guarded 25-file/95-test execution/composition impact matrix passes in 960 seconds. The total-operation qualification level is now owned and is the first selected level whose own axis has no reachable nominal operating point: its 920,547-byte source clears every stage before the plan - 32 scenarios of exactly 2,048 expanded actions each, no scenario near the 65,536 expanded-action semantic cap, and the unchanged checked-AHB bridge identity a4565d40 - and only the serialized plan crosses a limit, so the binder returns one serialized_plan_bytes exceeds the limit 16777216 at /plan with no partial ExecutionIR or plan. The evaluation reports expected_rejection plus one VIAL_SCALE_LIMIT_INTERACTION at /requested_counts/operations_total routed to .17.4, and t/1619's ladder boundary advances to the still-unowned total-operation limit_v1. Focused t/1619-t/1622 pass at Files=4, Tests=17 in 229 seconds. Both fiber qualification levels are now owned and reachable: simultaneously_live_fibers at 1,024 keeps every fiber live at once (1,024 total, 1,024 live, 1,041 source maps, 432,528-byte plan) and fibers_total at 8,192 runs 265 sequential all groups of at most 31 children so it reaches 8,192 fibers while its live width stays at the gate value of 32 (8,456 operations, 8,473 source maps, 3,222,659-byte plan); both are far below the 16-MiB cap and report accepted with no discrepancy. Reaching them required the fiber oracle to stop restating the gate's literals: _total_fiber_group_sizes and _live_fiber_nested_counts are now shared by the renderer and the oracle, so every level is checked against the recipe actually used rather than only the level whose numbers were typed. t/1605's ladder boundary advances to the still-unowned fiber limit_v1. Focused t/1604, t/1605, and new t/1623 pass at Files=3, Tests=14 in 27 seconds. Two costs are measured rather than assumed for the next slices: the total-operation limit_v1 of 1,000,000 needs a roughly 14-MB source parsed into a 1,000,000-action SemanticIR only to reach the same plan-cap rejection already proved at 65,536, so it buys no new evidence for a very heavy parse and should be weighed against the axes that are still reachable; and its over_limit_v1 of 1,000,001 is not divisible by the renderer's fixed 32-scenario fanout, so _render_ahb_vial confesses and that level needs a construction decision before it can be owned at all. The live-fiber ladder is now closed and is the first axis on this leaf to reach its own nominal cap: simultaneously_live_fibers limit_v1 is accepted at exactly 16,384 total fibers, 16,384 simultaneously live, 16,384 expanded operations, and 16,401 source maps over a 6,553,464-byte plan, well inside the 16-MiB bound, so nothing pre-empts the 16,384 simultaneous_live_fibers cap declared in FSM::Support::VIALExecutionContract and enforced at FSM::VIAL::ExecutionBuilder line 1018. The boundary is one fiber wide: over_limit_v1 adds exactly one 45-byte nested fiber record (738,151 -> 738,196 bytes), the parser still accepts all 16,385 expanded actions and the checked-AHB bridge is still built, and only the execution stage rejects with one VIAL_EXECUTION_LIMIT_ERROR at /operation_graph/maximum_simultaneous_live_fibers with no partial IR or plan and no VIAL_SCALE_LIMIT_INTERACTION, because no earlier owner intervenes. This closes a claim that had been standing unproved in the live fact card, which asserted this exact limit as reachable before any level owned it. Focused t/1604, t/1605, t/1623, and new t/1624 pass at Files=4, Tests=18 in 69 seconds. The leaf remains active for the total-operation limit/over-limit, total-fiber limit/over-limit, binding/type/map higher levels, final qualification, and cleanup slices; the still-unowned direct-IAL1 route will need the same semantic-first stage order when its 1-MiB source cap becomes authoritative. No public capability/support, performance, or capacity claim changes, and no total-operation capacity above the 1,024-operation gate is claimed as proved.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2: prove the exact live-fiber limit`
...
ladder, and the complete operations-per-scenario ladder. That operation ladder
is the first selected pair whose own nominal cap never wins: the serialized-plan
cap rejects 65,536 operations and the semantic expanded-action cap rejects
65,537, each recorded as a `VIAL_SCALE_LIMIT_INTERACTION` discrepancy routed to
`.17.4`. Its remaining slices are the higher total-operation, fiber, binding,
type, and map levels, final qualification, and cleanup, after which proposed
`.17.2.5` through `.17.2.7` and measurement `.17.3` follow.
