...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4`
  ...
  Verification: `Clean child-.1 commit 814e0f37e seals guarded per-profile reports. Active .2 implements the producer-derived twenty-profile isolated publisher, independently bounded raw/IPC envelopes, immutable same-volume recovery, and common-identity family/complete seals. Default/hostile tests and live documentation pass; clean implementation commit, guarded full capture, and separate reload remain. Provider evidence stays read-only and no external-tool, compile/runtime, support, budget, capacity, or boundary claim is added.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4: activate structural backend-emission measurement; HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.1: measure structural backend emission; HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2: implement structural measurement matrix publication`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2`
  ...
  Verification: `The ordered twenty-member inventory comes only from producer owned_shapes; level and canonical-emission authority route validation versus three/five-sample measurement. Separate close-on-exec children validate and atomically publish complete reports before bounded compact return. A guarded 334,757-byte calibration sets the 524,288-byte file ceiling; IPC/error ceilings are 65,536/4,096. Parent admission regenerates profile/order/sample/provider/dominance/artifact/common-identity invariants; exact staging alone recovers, while process, byte, collision, ambiguity, order, sample, or provider drift rejects. Default t/1662, t/1661, t/1660, and t/1183 pass without residue. Clean implementation commit, guarded capture, and separate reload remain; all execution/support/budget/capacity/API nonclaims stay closed.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2: activate structural measurement matrix publication; HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.4.2: implement structural measurement matrix publication`
...
| --- | --- | --- | --- |
| `2026-08-24` | `.17.3.4.2` structural measurement matrix implementation | producer-derived twenty-profile order and mode routing; isolated child lifecycle; calibrated file/IPC/error ceilings; immutable same-volume atomic publish/recovery; common identity; profile/sample/provider/dominance admission; exception/exit/signal/malformed/noncanonical/oversized/collision/ambiguous-staging negatives; default adapter/publisher/CI regressions; task/book/card/claim/containment continuity | `passed implementation, capture pending`; default t/1662 passes Files=1/Tests=5, t/1661 and t/1660 pass, and t/1183 passes Files=1/Tests=12. The largest canonical calibration report is 334,757 bytes; the enforced publication/IPC/error ceilings are 524,288/65,536/4,096 bytes. Synthetic 64-MiB allocation stays child-owned; exact staging recovery succeeds, ambiguous staging is preserved/rejected, and `.artifacts/tmp/vial-scale` is absent. A clean implementation commit is the mandatory next boundary before guarded full capture and independent reload. No external-tool, compile/run/IASIM/trace/result, support, budget, capacity, reached-boundary, parity, or public-API claim is added. |
