...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2`
  ...
  Verification: `ArchitectureScaleExecutionGraph now owns the binding, topology, fiber, type, and map gates, the complete random, serialized-plan, scenario, operations-per-scenario, live-fiber, total-fiber, and total-operation ladders, the complete execution-type and binding ladders. Each level is frozen by its own focused test - t/1604 through t/1628 - and each slice's exact sources, SemanticIR, bridge, workload and plan identities, measurements, diagnostics, deterministic reruns, mutation rejection, unowned-level closure, public capability isolation, and named regression gates are recorded in this file's per-slice `Acceptance Checklist (enforced)` sections; the mdBook chapter and the `vial-execution-scale-reachability` fact card carry the same numbers for readers. Four axes reach their own nominal cap: simultaneously_live_fibers at 16,384 and scenarios at 4,096 with accepted plans, fibers_total at 65,536 structurally before the 16-MiB serialized-plan cap rejects its plan, and operations_total one operation past 1,000,000. The operations-per-scenario pair and the total-operation qualification and limit levels never reach theirs, and each records one VIAL_SCALE_LIMIT_INTERACTION routed to .17.4. Two constructions are in use because the 1,048,576-byte VIAL source cap forbids one literal record per unit above 74,824 single-scenario resets, 74,656 fanned-out resets, and 22,536 fibers: gate and qualification levels are literal, the two highest fiber and total-operation levels use the ordinary repeat form. Proof method is now also selected per level: decision 0061 clause 8 makes the 1,000,000-operation limit preflight_dominated/not_materialized against its measured 65,536-operation witness rather than spending roughly 32 GiB to reach the same answer, while 1,000,001 is proved by a real run behind FSMGEN_VIAL_SCALE_EXACT at a raised 6,144-MiB descendant cutoff. The owned frontier is one published %OWNED_LEVELS declaration - 37 of the catalog's 65 shapes - that construct gates on and every sibling test derives its still-unowned boundary from, so owning a level no longer staleness-breaks its siblings. Stage order is now uniform: decision 0061 clause 4 evaluates VIAL source and SemanticIR before the canonical bridge on the direct-IAL1 route too, which is what makes the 4,096-declaration parser cap - not a bridge cap - the authority for 8,192 types, over a route whose own measured boundary is 1,043 types at the 16-MiB serialized-manifest cap. Decision 0072 now governs five levels that cannot be constructed at all: binding qualification/limit/over generate 1,933,429/3,866,741/3,866,800 direct-IAL1 bytes, while execution-type limit/over generate 2,413,815/2,413,852 HIAL bytes and 8,246,830/8,246,956 VIAL bytes against the 1,114,112-byte envelope. All five report envelope_unconstructible/not_constructed with the constructor's exact diagnostic, no retained source, no stage identity, a refused build, and both required records - the fixture-bound interaction plus the measured 2,054/2,055 binding or 1,043/1,044 type route boundary - routed to .17.4. Every published route boundary is now a whole-route claim carried through the semantic stage, the canonical bridge, and the public binder to a real accepted plan - 2,054 bindings at 2,664,611 bytes, 1,043 execution types at 1,493,527, and 46,294 source-map records at 16,777,026 - after a signoff review found two backed by bridge-stage evidence only and the third by no test at all. The leaf remains active for Knowledge Map card partitioning, the source-map levels, final qualification, and cleanup. No public capability, support, performance, or capacity claim changes, and a reached structural cap is reported as reached, never as supported capacity.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.2.4.2: prove every published route boundary end to end`
  Blocked by: `none - decision 0072 unblocks this leaf; it now implements the selected envelope_unconstructible treatment. Historical blocker: every level this leaf has left is a bindings, execution_types, or source_map_records level whose decision-0061 authority measurement contradicts: each is decided by the workload's own 1,114,112-byte construction envelope before any product cap, and events, types, and endpoint declarations have no compact repeat form to reach the product cap the way fibers and operations did. Authoring them now would freeze a fixture bound as a product limit. Unblocks when .17.2.4.3 selects the treatment; run .17.2.4.3 instead.`
...
boundary. Partitioning the reachability fact card, the source-map levels, final
qualification, and cleanup follow, after which proposed `.17.2.5` through
`.17.2.7` and measurement `.17.3` come next.
...
- The `vial-execution-scale-reachability` fact card is at 78.5% of its
  32,768-byte per-file bound after four ladder slices, each of which adds one
  paragraph. Compressing narration to signposts bought headroom twice, and that
  will not hold for the execution-type and source-map ladders still to come.
  `TOOLBOX.md` is explicit that a card at its own per-file bound is partitioned,
  not trimmed, so partition it at a stable topic boundary — the natural seam is
  routes and gates versus per-axis ladder outcomes — before the source-map
  ladder lands, and prove exact sources, stable partitions, answer-set equality,
  boundedness, and current routing with `scripts/check_knowledge_card_history.pl`.
