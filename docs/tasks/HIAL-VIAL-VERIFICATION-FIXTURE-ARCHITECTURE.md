...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.5.3`
  ...
  Verification: `Clean materialization commit bd5c0e628 closes .2. FSM::VIAL::Backend::VerilatorLifecycle now exclusively owns decision 0083's private admitted/prepared/tool_verified/compiled/ran/trace_validated/result_produced/assembled/cleaned chain for both public Runner and measurement callers. Closed content-addressed state/object records, exact mode/digest/census/predecessor validation, caller-sealed repository-volume roots, prepared-input and executable revalidation, actual plus workspace-normalized command identities, close-on-exec control-before-output supervision, inclusive capture accounting, integer monotonic timing, deadline-after-pipe-close enforcement, whole-process-group termination, and exact cleanup reject mutation, skips, replay, collision, symlink, partial state, overflow, signal, timeout, and launch failure without retry or widened authority. Runner is a thin total-function result adapter; lifecycle exceptions or malformed private projections become sanitized host errors with no artifact or untrusted cleanup claim. Guarded exact t1664 traverses every real Verilator state and returns the exact 12-artifact graph; independent t1559 AHB runtime parity passes. The broader t1558 public watcher passes six of seven top-level subtests, but its byte-identical repeated direct-drive run alone reproduces the pre-main 30-second stall after the first direct-drive run passed; active child .3.1 owns quiet-host/platform falsification. This node remains active until that child is dispositioned. No retry, security bypass, signing change, timeout widening, support, performance, capacity, reached-boundary, or public API widening is selected.`
  ...
  Children: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.5.3.1`
...
- ID: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.5.3.1`
  ...
  Verification: `Clean lifecycle implementation commit 037c962fd provides the production state/timing/cleanup authority and leaves this already tracked child as the exact unresolved platform prerequisite. Three guarded fresh binaries completed process-group creation and exec handoff in 3.7-4.1 ms, then produced zero bytes for the full 30-second deadline and were terminated/cleaned exactly. A read-only 3-second macOS sample of one live 1,847,672-byte ARM64 executable collected 2,389/2,389 main-thread samples at _dyld_start with only a 96-KiB footprint and no binary-image description, proving the observed interval preceded generated FSMGEN main/output. Concurrent census showed an unrelated repository producing many fresh Rust executables/dylibs while syspolicyd consumed 28.6% CPU; its unified log separately recorded a 30.179-second outbound policy timeout. That correlation is not yet a causal or general macOS claim. The exact 995-byte sample has SHA-256 54f039b4ffa59c9af9ba3cec1b53d10a0605d34c798e12cad1ebbfa4f2fb2ad2; its off-volume sampler output was copied byte-for-byte to repository-local staging, verified, consumed, and deleted with an absence census. This continuity-only activation authorizes exact host/OS/tool/metadata and quiet-host/control qualification; it changes no product, test, tool, security, signing, retry, deadline, support, or success classification.`
  Commit: `HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.17.3.5.3.1: activate macOS pre-main qualification`
