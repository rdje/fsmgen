# AXI IAL2 Manager Mixed Dynamic/Static Read-Data Runtime-Validation Behavior

Date: 2026-06-23
Owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.289`
...
This sample is the runtime sibling of the `.287` report-only sample. It keeps
the same one-dynamic plus one-concrete-static transaction set, generated
mixed `RID && RLAST` response-demux, scalar last-beat `RDATA`/`RRESP`
...
The dynamic transaction matches the captured dynamic `RID`; the static
...
VHDL remain later exact owners.

The `.287` report-only sample remains support-accounted and continues to emit
