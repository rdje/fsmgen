...
Completed contract `.2` selects implementation `.3` for unique local caller
target-local priority and deterministic ambiguous ownership rejection. Clean
contract commit `b44afcc51` activates `.3` continuity-only. SystemVerilog and
native Verilog are executable gates; the separately owned direct-VHDL unary-
reduction defect does not alter AHB behavior.
