...
Decision `0084` closes the host follow-up without changing the backend. On the
...
- Claim: On the recorded macOS 26.5.2/M4 Pro/APFS host, guarded qualification produced and successfully ran a 1,847,672-byte ARM64 executable plus byte-identical-path and fresh-link controls, and the unchanged public API/CLI integration watcher passed all seven top-level subtests in 167 seconds under a recorded no-compiler condition.
- Re-derive: Run the guarded qualification under an exact declared host condition to reconstruct the public VIAL lifecycle and inspect its host, binary, timing, control, and cleanup record independently; run the unchanged public integration watcher under the recorded zero-compiler condition.
- Falsify: Compare concurrent-link, quiet no-compiler, byte-identical different-path, fresh minimal C++, platform true, exact policy-log-window, and repeated public-integration controls; keep every timeout failed and require the static validator to reject altered counts, hashes, controls, or boundaries.
- Durability: Retain the bounded evidence projection, guarded producer, default static validator, decision 0084, book, task-tree, Knowledge Map card, claim registry, and doctrine gate in one Git history chain after consuming and exactly removing raw host records.
...
The durable response is a host-qualification diagnostic, not an execution
fallback. `t/1665-vial-macos-premain-qualification.t` runs only with an explicit
Darwin guard and a process-census label. It executes the primary exactly once,
keeps a timeout failed, then uses separate byte-identical-path, fresh-link, and
platform controls to test narrower hypotheses. If the primary stays alive past
two seconds it takes a read-only stack sample. Its canonical JSON is new-file-
only beneath `.artifacts/qualification/vial-macos-premain/`; every ancestor must
be a non-symlink directory on the repository device, and no absolute host path
is published. The normal regression validates the bounded retained evidence
without running the host probe. No retry, larger timeout, xattr/signature
change, security bypass, unrelated-process termination, support promise, or
public API change follows from this qualification.
