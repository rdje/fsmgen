...
and one-bit sized bitstring generic maps in the same APB/C4 shape.
...
integer, scalar expression, and one-bit sized bitstring actuals, aggregate VHDL record/array lowering,
VHDL package
...
VHDL `std_logic` actuals such as `ENABLE_DEFAULT => '1'`.
...
`ENABLE_DEFAULT => '1'`, before the requester/completer port maps. Other
...
`BACKEND-API-VALIDATION-FRONTIER.93` selects the next exact backend/API edge
after bounded APB/C4 generated-FSM one-bit VHDL generic-map actuals shipped.
...
APB/C4 generic maps beyond scalar integer, scalar expression, and one-bit
sized bitstring actuals,
