...
integer, `std_logic`, or `std_logic_vector` generic declaration. Other `?top`
VHDL shapes still parse into typed composition IR and then fail closed with the
scoped target-support diagnostic.
...
`RESET_VALUE => "10100101"`, bounded C2
...
bitstring generic maps such as `RESET_VALUE => "10100101"` before that port map.
The bounded C2
...
`BACKEND-API-VALIDATION-FRONTIER.88` selects the next exact backend/API edge
after bounded C1 standalone-DT multi-bit sized-bitstring VHDL generic-map
...
multi-bit sized bitstring actuals, APB/C4 generic maps,
