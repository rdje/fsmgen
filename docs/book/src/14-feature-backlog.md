...
`BACKEND-API-VALIDATION-FRONTIER.115.1` owns direct VHDL signed vector
negative numeric-literal multiplication lowering after selector leaf
`BACKEND-API-VALIDATION-FRONTIER.115` chose it from adjacent fail-closed
arithmetic probes.
...
numeric-literal division/modulo remain deferred; signed negative multiplication
is selected by `.115.1` but not yet shipped. Completed implementation
