...
plus the exact four-spawn `await_all` shape shown above for `while`. The
body-first `until` four-spawn twin, fan-outs beyond four, generated `do`, and
multi-pending post-`do` `await_any` stay fail-closed for now.
