...
The loop-contained `await_all` form also supports exactly two pending
generated spawns across that local blocking `do`. The local child must finish
first, and the later `await_all` drains both generated children before the
repeat check and the surrounding loop re-test. The `while` form is:

```lisp
(actor while_repeat_multi_spawn_do_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start) (input cond) (input loops (width 3))
  ...
  (transaction parent
    ...
    (while cond
      (repeat loops
        ...
        (await_all done)))   ;; drains w0_done and w1_done before re-entry
    (complete done))
  (transaction worker
    (complete done))
  (transaction helper
...
The body-first `until` form uses the same two-spawn drain contract:

```lisp
(actor until_repeat_multi_spawn_do_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start) (input cond) (input loops (width 3))
...
The `while` form also supports exactly three pending generated spawns across
the local blocking `do`:

```lisp
(actor while_repeat_three_spawn_do_drain
  (clock clk)
  (reset rst_n)
  (interface
    (input start) (input cond) (input loops (width 3))
  ...
  (transaction parent
    ...
    (while cond
      (repeat loops
        ...
        (spawn worker as w2)
        (spawn worker as w3)
        (do helper)          ;; waits for helper_done; w0/w1/w2/w3 remain pending
        (await_all done)))   ;; drains all four before re-entry
    (complete done))
  (transaction worker
    (complete done))
  (transaction helper
...
form also supports the same contract for exactly three pending generated
spawns:
...
```lisp
(actor while_repeat_three_spawn_do_await_any_then_all
  (clock clk)
  (reset rst_n)
  (interface
    (input start) (input cond) (input loops (width 3))
  ...
  (transaction parent
    ...
    (while cond
      (repeat loops
        ...
        (spawn worker as w2)
        (do helper)          ;; waits for helper_done; w0/w1/w2 remain pending
        (await_any done)     ;; observes any generated child done pulse
        (await_all done)))   ;; drains all three before repeat_check / re-entry
    (complete done))
  (transaction worker
    (complete done))
  (transaction helper
...
`until`, plus the exact `while` two- and three-spawn post-`do` `await_any`
plus later `await_all` shapes and the exact body-first `until` two-spawn
post-`do` `await_any` plus later `await_all` shape shown above. Body-first
`until` three-spawn, generated `do`, and four-or-wider post-`do`
multi-pending `await_any` shapes stay fail-closed for now.
