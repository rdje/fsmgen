...
- composition requests with no child instances, malformed child payloads,
  duplicate child names, unsupported child kinds, missing external generated
  child sources, wrong-kind external generated-child roots such as `?fsmc`
  resolving to `?dt:name` or `?dtc` resolving to `?fsm:name`, or external RTL
  metadata that is missing, non-flat, empty, duplicated, wrongly typed, or
  system-output directed
