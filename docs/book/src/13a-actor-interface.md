...
| `(reset rst_n)` | `(sreset rst_n)` (or `areset` per convention) |
...
**Counter width** is computed from the limit: `ceil(log2(N))`.
...
| `{drive}_start` | 1 | Asserted by drive calls to fire combinational DT |
| `{drive}_{param}` | 1 | Per-parameter signal, wired to actual value |
| `{transaction}_cnt` | 8 | Repeat counter |
| `{transaction}_wd` | log2(N) | Watchdog counter |
| `{transaction}_cc` | log2(M) | Latency cycle counter |
