...
division or modulo by zero is rejected before emission. Direct `.fsm` runtime
division and modulo expressions also reject numeric and exact-width
literal-zero divisors before HDL emission. Runtime RHS expressions with
nonzero literal divisors or dynamic signal divisors are still emitted as
AST/HDL expressions; the tool does not yet prove that every dynamic divisor is
nonzero. That analysis is tracked in
