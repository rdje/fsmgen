...
| Transaction control flow | shipped bounded surface | Body-bearing `(when COND body...)`, `(switch SELECTOR branches...)`, `(while COND body...)`, `(until COND body...)`, and `(repeat COUNT body...)`. | Control flow lowers to explicit decision states, branch states, loop counters, and exits. The shipped repeat-body clause surface is limited to documented drive, await, sample, update, set, shift, assemble, extract, store/load, wait clauses, top-level repeat-body local blocking do, top-level repeat-body generated blocking do with static params, and top-level repeat-body spawn with optional static params, optional bind handoffs, optional same-domain domain metadata, samples before same-body sync, same-body await_all, and single-pending same-body await_any. |
...
| Blocking child activation | shipped bounded surface | `(do child)` locally, including the documented top-level repeat-body local do subset, `(do child (params ...))` as the documented top-level repeat-body generated blocking do static-parameter subset, and `(do child (params ...) (bind ...))` through generated-child activation at top level. | Plain local `do` asserts child start and awaits a fresh child done pulse. Repeat-body local `do` reaches the repeat check only after that done pulse. Repeat-body generated blocking do with static params emits one generated instance named `{parent}_{child}_repeat_do_{ordinal}` and generated-top parameter binding for that lexical do site. Top-level parameterized/bound `do` keeps the broader generated child handoff surface outside repeat bodies. |
| Spawned generated children | shipped bounded surface | `(spawn child as instance [(params ...)] [(bind ...)] [(domain NAME)])`, `(await_all done)`, `(await_any done)`, and the documented top-level repeat-body spawn with optional static params, optional bind handoffs, optional same-domain domain metadata, sample-after-spawn before same-body sync, same-body await_all, and single-pending same-body await_any subset. | Lowering emits parent, child, and generated top scheduled `.fsm` artifacts. Instance start/done handoffs, named-drive handoffs, parameter overrides, port-binding handoffs, same-domain ownership metadata, and schedule-report generated-composition/clock-domain metadata are bounded public review surfaces. Repeat-body spawn reuses one static generated child instance across iterations after await_all or exactly-one-pending await_any observes fresh done; samples after spawn materialize before that sync state. |
...
Nested child activation, repeat-body do bindings, domain-qualified
repeat-body `do`, sample-before/after-do timing, spawn-after-sample ordering,
...
- Repeat-body do bindings, domain-qualified repeat-body `do`, multi-pending `await_any`,
  spawn-after-sample ordering, nested `while`, nested `until`, `stage`, `contract`, and cross-domain activation inside repeat bodies remain outside
