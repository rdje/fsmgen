...
- A cross-domain `(do CHILD)` directly inside any TOP-LEVEL body — a `(repeat ...)`
  ...
  branch bodies, IS shipped (the dual-CDC handshake runs when the branch is taken
  and re-runs each iteration inside a repeat/loop; see the cross-domain activation
  crossing row).
