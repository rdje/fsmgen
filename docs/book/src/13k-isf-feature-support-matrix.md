...
| Await and latency | shipped | `(await PORT)`, actor watchdogs, and transaction `(latency (min N) (max N))` where each latency bound is a positive decimal literal or declared actor constant that resolves to a positive integer. | Awaits lower to wait/test states and watchdog counters when configured. Latency counters and schedule-report storage metadata are emitted for the supported shapes. Actor parameters, runtime signals, arbitrary expressions, unknown symbols, and zero-valued constants fail closed as latency bounds. |
