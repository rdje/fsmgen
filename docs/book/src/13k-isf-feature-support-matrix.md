...
| Enum member values | shipped bounded surface | Local and package enum members in constants, scalar and aggregate/list parameter leaves, activation overrides, reusable-library use-site overrides, generated child transaction parameter defaults, transaction/rule/drive scalar values, scalar expression operands, switch values/selectors, and standalone conditions/guards. Declared actor constants and earlier scalar actor parameters may also back actor parameter scalar defaults, actor parameter aggregate/list leaves, and generated child transaction parameter defaults. | Authored enum, actor constant, or earlier actor-parameter tokens are preserved where scheduled `.fsm` can carry them; generated child transaction defaults literalize actor-static leaves before child/report publication while preserving enum tokens, activation and reusable-library static specialization contexts resolve to literals before generated-top emission, and actor-constant-backed and actor-parameter-backed actor parameter defaults resolve internally for scalar parameter consumers. Enum targets and operator-position enum members fail closed. |
