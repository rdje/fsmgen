...
Generated-child actor-to-actor route support is intentionally small. Each
route uses one scalar source child output, one scalar sink child input,
deterministic generated handoffs, and one named drive-call cycle. Multiple
routes are accepted only when they share the same source child, sink child,
parent transaction, and contiguous route segment. Generated-handoff remapping
or reuse, fan-in/fan-out data routing, route mux/storage, CDC/reset remapping,
ready/backpressure, payload protocols,
