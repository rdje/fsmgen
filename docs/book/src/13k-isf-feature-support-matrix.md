...
| Reusable procedures (inline) | shipped bounded surface | `(proc NAME (params (P (width N))...) BODY...)` defines a reusable parameterized block; `(call NAME actuals...)` calls it INLINE — one positional actual per value (in) parameter. | The inline call macro-expands the procedure body at the call site with each parameter replaced by its actual (a signal or a whole expression); the emitted `.fsm` is identical to the hand-written substituted clauses (no instance, no handshake). Calls work in any clause position, including `when`/`switch`/`while`/`until` bodies. Recursion (direct or transitive), unknown procedure, argument-count mismatch, and malformed `(proc ...)` fail closed. Out-parameters (`(out NAME ...)`) and the handshake call form (`(call ... as INST)`) are staged and currently fail closed with a targeted message. |
