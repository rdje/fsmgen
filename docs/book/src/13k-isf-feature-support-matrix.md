...
| Actor-owned banks | shipped bounded surface | `(storage (bank NAME (width N\|PARAM) (depth N)))` with pointer-selected `(store BANK IDX VALUE)` and `(load BANK IDX as TARGET)`, where `PARAM` is an actor-local scalar parameter default that resolves to a positive integer. | Banks lower to deterministic scalarized entries such as `data_0`, `data_1`, and so on, with resolved positive integer element widths. Same-cycle read/write behavior follows the documented scalarized FIFO path. Bank depths remain literal-only. |
...
use-site parameter-driven FIFO interface shape, bank depth, generated-top
