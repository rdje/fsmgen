...
generated ATL top, using the same spelling in
`isf/atl_resolved_child_pin_egress_vector_pipeline.isf`; one one-bit
...
Pin-egress route width is scalar one-bit for the multi-route subset or one
exact matching child-output/top-output vector width for the vector subset;
vector pin-egress multi-route sets remain deferred.
