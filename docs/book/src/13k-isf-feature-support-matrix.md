...
| Actor-owned banks | shipped bounded surface | `(storage (bank NAME (width N\|PARAM\|CONST\|PACKAGE.CONSTANT) (depth N\|PARAM\|CONST)))` with pointer-selected `(store BANK IDX VALUE)` and `(load BANK IDX as TARGET)`, where `PARAM` is an actor-local scalar parameter default, `CONST` is a declared actor constant, and `PACKAGE.CONSTANT` is a qualified imported package scalar constant that resolves to a positive integer width. | Banks lower to deterministic scalarized entries such as `data_0`, `data_1`, and so on, with resolved positive integer element widths and depths. Same-cycle read/write behavior follows the documented scalarized FIFO path. Unknown/unqualified/aggregate/path package constants, ambiguous local-enum/package-constant tokens, runtime signals, zero/non-scalar params, zero-valued constants, arbitrary expressions, package-constant-backed bank depths, dynamic depths, and duplicate scalarized signal names fail closed. |
