...
| Transaction control flow | shipped bounded surface | Body-bearing `(when COND body...)`, `(switch SELECTOR branches...)`, `(while COND body...)`, `(until COND body...)`, and `(repeat COUNT body...)`. | Control flow lowers to explicit decision states, branch states, loop counters, and exits. The shipped repeat-body clause surface is limited to documented drive, await, sample, update, set, shift, assemble, extract, store/load, wait clauses, top-level repeat-body local blocking do, and top-level repeat-body spawn with optional static params, optional bind handoffs, optional same-domain domain metadata, samples before same-body sync, same-body await_all, and single-pending same-body await_any. |
...
| Blocking child activation | shipped bounded surface | `(do child)` locally, including the documented top-level repeat-body local do subset, and `(do child (params ...) (bind ...))` through generated-child activation at top level. | Plain local `do` asserts child start and awaits a fresh child done pulse. Repeat-body local `do` reaches the repeat check only after that done pulse. Parameterized/bound `do` emits a generated child instance and generated top handoff wiring outside repeat bodies. |
...
Nested child activation, generated/parameterized/bound/domain-qualified
...
- Generated, parameterized, bound, or domain-qualified repeat-body `do`, multi-pending `await_any`,
