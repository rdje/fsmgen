...
| Static actor-network metadata | shipped bounded surface | One static actor instance through the direct actor-body `(instance NAME of ACTOR_TYPE)` clause. `(network ...)` is not part of the shipped surface. | The parser shell and schedule report preserve `actor_network.kind`, instance `name`, `actor_type`, and declaration spelling (`actor`). This slice does not resolve actor types, instantiate children, emit generated ATL tops, move data between actors, or change HDL. Multiple instances, groups, actor events, qualified triggers, and endpoint-aware drive movement remain backlog. |
| ATL v0 future contract | selected backlog contract with bounded shipped event wait | Future leaves reserve direct actor-body groups, qualified endpoints `pins.name`/`actor.port`/`actor.transaction`/`actor.event`/`group.name`, existing drive-body `(sink source)` movement, `(do actor.transaction)`, `(spawn actor.transaction as NAME)`, `(trigger actor.transaction)`, and `(await actor.event)`. `connect`, `transfer`, and `move` are not ATL v0 movement syntax. | Exactly one top-level transaction-body `(await actor.event)` may target the current single static actor instance; it lowers to a one-bit parent handoff input named `actor_event` and reports through `actor_network.event_waits[]`. The next selected trigger subset is one top-level transaction-body `(trigger actor.transaction)` lowered to a one-cycle parent output handoff named `actor_transaction_start`. Rule-level triggers, multiple/nested waits or triggers, fan-in/fan-out, payloads, cross-clock events/triggers, and groups remain fail-closed. No generated ATL child `.fsm`, ATL top, route mux, concurrent-group schedule, or HDL behavior is promised until a later implementation leaf advertises it. |
