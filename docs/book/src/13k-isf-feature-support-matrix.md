...
| Resources | shipped bounded surface | `(resources (resource NAME (kind rule_slot\|output_bundle) (arbiter priority) [(members OUTPUT...)] (users RULE...)))`. Parser-recognized backlog kinds are documented. | `rule_slot` and `output_bundle` plus `priority` arbitration gate the whole bound rule DT for one cycle. `output_bundle` may name explicit declared actor output members; member lists validate against bound rule output writes and report through `resource_arbitration[].members`. Backlog resource kinds, unsupported arbiter/kind combinations, non-output members, output-target users, and route mux/storage fail closed or remain deferred. |
