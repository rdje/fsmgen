...
| Resources | shipped bounded surface | `(resources (resource NAME (kind rule_slot\|output_bundle) (arbiter priority) (users RULE...)))`. Parser-recognized backlog kinds are documented. | `rule_slot` and `output_bundle` plus `priority` arbitration gate the whole bound rule DT for one cycle. `output_bundle` currently represents the named bundle through its bound rule users and their driven outputs/LHS targets; explicit bundle member-list syntax remains backlog. Backlog resource kinds or unsupported arbiter/kind combinations fail closed. |
