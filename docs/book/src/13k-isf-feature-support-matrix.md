...
| ATL v0 future contract | selected backlog contract with shipped fail-closed diagnostics | Future leaves reserve direct actor-body groups, qualified endpoints `pins.name`/`actor.port`/`actor.transaction`/`actor.event`/`group.name`, existing drive-body `(sink source)` movement, `(do actor.transaction)`, `(spawn actor.transaction as NAME)`, `(trigger actor.transaction)`, and `(await actor.event)`. `connect`, `transfer`, and `move` are not ATL v0 movement syntax. | Qualified `(await actor.event)`, transaction-body `(trigger actor.transaction)`, and rule-level `(trigger actor.transaction)` now fail closed with ATL-specific diagnostics before scheduled emission when the qualifier names a declared static actor instance. The next selected event subset is one top-level transaction-body `(await actor.event)` lowered to a one-bit parent handoff input named `actor_event`. No generated ATL child `.fsm`, ATL top, route mux, event payload, concurrent-group schedule, or HDL behavior is promised until a later implementation leaf advertises it. |
