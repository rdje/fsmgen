...
| ATL v0 future contract | selected backlog contract | Future leaves reserve direct actor-body groups, qualified endpoints `pins.name`/`actor.port`/`actor.transaction`/`actor.event`/`group.name`, existing drive-body `(sink source)` movement, `(do actor.transaction)`, `(spawn actor.transaction as NAME)`, `(trigger actor.transaction)`, and `(await actor.event)`. `connect`, `transfer`, and `move` are not ATL v0 movement syntax. | Not shipped as behavior yet. The first selected event/trigger boundary is targeted fail-closed diagnostics for reserved qualified forms before scheduled emission. No generated ATL child `.fsm`, ATL top, route mux, handoff storage, event payload, concurrent-group schedule, or HDL behavior is promised until a later implementation leaf advertises it. |
