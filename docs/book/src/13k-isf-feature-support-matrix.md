...
| Data manipulation | shipped bounded surface | `(shift_left REG BIT)`, `(shift_right REG BIT [(width N)])`, `(assemble PART... as TARGET)`, and `(extract WORD as FIELD... [(widths N...)])`. | Width evidence comes from declarations, samples, operation-local options, and structural derivation. `assemble` can infer exactly one missing part width from a known target and known siblings; `extract` can infer exactly one missing destination field width from a known source and known siblings. Accepted forms avoid placeholder widths; ambiguous, non-positive, or contradictory widths fail closed. |
