...
| Enum member values | shipped bounded surface | Local and package enum members in constants, scalar and aggregate/list parameter leaves, activation overrides, reusable-library use-site overrides, transaction/rule/drive scalar values, scalar expression operands, switch values/selectors, and standalone conditions/guards. Declared actor constants and earlier scalar actor parameters may also back actor parameter scalar defaults and aggregate/list leaves. | Authored enum, actor constant, or earlier actor-parameter tokens are preserved where scheduled `.fsm` can carry them; activation and reusable-library static specialization contexts resolve to literals before generated-top emission, while actor-constant-backed and actor-parameter-backed actor parameter defaults resolve internally for scalar parameter consumers. Enum targets and operator-position enum members fail closed. |
