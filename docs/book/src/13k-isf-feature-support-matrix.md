...
| Transaction control flow | shipped bounded surface | Body-bearing `(when COND body...)`, `(switch SELECTOR branches...)`, `(while COND body...)`, `(until COND body...)`, and `(repeat COUNT body...)`. | Control flow lowers to explicit decision states, branch states, loop counters, and exits. The shipped repeat-body clause surface is limited to documented drive, await, sample, update, set, shift, assemble, extract, store/load, wait clauses, top-level repeat-body local blocking do, top-level repeat-body generated blocking do with static params, bind handoffs, and same-domain domain metadata, and top-level repeat-body spawn with optional static params, optional bind handoffs, optional same-domain domain metadata, samples before same-body sync, same-body await_all, and single-pending same-body await_any. |
