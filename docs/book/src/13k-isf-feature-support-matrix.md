...
| Static actor-network metadata | shipped bounded surface | One static actor instance through either `(network (instance NAME of ACTOR_TYPE))` or the flat `(instance NAME of ACTOR_TYPE)` actor clause. | The parser shell and schedule report preserve `actor_network.kind`, instance `name`, `actor_type`, and declaration spelling. This slice does not resolve actor types, instantiate children, emit generated ATL tops, move data between actors, or change HDL. Multiple instances, groups, actor events, qualified triggers, and endpoint-aware drive movement remain backlog. |
