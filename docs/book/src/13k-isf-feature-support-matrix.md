...
side as `signal`, `literal`, or `expression`.
...
- Rule-trigger output bindings, snapshot-vs-live binding timing selection,
  additional binding reports beyond endpoint-kind metadata, and broader static
  binding conflict diagnostics are not shipped.
