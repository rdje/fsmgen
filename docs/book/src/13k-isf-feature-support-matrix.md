...
| Resources | shipped bounded surface | `(resources (resource NAME (kind rule_slot\|output_bundle\|transaction_start) (arbiter priority) [(members OUTPUT_OR_STORAGE_SIGNAL...)] (users RULE...)))`. For `transaction_start`, `NAME` is the target local transaction. Parser-recognized backlog kinds are documented. | `rule_slot`, `output_bundle`, and `transaction_start` plus `priority` arbitration gate the whole bound rule DT for one cycle. Unmembered `output_bundle` resources keep the implicit bound-rule output/LHS-target surface; explicit members are declared actor outputs or concrete actor-owned storage signals, validate against bound rule writes in those domains, and report through `resource_arbitration[].members`. `transaction_start` suppresses lower-priority rule triggers before the generated trigger fan-in DT. Backlog resource kinds, unsupported arbiter/kind combinations, generated-child transaction starts, actor-network trigger resources, bank-root/aggregate/inferred explicit members, output-target users, and route mux/storage fail closed or remain deferred. |
...
The shipped `rule_slot`/`priority`, `output_bundle`/`priority`, and
`transaction_start`/`priority` subsets can gate a whole bound rule DT for one
active cycle. Remaining resource kinds are documented names, not supported
