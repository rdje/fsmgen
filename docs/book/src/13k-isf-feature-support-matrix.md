...
evidence. Generated-child data movement between the two children, event
fan-in/fan-out, route mux/storage, CDC/reset remapping, ready/backpressure,
and payload protocols remain deferred.
The next selected ATL implementation leaf is one scalar generated-child
actor-to-actor route through that top: `(writer.payload reader.payload)`
after `reader.done` and before `writer.emit`. This is selected backlog until
the implementation lands; current behavior still fails closed for the
generated-child actor-to-actor route shape.
