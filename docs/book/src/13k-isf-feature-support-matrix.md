...
| Data manipulation | shipped bounded surface | `(shift_left REG BIT)`, `(shift_right REG BIT [(width N)])`, `(assemble PART... as TARGET)`, and `(extract WORD as FIELD... [(widths N...)])`. | Width evidence comes from declarations, samples, operation-local options, and structural derivation. `extract` can infer exactly one missing destination field width from a known source and known siblings. Accepted forms avoid placeholder widths; ambiguous, non-positive, or contradictory widths fail closed. |
...
Runtime expression divisor safety is fail-closed for literal zero:
...
- Dynamic division/modulo nonzero proof is not shipped. Literal-zero divisors
  are rejected, but arbitrary runtime scalar divisors are emitted unchanged.
