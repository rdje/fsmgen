...
| Immediate assertions | shipped bounded surface | Transaction-level `(assert COND [message])` — a combinational invariant that must hold every cycle; `COND` is any boolean expression over actor signals, with an optional trailing failure-message string. | A pure ISF construct that rides the ISF → `.fsm` → SV path as a thin `+assert` carrier: the ISF lowerer emits `(+assert (NAME COND ["msg"]) …)`, FSMGenFull parses it onto the module, `GeneratedModuleInfoBuilder` surfaces it into `module_info`, and `GeneratedModuleEmitter` emits `assert (COND) else $error("message")` inside `` `ifndef SYNTHESIS `` / `always_comb` (a name-based default message when none is given). Signals referenced only by an assert are kept alive as ports (classified `INPUT` by their assert reference, not pruned). Verilog (non-SV) output stays assertion-free; `yosys` skips the guarded block. A missing condition or extra operands fail closed. The immediate / combinational sibling of the temporal `(contract …)`. |
