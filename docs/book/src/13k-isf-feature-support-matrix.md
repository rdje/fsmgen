...
`rule_slot`/`round_robin`, and bounded `transaction_start`/`round_robin`
subsets can gate a whole bound rule DT for one active cycle. Remaining
resource kinds are documented names, not supported runtime arbitration
behavior yet.
