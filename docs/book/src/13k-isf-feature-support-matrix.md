...
data-route/generated-child coupling excludes the one shipped exception. The
current exception is exactly one scalar top-level input pin routed to one
resolved child input through the generated ATL top, using
`(worker.payload pins.payload)` in
`isf/atl_resolved_child_pin_ingress_pipeline.isf`. Route evidence remains in
...
CDC/reset remapping, ready/backpressure, and payload protocols remain backlog.
