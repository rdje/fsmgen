...
| Resources | shipped bounded surface | `(resources (resource NAME (kind rule_slot\|output_bundle) (arbiter priority) [(members OUTPUT_OR_STORAGE_SIGNAL...)] (users RULE...)))`. Parser-recognized backlog kinds are documented. | `rule_slot` and `output_bundle` plus `priority` arbitration gate the whole bound rule DT for one cycle. Unmembered `output_bundle` resources keep the implicit bound-rule output/LHS-target surface; explicit members are declared actor outputs or concrete actor-owned storage signals, validate against bound rule writes in those domains, and report through `resource_arbitration[].members`. Backlog resource kinds, unsupported arbiter/kind combinations, bank-root/aggregate/inferred explicit members, output-target users, and route mux/storage fail closed or remain deferred. |
