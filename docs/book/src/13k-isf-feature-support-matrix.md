...
| Await and latency | shipped | `(await PORT)`, await-local `(watchdog N)` overrides using positive decimal literals or declared positive actor constants, actor watchdogs, and transaction `(latency (min N) (max N))` where each latency bound is a positive decimal literal or declared actor constant that resolves to a positive integer. | Awaits lower to wait/test states and watchdog counters when configured. One transaction has one watchdog counter, so distinct per-await limits in one transaction fail closed. Latency counters and schedule-report storage metadata are emitted for the supported shapes. Actor parameters, runtime signals, arbitrary expressions, unknown symbols, and zero-valued constants fail closed as watchdog or latency bounds. |
...
| Temporal contracts | shipped bounded surface | Top-level `(contract NAME (eventually SIGNAL within CYCLES))`; nested `(within CYCLES)` remains an alias. `CYCLES` may be a positive integer literal or a declared actor constant that resolves to a positive integer. | Lowering emits monitor state/storage in scheduled `.fsm`; reports expose the resolved integer in `temporal_contracts[].within_cycles`; SystemVerilog projects the sticky fail bit into a verification-only assertion under `` `ifndef SYNTHESIS``. Verilog output remains assertion-free. |
