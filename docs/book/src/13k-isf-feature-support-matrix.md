...
| ATL v0 future contract | selected backlog contract with bounded shipped event and trigger handoffs | Future leaves reserve direct actor-body groups, qualified endpoints `pins.name`/`actor.port`/`actor.transaction`/`actor.event`/`group.name`, existing drive-body `(sink source)` movement, `(do actor.transaction)`, `(spawn actor.transaction as NAME)`, `(trigger actor.transaction)`, and `(await actor.event)`. `connect`, `transfer`, and `move` are not ATL v0 movement syntax. | Exactly one top-level transaction-body `(await actor.event)` may target the current single static actor instance; it lowers to a one-bit parent handoff input named `actor_event` and reports through `actor_network.event_waits[]`. Exactly one top-level transaction-body `(trigger actor.transaction)` may target that instance; it lowers to a one-cycle parent output handoff named `actor_transaction_start` and reports through `actor_network.transaction_triggers[]`. The next selected data-movement code leaf reserves qualified actor endpoint drive-body pairs as ATL fail-closed forms before generated routing behavior. Rule-level triggers, multiple/nested waits or triggers, fan-in/fan-out, payloads, cross-clock events/triggers, and groups remain fail-closed. No generated ATL child `.fsm`, ATL top, route mux, concurrent-group schedule, or HDL event wiring is promised until a later implementation leaf advertises it. |
