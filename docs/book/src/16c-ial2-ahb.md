...
after emitted mux evidence; `.7` owns a separated-bank contract and `.8` later
implementation. See the direct-seed audit below.
...
and now also locks that mux structure. `.7` must select separate current/next
address-control storage plus an explicit relaunch boundary; `.8` later owns
implementation. See the historical
[direct contract selection](../../IAL2_AHB_DIRECT_SUBORDINATE_PIPELINED_ACTIVE_TRANSFER_CONTRACT_SELECTION.md)
and the current
[lowering-substrate audit](../../IAL2_AHB_DIRECT_SUBORDINATE_COMPLETION_CAPTURE_SUBSTRATE_AUDIT.md).
...
realization unsafe. `.7` owns a separated-bank contract and `.8` later
implementation.
