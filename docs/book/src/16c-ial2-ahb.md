...
FSMGen ships fifty-five public bounded AHB IAL2 entrypoints today:
...
family now also includes exact-three and exact-four requester insertion with
matching `.ahb` aliases. The two-subordinate family includes the exact-three
generic/profile pair and the exact-four generic source.
...
ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif -> amba_requester_busy_insert_four.isf + ahb_lite_subordinate_byte_lane_hburst_seq.isf + ahb_interconnect.isf -> amba_requester_busy_insert_four.fsm + ahb_lite_subordinate_byte_lane_hburst_seq.fsm + ahb_interconnect.fsm + ahb_tb.fsm -> HDL module ahb_tb
...
ppif/ahb_interconnect_two_subordinate_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif -> amba_requester_busy_insert_four.isf + ahb_status_subordinate_byte_lane_hburst_seq.isf + ahb_control_subordinate_byte_lane_hburst_seq.isf + ahb_interconnect.isf -> amba_requester_busy_insert_four.fsm + ahb_status_subordinate_byte_lane_hburst_seq.fsm + ahb_control_subordinate_byte_lane_hburst_seq.fsm + ahb_interconnect.fsm + ahb_tb.fsm -> HDL module ahb_tb
...
ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ahb -> amba_requester_busy_insert_four.isf + ahb_lite_subordinate_byte_lane_hburst_seq.isf + ahb_interconnect.isf -> amba_requester_busy_insert_four.fsm + ahb_lite_subordinate_byte_lane_hburst_seq.fsm + ahb_interconnect.fsm + ahb_tb.fsm -> HDL module ahb_tb
...
| --- | --- | --- |
| Guided mode | The fifty-five public AHB sources listed above, including generic plus matching `.ahb` exact-one through exact-four requesters, paired exact-one through exact-four one-subordinate `.ppif`/`.ahb` source pairs, paired exact-one through exact-three two-subordinate source pairs, and the generic two-subordinate exact-four source | Bounded requester/subordinate/interconnect sources, exact-one through exact-four requester BUSY insertion across generic and alias surfaces, selected byte-lane and HBURST `SEQ` endpoint/aggregate families, selected BUSY-parking families and aliases, paired exact-one through exact-four BUSY-inserting-requester/BUSY-parking-subordinate aggregates across one window, exact-one through exact-three pairing across two windows, and generic exact-four pairing across two windows. |
...
The requester generic/profile checkpoint established 328 protocol fixtures,
369 supported-smoke plus strict fixtures, and 52 AHB paths split 26 `.ppif` /
26 `.ahb`. See the
...
### Exact-four paired generic and profile-alias composition

The additive generic composition and its byte-identical profile alias ship at:
...
It embeds `amba_requester_busy_insert_four`, keeps `(busy-before-beat 2)` and
`(busy-beats 4)`, and pairs that requester with the existing byte-lane
HBURST-aware subordinate whose in-word `SEQ` context parks on BUSY. Existing
lowering emits three IAL1 review artifacts and four IAL0 review artifacts under
top module `ahb_tb`; the requester counter remains width three and retires
`4 -> 3 -> 2 -> 1 -> 0`.

Run the public review and HDL-verification surfaces with the canonical guard:

```bash
scripts/run_with_ram_guard.sh --host-max-pct 100 --process-max-rss-mb 4096 -- ./bin/fsmgen --quiet --strict --check --json ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif
scripts/run_with_ram_guard.sh --host-max-pct 100 --process-max-rss-mb 4096 -- ./bin/fsmgen --quiet --emit-schedule-json ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif
scripts/run_with_ram_guard.sh --host-max-pct 100 --process-max-rss-mb 4096 -- ./bin/fsmgen --quiet --strict --emit-semantic-json ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif
scripts/run_with_ram_guard.sh --host-max-pct 100 --process-max-rss-mb 4096 -- ./bin/fsmgen --quiet --strict --verify-hdl ppif/ahb_interconnect_requester_busy_insert_four_byte_lane_hburst_seq_busy_park.ppif
```

Focused t/1537 proves the generic source's exact support and source identity, schedule/artifact
shape, normalized semantic JSON, repo-relative read-only shell-disabled MCP,
repository-local temporary output, public verification, and assertion-enabled
Verilator 5.046 `--timing` runtime. The runtime observes five presentations,
four completed data beats, one BUSY episode, four qualified BUSY events, one
resumed pending `SEQ`, and final storage `32'h44332211`; requester, subordinate,
and one-hot fabric ownership remain stable throughout BUSY. Focused t/1538
proves the alias is byte-identical, removes only profile-alias residue, keeps
the exact 3 IAL1/4 IAL0 artifacts and width-three counter, matches strict,
schedule, normalized-semantic, real read-only shell-disabled MCP, repository-
local-output, public `--verify-hdl`, and diagnostic surfaces, and preserves
adjacent generic/alias identities without compiling another simulation.

That one-window generic/profile checkpoint is 330 protocol fixtures, 371
supported-smoke plus strict fixtures, and 54 AHB paths split 27 `.ppif` / 27
`.ahb`. The later two-window exact-four generic source moves current accounting
to 331/372/55 split 28 `.ppif`/27 `.ahb`. See
the [shipped behavior](../../IAL2_AHB_EXACT_FOUR_PAIRED_BUSY_COMPOSITION_BEHAVIOR.md).
...
`.ahb`; its matching alias established the 328/369/52 requester checkpoint split
26/26. The matching
...
path still does not ship during activation.
Implementation `.3` now ships the selected generic exact-four paired source
through existing generators. Focused t1537 proves exact support, strict/
schedule/artifact/normalized-semantic/read-only-MCP/public-verifier parity and
assertion-enabled 5/4/1/4/1/`44332211` runtime, with all temporary workspaces
repository-local. That generic-only checkpoint was 329/370/53 split 27
`.ppif` / 26 `.ahb`; the matching alias was still separate. See the
...
Implementation `.825` now ships the byte-identical profile alias through the
existing suffix/lowering machinery. Focused t1538 proves exact support,
report/artifact/schedule/normalized-semantic/read-only-MCP/repository-local-
output/verifier/diagnostic parity in 4 top-level subtests and 88 nested
assertions without a second simulation. That one-window checkpoint is
330/371/54 split 27 `.ppif`/27 `.ahb`; t1537 remains the shared assertion-
enabled runtime.
...
Implementation `.3` now ships the selected generic source and exact support
entry through existing generators. t1539 proves the six-field source delta,
strict/report/4-IAL1/5-IAL0 artifacts, normalized semantic schema 1, real repo-
relative read-only shell-disabled MCP, public verifier, repository-local
output, explicit unmatched-neighbor diagnostics, and Verilator 5.046
`--timing`/`-j 1` with all assertions. Runtime observes 2 commands / 10
presentations / 8 data beats / 2 BUSY episodes / 8 qualified BUSY events / 2
resumed `SEQ` / status `0x44332211` / control `0x88776655`, while requester,
