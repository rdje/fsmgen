...
report as `65535`, and actor-level watchdog constants, actor scalar
parameters, or qualified imported package scalar constants report as their
resolved positive integer while the declaration remains visible in
`actor_constants[]`, `actor_params[]`, or package/import metadata and embedded
package `+constants` entries.
