...
- Width-bearing actor interface ports, transaction-local ports, and
  ...
  overrides. Direct transaction `set` RHS scalar values and scalar operands
  inside transaction `set` RHS expressions may also use local and
  ...
  carriers. Enum members in expression operator position, conditions, set
  targets, rules, drives, parameters, and other non-transaction-set RHS
  contexts remain backlog, as do aggregate paths outside transaction `set` RHS
