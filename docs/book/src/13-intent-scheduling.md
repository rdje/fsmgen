...
- Reusable library imports currently ship the resolver/review-artifact slice:
  ...
  Actor top-level interface widths and actor-owned scalar storage widths may
  use actor-local scalar parameter defaults that resolve to positive integers.
  Use-site FIFO interface shape, bank shape, generated-top
  respecialization, arbitrary-depth generation beyond the first `DEPTH=4`
  fixture, automatic non-zero reset values, standalone transaction/drive
  exports, package/imported constants beyond actor-local constants, derived
  parameter expressions, and nested library imports remain backlog work.
