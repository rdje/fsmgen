...
- **Parameters are specialization defaults**. Actor-level `(params ...)`
  ...
  members, or qualified imported package scalar constants in scalar positions
  and scalar aggregate/list leaves; actor constants and actor scalar parameter
  defaults are published as literal child defaults while child-local
  ...
  Actor-owned scalar storage widths may use the same qualified package scalar
  constant form when it resolves to a positive integer; parser handoff,
  scheduled `.fsm`, `inferred_storage[]`, width evidence, and HDL publish the
  resolved integer width. Bank widths/depths and transaction-local port widths
  do not inherit that package-constant widening.
...
- Reusable library imports currently ship the resolver/review-artifact slice:
  ...
  positive integers. Actor-owned bank widths and actor-owned bank depths may
  use actor-local scalar parameter defaults or declared actor constants that
  ...
  interface width and actor-owned scalar storage width scalar-constant
  subsets, derived parameter expressions, and nested library imports remain
  backlog work.
...
- Width-bearing actor interface ports, transaction-local ports, and
  ...
  transaction parameters, declared actor constants, actor-local scalar
  parameter defaults, or qualified imported package scalar constants by name:
  `(transaction worker (params (BASE 8) (WIDTH BASE) (LANES (BASE DEFAULT_WIDTH shared.LANE0))) ...)`.
  Actor constants and actor scalar parameter defaults publish literal child
  `+params` and generated-composition defaults. Earlier transaction parameter
  dependency tokens, enum-backed transaction defaults, and qualified package
  constant defaults keep their authored tokens because the generated child
  artifact carries the needed declarations, package imports, and embedded
  package roots. Forward/self/cyclic/non-scalar transaction-parameter
  dependencies, runtime signals, unknown symbols, non-scalar actor parameters,
  unqualified package constants, aggregate package constants, package
...
- `(resources ...)` is structurally validated by the parser and now has four
  ...
  `storage_port`. `rule_slot`, `output_bundle`, `transaction_start`, and
  `storage_port` also have a bounded `round_robin` arbiter for declared rule
  users.
  ...
  explicit. `round_robin` remains unsupported for backlog resource kinds and
  other non-selected resource surfaces.
  ...
  bounded `round_robin` `rule_slot`, `output_bundle`, `transaction_start`,
