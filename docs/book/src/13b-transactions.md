...
repeat check can loop. That when-contained nested spawn reuses the static
generated-child handoff model, preserves source-order samples before the spawn
or sync state, and keeps the nested repeat check gated by that instance's
...
cases and this when-contained single-spawn case, broader outstanding-child
...
sample timing applies to the documented when-contained nested single-spawn
subset before its mandatory same-body `await_all`.
...
switch-contained nested spawn, and a `do` while that nested spawn is pending
remain fail-closed.
...
materializes before the sync state. The when-contained nested single-spawn
subset follows the same rule, but it is narrower: exactly one generated spawn
is pending in the nested repeat body and the sync must be same-body
`await_all`; nested `await_any`, multiple nested spawns, switch-contained
spawn nesting, and `do` while that nested spawn is pending are rejected until
their own contracts ship.
...
above, to a repeat directly inside a top-level `when` body. The shipped
repeat-body local `(do child)` subset and the shipped when-contained and
switch-contained repeat local/generated `do` exceptions apply only to their
documented repeat placements, not to repeats nested under `while` or `until`.
