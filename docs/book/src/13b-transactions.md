...
contexts fail closed. Inline dynamic waits are supported in `when` bodies when
no pending sample crosses the wait. Inline dynamic waits in `switch`,
`repeat`, `while`, and `until` bodies currently remain fail-closed with
