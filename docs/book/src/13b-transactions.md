...
chains and repeats under those chains, through `(crossings (activation ...))` is
documented in the control-flow and actor interface chapters; deeper-nested
cross-domain activation and mismatched generated-do domain metadata remain
deferred.
...
clauses, called with `(call NAME actuals...)`. A call can **inline** the body at the
call site (default) or, with a trailing `as INST`, call the procedure as its own
one-shot hardware block via the `(do)` handshake — you pick the convention per call
site. Procedures are documented in full, with many examples, in their own chapter:
