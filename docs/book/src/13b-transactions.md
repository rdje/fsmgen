...
overwrite a pending sample alias plus independent shift states; other successor
...
successors, including completion, independent scalar setter, and independent
shift successors, while preserving false, other-case, and fallthrough exits.
Setters and shifts that read or overwrite a pending sample alias remain
...
independent shift successors. The same pending-sample rule is also supported
in `repeat`, `while`, and `until` bodies for sample-compatible body successors.
