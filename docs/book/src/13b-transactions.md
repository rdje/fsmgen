...
nested repeat check gated by that instance's
...
spawn is pending in the nested repeat body. The when-contained and
switch-contained subsets may use same-body `await_all` or single-pending
same-body `await_any`; multiple nested spawns and `do` while that nested spawn
is pending are rejected until their own contracts ship.
...
top-level `when` body or top-level `switch` branch. The shipped repeat-body
local `(do child)` subset and the
