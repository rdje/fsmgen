...
raw-`ARLEN` burst-length capture, runtime beat-count/`RLAST` validation, and
...
beyond that selected write `BID` multi-static shape, scoreboards, group-local
...
variants, and VHDL remain deferred. This manifest boundary, the downstream handoff, the public
