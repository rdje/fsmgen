...
roots, plus bounded `auxiliary_assignments[]` scalar-string assignment-line
entries, bounded `ports[]` core entry keys, composition-top port extension
...
kinds, structural port, net, link, instance shallow, nested instance
...
`BACKEND-API-VALIDATION-FRONTIER.97.1`, which locks the C3 external-RTL
non-packed aggregate VHDL generic-map boundary after APB/C4 non-packed
aggregate actuals were locked fail-closed before VHDL emission.
...
literal under the active `.97.1` hardening owner, full aggregate VHDL
