...
`BACKEND-API-VALIDATION-FRONTIER.89.1`, which implements only bounded C1
standalone-DT packed-map VHDL generic-map actuals after `.89` selected that
exact edge.
...
sized bitstring, packed-list actuals, and the active packed-map slice, APB/C4
