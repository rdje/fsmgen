...
The historical difference was cost, and it was measured rather than assumed. Before decision `0078`, building the operation graph cost about 5.0 KiB of resident state per operation; the over-limit rejection took 11 seconds and peaked at 5,216 MiB because the builder allocated nearly all one million records before consulting the cap.
The repair does not weaken the guard. The builder now rederives the compact selected total and rejects before graph allocation, so the same observed product outcome completes in one second under the standard 4,096-MiB cutoff:
...
Provider-free runtime-stream construction is complete through `FSM::VIAL::ArchitectureScaleRuntimeStream`. It owns exactly fifteen qualification shapes: portable SystemVerilog, portable VHDL, and qualified OSVVM, each at the reference, gate-candidate, qualification-candidate, limit, and over-limit level. `construct` admits only the frozen checked-AHB HIAL/VIAL source pair and derives the qualified tool selector from the catalog, so callers cannot supply a replacement tool profile; for example, construct `backend_profile => 'sv_portable_verilator'`, `level => 'gate_candidate_v1'`, and the checked HIAL/VIAL texts, then pass that construction as the sole field to `evaluate`. Evaluation rebuilds ordinary SemanticIR, bridge manifest, ExecutionIR, backend inputs, and plan twice through the completed backend-emission foundation and content-addresses the equal projections. The returned report is construction evidence, not a runtime result: `construct`, `semantic`, `bridge`, and `plan` are complete, while `emit`, `compile`, `run`, `trace_validate`, and `result_produce` are `not_run_measurement_required`. Its backend handoff freezes five canonical stage digests, the checked two-scenario/21-operation/four-fiber plan, exact backend schema and structural authority, and the digest of two backend-input artifacts; it emits no backend artifact, opens no OSVVM provider, and starts no external process. The trace expectation retains `checked_ahb_reference_v1`, the common 10,000 gate, the portable-only 15,000 qualification override, the independently retained 100,000 VHDL/OSVVM qualifications, and 8,000,002/8,000,003-record plus 67,108,864/67,108,865-byte limit/over-limit specifications with earliest-cap authority. Compile/run expectations name Verilator 5.046 or GHDL 6.0.0 LLVM-JIT, OSVVM 2026.05 where applicable, exact command authorities, 120-second analyze/compile, 60-second VHDL elaboration, 30-second run, 8-MiB compile transcript, and 64-MiB run/trace/result envelopes. The unmaterialized normalized result must use `fsmgen.verification_result_manifest.v1` and pass its semantic oracle. Canonical regeneration rejects changed constructions, reports, and support claims; repository-derived staging removes its exact tree after success and consumer failure. Focused `t/1651` covers all fifteen provider-free shapes.

Decision `0083` audits that provider-free expectation before portable runtime measurement. Qualified public probes show that the common 100,000-record candidate cannot fit the portable Runner's 64-MiB output envelope, while the complete public artifact graph adds a separate 64-MiB authority. Implemented `FSM::VIAL::ArchitectureScalePortableRuntimeMaterialization` now owns exact tracked reset-sampling sources at 10,000 gate and 15,000 qualification records. It verifies source bytes, permits only the selected reset/timeout/window/terminal-expectation deltas, rebuilds ordinary SemanticIR/bridge/ExecutionIR/plan/emission twice, and projects every record family before tool execution. Exact guarded public reruns produce 19-artifact graphs of 32,098,531 and 47,505,049 bytes; the latter remains 2,826,599 bytes below the 50,331,648-byte three-quarter admission ceiling. Reference remains correctness-only, nominal record limit/excess remain byte-preflight dominated and unexecuted, 20,000/25,000/100,000 retain their exact earlier dominance classifications, and VHDL/OSVVM remain at their separately owned 100,000 candidate. The same decision selects a private content-addressed Verilator lifecycle shared by the public Runner and process-isolated measurement stages; that lifecycle is the next implementation boundary. [Portable Verilator Runtime Measurement](16dc-vial-portable-verilator-runtime-measurement.md) documents the tracked sources, record equation and family table, final graph headroom, state chain, timeout/process containment, reverify workflow, and nonclaims.
Decision `0077` resolves the balanced-portable route without widening an existing protocol. It preserves the fixed AHB bridge and private-nonportable revision-1 scale route, then adds a separate direct-IAL1 `architecture_scale_probe` profile named `balanced_portable`, revision `2`. The implemented bridge, execution, backend-input, and portable-SystemVerilog entrypoints are caller-sealed to the balanced composer/qualifier. Bridge admission requires the complete parsed/scheduled/lowered profile and emits a content-addressed manifest; later admissions independently revalidate complete identities and shapes before binding or rendering. The admitted shape has 126 bound data endpoints, 32 probes, 16 single-active transaction aliases, 109 endpoint-backed fields per alias, and 128 events, yielding exactly `1 + 1 + 126 + 32 + 16 + 1,744 + 128 = 2,048` genuine bindings without event-input or adapter-state padding. Public callers and near misses fail closed, while existing public AHB, emission, and private revision-1 routes remain unchanged. Canonical composition and exact `sv_portable_verilator` structural emission are complete under `.17.2.7.2.3-.4` and documented in [Balanced Portable Composition](16da-vial-balanced-portable-composition.md); byte-equal emission produces eight artifacts/three sources/503,279 source bytes/3,605 maps covering all 1,024 operations and 2,048 bindings. Compilation, runtime, trace, result, support, performance, capacity, and reached-boundary claims remain false.

Backend-emission selection is complete under decision `0075`. Portable profiles start from the checked AHB reference, repeat its real response expectation before `scoreboard_check`, and use expanded operation total `T` as a profile-local axis through ordinary parsing, the checked-AHB bridge, public `PlanBuilder`, and canonical ExecutionIR. Portable-SV oracle revision 2 selects `T=21/1,024/4,096/6,318`: eight artifacts/three sources, with the limit at 16,774,723 source bytes/6,351 maps; the pre-cap `T=6,319` graph is 16,777,362 bytes and rejects atomically at `/artifacts` under the unchanged 16-MiB cap.
Portable VHDL selects `T=21/128/512/29,506`: seventeen artifacts/six sources/21 checks, with the limit at 16,777,107 source bytes/29,544 maps; `29,507` renders 16,777,675 bytes and rejects. OSVVM uses the same ladder, copies those six sources byte-for-byte, and adds one fixed 4,351-byte adapter: its selected limit totals 16,781,458 wrapper-source bytes/29,551 maps, while `29,507` rejects in the portable foundation. Decision `0090` explains the two-operation refinement from the historical boundary: trace v2 adds a fixed 1,504-byte authenticated snapshot emitter and a 21st structural check, without changing the 16-MiB cap. Native UVM remains a selected review profile: `T=21` emits sixteen artifacts/ten sources/75 maps/fourteen checks; `T=22` is the first unsupported shape and must reject negotiation, so larger levels are not constructed.
Portable VHDL indexes metadata and generated-line anchors once per source: guarded canonical emission accepts `T=29,506` twice at 16,777,107 bytes/29,544 maps with all 21 checks and retains exact atomic `T=29,507` rejection. OSVVM provider verification is reusable through one sealed callback evaluation, and all 59 portable maps are validated then translated after seven adapter entries for 66-map reference closure. Native UVM rejects T=22, larger plans, and changed same-count expectation shapes before artifact construction. The repairs are qualified together; this is structural durability, not compile, runtime, support, performance, or capacity evidence.
`FSM::VIAL::BackendEmissionAuthority` is the single closed catalog/discovery source. It distinguishes portable SV's three source/eight total artifacts; portable VHDL's six/17 inventory, 59 reference maps, and 21 checks; OSVVM's six-source portable foundation, one 4,351-byte adapter, seven total sources, sixteen artifacts, portable-only 16-MiB boundary, 66 maps, and 12+21 checks; and native UVM's ten/sixteen inventory, 16-MiB and one-million-map enforced caps, and selected 21-operation/75-map/14-check/25-mapping matrix. Unknown, missing, obsolete, or contradictory fields fail closed. VIAL still accepts a 128-byte source identifier and rejects 129; generated maxima are 131/142/142/154 bytes below 255. Completed generator foundation `.17.2.6.3.1` retains a private profile-neutral qualification mode: it admits only the frozen checked-AHB source pair, independently regenerates ordinary SemanticIR, bridge manifest, canonical ExecutionIR, backend inputs, and target-neutral plan, preserves the frozen 21-operation non-emission report, and cleans success or consumer-failure staging below `.artifacts/tmp/vial-scale/`. Completed portable-SV child `.17.2.6.3.2` owns exactly the five selected routes through that boundary. Its caller-sealed revision-2 profile module emits T=21/1,024/4,096/6,318 twice and proves the exact eight-artifact/three-source inventories at 164,507/2,803,857/10,910,865/16,774,723 source bytes with 54/1,057/4,129/6,351 complete maps; T=6,319 returns only `VIAL_BACKEND_LIMIT_EXCEEDED` at `/artifacts`, with no partial plan, operation, manifest, map, or artifact graph. The schema and shared discriminator are explicitly revision 2; the common role labels remain stable because all four profiles share them. Each accepted oracle freezes all three source hashes and ordered relative paths, checks every map's generated-source span, compares the exact mapped operation-ID set, validates encoded source-map and manifest closure, observes a stable 113-byte maximum generated identifier below the separate 255-byte limit, content-addresses the result, and rejects report mutation by canonical regeneration. Completed portable-VHDL child `.17.2.6.3.3`, refined by trace-v2 child `.17.3.6.1`, adds exactly five selected routes through the same boundary. Its caller-sealed profile module emits T=21/128/512/29,506 twice and proves the exact seventeen-artifact/six-source inventories at 118,064/176,433/388,401/16,777,107 source bytes with 59/166/550/29,544 complete maps and 21 passing structural checks; T=29,507 returns only `VIAL_VHDL_BACKEND_LIMIT_EXCEEDED` at `/artifacts`, with no partial plan, operation, manifest, map, validation, or artifact graph. Each accepted VHDL oracle freezes all six source hashes and ordered relative paths, checks every generated-line span, compares the exact mapped operation-ID set, validates encoded source-map, validation, and manifest closure, observes a stable 37-byte maximum generated identifier for this selected ladder below the separate 255-byte limit, content-addresses the result, and rejects report mutation by canonical regeneration. Completed OSVVM child `.17.2.6.3.4`, with the same trace-v2 refinement, adds the same five selected T levels through one exact provider verification per evaluation and two callback-scoped defensive emissions. T=21/128/512/29,506 prove sixteen artifacts and seven sources at 122,415/180,784/392,752/16,781,458 bytes, with 66/173/557/29,551 complete maps: seven adapter entries precede every translated portable entry. The oracle freezes the adapter and six byte-identical portable source hashes, exact provider commit/tree/manifest identity and 14/13/14/0 repository/gitlink/licence/notice census, seven advanced mapping statuses, six semantic guards, twelve wrapper checks plus 21 portable prerequisite checks, complete operation IDs, evidence/manifest encoding, a stable 37-byte generated-identifier maximum, content identity, and byte-equal reruns. T=29,507 returns only `VIAL_OSVVM_PORTABLE_FOUNDATION_ERROR` at `/portable_foundation`, with no partial provider or artifact evidence. The modules remain pure: `prove -Iperl t/1645-vial-architecture-scale-backend-emission-foundation.t t/1646-vial-architecture-scale-backend-emission-portable-sv.t t/1647-vial-architecture-scale-backend-emission-portable-vhdl.t t/1648-vial-architecture-scale-backend-emission-osvvm.t t/1649-vial-architecture-scale-backend-emission-native-uvm.t t/1650-vial-architecture-scale-backend-emission-family-qualification.t` reproduces all completed profile outcomes without executing Verilator, GHDL, a SystemVerilog parser/compiler, or another external runtime and leaves no scale-stage residue. Completed native-UVM child `.17.2.6.3.5` owns five profile outcomes but emits only exact T=21: sixteen artifacts/ten SystemVerilog sources/138,345 bytes/75 structural maps, with six intentional operation associations, fourteen passing checks, 25 selected mappings, seven review stages, five closure checks, and a 49-byte selected generated-symbol maximum below 255. All four non-reference levels execute the adjacent T=22 negotiation rejection; later levels are preflight-dominated/not-constructed, and direct T=128 plus changed-same-count T=21 witnesses also reject without partial evidence. Completed family closure `.17.2.6.3.6` independently derives all twenty catalog outcomes twice and freezes the exact thirteen-emitted/seven-rejected partition. It reevaluates one reference and adjacent rejection per profile, retains authority/oracle applicability, profile-specific structural evidence and diagnostics, content identities, byte-equal reruns, direct-helper and hostile-mutation rejection, exact same-volume cleanup, and all structural-only nonclaims. The current 4,096-MiB guarded focused scale suite retains the same structural-only nonclaims. No support, performance, runtime, whole-product capacity, native-UVM execution, or cross-backend claim is added.
...
### Execution and checking measurement boundary

The semantic/bridge adapter, publisher, and exact matrix are complete. `.17.3.3` now also provides caller-sealed `FSM::VIAL::ArchitectureScaleExecutionCheckingMeasurement` for `execution_graph_v1` and `checking_state_v1`: callers select only repository root, owned axis, and level; the adapter reconstructs the canonical producer twice and admits no caller source, SemanticIR, bridge manifest, plan, report, or tool result. Its family vocabulary is `construct -> parse_validate -> bridge -> bind_plan`; each planned payload is independently rerun twice, recorded by the common controller, and regenerated again by report validation. Accepted gate/qualification authorities retain one correctness validation plus exactly three/five raw measured records; an authoritative parser or bridge rejection is validated without timing. A `preflight_dominated` construction runs only `construct` and `bind_plan`, so it cannot claim parse/bridge work that its authority forbids. An `envelope_unconstructible` source-free outcome stays outside the common controller because no canonical workload identity exists: the report retains the canonical family evaluation but invents no identity, input, validation record, sample, artifact, or reached boundary. The controller still emits its full twelve-stage record, runs exact cleanup, and marks every unplanned processing stage `not_run`. Correctness validation, measurement, and independent report regeneration all require the real guard; run the exact adapter proof through `FSMGEN_VIAL_SCALE_EXECUTION_CHECKING_EXACT=1 scripts/run_with_ram_guard.sh -- prove -Iperl t/1659-vial-architecture-scale-execution-checking-measurement.t`, because setting guard-looking variables manually is not enforcement. The exact publisher freezes only the canonical producers' ten execution and eight checking axes at four selected levels: 72 profile sets, never a caller inventory or report collection. A controller-backed report establishes one clean Git/host/tool/enforced-guard capture identity; each immutable content-addressed profile envelope carries that provenance, including an `envelope_unconstructible` nested report that remains free of invented workload, validation, sample, or controller identity. Byte-equal same-capture evidence resumes, while different bytes, revisions, host/tool/guard identities, or ambiguous staging reject; family/full manifests remain absent until every exact child validates. Inspect without measuring through `scripts/run_vial_execution_checking_measurement_matrix.pl --inventory`; capture with `scripts/run_with_ram_guard.sh -- scripts/run_vial_execution_checking_measurement_matrix.pl`; independently revalidate with the same wrapper plus `--validate`. `.17.3.3.1` supplies concurrency-safe raw-stage orphan recovery without manual deletion or a weaker threshold. `.17.3.3.2` now runs every full profile capture, immutable resume, and family/complete revalidation in a separate guard-visible child. The child preserves complete adapter regeneration and atomic publication/reload, then returns only a canonical closed compact result through a close-on-exec pipe bounded to 1,048,576 bytes; exceptions are bounded to 4,096 bytes, and signal, nonzero exit, malformed/noncanonical/oversized output, identity drift, and artifact drift reject before the coordinator retains the entry. The long-lived parent therefore cannot accumulate full-report allocator high-water state. Nineteen pre-repair reports remain immutable revision-`c7493e3d` interruption evidence: they can be independently revalidated through the repaired path, but cannot be relabelled or mixed into a later repaired revision's common-identity seal.

Guarded capture was intentionally diagnostic: it first exposed long-lived coordinator allocation, then late total-operation and serialized-plan enforcement, and finally redundant accepted-plan random evaluation. Each exact incompatible prefix is preserved under a revision-keyed, content-addressed same-volume archive and is never relabelled into a later seal. The publisher now runs capture, immutable resume, and validation in separate guard-visible profile children, so the coordinator retains only a bounded canonical result. Failures, signals, malformed or oversized child results, identity drift, publication collisions, and artifact drift reject before admission.

Decision `0078` moves the unchanged one-million-operation authority immediately after scenario selection. `ExecutionBuilder` independently walks compact ordinary, repeat, and parallel action trees with saturating arithmetic, requires equality with SemanticBuilder's validated per-scenario count, and rejects excess before bridge indexing or graph/source-map/plan allocation; the terminal check remains. The exact compact boundary accepts 1,000,000 and returns the historical diagnostic at 1,000,001 under the unchanged guard.

Decisions `0078` and `0079` also preserve the unchanged 16,777,216-byte plan authority without a workload-specific threshold. Projection counts occurrences, streams each generated or replay-validated decision once into a private SHA-256-sealed, u32-framed canonical UTF-8 transcript bounded to 17,039,360 bytes, and accounts for scenario IDs plus source maps against the shared base-plan schema. Known excess rejects before final lists, ExecutionIR, or report; acceptance independently rewalks expectations, validates and consumes each record once, deletes the transcript, and must equal terminal canonical bytes. Exact guarded evidence preserves 8,440 acceptance at 16,775,415 bytes, historical `/plan` rejection at 8,441 and 32,768, 65,536/65,537 dominance order, attempt 999,999, frozen random/replay identities, and profile 23's clean bind-plan admission. Caps, guards, timeouts, reruns, and nonclaims remain unchanged.

The fresh revision-`9c0209c22` capture now seals all 40 execution and 32 checking profiles, both family manifests, and accepted complete identity `execution-checking-matrix/0548629720a9f4c8636c912396cfaefff523da92fc800594618db68b788b91d0`. Its 75 active files occupy 7,312,415 bytes and account for 41 accepted, 19 expected-rejection, five inapplicable-measurement, two preflight-dominated, 31 applicable-measurement, 62 controller-applicable, ten controller-inapplicable, 119 raw-measurement, ten source-free, and zero excluded records. An initial 88% host-memory stop occurred while revalidating the already immutable execution family; it left no raw staging, and exact same-identity resume completed checking and the aggregate seal. A separate guarded `--validate` then canonically reloaded the same identity and partition with zero diagnostics. This proves a complete clean-revision construction-measurement publication and its recovery path—not architecture capacity, a reached boundary, public performance budgets, backend emission, provider/tool access, compile/run/trace/result behavior, support, or public API.

### Structural backend-emission measurement boundary

`.17.3.4.1` adds caller-sealed
`FSM::VIAL::ArchitectureScaleBackendEmissionMeasurement`. It measures only
FSMGen's deterministic structural generation. Callers supply only repository
root, one decision-`0075` backend profile, and one owned level; the adapter
reads the frozen checked-AHB HIAL/VIAL pair itself and rejects caller-created
stage products or evidence. The sole private handoff from
`ArchitectureScaleBackendEmission` is sealed to this exact adapter and returns
canonical JSON projections, so measurement creates no second public parser,
binder, planner, or backend route.

The adapter plans only the FSMGen-owned prefix of the common controller:

```text
construct -> parse_validate -> bridge -> bind_plan -> emit
```

Every record still contains the controller's complete twelve-stage vocabulary.
`compile_analyze`, `elaborate`, `run`, `trace_validate`, `result_produce`, and
`publish` are explicitly `not_run`; `cleanup` remains controller-owned and must
succeed. Each planned payload is generated twice inside its isolated worker,
then its closed artifact/count/oracle evidence is regenerated by report
validation. `parse_validate` derives the anchored profile source and exact
SemanticIR identity. `bridge` additionally proves the canonical bridge and
backend-input identities. `bind_plan` adds ExecutionIR, plan, route metrics,
and their digests. `emit` invokes the existing complete profile evaluation and
retains exact artifact/source/byte/map/check/identifier/content identities or
the exact atomic rejection. These are cumulative canonical route boundaries;
they do not pretend that a later stage's prerequisites cost zero.

Only emitted gate and qualification routes retain performance samples:

| Profile | Three-sample gate | Five-sample qualification | Correctness-only outcomes |
|---|---:|---:|---|
| portable SystemVerilog | `T=1,024` | `T=4,096` | reference and emitted `T=6,318` limit; atomic `T=6,319` rejection |
| portable VHDL | `T=128` | `T=512` | reference and emitted `T=29,506` limit; atomic `T=29,507` rejection |
| OSVVM-qualified VHDL | `T=128` | `T=512` | reference and emitted `T=29,506` limit; portable-foundation `T=29,507` rejection |
| native UVM selected review | none | none | emitted reference, adjacent `T=22` negotiation rejection, and larger preflight-dominated shapes |

Every measurement request first retains ordinal-zero correctness validation.
An authoritative non-emission is `validated_not_measured`, with no fabricated
sample or exclusion. Native-UVM larger shapes plan only `construct` and `emit`;
parse, bridge, and bind/plan remain `not_run` because decision `0075` classifies
them as preflight-dominated/not-constructed. Reference, limit, and excess
levels are invoked through `validate_profile`, never mislabeled as gate or
qualification timing evidence.

OSVVM's repository-local 2026.05 materialization is a special prerequisite,
not a tool run. The existing callback-scoped backend verifies the sealed
provider tree and reuses that read-only evidence while `emit` constructs the
artifact graph. The report therefore marks provider verification as
applicable, read-only, and included in `emit`, while
`external_verification_tool` remains false. No compiler, simulator, external
process, HDL runtime, trace, or result producer is started.

The same real RAM guard is mandatory for correctness validation, measurement,
and independent report regeneration. The focused exact proof is:

```bash
FSMGEN_VIAL_SCALE_BACKEND_EMISSION_MEASUREMENT_EXACT=1 \
  scripts/run_with_ram_guard.sh -- \
  prove -Iperl \
