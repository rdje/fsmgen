...
Portable VHDL now indexes metadata and generated-line anchors once per source: guarded canonical emission accepts `T=29,508` twice at 16,776,739 bytes/29,546 maps with all twenty checks and retains exact atomic `T=29,509` rejection. OSVVM provider verification is reusable through one sealed callback evaluation: entry verifies the exact repository-local OSVVM 2026.05 root once, every matching local emission receives a defensive result copy, and the non-constructible capability is deleted on callback success or failure. A different root, a forged object, or a retained stale handle returns no artifacts; standalone emission still verifies independently and remains byte-identical. This is structural durability, not compile, runtime, support, performance, or capacity evidence. OSVVM must still translate every portable map; native UVM must reject the unevaluated extra expectation at negotiation. Catalog repair then records exact inventories, OSVVM's portable-only 16-MiB cap, and native UVM's one-million-map limit.
VIAL accepts a 128-byte source identifier and rejects 129 bytes; generated maxima at that edge are 131/142/142/154 bytes for portable SV/portable VHDL/OSVVM/native UVM, below the separate 255-byte cap. Emitters remain pure in-memory graph builders; later scale infrastructure alone may stage below repository-local `.artifacts/tmp/vial-scale/` and must remove it exactly. Until tracked generator/watchers land, these re-derived and falsified figures retain an explicit executable-durability gap owned by active `.17.2.6`; they change no current output and make no support, performance, runtime, whole-product capacity, native-UVM execution, or cross-backend claim.
