...
The fresh revision-`14c61936` capture confirms that repair in the complete publisher: the former `operations_total/over_limit_v1` profile now seals under the unchanged guard, and the execution family advances through 22 atomic reports. The next profile, `random_attempts/limit_v1`, exposes a separate correctness-validation defect after completing its one-million-attempt controller run. A direct guarded adapter reproduction retains `VIAL_SCALE_MEASUREMENT_TIMEOUT` at `/stage_measurements/bind_plan/timeout`: construct, parse/validate, and bridge accept, but the bind-plan worker reaches its fixed 300-second effective ceiling, receives signal 15, and produces no partial output. The repository RAM guard does not terminate it, cleanup removes the complete raw stage, the failed profile is not published, and family/full manifests remain withheld. Source tracing identifies redundant nesting: the worker already reruns every stage payload twice, while each bind-plan payload invokes the twice-evaluated canonical-family helper again. The already published reports remain immutable resume evidence. Active child `.17.3.3.2.2` owns removal of only that duplicate evaluation layer, a focused call-count regression, exact canonical-output equality, and same-revision resume. No timeout, guard, attempt count, replay/determinism contract, performance budget, support, capacity, or reached-boundary claim changes from this interruption.
