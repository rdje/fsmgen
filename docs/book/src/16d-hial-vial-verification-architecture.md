...
The fresh revision-`14c61936` capture confirms that repair in the complete publisher: the former `operations_total/over_limit_v1` profile now seals under the unchanged guard, and the execution family advances through 22 atomic reports. The next profile, `random_attempts/limit_v1`, exposes a separate correctness-validation defect after completing its one-million-attempt controller run. The worker exits with the matrix admission message that correctness validation did not accept; the guard does not terminate it, raw staging is empty, the failed profile is not published, and family/full manifests remain withheld. The already published reports remain immutable resume evidence. Active child `.17.3.3.2.2` owns retention of the nested stage diagnostic, comparison with the canonical million-attempt t/1614 authority, repair at the first proven disagreement, and same-revision resume. No guard, attempt count, replay contract, performance budget, support, capacity, or reached-boundary claim changes from this interruption.
