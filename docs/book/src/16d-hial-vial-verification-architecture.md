...
The VHDL limit acceptance waits on linearizing per-operation whole-metadata validation that currently exceeds the selected 300-second ceiling. OSVVM must reuse one sealed exact provider preflight and translate every detailed portable map instead of replacing them with six whole-file placeholders. Native UVM must close negotiation because an extra expectation currently appears only in a coarse map association and has no emitted evaluator. Catalog repair then records exact inventories, the portable-only meaning of OSVVM's 16-MiB cap, and native UVM's one-million-map limit.
