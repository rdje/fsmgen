...
The clean revision-`e4b95dbd` restart subsequently seals every one of the 40 execution profiles and the execution-family manifest, including the exact 16,384-live-fiber acceptance and 16,385 rejection boundary. It then seals 17 checking profiles before `random_occurrences/qualification_candidate_v1`. Canonical checking-state evaluation correctly expects `VIAL_EXECUTION_LIMIT_ERROR` at `/plan` because 32,768 occurrences exceed the unchanged 16,777,216-byte serialized-plan cap; the measured bind-plan stage instead times out after 300 seconds. The builder currently materializes all random decisions and source maps, constructs ExecutionIR and ExecutionReport, and canonical-serializes the complete plan before applying that cap. Active child `.17.3.3.2.3` therefore requires a general independently derived plan-projection byte preflight—never a special-case occurrence threshold—before known-excess materialization. Accepted plans must prove exact equality between preflight and terminal canonical bytes, and the terminal serializer/cap remains defense in depth. Exact 8,440 acceptance, 8,441 rejection, 32,768 rejection, 65,536 preflight dominance, and 65,537 count-cap dominance remain unchanged obligations. The 57-profile prefix and execution-family seal now reside in the same-volume revision-keyed history archive with a closed manifest: independent reconstruction accounts for all 57 reports, the family seal, every byte and payload hash, and canonical aggregate digests; the active execution/checking namespace and raw staging are empty. Checking-family and complete manifests remain withheld pending a fresh repaired-revision 72-profile capture.
