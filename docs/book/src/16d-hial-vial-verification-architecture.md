...
The first clean repaired-revision capture proves that isolation boundary, then exposes a separate product defect instead of hiding it. Profiles one through nineteen publish at revision `b9463c6`; the isolated `operations_total/over_limit_v1` worker then reaches 4,869.5 MiB and the unchanged 4,096-MiB guard terminates it. `ExecutionBuilder::_build_operations` currently checks `expanded_operations_total` only after emitting and source-mapping the entire selected operation graph, and the older exact boundary test consequently asks for a separate 6,144-MiB guard. Decision `0078` selects the long-term repair: after exact scenario selection, the builder independently rederives each compact action tree's expanded count, requires equality with SemanticBuilder's validated `action_count`, aggregates the selected total with bounded arithmetic, and applies the existing one-million-operation cap before bridge indexing, bindings, operations, source maps, or plan bytes are allocated. The existing diagnostic and terminal check remain unchanged; this is genuine product enforcement, not a matrix skip or invented preflight outcome. The 19-report/1,739,213-byte failed prefix is revision-archived with a reproducible canonical manifest. Exact 72-profile completion, family/full manifests, performance-budget, support, capacity, reached-boundary, provider, backend, compile, run, trace, result, and whole-product claims remain pending.
