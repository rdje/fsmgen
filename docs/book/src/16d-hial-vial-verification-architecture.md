...
Decision `0077` resolves the balanced-portable route without widening an existing protocol. It preserves the fixed AHB bridge and private-nonportable revision-1 scale route, then adds a separate direct-IAL1 `architecture_scale_probe` profile named `balanced_portable`, revision `2`. The implemented bridge and execution entrypoints are caller-sealed to the balanced composer. Bridge admission requires the complete parsed/scheduled/lowered profile and emits a content-addressed manifest; execution admission independently revalidates that manifest before and after binding. The admitted shape has 126 bound data endpoints, 32 probes, 16 single-active transaction aliases, 109 endpoint-backed fields per alias, and 128 events, yielding exactly `1 + 1 + 126 + 32 + 16 + 1,744 + 128 = 2,048` genuine bindings without event-input or adapter-state padding. Public callers and near misses fail closed, while the existing public AHB and private revision-1 routes remain unchanged. Canonical composition is complete under `.17.2.7.2.3` and is documented in [Balanced Portable Composition](16da-vial-balanced-portable-composition.md). The sole eligible emitter remains `sv_portable_verilator`, but it cannot consume the new capability until `.17.2.7.2.4` adds exact bridge/ExecutionIR negotiation. Public planning, other backends, runtime, support, performance, capacity, and reached-boundary claims remain unchanged.
