...
Decision `0077` resolves the balanced-portable route before implementation. It preserves the fixed AHB bridge and private-nonportable revision-1 scale route, then selects a separate direct-IAL1 `architecture_scale_probe` profile named `balanced_portable`, revision `2`. That profile is admitted only through bridge and execution entrypoints caller-sealed to the balanced composer. Its sole eligible emitter is `sv_portable_verilator`, which must negotiate both the qualification capability and the exact bridge/ExecutionIR shape instead of trusting a capability label. The shape retains 126 bound data endpoints, 32 probes, 16 transaction aliases, 109 fields per alias, and 128 events, yielding exactly `1 + 1 + 126 + 32 + 16 + 1,744 + 128 = 2,048` bindings without event-input or adapter-state padding. Implementation remains owned by `.17.2.7.2.2`-`.4`; public planning, other backends, runtime, support, performance, capacity, and reached-boundary claims remain unchanged. These fixtures change no parser, SemanticIR, bridge, ExecutionIR, backend, runtime, public product API, capability, support, performance, or capacity behavior.
