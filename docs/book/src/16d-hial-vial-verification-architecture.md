...
Fresh revision `037e56f09` seals 22 profiles before `random_attempts/limit_v1` fails correctness validation. Projection and accepted materialization each search for the same decision, while family and controller each rerun twice. One guarded million-attempt search takes 61.094573 seconds, so eight have a 488.756584-second lower bound inside the unchanged 300-second stage. The guard remains active; profile 23, aggregate seals, and raw residue remain absent. Decision `0079` selects one private bounded canonical transcript: projection searches once, known excess still allocates no final list/map/IR/report, and accepted materialization consumes the closed record. Reruns, bytes, identities, limits, timeouts, and nonclaims stay unchanged; implementation and a fresh matrix remain pending.
