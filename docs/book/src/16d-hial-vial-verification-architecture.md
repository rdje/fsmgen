...
Active experimental slice `.13.2` now owns exact open-source tool and UVM-
source inspection plus reusable compile/elaboration feasibility probes. This
activation selects and runs nothing; generated bytes, capabilities, and all
qualification states remain unchanged until its implementation is measured.
