...
The first clean repaired-revision capture proves that isolation boundary, then exposes a separate product defect instead of hiding it. Profiles one through nineteen publish at revision `b9463c6`; the isolated `operations_total/over_limit_v1` worker then reaches 4,869.5 MiB and the unchanged 4,096-MiB guard terminates it. The cause was `ExecutionBuilder::_build_operations` checking `expanded_operations_total` only after emitting and source-mapping the entire selected operation graph; the older exact boundary test consequently needed a separate 6,144-MiB guard.

Decision `0078` is now implemented as product enforcement. Immediately after exact scenario selection, `ExecutionBuilder` independently walks each compact action tree, including parallel fibers and repeated bodies, using saturating bounded addition and multiplication. Each derived scenario count must equal SemanticBuilder's validated `action_count`; malformed structure or a disagreement fails as an internal consistency error. The selected total then meets the existing one-million-operation cap before bridge indexing, bindings, types, operations, source maps, or plan bytes are allocated. The terminal check remains as defense in depth. A compact boundary oracle accepts exactly 1,000,000 and returns the byte-identical historical limit diagnostic at 1,000,001. The real `over_limit_v1` evaluation passes in one second under the unchanged 4,096-MiB guard, remains an observed `expected_rejection`, and proves bridge indexing is never entered. This is neither a matrix skip nor an invented preflight outcome. The 19-report/1,739,213-byte failed prefix remains revision-archived with a reproducible canonical manifest. A fresh common-revision 72-profile capture, family/full manifests, performance-budget, support, capacity, reached-boundary, provider, backend, compile, run, trace, result, and whole-product claims remain pending.

The fresh revision-`14c61936` capture confirms that repair in the complete publisher: the former `operations_total/over_limit_v1` profile now seals under the unchanged guard, and the execution family advances through 22 atomic reports. The next profile, `random_attempts/limit_v1`, exposed a separate correctness-validation defect after completing its one-million-attempt controller run. A direct guarded adapter reproduction retained `VIAL_SCALE_MEASUREMENT_TIMEOUT` at `/stage_measurements/bind_plan/timeout`: construct, parse/validate, and bridge accepted, but the bind-plan worker reached its fixed 300-second effective ceiling, received signal 15, and produced no partial output. Source tracing found redundant nesting: the worker already reran every stage payload twice, while each bind-plan payload invoked the twice-evaluated canonical-family helper again. The repair factors one closed producer-evaluation validator: canonical admission still invokes it twice, and the worker still invokes each bind-plan payload twice, but neither payload nests another pair. `ExecutionRandom` also precomputes the immutable SHA-256 input prefix and bounded attempt bytes once and omits rejection/modulo only when the range exactly fills the selected bit space; the algorithm ID, digest input bytes, accept predicate, attempt cap, and returned values are unchanged. A guarded exact adapter run now returns `accepted_validation` with an accepted bind-plan record, exact one-million-attempt metrics, no diagnostic, and complete cleanup; the direct gate/qualification/limit/one-over suite preserves every frozen plan/replay identity and exhaustion diagnostic. No timeout, guard, attempt count, replay/determinism contract, performance budget, support, capacity, or reached-boundary claim changes.

The clean revision-`e4b95dbd` restart subsequently seals every one of the 40 execution profiles and the execution-family manifest, including the exact 16,384-live-fiber acceptance and 16,385 rejection boundary. It then seals 17 checking profiles before `random_occurrences/qualification_candidate_v1`. Canonical checking-state evaluation correctly expects `VIAL_EXECUTION_LIMIT_ERROR` at `/plan` because 32,768 occurrences exceed the unchanged 16,777,216-byte serialized-plan cap; the measured bind-plan stage instead timed out after 300 seconds because the builder materialized all random decisions and source maps, ExecutionIR, ExecutionReport, and complete canonical plan bytes before applying that cap.

`.17.3.3.2.3` now enforces the same cap through a general canonical projection rather than a workload-specific threshold. The builder first applies the existing random-occurrence cap. It then walks each would-be generated or replayed decision without retaining the decision list, canonical-encodes that one record and its corresponding source-map record, and accounts for its scenario occurrence ID. A shared private report builder constructs the fixed base plan with empty projected arrays and a same-length placeholder plan identity. Saturating addition combines those independently encoded element lengths, their exact commas, and the base bytes. The stream continues validating random generation or replay even after the byte total saturates, so an earlier random/replay authority cannot be hidden by plan excess. The report projection seam is callable only from `ExecutionBuilder`; public callers still require an exact `ExecutionIR`.

The unchanged 16,777,216-byte limit now runs before the random-decision list, its source-map suffix, ExecutionIR, or the terminal report can be allocated for a known-excess plan. Accepted plans take the ordinary materialization route, and the terminal canonical byte count must equal the preflight result before the original terminal cap runs again. Exact guarded evidence preserves 8,440 acceptance at 16,775,415 bytes, the byte-identical historical `/plan` rejection at 8,441 and 32,768, 65,536 preflight dominance, and 65,537 random-count-cap dominance. The former qualification blocker now exits through the measurement adapter as `expected_rejection` / `validated_not_measured`, with no diagnostic, performance sample, or staging residue. This two-pass accepted-plan design deliberately spends deterministic CPU to keep preflight memory bounded; it adds no capacity, throughput, performance-budget, or support claim.

The 57-profile prefix and execution-family seal remain in the same-volume revision-keyed history archive with a closed manifest: independent reconstruction accounts for all 57 reports, the family seal, every byte and payload hash, and canonical aggregate digests. The active execution/checking namespace and raw staging are empty. Checking-family and complete manifests remain withheld pending a fresh clean-revision 72-profile capture.

Decision `0079` is now implemented in `perl/FSM/VIAL/ExecutionBuilder.pm`. Projection generates or replay-validates once and keeps only a private SHA-256-sealed, u32-framed canonical UTF-8 scalar bounded to 17,039,360 bytes. Accepted materialization rewalks expectations, validates closed records, consumes each once, and deletes the transcript without another random search; known excess still builds no final list, random-map suffix, ExecutionIR, or report. Focused random/replay, million-attempt, and checking-state watchers cover call count, corruption, precedence, attempt 999,999, frozen identities, and 8,440/8,441/32,768 outcomes. Profile 23's guarded adapter accepts bind-plan cleanly with zero diagnostics. Reruns, limits, guards, timeouts, and claims stay unchanged; its archive is untouched and the clean 72-profile reseal remains pending.
