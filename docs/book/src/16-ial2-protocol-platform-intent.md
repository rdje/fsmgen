...
| AHB | Checked-in bounded requester, word-only subordinate, byte-lane/narrow-transfer subordinate, one-subordinate interconnect, selected two-subordinate interconnect, and selected generic aggregate byte-lane propagation `.ppif` samples; matching selected `.ahb` profile aliases for the non-aggregate-byte-lane surfaces; and direct `.fsm` seeds. | Bounded requester clauses for local command/status, AHB bus bindings, burst, transfer, and response behavior; bounded subordinate clauses for control, bus, one-register storage, selected NONSEQ/error transfer behavior, and selected byte/halfword/word little-endian lane behavior; selected one-requester static-window interconnect/decode shapes, including subordinate-owned byte-lane propagation through the selected generic aggregate `.ppif` sources. | AHB completer behavior, broader interconnect/decode beyond the selected one-subordinate and two-subordinate static-window aggregates, matching aggregate byte-lane `.ahb` aliases selected next but not shipped yet, optional subordinate signals, burst continuation, scoreboards, full AHB manager behavior beyond the bounded requester, direct backend behavior, verification-output generation, backend-language variants, and VHDL remain future task-tree-owned work. |
...
`.ppif` coverage, selected generic aggregate byte-lane propagation `.ppif`
coverage, matching selected `.ahb` profile aliases for the non-aggregate-byte-lane
surfaces, the older direct `.fsm` AMBA requester seed, and a direct lower-layer
