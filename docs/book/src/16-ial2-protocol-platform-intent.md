...
| AHB | Checked-in bounded requester, word-only and byte-lane subordinate families, one- and two-subordinate interconnects, selected aggregate byte-lane/in-word-`SEQ`/HBURST/BUSY-park/paired-BUSY `.ppif` samples, matching selected `.ahb` profile aliases, including aggregate HBURST and aggregate BUSY-park surfaces, and direct `.fsm` seeds. | Bounded requester local command/status, bus, burst, transfer, response, fixed-wrap, and selected BUSY-insertion behavior; bounded subordinate control, bus, one-register storage, selected byte/halfword/word lane behavior, selected in-word `SEQ`, byte-only HBURST `WRAP4`/`INCR4`, and BUSY parking; selected one-/two-subordinate static-window interconnect/decode shapes with subordinate-owned byte-lane, in-word `SEQ`, HBURST, and BUSY-park propagation through matching aggregate `.ppif`/`.ahb` surfaces, including paired BUSY-inserting-requester/BUSY-parking-subordinate compositions. | AHB completer behavior, broader interconnect/decode beyond the selected static-window aggregates, optional subordinate signals beyond the shipped HBURST endpoint binding, wider/indefinite HBURST continuation beyond the bounded byte-only `WRAP4`/`INCR4` sources, policy/runtime or multiple BUSY insertion, distinct bus-BUSY status, boundary-free active-transfer pipelining, scoreboards, full manager behavior beyond the bounded requester, direct backend behavior, verification-output generation, backend-language variants, and VHDL remain future task-tree-owned work. |
