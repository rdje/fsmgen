...
| AHB | Checked-in bounded requester, word-only subordinate, byte-lane/narrow-transfer subordinate, byte-lane in-word `SEQ` subordinate, byte-lane HBURST `WRAP4`/`INCR4` `SEQ` subordinate, one-subordinate interconnect, selected two-subordinate interconnect, selected aggregate byte-lane propagation `.ppif` samples, selected aggregate byte-lane in-word `SEQ` `.ppif` samples, selected aggregate HBURST-aware byte-lane `SEQ` `.ppif` samples, selected aggregate HBURST-aware byte-lane `SEQ` `.ppif` samples with BUSY-in-burst parking, matching selected `.ahb` profile aliases except the aggregate HBURST and aggregate BUSY-park aliases, and direct `.fsm` seeds. | Bounded requester clauses for local command/status, AHB bus bindings, burst, transfer, and response behavior; bounded subordinate clauses for control, bus, one-register storage, selected NONSEQ/error transfer behavior, selected byte/halfword/word little-endian lane behavior, selected byte/halfword in-word `SEQ` continuation, and selected byte-only HBURST `WRAP4`/`INCR4` in-word `SEQ` continuation; selected one-requester static-window interconnect/decode shapes, including subordinate-owned byte-lane propagation through the selected aggregate `.ppif` and `.ahb` sources, subordinate-owned byte/halfword in-word `SEQ` propagation through selected aggregate `.ppif` and `.ahb` sources, and subordinate-owned byte-only HBURST `WRAP4`/`INCR4` in-word `SEQ` propagation through selected aggregate `.ppif` sources, including the aggregate BUSY-park variants that hold each child's in-word `SEQ` burst context across an `HTRANS=BUSY` beat instead of clearing it. | AHB completer behavior, broader interconnect/decode beyond the selected one-subordinate and two-subordinate static-window aggregates, optional subordinate signals beyond the shipped HBURST endpoint binding, wider/indefinite HBURST burst continuation beyond the bounded endpoint and aggregate byte-only `WRAP4`/`INCR4` sources, matching aggregate HBURST `.ahb` aliases, scoreboards, full AHB manager behavior beyond the bounded requester, direct backend behavior, verification-output generation, backend-language variants, and VHDL remain future task-tree-owned work. |
