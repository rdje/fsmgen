...
| AHB | Checked-in bounded requester, word-only and byte-lane subordinate families, one- and two-subordinate interconnects, selected aggregate byte-lane/in-word-`SEQ`/HBURST/BUSY-park/paired-BUSY `.ppif` samples, matching selected `.ahb` profile aliases, including the exact-two requester, the generic plus matching `.ahb` one-subordinate exact-two paired sources, the generic two-subordinate exact-two paired source, aggregate HBURST and aggregate BUSY-park surfaces, and direct `.fsm` seeds. | Bounded requester local command/status, bus, burst, transfer, response, fixed-wrap, exact-one and exact-two BUSY insertion across generic `.ppif` and matching `.ahb` requester surfaces, and separated address/data ownership; bounded subordinate control, bus, one-register storage, selected byte/halfword/word lanes, in-word `SEQ`, byte-only HBURST `WRAP4`/`INCR4`, BUSY parking, and one accepted active-phase bank; selected one-/two-subordinate static-window interconnect/decode shapes with retained one-hot data-phase ownership and matching aggregate `.ppif`/`.ahb` surfaces, plus generic and matching `.ahb` one-subordinate exact-two requester/BUSY-parking compositions and the generic two-subordinate exact-two pairing. | AHB completer behavior, broader interconnect/decode beyond the selected static-window aggregates, optional subordinate signals beyond the shipped HBURST endpoint binding, wider/indefinite HBURST continuation beyond the bounded byte-only `WRAP4`/`INCR4` sources, BUSY counts beyond exact one/two, policy/runtime BUSY insertion, the matching two-subordinate exact-two `.ahb` alias, distinct bus-BUSY status, general/deeper queues, multiple outstanding transfers, scoreboards, full manager behavior beyond the bounded requester, direct backend behavior, verification-output generation, backend-language variants, and VHDL remain future task-tree-owned work. |
