...
top-level input pin, the sink is a resolved child input endpoint, the route is
the only vector pin-ingress route in the parent transaction, and the two
...
vector pin-ingress multi-route scheduling.
