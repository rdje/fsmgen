# AXI IAL2 Manager Three-Static Mixed Dynamic/Static Read-Data Burst-Length Behavior

Task-tree owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.333`
...
The sample extends the `.330` three-static mixed dynamic/static last-beat
read-data shape. It covers exactly one dynamic read transaction followed by
...
The generator now accepts this three-static mixed dynamic/static
burst-length boundary only when:

- `response-demux.read` is generated one-dynamic plus three-static mixed
...
- raw request-time ARLEN storage `axi0_r0_arlen_q`, `axi0_r1_arlen_q`,
  `axi0_r2_arlen_q`, and `axi0_r3_arlen_q`;
- request-guarded capture rules `axi0_r0_burst_length_capture`,
  `axi0_r1_burst_length_capture`, `axi0_r2_burst_length_capture`, and
  `axi0_r3_burst_length_capture`; and
- scalar last-beat `RDATA`/`RRESP` capture rules that remain guarded only by
  `axi0_r0_complete`, `axi0_r1_complete`, `axi0_r2_complete`, and
  `axi0_r3_complete`.
...
The report-only shape does not emit runtime beat-count storage,
expected-beat storage, ARLEN bound assertions, or RLAST timing assertions.
Runtime beat-count/`RLAST` validation over this three-static raw-`ARLEN`
boundary, multi-beat output banks, two-dynamic-plus-static shapes, broader
mixed dynamic/static cardinalities, same-cycle widening,
...
The implementation is covered by the support-accounted public sample,
focused dynamic transaction-ID test coverage, support-accounting coverage,
syntax checks, schedule/check/semantic/HDL probes, mdBook, Knowledge Map,
memory, diff, and doctrine gates recorded in the owning task-tree leaf.
