...
- and explicit-toplink actual sources plus source-side top-port bit/slice, source-side child-output bit/slice, and concat expressions now also land directly on realized child-input bindings or declared top-output assignments through that typed structural layer, so `=open`, `=0`, `=1`, exact-width `=N'b...`, `=N'd...`, `=N'o...`, `=N'h...`, declared-top forms such as `payload_bus[15:8]`, child-output forms such as `producer.payload[7:4]`, and bounded concat forms such as `header_bus,status_bus[0],=1,payload_bus[3:0]` no longer need fake carrier nets or fake same-name top-input inference just to reach the emitter,
