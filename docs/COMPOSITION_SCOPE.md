...
- composition generation results now also expose a user-facing provenance summary:
  ...
  - and it now also covers malformed `?ports` and `?toplink` parser items when top-port or top-link token flatness/shape/sizing would otherwise fail through older raw wording,
