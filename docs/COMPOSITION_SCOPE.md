...
- and composition tops themselves now also surface direct-result top-level `intent_hir` and `lowered_rtl_ir`, with `module_info` mirroring the same bounded top-port, child-count, lane, internal-net, instance, and auxiliary-assignment summaries,
