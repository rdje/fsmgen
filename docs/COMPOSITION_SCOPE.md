...
- source-side top-port expression forms are currently limited to `name[index]`, `name[msb:lsb]`, and flat comma-separated concat source forms whose operands are declared whole top-port references, top-port bit/slice expressions, one-bit scalar actuals `=0` / `=1`, or exact-width binary/decimal/octal/hex literal actuals,
