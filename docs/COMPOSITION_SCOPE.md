...
- declared aggregate top-port member/item operands such as `in_frame.tag` or `in_frame.payload[1]` also contribute exact declared leaf widths during explicit-toplink concat inference, so one remaining omitted whole top-port operand may be sized from the target remainder, but the aggregate root itself must already have a declared or previously inferred aggregate contract; broad aggregate autovivification from only `in_frame.tag` remains future work,
