...
- per-instance parameter/generic overrides now preserve validated values through `FSM::Composition::Instance`, `FSM::Composition::RealizedInstance`, structural RTL IR, and current SystemVerilog instance-parameter emission; symbolic `.rtlif` declaration defaults and symbolic override values are resolved after package imports and before planning, so unresolved names still abort before HDL emission; VHDL generic-map lowering and generated-child parameterization remain future follow-ups, not raw payload escape hatches,
