...
- `(?rtl:instance_name (module module_name) (params (NAME value) ...))` is the first shipped semantic parameter/generic override form; overrides must be scalar integer literals or bounded literal aggregate payloads, must name parameters/generics declared by the loaded `.rtlif` contract, and aggregate overrides must match the aggregate shape inferred from the `.rtlif` default value,
