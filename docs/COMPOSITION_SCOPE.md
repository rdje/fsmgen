...
- explicit `?toplink` top-port bit/slice expressions may currently appear only on the source side, must reference declared top inputs, and may target only realized child input ports,
