...
- composition-root `+constants` / `+enums` plus imported `?pkg:name` packages may currently provide named literal actuals such as `=RESET_BYTE`, `=BYTES[1]`, `=FRAME.flag`, `=mode.BUSY`, `=shared.RESET_BYTE`, `=shared.mode.BUSY`, `=shared.BYTES[1]`, or `=shared.FRAME.flag` for explicit `?toplink` direct actuals and concat operands only; aggregate-backed local or imported symbols must resolve all the way to a scalar leaf, and this still does not open a broader typed source-expression, whole-aggregate, or general symbol-resolution lane,
