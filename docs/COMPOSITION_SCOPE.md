...
- `(?rtl:instance_name (module module_name) (params (NAME value) ...))` is the first shipped semantic parameter/generic override form; overrides must be scalar integer literals, bounded literal aggregate payloads, or resolved composition-top/imported-package symbols, must name parameters/generics declared by the loaded `.rtlif` contract, and aggregate overrides must match the aggregate shape inferred from the `.rtlif` default value,
- the shipped `.rtlif` mini-contract is one `(?rtlif:module_name ...)` root with declaration-ordered port tokens such as `clk`, `data_in<8`, `txd>`, `core_clk:clock`, and `rst_async_n:reset`, plus one optional `(params (NAME default_value) ...)` declaration block for external RTL parameters/generics; declaration defaults may use literals or package-qualified symbols from packages imported by the consuming composition source, but deliberately do not resolve unqualified top-local names,
