...
- the shipped `.rtlif` mini-contract is one `(?rtlif:module_name ...)` root with declaration-ordered port tokens such as `clk`, `data_in<8`, `txd>`, `core_clk:clock`, and `rst_async_n:reset`, plus one optional `(params (NAME default_value) ...)` declaration block for external RTL parameters/generics; declaration defaults may use literals, bounded scalar expressions, matching-shape leafwise aggregate expressions, or package-qualified symbols from packages imported by the consuming composition source, but deliberately do not resolve unqualified top-local names,
- explicit `.rtlif` type annotations are currently limited to `data`, `clock`, and `reset`,
- typed `.rtlif` `clock` / `reset` tokens let custom-named RTL system ports auto-wire through composition without reviving broader hidden inference, and those system-role annotations are input-only, so `core_clk>:clock` / `rst_async_n>:reset` are rejected while ordinary `data` outputs such as `txd>:data` remain valid,
