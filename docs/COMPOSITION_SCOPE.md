...
- per-instance parameter/generic overrides are a planned semantic instantiation contract, not a current raw payload escape hatch; they must preserve typed override values into IR and lower to target-specific SystemVerilog parameters or VHDL generics only after validation,
