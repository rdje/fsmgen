...
- Work unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.677`
- Date: `2026-06-28`
- Status: selected
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.678` to directly implement exactly these
public sources:
...
`apb_composition_multi_peripheral_multi_register_sideband_protection_generalized_five_register_status_back_to_back`.
The selected source object is
`fsmgen-apb-composition-multi-peripheral-multi-register-sideband-protection-generalized-five-register-status-back-to-back`.
The selected source anchor section is
`requester-multi-peripheral-composition-multi-register-sideband-protection-generalized-five-register-status-back-to-back`.
...
This selection read `.676` selector, `.675` data16 no-policy five-register
behavior, `.672` 32-bit no-policy five-register behavior, `.665` 32-bit
protected generalized behavior, `.668` data16 protected generalized behavior,
`.670` cardinality audit, current `ApbCompleter` and `ApbComposition`
protected generalized predicates and residue, `RegressionCorpus`,
...
The selected direction is the smallest protected cardinality sibling after
the no-policy five-register pair shipped for both 32-bit and data16. Current
32-bit protected generalized helpers still use `maximum_count = 4`; the
matching 32-bit no-policy helpers already use `maximum_count = 5`.

## Selected Public Source Contract

The `.678` implementation shall keep the existing selected 32-bit
...
The admitted 32-bit protected generalized family after `.678` shall be:
...
The public representative selected for `.678` shall use five registers per
peripheral:
...
32-bit sideband-aware protected generalized two-peripheral family from
...
The selected policy matrix is inherited from the shipped `.665` protected
generalized family:
...
remove broad cardinality residue for this selected 32-bit protected
...
`.678` must keep diagnostics fail-closed. The selected widening may accept
only the bounded 32-bit sideband-aware protected generalized register-set
...
- selected 32-bit protected generalized storage with fewer than two or more
  than five registers;
...
- data16 protected five-register generalized register sets;
- no-policy five-register generalized register sets carrying any
...
`.678` closeout validation must cover:
...
- malformed storage diagnostics for wrong `reg4` address, wrong register
  ...
  missing `access-policy`, wrong `reg4` policy, data16 protected
