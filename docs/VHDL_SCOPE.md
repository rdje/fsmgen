...
- The direct scaffold now includes the first arithmetic RHS expression family:
  same-width vector addition and subtraction chains lower through `numeric_std`
  unsigned casts.
...
3. **Supported constructs in the first lane:**
   ...
   - Same-width vector addition and subtraction RHS chains in the generated
     direct combinational mux shape
...
- Shipped for the current scaffold subset:
  ...
  and subtraction RHS expression chains.
...
- Focused direct VHDL generation tests cover pipeline and CLI routing,
  ...
  same-width vector addition/subtraction-chain lowering, broader
