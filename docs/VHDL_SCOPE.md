...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.125.1` owns only direct VHDL
  non-signed vector positive numeric-literal modulo lowering after selection
  proved literal-first modulo still fails closed; broad VHDL expression parity,
  aggregate record/array lowering, package emission, GHDL validation,
  composition parity, and full backend parity remain outside this
  implementation until an exact child owns them.
