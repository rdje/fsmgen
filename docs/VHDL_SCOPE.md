...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.117` owns only the next
  backend/API edge selection after signed vector negative numeric-literal
  division lowering shipped; unsigned negative numeric-literal arithmetic,
  signed negative numeric-literal modulo, broad VHDL expression parity,
