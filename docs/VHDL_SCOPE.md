...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.129.1` owns only direct VHDL
  non-signed vector positive numeric-literal multiplication with two literal
  operands after selection proved literal-literal division/modulo still fail
