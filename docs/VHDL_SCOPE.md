...
- The direct scaffold now includes signed vector numeric-literal
  ...
  subtraction, multiplication, division, and modulo subsets now also include
  negative decimal numeric literals for signed vector targets and same-width
  signed vector operands.
- The direct scaffold now includes non-signed vector negative numeric-literal
  addition/subtraction/multiplication/division/modulo RHS assignments through
  unsigned arithmetic over target-width two-complement literals, with
  multiplication/division/modulo resized back to the target width.
...
- The direct scaffold now includes non-signed vector output-port next-signal
  assignments from negative decimal literals, lowered through target-width
  `std_logic_vector(to_signed(NEGATIVE_VALUE, WIDTH))`.
...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.127.1` owns only direct VHDL
  non-signed vector positive numeric-literal division with a literal-first
  operand order after selection proved literal-first modulo and literal-literal
  arithmetic still fail closed; broad VHDL expression parity, aggregate
  record/array lowering, package emission, GHDL validation, composition parity,
  and full backend parity remain outside this implementation until an exact
  child owns them.
...
- Bounded direct vector output-port decimal literal assignments now lower
  ...
  parity.
- Bounded direct signed vector output-port decimal literal assignments now
  lower through target-width `to_signed`; this is not broad expression-literal
  parity.
...
1. **Direct-root scaffold plus four bounded composition structural tops**
   ...
   - C2 generated-FSM aggregate actuals that do not lower to one packed literal
     are locked fail-closed by `BACKEND-API-VALIDATION-FRONTIER.99.1`; this is
     not record/array VHDL generic support
...
3. **Composition structural emission from typed structural RTL IR**
   ...
   - Reject generated-FSM child instances outside exact shipped or active
     ...
     assignments
   - Declared aggregate structural VHDL ports/nets/types are locked fail-closed
     by `BACKEND-API-VALIDATION-FRONTIER.101.1`; this is not VHDL record/array
     declaration support
...
4. **Supported constructs in the first lane:**
   ...
   - Non-signed vector positive numeric-literal division for the signal-first
     decimal RHS shape, such as `A / 2`
   ...
   - Typed read-only non-signed four-state `input logic` and
     `input logic [MSB:LSB]` direct-root ports, lowered as `std_logic` /
     `std_logic_vector`
...
5. **Intentionally deferred:**
   ...
   - VHDL external validation (GHDL); blocker reconfirmed by
     `BACKEND-API-VALIDATION-FRONTIER.102.1` because `ghdl` is unavailable
...
- Shipped for the current scaffold subset:
  ...
  positive numeric-literal multiplication/division RHS assignments, generated direct-root
...
- Focused direct VHDL generation tests cover pipeline and CLI routing,
  ...
  vector output-port negative decimal literal assignment lowering,
  ...
  scalar output-port negative decimal literal assignment lowering,
  ...
  non-signed vector positive numeric-literal multiplication/division RHS lowering,
  ...
  broader composition/top VHDL fail-closed diagnostics, and declared aggregate
...
- Consider external VHDL validation via GHDL; the current environment blocker is
  reconfirmed by `BACKEND-API-VALIDATION-FRONTIER.102.1`
