...
- The direct scaffold now includes signed vector numeric-literal
  ...
  subtraction, multiplication, and division subsets now also include negative
  decimal numeric literals for signed vector targets and same-width signed
  vector operands.
...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.117.1` owns only direct VHDL
  signed vector negative numeric-literal modulo lowering after `.117`
  selected it from the adjacent fail-closed arithmetic probes; unsigned
  negative numeric-literal arithmetic, broad VHDL expression parity,
