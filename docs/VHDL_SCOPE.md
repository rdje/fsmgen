...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.124.1` owns only direct VHDL
  non-signed vector positive numeric-literal division lowering after selection
  proved positive numeric-literal modulo and literal-first multiplication still
  fail closed; broad VHDL expression parity, aggregate record/array lowering,
