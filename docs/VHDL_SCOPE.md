...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.128.1` owns only direct VHDL
  non-signed vector positive numeric-literal modulo with a literal-first operand
  order after selection proved literal-literal arithmetic still fails closed;
