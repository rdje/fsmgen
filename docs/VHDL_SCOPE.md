...
- The direct scaffold now includes non-signed vector negative numeric-literal
  addition RHS assignments through unsigned arithmetic over target-width
  two-complement literals.
...
- The direct scaffold now includes scalar output-port next-signal assignments
  ...
  plain scalar and signed one-bit alias targets.
- Active leaf `BACKEND-API-VALIDATION-FRONTIER.119.1` owns only direct VHDL
  non-signed vector negative numeric-literal subtraction lowering after `.119`
  selected it from adjacent fail-closed arithmetic probes; non-signed vector
  negative numeric-literal multiplication/division/modulo, broad VHDL
  ...
  leaf until an exact child owns them.
...
- Focused direct VHDL generation tests cover pipeline and CLI routing,
  ...
  non-signed vector negative numeric-literal addition RHS lowering,
