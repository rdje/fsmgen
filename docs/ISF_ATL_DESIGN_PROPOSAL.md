...
multi-route extension, bounded same-child mixed scalar/vector pin-ingress
route-set extension, scalar pin-egress data route, one exact-width vector
pin-egress route, bounded same-child scalar pin-egress multi-route extension,
bounded same-child vector pin-egress multi-route extension, and bounded
same-child mixed scalar/vector pin-egress route-set extension described below.
Interface binding beyond that selected
...
The exact-width vector version of that pin-ingress step is now shipped through
`isf/atl_resolved_child_pin_ingress_vector_pipeline.isf`. It uses the same
drive-body route spelling but requires the top-level input pin and the
resolved child input endpoint to declare the same positive width. The shipped
fixture uses width 8 and reports `kind: "vector_pin_to_actor_handoff"` with
`width_source: "top_level_input_pin_resolved_child_endpoint_exact_width"`.
The generated top wires public top `payload` into the parent, then wires the
parent handoff `worker_payload` into child `payload` at that exact width.
Mismatched widths fail closed before scheduled `.fsm` emission. This vector
leaf does not claim mixed scalar/vector route sets, width adaptation, packing,
...
pin-ingress path is now shipped through
`isf/atl_resolved_child_pin_ingress_vector_multi_pipeline.isf`. It routes
`(worker.payload pins.payload)` and `(worker.sideband pins.sideband)` with
adjacent argument-free drive calls before the child trigger. Each route proves
its own top-level input pin and resolved child input endpoint width; the
fixture uses route-local widths 8 and 4. The generated top wires both vector
input pins through separate parent sink handoffs to separate child inputs, and
the public report keeps both routes in `actor_network.data_movements[]` as
`vector_pin_to_actor_handoff` entries with
...
pin-ingress subset described next, width adaptation, route mux/storage,
fan-in/fan-out, CDC/reset remapping, ready/backpressure, and payload protocols
remain deferred.

The mixed scalar/vector extension of that same one-child pin-ingress path is
now shipped through
`isf/atl_resolved_child_pin_ingress_mixed_pipeline.isf`. It routes
`(worker.payload pins.payload)` as an exact-width vector path at width 8 and
`(worker.valid pins.valid)` as a scalar one-bit path through adjacent
argument-free drive calls before the child trigger. The generated top wires
both top-level input pins through separate parent sink handoffs to separate
child inputs. The public report keeps both routes in
`actor_network.data_movements[]`, preserving route-local `kind`, `width`, and
`width_source`: vector payload uses
`top_level_input_pin_resolved_child_endpoint_exact_width`, while scalar valid
uses `top_level_pin_scalar_one_bit`. Width adaptation, route mux/storage,
fan-in/fan-out, CDC/reset remapping, ready/backpressure, and payload protocols
remain deferred.

The bounded multi-route extension of that same one-child pin-ingress path is
also shipped through `isf/atl_resolved_child_pin_ingress_multi_pipeline.isf`.
It routes `(worker.payload pins.payload)` and
`(worker.sideband pins.sideband)` with adjacent argument-free drive calls before
...
The exact-width vector version of that pin-egress step is now shipped through
`isf/atl_resolved_child_pin_egress_vector_pipeline.isf`. It uses the same
drive-body route spelling but requires the resolved child output endpoint and
top-level output pin to declare the same positive width. The shipped fixture
uses width 8 and reports `kind: "vector_actor_to_pin_handoff"` with
...
`width_source: "top_level_output_pin_resolved_child_endpoint_exact_width"`.
Broader mixed scalar/vector route sets outside the bounded same-child
pin-egress subset described next, width adaptation, route mux/storage,
fan-in/fan-out, CDC/reset remapping, ready/backpressure, and payload protocols
remain deferred.

The mixed scalar/vector extension of that same one-child pin-egress path is now
shipped through
`isf/atl_resolved_child_pin_egress_mixed_pipeline.isf`. It routes
`(pins.result worker.payload)` as an exact-width vector path at width 8 and
`(pins.valid worker.valid)` as a scalar one-bit path through adjacent
argument-free drive calls after the child event wait. The generated top wires
both child outputs through separate parent source handoffs to separate
top-level output pins. The public report keeps both routes in
`actor_network.data_movements[]`, preserving route-local `kind`, `width`, and
`width_source`: vector result uses
`top_level_output_pin_resolved_child_endpoint_exact_width`, while scalar valid
uses `top_level_output_pin_scalar_one_bit`. Width adaptation, route
...
The ATL resolved-child exact-width vector pin-ingress generated-top fixture is
shipped as `isf/atl_resolved_child_pin_ingress_vector_pipeline.isf`. It keeps
one resolved `worker` child and one trigger/event pair, adds one top-level
input pin `payload` at width 8, and routes it into child input `payload` at the
same width. Lowering emits parent, child, and generated top `.fsm` artifacts,
reports the route through `actor_network.data_movements[]` with
`vector_pin_to_actor_handoff`, preserves child `payload` as an 8-bit input
port, and wires the exact-width parent handoff to the child in the generated
top. This fixture still does not claim mixed scalar/vector route sets, width
adaptation, route mux/storage, fan-in/fan-out, CDC/reset remapping,
ready/backpressure, payload protocols, recursive actor networks, or permanent
actor grouping.

The ATL resolved-child exact-width vector pin-ingress multi-route generated-top
fixture is shipped as
`isf/atl_resolved_child_pin_ingress_vector_multi_pipeline.isf`. It keeps one
resolved `worker` child and one parent transaction, then routes
`(worker.payload pins.payload)` at width 8 and
`(worker.sideband pins.sideband)` at width 4 through adjacent argument-free
drive calls before the child trigger. Lowering emits parent, child, and top
`.fsm` artifacts, reports both routes as `vector_pin_to_actor_handoff`,
preserves both child vector inputs as module ports, and wires each route at
its route-local exact width.

The ATL resolved-child mixed scalar/vector pin-ingress generated-top fixture is
shipped as `isf/atl_resolved_child_pin_ingress_mixed_pipeline.isf`. It keeps
one resolved `worker` child and one parent transaction, then routes
`(worker.payload pins.payload)` at width 8 and `(worker.valid pins.valid)` at
width 1 through adjacent argument-free drive calls before the child trigger.
Lowering emits parent, child, and top `.fsm` artifacts, reports the routes as
`vector_pin_to_actor_handoff` and `scalar_pin_to_actor_handoff`, preserves both
child inputs as module ports, and wires each route at its route-local width.

The bounded multi-route extension of that same one-child pin-ingress fixture is
shipped as `isf/atl_resolved_child_pin_ingress_multi_pipeline.isf`. It keeps one
resolved `worker` child and one parent transaction, then routes
`(worker.payload pins.payload)` and `(worker.sideband pins.sideband)` through
adjacent argument-free drive calls before the child trigger. Lowering emits
separate drive states, drive request signals, generated parent sink handoffs,
child `+interface` roles for both routed inputs, and generated-top wiring for
...
The ATL resolved-child exact-width vector pin-egress generated-top fixture is
shipped as `isf/atl_resolved_child_pin_egress_vector_pipeline.isf`. It keeps
one resolved `worker` child and one trigger/event pair, adds one top-level
output pin `result` at width 8, and routes child output `payload` into it at
the same width. Lowering emits parent, child, and generated top `.fsm`
...
not claim width adaptation, route mux/storage, fan-in/fan-out, CDC/reset
remapping, ready/backpressure, payload protocols, recursive actor networks, or
permanent actor grouping.

The ATL resolved-child mixed scalar/vector pin-egress generated-top fixture is
shipped as `isf/atl_resolved_child_pin_egress_mixed_pipeline.isf`. It keeps
one resolved `worker` child and one parent transaction, then routes
`(pins.result worker.payload)` at width 8 and `(pins.valid worker.valid)` at
width 1 through adjacent argument-free drive calls after the child event wait.
Lowering emits parent, child, and top `.fsm` artifacts, reports the routes as
`vector_actor_to_pin_handoff` and `scalar_actor_to_pin_handoff`, preserves both
child outputs as module ports, and wires each route at its route-local width.
...
The generated-child one-child top widens that pin-to-actor movement only for
one exact-width vector route into a resolved child. The source spelling stays
`(actor.endpoint pins.input_pin)`, but `pins.input_pin` and the resolved child
input endpoint must have the same positive width. The report kind is
`vector_pin_to_actor_handoff`, the `width` is the matched endpoint width, and
the `width_source` is
`top_level_input_pin_resolved_child_endpoint_exact_width`. It is not a general
direct-static-actor pin movement widening and does not ship vector
pin-ingress multi-route sets by itself; the bounded same-child vector
pin-ingress multi-route subset is a separate shipped generated-child
extension documented below.
...
The generated-child one-child top widens that actor-to-pin movement only for
one exact-width vector route from a resolved child. The source spelling stays
`(pins.output_pin actor.endpoint)`, but the resolved child output endpoint and
`pins.output_pin` must have the same positive width. The report kind is
...
Focused coverage rejects extra `(on ...)` boundaries before the source
