...
- Work unit: `IAL2-FEATURE-COMPLETENESS-FRONTIER.671`
- Date: `2026-06-28`
- Status: selected
- Scope: public contract selection only for the first bounded APB
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.672` to directly implement exactly these
public sources:
...
`apb_composition_multi_peripheral_multi_register_sideband_generalized_five_register_status_back_to_back`.
The selected source object is
`fsmgen-apb-composition-multi-peripheral-multi-register-sideband-generalized-five-register-status-back-to-back`.
The selected source anchor section is
`requester-multi-peripheral-composition-multi-register-sideband-generalized-five-register-status-back-to-back`.
...
This selection read the `.670` cardinality readiness audit, `.669` selector,
`.668` data16 protected generalized behavior, `.667` data16 protected
generalized contract, `.665` 32-bit protected generalized behavior, `.662`
data16 no-policy generalized behavior, `.660` 32-bit no-policy generalized
behavior, `.659` source-shape contract, current `ApbCompleter` and
...
The selected first cardinality widening stays on the 32-bit no-policy path
because that path isolates register count from data16 stride/strobe changes
and protection-policy matrix changes.

## Selected Public Source Contract

The `.672` implementation shall keep the existing selected 32-bit
...
The admitted 32-bit no-policy generalized family after `.672` shall be:
...
The public representative selected for `.672` shall use five registers per
peripheral:
...
`.672` must keep diagnostics fail-closed. The selected widening may accept
...
`.672` closeout validation must cover:
...
- malformed storage diagnostics for wrong `reg4` address, wrong register
  ...
  register-local `access-policy`, data16 five-register, protected
  five-register, more-than-five-register, and more-than-two-peripheral
  variants;
