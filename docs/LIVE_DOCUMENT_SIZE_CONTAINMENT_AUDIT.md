...
current audience and canonical role. The completed
[utility/retirement audit](LIVE_DOCUMENT_UTILITY_RETIREMENT_AUDIT.md) selects
retain/merge/supersede/archive/delete/re-form semantics for every governed
family, identifies completed guide-migration and verbose harness-bootstrap
representations, assigns separate atomic owners, and deletes nothing in `.16`.
...
| `ROADMAP_V2.md` | baseline 10,351 lines / 764,316 bytes; `.9` activation source 10,451 / 772,074 / longest 2,297; post-`.9` live view 317 / 14,606 / longest 182 | post-`.9` peak 49.5% of derived 640-line / 64-KiB / 512-line-byte bounds | normal migrated snapshot; current/future direction, dependencies, concise milestone outcomes, and horizons remain live while the exact activation object is descriptor-backed history |
...
| Document utility and retirement | Before a family is partitioned, classify its current audience, canonical role, unique value, duplication, staleness, and historical retention; select retain, merge, supersede, archive, delete, or re-form with evidence | Completed `.16` audits all 22 families, assigns existing owners plus `.24`/`.25`, requires fail-closed deletion proof, and deletes nothing |
...
| Change and rationale ledgers | Decision 0046 selected both roles, but before migration the director reopened whether `CHANGES.md` and analogous agent-era crash/restart documents retain present value. The `.3` registry supports bounded ledger topology, while source/test inspection leaves post-cutover append anchoring unproved | `.4` is deferred with no migration; pending `.26` first inventories the broader legacy-continuity cohort without inferring candidates or outcomes. `.5`/`.11` must not substitute for either review |
...
| Roadmap | Keep current/future direction and concise milestone outcomes live; prove old shipped narration against the book, task records, and exact history, then delete-with-link or retain only verified archival descriptors | `.9` complete: 317-line strategic view plus exact 10,451-line activation-source descriptor and executed retrieval verifier |
