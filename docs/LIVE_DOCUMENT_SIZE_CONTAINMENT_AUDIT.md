...
| Change and rationale ledgers | Decision 0046 retains both roles. The `.3` registry now binds each opted-in ledger to one bounded current file and index, contiguous whole-entry ranges, exact source/entry reconstruction, content-addressed seals, and finite live-range/line/byte limits before descriptor-backed archival | `.3` ships the neutral schema with no ledger record or migration; `.4`/`.5` alone opt in and migrate `CHANGES.md`/conditional `DEVELOPMENT_NOTES.md` |
