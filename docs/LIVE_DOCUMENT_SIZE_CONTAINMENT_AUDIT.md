...
| Task evidence and index | Each active root keeps metadata plus the live ancestor/frontier. Decision 0042 and `.6` now define a finite JSONL manifest whose content-addressed Markdown segments must match complete terminal subtrees at an exact source revision; compact `version_object` terminals must retrieve, digest-check, count, and validate the full archived subtree. The cross-tree live index remains unchanged in this contract slice | `.6` ships the optional schema/checker without migrating a tree; `.7` migrates the active IAL2 outlier, completed outliers, and cross-tree index |
