...
It composes the shipped exact-three requester with the shipped one-subordinate
byte-lane/HBURST-SEQ/BUSY-parking endpoint through the existing three-child
`ahb_tb` architecture. Proposed leaf
`IAL2-AHB-EXACT-THREE-PAIRED-BUSY-COMPOSITION-READINESS-AUDIT.3` owns the
...
source object:
  fsmgen-ahb-interconnect-requester-busy-insert-three-byte-lane-hburst-seq-busy-park

anchor:
  document = ARM-AMBA-AHB-IHI0033-C-2021-09
  section  = bounded-ahb-interconnect-requester-busy-insert-three-byte-lane-hburst-seq-busy-park
...
The long identity distinguishes exact-three requester cardinality from the
existing exact-one and exact-two pairings. It selects source data through the
existing generators, not a fourth AHB generator or a new report schema.
...
1. source, intent, object, and anchor identities replace `two` with `three`;
2. embedded requester object becomes `amba_requester_busy_insert_three`;
3. requester transfer uses `(busy-beats 3)` beside the unchanged
...
corresponding exact-two clauses.

Only the generic `.ppif` source ships in `.3`. A matching `.ahb` alias and the
two-subordinate exact-three topology require separate contract, accounting,
...
children[requester].object_name                   = amba_requester_busy_insert_three
children[requester].busy_insertion.before_beat    = 2
children[requester].busy_insertion.beats          = 3
children[subordinate].transfer.seq_policy.parks_on = [busy]
composition.seq_policy_propagation...parks_on      = [busy]
```

The requester retains a width-two `ahb_busy_remaining_q`; rules load three,
decrement greater-than-one events, clear at one, and resume pending `SEQ`,
giving `3 -> 2 -> 1 -> 0`. No aggregate-level `busy_flow` duplicate is added.
The requester owns insertion cardinality; the subordinate and propagation
reports own BUSY parking. Existing generic alias residue and substantive AHB
residue remain truthful until their own slices.
...
- schedule JSON exposes the source identity, exact artifacts, numeric
  `before_beat=2`/`beats=3`, one-hot response ownership, and child/propagated
  BUSY parking;
...
t/data/ahb_exact_three_paired_busy_composition_tb.svt
```

t1531 owns source identity and structure; strict check and exact support;
schedule/report/artifact assertions; width-two counter structure; normalized
semantic JSON; real read-only MCP; repo-local outdir; `--verify-hdl`; and one
generated-HDL runtime compiled without `--no-assert`.
...
At qualified BUSY events the requester counter must be 3, 2, then 1 and must
be zero at resumed `SEQ`. Address/control/write-data and beat counters,
...
Current accounting is 322 protocol paths, 363 supported-smoke paths, 363
strict paths, and 46 AHB paths split 23 `.ppif` / 23 `.ahb`. The selected one
generic source projects:

```text
protocol paths:          323
supported-smoke paths:   364
strict-supported paths:  364
AHB IAL2 paths:           47
  generic .ppif:          24
  .ahb aliases:           23
```
...
Focused preservation must include the assertion-clean direct seed, generic and
alias exact-two paired one-/two-window families, standalone exact-three
requester and alias, fabric/endpoint arbitration owners, current-book truth,
support/capability, normalized semantic/MCP, and doctrine gates. Existing
owners t1520, t1523-t1526, and t1528-t1530 remain unchanged; `.3` may choose the
smallest evidence-complete subset while t1531 owns the new runtime.
...
guard occupancy is never capacity truth.

Contract closeout measured 58.4% Stats-compatible capacity
(14.02/24.00 GiB) with kernel pressure `1` (normal). The synchronized
Knowledge Map contains 1,023 facts / 5,206 question keys; mdBook generated 72
files / 16,132,266 bytes and that output was removed without residue. All six
doctrine gates pass.
...
Rollback removes those exact additions and restores 322/363/46 accounting.
All existing requester, subordinate, interconnect, paired, semantic/MCP,
backend, HIAL/VIAL, and VHDL behavior remains intact.

## Explicit Deferrals

The matching `.ahb` alias, two-subordinate exact-three composition, BUSY counts
above three, counter-width generalization, runtime/policy/random/multiple-point
insertion, distinct bus-BUSY status, larger/indefinite bursts, optional AHB
signals, queues/outstanding transfers, managers, broader fabrics, generic
priority changes, direct backends, AXI/APB expansion, HIAL/VIAL activation,
verification-output generation, backend variants, VHDL, and decision 0020
remain separate task-tree-owned work.
