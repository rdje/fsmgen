# AXI IAL2 Manager Multiple Mixed Dynamic/Static Read-Data Runtime-Validation Readiness Audit

Task-tree owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.311`
...
- `.310` multiple mixed dynamic/static report-only raw-`ARLEN` burst-length
  behavior and its public sample.
- `.309` multiple mixed dynamic/static report-only raw-`ARLEN` readiness
  audit.
...
- `.299` multiple mixed dynamic/static read single-beat `RID` response-demux
  behavior.
- `.289` one-dynamic plus one-static mixed runtime beat-count/`RLAST`
  validation behavior.
...
The `.310` report-only implementation already proves the exact multiple mixed
dynamic/static last-beat source shape:

- generated multiple mixed dynamic/static read burst-last response-demux;
- exactly one transaction-local dynamic read transaction and exactly two
  concrete static read transactions;
- complete ordered `r0`, `r1`, and `r2` read-data transaction bindings;
- request-time raw-`ARLEN` storage and capture rules per covered transaction;
  and
- scalar `RDATA`/`RRESP` capture guarded by generated multiple mixed
  `RID && RLAST` completion pulses.

The generic runtime-validation machinery used by `.289` and `.264` is already
transaction-list driven. Once the multiple mixed last-beat read-data coverage
branch admits `validation runtime-assertion`, the common normalizer and
generation helpers derive:
...
For the multiple mixed read demux, the matched-beat expression resolves through
the response-demux transaction states already emitted by `.303` and consumed
by `.307` and `.310`: the dynamic transaction matches raw accepted read beats
by captured dynamic `RID`, while each static transaction matches raw accepted
read beats by its reserved concrete `RID`. The matched-beat expression does
not include `RLAST`; the runtime assertions check whether `RLAST` appears
exactly on the expected final beat.

Because the public `burst-length` syntax and runtime report vocabulary are
already shipped by earlier runtime slices, a separate public contract-selection
leaf would not add clarity. The next owner can be a direct behavior slice whose
main code change is widening the multiple mixed last-beat coverage predicate
from report-only to report-only or runtime-assertion, while preserving the
existing fail-closed single-beat and multi-beat boundaries.

## Selected .312 Boundary

`.312` should implement only the runtime-validation sibling of the `.310`
report-only shape:
...
- scalar last-beat `RDATA`/`RRESP` capture remains guarded only by generated
  multiple mixed `RID && RLAST` completion pulses.

The public sample should be the runtime sibling of `.310`:
...
`.312` should preserve fail-closed diagnostics for:
...
last-beat burst-length metadata for the multiple mixed dynamic/static branch.

## Validation Gates For .312
...
- guarded direct schedule/check/semantic/verify-HDL probes for the new runtime
  ...
  before execution;
- preservation probes for the `.310` report-only sample;
...
- focused fail-closed probes for single-beat burst-length and multi-beat
  shapes;
...
variants, and VHDL remain separate exact owners.

## Rollback

Rollback is the `.311` docs-only audit commit. Reverting it restores `.311` as
the active runtime-validation readiness audit and removes the `.312` direct
