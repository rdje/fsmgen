# AXI IAL2 Manager Three-Static Mixed Dynamic/Static Read-Data Runtime-Validation Behavior

Task-tree owner: `IAL2-FEATURE-COMPLETENESS-FRONTIER.335`
...
This sample is the runtime sibling of the `.333` report-only sample. It keeps
the same one dynamic plus three concrete static read transaction set,
generated mixed `RID && RLAST` response-demux, scalar last-beat
`RDATA`/`RRESP` capture, and request-captured raw `ARLEN` metadata, changing
only `burst-length.validation` to `runtime-assertion`.
...
- `read-data.read` with `capture-scope last-beat`, `completion-source
  ...
  last-beat-by-rid`;
- `read-data.read.transactions` covers the ordered `r0`, `r1`, `r2`, and
  `r3` transaction set exactly once; and
...
transaction matches its reserved concrete `RID`. The matched-beat counter
path is not gated by `RLAST`; the runtime assertions check whether `RLAST`
appears exactly on the expected final beat.
...
The public four-transaction sample therefore reports sixteen generated
...
The `.333` report-only sample remains support-accounted and continues to emit
raw `ARLEN` capture without expected-beat storage, beat counters, beat-count
rules, or runtime assertions.

`.335` selects `.336`, readiness audit for generated three-static mixed
...
checks, schedule/check/semantic/HDL probes, mdBook, Knowledge Map, memory,
diff, and doctrine gates recorded in the owning task-tree leaf.
