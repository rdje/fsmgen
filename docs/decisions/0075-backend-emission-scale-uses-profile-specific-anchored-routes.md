...
with no artifact graph. The catalog repair remains owned by the final sibling.
...
- **Durability gap:** all four backend behavior repairs now have RED controls
  and continuing exact watchers. The decision, matching Knowledge Map facts,
  active task-tree owner, and synchronized mdBook retain catalog alignment and
