...
checks passing and retains the exact atomic `T=29,509` rejection. Items 2-4 and
the catalog repair remain owned by the ordered sibling leaves.
...
- **Durability gap:** the portable-VHDL repair now has RED controls and an exact
  continuing boundary watcher. The decision, matching Knowledge Map fact,
  active task-tree owner, and synchronized mdBook retain the other selected
  routes; their repair watchers and the canonical generator remain active
  `.17.2.6` work before every figure becomes a durable derived gate.
