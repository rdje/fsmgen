...
portable evidence publishes no wrapper artifacts. Item 4 plus the catalog
repair remain owned by the ordered sibling leaves.
...
- **Durability gap:** the portable-VHDL, sealed OSVVM-provider, and translated
  OSVVM-map repairs now have RED controls and continuing exact watchers. The
  decision, matching Knowledge Map facts, active task-tree owner, and
  synchronized mdBook retain native-UVM negotiation, catalog alignment, and
  the canonical generator as active `.17.2.6` work before every figure becomes
  a durable derived gate.
