...
| [0025](0025-project-document-interim-lifecycle.md) | Interim lifecycle split for project documents | convention | `CHANGES.md` is per-slice; `DEVELOPMENT_NOTES.md` is rationale-only when warranted; `ROADMAP_STATUS.md` and `LIVE_ACHIEVEMENT_STATUS.md` stay frozen pending review. |
...
| [0028](0028-source-facing-hir-is-a-distinct-private-pre-ial-layer.md) | Source-facing HIR is a distinct private pre-IAL layer | architecture | Adds a private `SourceHIR` above IAL2/IAL1, first rendered byte-for-byte to the shipped valid-ready `.ppif` path without changing `IntentHIR` or exposing a public builder. |
