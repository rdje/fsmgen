...
| [0014](0014-protocol-platform-intent-surface-and-layered-lowering.md) | Protocol/platform intent surface and layered lowering for IAL2 | architecture | Future IAL2 must use a protocol/platform-generic file surface; `.pif`, `.ppi`, and `.ppif` are open candidates; protocol-specific extensions such as `.axi` are rejected; direct IAL2 → IAL0 lowering is forbidden. |
