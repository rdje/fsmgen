...
contract selection, without changing behavior. The audit found that current
APB parser/generator/report support remains fixed to one requester, one
completer, and one composition object, with no selected peripheral list,
address map, decode priority, response mux, diagnostics, report schema, or
sample/support/test contract for true multi-peripheral topology. The selected
direction is APB-specific: IAL2 source will describe APB topology/address-map
intent and parameter/generic-like instantiation bindings, then lower into a
...
(queue-depth 1) (overflow reject))` under `(transfer ...)`, requiring public
`accepted`, `busy`, and 2-bit `status` response fields. The selected completer
vocabulary is `(timing-policy (setup-admission adjacent))`. The first
implementation family is status-observable requester-transfer, one-register
completer, and fixed one-requester/one-completer composition, each with
`.ppif` and `.apb` coverage. Multi-peripheral propagation,
...
`.609` now ships that selected bounded APB multi-peripheral status
back-to-back family for
`ppif/apb_composition_multi_peripheral_status_back_to_back.ppif` and `.apb`.
The selected shape is 32-bit, no-sideband, exactly two peripheral completers,
and the existing static non-overlapping address-map/decode contract. The
requester reuses the `.607` depth-1 queued overflow-reject policy and exposes
`accepted/busy/status`; every peripheral completer declares adjacent setup
admission. The generated interconnect remains propagation-only, decodes queued
setup through current `PSEL/PADDR` with `PENABLE=0`, forwards `PENABLE`, and
keeps unmapped errors active-access only. Reports add aggregate
`back_to_back_policy`, remove broad `apb_back_to_back_policy_deferred` only for
the selected surfaces, and keep narrowed
`apb_additional_back_to_back_policies_deferred` for sideband/data16/protection
variants, deeper queues, alternate overflow, multiple active APB transfers,
direct backend, verification-output, backend-language variants, AXI, AHB, and
VHDL.
`.610` now selects `.611`, an APB sideband-aware back-to-back timing-policy
readiness audit, without behavior changes. This is the nearest remaining APB
timing residue because the shipped data16 and protection APB families are
sideband-aware extensions, and the current queued requester path stores only
the no-sideband request payload fields. The audit must settle queued
`PPROT/PSTRB` capture, fixed and multi-peripheral propagation boundaries,
adjacent completer setup with byte lanes and endpoint-local policies,
report/residue movement, diagnostics, validation, and rollback before any
sideband-aware back-to-back implementation. Data16/protection back-to-back
...
verification-output, backend-language variants, AXI, AHB, and VHDL remain
deferred.
`.612` now ships that bounded requester-first sideband APB timing family for
`ppif/apb_requester_transfer_sideband_status_back_to_back.ppif` and `.apb`.
The selected 32-bit requester adds `queued_prot` and `queued_wstrb`, samples
`req_prot/req_wstrb` when a request is accepted into the queued slot, relaunches
queued setup with `PPROT=queued_prot` and `PSTRB=queued_wstrb` masked by the
queued write bit, and keeps direct terminal-cycle acceptance sideband-aware.
Reports remove broad `apb_back_to_back_policy_deferred` only for the selected
sideband requester surfaces and retain narrowed future-policy residue for
fixed/multi-peripheral composition, completer propagation, data16/protection
variants, deeper queues, alternate overflow, direct backend,
verification-output, backend-language variants, AXI, AHB, and VHDL.
`.613` now audits sideband-aware APB completer and composition back-to-back
...
and the sideband completer/composition substrates already sample or propagate
...
`ppif/apb_completer_sideband_back_to_back.ppif`, its `.apb` alias,
`ppif/apb_composition_sideband_status_back_to_back.ppif`, and its `.apb`
alias. Reports for the selected sideband completer expose adjacent setup
admission with `PPROT width 3` and `PSTRB width 4`; reports for the selected
...
back-to-back propagation, `.612` shipped sideband requester queued
`PPROT/PSTRB`, and `.615` shipped sideband adjacent completer setup plus fixed
composition propagation. Existing sideband multi-peripheral samples already
propagate `PPROT/PSTRB` through the generated interconnect, but still report
broad `apb_back_to_back_policy_deferred`; a temporary combined candidate failed
exactly at the current no-sideband-only multi-peripheral timing guard. `.617`
must settle exact samples, endpoint/interconnect compatibility, report/support
...
selected completer is 32-bit, sideband-aware, no-policy, two-register storage
with `PPROT width 3`, `PSTRB width 4`, `wait_cycles width 4`, `reg0` at
address `0`, `reg1` at address `4`, and adjacent setup admission; the selected
composition is fixed one-requester/one-completer over the `.612` sideband
requester. Multi-peripheral multi-register timing propagation,
...
sideband-aware two-register no-policy shape (`reg0` at address `0`, `reg1` at
address `4`, `PPROT width 3`, `PSTRB width 4`), decodes both registers, and
applies byte-lane writes. The selected fixed composition combines that
completer with the `.612` sideband requester queue, exposes aggregate
`back_to_back_policy`, removes broad back-to-back residue only for the selected
standalone/fixed surfaces, and keeps narrowed future-policy residue explicit.
`.623` selected the next APB data16/protection back-to-back owner after this
prerequisite shipped, without behavior changes first.
...
and its `.apb` alias. The selected family combines `accepted/busy/status`
depth-1 queued requester timing, 16-bit data, `PSTRB width 2`, a two-register
no-policy completer with `reg0` at address `0` and `reg1` at address `2`, and
fixed one-requester/one-completer composition. Reports should remove broad
back-to-back residue only for those selected data16 timing surfaces while
retaining remaining-width and protection-policy residue. Protection-only
...
protection enforcement completer-owned. `.632` now selects `.633`, public
...
but still have no aggregate `back_to_back_policy` and keep broad
...
family, without behavior changes. The audit confirmed fixed sideband
no-policy multi-register timing and sideband data16 no-policy multi-register
timing are support-accounted on fixed compositions, while multi-peripheral
no-policy timing is currently one-register per peripheral. In-memory
two-register no-policy multi-peripheral candidates fail closed at the current
...
back-to-back timing family, without behavior changes. The selector found that
`.625` already ships fixed-composition data16 no-policy reg0/reg1 timing,
`.634` already ships data16 multi-peripheral timing for the protected
status/control shape, and `.642` already ships the 32-bit no-policy reg0/reg1
multi-peripheral family. No public data16 no-policy multi-peripheral
multi-register back-to-back `.ppif` or `.apb` source exists yet, and the
current multi-peripheral timing guard still admits the sideband data16 family
only for the selected data16-protection status/control storage shape. `.644`
must settle exact public source names, status/control window shape, requester,
completer, and interconnect timing requirements, no-policy `reg0`/`reg1`
storage requirements, report/residue movement, support-accounting identities,
diagnostics, validation, rollback, and docs before implementation.
...
`.656` now ships the selected bounded APB sideband-aware 32-bit protection
multi-peripheral multi-register back-to-back timing behavior for exactly
`ppif/apb_composition_multi_peripheral_multi_register_sideband_protection_status_back_to_back.ppif`
and its `.apb` alias. The generated requester exposes
`accepted/busy/status`, accepts one active transfer plus one queued next
...
exact owners.
`.659` now selects `.660`, direct implementation of exactly
`ppif/apb_composition_multi_peripheral_multi_register_sideband_generalized_status_back_to_back.ppif`
and its `.apb` alias, without behavior changes. The selected contract is
bounded to 32-bit sideband-aware APB, one requester, exactly two peripheral
completers, no register-local access policies, matching source-ordered
`reg0..regN` register sets, two to four registers per peripheral, public
representative `reg0/reg1/reg2` at local addresses `0/4/8`,
...
interconnect decode. Data16 generalized register sets, protected generalized
...
selector.
`.663` now selects `.664`, public contract selection for bounded APB
...
follows `.662` because no-policy generalized timing is shipped for both
selected 32-bit and data16 widths, while protected generalized register sets
still need a public access-policy matrix for `reg2..regN` before any timing
guard widens. `.664` must settle public source names, register cardinality,
local-address rules, protected policy matrix, report/support shape,
diagnostics, validation, rollback, docs, Knowledge Map, and the next owner.
Data16 protected generalized register sets, broader cardinality/peripheral
...
transfers, bus matrices, scoreboards, direct backend, verification-output,
backend-language variants, AXI, AHB, and VHDL remain deferred.
`.665` now ships that selected bounded APB sideband-aware 32-bit protected
generalized register-set multi-peripheral back-to-back timing behavior. The
new `.ppif` and `.apb` sources lower through generated `apb_requester`,
`apb_status_regs`, `apb_control_regs`, `apb_interconnect`, and `apb_tb`
review artifacts. The requester relaunches queued 32-bit `PWDATA` plus
`PPROT/PSTRB`; the interconnect decodes status/control windows at `0` and
`256`, translates `PADDR_CONTROL`, muxes selected responses, and owns no
protection predicate. Each peripheral completer owns protection enforcement:
`reg0` reads are allowed, `reg0` writes require privileged `PPROT[0] == 1`,
and every `reg1..regN` read/write requires privileged `PPROT[0] == 1`. The
public representative uses protected `reg0/reg1/reg2` at local addresses
`0/4/8`; the admitted family remains bounded to matching protected
`reg0..regN` register sets with two to four registers per peripheral. Data16
...
backend-language variants, AXI, AHB, and VHDL remain deferred.
`.667` now selects `.668`, direct implementation of exactly
`ppif/apb_composition_multi_peripheral_multi_register_sideband_data16_protection_generalized_status_back_to_back.ppif`
and its `.apb` alias, without behavior changes. The selected public contract
is bounded to 16-bit sideband-aware APB, one requester, exactly two peripheral
completers, matching protected `reg0..regN` register sets with two to four
registers per peripheral, public representative `reg0/reg1/reg2` at local
addresses `0/2/4`, status/control windows at `0` and `258`, queue-depth `1`,
overflow `reject`, adjacent setup on every peripheral, and propagation-only
interconnect decode. The selected policy matrix preserves `.665`: `reg0`
reads are allowed, `reg0` writes require privileged `PPROT[0] == 1`, and every
`reg1..regN` read/write requires privileged `PPROT[0] == 1`. Broader
...
verification-output, backend-language variants, AXI, AHB, and VHDL remain
deferred.
`.668` now ships that selected bounded APB sideband-aware data16 protected
generalized register-set multi-peripheral back-to-back timing behavior. The
new `.ppif` and `.apb` sources lower through generated `apb_requester`,
`apb_status_regs`, `apb_control_regs`, `apb_interconnect`, and `apb_tb`
review artifacts. The requester relaunches queued 16-bit `PWDATA` plus
...
and every `reg1..regN` read/write requires privileged `PPROT[0] == 1`. The
public representative uses protected `reg0/reg1/reg2` at local addresses
`0/2/4`; the admitted family remains bounded to matching protected
...
scoreboards, alternate protection-policy matrices, direct backend,
verification-output, backend-language variants, AXI, AHB, and VHDL remain
deferred. `.669` now owns the next APB timing/register-set residue selector.
`.669` now selects `.670`, readiness audit for broader APB generalized
register-set cardinality beyond the selected two-to-four-register families,
without behavior changes. The selector follows `.668` because the selected
...
`.675` now ships that selected APB sideband-aware data16 no-policy
five-register generalized register-set timing behavior through the
byte-identical `.ppif`/`.apb` source pair. The admitted data16 no-policy
two-peripheral generalized family now accepts source-ordered `reg0..regN`
sets with two, three, four, or five registers, 2-byte spacing, 16-bit data,
reset `0`, no register-local `access-policy`, queue-depth `1`, overflow
`reject`, adjacent setup on both peripheral completers, status/control
windows `0` and `258`, and propagation-only interconnect decode. Reports show
both status and control peripheral register arrays as
...
`.677` now selects `.678`, direct implementation of exactly
...
propagation-only interconnect decode. `.678` must add the source pair,
...
docs, and Knowledge Map coverage while keeping data16 protected
...
`.681` now ships that selected bounded APB sideband-aware data16 protected
five-register generalized `reg0..regN` register-set multi-peripheral timing
...
register-local privileged `PPROT[0]` access-policy matrix. Support accounting
now tracks the new PPIF and profile-alias identities, reports keep
status/control register arrays as `[reg0, reg1, reg2, reg3, reg4]`,
generated artifacts carry `reg3/reg4` storage/read/write/byte-lane and denied
protection branches, and more-than-five-register, more-than-two-peripheral,
deeper-queue, alternate-overflow, accepted-less, multiple-active,
alternate-policy, interconnect-owned-policy, bus-matrix, scoreboard,
direct-backend, verification-output, backend-language variant, AXI, AHB, and
VHDL behavior remain deferred.
...
behavior remain deferred. `.689` selected documentation-only follow-on leaves
`.690` through `.693` for protocol-wide mdBook coverage of IAL2 AXI, APB, and
...
and VHDL remain deferred.
`.719` now selects `.720`, a no-behavior AHB interconnect/decode readiness
audit, after requester and subordinate `.ppif`/`.ahb` public entrypoints
shipped. Live schedule probes confirmed requester-side
`ahb_interconnect_decode_deferred` and subordinate-side
...
`.722`, generated-substrate audit, before implementation. The selected future
source is `ppif/ahb_interconnect.ppif`, with report schema
...
widening is exactly one requester, two uniquely named subordinate objects, two
subordinate child bindings, two non-overlapping static windows, requester/global
AHB wiring in the interconnect `(wiring ...)` block, per-subordinate bus names
from each subordinate object, report topology
...
`.736` now selects `.737`, direct implementation of that first bounded AHB
byte-lane/narrow-transfer subordinate source. The selected future source is
`ppif/ahb_lite_subordinate_byte_lane.ppif`, with intent/module
`ahb_lite_subordinate_byte_lane`, support identity
`intent.ppif_ahb_lite_subordinate_byte_lane`, byte/halfword/word `HSIZE`
...
probes confirmed one- and two-subordinate aggregate candidates can parse
byte-lane subordinate policies and emit byte-lane subordinate review
artifacts, while report/residue movement and `.ahb` alias sequencing still
needed selection.
...
`.743` now ships `ppif/ahb_interconnect_byte_lane.ppif` and
`ppif/ahb_interconnect_two_subordinate_byte_lane.ppif`, both lowering through
generated byte-lane subordinate `.isf`/`.fsm` review artifacts before the
aggregate `ahb_tb` HDL entry. The new sources support-account as
...
optional signals, broader AHB, direct backend, verification-output,
backend-language variants, AXI/APB, and VHDL remain deferred.
`.758` now ships those two generic `.ppif` aggregate `SEQ` sources:
`ppif/ahb_interconnect_byte_lane_seq.ppif` and
...
`narrow_transfer_policy` plus `transfer.seq_policy`. Aggregate-propagation
...
records support identities `intent.ahb_profile_alias_interconnect_byte_lane_seq`
and `intent.ahb_profile_alias_interconnect_two_subordinate_byte_lane_seq`,
coverage keys
`ial2_ahb_profile_alias_interconnect_byte_lane_seq_pipeline_cli` and
`ial2_ahb_profile_alias_interconnect_two_subordinate_byte_lane_seq_pipeline_cli`,
preservation of `composition.byte_lane_propagation` and
`composition.seq_policy_propagation`, and alias-only child `.ahb alias
exposure` residue cleanup. No behavior changed in `.759`.
`.760` now ships those matching aggregate byte-lane in-word `SEQ` `.ahb`
aliases. They support-account as
...
source kind `ial2_profile_alias`, lower through generated byte-lane `SEQ`
subordinate `.isf`/`.fsm` review artifacts before aggregate `ahb_tb`, preserve
`composition.byte_lane_propagation`, `composition.seq_policy_propagation`,
child `narrow_transfer_policy`, and child `transfer.seq_policy`, and remove
aggregate/requester/subordinate profile-alias residue plus child `.ahb alias
exposure` wording from alias report trees. Generic aggregate `SEQ` `.ppif`
reports keep source-surface alias residue. HBURST length/wrap,
...
endpoint+aggregate `.ppif`/`.ahb` family completed. All endpoint burst-context
registers already exist for the shipped `WRAP4`/`INCR4` path, BUSY is currently
folded into the burst-history clear alongside IDLE (the `ahb_seq_idle_clear`
transaction fires on `(| (== HTRANS idle) (== HTRANS busy))`), and the
endpoint/aggregate residue already defer BUSY-in-burst continuation/handling, so
parking is a bounded clear-versus-park decode edit plus report/residue narrowing
inside the shipped byte-only window. Halfword/word burst `SEQ`, wider/indefinite
...
shipped source and its `t/1491`) that replaces `(ignored-transfer busy)` with the
...
`source_kind: ial2_profile_alias`, expected module
`ahb_lite_subordinate_byte_lane_hburst_seq_busy_park`). A reserved `.ahb`-label
probe confirms the alias strict-checks, preserves the generated `.isf`/`.fsm`
artifacts and the `parks_on = [busy]` / `clears_on` BUSY-park report shape, and
drops `ahb_subordinate_profile_alias_deferred` plus the `.ahb alias exposure`
residue wording through the existing suffix-keyed suppression with no adapter
...
step in the established endpoint → aggregate cadence (byte-lane, byte-lane `SEQ`,
HBURST-aware byte-lane `SEQ` each shipped that way): the endpoint BUSY-park
residue defers `aggregate propagation` while the aggregate residue still lists
`BUSY-in-burst handling` first among remaining burst work. The mechanism is
bounded — the interconnect `_seq_policy_propagation_report` already clones each
child's `seq_policy` verbatim, so a child declared with `(parked-transfer busy)`
automatically forwards its `parks_on = [busy]` and BUSY-free `clears_on` into the
aggregate `composition.seq_policy_propagation` report, and the delta is new
aggregate stems plus residue narrowing. Requester-side BUSY insertion (the
requester never drives bus `HTRANS = BUSY`), halfword/word burst `SEQ`,
wider/indefinite bursts, and optional AHB signals are larger and remain deferred.
...
before `.774`: the interconnect composes child subordinate FSMs by calling
`AhbSubordinate->generate` per child and clones each child `seq_policy` verbatim,
so a child declared with `(parked-transfer busy)` parks BUSY through the shipped
endpoint machinery with no interconnect generator, parser, or report change, and
the child `seq_ok_base` fail-closed path carries through composition. The bounded
delta is new aggregate stems whose child transfer uses `(parked-transfer busy)`
...
`ial2_ppif_ahb_interconnect_byte_lane_hburst_seq_busy_park_pipeline_cli`, module
`ahb_tb`, child count 3) and
...
`(parked-transfer busy)`. Each child now holds the in-word `SEQ` burst context
across an accepted `HTRANS = BUSY` beat instead of clearing it, so every child
`composition.seq_policy_propagation` entry reports `parks_on = [busy]` and a
BUSY-free `clears_on`. No interconnect generator/parser/report code changed — the
verbatim `seq_policy` clone forwards the child park — and only the aggregate
HBURST residue at `AhbInterconnect.pm:1401` narrowed to record shipped
BUSY-in-burst parking (the base non-HBURST aggregate residue is untouched).
Focused coverage is `t/1496`; `t/248` moved to 295 protocol / 336 total
supported-smoke entries; the non-parking aggregate HBURST `t/1492`/`t/1493` and
...
requester preservation. `.788` closed at 309 protocol / 350 supported-smoke
and strict entries. `.789` selected the matching profile alias, and `.790` now
ships `ppif/ahb_requester_busy_insert.ahb` as a byte-identical mirror of the
generic source, support-accounted as
`intent.ahb_profile_alias_requester_busy_insert` with coverage
`ial2_ahb_profile_alias_requester_busy_insert_pipeline_cli`. It preserves the
BUSY report/artifacts/module and relies on existing suffix handling to remove
only `ahb_profile_alias_deferred`; no adapter or generator changed. Focused
...
`.792` now confirms the candidate is implementation-ready after one public
contract selection and advances to `.793`. No parser, endpoint-generator,
interconnect wiring, composition-top, or HDL substrate repair is needed; the
report delta is an additive conditional clone of requester `busy_insertion`.
Generated `ahb_tb` already exposes the command/status ports and deterministic
internal requester-bus/subordinate-state observation points needed to prove the
held BUSY, parked subordinate context, resumed `SEQ`, four accepted beats, and
final storage. `.793` must freeze exact source/support/report/test names and
keep the first slice to one generic `.ppif`, one subordinate.
`.793` now selects `.794`, direct implementation of
...
module `ahb_tb`, semantic root `top`. `AhbInterconnect::_child_report` will
conditionally clone requester `busy_insertion`; no duplicated top summary is
...
two-subordinate paired BUSY sibling. An in-memory candidate already composes
the shipped BUSY requester with status/control BUSY-parking subordinates,
reports four children, propagates requester `busy_insertion`, preserves both
`parks_on=[busy]` policies, and generates the expected requester/status/control/
interconnect/top review artifacts. Direct implementation waits for `.798` to
freeze the two-window generated-HDL proof and resolve a pre-existing report
contradiction: the broader two-subordinate residue still calls BUSY continuation
deferred while the dedicated burst residue correctly says BUSY parking ships.
Decision 0020 and proposed audits remain inactive.
`.798` now proves that four-child candidate across public check, schedule,
semantic, review-artifact, SystemVerilog, and Yosys surfaces: module `ahb_tb`,
29 signals, exact status/control windows, four IAL1/five IAL0 artifacts,
requester `busy_insertion`, both child/propagated `parks_on=[busy]`, and clean
21,656-line HDL. No parser/generator/wiring/phase/decode/mux/top/HDL prerequisite
was found. `.799` is selected first as a report-only repair: parked
two-subordinate sources must stop calling BUSY continuation deferred, while
...
decision 0020 and proposed audits remain inactive.
`.800` now selects `.801`, direct implementation of generic
`ppif/ahb_interconnect_requester_busy_insert_two_subordinate_byte_lane_hburst_seq_busy_park.ppif`.
The additive source changes only identity/anchor and swaps the existing
two-subordinate BUSY-park aggregate to `amba_requester_busy_insert`; it keeps
status `[0,4)`, control `[4,8)`, exact four IAL1/five IAL0 artifacts, module
`ahb_tb`, four children, 29 signals, and semantic root `top`. One support entry
moves accounting to 313 protocol / 354 supported-smoke+strict. Focused t/1515
must run status-base-0 then control-base-4 byte `INCR4` writes, prove
`NONSEQ,SEQ,BUSY,SEQ,SEQ`, selected-child parking, unselected-child
...
`.806` now completes that truthfulness repair. The current protocol-navigation
row, AHB mode/requester guidance, and aggregate HBURST, aggregate BUSY-park,
and paired BUSY behavior/fact pairs all point to the shipped alias owners.
Historical "not shipped in this slice" records remain unchanged. Focused
t/1518 binds the six alias paths to positive current claims and rejects the
...
`IAL2-AHB-PIPELINED-ACTIVE-TRANSFER-AUDIT.2` now freezes the repair contract
without changing behavior. The second address phase is accepted at cycle 37,
while generated `ahb_access_done_q` arrives at cycle 48, so the completion
event must be captured at the bus-visible `HSEL && HREADY && active HTRANS`
edge. The selected shared-generator architecture banks exactly one accepted
next HADDR/HTRANS/optional HBURST/HWRITE/HSIZE/wait_cycles phase, never
address-phase-captures data-phase HWDATA, drives the following data phase
not-ready, and relaunches it after the current FSM tail. Final ERROR plus IDLE
cancels; final ERROR plus an active phase captures it for later independent
evaluation. `.3` owns implementation and additive `phase_pipeline` reporting;
...
`IAL2-FEATURE-COMPLETENESS-FRONTIER.808` selected
`IAL2-AHB-REQUESTER-MULTI-BUSY-INSERTION-READINESS-AUDIT` as the next exact
owner. `.1` proved that the pre-repair one-bit procedural insertion exposed one
BUSY transition episode but ten `HGRANT && HREADY` BUSY edges when continuously
ready, contradicting report `busy_insertion.beats=single`; the former t/1498
and paired harnesses counted only transitions. The imported Arm
...
counter; public ready-low and grant-low probes pass for 32 clocks. `.3` now
ships the repair: assertion-enabled t/1498 proves continuous, 32-clock
ready-low, and 32-clock grant-low cases with one qualified BUSY event and four
...
`.6` selected `.7`, which now ships the matching byte-identical
...
the disposable generated-HDL composition: one BUSY episode contains exactly
two qualified events, requester/subordinate/interconnect ownership and storage
remain stable, one pending `SEQ` resumes, four data beats complete cleanly, and
final storage is `32'h44332211`. No substrate repair is required; proposed
`.2` now selects generic source
...
`.4` now selects the byte-identical matching `.ahb` alias contract for proposed
`.5`: existing suffix handling, projected 318/359/42 accounting, focused t1524
normalized semantic JSON/real read-only MCP parity, and shared t1523 runtime.
No alias source or shipped behavior changes in `.4`; the two-subordinate
exact-two shape and decision 0020 remain separate/inactive.
`.5` now ships that byte-identical matching `.ahb` alias through the existing
suffix cleanup and unchanged requester/subordinate/interconnect/top generator
architecture. That checkpoint was 318 protocol / 359 supported+strict / 42
AHB paths, split 21 `.ppif` and 21 `.ahb`. Focused t1524 proves strict check,
schedule JSON, normalized semantic JSON, real read-only MCP introspection,
review artifacts, HDL verification, diagnostics, and preservation without a
second runtime; t1523 remains the shared generated-HDL proof.
`.6` now proves the two-subordinate exact-two shape is runtime-ready through
the existing four-child architecture, without shipping it. A disposable
candidate preserves status `[0,4)` and control `[4,8)` windows, exact requester/
status/control/interconnect artifacts, numeric requester `beats=2`, both
child/propagated `parks_on=[busy]`, and retained one-hot response ownership.
Generated HDL passes two commands with exactly four qualified BUSY events
(two per command), two resumed SEQ events (one per command), stable selected/
unselected subordinate and interconnect ownership, no BUSY data completion,
clean status, and final `44332211`/`88776655` storage. Strict check, normalized
semantic JSON, and real read-only MCP agree on four-child top `ahb_tb`; the
disposable source truthfully remains support-unmatched. No generator or API
repair is needed, so proposed `.7` owns no-behavior generic public-contract
selection after clean `.6`. Decision 0020 remains inactive.
`.7` selected `.8` implementation of topology-first generic source
`ppif/ahb_interconnect_two_subordinate_requester_busy_insert_two_byte_lane_hburst_seq_busy_park.ppif`.
Topology-first naming avoids ambiguous `...busy_insert_two_two_subordinate...`
while preserving the existing exact-two requester and two-subordinate
families. The contract freezes the current four-child artifacts/windows/owner
policy, numeric requester `beats=2`, both BUSY parks, support identity,
t1525's one two-command runtime, normalized semantic JSON, and real read-only
MCP parity. Projected accounting is 319 protocol / 360 supported+strict / 43
AHB paths split 22 `.ppif`/21 `.ahb`. `.8` now ships that source through the
existing generators. Focused t1525 proves the frozen source delta, strict
support, schedule/report/artifacts, normalized semantic JSON, the real
read-only shell-disabled MCP adapter, outdir, HDL verification, and one
two-command generated-HDL runtime totaling four qualified BUSY events, two
resumed `SEQ` events, eight data beats, and final `44332211`/`88776655`
status/control storage. No parser, generator, semantic model, or MCP API
changed. The matching alias and decision 0020 remain separate/inactive. See
`docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_TWO_PAIRED_BUSY_COMPOSITION_BEHAVIOR.md`.
Parent selector `.810` now chooses proposed `.811` data-only implementation of
the byte-identical matching `.ahb` alias. A reserved-suffix strict/schedule/
normalized-semantic/real read-only MCP probe preserves the four-child
artifacts, windows, exact-two requester, both BUSY parks, and retained owner
while existing suffix handling removes only alias residue. Projected accounting
is 320/361/44 split 22 `.ppif`/22 `.ahb`; t1526 will prove parity without a
second runtime, leaving t1525 shared. No behavior changes in `.810`; decision
...
`.811` now ships that byte-identical `.ahb` alias through the existing suffix
handling and unchanged four-child generators at 320/361/44, evenly split 22
`.ppif`/22 `.ahb`. Focused t1526 passes byte/report/artifact/strict/schedule/
normalized-semantic/real read-only MCP/outdir/verifier/diagnostic and
preservation parity in four subtests without a second simulation; t1525
remains the shared two-window runtime. No parser, generator, semantic model, or
MCP API changed. See
