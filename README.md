...
- `docs/tasks/ISF-LOOP-AND-DEEPER-REPEAT-BODY-MULTI-PENDING-AWAITANY-LOWERING.md` — active `R14` task tree (scheduler-frontier #6, completes the repeat-body-activation nesting frontier) lifting the multi-pending `(await_any done)` + later `(await_all done)` deferral for loop-contained / deeper-nested repeats (cross-domain `do` is excluded — net-new CDC lowering).
