...
`BACKEND-API-VALIDATION-FRONTIER.114`: select the next exact backend/API edge
after `.113.1` shipped direct VHDL signed vector negative numeric-literal
addition. The shipped signed edge lowers an 8-bit signed `SUM = (+ A -1)`
fixture to `SUM <= A + to_signed(-1, 8);`. Unsigned negative numeric-literal
arithmetic, signed negative numeric-literal operators beyond addition, broad
VHDL expression parity, aggregate, package, GHDL, composition-parity, and
normalized semantic stabilization work remain deferred until exact leaves own
them.
