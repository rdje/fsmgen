...
`BACKEND-API-VALIDATION-FRONTIER.112.1`: lower direct VHDL scalar output-port
negative decimal literal assignments after `.112` selected the edge from plain
and signed one-bit output probes that failed at arithmetic expressions `'-1'`
and `'-2'`. The selected edge is narrower than broad VHDL expression,
