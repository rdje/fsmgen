...
`BACKEND-API-VALIDATION-FRONTIER.128.1`: lower direct VHDL non-signed vector
positive numeric-literal modulo with a literal-first operand order after `.128`
selected that exact edge. The shipped literal-first positive division edge lowers
...
Literal-first modulo and literal-literal arithmetic still fail closed, with
literal-first modulo owned by the active `.128.1` leaf.
