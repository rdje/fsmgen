...
- `docs/tasks/ISF-DEEPER-NESTED-REPEAT-BODY-LOCAL-DO-LOWERING.md` — active `R14` task tree (scheduler-frontier #3) enabling a plain local `(do child)` at deeper branch nesting (`when⁺ → repeat`, `switch → when⁺ → repeat`) to lower; a clean validator relaxation (deeper-nested generated-do and spawn stay deferred).
