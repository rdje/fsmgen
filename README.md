...
- `docs/tasks/COMPOSITION-TYPE-BACKLOG-EXHAUSTION.md` — active Composition/type backlog tree selected for PNT exhaustion; current frontier selects the next backend-owned struct/record default-lowering value class after deferring aggregate member/index autogrowth behind one complete root-shape proof or explicit syntax contract.
