...
- `docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md` — completed direct-VHDL correctness tree; `.2` ships scalar/static-bit identity, required-only vector folds, signed casts, condition context, and targeted unsupported-shape rejection without widening public source syntax.
- `docs/tasks/ISF-ASSERT-NESTED-BITWISE-PRECEDENCE-REPAIR.md` — completed correctness tree; concurrent-check intermediate substitution now preserves CoreAST grouping, and t1507 proves unchanged illegal/legal admission plus a separate assertion-enabled legal-`0x00000004` transaction.
- `docs/tasks/MDBOOK-VHDL-INTRODUCTION-BOUNDARY-SYNC.md` — completed no-behavior truth-sync tree; Chapters 00 and 10 now align with Chapter 14's bounded shipped direct plus exact C1/C2/C3/APB-C4 VHDL subset without claiming full-backend or external-compiler qualification.
- `docs/tasks/TASK-TREE-FROZEN-LEGACY-DOC-WORKFLOW-SYNC.md` — completed no-behavior continuity-doctrine tree; `.1` removes active instructions to write the four decision-`0007`-frozen blobs, routes state through the current memory layers, and preserves decision `0019` plus the commit-per-leaf workflow.
- `docs/tasks/BIN-FSMGEN-IMPORT-TREE-JUL29-REFRESH.md` — proposed no-behavior bootstrap architecture refresh selected by parent `.835`; activation-time closure is 228 project files / 227 packages / 19 IAL2 owners versus the maintained note's stale 213 / 212 / 5 snapshot.
- `docs/tasks/IAL1-VERIFICATION-CODE-GENERATION-FRONTIER.md` — completed IAL1-first verification-code generation tree.
- `docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md` — proposed, inactive architecture tree naming current synthesizable IAL0/1/2 as HIAL and auditing a peer pure-verification VIAL with a typed bridge, native SV/UVM and VHDL verification lowerings, and separate event-capable compiled Verilator versus full-language/UVM validation profiles; parent `.833` selects the smaller mdBook VHDL truth repair first without narrowing this requirement.
...
- `docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_THREE_PAIRED_BUSY_COMPOSITION_PROFILE_ALIAS_BEHAVIOR.md` — documents `.821` shipment of that byte-identical `.ahb` alias, exact support/report/artifact/semantic/read-only-MCP/verifier parity in t1534, shared assertion-enabled t1533 runtime, its 326/367/50 checkpoint, the exact-four requester pair's 328/369/52 checkpoint, the one-window exact-four paired 330/371/54 checkpoint, and the later two-window exact-four generic/profile 332/373/56 current boundary.
...
- `docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_BUSY_COMPOSITION_CONTRACT_SELECTION.md` — freezes the exact generic two-window exact-four source/support/t1539/testbench contract, projected 331/372/55 split 28 `.ppif`/27 `.ahb`, preservation/rollback boundaries, and a separate data-only implementation leaf.
- `docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_BUSY_COMPOSITION_BEHAVIOR.md` — documents the shipped byte-identical generic/`.ahb` two-window exact-four pair, exact 4 IAL1/5 IAL0 architecture, normalized semantic/read-only-MCP/public-verifier surfaces, focused t1540 parity without simulation, shared repository-local assertion-enabled t1539 10/8/2/8/2/`44332211`/`88776655` runtime, neighbor preservation, and current 332/373/56 split 28/28 accounting.
- `docs/IAL2_POST_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_COMPOSITION_NEXT_OWNER_SELECTION.md` — records `.827` selection and `.828` shipment of the byte-identical two-window exact-four `.ahb` alias after exact same-volume strict/artifact/semantic/read-only-MCP/verifier proof, at 332/373/56 split 28/28 with focused t1540 parity and shared t1539 runtime.
- `docs/IAL2_POST_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_ALIAS_NEXT_OWNER_SELECTION.md` — records `.829` selection of generalized literal requester BUSY-count readiness after exact-one-through-four completion, including the exact-five admission boundary, already-generic minimum-width evidence, finite-range audit contract, and separate HIAL/VIAL/priority/VHDL/scale owners.
- `docs/IAL2_AHB_REQUESTER_GENERALIZED_BUSY_COUNT_RANGE_READINESS_AUDIT.md` — selects a future canonical literal `busy-beats` range of 2..16 after 46 structural/report/read-only-MCP/diagnostic checks and all-assertion count-5/8/16 runtime, while keeping current `2..4`, source bytes, support accounting, dynamic policy, and broader owners unchanged until contract/implementation.
- `docs/IAL2_AHB_REQUESTER_GENERALIZED_BUSY_COUNT_RANGE_CONTRACT_SELECTION.md` — freezes canonical decimal literal `busy-beats` 2..16 admission, exact diagnostics, unchanged width/retirement, unified numeric residue, no per-count public fixtures, focused t1541 runtime, and the touched t1535 same-volume workspace repair before proposed implementation `.3`.
- `docs/IAL2_AHB_REQUESTER_GENERALIZED_BUSY_COUNT_RANGE_BEHAVIOR.md` — documents shipped canonical decimal `busy-beats` 2..16, absence-as-exact-one compatibility, minimum widths, numeric residue, focused t1541 semantic/MCP/verifier and seven all-assertion runtime scenarios, unchanged 332/373/56 accounting, and same-volume verification migration.
- `docs/IAL2_POST_GENERALIZED_BUSY_COUNT_NEXT_OWNER_SELECTION.md` — records `.830` selection of the protocol-neutral transaction-invoked named-drive priority audit after proving direct rule/transaction assignments already suppress correctly while separate `drive` provenance can still leave conflicting selectors enabled.
- `docs/IAL2_POST_NAMED_DRIVE_PRIORITY_NEXT_OWNER_SELECTION.md` — records `.831` selection of the direct-VHDL unary OR/AND/XOR reduction audit after the named-drive probe emitted foreign `(|drive_zero_start)` syntax, including scalar/vector semantics, compiler-qualification limits, candidate comparison, preservation, and rollback.
- `docs/IAL2_POST_DIRECT_VHDL_REDUCTION_NEXT_OWNER_SELECTION.md` — records `.832` selection of the nested-bitwise concurrent-assertion precedence audit after direct-VHDL reduction lowering ships, including the correct factored AXI guard, malformed inlined property, legal-`0x00000004` coverage gap, candidate comparison, validation, and rollback.
- `docs/IAL2_POST_ASSERTION_PRECEDENCE_NEXT_OWNER_SELECTION.md` — records `.833` selection of the mdBook VHDL introduction/backend-summary truth repair after assertion precedence ships, including the intra-book contradiction, bounded shipped VHDL boundary, candidate comparison, preservation, validation, and rollback.
- `docs/IAL2_POST_VHDL_BOOK_SYNC_NEXT_OWNER_SELECTION.md` — records `.834` selection of the frozen-legacy task-tree workflow sync after the VHDL book repair, including exact stale instructions, decisions `0007`/`0019`, candidate comparison, preservation, validation, and rollback.
- `docs/IAL2_POST_FROZEN_WORKFLOW_SYNC_NEXT_OWNER_SELECTION.md` — records `.835` selection of the live `bin/fsmgen` import-tree refresh after workflow repair, including the measured 228/227/IAL2-19 closure, explained same-volume singleton delta, candidate comparison, preservation, validation, and rollback.
- `docs/ISF_ASSERT_NESTED_BITWISE_PRECEDENCE_READINESS_AUDIT.md` — isolates the malformed assertion to loss of child-parent precedence when a CoreAST intermediate is rendered and textually substituted, selects explicit substitution-boundary grouping, and freezes direct/inlined/property/runtime coverage for implementation `.2`.
- `docs/ISF_ASSERT_NESTED_BITWISE_PRECEDENCE_BEHAVIOR.md` — documents shipped grouped intermediate substitution, direct/temporal property output, the corrected AXI guard assertion, and the separate negative-behavior versus legal-only all-assertion t1507 harnesses.
- `docs/DIRECT_VHDL_REDUCTION_EXPRESSION_READINESS_AUDIT.md` — proves the named-drive operand is scalar and records the implementation-time reconciliation from blanket vector rejection to explicit helper folds required to preserve shipped AMBA/APB VHDL.
- `docs/DIRECT_VHDL_REDUCTION_EXPRESSION_BEHAVIOR.md` — documents shipped scalar/static-bit identity/complement, required-only `std_logic` vector folds, signed casts, condition context, targeted unsupported-shape rejection, token-free named-drive/AMBA/APB VHDL, and the separate compiler-qualification boundary.
- `docs/ISF_RULE_TRANSACTION_NAMED_DRIVE_PRIORITY_READINESS_AUDIT.md` — records t1542 direct-control versus named-drive runtime evidence, the shared-caller identity boundary, disposable target-local feasibility, and `.2` selection of unique-caller masking plus ambiguous-caller fail-closed contract work.
- `docs/ISF_RULE_TRANSACTION_NAMED_DRIVE_PRIORITY_CONTRACT_SELECTION.md` — freezes `.2` exact-one-local-caller bidirectional target-local priority, deterministic ambiguous-caller failure, stable report/semantic boundaries, SV/Verilog gates, and the separately owned unqualified VHDL reduction-expression residue before `.3`.
- `docs/ISF_RULE_TRANSACTION_NAMED_DRIVE_PRIORITY_BEHAVIOR.md` — documents shipped `.3` exact-one-local-caller named-drive priority in both directions, target-local masking with unrelated-output survival, unique/ambiguous fail-closed behavior, unchanged public schemas, executable SystemVerilog/native-Verilog proof, and the decision-0023 direct-VHDL boundary.
