...
`BACKEND-API-VALIDATION-FRONTIER.107`: select the next exact backend/API edge
after direct VHDL generated typed non-signed four-state input port declaration
lowering shipped. The shipped logic-input-port slice is narrower than broad
VHDL expression, aggregate, package, GHDL, composition-parity, or normalized
