...
`BACKEND-API-VALIDATION-FRONTIER.124`: select the next exact backend/API edge
after `.123.1` shipped direct VHDL non-signed vector positive
numeric-literal multiplication. The shipped positive multiplication edge
...
still fail closed. The shipped non-signed modulo edge lowers an 8-bit
