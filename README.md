...
`BACKEND-API-VALIDATION-FRONTIER.121.1`: lower direct VHDL non-signed vector
negative numeric-literal division after `.121` selected it from adjacent
fail-closed arithmetic probes. The shipped non-signed multiplication edge
...
The selected division edge is the 8-bit non-signed `QUOT = (/ A -2)` fixture,
which currently fails at arithmetic expression `'A / -2'` until `.121.1`
ships. Non-signed vector negative numeric-literal modulo,
