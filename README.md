...
`BACKEND-API-VALIDATION-FRONTIER.120.1`: lower direct VHDL non-signed vector
negative numeric-literal multiplication after `.120` selected it from adjacent
fail-closed arithmetic probes. The shipped non-signed subtraction edge lowers
an 8-bit non-signed `DIFF = (- A -1)` fixture to
`DIFF <= std_logic_vector(unsigned(A) - unsigned(to_signed(-1, 8)));`.
Non-signed vector negative numeric-literal division/modulo,
