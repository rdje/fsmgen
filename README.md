...
- `docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_THREE_PAIRED_BUSY_COMPOSITION_PROFILE_ALIAS_BEHAVIOR.md` — documents `.821` shipment of that byte-identical `.ahb` alias, exact support/report/artifact/semantic/read-only-MCP/verifier parity in t1534, shared assertion-enabled t1533 runtime, its 326/367/50 checkpoint, and the later generic exact-four 327/368/51 checkpoint before the matching alias moves current accounting to 328/369/52.
...
- `docs/IAL2_AHB_REQUESTER_EXACT_FOUR_BUSY_EVENT_PROFILE_ALIAS_BEHAVIOR.md` — documents shipment of that byte-identical `.ahb` alias, exact width-three/report/artifact/semantic/read-only-MCP/verifier parity, current 328/369/52 split 26/26, focused t1536 without simulation, and shared assertion-enabled t1535 runtime.
