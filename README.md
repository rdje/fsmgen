...
`BACKEND-API-VALIDATION-FRONTIER.126.1`: lower direct VHDL non-signed vector
positive numeric-literal multiplication with a literal-first operand order
after `.126` selected that exact edge. The shipped signal-first positive
...
Literal-first multiplication/division/modulo still fail closed until exact
