...
- `docs/tasks/ISF-LOOP-CONTAINED-REPEAT-BODY-GENERATED-DO-LOWERING.md` — active `R14` task tree (scheduler-frontier #2) enabling a same-domain generated `(do child (params ...))` inside a `(repeat ...)` directly in one `(while ...)`/`(until ...)` body to lower; threads the generated-child activation through the loop-body path (cross-domain do and spawn stay deferred).
