...
- `docs/tasks/ISF-DEEPER-NESTED-REPEAT-BODY-GENERATED-DO-LOWERING.md` — active `R14` task tree (scheduler-frontier #4) enabling a same-domain generated `(do child (params ...))` at deeper branch nesting (`when⁺ → repeat`, `switch → when⁺ → repeat`) to lower + instantiate its child; a 3-part lowering change (collector recursion, param threading, validator relaxation), cross-domain and spawn deferred.
