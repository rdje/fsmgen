...
`BACKEND-API-VALIDATION-FRONTIER.112`: select the next exact backend/API edge
after direct VHDL non-signed vector output-port negative decimal literal
assignment lowering shipped. The completed `.111.1` edge lowers vector output
assignments such as `OUT_next <= -1;` to
`OUT_next <= std_logic_vector(to_signed(-1, 8));`. Scalar negative literals
remain deferred, and this is narrower than broad VHDL expression, aggregate,
package, GHDL, composition-parity, or normalized semantic stabilization work.
