...
- `docs/tasks/ISF-FIELD-STRUCTURED-STORAGE-FRONTIER.md` — active ISF storage-metadata frontier; `.1` selected a metadata-only scalar storage-field contract, `.2` shipped parser validation plus `inferred_storage[].fields` report projection, `.3` selected support-accounting promotion as the next residual, and `.4` promotes the shipped scalar field metadata surface into a file-backed public sample before broader reset derivation, access behavior, aggregate, bank, or packet layout work.
