...
`BACKEND-API-VALIDATION-FRONTIER.128`: select the next exact backend/API edge
after `.127.1` shipped direct VHDL non-signed vector positive numeric-literal
literal-first division. The shipped literal-first positive division edge lowers
...
Literal-first modulo and literal-literal arithmetic still fail closed.
