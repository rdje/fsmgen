...
- `docs/tasks/ISF-TIMING-PARAM-ACTIVATION-OVERRIDE-DIAGNOSTIC-PRECISION.md` — active `R14` task tree splitting the aggregated static-timing override gate into four sub-axis-specific gates (repeat-count, wait-count, latency-bound, watchdog-limit) each with its own targeted diagnostic; regression target `t/1373-isf-timing-param-sub-axis-diagnostic.t`.
