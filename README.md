...
- `docs/tasks/ISF-LOOP-CONTAINED-AND-DEEPER-NESTED-REPEAT-BODY-SPAWN-LOWERING.md` — active `R14` task tree (scheduler-frontier #5) enabling the basic `(spawn ...)` + same-body `(await_all done)` subset inside a loop-contained or deeper-nested repeat; gate relaxation + a drain-requirement rule for these contexts (complex spawn variants and cross-domain stay deferred).
