...
`BACKEND-API-VALIDATION-FRONTIER.110.1`: lower direct VHDL signed vector
output-port negative decimal literal assignments after `.110` selected the
edge from a signed 8-bit output probe that failed at arithmetic expression
`'-1'`. Scalar and non-signed vector negative literals remain deferred, and
the selected edge is narrower than broad VHDL expression, aggregate, package,
GHDL, composition-parity, or normalized semantic stabilization work.
