...
- `perl/FSM/Support/HDLGeneratorResultContract.pm` — bounded top-level result contract plus narrow `source_info`/`module_info` identity slices, bounded source/module/statistics summary slices, and reused semantic-layer shell contracts for in-process `HDLGenerator` embedders.
