...
- `perl/FSM/Support/HDLGeneratorResultContract.pm` — bounded top-level result contract plus delegated nested `source_info`/`module_info`/`statistics` owners, delegated shell-only `composition_spec`/`fsm_module`/`raw_ast`/`resolved_package_imports` owners, advertised stable subsurfaces for `source_info`/`module_info`/`statistics` rather than whole-hash promises, explicitly raw `composition_plan`/`composition_report` compatibility branches, and reused semantic-layer shell contracts rather than separate whole-hash promises for in-process `HDLGenerator` embedders.
