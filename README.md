...
`BACKEND-API-VALIDATION-FRONTIER.120`: select the next exact backend/API edge
after `.119.1` shipped direct VHDL non-signed vector negative
numeric-literal subtraction. The shipped non-signed edge lowers an 8-bit
non-signed `DIFF = (- A -1)` fixture to
