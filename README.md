...
Current primary target is SystemVerilog, with Verilog conversion support and a scoped direct-root VHDL scaffold for the accepted single-FSM subset, including delayed-pulse clock-branch lowering, generic-bearing direct-root module headers with typed scalar/vector sized-literal defaults, binary scalar addition RHS lowering, and same-width addition/subtraction/multiplication/division/modulo/XOR RHS/chain lowering.
