...
- `docs/tasks/ISF-FIELD-STRUCTURED-STORAGE-FRONTIER.md` — active ISF storage-metadata frontier; `.1` selected a metadata-only scalar storage-field contract and `.2` implements parser validation plus `inferred_storage[].fields` report projection before broader reset derivation, access behavior, aggregate, bank, or packet layout work.
