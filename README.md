...
`BACKEND-API-VALIDATION-FRONTIER.129.1`: lower direct VHDL non-signed vector
positive numeric-literal multiplication with two literal operands after `.129`
selected that exact edge. The shipped literal-first positive modulo edge lowers an
...
Literal-literal multiplication/division/modulo still fail closed, with
literal-literal multiplication owned by the active `.129.1` leaf.
