...
- `docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md` — proposed, inactive architecture tree naming current synthesizable IAL0/1/2 as HIAL and auditing a peer pure-verification VIAL with a typed bridge, native SV/UVM and VHDL verification lowerings, and separate event-capable compiled Verilator versus full-language/UVM validation profiles; completed direct-VHDL reduction work leaves it for proposed parent selector `.832` to compare independently.
