...
- `perl/FSM/Support/HDLGeneratorResultContract.pm` — bounded top-level result contract plus narrow source/module identity slices, bounded `module_info`/`statistics` summary slices, and reused semantic-layer shell contracts for in-process `HDLGenerator` embedders.
