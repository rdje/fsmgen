...
one-bit sized bitstring generic maps such as `ENABLE_DEFAULT => '1'` before the
requester/completer child port maps. Scalar signed arithmetic remains explicitly fail-closed; full aggregate
...
scalar expression, and one-bit sized bitstring actuals, VHDL package
