...
- `docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md` — proposed direct-VHDL correctness tree for the discovered unary reduction-token leak; it remains inactive and separate from named-drive priority semantics.
- `docs/tasks/IAL1-VERIFICATION-CODE-GENERATION-FRONTIER.md` — completed IAL1-first verification-code generation tree.
- `docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md` — proposed, inactive architecture tree naming current synthesizable IAL0/1/2 as HIAL and auditing a peer pure-verification VIAL with a typed bridge, native SV/UVM and VHDL verification lowerings, and separate event-capable compiled Verilator versus full-language/UVM validation profiles; completed selector `.830` keeps it proposed while selecting the narrower named-drive priority audit.
