...
`BACKEND-API-VALIDATION-FRONTIER.131.1`: implement the selected direct VHDL
non-signed vector positive numeric-literal literal-literal modulo edge after
`.131` selected it from the post-`.130.1` boundary. The shipped literal-literal positive division edge
...
Literal-literal modulo still fails closed until `.131.1` lands.
