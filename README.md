...
`BACKEND-API-VALIDATION-FRONTIER.127.1`: lower direct VHDL non-signed vector
positive numeric-literal division with a literal-first operand order after
`.127` selected that exact edge. The shipped literal-first positive multiplication
...
Literal-first division/modulo and literal-literal arithmetic still fail closed,
with literal-first division owned by the active `.127.1` leaf.
