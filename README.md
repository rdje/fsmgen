...
- `perl/FSM/Support/HDLGeneratorResultContract.pm` — bounded top-level result contract plus delegated nested `source_info`/`module_info`/`statistics` owners, a delegated shell-only `resolved_package_imports` owner, advertised stable subsurfaces for `source_info`/`module_info`/`statistics` rather than whole-hash promises, explicitly raw `fsm_module`/`raw_ast`/`resolved_package_imports` and composition-spec/plan/report compatibility branches, and reused semantic-layer shell contracts rather than separate whole-hash promises for in-process `HDLGenerator` embedders.
