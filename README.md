...
- `docs/tasks/ISF-DEEPER-NESTED-REPEAT-BODY-ACTIVATION-DIAGNOSTIC-PRECISION.md` — active `R14` task tree that emits a targeted `deeper-nested repeat-body <do|spawn> remains deferred` diagnostic for deeper-when and when-inside-switch cases at the two repeat-body subset entry points; broader deeper-nested implementation remains a future leaf; regression target `t/1375-isf-deeper-nested-repeat-body-activation-diagnostic.t`.
