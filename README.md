...
`BACKEND-API-VALIDATION-FRONTIER.125`: select the next exact backend/API edge
after `.124.1` shipped direct VHDL non-signed vector positive
numeric-literal division. The shipped positive division edge lowers an 8-bit
non-signed `QUOT = (/ A 2)` fixture to
`QUOT <= std_logic_vector(resize(unsigned(A) / to_unsigned(2, 8), 8));`.
The shipped positive multiplication edge lowers an 8-bit non-signed
`PROD = (* A 2)` fixture to
`PROD <= std_logic_vector(resize(unsigned(A) * to_unsigned(2, 8), 8));`.
Positive numeric-literal modulo and literal-first arithmetic still fail closed.
The shipped non-signed modulo edge lowers an 8-bit
