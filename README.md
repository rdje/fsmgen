...
- `docs/tasks/DIRECT-VHDL-REDUCTION-EXPRESSION-LOWERING.md` — active direct-VHDL correctness tree for the discovered unary reduction-token leak; completed no-behavior audit `.1` selects scalar identity/complement plus deterministic rejection of unsupported reductions, and clean audit commit `16f6140c4` activates implementation `.2`.
- `docs/tasks/IAL1-VERIFICATION-CODE-GENERATION-FRONTIER.md` — completed IAL1-first verification-code generation tree.
- `docs/tasks/HIAL-VIAL-VERIFICATION-FIXTURE-ARCHITECTURE.md` — proposed, inactive architecture tree naming current synthesizable IAL0/1/2 as HIAL and auditing a peer pure-verification VIAL with a typed bridge, native SV/UVM and VHDL verification lowerings, and separate event-capable compiled Verilator versus full-language/UVM validation profiles; completed selector `.831` keeps it proposed while selecting the narrower direct-VHDL reduction audit.
...
- `docs/DIRECT_VHDL_REDUCTION_EXPRESSION_READINESS_AUDIT.md` — proves the named-drive operand is scalar, selects scalar unary-reduction identity/complement and vector/range/unresolved/compound fail-closed handling, preserves public source arity, and routes behavior only to implementation `.2`.
