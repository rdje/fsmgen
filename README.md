...
driver and FSMGEN toolbox. `IAL2-FEATURE-COMPLETENESS-FRONTIER.227` now ships
...
read-data. `IAL2-FEATURE-COMPLETENESS-FRONTIER.239` audited dynamic runtime
beat-count/`RLAST` validation readiness and selected direct bounded generated
implementation. The next active IAL2 frontier is
`IAL2-FEATURE-COMPLETENESS-FRONTIER.240`, generated dynamic runtime
beat-count/`RLAST` validation over that generated dynamic last-beat boundary.
...
single-active dynamic write `BID` response matching, and generated
single-active dynamic read single-beat `RID` response matching. Selector
...
support-accounted sample. Implementation `.227` also ships the selected
single-beat dynamic read shape: explicit `response-demux.read` with exactly one
...
dynamic/static demux, same-cycle recapture, same-ID ordering, read-data
...
- `docs/tasks/ISF-FIELD-STRUCTURED-STORAGE-FRONTIER.md` — active ISF storage-metadata frontier; `.1` selected a metadata-only scalar storage-field contract, `.2` shipped parser validation plus `inferred_storage[].fields` report projection, `.3` selected support-accounting promotion as the next residual, `.4` shipped `isf/storage_fields.isf` as the support-accounted public sample, and `.5` selects the next residual or closes the remaining field-structured storage work before broader reset derivation, access behavior, aggregate, bank, or packet layout work.
