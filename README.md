...
`BACKEND-API-VALIDATION-FRONTIER.122`: select the next exact backend/API edge
after `.121.1` shipped direct VHDL non-signed vector negative
numeric-literal division. The shipped non-signed division edge lowers an
8-bit non-signed `QUOT = (/ A -2)` fixture to
...
Non-signed vector negative numeric-literal modulo,
