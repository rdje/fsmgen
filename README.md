...
`BACKEND-API-VALIDATION-FRONTIER.122.1`: lower direct VHDL non-signed vector
negative numeric-literal modulo after `.122` selected it from the remaining
fail-closed adjacent arithmetic probe. The shipped non-signed division edge
...
The selected modulo edge is the 8-bit non-signed `REM = (% A -2)` fixture,
which currently fails at arithmetic expression `'A % -2'` until `.122.1`
ships. Broad VHDL expression parity, aggregate, package, GHDL, composition-parity,
