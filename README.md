...
- `docs/tasks/ISF-LOOP-CONTAINED-REPEAT-BODY-ACTIVATION-DIAGNOSTIC-PRECISION.md` — active `R14` task tree that emits a targeted `loop-contained repeat-body <do|spawn> remains deferred` diagnostic when a `(repeat ...)` with `do` or `spawn` body clauses is nested inside `(while ...)` or `(until ...)`; broader loop-contained implementation remains a future leaf; regression target `t/1374-isf-loop-contained-repeat-body-activation-diagnostic.t`.
