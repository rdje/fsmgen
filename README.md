...
- `docs/IAL2_AHB_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_BUSY_COMPOSITION_BEHAVIOR.md` — documents the shipped generic two-window exact-four source, exact 4 IAL1/5 IAL0 architecture, normalized semantic/read-only-MCP/public-verifier surfaces, repository-local assertion-enabled t1539 10/8/2/8/2/`44332211`/`88776655` runtime, neighbor preservation, and current 331/372/55 split 28/27 accounting.
- `docs/IAL2_POST_TWO_SUBORDINATE_EXACT_FOUR_PAIRED_COMPOSITION_NEXT_OWNER_SELECTION.md` — records `.827` selection of pending `.828`, the byte-identical two-window exact-four `.ahb` alias, after an exact same-volume strict/artifact/semantic/read-only-MCP/verifier probe, projected at 332/373/56 split 28/28 with shared t1539 runtime.
