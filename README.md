...
implementation blueprint before any future Rust/Rust-Wasm, browser JavaScript,
...
- `docs/tasks/BACKEND-LANGUAGE-PORTABILITY-CONTRACT-FRONTIER.md` — active backend-language portability contract tree; `.2.1` captured the variant-parity doctrine, `.2.2` completed the readiness audit, `.2.3` selected the portable in-memory request/result API, `.2.4` selected the source-catalog plus artifact-sink host abstraction, `.2.5` selected the Perl-reference parity harness, and `.2.6` is the active mdBook language-X blueprint selector.
