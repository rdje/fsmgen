...
`BACKEND-API-VALIDATION-FRONTIER.130.1`: implement the selected direct VHDL
non-signed vector positive numeric-literal literal-literal division edge after
`.130` selected it from the post-`.129.1` boundary. The shipped
literal-literal positive
multiplication edge lowers an 8-bit non-signed `PROD = (* 2 3)` fixture to
...
Literal-literal division still fails closed until `.130.1` lands; literal-literal
modulo remains deferred beyond `.130.1`.
