...
`BACKEND-API-VALIDATION-FRONTIER.126`: select the next exact backend/API edge
after `.125.1` shipped direct VHDL non-signed vector positive numeric-literal
modulo. The shipped positive modulo edge lowers an 8-bit non-signed
`REM = (% A 2)` fixture to
`REM <= std_logic_vector(resize(unsigned(A) mod to_unsigned(2, 8), 8));`.
The shipped positive division edge lowers an 8-bit
