...
`BACKEND-API-VALIDATION-FRONTIER.129`: select the next exact backend/API edge
after `.128.1` shipped direct VHDL non-signed vector positive numeric-literal
literal-first modulo. The shipped literal-first positive modulo edge lowers an
...
Literal-literal arithmetic still fails closed.
