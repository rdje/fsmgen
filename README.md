...
- `docs/tasks/ISF-FIELD-STRUCTURED-STORAGE-FRONTIER.md` — active ISF storage-metadata frontier; `.1` selected a metadata-only scalar storage-field contract, `.2` shipped parser validation plus `inferred_storage[].fields` report projection, and `.3` selects the next residual or closes the frontier before broader reset derivation, access behavior, aggregate, bank, or packet layout work.
