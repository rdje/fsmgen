...
- `docs/tasks/ARCHITECTURE-DEBT-FRONTIER.md` — active architecture-debt frontier owner tree for direct-backend convergence and large ISF parser/lowerer extraction work.
...
`ARCHITECTURE-DEBT-FRONTIER.3`, which selects or defers one stable ISF
parser/lowerer extraction family. Completed implementation leaf
`ARCHITECTURE-DEBT-FRONTIER.2.1` projects direct backend storage/helper
declaration-plan entries into `structural_rtl_ir.nets[]` without rerouting HDL
emission. The shipped literal-literal positive modulo edge lowers
