...
Owns the bounded AST-factorization and substitution family for the older direct
generated-module SystemVerilog backend. This package runs the global AST
factorization pass over prepared enable expressions, coordinates the iterative
post-substitution factorization fixpoint, and exposes substituted AST recovery
for downstream direct-backend owners like consolidated intermediate-signal
emission and runtime-AST support.
...
sub run_second_pass_factorization ($self, $factorizer) {
    my $ctx = $self->{flattened_dt};
    fsm_debug("[ASTFactorizationSupport.pm][run_second_pass_factorization()] Delegating iterative post-substitution factorization to FSM::HDL::Factorization::Fixpoint", 3);
...
Runs the prepared direct-backend AST-factorization pipeline over the enable
...
factorizer for later substituted-AST lookup.

=head2 run_second_pass_factorization

Delegates the iterative post-substitution factorization phase to
C<FSM::HDL::Factorization::Fixpoint> while keeping the direct SystemVerilog
backend ownership boundary explicit.
