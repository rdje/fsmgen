...
sub group_assignments_by_rhs ($self, $lhs) {
    # Group assignments by RHS value for unified analysis
    my $lhs_analysis = $self->{assignment_analysis}->{$lhs};
    
    fsm_debug("  Grouping assignments by RHS value:", 3);
    ...
    for my $assignment (@{$lhs_analysis->{assignments}}) {
        ...
        # Add this assignment to the RHS group
        ...
        fsm_debug("    RHS '$rhs' from DT '$dt' with condition '$debug_condition'", 3);
    }
    
    my $rhs_count = scalar(keys %{$lhs_analysis->{rhs_groups}});
    fsm_debug("  Grouped into $rhs_count unique RHS values", 3);
}

sub generate_complete_enable_structure ($self, $lhs) {
    # Generate complete enable structure for all RHS groups
    my $lhs_analysis = $self->{assignment_analysis}->{$lhs};
    
    fsm_debug("  Generating complete enable structure:", 3);
    ...
    for my $dt_name (sort keys %dt_assignments) {
        ...
        for my $rhs (sort keys %{$dt_assignments{$dt_name}}) {
            ...
            my @condition_asts;
            for my $assignment (@$assignments) {
                # The conditions are now ASTs, not strings - use them directly
            ...
            # Build the complete enable expression as AST: dt_enable && conditions
            ...
            # Store the DT-specific enable info
            my $dt_enable_info = {
                dt => $dt_name,
                enable_name => $dt_enable_name,
                enable_ast => $complete_enable_ast,  # Store AST instead of string
                shared_signal => undef  # Will be filled if sharing is detected
            };
            ...
            fsm_debug("    DT-specific enable: $dt_enable_name = $debug_expr", 3);
        }
    }
    ...
    for my $rhs (sort keys %{$lhs_analysis->{rhs_groups}}) {
        ...
        for my $dt_enable_info (@{$rhs_group->{dt_specific_enables}}) {
            # Each DT enable info now has an enable_ast instead of enable_expr
        ...
        # Store the LHS-level enable info with AST
        $rhs_group->{lhs_level_enable} = {
            name => $lhs_enable_name,
            ast => $lhs_enable_ast,  # Store AST instead of string expression
            rhs_value => $rhs
        };
        
        # Debug with SystemVerilog conversion for display only
        my $debug_expr = $lhs_enable_ast->to_systemverilog();
        fsm_debug("    LHS-level enable: $lhs_enable_name = $debug_expr", 3);
    }
}
