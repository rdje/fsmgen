...
sub run_post_substitution_factorization ($self, %args) {
    ...
    for (my $pass_number = 2; $pass_number <= $max_pass_number; $pass_number++) {
        ...
        my $fed_count = $ctx->{backend_sv}->feed_current_asts_to_second_pass($pass_factorizer);
        ...
        my $update_count = $ctx->update_original_asts_with_second_pass_substitutions($pass_factorizer);
