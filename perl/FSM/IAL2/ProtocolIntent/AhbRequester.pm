...
sub _emit_isf($contract) {
    ...
    my $busy_counter_width = $multiple_busy ? _counter_width($busy_beats) : undef;
    ...
    my @busy_storage = $multiple_busy
        ? (
            ...
            "    (var ahb_busy_remaining_q (width $busy_counter_width) (reset 0)))",
        )
        : "    (var $internal_done (width 1) (reset 0)))";
    ...
    return join("\n",
        ...
        "    (set ahb_response_q $bus->{response}{name})",
        "    (set ahb_read_data_q $bus->{read_data}{name}))",
        "",
        ...
        @transfer_busy_drive,
        "  (drive okay_beat",
        _status_drive_lines($status, beat_done => 1, read_data => 'ahb_read_data_q', response => $response->{okay}),
        "    ($bus->{request} 1)",
        "    ($bus->{transfer}{name} $transfer->{idle}))",
        "",
        "  (drive retry_seen",
        ...
        "        (when (! (== beat_index_q 0))",
        "          (drive transfer_seq)))",
        "      (continue-when ahb_address_pending_q)",
        "      (continue-when ahb_data_pending_q)",
        "      (set ahb_response_pending_q 0)",
        "        (when (== ahb_response_q $response->{okay})",
        "          (drive okay_beat)",
        "          (set last_resp_q $response->{okay})",
        "          (when (! write_q) (set last_read_data_q ahb_read_data_q))",
        ...
        "            (when wrap_mode_q",
        "              (set addr_q (+ addr_q addr_step_q))",
        "              (when (== addr_q wrap_high_q) (set addr_q wrap_base_q)))",
        "            (when (! wrap_mode_q) (set addr_q (+ addr_q addr_step_q)))))",
        "        (when (== ahb_response_q $response->{error})",
        "          (set last_error_q 1)",
        "          (set last_resp_q $response->{error})",
        "          (set beats_remaining_q 0)",
        "          (set burst_active_q 0)",
        "          (drive error_done))",
        "        (when (== ahb_response_q $response->{retry})",
        "          (set last_retry_q 1)",
        "          (set last_resp_q $response->{retry})",
        "          (drive retry_seen))",
        "        (when (== ahb_response_q $response->{split})",
        "          (set last_split_q 1)",
