package FSM::IAL2::ProtocolIntent::AxiAwDriver;
...
# until AWREADY, then completes with a one-cycle done pulse. It complements the
# capacity/status response core; it drives no W/AR/R channels in this slice.
...
sub generate($self, @args) {
    ...
    confess "FSM::IAL2::ProtocolIntent::AxiAwDriver->generate expects exactly one contract hash reference\n"
    ...
    return {
        layer => 'IAL2',
        kind  => 'protocol_intent.axi_aw_driver',
        mode  => $report->{mode},
        generated_ial1 => {
            format => 'isf',
            name   => $isf_name,
...
sub _validate_constructor_receiver($class) {
    confess "FSM::IAL2::ProtocolIntent::AxiAwDriver->new must be called with the FSM::IAL2::ProtocolIntent::AxiAwDriver class invocant\n"
        unless defined($class) && !ref($class) && $class eq 'FSM::IAL2::ProtocolIntent::AxiAwDriver';
}
...
sub _validate_object_receiver($self, $method) {
    confess "FSM::IAL2::ProtocolIntent::AxiAwDriver->$method must be called on an FSM::IAL2::ProtocolIntent::AxiAwDriver object\n"
        unless blessed($self) && $self->isa('FSM::IAL2::ProtocolIntent::AxiAwDriver');
}

sub _normalize_contract($raw) {
    my $kind = _required_scalar($raw, 'kind');
    confess "AXI AW driver IAL2 contract kind must be axi_aw_driver\n"
        unless $kind eq 'axi_aw_driver';

    my $protocol = lc _required_scalar($raw, 'protocol');
    confess "AXI AW driver IAL2 contract profile must be axi4 in this slice\n"
    ...
    confess "AXI AW driver IAL2 contract role must be manager-to-subordinate\n"
    ...
    _reject_duplicate_signal_names(
        ...
        qw(addr_q id_q len_q size_q burst_q),
    );
...
sub _build_report(%args) {
    ...
    return {
        schema => 'fsmgen.ial2.protocol_intent.axi_aw_driver.v1',
        mode   => 'driver',
        layering => {
            source_layer          => 'IAL2',
        ...
        enforced_static_rules => [
            'profile must be axi4 and the object must be axi-aw-driver',
            'role must be manager-to-subordinate',
            'AW address width is 32 in this slice',
            'AW id width is 4 in this slice',
            'AW length width is 8, size width is 3, burst width is 2 in this slice',
            'the first slice drives the full burst-describing AW payload (address, id, length, size, burst)',
...
sub _unsupported_residue {
    return [
        {
            id     => 'axi_aw_driver_id_width_fixed',
            detail => 'AWID width is pinned to 4 in this slice; configurable ID width remains deferred.',
        },
        {
            id     => 'axi_aw_driver_attributes_deferred',
            detail => 'AWLOCK/AWCACHE/AWPROT/AWQOS/AWREGION/AWUSER are not driven in this slice.',
        },
        {
            id     => 'axi_aw_driver_w_channel_deferred',
            detail => 'W write-data drive (WVALID/WDATA/WSTRB/WLAST) is a later increment, naturally an AW+W bundle.',
        },
        {
            id     => 'axi_aw_driver_ar_channel_deferred',
            detail => 'AR read-address drive remains outside this AW-only slice.',
        },
        {
            id     => 'axi_aw_driver_burst_address_generation_deferred',
            detail => 'INCR/WRAP address stepping and multi-beat sequencing remain future work.',
        },
        {
            id     => 'axi_aw_driver_capacity_core_integration_deferred',
            detail => 'Integration with the AXI manager capacity/status response core remains future work.',
        },
        {
            id     => 'axi_aw_driver_profile_alias_deferred',
            detail => '.axi profile-alias surfacing of the AW driver remains an unsupported candidate; this slice supports only generic .ppif.',
        },
        {
            id     => 'axi_aw_driver_verification_output_deferred',
...
sub _required_scalar($raw, $field) {
    confess "AXI AW driver IAL2 contract is missing required scalar field '$field'\n"
        unless exists($raw->{$field});
    return _nonempty_scalar($raw->{$field}, $field);
}

sub _required_hash($raw, $field) {
    confess "AXI AW driver IAL2 contract field '$field' must be a hash reference\n"
...
sub _required_identifier_field($raw, $field, $label) {
    confess "AXI AW driver IAL2 contract is missing required field '$label'\n"
        unless exists($raw->{$field});
...
sub _normalize_width_binding($raw, $field, $expected_width) {
    ...
    confess "AXI AW driver IAL2 contract $field.width must be $expected_width in this slice\n"
...
sub _normalize_reset($raw_reset) {
    confess "AXI AW driver IAL2 contract is missing required reset binding\n"
...
sub _normalize_source_anchors($anchors) {
    confess "AXI AW driver IAL2 contract source.anchors must be an array reference\n"
    ...
    for my $index (0 .. $#$anchors) {
        ...
        if (ref($anchor) eq 'HASH') {
            for my $required (qw(document section page)) {
                confess "AXI AW driver IAL2 contract source.anchors[$index] is missing '$required'\n"
...
sub _reject_duplicate_signal_names(@names) {
    ...
    for my $name (@names) {
        ...
        confess "AXI AW driver IAL2 contract duplicates interface/internal signal '$name'\n"
            if $seen{$name}++;
    }
}

sub _nonempty_scalar($value, $field) {
    confess "AXI AW driver IAL2 contract field '$field' must be a non-empty scalar\n"
        if !defined($value) || ref($value) || $value eq '';
    return $value;
}

sub _identifier_value($value, $field) {
    confess "AXI AW driver IAL2 contract field '$field' must be an ISF identifier\n"
        unless defined($value) && !ref($value) && $value =~ /\A[A-Za-z_][A-Za-z0-9_]*\z/;
    return $value;
}

sub _positive_integer($value, $field) {
    confess "AXI AW driver IAL2 contract field '$field' must be a positive integer\n"
        unless defined($value) && !ref($value) && $value =~ /\A[1-9][0-9]*\z/;
    return int($value);
}

sub _bool_value($value, $field) {
    confess "AXI AW driver IAL2 contract field '$field' must be boolean 0 or 1\n"
