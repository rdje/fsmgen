...
sub _response_demux_dynamic_read_issue_order_queue_plan(%args) {
    ...
    if (!$all_dynamic) {
        confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic-id-reuse issue-order-queue requires exactly two all-dynamic read transactions, exactly three all-dynamic read transactions with response_scope single-beat or burst-last, or exactly one dynamic plus one concrete static read transaction with response_scope single-beat, in this slice\n"
            unless @read_transactions == 2
                && $dynamic_count == 1
                && @concrete_static == 1
                && $response_scope eq 'single-beat';
