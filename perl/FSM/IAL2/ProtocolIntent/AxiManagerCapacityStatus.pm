...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_dynamic_issue_order_queue_demux'
        ...
        my $supported = $supported_boundaries{$capture_scope};
        my $queue_diagnostic = 'AXI manager capacity/status IAL2 contract read_data.read dynamic issue-order queue coverage requires generated dynamic read issue-order queue single-beat response_demux with capture_scope single-beat and no burst_length metadata or generated dynamic read issue-order queue burst-last response_demux with capture_scope last-beat and no burst_length metadata in this slice';
