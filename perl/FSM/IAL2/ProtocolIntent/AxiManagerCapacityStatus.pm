...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_multi_mixed_dynamic_static_read_demux'
        ...
        confess "AXI manager capacity/status IAL2 contract read_data.read multiple mixed dynamic/static coverage requires exactly one dynamic read transaction and two concrete static read transactions, or scalar single-beat/last-beat read-data with exactly one dynamic read transaction and three concrete static read transactions, in this slice\n"
            unless @dynamic_transactions == 1
                && (@static_transactions == 2 || $supports_three_static_scalar_read_data);
