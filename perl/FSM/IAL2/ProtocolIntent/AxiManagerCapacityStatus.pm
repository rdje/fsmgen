...
sub _reject_unsupported_top_level_fields($raw) {
    my %allowed = map { $_ => 1 } qw(
        ...
        read_submit reset source source_object_id status submit_policy
...
sub _build_report(%args) {
    ...
    return {
        ...
        unsupported_residue => [
            {
                id     => 'public_ppif_capacity_status_syntax',
                detail => 'Public .ppif syntax for this manager object remains a later exact owner.',
            },
            {
            ...
            },
            {
                ...
                detail => 'ID allocation, user ID validation, same-ID ordering, different-ID interleaving, BID/RID response matching, and burst/last-beat tracking remain outside this capacity/status shell.',
            },
            {
