...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_demux_and_queue_head_demux') {
        ...
        my $mixed_diagnostic = 'AXI manager capacity/status IAL2 contract read_data.read mixed auto-ID plus queue-head coverage requires one read auto-ID transaction and one depth-2 concrete same-ID read queue group with capture_scope single-beat or last-beat in this slice';
        ...
        confess "AXI manager capacity/status IAL2 contract read_data.read mixed auto-ID plus queue-head burst_length.validation runtime-assertion remains separately owned in this slice; use validation report-only for the published mixed burst-length boundary\n"
            if ($args{burst_length_validation} // '') eq 'runtime_assertion';
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'same_id_ordering_policy accepts explicit read/write concrete-id-reuse reject policies plus issue-order-queue admitted-request pulse generation, and generates bounded read single-beat, read burst-last, or write depth-2/depth-3 concrete same-ID queue state plus queue-head response demux for selected public response-demux-only shapes, including multiple independent read single-beat, read burst-last, and write groups, plus selected single-group read single-beat depth-3 scalar read-data queue-head shape, selected single-group read burst-last depth-3 scalar last-beat read-data, report-only raw-ARLEN burst-length, runtime beat-count/RLAST validation, runtime-validation multi-beat output-bank queue-head shapes, selected multiple/mixed depth-3 runtime-validation multi-beat output-bank queue-head shapes, and selected same-family mixed auto-ID plus depth-2 concrete queue-head read burst-last report-only raw-ARLEN burst-length shape',
