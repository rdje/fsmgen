...
sub _response_demux_dynamic_write_transaction(%args) {
    ...
    confess "AXI manager capacity/status IAL2 contract response_demux.write dynamic ID matching requires every write transaction to use dynamic IDs in this slice\n"
        unless @dynamic == @write_transactions;
...
sub _normalize_response_demux_read(%args) {
    ...
    if (ref($args{dynamic_read_transaction}) eq 'HASH') {
        ...
        confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic ID matching supports multiple dynamic read transactions only with response_scope single-beat in this slice\n"
            if $multi_dynamic;
...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_dynamic_demux'
        ...
        my $burst_length_validation = $args{burst_length_validation} // '';
        my $dynamic_last_beat_burst_length = $has_burst_length
        ...
        confess "AXI manager capacity/status IAL2 contract read_data.read dynamic coverage requires exactly one dynamic read transaction\n"
            unless @transactions == 1;
        my @completion_signals = @{$response_demux->{generated_completion_signals} || []};
        confess "AXI manager capacity/status IAL2 contract read_data.read dynamic coverage requires exactly one generated dynamic completion signal\n"
            unless @completion_signals == 1;

        return {
            transactions                     => \@transactions,
            completion_signal_by_transaction => { $transactions[0] => $completion_signals[0] },
            missing_diagnostic               => 'read response_demux dynamic transaction(s)',
            uncovered_diagnostic             => 'generated dynamic read response_demux transaction',
            completion_validity              => $supported->{completion_validity},
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'read_data.read transaction outputs must exactly cover read response_demux auto transactions, supported generated read queue-head transactions, or the one supported generated dynamic read transaction',
