...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'concrete same-ID queue-head response_demux is generated for bounded depth-2/depth-3 response-demux-only shapes: one-or-more-group read single-beat, read burst-last, or write groups; all require issue-order-queue policy, duplicate concrete-ID groups, and no same-family auto_id_lifecycle demux, and read_data consumption is supported for one-or-more generated depth-2 read single-beat queue-head groups, one selected generated depth-3 read single-beat queue-head group, or selected multiple/mixed depth-3 read single-beat queue-head groups through generated scalar capture, plus one-or-more generated depth-2 read burst-last queue-head groups or one selected generated depth-3 read burst-last queue-head group through generated read burst-last queue-head last-beat capture with no burst_length metadata, report-only raw-ARLEN burst-length metadata, runtime-assertion beat-count/RLAST validation metadata, or selected runtime-assertion multi-beat output-bank capture',
