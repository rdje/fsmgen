...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_mixed_dynamic_static_issue_order_queue_demux'
        ...
        my $mixed_queue_last_beat_supported_burst_length = $has_burst_length
            ...
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && ($burst_length_validation eq 'report_only' || $burst_length_validation eq 'runtime_assertion');
        my $queue_diagnostic = 'AXI manager capacity/status IAL2 contract read_data.read mixed dynamic/static issue-order queue coverage requires generated mixed dynamic/static read issue-order queue single-beat response_demux with capture_scope single-beat and no burst_length metadata, generated mixed dynamic/static read issue-order queue burst-last response_demux with capture_scope last-beat and no burst_length metadata, or generated mixed dynamic/static read issue-order queue burst-last response_demux with capture_scope last-beat and report-only or runtime-assertion burst_length metadata, over exactly one dynamic read transaction plus one concrete static read transaction in one depth-2 generated mixed queue in this slice';
        confess "$queue_diagnostic\n"
            unless ref($supported) eq 'HASH'
                && $transaction_completion_source eq $supported->{transaction_completion_source}
                && ($response_demux->{response_scope} // '') eq $supported->{response_scope}
                && (!$has_burst_length || $mixed_queue_last_beat_supported_burst_length);
