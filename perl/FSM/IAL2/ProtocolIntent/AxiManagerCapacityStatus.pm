...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_multi_mixed_dynamic_static_read_demux'
        ...
        my $supports_three_static_runtime_burst_length = $multi_mixed_last_beat_supported_burst_length
            && $burst_length_validation eq 'runtime_assertion'
            && @static_transactions == 3;
        confess "AXI manager capacity/status IAL2 contract read_data.read multiple mixed dynamic/static coverage requires exactly one dynamic read transaction and two concrete static read transactions, scalar single-beat/last-beat read-data with exactly one dynamic read transaction and three concrete static read transactions and no burst_length metadata, or scalar last-beat raw-ARLEN read-data with report-only or runtime-assertion validation and exactly one dynamic read transaction and three concrete static read transactions, in this slice\n"
            unless @dynamic_transactions == 1
