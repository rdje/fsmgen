...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_queue_head_demux') {
        ...
        confess "$queue_group_diagnostic\n"
            if $capture_scope ne 'multi-beat' && @$groups != 1;
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'concrete same-ID queue-head response_demux is generated only for bounded two-transaction depth-2 shapes: one-group read single-beat, one-or-more-group read burst-last response-demux-only, or one-group write, with issue-order-queue policy, duplicate concrete-ID groups, no same-family auto_id_lifecycle demux, and read_data consumption supported for exactly one generated read queue-head group through generated read single-beat queue-head capture, generated read burst-last queue-head last-beat capture, report-only raw-ARLEN burst-length capture, and runtime beat-count/RLAST validation on generated read burst-last queue-head last-beat capture, plus one-or-more generated read burst-last queue-head groups through generated multi-beat output-bank capture in this slice',
