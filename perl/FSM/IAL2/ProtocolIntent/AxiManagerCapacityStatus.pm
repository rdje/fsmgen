...
sub _response_demux_dynamic_read_transaction(%args) {
    ...
    if (@dynamic != @read_transactions) {
        ...
        confess "AXI manager capacity/status IAL2 contract response_demux.read mixed dynamic/static ID matching supports exactly one dynamic read transaction plus one, two, or three pairwise-distinct concrete static read transactions in this slice\n"
            unless $supported_mixed_shape;
...
sub _response_demux_mixed_dynamic_static_read_transaction(%args) {
    ...
    confess "Internal error: mixed dynamic/static read demux requires one dynamic and one, two, or three static transactions\n"
...
sub _normalize_response_demux_read(%args) {
    ...
    if (ref($args{dynamic_read_transaction}) eq 'HASH') {
        ...
        if (@static_states) {
            confess "AXI manager capacity/status IAL2 contract response_demux.read mixed dynamic/static burst-last ID matching supports exactly one dynamic read transaction plus one, two, or three pairwise-distinct concrete static read transactions in this slice\n"
...
sub _multi_mixed_dynamic_static_read_response_demux_covers_multi_beat_boundary($contract) {
    ...
    return ref($dynamic_transactions) eq 'ARRAY'
        && @$dynamic_transactions == 1
        && ref($static_transactions) eq 'ARRAY'
        && (@$static_transactions == 2 || @$static_transactions == 3);
}
