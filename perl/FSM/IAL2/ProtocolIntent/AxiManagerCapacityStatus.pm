...
sub _normalize_contract($raw) {
    ...
    my $response_demux = exists($raw->{response_demux})
        ? _normalize_response_demux(
            ...
            read_data_present  => exists($raw->{read_data}) ? 1 : 0,
        )
        : undef;
...
sub _reject_dynamic_transaction_behavior_interactions(%args) {
    ...
    if (%dynamic_policy_by_family && ref($transactions) ne 'ARRAY') {
        ...
        my $policy = $dynamic_policy_by_family{$family};
        confess "AXI manager capacity/status IAL2 contract same_id_ordering_policy.$family dynamic-id-reuse $policy requires transactions metadata\n";
    }
    ...
    for my $family (sort keys %dynamic_policy_by_family) {
        ...
        my $policy = $dynamic_policy_by_family{$family};
        confess "AXI manager capacity/status IAL2 contract same_id_ordering_policy.$family dynamic-id-reuse $policy requires at least one dynamic $family transaction\n";
    }
    ...
    for my $family (qw(write read)) {
        ...
        if (ref($args{raw_same_id_ordering_policy}) eq 'HASH'
            ...
            confess "AXI manager capacity/status IAL2 contract same_id_ordering_policy.$family concrete-id-reuse policy does not cover dynamic $family transaction ID metadata; select dynamic-id-reuse reject or dynamic-id-reuse issue-order-queue for transaction(s): $transactions_text\n";
        }
    }
...
sub _raw_same_id_ordering_dynamic_policy_families($raw) {
    ...
    for my $family (qw(write read)) {
        ...
        $families{$family} = _nonempty_scalar(
...
sub _normalize_response_demux(%args) {
    ...
    if (exists $raw->{write}) {
        ...
        my $dynamic_write_issue_order_queue = _response_demux_dynamic_write_issue_order_queue_plan(
            manager_name             => $args{manager_name},
            transactions             => $args{transactions},
        ...
        my $dynamic_write_transaction = ref($dynamic_write_issue_order_queue) eq 'HASH'
            ? undef
            : _response_demux_dynamic_write_transaction(
            manager_name             => $args{manager_name},
            transactions             => $args{transactions},
            write_family             => $write_family,
            write_lifecycle          => $write_lifecycle,
        ...
        my $queue_head_plan = ref($dynamic_write_transaction) eq 'HASH'
            || ref($dynamic_write_issue_order_queue) eq 'HASH'
            ? undef
        ...
        $normalized{write} = _normalize_response_demux_write(
            ...
            dynamic_write_issue_order_queue => $dynamic_write_issue_order_queue,
        );
    }
    ...
    my $mode = $has_read_contract
        ? (!$has_write_contract
            && ($read_mode eq 'bounded_dynamic_read_rid_demux_contract'
                ...
                || $read_mode eq 'bounded_dynamic_read_rid_rlast_issue_order_queue_demux_contract'
...
sub _response_demux_dynamic_write_transaction(%args) {
    ...
    if (@dynamic != @write_transactions) {
        my $one_dynamic_multi_static = @dynamic == 1
            && (@concrete_static == 1 || @concrete_static == 2 || @concrete_static == 3);
        my $two_dynamic_one_static = @dynamic == 2 && @concrete_static == 1;
        my $supported_mixed_shape = ($one_dynamic_multi_static || $two_dynamic_one_static)
            && @write_transactions == @dynamic + @concrete_static;
        confess "AXI manager capacity/status IAL2 contract response_demux.write mixed dynamic/static ID matching supports exactly one dynamic write transaction plus one, two, or three pairwise-distinct concrete static write transactions, or exactly two dynamic write transactions plus one concrete static write transaction, in this slice\n"
    ...
    if (!$multi_dynamic) {
        ...
        for my $state (@states) {
            $state->{same_cycle_release_recapture_policy} = 'multi_active_unique_dynamic_write';
    ...
    for my $state (@states) {
        ...
        my @sibling_request_block_exprs = map { _not_expr($_) } @sibling_request_exprs;
        my @active_same_id_block_exprs = map { _not_expr($_) } @active_same_id_exprs;
        $state->{capture_guard} = _and_expr(
            $state->{request_acceptance_expr},
            _not_expr($state->{busy_signal}),
            ($multi_dynamic ? @sibling_request_block_exprs : ()),
            ($multi_dynamic ? @active_same_id_block_exprs : ()),
        );
        if ($multi_dynamic) {
            $state->{sibling_request_block_exprs} = \@sibling_request_block_exprs;
            $state->{active_same_id_block_exprs} = \@active_same_id_block_exprs;
        }
    }
        ...
        ? {
            ...
            transactions                => [
                map {
                    ...
                    if ($_->{same_cycle_release_recapture_policy}) {
                        $entry{release_recapture_rule} = $_->{release_recapture_rule};
                        $entry{same_cycle_release_recapture_policy} = $_->{same_cycle_release_recapture_policy};
                        $entry{release_recapture_source} = $_->{release_recapture_source};
                        $entry{release_recapture_transaction} = $_->{release_recapture_transaction};
                    }
                    +{
                        %entry,
                    }
...
sub _response_demux_dynamic_write_issue_order_queue_plan(%args) {
    my @write_transactions = grep { ($_->{kind} // '') eq 'write' } @{$args{transactions} || []};
    my @dynamic = grep {
        ref($_->{id}) eq 'HASH' && ($_->{id}{policy} // '') eq 'dynamic'
    } @write_transactions;
    ...
    confess "AXI manager capacity/status IAL2 contract response_demux.write dynamic-id-reuse issue-order-queue requires one shared write request ID source in this slice\n"
...
sub _response_demux_dynamic_read_issue_order_queue_plan(%args) {
    ...
    my $dynamic_count = scalar(@dynamic);
    my $response_scope = ref($args{raw_read}) eq 'HASH'
        ? ($args{raw_read}{response_scope} // '')
        : '';
    ...
    my $all_dynamic = @read_transactions == $dynamic_count;
    if (!$all_dynamic) {
        confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic-id-reuse issue-order-queue requires exactly two all-dynamic read transactions, exactly three all-dynamic read transactions with response_scope single-beat or burst-last, or exactly one dynamic plus one concrete static read transaction with response_scope single-beat or burst-last, in this slice\n"
            unless @read_transactions == 2
                && $dynamic_count == 1
                && @concrete_static == 1
                && ($response_scope eq 'single-beat' || $response_scope eq 'burst-last');
        confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic-id-reuse issue-order-queue requires read-max-pending at least the mixed read transaction count\n"
            unless ($args{read_max_pending} // 0) >= 2;

        my $burst_last = $response_scope eq 'burst-last';
        ...
        my %state_by_transaction = (
            ...
            $static_transaction->{name} => {
                ...
                family_width            => $args{read_family}{width},
                concrete_id             => $static_id->{value},
                concrete_id_literal     => $static_id_literal,
                request_acceptance_expr => _same_id_admitted_request_guard_expr(
                    request_event    => $static_transaction->{request_event},
        ...
        my @states = map { $state_by_transaction{$_->{name}} } @read_transactions;
    ...
    my $supported_dynamic_read_queue = $dynamic_count == 2
        || ($dynamic_count == 3 && ($response_scope eq 'single-beat' || $response_scope eq 'burst-last'));
    confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic-id-reuse issue-order-queue requires exactly two all-dynamic read transactions, or exactly three all-dynamic read transactions with response_scope single-beat or burst-last in this slice\n"
        unless $all_dynamic && $supported_dynamic_read_queue;
    confess "AXI manager capacity/status IAL2 contract response_demux.read dynamic-id-reuse issue-order-queue requires read-max-pending at least the dynamic read transaction count\n"
        unless ($args{read_max_pending} // 0) >= $dynamic_count;
...
sub _response_demux_mixed_dynamic_static_write_transaction(%args) {
    ...
    my $one_dynamic_multi_static = @dynamic_transactions == 1
        && (@static_transactions == 1 || @static_transactions == 2 || @static_transactions == 3);
    my $two_dynamic_one_static = @dynamic_transactions == 2 && @static_transactions == 1;
    ...
    my $completion_fanin = _fanin_expression([map { $_->{completion_event} } @write_transactions]);
    my @dynamic_states;
    for my $dynamic_transaction (@dynamic_transactions) {
        my $dynamic_id = $dynamic_transaction->{id};
    ...
    for my $dynamic_state (@dynamic_states) {
        my @sibling_request_exprs = map { $_->{request_acceptance_expr} }
            grep { $_->{transaction} ne $dynamic_state->{transaction} } @dynamic_states;
        my @active_same_id_exprs = map {
            _and_expr(
                $_->{busy_signal},
                _eq_expr($_->{selected_id_signal}, $dynamic_state->{request_id_source}),
            )
        } grep { $_->{transaction} ne $dynamic_state->{transaction} } @dynamic_states;
        ...
        if ($multi_dynamic) {
            $dynamic_state->{sibling_request_block_exprs} = \@sibling_request_block_exprs;
            $dynamic_state->{active_same_id_block_exprs} = \@active_same_id_block_exprs;
        }
    }
    for my $static_state (@static_states) {
        ...
        my @dynamic_request_block_exprs = map { _not_expr($_) } @dynamic_request_exprs;
        my @sibling_static_request_block_exprs = map { _not_expr($_) } @sibling_static_request_exprs;
        $static_state->{dynamic_request_block_exprs} = \@dynamic_request_block_exprs;
        $static_state->{sibling_static_request_block_exprs} = \@sibling_static_request_block_exprs;
        $static_state->{capture_guard} = _and_expr(
            $static_state->{request_acceptance_expr},
            _not_expr($static_state->{busy_signal}),
            @dynamic_request_block_exprs,
            @sibling_static_request_block_exprs,
        );
    }

    my @dynamic_transaction_names = map { $_->{transaction} } @dynamic_states;
    my @static_transaction_names = map { $_->{transaction} } @static_states;
    my @completion_signals = (
        (map { $_->{completion_event} } @dynamic_states),
    ...
    $dynamic_capture{same_id_conflict_policy} = 'active_dynamic_ids_must_be_unique'
        if @dynamic_states > 1;
    ...
    _response_demux_mark_mixed_dynamic_static_write_recapture(\%entry)
        ...
        || (@dynamic_states == 2 && @static_states == 1);
    return \%entry;
}
...
sub _response_demux_dynamic_read_transaction(%args) {
    ...
    if (@dynamic != @read_transactions) {
        ...
        confess "AXI manager capacity/status IAL2 contract response_demux.read mixed dynamic/static ID matching supports exactly one dynamic read transaction plus one, two, or three pairwise-distinct concrete static read transactions, or exactly two dynamic read transactions plus one concrete static read transaction, in this slice\n"
            unless $supported_mixed_shape;
    ...
    return qw(same_id_ordering read_data_interleaving bursts)
        if ref($read) eq 'HASH'
            && _response_demux_transaction_completion_source_is_dynamic_issue_order_queue(
                $read->{transaction_completion_source},
            );
...
sub _normalize_response_demux_write(%args) {
    ...
    if (ref($args{dynamic_write_issue_order_queue}) eq 'HASH') {
        my $plan = $args{dynamic_write_issue_order_queue};
...
sub _normalize_response_demux_read(%args) {
    ...
    if (ref($args{dynamic_read_issue_order_queue}) eq 'HASH') {
        ...
        if ($response_scope eq 'single-beat') {
            ...
            my $mixed_dynamic_static_queue =
                ref($plan->{static_transactions}) eq 'ARRAY'
                && @{$plan->{static_transactions}};
            %scope_fields = (
                ...
                transaction_completion_source => $mixed_dynamic_static_queue
                    ? $plan->{transaction_completion_source}
                    : 'generated_dynamic_issue_order_queue_demux_last_beat',
                transaction_completion_semantics => $mixed_dynamic_static_queue
                    ? $plan->{transaction_completion_semantics}
    ...
    if (ref($args{dynamic_read_transaction}) eq 'HASH') {
        ...
        if ($response_scope eq 'single-beat') {
            ...
            if (@dynamic_states == 1 && !@static_states) {
                ...
                (@dynamic_states == 1 && (@static_states == 1 || @static_states == 2 || @static_states == 3))
                || (@dynamic_states == 2 && @static_states == 1)
            ) {
        ...
        if (@static_states) {
            my $one_dynamic_static_set =
                @dynamic_states == 1
                && (@static_states == 1 || @static_states == 2 || @static_states == 3);
            my $two_dynamic_one_static =
                @dynamic_states == 2
                && @static_states == 1;
            confess "AXI manager capacity/status IAL2 contract response_demux.read mixed dynamic/static burst-last ID matching supports exactly one dynamic read transaction plus one, two, or three pairwise-distinct concrete static read transactions, or exactly two dynamic read transactions plus one concrete static read transaction in this slice\n"
                unless ($one_dynamic_static_set || $two_dynamic_one_static)
                    && @states == @dynamic_states + @static_states;
            my $multi_mixed = @dynamic_states > 1 || @static_states > 1;
            my %entry = (
                mode                         => $multi_mixed
                ...
                last_signal_direction         => 'generated_input',
                last_signal_width             => $last_signal_width,
                transaction_completion_source => $multi_mixed
...
sub _response_demux_mark_single_active_dynamic_read_recapture($entry, %args) {
    ...
    return unless ref($capture) eq 'HASH';
    $capture->{release_recapture_rule} = $state->{release_recapture_rule};
    $capture->{same_cycle_release_recapture_policy} = $state->{same_cycle_release_recapture_policy};
    $capture->{release_recapture_source} = $state->{release_recapture_source};
...
sub _response_demux_mark_multi_active_dynamic_read_recapture($entry, %args) {
    ...
    my $capture = $entry->{dynamic_capture};
    return unless ref($capture) eq 'HASH';
    my $transactions = $capture->{transactions};
    return unless ref($transactions) eq 'ARRAY';

    for my $capture_entry (@$transactions) {
        next unless ref($capture_entry) eq 'HASH';
        my $state = $state_by_transaction{$capture_entry->{transaction} // ''};
        next unless ref($state) eq 'HASH';
        $capture_entry->{release_recapture_rule} = $state->{release_recapture_rule};
        $capture_entry->{same_cycle_release_recapture_policy} =
            $state->{same_cycle_release_recapture_policy};
        $capture_entry->{release_recapture_source} = $state->{release_recapture_source};
...
sub _response_demux_mark_mixed_dynamic_static_write_recapture($entry) {
    ...
    return unless ref($dynamic_states) eq 'ARRAY'
        && (@$dynamic_states == 1 || @$dynamic_states == 2);
    return unless ref($static_states) eq 'ARRAY';
    my $one_dynamic_multi_static =
        @$dynamic_states == 1 && (@$static_states == 1 || @$static_states == 2 || @$static_states == 3);
    my $two_dynamic_one_static =
        @$dynamic_states == 2 && @$static_states == 1;
    return unless $one_dynamic_multi_static || $two_dynamic_one_static;

    return if grep { ref($_) ne 'HASH' } @$dynamic_states;
    return if grep { ref($_) ne 'HASH' } @$static_states;

    my $release_recapture_source =
        ($entry->{transaction_completion_source} // 'generated_mixed_dynamic_static_demux') . '_completion';

    my $dynamic_policy = $two_dynamic_one_static
        ? 'mixed_dynamic_static_multi_active_dynamic_write'
        : 'mixed_dynamic_static_dynamic_write';
    my %dynamic_state_by_transaction;
    for my $dynamic_state (@$dynamic_states) {
        $dynamic_state->{same_cycle_release_recapture_policy} = $dynamic_policy;
        ...
        $dynamic_state_by_transaction{$dynamic_state->{transaction} // ''} = $dynamic_state;
    }
    ...
    if (ref($dynamic_capture) eq 'HASH') {
        ...
        if (ref($capture_transactions) eq 'ARRAY') {
            for my $capture_entry (@$capture_transactions) {
                next unless ref($capture_entry) eq 'HASH';
                my $dynamic_state =
                    $dynamic_state_by_transaction{$capture_entry->{transaction} // ''};
                next unless ref($dynamic_state) eq 'HASH';
    ...
    $entry->{static_capture} =
        @static_capture == 1 && @$dynamic_states == 1 ? $static_capture[0] : \@static_capture;
}
...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_multi_mixed_dynamic_static_read_demux'
        ...
        confess "$mixed_diagnostic\n"
            unless ref($supported) eq 'HASH'
                ...
                && (
                    !$has_burst_length
                    || $multi_mixed_last_beat_supported_burst_length
                    || $multi_mixed_multi_beat_supported_burst_length
                );

        my @dynamic_transactions = @{$response_demux->{dynamic_transactions} || []};
        my @static_transactions = @{$response_demux->{static_transactions} || []};
        my $scalar_without_burst_length = !$has_burst_length
            && ($capture_scope eq 'single-beat' || $capture_scope eq 'last-beat');
        my $supports_three_static_scalar_read_data = $scalar_without_burst_length
            && @static_transactions == 3;
        my $supports_two_dynamic_one_static_single_beat_read_data = $scalar_without_burst_length
            && $capture_scope eq 'single-beat'
            && $transaction_completion_source eq 'generated_multi_mixed_dynamic_static_read_demux'
            && ($response_demux->{response_scope} // '') eq 'single_beat'
            && @dynamic_transactions == 2
            && @static_transactions == 1;
        my $supports_two_dynamic_one_static_last_beat_read_data = $scalar_without_burst_length
            && $capture_scope eq 'last-beat'
            && $transaction_completion_source eq 'generated_multi_mixed_dynamic_static_read_demux_last_beat'
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && @dynamic_transactions == 2
        ...
        my $supports_two_dynamic_one_static_report_only_burst_length = $multi_mixed_last_beat_supported_burst_length
            ...
            && @dynamic_transactions == 2
            && @static_transactions == 1;
        my $supports_two_dynamic_one_static_runtime_burst_length = $multi_mixed_last_beat_supported_burst_length
            && $burst_length_validation eq 'runtime_assertion'
            && @dynamic_transactions == 2
            && @static_transactions == 1;
        my $supports_two_dynamic_one_static_multi_beat_burst_length = $multi_mixed_multi_beat_supported_burst_length
            && @dynamic_transactions == 2
            && @static_transactions == 1;
        my $supports_three_static_report_only_burst_length = $multi_mixed_last_beat_supported_burst_length
            && $burst_length_validation eq 'report_only'
            && @static_transactions == 3;
        my $supports_three_static_runtime_burst_length = $multi_mixed_last_beat_supported_burst_length
            && $burst_length_validation eq 'runtime_assertion'
            && @static_transactions == 3;
        ...
        ) || $supports_two_dynamic_one_static_single_beat_read_data
            || $supports_two_dynamic_one_static_last_beat_read_data
            || $supports_two_dynamic_one_static_report_only_burst_length
            || $supports_two_dynamic_one_static_runtime_burst_length
            || $supports_two_dynamic_one_static_multi_beat_burst_length;
        confess "AXI manager capacity/status IAL2 contract read_data.read multiple mixed dynamic/static coverage requires exactly one dynamic read transaction and two concrete static read transactions, scalar single-beat/last-beat read-data with exactly one dynamic read transaction and three concrete static read transactions and no burst_length metadata, scalar single-beat or last-beat read-data with exactly two dynamic read transactions and one concrete static read transaction and no burst_length metadata, scalar last-beat raw-ARLEN read-data with report-only or runtime-assertion validation and exactly one dynamic read transaction and three concrete static read transactions, scalar last-beat raw-ARLEN read-data with report-only or runtime-assertion validation and exactly two dynamic read transactions and one concrete static read transaction, or runtime-assertion multi-beat output-bank read-data with exactly one dynamic read transaction and three concrete static read transactions or exactly two dynamic read transactions and one concrete static read transaction, in this slice\n"
            unless $supports_declared_mixed_transaction_set;

        my @transactions = (@dynamic_transactions, @static_transactions);
        my @completion_signals = @{$response_demux->{generated_completion_signals} || []};
        confess "AXI manager capacity/status IAL2 contract read_data.read multiple mixed dynamic/static coverage requires one generated completion signal per covered read transaction\n"
            unless @completion_signals == @transactions;
        my %completion_signal_by_transaction;
        @completion_signal_by_transaction{@transactions} = @completion_signals;
        ...
        return {
            transactions                     => \@transactions,
            completion_signal_by_transaction => \%completion_signal_by_transaction,
            missing_diagnostic               => 'read response_demux multiple mixed dynamic/static transaction(s)',
            uncovered_diagnostic             => 'generated multiple mixed dynamic/static read response_demux transactions',
            completion_validity              => $supported->{completion_validity},
    ...
    if ($transaction_completion_source eq 'generated_dynamic_issue_order_queue_demux'
        ...
        my $has_burst_length = $args{has_burst_length};
        my $burst_length_validation = $args{burst_length_validation} // '';
        my $queue_last_beat_supported_burst_length = $has_burst_length
            && $capture_scope eq 'last-beat'
            && $transaction_completion_source eq 'generated_dynamic_issue_order_queue_demux_last_beat'
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && ($burst_length_validation eq 'report_only' || $burst_length_validation eq 'runtime_assertion');
        my $queue_multi_beat_supported_burst_length = $has_burst_length
            && $capture_scope eq 'multi-beat'
            && $transaction_completion_source eq 'generated_dynamic_issue_order_queue_demux_last_beat'
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && $burst_length_validation eq 'runtime_assertion';
        my $queue_diagnostic = 'AXI manager capacity/status IAL2 contract read_data.read dynamic issue-order queue coverage requires generated dynamic read issue-order queue single-beat response_demux with capture_scope single-beat and no burst_length metadata, generated dynamic read issue-order queue burst-last response_demux with capture_scope last-beat and no burst_length metadata over two dynamic transactions or one depth-3 all-dynamic queue, generated dynamic read issue-order queue burst-last response_demux with capture_scope last-beat and report-only/runtime-assertion burst_length metadata over two dynamic transactions, generated dynamic read issue-order queue burst-last response_demux with capture_scope last-beat and report-only/runtime-assertion burst_length metadata over one depth-3 all-dynamic queue, generated dynamic read issue-order queue burst-last response_demux with capture_scope multi-beat and runtime-assertion burst_length metadata over two dynamic transactions, or generated dynamic read issue-order queue burst-last response_demux with capture_scope multi-beat and runtime-assertion burst_length metadata over one depth-3 all-dynamic queue in this slice';
        confess "$queue_diagnostic\n"
            unless ref($supported) eq 'HASH'
                && $transaction_completion_source eq $supported->{transaction_completion_source}
                && ($response_demux->{response_scope} // '') eq $supported->{response_scope}
                && (!$has_burst_length || $queue_last_beat_supported_burst_length || $queue_multi_beat_supported_burst_length);
        ...
        my $valid_depth3_last_beat_supported_burst_length_queue = @transactions == 3
            ...
            && @group_transactions == 3
            && join("\0", @group_transactions) eq join("\0", @transactions);
        my $valid_depth3_multi_beat_supported_burst_length_queue = @transactions == 3
            && $has_burst_length
            && $burst_length_validation eq 'runtime_assertion'
            && $capture_scope eq 'multi-beat'
            && $transaction_completion_source eq 'generated_dynamic_issue_order_queue_demux_last_beat'
        ...
        confess "$queue_diagnostic\n"
            unless $valid_depth2_queue
                ...
                || $valid_depth3_multi_beat_supported_burst_length_queue;
        ...
        return {
            transactions                     => \@transactions,
            completion_signal_by_transaction => \%completion_signal_by_transaction,
            missing_diagnostic               => 'read response_demux dynamic issue-order queue transaction(s)',
            uncovered_diagnostic             => 'generated dynamic read issue-order queue response_demux transactions',
            completion_validity              => $supported->{completion_validity},
            dynamic_issue_order_queue_read_response_demux => 1,
        };
    }

    if ($transaction_completion_source eq 'generated_mixed_dynamic_static_issue_order_queue_demux'
        || $transaction_completion_source eq 'generated_mixed_dynamic_static_issue_order_queue_demux_last_beat') {
        my %supported_boundaries = (
            'single-beat' => {
                response_scope                => 'single_beat',
                transaction_completion_source => 'generated_mixed_dynamic_static_issue_order_queue_demux',
        ...
        my $has_burst_length = $args{has_burst_length};
        my $burst_length_validation = $args{burst_length_validation} // '';
        my $mixed_queue_last_beat_supported_burst_length = $has_burst_length
            && $capture_scope eq 'last-beat'
            && $transaction_completion_source eq 'generated_mixed_dynamic_static_issue_order_queue_demux_last_beat'
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && ($burst_length_validation eq 'report_only' || $burst_length_validation eq 'runtime_assertion');
        my $mixed_queue_multi_beat_supported_burst_length = $has_burst_length
            && $capture_scope eq 'multi-beat'
            && $transaction_completion_source eq 'generated_mixed_dynamic_static_issue_order_queue_demux_last_beat'
            && ($response_demux->{response_scope} // '') eq 'burst_last'
            && $burst_length_validation eq 'runtime_assertion';
        my $queue_diagnostic = 'AXI manager capacity/status IAL2 contract read_data.read mixed dynamic/static issue-order queue coverage requires generated mixed dynamic/static read issue-order queue single-beat response_demux with capture_scope single-beat and no burst_length metadata, generated mixed dynamic/static read issue-order queue burst-last response_demux with capture_scope last-beat and no burst_length metadata, generated mixed dynamic/static read issue-order queue burst-last response_demux with capture_scope last-beat and report-only or runtime-assertion burst_length metadata, or generated mixed dynamic/static read issue-order queue burst-last response_demux with capture_scope multi-beat and runtime-assertion burst_length metadata, over exactly one dynamic read transaction plus one concrete static read transaction in one depth-2 generated mixed queue in this slice';
        confess "$queue_diagnostic\n"
            unless ref($supported) eq 'HASH'
                && $transaction_completion_source eq $supported->{transaction_completion_source}
                && ($response_demux->{response_scope} // '') eq $supported->{response_scope}
                && (!$has_burst_length
                    || $mixed_queue_last_beat_supported_burst_length
                    || $mixed_queue_multi_beat_supported_burst_length);
        ...
        my @completion_signals = @{$response_demux->{generated_completion_signals} || []};
        confess "AXI manager capacity/status IAL2 contract read_data.read mixed dynamic/static issue-order queue coverage requires one generated completion signal per covered read transaction\n"
            unless @completion_signals == @transactions;
        my %completion_signal_by_transaction;
        @completion_signal_by_transaction{@transactions} = @completion_signals;
        ...
        return {
            transactions                     => \@transactions,
            completion_signal_by_transaction => \%completion_signal_by_transaction,
            missing_diagnostic               => 'read response_demux mixed dynamic/static issue-order queue transaction(s)',
            uncovered_diagnostic             => 'generated mixed dynamic/static read issue-order queue response_demux transactions',
            completion_validity              => $supported->{completion_validity},
            mixed_dynamic_static_issue_order_queue_read_response_demux => 1,
        };
    }
...
sub _normalize_same_id_ordering_policy_family($raw, $family) {
    ...
    if (exists $raw->{dynamic_id_reuse}) {
        ...
        confess "AXI manager capacity/status IAL2 contract same_id_ordering_policy.$family.dynamic_id_reuse must be reject or issue-order-queue in this slice\n"
            unless $policy =~ /\A(?:reject|issue-order-queue)\z/;

        $entry{dynamic_id_reuse} = $policy eq 'issue-order-queue'
            ? {
                policy                       => 'issue_order_queue',
                implementation_status        => 'selected_not_generated',
                enforcement                  => 'not_generated',
...
sub _build_same_id_ordering(%args) {
    ...
    $dynamic_policy = _same_id_ordering_dynamic_policy_with_issue_order_queue_behavior(
        policy         => $dynamic_policy,
        response_demux => $args{response_demux},
...
sub _same_id_ordering_policy_residue(%args) {
    ...
    push @residue, 'dynamic_per_id_issue_order_queues'
        if _same_id_ordering_dynamic_policy_selects_issue_order_queue($args{dynamic_policy})
            && !_same_id_ordering_dynamic_issue_order_queue_all_generated($args{dynamic_policy});
    return \@residue;
}

sub _same_id_ordering_dynamic_policy_selects_issue_order_queue($policy) {
    ...
    for my $family_name (qw(read write)) {
        ...
        return 1
            if ref($entry) eq 'HASH'
                && ($entry->{policy} // '') eq 'issue_order_queue';
    }
    return 0;
}

sub _same_id_ordering_dynamic_issue_order_queue_all_generated($policy) {
    return 0 unless ref($policy) eq 'HASH';

    my @families = grep {
        ref($policy->{$_}) eq 'HASH'
            && ($policy->{$_}{policy} // '') eq 'issue_order_queue'
    } qw(write read);
    return 0 unless @families;
    my @generated = grep { $policy->{$_}{generated_queue_behavior} } @families;
    return @generated == @families ? 1 : 0;
}
...
sub _same_id_ordering_dynamic_policy_with_response_demux_coverage(%args) {
    ...
    for my $family_name (qw(write read)) {
        ...
        next unless ($policy->{$family_name}{policy} // '') eq 'reject';
...
sub _same_id_ordering_dynamic_policy_with_issue_order_queue_behavior(%args) {
    my $policy = _clone_jsonish($args{policy});
    return $policy unless ref($policy) eq 'HASH';

    my $behavior = $args{same_id_issue_order_queue_behavior};
    for my $family_name (qw(write read)) {
        next unless ref($policy->{$family_name}) eq 'HASH';
        next unless ($policy->{$family_name}{policy} // '') eq 'issue_order_queue';
        my $queue_family = _same_id_issue_order_queue_family_behavior($behavior, $family_name);
        ...
        if ($family_name eq 'read') {
            ...
            $policy->{$family_name}{first_generated_scope} =
                $mixed_dynamic_static
                    ? ref($entry) eq 'HASH' && defined $entry->{last_signal}
                        ? 'read_rid_rlast_one_dynamic_one_static_transaction'
                        : 'read_rid_one_dynamic_one_static_transaction'
                : ref($entry) eq 'HASH' && defined $entry->{last_signal}
                    ? $depth == 3
                        ? 'read_rid_rlast_three_dynamic_transactions'
                        : 'read_rid_rlast_two_dynamic_transactions'
                    : $depth == 3
                        ? 'read_rid_three_dynamic_transactions'
                        : 'read_rid_two_dynamic_transactions';
        }
...
sub _same_id_ordering_policy_has_dynamic_issue_order_queue($policy) {
    return 0 unless ref($policy) eq 'HASH';
    return ref($policy->{dynamic_id_reuse}) eq 'HASH'
        && ($policy->{dynamic_id_reuse}{policy} // '') eq 'issue_order_queue';
}
...
sub _build_same_id_issue_order_queue_behavior(%args) {
    ...
    my @dynamic_issue_order_queue_families = grep {
...
sub _response_demux_family_has_dynamic_issue_order_queue_contract($response_demux, $family) {
    return 0 unless ref($response_demux) eq 'HASH';
    my $entry = $response_demux->{$family};
    return 0 unless ref($entry) eq 'HASH';
    return _response_demux_transaction_completion_source_is_dynamic_issue_order_queue(
        $entry->{transaction_completion_source},
    );
}
...
sub _response_demux_transaction_completion_source_is_dynamic_issue_order_queue($source) {
    ...
    return 1 if ($source // '') eq 'generated_mixed_dynamic_static_issue_order_queue_demux_last_beat';
    return 0;
}
...
sub _build_dynamic_same_id_issue_order_queue_behavior(%args) {
    ...
    my $entry = $demux->{$family_name};
    return undef unless ref($entry) eq 'HASH' && !$entry->{generated_behavior};
    return undef unless _response_demux_transaction_completion_source_is_dynamic_issue_order_queue(
        $entry->{transaction_completion_source},
    );
    ...
    my $group = $groups->[0];
    my $group_depth = ref($group) eq 'HASH' ? ($group->{depth} // 0) : 0;
    ...
    my $mixed_dynamic_static_queue = ($entry->{transaction_completion_source} // '')
        =~ /\Agenerated_mixed_dynamic_static_issue_order_queue_demux(?:_last_beat)?\z/;
    my $supported_dynamic_queue = $group_depth == 2 && $transaction_count == 2;
    $supported_dynamic_queue ||= $family_name eq 'write'
        && $group_depth == 3
        && $transaction_count == 3;
    $supported_dynamic_queue ||= $family_name eq 'read'
        && ($entry->{response_scope} // '') eq 'single_beat'
        && !defined($entry->{last_signal})
        && $group_depth == 3
        && $transaction_count == 3;
    ...
    for my $slot (0 .. ($group_depth - 1)) {
        for my $transaction (@transactions) {
            my $signal = "${prefix}_slot${slot}_$transaction->{transaction}_q";
            $slot_signal{$slot}{$transaction->{transaction}} = $signal;
            push @storage, { name => $signal, width => 1 };
        }
        my $id_signal = "${prefix}_slot${slot}_id_q";
        $slot_id_signal{$slot} = $id_signal;
        push @storage, { name => $id_signal, width => $id_family->{width} };
    }
...
sub _same_id_issue_order_queue_report_groups($family) {
    ...
    return [
        map {
            ...
            $report->{concrete_id} = $group->{concrete_id}
                if defined $group->{concrete_id};
            for my $field (qw(
                runtime_id_queue_key
                request_id_source
                response_demux_strategy
            )) {
                $report->{$field} = $group->{$field}
                    if defined $group->{$field};
            }
            if ($group->{dynamic_issue_order_queue}) {
                $report->{same_transaction_recapture_policy} =
...
sub _same_id_issue_order_queue_signal_names($behavior) {
    ...
    for my $family_name (qw(write read)) {
        for my $group (_same_id_issue_order_queue_groups($behavior, $family_name)) {
            push @signals, map { _same_id_issue_order_queue_storage_entry_name($_) }
                @{$group->{storage} || []};
        }
    }
...
sub _same_id_issue_order_queue_storage_entry_name($entry) {
    return $entry->{name} if ref($entry) eq 'HASH';
    return $entry;
}

sub _same_id_issue_order_queue_storage_entry_width($entry) {
    return $entry->{width} if ref($entry) eq 'HASH' && defined $entry->{width};
    return 1;
}
...
sub _response_demux_dynamic_request_id_inputs($contract) {
    ...
    for my $family (qw(write read)) {
        my $entry = $demux->{$family};
        next unless ref($entry) eq 'HASH' && $entry->{generated_behavior};
        push @signals, $entry->{request_id_source}
            if _response_demux_transaction_completion_source_is_dynamic_issue_order_queue(
                $entry->{transaction_completion_source},
            )
...
sub _response_demux_dynamic_release_recapture_rule_lines($contract) {
    my @lines;
    for my $state (_response_demux_dynamic_transaction_states($contract)) {
        next unless _response_demux_state_has_dynamic_release_recapture($state);
        my @extra_guards;
        if (_response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_recapture($state)) {
            @extra_guards = (
...
sub _response_demux_state_has_dynamic_release_recapture($state) {
    ...
    return 1 if _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_recapture($state);
    return 0;
}

sub _response_demux_state_has_single_active_dynamic_recapture($state) {
    my $family = $state->{family} // '';
    my $policy = $state->{same_cycle_release_recapture_policy} // '';
    return 1 if $family eq 'write' && $policy eq 'single_active_dynamic_write';
    return 1 if $family eq 'read' && $policy eq 'single_active_dynamic_read';
    return 0;
}

sub _response_demux_state_has_multi_active_dynamic_write_recapture($state) {
    my $family = $state->{family} // '';
    my $policy = $state->{same_cycle_release_recapture_policy} // '';
    return $family eq 'write' && $policy eq 'multi_active_unique_dynamic_write';
}

sub _response_demux_state_has_multi_active_dynamic_read_recapture($state) {
    my $family = $state->{family} // '';
    my $policy = $state->{same_cycle_release_recapture_policy} // '';
    return $family eq 'read' && $policy eq 'multi_active_unique_dynamic_read';
}

sub _response_demux_state_has_multi_active_dynamic_recapture($state) {
    return 1 if _response_demux_state_has_multi_active_dynamic_write_recapture($state);
    return 1 if _response_demux_state_has_multi_active_dynamic_read_recapture($state);
    return 0;
}
...
sub _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_write_recapture($state) {
    my $family = $state->{family} // '';
    my $policy = $state->{same_cycle_release_recapture_policy} // '';
    return $family eq 'write' && $policy eq 'mixed_dynamic_static_multi_active_dynamic_write';
}

sub _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_read_recapture($state) {
    my $family = $state->{family} // '';
    my $policy = $state->{same_cycle_release_recapture_policy} // '';
    return $family eq 'read' && $policy eq 'mixed_dynamic_static_multi_active_dynamic_read';
}

sub _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_recapture($state) {
    return 1 if _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_write_recapture($state);
    return 1 if _response_demux_state_has_mixed_dynamic_static_multi_active_dynamic_read_recapture($state);
    return 0;
}
...
sub _same_id_issue_order_queue_storage_lines($contract) {
    ...
    return map {
        "    (var " . _same_id_issue_order_queue_storage_entry_name($_)
            . " (width " . _same_id_issue_order_queue_storage_entry_width($_) . "))"
    } @signals;
}
        ...
        for my $group (_same_id_issue_order_queue_groups($behavior, $family_name)) {
            for my $transition (_same_id_issue_order_queue_transition_specs($group)) {
                push @lines, _auto_id_rule(
                    $transition->{name},
                    $transition->{guard},
                    _same_id_issue_order_queue_assignments($group, $transition->{to}, $transition->{from}, $transition),
                );
            }
        }
    }
...
sub _same_id_issue_order_queue_transition_specs($group) {
    return _dynamic_same_id_issue_order_queue_transition_specs($group)
        if $group->{dynamic_issue_order_queue};
...
sub _dynamic_same_id_issue_order_queue_transition_specs($group) {
    my @transactions = grep { ref($_) eq 'HASH' } @{$group->{transactions} || []};
    return () unless @transactions;

    my @names = map { $_->{transaction} } @transactions;
    my %enqueue_expr = map { $_->{transaction} => $_->{request_acceptance_expr} // $_->{admitted_pulse} } @transactions;
    my %selected_match_expr = map {
        $_ => _dynamic_same_id_issue_order_queue_selected_match_expr(
            $group,
            $_,
            include_response => 1,
            include_last     => 1,
    ...
    my @specs;
    for my $from (_same_id_issue_order_queue_state_sequences(\@names, $group->{depth})) {
        my %present = map { $_ => 1 } @$from;
        my @dequeue_options = (undef, @$from);

        for my $dequeue (@dequeue_options) {
            my @after_dequeue = defined $dequeue
                ? grep { $_ ne $dequeue } @$from
                : @$from;
            my %remaining = map { $_ => 1 } @after_dequeue;
            my @enqueue_options = (undef);
            push @enqueue_options, grep { !$remaining{$_} } @names
            ...
            for my $enqueue (@enqueue_options) {
                ...
                my @dequeue_terms = defined $dequeue
                    ? (
                        $selected_match_expr{$dequeue},
                        map { $_ eq $dequeue ? () : _not_expr($selected_match_expr{$_}) } @names,
                    )
                    : map { _not_expr($selected_match_expr{$_}) } @names;
                my @enqueue_terms = defined $enqueue
                    ? (
                        $enqueue_expr{$enqueue},
                        map { $_ eq $enqueue ? () : _not_expr($enqueue_expr{$_}) } @names,
                ...
                push @specs, {
                    name    => "$group->{prefix}_" . _dynamic_same_id_issue_order_queue_transition_suffix($from, $dequeue, $enqueue),
                    from    => [@$from],
                    to      => \@to,
                    dequeue => $dequeue,
                    enqueue => $enqueue,
...
sub _same_id_issue_order_queue_assignments($group, $state, $from_state = undef, $transition = undef) {
    return _dynamic_same_id_issue_order_queue_assignments($group, $state, $from_state, $transition)
        if $group->{dynamic_issue_order_queue};
...
sub _dynamic_same_id_issue_order_queue_assignments($group, $state, $from_state = undef, $transition = undef) {
    my %occupied;
    for my $slot (0 .. $#$state) {
        $occupied{$slot}{$state->[$slot]} = 1;
    }
    ...
    my %from_occupied;
    my %from_slot_by_transaction;
    if (defined $from_state) {
        for my $slot (0 .. $#$from_state) {
            my $transaction = $from_state->[$slot];
            $from_occupied{$slot}{$transaction} = 1;
            $from_slot_by_transaction{$transaction} = $slot;
        }
    }

    my $dequeue = ref($transition) eq 'HASH' ? $transition->{dequeue} : undef;
    my $enqueue = ref($transition) eq 'HASH' ? $transition->{enqueue} : undef;
    my %retained_slot_by_transaction;
    if (defined $from_state) {
        for my $slot (0 .. $#$from_state) {
            my $transaction = $from_state->[$slot];
            next if defined($dequeue) && $transaction eq $dequeue;
            $retained_slot_by_transaction{$transaction} = $slot;
        }
    }
    ...
    for my $slot (0 .. (($group->{depth} // 0) - 1)) {
        my $to_transaction = $state->[$slot];
        my $from_transaction = defined($from_state) ? $from_state->[$slot] : undef;
        my $id_signal = $group->{slot_id_signals}{$slot};
        if (!defined $to_transaction) {
            push @assignments, [$id_signal, _sized_decimal_literal($group->{id_width}, 0)]
                if defined $from_transaction;
            next;
        }

        if (exists $retained_slot_by_transaction{$to_transaction}) {
            my $from_slot = $retained_slot_by_transaction{$to_transaction};
            next if defined($from_state) && $from_slot == $slot;
            push @assignments, [$id_signal, $group->{slot_id_signals}{$from_slot}];
            next;
        }
...
sub _same_id_issue_order_queue_head_match_expr($group, $transaction_name) {
    return _dynamic_same_id_issue_order_queue_selected_match_expr(
        $group,
        $transaction_name,
        include_response => 1,
        include_last     => 1,
...
sub _same_id_issue_order_queue_active_match_expr($group, $transaction_name) {
    return _dynamic_same_id_issue_order_queue_selected_match_expr($group, $transaction_name, include_response => 0)
        if $group->{dynamic_issue_order_queue};
...
sub _same_id_issue_order_queue_remaining_after_dequeue_expr($group, $transaction_name) {
    return _dynamic_same_id_issue_order_queue_remaining_after_dequeue_expr($group, $transaction_name)
        if $group->{dynamic_issue_order_queue};
...
sub _dynamic_same_id_issue_order_queue_raw_slot_match_expr($group, $slot, %args) {
    my @terms = (
        _same_id_issue_order_queue_slot_any_expr($group, $slot),
        _eq_expr($group->{response_id_signal}, $group->{slot_id_signals}{$slot}),
    );
    unshift @terms, $group->{response_event}
        if $args{include_response};
    push @terms, $group->{last_signal}
        if $args{include_last} && defined $group->{last_signal};
    return _and_expr(@terms);
}

sub _dynamic_same_id_issue_order_queue_selected_slot_match_expr($group, $slot, %args) {
    return _and_expr(
        _dynamic_same_id_issue_order_queue_raw_slot_match_expr($group, $slot, %args),
        map {
            _not_expr(_dynamic_same_id_issue_order_queue_raw_slot_match_expr($group, $_, %args))
        } 0 .. ($slot - 1),
    );
}

sub _dynamic_same_id_issue_order_queue_selected_match_expr($group, $transaction_name, %args) {
    return _or_expr(map {
        _and_expr(
            _dynamic_same_id_issue_order_queue_selected_slot_match_expr($group, $_, %args),
            $group->{slot_signals}{$_}{$transaction_name},
        )
    } 0 .. (($group->{depth} // 0) - 1));
}

sub _dynamic_same_id_issue_order_queue_remaining_after_dequeue_expr($group, $transaction_name) {
    return _or_expr(map {
        _and_expr(
            $group->{slot_signals}{$_}{$transaction_name},
            _not_expr(_dynamic_same_id_issue_order_queue_selected_slot_match_expr(
                $group,
                $_,
                include_response => 1,
                include_last     => 1,
...
sub _same_id_issue_order_queue_assertion_specs_for_group($group) {
    return _dynamic_same_id_issue_order_queue_assertion_specs_for_group($group)
        if $group->{dynamic_issue_order_queue};
...
sub _dynamic_same_id_issue_order_queue_assertion_specs_for_group($group) {
    return () unless @{$group->{transactions} || []};

    my $prefix = $group->{prefix};
    my @slot_indices = 0 .. (($group->{depth} // 0) - 1);
    my @slot_any = map { _same_id_issue_order_queue_slot_any_expr($group, $_) } @slot_indices;
    my @enqueue_pulses = map { $_->{request_acceptance_expr} // $_->{admitted_pulse} } @{$group->{transactions} || []};
    my @raw_selected_matches = map {
        _dynamic_same_id_issue_order_queue_selected_match_expr($group, $_->{transaction}, include_response => 1)
    } @{$group->{transactions} || []};
    my @final_selected_matches = map {
        _dynamic_same_id_issue_order_queue_selected_match_expr(
            $group,
            $_->{transaction},
            include_response => 1,
            include_last     => 1,
    ...
    my $dequeue = _or_expr(@final_selected_matches);
    ...
    for my $slot (@slot_indices) {
        push @assertions, {
            ...
            message   => "$group->{family} dynamic same-ID issue-order queue slot $slot is one-hot-or-empty",
        };
    }
    ...
    push @assertions,
        {
            ...
            message   => "$group->{family} dynamic same-ID issue-order queue is compact",
        },
        {
        ...
        },
        {
            ...
            message   => "$group->{family} dynamic same-ID issue-order queue enqueue has space or selected dequeue",
        },
        {
            name      => "${prefix}_response_requires_nonempty",
            condition => _implies_expr($group->{response_event}, $slot_any[0]),
            message   => "$group->{family} dynamic same-ID response requires a nonempty queue",
        },
        {
            name      => "${prefix}_response_has_selected_match",
            condition => _implies_expr($group->{response_event}, _or_expr(@raw_selected_matches)),
            message   => "$group->{family} dynamic same-ID response matches at least one captured runtime ID",
        },
        {
            name      => "${prefix}_response_selected_match_onehot0",
            condition => _implies_expr(
                $group->{response_event},
                _same_id_at_most_one_expr(@raw_selected_matches),
            ),
            message   => "$group->{family} dynamic same-ID response selects at most one transaction",
        },
        {
            name      => "${prefix}_dequeue_requires_nonempty",
            condition => _implies_expr($dequeue, $slot_any[0]),
            message   => "$group->{family} dynamic same-ID issue-order queue dequeue requires a nonempty queue",
        };
    push @assertions, {
        name      => "${prefix}_nonlast_no_dequeue",
        condition => _implies_expr(
            _and_expr($group->{response_event}, _or_expr(@raw_selected_matches), _not_expr($group->{last_signal})),
            _not_expr($dequeue),
        ),
        message   => "$group->{family} dynamic same-ID non-last response beat does not dequeue",
    } if defined $group->{last_signal};

    for my $transaction (@{$group->{transactions} || []}) {
        my $name = $transaction->{transaction};
        my $selected_match = _dynamic_same_id_issue_order_queue_selected_match_expr(
            $group,
            $name,
            include_response => 1,
            include_last     => 1,
        ...
        push @assertions,
            {
                ...
                message   => "$group->{family} dynamic same-ID issue-order queue transaction $name appears in at most one slot",
            },
            {
                name      => "${prefix}_${name}_no_duplicate_after_dequeue",
                condition => _implies_expr(
                    $transaction->{request_acceptance_expr} // $transaction->{admitted_pulse},
                    _not_expr(_dynamic_same_id_issue_order_queue_remaining_after_dequeue_expr($group, $name)),
                ),
                message   => "$group->{family} dynamic same-ID issue-order queue enqueue for $name does not duplicate a remaining transaction",
            },
            {
                name      => "${prefix}_${name}_completion_selected_match",
                condition => _implies_expr($transaction->{completion_event}, $selected_match),
                message   => "$group->{family} dynamic same-ID issue-order queue completion for $name follows selected runtime-ID match",
            };
    }
...
sub _dynamic_same_id_issue_order_queue_transition_suffix($from, $dequeue, $enqueue) {
    my $from_label = @$from ? join('_', @$from) : 'empty';
    return "${from_label}_dequeue_${dequeue}_enqueue_$enqueue"
        if defined($dequeue)
            && defined($enqueue)
            && $dequeue ne $enqueue
            && @$from > 1;
    return _same_id_issue_order_queue_transition_suffix($from, $dequeue, $enqueue);
}
...
sub _response_demux_mixed_dynamic_static_assertion_specs_for_family($contract, $family, $dynamic_states, $static_states) {
    ...
    if (@$dynamic_states > 1) {
        for my $dynamic_state (@$dynamic_states) {
            ...
            push @assertions, {
                name      => $dynamic_state->{request_no_active_same_id_assertion},
                condition => _implies_expr(
                    $dynamic_state->{request_acceptance_expr},
                    _not_expr(_or_expr(@active_same_id_exprs)),
                ),
                message   => "$contract->{name} $dynamic_state->{transaction} dynamic request does not reuse an active sibling ID",
            };
        }

        for my $left_index (0 .. $#$dynamic_states) {
            for my $right_index ($left_index + 1 .. $#$dynamic_states) {
                my $left = $dynamic_states->[$left_index];
                my $right = $dynamic_states->[$right_index];
                push @assertions, {
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'dynamic transaction IDs fail closed with same-family auto_id_lifecycle, concrete-id-reuse same_id_ordering assumptions that do not also select dynamic-id-reuse reject or dynamic-id-reuse issue-order-queue, dynamic response_demux plus same-family dynamic-id-reuse policy, mixed dynamic/static response_demux outside the selected one-dynamic plus one-, two-, or three-concrete-static write BID demux, two-dynamic plus one-concrete-static write BID demux, one-dynamic plus one-, two-, or three-concrete-static read RID/RID-and-RLAST demux, and two-dynamic plus one-concrete-static read single-beat RID or burst-last RID/RLAST demux contracts, and dynamic read_data shapes outside selected scalar single-beat, scalar last-beat, queue-backed scalar single-beat, queue-backed scalar last-beat, queue-backed report-only/runtime-assertion raw-ARLEN scalar last-beat, queue-backed runtime-assertion raw-ARLEN multi-beat output-bank, report-only raw-ARLEN scalar last-beat, runtime-assertion raw-ARLEN scalar last-beat, or runtime-assertion raw-ARLEN multi-beat output-bank generated dynamic read response_demux until those dynamic matching shapes are explicitly owned',
            ...
            'same_id_ordering_policy accepts explicit read/write concrete-id-reuse reject policies, dynamic-id-reuse reject parser/report metadata, dynamic-id-reuse issue-order-queue parser/report metadata, generated bounded two- or three-transaction all-dynamic write BID, two- or three-transaction read single-beat RID, and two- or three-transaction read burst-last RID/RLAST dynamic issue-order queue behavior with state-key-preserving same-transaction captured request-ID refresh for selected response-demux shapes, metadata-only selected-not-generated dynamic issue-order queue reports outside those shapes, and concrete issue-order-queue admitted-request pulse generation, generates bounded read single-beat, read burst-last, or write depth-2/depth-3 concrete same-ID queue state plus queue-head response demux for selected public response-demux-only shapes, including multiple independent read single-beat, read burst-last, and write groups, gates generated multi-group queue-head admitted requests with counted request-set capacity fit guards, replaces those counted families family-wide request onehot assertions with per-concrete-ID group request assertions, and supports selected single-group read single-beat depth-3 scalar read-data queue-head shape, selected single-group read burst-last depth-3 scalar last-beat read-data, report-only raw-ARLEN burst-length, runtime beat-count/RLAST validation, runtime-validation multi-beat output-bank queue-head shapes, selected multiple/mixed depth-3 runtime-validation multi-beat output-bank queue-head shapes, and selected same-family mixed auto-ID plus depth-2 concrete queue-head read burst-last report-only raw-ARLEN burst-length and runtime beat-count/RLAST validation shapes',
            'response_demux requires id_families, transactions, and either selected-family auto_id_lifecycle metadata, selected same-id-ordering concrete-id-reuse issue-order-queue metadata with a duplicate concrete-ID group, one or more all-dynamic selected write transactions for the bounded dynamic write BID demux contracts, exactly two or three all-dynamic selected write transactions plus dynamic-id-reuse issue-order-queue for the bounded dynamic write BID issue-order queue contract, one dynamic plus one, two, or three concrete static write transactions or two dynamic plus one concrete static write transaction for the bounded mixed dynamic/static write BID demux contracts, one or more all-dynamic selected read transactions for the bounded dynamic read RID/RID-and-RLAST demux contracts, exactly two all-dynamic selected read transactions plus dynamic-id-reuse issue-order-queue for the bounded dynamic read RID or RID/RLAST issue-order queue contracts, exactly three all-dynamic selected read single-beat transactions plus dynamic-id-reuse issue-order-queue for the bounded dynamic read RID issue-order queue contract, exactly three all-dynamic selected read burst-last transactions plus dynamic-id-reuse issue-order-queue for the bounded dynamic read RID/RLAST issue-order queue contract, one dynamic plus one concrete static read transaction plus dynamic-id-reuse issue-order-queue and response_scope single-beat or burst-last for the bounded mixed dynamic/static read RID/RID-and-RLAST issue-order queue contracts, or one dynamic plus one, two, or three concrete static read transactions plus the selected two dynamic plus one concrete static read transaction shape for the bounded mixed dynamic/static read RID/RID-and-RLAST demux contracts',
            ...
            'response_demux.read requires response_event equal to read_complete, response_scope single_beat or burst_last, read ID-family metadata, read transactions, and read auto_id_lifecycle metadata, selected concrete same-ID queue-head metadata, selected all-dynamic read transactions, one dynamic plus one, two, or three concrete static read transactions for single-beat or burst-last mixed dynamic/static read contracts, the selected one-dynamic plus one-concrete-static mixed dynamic/static read single-beat or burst-last issue-order queue contract, or two dynamic plus one concrete static read transaction for the selected single-beat or burst-last mixed dynamic/static read contract',
            'response_demux.read response_scope single_beat generates bounded single-beat read RID demux behavior for explicit opt-in auto-ID, concrete queue-head, mixed auto-ID/queue-head, single-active dynamic read, bounded multiple all-dynamic read, selected two- or three-dynamic all-dynamic issue-order queue read, the selected one-dynamic plus one-concrete-static mixed dynamic/static issue-order queue read, bounded one-dynamic plus one-, two-, or three-concrete-static mixed dynamic/static read contracts, or the selected bounded two-dynamic plus one-concrete-static mixed dynamic/static read contract',
            'response_demux.read response_scope burst_last requires one-bit last_signal metadata and generates matched-RID-and-RLAST last-beat completion behavior for explicit opt-in auto-ID, concrete queue-head, mixed auto-ID/queue-head, single-active dynamic read, bounded multiple all-dynamic read, selected two- or three-dynamic all-dynamic issue-order queue read, the selected one-dynamic plus one-concrete-static mixed dynamic/static issue-order queue read, bounded one-dynamic plus one-, two-, or three-concrete-static mixed dynamic/static read contracts, or the selected bounded two-dynamic plus one-concrete-static mixed dynamic/static read contract',
            'response_demux transaction_completion must be generated; selected auto-ID families make transaction completion names generated demux pulse outputs; bounded concrete same-ID queue-head response-demux shapes make transaction completion names generated queue-head demux pulse outputs; selected all-dynamic dynamic-id-reuse issue-order-queue shapes make transaction completion names generated dynamic issue-order queue demux pulse outputs; selected mixed dynamic/static dynamic-id-reuse issue-order-queue shapes make transaction completion names generated mixed dynamic/static issue-order queue demux pulse outputs; same-family mixed auto-ID plus concrete queue-head response-demux shapes make both auto-ID and queue-head transaction completion names generated demux pulse outputs; selected all-dynamic response-demux families make selected dynamic transaction completion names generated dynamic demux pulse outputs; selected mixed dynamic/static write response-demux makes selected write transaction completion names generated mixed dynamic/static demux pulse outputs, including the bounded one-dynamic plus two- or three-concrete-static write shapes and the bounded two-dynamic plus one-concrete-static write shape through generated multi mixed dynamic/static demux pulse outputs; selected mixed dynamic/static read response-demux makes selected read transaction completion names generated mixed dynamic/static demux pulse outputs, including the bounded one-dynamic plus two- or three-concrete-static read single-beat and burst-last shapes and the bounded two-dynamic plus one-concrete-static read single-beat or burst-last shape through generated multi mixed dynamic/static read demux pulse outputs',
            'concrete same-ID queue-head response_demux is generated for bounded depth-2/depth-3 response-demux-only shapes: one-or-more-group read single-beat, read burst-last, or write groups; standalone queue-head shapes require issue-order-queue policy and duplicate concrete-ID groups, while same-family mixed auto-ID plus concrete queue-head response_demux is supported for selected response-demux-only read single-beat, read burst-last, and write shapes with one or more auto-ID transactions plus duplicate concrete same-ID groups; read_data consumption is supported for one-or-more generated depth-2 read single-beat queue-head groups, one selected generated depth-3 read single-beat queue-head group, or selected multiple/mixed depth-3 read single-beat queue-head groups through generated scalar capture, plus one-or-more generated depth-2 read burst-last queue-head groups, one selected generated depth-3 read burst-last queue-head group with no burst_length metadata, report-only raw-ARLEN burst-length metadata, runtime-assertion beat-count/RLAST validation metadata, or selected runtime-assertion multi-beat output-bank capture, and selected multiple/mixed depth-3 read burst-last queue-head groups through generated scalar last-beat capture with no burst_length metadata, report-only raw-ARLEN burst-length metadata, runtime-assertion beat-count/RLAST validation metadata, or runtime-assertion multi-beat output-bank capture; read_data consumption for same-family mixed auto-ID plus concrete queue-head response_demux is supported for selected read single-beat scalar, read burst-last scalar last-beat, and read burst-last report-only raw-ARLEN burst-length or runtime-assertion beat-count/RLAST validation shapes with one auto-ID transaction plus one depth-2 concrete same-ID read queue group; dynamic read_data consumption is supported for one-or-more generated all-dynamic read transactions in scalar single-beat mode with no dynamic burst_length metadata, scalar last-beat mode with no dynamic burst_length metadata, scalar last-beat mode with report-only raw-ARLEN dynamic burst_length metadata or runtime-assertion raw-ARLEN beat-count/RLAST validation metadata, and runtime-assertion raw-ARLEN multi-beat output-bank mode with complete all-dynamic transaction output-bank bindings; dynamic issue-order queue read_data consumption is supported for selected two-transaction generated all-dynamic read issue-order queue response_demux in scalar single-beat mode with no burst_length metadata, scalar last-beat mode with no burst_length metadata, scalar last-beat mode with report-only raw-ARLEN burst_length metadata or runtime-assertion raw-ARLEN beat-count/RLAST validation metadata, and runtime-assertion raw-ARLEN multi-beat output-bank mode with complete two-transaction output-bank bindings, and for the selected three-transaction depth-3 generated all-dynamic read burst-last issue-order queue response_demux in scalar last-beat mode with no burst_length metadata, report-only raw-ARLEN burst_length metadata, runtime-assertion raw-ARLEN beat-count/RLAST validation metadata, or runtime-assertion raw-ARLEN multi-beat output-bank mode with complete three-transaction output-bank bindings; mixed dynamic/static issue-order queue read_data consumption is supported for the selected one-dynamic plus one-concrete-static generated mixed dynamic/static read issue-order queue response_demux in scalar single-beat and scalar last-beat modes with no burst_length metadata, plus scalar last-beat mode with report-only raw-ARLEN burst_length metadata, runtime-assertion raw-ARLEN beat-count/RLAST validation metadata, or runtime-assertion raw-ARLEN multi-beat output-bank mode with complete two-transaction output-bank bindings; mixed dynamic/static read_data consumption is supported for selected one-dynamic plus one-, two-, or three-concrete-static generated mixed read demux transactions in scalar single-beat and scalar last-beat modes with no burst_length metadata, selected two-dynamic plus one-concrete-static generated mixed read demux transactions in scalar single-beat and scalar last-beat modes with no burst_length metadata, for selected one-dynamic plus one-, two-, or three-concrete-static generated mixed read demux transactions and the selected two-dynamic plus one-concrete-static generated mixed read demux transaction set in scalar last-beat mode with report-only raw-ARLEN burst_length metadata or runtime-assertion raw-ARLEN beat-count/RLAST validation metadata, and for selected one-dynamic plus one-, two-, or three-concrete-static generated mixed read demux transactions and the selected two-dynamic plus one-concrete-static generated mixed read demux transaction set in runtime-assertion raw-ARLEN multi-beat output-bank mode with complete one-dynamic plus one-, two-, or three-concrete-static or two-dynamic plus one-concrete-static transaction output-bank bindings',
            ...
            'read_data.read transaction outputs must exactly cover read response_demux auto transactions, supported generated read queue-head transactions, the supported generated dynamic read transaction set, the supported generated dynamic issue-order queue read transaction set, the supported generated mixed dynamic/static issue-order queue read transaction set, or the supported generated mixed dynamic/static read transaction set',
...
sub _report_response_demux($contract) {
    ...
    for my $family (qw(write read)) {
        ...
        delete $demux->{$family}{dynamic_queue_transaction_state};
...
sub _multi_mixed_dynamic_static_read_response_demux_covers_multi_beat_boundary($contract) {
    my $demux = $contract->{response_demux};
    return 0 unless ref($demux) eq 'HASH'
        && $demux->{generated_behavior}
        && ref($demux->{read}) eq 'HASH'
        && $demux->{read}{generated_behavior}
        && ($demux->{read}{transaction_completion_source} // '') eq 'generated_multi_mixed_dynamic_static_read_demux_last_beat'
        && ($demux->{read}{response_scope} // '') eq 'burst_last';
    ...
    return ref($dynamic_transactions) eq 'ARRAY'
        && ref($static_transactions) eq 'ARRAY'
        && (
            (@$dynamic_transactions == 1 && (@$static_transactions == 2 || @$static_transactions == 3))
            || (@$dynamic_transactions == 2 && @$static_transactions == 1)
        );
}
...
sub _read_data_covers_multi_beat_by_rid_interleaving($contract) {
    ...
    return 0 unless _same_id_ordering_covers_response_demux_family($contract, 'read')
        ...
        || _multi_mixed_dynamic_static_read_response_demux_covers_multi_beat_boundary($contract);
    ...
    if (ref($ordering->{dynamic_id_reuse_policy}) eq 'HASH') {
        for my $family_name (qw(read write)) {
            ...
            for my $field (qw(
                ...
                dynamic_issue_order_queue_covered
