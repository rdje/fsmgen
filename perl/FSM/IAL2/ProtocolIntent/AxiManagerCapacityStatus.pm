...
sub _build_same_id_issue_order_queue_behavior(%args) {
    ...
    for my $group (@$groups) {
        ...
        my $read_burst_last_depth3_shape = $family_name eq 'read'
            ...
            && $group_depth == 3
            && $transaction_count == 3;
        return undef unless $depth2_shape || $read_single_beat_depth3_shape || $read_burst_last_depth3_shape;
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'same_id_ordering_policy accepts explicit read/write concrete-id-reuse reject policies plus issue-order-queue admitted-request pulse generation, and generates bounded read single-beat, read burst-last, or write depth-2 concrete same-ID queue state plus queue-head response demux for selected public sample shapes, including multiple independent read single-beat, read burst-last, and write groups, plus selected single-group read single-beat depth-3 response-demux and scalar read-data queue-head shapes and selected single-group read burst-last depth-3 response-demux-only, scalar last-beat read-data, report-only raw-ARLEN burst-length, runtime beat-count/RLAST validation, and runtime-validation multi-beat output-bank queue-head shapes',
            ...
            'response_demux transaction_completion must be generated; selected auto-ID families make transaction completion names generated demux pulse outputs, while the bounded read single-beat, read burst-last, and write depth-2 concrete same-ID queue-head shapes plus the selected single-group read single-beat and read burst-last depth-3 queue-head response-demux shapes also make transaction completion names generated demux pulse outputs',
            'concrete same-ID queue-head response_demux is generated for bounded two-transaction depth-2 shapes: one-or-more-group read single-beat response-demux-only or scalar read-data capture, one-or-more-group read burst-last response-demux-only, or one-or-more-group write, plus selected single-group read single-beat depth-3 response-demux-only or scalar read-data shapes and selected single-group read burst-last depth-3 response-demux-only or scalar last-beat read-data shapes; all require issue-order-queue policy, duplicate concrete-ID groups, and no same-family auto_id_lifecycle demux, and read_data consumption is supported only for one-or-more generated depth-2 read single-beat queue-head groups or one selected generated depth-3 read single-beat queue-head group through generated read single-beat queue-head scalar capture, plus one-or-more generated depth-2 read burst-last queue-head groups or one selected generated depth-3 read burst-last queue-head group through generated read burst-last queue-head last-beat capture with no burst_length metadata, report-only raw-ARLEN burst-length metadata, runtime-assertion beat-count/RLAST validation metadata, or selected runtime-assertion multi-beat output-bank capture',
