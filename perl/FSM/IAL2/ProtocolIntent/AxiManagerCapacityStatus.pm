...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_queue_head_demux') {
        ...
        if (($multi_group_last_beat_without_burst_length || $multi_group_last_beat_with_report_only_burst_length)
            && ref($groups) eq 'ARRAY'
            && @$groups == 1
        ) {
            my $group = $groups->[0];
        ...
        for my $group (@$groups) {
            ...
            my $valid_depth3_group = ($single_beat_depth3_coverage || $last_beat_depth3_coverage)
                && $depth == 3
                && ref($group_transactions) eq 'ARRAY'
