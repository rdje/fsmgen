...
sub _read_data_response_demux_transaction_coverage(%args) {
    ...
    if ($transaction_completion_source eq 'generated_queue_head_demux') {
        ...
        if ($multi_group_last_beat_without_burst_length && ref($groups) eq 'ARRAY' && @$groups == 1) {
            my $group = $groups->[0];
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'concrete same-ID queue-head response_demux is generated for bounded two-transaction depth-2 shapes: one-or-more-group read single-beat response-demux-only or scalar read-data capture, one-or-more-group read burst-last response-demux-only, or one-or-more-group write, plus selected single-group read single-beat depth-3 response-demux-only or scalar read-data shapes and selected single-group read burst-last depth-3 response-demux-only or scalar last-beat read-data shape; all require issue-order-queue policy, duplicate concrete-ID groups, and no same-family auto_id_lifecycle demux, and read_data consumption is supported only for one-or-more generated depth-2 read single-beat queue-head groups or one selected generated depth-3 read single-beat queue-head group through generated read single-beat queue-head scalar capture, plus one-or-more generated depth-2 read burst-last queue-head groups or one selected generated depth-3 read burst-last queue-head group through generated read burst-last queue-head last-beat capture with no burst_length metadata, generated read burst-last queue-head last-beat capture with report-only raw-ARLEN burst-length metadata, generated read burst-last queue-head last-beat capture with runtime-assertion beat-count/RLAST validation metadata, and generated multi-beat output-bank capture in this slice',
