...
sub _normalize_read_data_read(%args) {
    ...
    confess "AXI manager capacity/status IAL2 contract read_data.read queue-head burst_length runtime validation is not supported in this slice\n"
        if $coverage->{queue_head_response_demux}
            && $capture_scope eq 'last-beat'
            && defined $burst_length
            && ($burst_length->{burst_length_validation} // '') ne 'report_only';
...
sub _build_report(%args) {
    ...
    return {
        ...
        enforced_static_rules => [
            ...
            'concrete same-ID queue-head response_demux is generated only for bounded one-group two-transaction depth-2 shapes: read single-beat, read burst-last, or write, with issue-order-queue policy, duplicate concrete-ID group, no same-family auto_id_lifecycle demux, and read_data consumption supported for generated read single-beat queue-head capture, generated read burst-last queue-head last-beat capture, and report-only raw-ARLEN burst-length capture on generated read burst-last queue-head last-beat capture in this slice',
