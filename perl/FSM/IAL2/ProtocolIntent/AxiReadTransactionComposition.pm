package FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition;
...
sub generate($self, @args) {
    ...
    confess "FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition->generate expects exactly one contract hash reference\n"
    ...
    _add_single_child(
        ...
        object_name      => 'axi_read_transaction_coordinator',
    ...
    confess "Error: AXI read transaction composition expected three generated IAL1 children\n"
        unless @ial1_items == 3;
    confess "Error: AXI read transaction composition expected three generated schedule reports\n"
        unless @schedule_reports == 3;
    confess "Error: AXI read transaction composition expected three generated leaf .fsm artifacts\n"
        unless keys(%fsm_files) == 3;

    my $top = _build_structural_top($contract, \%fsm_files);
    confess "Error: AXI read transaction composition generated duplicate .fsm artifact '$top->{entry_artifact}'\n"
        if exists $fsm_files{$top->{entry_artifact}};
    $fsm_files{$top->{entry_artifact}} = $top->{text};
    push @ial0_items, $top->{ial0_item};

    confess "Error: AXI read transaction composition expected four generated IAL0 artifacts\n"
    ...
    return {
        layer => 'IAL2',
        kind  => 'protocol_intent.axi_read_transaction_composition',
        mode  => $report->{mode},
        generated_ial1 => {
            format => 'isf',
            items  => \@ial1_items,
...
sub _validate_constructor_receiver($class) {
    confess "FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition->new must be called with the FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition class invocant\n"
        unless defined($class) && !ref($class)
            && $class eq 'FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition';
}
...
sub _validate_object_receiver($self, $method) {
    confess "FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition->$method must be called on an FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition object\n"
        unless blessed($self)
            && $self->isa('FSM::IAL2::ProtocolIntent::AxiReadTransactionComposition');
}

sub _normalize_contract($raw) {
    my $kind = _required_scalar($raw, 'kind');
    confess "AXI read transaction composition IAL2 contract kind must be axi_read_transaction_composition\n"
        unless $kind eq 'axi_read_transaction_composition';

    my $protocol = lc _required_scalar($raw, 'protocol');
    confess "AXI read transaction composition IAL2 contract profile must be axi4 in this slice\n"
    ...
    confess "AXI read transaction composition IAL2 contract role must be manager\n"
        unless $role eq 'manager';

    my $clock = _required_identifier($raw, 'clock');
    my $reset = _normalize_reset($raw->{reset});
    confess "AXI read transaction composition reset must be asynchronous active-low in this slice\n"
...
sub _generate_transaction_coordinator($self, $contract) {
    my $isf_name = 'axi_read_transaction_coordinator.isf';
    ...
    return {
        layer => 'IAL1',
        kind  => 'protocol_intent.axi_read_transaction_coordinator',
        mode  => 'coordinator',
        generated_ial1 => {
            format => 'isf',
            name   => $isf_name,
        ...
        report => {
            schema => 'fsmgen.ial2.protocol_intent.axi_read_transaction_coordinator.v1',
            mode   => 'coordinator',
            generated_artifacts => {
                ...
                hdl_entry => {
                    ...
                    module         => 'axi_read_transaction_coordinator',
                    entry_artifact => 'axi_read_transaction_coordinator.fsm',
                },
            },
...
sub _transaction_coordinator_isf($contract) {
    ...
    return join("\n",
        ...
        '',
        "  (rule admit (& (! active_q) (! ar_busy_i) (! r_busy_i) $cmd->{start} (! ${address}[0]) (! ${address}[1]))",
        '    (set active_q 1)',
        ...
        '',
        '  (rule arm_response (& active_q (! response_armed_q) ar_done_i (! r_busy_i))',
        ...
        '',
        '  (rule finish_response (& active_q response_armed_q r_done_i)',
        '    (set active_q 0)',
        '    (set response_armed_q 0)',
        "    (set $status->{busy} 0)",
        "    (set $status->{transaction_done} 1)",
        "    (set $status->{response_id_match} (== captured_rid_i expected_arid_q))",
        "    (set $status->{response_last_match} captured_rlast_i))",
        '',
...
sub _add_single_child(%args) {
    ...
    confess "Error: AXI read transaction composition expected one generated child .fsm for '$args{object_name}'\n"
        unless @fsm_names == 1;
    ...
    for my $fsm_name (@fsm_names) {
        confess "Error: AXI read transaction composition generated duplicate .fsm artifact '$fsm_name'\n"
...
sub _build_structural_top($contract, $fsm_files) {
    ...
    for my $artifact (@child_artifacts) {
        confess "Error: AXI read transaction composition is missing generated child .fsm artifact '$artifact'\n"
    ...
    my @lines = (
        ...
        '  (?wiring:axi_read_transaction_wiring',
        (map { "    $_" } _wiring_lines()),
        '  )',
        ')',
        '',
    ...
    return {
        ...
        report_entry => {
            ...
            port_policy     => {
                ...
                axi_wiring => 'explicit_flat_fixed_single_beat_ar_r_transaction',
            },
        },
    };
}
...
sub _build_report(%args) {
    ...
    return {
        schema => 'fsmgen.ial2.protocol_intent.axi_read_transaction_composition.v1',
        ...
        composition => {
            ...
            topology             => 'flat_fixed_single_beat_ar_r_transaction',
            child_instance_count => 3,
            net_count             => 41,
            resolved_link_count   => 44,
            children => [
                ...
                { role => 'transaction-coordinator', instance_name => 'transaction_coordinator', object_name => 'axi_read_transaction_coordinator' },
            ],
            wiring_policy => 'explicit_flat_fixed_single_beat_ar_r_transaction',
        ...
        ar_driver_reuse => {
            ...
            fixed_metadata      => { arlen => 0, arsize => 2, arburst => 1, arburst_name => 'INCR' },
        },
        r_acceptor_reuse => {
        ...
        transaction_coordinator => {
            actor_name              => 'axi_read_transaction_coordinator',
            ...
            r_arm_policy            => 'arm_after_request_completion',
            busy_policy             => 'admission_through_r_response_retirement',
            request_done_policy     => 'one_pulse_when_ar_completion_arms_r',
            transaction_done_policy => 'one_pulse_after_r_beat_retirement',
            response_id_policy      => 'captured_rid_matches_retained_admitted_arid',
            response_last_policy    => 'captured_rlast_matches_fixed_single_beat_expectation',
            mismatch_policy         => 'terminal_completion_with_match_zero_and_assertion',
            response_status_policy  => 'raw_rresp_capture_not_success_interpretation',
        },
        children => [
...
sub _enforced_static_rules {
    return [
        'profile must be axi4, object must be axi-read-transaction-composition, and role must be manager',
        'clock and asynchronous active-low reset are shared by all three generated children',
        'one idle admission atomically captures aligned address width 32 and ID width 4',
        'AR metadata is fixed to ARLEN 0, ARSIZE 2, and ARBURST INCR for one four-byte beat',
        'flat C4 topology reuses unchanged generated AR driver and R beat acceptor actors plus one transaction coordinator',
        'private AR-start and R-arm/status bindings isolate child-to-child links while captured RID and RLAST fan out from one source',
        'R is armed only after the owned AR request accepts',
        'aggregate busy spans admission through R retirement and request/transaction done are distinct one-cycle pulses',
        'captured RID and RLAST are checked against retained admitted ARID and the fixed-one-beat expectation; either mismatch terminally completes with status zero plus assertion',
        'captured RID width 4, RDATA width 32, RRESP width 2, and RLAST remain raw transaction results and RRESP is not interpreted as success',
...
sub _unsupported_residue {
    return [
        { id => 'axi_read_transaction_composition_capacity_core_integration_deferred', detail => 'Capacity/status read submit/completion, read-data storage, and response-demux integration remain future work.' },
        { id => 'axi_read_transaction_composition_outstanding_queueing_deferred', detail => 'Multiple outstanding reads, queues, adjacent back-to-back admission, and response buffering remain future work.' },
        { id => 'axi_read_transaction_composition_id_allocation_ordering_demux_deferred', detail => 'Dynamic ID allocation/reuse, same-ID ordering, RID demux, and read-data interleaving remain future work.' },
        { id => 'axi_read_transaction_composition_dynamic_multi_beat_deferred', detail => 'Dynamic AR metadata, repeated or multi-beat R receipt, ARLEN/RLAST counting, response aggregation, and burst progression remain future work.' },
        { id => 'axi_read_transaction_composition_narrow_unaligned_attributes_deferred', detail => 'Narrow or unaligned transfers, wrapping bursts, and extended AR attributes remain future work.' },
        { id => 'axi_read_transaction_composition_response_status_aggregation_deferred', detail => 'Raw RRESP is captured; protocol-neutral status mapping, multi-beat aggregation, and error-RDATA usability policy remain future work.' },
        { id => 'axi_read_transaction_composition_extended_r_monitoring_deferred', detail => 'Extended Issue L R sidebands and generated subordinate stall/payload-stability monitoring remain future work.' },
        { id => 'axi_read_transaction_composition_write_channels_deferred', detail => 'Write-address, write-data, and write-response behavior remain outside this read composition.' },
        { id => 'axi_read_transaction_composition_transaction_interface_deferred', detail => 'Decision 0020 protocol-neutral transaction interfaces remain director-gated future work.' },
        { id => 'axi_read_transaction_composition_profile_alias_deferred', detail => '.axi profile-alias surfacing remains unsupported; this source uses generic .ppif only.' },
        { id => 'axi_read_transaction_composition_verification_output_deferred', detail => 'Direct verification-output generation from this IAL2 source remains future work.' },
        { id => 'axi_read_transaction_composition_backend_variants_deferred', detail => 'Direct backend lowering, backend-language variants, and VHDL behavior remain future work.' },
        { id => 'axi_read_transaction_composition_other_protocols_unchanged', detail => 'AHB and APB behavior remain unchanged.' },
    ];
}

sub _required_scalar($raw, $field) {
    confess "AXI read transaction composition IAL2 contract is missing required scalar field '$field'\n"
        unless exists($raw->{$field}) && !ref($raw->{$field});
    return _nonempty_scalar($raw->{$field}, $field);
}

sub _required_hash($raw, $field) {
    confess "AXI read transaction composition IAL2 contract field '$field' must be a hash reference\n"
...
sub _required_identifier_field($raw, $field, $label) {
    confess "AXI read transaction composition IAL2 contract is missing required field '$label'\n"
        unless exists($raw->{$field}) && !ref($raw->{$field});
...
sub _normalize_width_binding($raw, $field, $expected_width) {
    ...
    confess "AXI read transaction composition IAL2 contract $field.width must be $expected_width in this slice\n"
        unless $width == $expected_width;
    return { name => $name, width => $width };
}

sub _normalize_reset($raw_reset) {
    confess "AXI read transaction composition IAL2 contract is missing required reset binding\n"
...
sub _normalize_source_anchors($anchors) {
    confess "AXI read transaction composition IAL2 contract source.anchors must be an array reference\n"
        unless ref($anchors) eq 'ARRAY';
    my @normalized;
    for my $index (0 .. $#$anchors) {
        my $anchor = $anchors->[$index];
        confess "AXI read transaction composition IAL2 contract source.anchors[$index] must be a hash reference\n"
            unless ref($anchor) eq 'HASH';
        for my $required (qw(document section page)) {
            confess "AXI read transaction composition IAL2 contract source.anchors[$index] is missing '$required'\n"
...
sub _reject_duplicate_signal_names(@names) {
    ...
    for my $name (@names) {
        ...
        confess "AXI read transaction composition IAL2 contract duplicates signal '$name'\n"
            if $seen{$name}++;
    }
}

sub _nonempty_scalar($value, $field) {
    confess "AXI read transaction composition IAL2 contract field '$field' must be a non-empty scalar\n"
...
sub _identifier_value($value, $field) {
    ...
    confess "AXI read transaction composition IAL2 contract field '$field' must be an ISF identifier\n"
        unless $identifier =~ /\A[A-Za-z_][A-Za-z0-9_]*\z/;
    return $identifier;
}

sub _positive_integer($value, $field) {
    confess "AXI read transaction composition IAL2 contract field '$field' must be a positive integer\n"
        unless defined($value) && !ref($value) && "$value" =~ /\A[0-9]+\z/ && $value > 0;
    return 0 + $value;
}

sub _bool_value($value, $field) {
    confess "AXI read transaction composition IAL2 contract field '$field' must be boolean 0 or 1\n"
