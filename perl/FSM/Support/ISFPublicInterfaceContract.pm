...
sub isf_public_interface_schedule_report_multi_file_scope {
    return 'single-clock multi-file report transaction/state/dt/storage summaries describe the parent module; actor_network summarizes static ATL actor declarations, resolved library-qualified type metadata and child artifact names, selected parent-handoff event waits, transaction triggers, temporary trigger batches, scalar data movements, and selected generated ATL tops without embedding child reports or HDL event wiring; the bounded generated-child actor-to-actor route set reports each same-source same-sink scalar route through data_movements[] and its generated top through generated_tops[]; generated_composition summarizes generated top, child files, spawned instances, handoffs, and bindings; library_uses summarizes resolved reusable library actor instances and their child scheduled .fsm artifacts; multi-domain reports describe the generated top at the top level and expose per-domain scheduled artifacts plus crossing metadata through clock_domains and crossings; child and domain scheduled .fsm text remains available through lower_result files';
}
