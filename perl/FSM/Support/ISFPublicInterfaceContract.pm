...
sub build_isf_public_interface_contract {
    return {
        ...
        schedule_report_transaction_port_binding_timing_values => isf_public_interface_schedule_report_transaction_port_binding_timing_values(),
        schedule_report_transaction_port_binding_authored_timing_mode_values => isf_public_interface_schedule_report_transaction_port_binding_authored_timing_mode_values(),
...
sub isf_public_interface_actor_shell_value_shape {
    return 'actor_name is scalar; transactions is an array reference; interface is a hash reference; storage is an optional array reference for authored scalar storage and bank declarations when actor-owned storage is declared; scalar width-based storage variables may carry optional parser-validated declarative fields metadata with literal bit ranges, non-overlap, optional access/reset/inline enum metadata, and report-only inferred_storage[].fields projection; actor interface widths, scalar storage widths, bank widths, and bank depths are resolved positive integers from literals, actor constants, actor-local scalar parameters, or qualified imported package scalar constants; transaction-local port widths are resolved positive integers from literals, same-transaction scalar parameter defaults on generated child or direct transactions, actor constants, actor-local scalar parameters, or qualified imported package scalar constants; data-operation explicit width evidence for shift, assemble, and extract accepts positive literal, same-transaction scalar parameter default on generated child or direct transactions, actor constant, actor-local scalar parameter, or qualified imported package scalar constant sources before scheduler publication; constants is an optional array reference for actor-local compile-time integer constants; same-transaction scalar parameter defaults may back static transaction wait counts, positive latency bounds, and positive top-level await-local watchdog limits while shadowing actor-level static names and remaining local lowering inputs; generated child activation overrides for repeat, wait, latency, top-level await-local watchdog, and contract-window transaction parameters are accepted only when they preserve the child default value, with mismatches failing closed until per-activation specialization ships; imported package scalar constants are exposed through bounded package constant symbol metadata and may back actor parameter defaults and generated-child transaction parameter defaults while preserving authored qualified tokens where the published artifact carries package imports, may back generated activation parameter override values while publishing resolved literal bindings, may back static transaction wait counts while publishing the authored qualified token in transaction_waits[], may back static repeat counts, with zero counts lowering as no-op repeat regions, static-zero repeat-body do/spawn activations including syntactically valid parameterized, bound, or domain-annotated variants pruning child/top artifacts when their targets are not otherwise live, and positive counts preserving the authored qualified token in scheduled .fsm repeat-counter loads, and may back positive actor-level and await-local watchdog limits while reporting resolved integers; clock_domains is null for legacy one-clock actors or an optional live metadata hash for accepted clock-domain declarations; crossings is an optional array reference for accepted clock-domain event crossing declarations; actor_network is null when omitted or a bounded static_declaration metadata hash for the current ATL subset, including direct static actor instance metadata from verbose instances or compact instance aliases, resolved library-qualified actor type metadata and child artifact names, report-only static group metadata from verbose groups or compact concurrent aliases, selected parent-handoff event waits, selected parent-handoff transaction triggers, selected temporary trigger batches, selected generated ATL tops, and selected scalar or exact-width vector actor-to-actor data movements including the bounded generated-child reader-to-writer handoff route when present; unsupported qualified ATL trigger and endpoint-aware drive movement variants fail closed before scheduled emission when the qualifier names a static actor instance';
}
...
sub isf_public_interface_actor_shell_timing_shape {
    return 'clock is a non-empty scalar; legacy single-clock actors omitted clock default to clk, and clock is the default-domain clock when clock_domains is present; legacy single-clock actors omitted reset default to async active-low rst_n, while reset is null only when the selected default clock-domain reset is omitted or is a hash with scalar name, kind, and polarity for the default domain; watchdog is a positive integer, omitted watchdog defaults to 65535 (2^16 - 1), and actor-level positive actor constants, actor scalar parameters, or qualified imported package scalar constants resolve to the same scalar; source clock, reset, watchdog, interface, resources, storage, clock-domains, and crossings clauses are singleton actor clauses; multi-domain scheduler lower emits domain-specific scheduled .fsm artifacts plus a generated multi-domain top, and report calls expose bounded domain and crossing metadata';
}
...
sub isf_public_interface_actor_shell_transaction_shape {
    return 'transactions is an array of public transaction shell entries; each entry has unique non-empty scalar name, ports hash with inputs/outputs arrays whose entries carry resolved positive integer widths, clauses array, and optional scalar domain ownership metadata; same-transaction port width parameters, actor constants, actor-local scalar parameters, and qualified imported package scalar constants are accepted when they resolve to positive integers, while clause payload contents remain private scheduler input';
}
...
sub isf_public_interface_schedule_report_transaction_port_binding_timing_values {
    return [
        qw(
            activation_region
...
sub isf_public_interface_schedule_report_transaction_port_binding_authored_timing_mode_values {
    return [
        qw(
            snapshot
            live
...
sub isf_public_interface_schedule_report_watchdog_shape {
    return 'scalar watchdog limit; omitted watchdog clauses default to 65535 (2^16 - 1), and actor-level actor constants, actor scalar parameters, or qualified imported package scalar constants report as resolved integers; await-local watchdog overrides do not change this actor-level report scalar';
}
...
sub isf_public_interface_schedule_report_actor_network_resolved_instance_keys {
    return [
        @{isf_public_interface_schedule_report_actor_network_instance_keys()},
        qw(
            type_resolution
...
sub isf_public_interface_schedule_report_actor_network_group_keys {
    return [
        qw(
            declaration
            members
...
sub isf_public_interface_schedule_report_actor_network_association_schedule_keys {
    return [
        qw(
            association
            context
...
sub isf_public_interface_schedule_report_actor_network_data_movement_keys {
    return [
        qw(
            context
            drive
...
sub isf_public_interface_schedule_report_actor_network_event_wait_keys {
    return [
        qw(
            context
            event
...
sub isf_public_interface_schedule_report_multi_file_scope {
    return 'single-clock multi-file report transaction/state/dt/storage summaries describe the parent module; actor_network summarizes static ATL actor declarations from verbose instances or compact instance aliases, resolved library-qualified type metadata and child artifact names, report-only group metadata from verbose groups or compact concurrent aliases, selected parent-handoff event waits, transaction triggers, temporary trigger batches, scalar or exact-width vector data movements, and selected generated ATL tops without embedding child reports or HDL event wiring; the bounded generated-child actor-to-actor route set reports each same-source same-sink scalar or same-width vector route through data_movements[] and its generated top through generated_tops[]; generated_composition summarizes generated top, child files, spawned instances, handoffs, and bindings; library_uses summarizes resolved reusable library actor instances and their child scheduled .fsm artifacts; multi-domain reports describe the generated top at the top level and expose per-domain scheduled artifacts plus crossing metadata through clock_domains and crossings; child and domain scheduled .fsm text remains available through lower_result files';
}
...
sub isf_public_interface_schedule_report_storage_width_shape {
    return 'positive integer bit width when present; currently present for declared actor-owned storage, inferred scheduler counters, and register storage with known ISF width evidence; declared typed actor-owned storage may also expose authored type and resolved type_kind without exposing raw type-spec hashes; declared scalar actor-owned storage may expose optional fields[] metadata entries with name, msb, lsb, width, and optional access, reset, and enum metadata';
}
...
sub isf_public_interface_live_document_paths {
    return [
        qw(
            ...
            docs/book/src/13l-procedures.md
            docs/book/src/13m-local-variables.md
