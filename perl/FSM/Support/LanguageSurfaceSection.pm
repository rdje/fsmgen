            ...
            entries => [
                ...
                },
                {
                    ...
                    current_boundary => join(' ',
                        ...
                        'Broader mixed issue-order queue cardinality beyond that selected write BID multi-static shape, APB address widths other than 32, wait-count widths other than 4, data widths beyond the selected sideband-aware 16/32-bit boundary, additional APB PPROT policy families, APB back-to-back variants beyond the selected fixed/status, selected sideband-aware fixed/status, selected sideband-aware protected fixed multi-register status, selected sideband-aware data16 fixed multi-register status, no-sideband multi-peripheral status, and sideband-aware multi-peripheral status families, scoreboards, group-local simultaneous enqueue widening, packed burst-vector outputs, alternate full burst payload assembly, aliases beyond .axi and .apb, platform clauses, full AXI manager behavior, direct backend lowering, verification-output generation, backend-language variants, and VHDL remain deferred',
                    ),
                },
                {
                    suffix => '.axi',
