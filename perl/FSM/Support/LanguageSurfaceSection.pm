        ...
        file_surfaces => {
            ...
            entries => [
                ...
                },
                {
                    ...
                    current_boundary => 'bounded public .ppif covers one-channel Valid-Ready sources, multi-channel Valid-Ready bundles, and one-object AXI manager capacity/status sources; every .ppif path lowers through generated .isf before generated .fsm. Support-accounted AXI manager coverage includes capacity/status, ID-family metadata, transaction envelopes and fan-in, concrete-ID assertions, bounded auto-ID lifecycle, same-ID reject and issue-order-queue policy, generated auto-ID write/read response-demux, generated single/last/multi-beat read-data capture, burst-length/runtime validation, scalar RRESP aggregation, one-or-more read burst-last queue-head groups, one-or-more write queue-head groups, read single-beat queue-head response-demux including multiple response-demux-only and scalar read-data groups, selected multiple/mixed depth-3 scalar/runtime/multi-beat queue-head read-data groups, and same-family mixed auto-ID plus concrete queue-head response-demux with scalar read-data for read single-beat and read burst-last plus report-only burst-length, runtime validation, and generated multi-beat output-bank behavior for the read burst-last runtime-validation shape. Broader mixed-family burst-length/runtime validation beyond that selected same-family mixed read burst-last shape, group-local simultaneous enqueue widening, packed burst-vector outputs, alternate full burst payload assembly, aliases, platform clauses, full AXI manager behavior, direct backend lowering, verification-output generation, backend-language variants, and VHDL remain deferred',
                },
            ],
