...
sub build_normalized_semantic_structural_rtl_ir_contract {
    return {
        ...
        guidance => [
            ...
            'The net entry key family describes the current `nets[]` entry schema emitted by composition tops and by direct roots for declaration-only storage/helper nets plus top-level state/standalone-DT enable wires projected from already-prepared direct backend state.',
            'Direct-root structural nets claim only declaration-only storage/helper entries and top-level state/standalone-DT enable-wire entries in this bounded slice; they do not claim DT-specific/LHS WEN/EN wires or assignment connectivity, and those direct entries use a null source and an empty targets array.',
