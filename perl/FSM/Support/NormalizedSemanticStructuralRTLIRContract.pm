...
sub build_normalized_semantic_structural_rtl_ir_contract {
    return {
        ...
        guidance => [
            ...
            'The net entry key family describes the current `nets[]` entry schema emitted by composition tops and by direct roots for declaration-only storage/helper nets projected from the backend internal declaration plan.',
            'Direct-root structural nets do not claim generated enable wires or assignment connectivity in this bounded slice; declaration-only entries use a null source and an empty targets array.',
