...
sub _validate_child_transaction_refs($self, $actor) {
    ...
    for my $ref (@child_refs) {
        ...
        confess "Transaction '$tx_name': when-body nested repeat do supports only local child targets; target '$target' is already generated by another activation site\n"
            if $keyword eq 'do'
                && $label eq 'repeat body'
                && $repeat_parent_label eq 'when body'
                && _repeat_body_do_is_generated_activation($clause, $target, \%generated_children);
...
sub _validate_repeat_body_spawn_subset {
    ...
    my $when_body_repeat = $label eq 'when body' && ($context_depths->{when} // 0) == 1;
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'do') {
            confess "Transaction '$tn': repeat-body do is supported only for top-level repeat clauses or top-level when-body nested repeat clauses\n"
            ...
            confess "Transaction '$tn': when-body nested repeat do supports only local '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
                if $when_body_repeat && ($uses_generated_params || $uses_bindings || $uses_domain);
...
sub _validate_repeat_body_do_subset {
    ...
    my $tn = $ref->{tx_name};
    my $clause = $ref->{clause};
    if (($ref->{repeat_parent_label} // '') eq 'when body') {
        confess "Transaction '$tn': when-body nested repeat do supports only local '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
