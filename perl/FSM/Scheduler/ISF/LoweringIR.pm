        ...
        if ($keyword eq 'do'
            ...
            my $nested_generated_child_do = (
                ...
                && (
                    ...
                    || (($repeat_parent_label eq 'when body' || $repeat_parent_label eq 'switch branch')
                        && $uses_generated_params
                        && (!$uses_bindings || $repeat_parent_label eq 'when body'))
                )
            );
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'do') {
            ...
            if ($when_body_repeat) {
                ...
                confess "Transaction '$tn': switch-branch nested repeat do supports only plain '(do child)' or static '(do child (params ...))' without '(bind ...)' or '(domain ...)' subclauses\n"
    ...
    if (my $nested_do_label = _nested_repeat_local_do_label($ref->{repeat_parent_label})) {
        ...
        if ($nested_do_label eq 'when-body') {
            for my $subclause (@subclauses) {
                confess "Transaction '$tn': when-body nested repeat do supports only plain '(do child)', static '(do child (params ...))', or static bound '(do child (params ...) (bind ...))' without '(domain ...)' subclauses\n"
                    unless ref($subclause) eq 'ARRAY'
                        && @$subclause
                        && defined($subclause->[0])
                        && !ref($subclause->[0])
