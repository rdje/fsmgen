...
sub _assert_when_body_local_do {
    ...
    my $target = $clause->[1];
    my $has_param_overrides = grep {
        ref($_) eq 'ARRAY' && @$_ && defined($_->[0]) && !ref($_->[0]) && $_->[0] eq 'params'
    } @{$clause}[2 .. $#$clause];
    confess "Transaction '$tn': $label generated '(do " . ($target // '?') . " ...)' is not yet supported; a plain or bound local '(do child)' is supported directly in a $label, but a generated child activation (a '(params ...)' override, or a target generated elsewhere) is deferred\n"
        if $has_param_overrides
