...
sub _validate_repeat_body_spawn_subset {
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'do') {
            ...
            confess "Transaction '$tn': $nested_do_label nested repeat do supports only local '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
