...
use FSM::Scheduler::ISF::ATLGeneratedTop qw(
...
my %SUPPORTED_TRANSACTION_CLAUSES = (
    ...
    switch => {
        map { $_ => 1 } qw(
            ...
            extract when store load wait do spawn await_all await_any
        )
    },
    repeat => {
        map { $_ => 1 } qw(
...
sub build_module($self, $actor) {
    ...
    mark_atl_data_link_child_interface_ports(\%child_irs, \@atl_top_instances);
    ...
    if (@atl_top_instances) {
        $parent_ir->{actor_network}{generated_tops} = [
            map { atl_generated_top_report_entry($_) } @atl_top_instances
        ];
    }
...
sub _is_atl_actor_to_pin_handoff_kind {
    my ($kind) = @_;
    return defined($kind)
        && !ref($kind)
        && ($kind eq 'scalar_actor_to_pin_handoff' || $kind eq 'vector_actor_to_pin_handoff');
}
...
sub _ensure_atl_child_data_source_ports {
    ...
    for my $movement (@{$data_movements || []}) {
        next unless (_is_atl_actor_to_pin_handoff_kind($movement->{kind} // '')
        ...
        my $instance_role = _is_atl_actor_route_handoff_kind($movement->{kind} // '')
            ? 'source instance'
            : 'instance';
        my $port_label = (_is_atl_actor_route_handoff_kind($movement->{kind} // '')
                || _is_atl_actor_to_pin_handoff_kind($movement->{kind} // '')) && $width > 1
            ? 'child output port'
...
sub _atl_generated_top_data_links($context, $instance, $trigger, $event_wait, $data_movements, $child_ports) {
    ...
    if (_is_atl_actor_to_pin_handoff_kind($movement->{kind} // '')) {
        my (@links, %top_sink_ports, %child_source_ports, %parent_source_ports);
        for my $route (@movements) {
            my $route_width = $route->{width} || 1;
            confess "$context can only wire resolved child output to top-level output pin data movement when sink is a top-level pin\n"
                unless ($route->{source} // '') eq 'external_handoff'
                    && ($route->{sink} // '') eq 'top_level_pin'
                    && ($route->{sink_instance} // '') eq 'pins';
            confess "$context data movement must source from resolved actor instance '$instance'\n"
                unless ($route->{source_instance} // '') eq $instance;
            ...
            my $top_port = $route->{sink_signal};
            confess "$context data movement '$route->{drive}' requires generated parent source handoff and top sink pin signals\n"
                unless _non_empty_scalar($parent_port) && _non_empty_scalar($top_port);
            confess "$context data movement '$route->{drive}' reuses child output port '$child_port' in the current one-to-one pin-egress subset\n"
                if $child_source_ports{$child_port}++;
            confess "$context data movement '$route->{drive}' reuses top-level output pin '$top_port' in the current one-to-one pin-egress subset\n"
                if $top_sink_ports{$top_port}++;
            confess "$context data movement '$route->{drive}' reuses generated parent source handoff '$parent_port'\n"
                if $parent_source_ports{$parent_port}++;
        ...
        for my $ref (_live_child_action_refs_from_transaction_clauses($tx->{clauses}, $tx->{name}, $actor)) {
            ...
            if (
                ...
                && ($label eq 'transaction body' || $label eq 'repeat body'
                    || $label eq 'when body' || $label eq 'switch branch'
                    || $label eq 'while body' || $label eq 'until body')
...
sub _scan_activation_do_use {
    ...
    for my $clause (@$clauses) {
        ...
        if ($kw eq 'when' || $kw eq 'repeat' || $kw eq 'while' || $kw eq 'until') {
            ...
            my $child_branch_contained_when = (
                $kw eq 'when'
                    && (
                        (
                            $top_level_container
...
sub _actor_param_declarations {
    ...
    for my $param (@{$actor->{params} || []}) {
        ...
        my %resolved_value;
        if (ref($value)
            || (!ref($value) && defined($value) && (_is_hdl_identifier($value) || _is_enum_member_reference($value) || _is_package_constant_path_reference_shape($value))))
        {
    ...
    if (!ref($value)) {
        ...
        if (my $package_constant = _actor_package_constant_reference($actor, $value)) {
            ...
            if (defined $constant_payload) {
                ...
                confess "$context package constant '$package_name.$constant_name' aggregate/member path '$value' remains deferred; actor parameter defaults accept only qualified package scalar constants in this slice\n"
                    if $suffix ne '';
        ...
        if (_is_hdl_identifier($value)) {
            ...
            confess "$context token '$value' is a runtime interface signal; actor parameter defaults accept literals, declared actor constants, earlier scalar actor parameters, enum members, and qualified package scalar constants only\n"
                if _actor_interface_signal_by_name($actor, $value);

            confess "$context token '$value' is not a declared actor constant, earlier scalar actor parameter, enum member, or qualified package scalar constant\n";
        }
        confess "$context uses unsupported parameter value '$value'; actor parameter aggregate/list defaults accept numeric, exact-width, actor constant, earlier scalar actor parameter, enum member, and qualified package scalar constant literal leaves only\n";
    }
...
sub _static_data_width_evidence_value {
    ...
    if (my $package_constant = _actor_package_constant_reference($actor, $token)) {
        my ($package_name, $constant_name, $suffix) = @$package_constant;
        my $constant_payload = _actor_package_constant_payload($actor, $package_name, $constant_name);
        if (defined $constant_payload) {
            confess "Transaction '$tn': $context token '$token' is ambiguous: it matches local enum member '$token' and imported package constant '$token'\n"
                if $suffix eq '' && _actor_local_enum_member_exists($actor, $package_name, $constant_name);
            confess "Transaction '$tn': $context package constant '$package_name.$constant_name' aggregate/member path '$token' remains deferred; data-operation width evidence accepts only qualified package scalar constants in this slice\n"
                if $suffix ne '';
            ...
            my $integer_value = defined($constant_value) && !ref($constant_value)
                ...
                : undef;
            confess "Transaction '$tn': $context package constant '$package_name.$constant_name' must resolve to a positive integer scalar\n"
                unless defined($integer_value) && $integer_value > 0;
            return $integer_value;
        }

        confess "Transaction '$tn': $context references unknown package constant '$token'\n"
            if $suffix eq '' && !_actor_local_enum_member_exists($actor, $package_name, $constant_name);
    }
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'spawn') {
            ...
            my $allowed_branch_spawn_after_prior_await_any_local_do_before_drain =
                ...
                && @pending_spawns;
            my $allowed_branch_spawn_after_prior_await_any_generated_child_do_before_drain =
                ($when_body_repeat || $switch_branch_repeat)
                && $awaiting_multi_pending_drain
                && !$pending_local_do_before_drain
                && $pending_generated_do_before_drain
            ...
            confess "Transaction '$tn': repeat-body spawn cannot follow multi-pending await_any before same-body await_all drains outstanding children\n"
                if $awaiting_multi_pending_drain
                    && !$allowed_branch_spawn_after_prior_await_any_local_do_before_drain
                    && !$allowed_branch_spawn_after_prior_await_any_generated_child_do_before_drain;
            my $allowed_branch_spawn_after_local_do_before_drain =
                ($when_body_repeat || $switch_branch_repeat)
                && $pending_local_do_before_drain
                && !$pending_generated_do_before_drain
                && @pending_spawns;
            ...
            confess "Transaction '$tn': $pending_local_do_label nested repeat spawn cannot follow local do while generated spawns are pending; drain with same-body '(await_all done)' before spawning again\n"
                if defined $pending_local_do_label
                    && $pending_local_do_before_drain
                    && @pending_spawns
                    && !$allowed_branch_spawn_after_local_do_before_drain;
        ...
        if ($keyword eq 'await_all' || $keyword eq 'await_any') {
            ...
            my $allowed_branch_static_parameter_generated_do_before_post_await_any =
                ...
                && @pending_spawns > 1
                && !$awaiting_multi_pending_drain;
            my $allowed_branch_static_bound_generated_do_before_post_await_any =
                ($when_body_repeat || $switch_branch_repeat)
                && !$pending_local_do_before_drain
                && $pending_generated_do_before_drain
                && ($pending_generated_do_kind_before_drain // '') eq 'generated do with static params and bindings'
                ...
                && @pending_spawns > 1
                && !$awaiting_multi_pending_drain;
            my $allowed_branch_static_domain_generated_do_before_post_await_any =
                ($when_body_repeat || $switch_branch_repeat)
                && !$pending_local_do_before_drain
                && $pending_generated_do_before_drain
                && ($pending_generated_do_kind_before_drain // '') eq 'generated do with static params and same-domain metadata'
...
# ISF-CONDITIONAL-CHILD-ACTIVATION: a `(do child)` directly in a branch/loop body
# is supported for the LOCAL (sibling) form — a `(do child)` whose target is a
# sibling transaction (not a generated child), optionally with `(bind ...)` port
# bindings, which `_ir_do`'s non-generated path lowers. A GENERATED child
# activation (a `(do child (params ...))` override, or a target generated by
# another activation site) needs the generated-child instance machinery in the
# branch/loop region and is deferred to a later slice — it fails closed here.
sub _expand_when { my ($cl,$tn,$ir,$ps,$drives,$wd,$widths,$counters,$storage_roles,$actor,$bank_accesses,$spawn_refs,$constant_values,$generated_children,$repeat_do_ordinal_ref)=@_; my @s; my $bstate=_ir_when($cl,$tn,$$ir++); push @s,$bstate; my @body_states; my @lp; my @dps;
    for my $bc(@{$bstate->{body_clauses}}){next unless ref($bc)eq'ARRAY';my$bk=$bc->[0];
        ...
        elsif($bk eq'await_any'){_push_sample_state(\@body_states,$tn,\@lp,$ir);push @body_states,_ir_sync_any($tn,$$ir++,\@dps);@dps=()}
...
sub _expand_switch { my ($cl,$tn,$ir,$ps,$drives,$wd,$widths,$counters,$storage_roles,$actor,$bank_accesses,$spawn_refs,$constant_values,$generated_children,$repeat_do_ordinal_ref)=@_; my $signal=$cl->[1]; my @branches; my @branch_state_names; my @branch_end_names; my %seen_val; my @s;
    for my $i(2..$#$cl){my$br=$cl->[$i];next unless ref($br)eq'ARRAY'&&@$br>=2;my$val=$br->[0];my@bc=@{$br}[1..$#$br];
        ...
        confess "Switch '$tn': duplicate value '$val'\n" if$seen_val{$seen_key}++;my@body_states;my@lp;my@dps;
        for my $bc2(@bc){next unless ref($bc2)eq'ARRAY';my$bk2=$bc2->[0];
            ...
            elsif($bk2 eq'do'){_push_sample_state(\@body_states,$tn,\@lp,$ir);my $cond_ord=scalar(grep { ref($_)eq'HASH'&&$_->{branch_do} } @{$spawn_refs||[]});my $do_ref=_conditional_do_ref_from_clause($bc2,$tn,$cond_ord,$constant_values||{},$actor,$generated_children,'switch branch');if($do_ref->{generated_child}){push @$spawn_refs,_clone_isf_value($do_ref) if ref($spawn_refs)eq'ARRAY';push @body_states,_ir_do($bc2,$tn,$$ir++,$do_ref,'switch branch')}else{push @body_states,_ir_do($bc2,$tn,$$ir++,undef,'switch branch')}}
            elsif($bk2 eq'spawn'){_push_sample_state(\@body_states,$tn,\@lp,$ir);my $sref=_spawn_ref_from_clause($bc2,$tn,$constant_values,$actor,'switch branch');push @$spawn_refs,$sref if ref($spawn_refs)eq'ARRAY';push @dps,"$sref->{instance}_done";push @body_states,_ir_spawn($bc2,$tn,$$ir++)}
            elsif($bk2 eq'await_all'){_push_sample_state(\@body_states,$tn,\@lp,$ir);push @body_states,_ir_sync_all($tn,$$ir++,\@dps);@dps=()}
            elsif($bk2 eq'await_any'){_push_sample_state(\@body_states,$tn,\@lp,$ir);push @body_states,_ir_sync_any($tn,$$ir++,\@dps);@dps=()}
...
sub _expand_loop_body {
    ...
    for my $bc (@$body_clauses) {
        ...
        if ($bk eq 'drive') {
            ...
            my $do_ref = _conditional_do_ref_from_clause($bc, $tn, $cond_ord, $constant_values || {}, $actor, $generated_children, $body_label);
            if ($do_ref->{generated_child}) {
                push @$spawn_refs, _clone_isf_value($do_ref) if ref($spawn_refs) eq 'ARRAY';
                push @states, _ir_do($bc, $tn, $$ir++, $do_ref, $body_label);
            } else {
                push @states, _ir_do($bc, $tn, $$ir++, undef, $body_label);
            }
        } elsif ($bk eq 'spawn') {
            _push_sample_state(\@states, $tn, \@lp, $ir);
            my $sref = _spawn_ref_from_clause($bc, $tn, $constant_values, $actor, $body_label);
            push @$spawn_refs, $sref if ref($spawn_refs) eq 'ARRAY';
            push @dps, "$sref->{instance}_done";
            push @states, _ir_spawn($bc, $tn, $$ir++);
        } elsif ($bk eq 'await_all') {
            _push_sample_state(\@states, $tn, \@lp, $ir);
            push @states, _ir_sync_all($tn, $$ir++, \@dps);
            @dps = ();
        } elsif ($bk eq 'await_any') {
            _push_sample_state(\@states, $tn, \@lp, $ir);
            push @states, _ir_sync_any($tn, $$ir++, \@dps);
            @dps = ();
        } elsif ($bk eq 'repeat') {
...
sub _watchdog_limit_cycles {
    ...
    if (defined($token) && !ref($token) && _is_hdl_identifier($token)) {
        ...
        if ($param) {
            my $param_value = _non_negative_integer_from_literal(_param_resolved_value($param));
            confess "Transaction '$tn': watchdog parameter '$token' must resolve to a positive cycle count in $label\n"
                unless defined($param_value) && $param_value > 0;
            return $param_value;
        }
