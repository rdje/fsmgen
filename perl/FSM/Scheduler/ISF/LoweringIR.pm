...
sub _validate_child_transaction_refs($self, $actor) {
    ...
    for my $ref (@child_refs) {
        ...
        if ($keyword eq 'do'
            ...
            my $when_body_generated_child_do = ($repeat_parent_label eq 'when body')
                && !_repeat_body_do_uses_generated_params($clause)
                && !_repeat_body_do_uses_bindings($clause)
                && !_repeat_body_do_uses_domain($clause)
            ...
            confess "Transaction '$tx_name': $nested_do_label nested repeat do supports only local child targets; target '$target' is already generated by another activation site\n"
                if defined($nested_do_label) && !$when_body_generated_child_do;
        }
...
sub _validate_repeat_body_spawn_subset {
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'do') {
            ...
            my $nested_do_kind = defined($nested_do_label) ? _nested_repeat_plain_do_kind($nested_do_label) : undef;
            confess "Transaction '$tn': $nested_do_label nested repeat do supports only $nested_do_kind '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
                if defined($nested_do_label) && ($uses_generated_params || $uses_bindings || $uses_domain);
...
sub _validate_repeat_body_do_subset {
    ...
    if (my $nested_do_label = _nested_repeat_local_do_label($ref->{repeat_parent_label})) {
        my $nested_do_kind = _nested_repeat_plain_do_kind($nested_do_label);
        confess "Transaction '$tn': $nested_do_label nested repeat do supports only $nested_do_kind '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
...
sub _nested_repeat_plain_do_kind {
    my ($nested_do_label) = @_;
    return 'plain' if ($nested_do_label // '') eq 'when-body';
    return 'local';
}
