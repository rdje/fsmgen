        ...
        if ($keyword eq 'do'
            ...
            my $nested_generated_child_do = (
                ...
                && (
                    ...
                    || ($repeat_parent_label eq 'when body' && $uses_generated_params)
                )
            );
...
sub _validate_repeat_body_spawn_subset {
    ...
    for my $body_clause (@{$clause}[2 .. $#$clause]) {
        ...
        if ($keyword eq 'do') {
            ...
            if ($when_body_repeat) {
                confess "Transaction '$tn': when-body nested repeat do supports only plain '(do child)' or static '(do child (params ...))' without '(bind ...)' or '(domain ...)' subclauses\n"
                    if $uses_bindings || $uses_domain;
            } elsif ($switch_branch_repeat) {
                confess "Transaction '$tn': switch-branch nested repeat do supports only plain '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
                    if $uses_generated_params || $uses_bindings || $uses_domain;
            }
...
sub _validate_repeat_body_do_subset {
    ...
    if (my $nested_do_label = _nested_repeat_local_do_label($ref->{repeat_parent_label})) {
        ...
        if ($nested_do_label eq 'when-body') {
            ...
            confess "Transaction '$tn': $nested_do_label nested repeat do supports only plain '(do child)' without '(params ...)', '(bind ...)', or '(domain ...)' subclauses\n"
                if $uses_generated_params || $uses_bindings || $uses_domain;
        }
        return 1;
    }
