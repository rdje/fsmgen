...
sub _transaction_summary($self, $ir) {
    ...
    for my $s (@{$ir->{states}}) {
        my ($tx_name) = ($s->{name} =~ /^(\w+?)_(?:idle|drive|await|atl_trigger|atl_trigger_batch|done|repeat|sample|max_chk|when|switch|update|set|shift|asm|ext|extract|store|load|do|spawn|phase|stage|contract|wait|while|until)_/);
