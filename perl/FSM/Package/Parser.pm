...
sub parse_package_constants_block ($self, $package_name, $child_ast, $symbols, $expression_builder) {
    ...
    confess
        ...
        "but package symbol section shape is blocked because '+constants' currently requires a non-empty list of '(NAME scalar_value)' entries."
    ...
    for my $constant_def (@$constants_list) {
        confess
            ...
            "but package symbol entry shape is blocked because each '+constants' entry must be a pair '(NAME scalar_value)'."
        ...
        confess
            ...
            "but package symbol token shape is blocked because each '+constants' entry must use an HDL-identifier-compatible name and a scalar value token."
            unless $self->is_contract_identifier($resolved_name)
                && defined($resolved_value)
                && !ref($resolved_value);

        my $canonical_payload = $self->canonicalize_package_symbol_literal_payload(
