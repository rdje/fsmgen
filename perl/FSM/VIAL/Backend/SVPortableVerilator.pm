...
sub _emit($raw) {
    ...
    my $compile = _command_record(
        ...
        arguments => [
            ...
            '--top-module', $top,
            '--Mdir', ".artifacts/tmp/vial/$operation_id/work/$BACKEND_PROFILE/obj",
            map { "$raw->{artifact_root}/$_" } ($dut_rel, $runtime_rel, $tb_rel),
        ],
        inputs => [map { "$raw->{artifact_root}/$_" } ($dut_rel, $runtime_rel, $tb_rel)],
        expected_outputs => [
            ".artifacts/tmp/vial/$operation_id/work/$BACKEND_PROFILE/obj/V$top",
        ],
    );
...
package fsmgen_vial_runtime_pkg;
  ...
  function automatic string vial_trace_record(
    ...
    input longint unsigned sequence,
    input string payload_json
  );
    string sequence_json;
    sequence_json.itoa(sequence);
...
sub _render_fixture(%arg) {
    ...
    $push->('  longint unsigned vial_sequence;');
    ...
    $push->("    if (vial_transaction_active && !vial_transaction_accepted && $hsel && $hready && ($htrans == 2'h2)) begin");
    ...
    $push->('    end');
    $push->("    if (vial_transaction_active && !$hreadyout)");
    $push->('      vial_event_held_count = vial_event_held_count + 1;');
    $push->("    if (vial_transaction_active && $hresp)");
    $push->('      vial_event_error_count = vial_event_error_count + 1;');
    $push->("    if (vial_transaction_active && vial_transaction_accepted && $hreadyout) begin");
    ...
    for my $index (0 .. $#{$execution->{operation_graph}{operations}}) {
        ...
        my @body = _render_operation($op, $execution, $bridge, \%backend, \%endpoint, \%symbol);
        $push->("    $_") for @body;
        $push->('  endtask');
        push @spec, _map_spec(
    ...
    for my $index (0 .. $#{$execution->{scenarios}}) {
        ...
        my $end_payload = $JSON->encode({
            scenario_id => $scenario->{scenario_id},
            status => 'passed',
        });
        my $start = @line + 1;
        $push->("  task automatic $scenario_symbol;");
        $push->('    string run_id_json;');
        $push->('    run_id_json = "' . _sv_string($run_id_json) . '";');
        $push->('    $display("FSMGEN_VIAL_TRACE_V1\\t%s", vial_trace_record("scenario_start", "'
            . $execution->{plan_id} . '", run_id_json, vial_sequence, "'
    ...
    for my $ep (@{$bridge->{endpoints}}) {
        next if $ep->{direction} eq 'output';
        my $name = $backend{$ep->{endpoint_id}}{target_name};
        $push->("    $name = 0;");
    }
    $push->("    $hready = 1;");
    ...
    my @summary = map {{%$_, status => 'passed'}} @runs;
    my $footer_payload = $JSON->encode({
        clean_termination => JSON::PP::true,
        counts => {
            footer => 1,
            header => 1,
            scenario_end => scalar(@runs),
            scenario_start => scalar(@runs),
        },
        scenario_completion_summaries => \@summary,
        status => 'passed',
    });
    $push->('    $display("FSMGEN_VIAL_TRACE_V1\\t%s", vial_trace_record("footer", "'
        . $execution->{plan_id} . '", "null", vial_sequence, "'
        . _sv_string($footer_payload) . '"));');
    $push->('    $finish;');
    $push->('  end');
    push @spec, _map_spec(
...
sub _render_operation($op, $execution, $bridge, $backend, $endpoint, $symbol) {
    ...
    if ($op->{kind} eq 'reset') {
        ...
        return (
            "$reset = $active;",
            "repeat ($input{cycles}) @(posedge $clock);",
            "@(negedge $clock);",
            "$reset = $inactive;",
            'vial_transaction_active = 0;',
            'vial_transaction_accepted = 0;',
            map({ 'vial_event_' . _sv_slug($_->{name}) . '_count = 0;' } @{$execution->{events}}),
        );
    }
    if ($op->{kind} eq 'start') {
    ...
    if ($op->{kind} eq 'await') {
        ...
        return ("while (!($condition)) vial_inactive_barrier();");
    }
    ...
    if ($op->{kind} eq 'expect') {
        ...
        return (
            "if (!($condition)) begin",
            '  $display("VIAL expectation failed: ' . _sv_string($op->{effects}[0]{target_id}) . '");',
            'end',
        );
    }
    if ($op->{kind} eq 'inject') {
        return ('// The selected substitution is folded into the following transaction drive.');
    }
    if ($op->{kind} eq 'scoreboard_expect') {
        return ('// Fixed-capacity expected transaction data is statically materialized by this task.');
    }
    if ($op->{kind} eq 'scoreboard_check') {
        return ('// The in-order scoreboard check reads the transaction adapter snapshot.');
    }
    if ($op->{kind} eq 'drive') {
