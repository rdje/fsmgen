...
use FSM::IAL2::ProtocolIntent::AxiWriteBurst4RequestComposition;
...
sub parse_source($self, @args) {
    ...
    return FSM::IAL2::ProtocolIntent::AxiAwDriver->new(debug => $self->{debug})->generate($contract)
    ...
    return FSM::IAL2::ProtocolIntent::AxiReadBurst4TransactionComposition->new(debug => $self->{debug})->generate($contract)
        if _is_axi_read_burst4_transaction_composition_contract($contract);
    ...
    return FSM::IAL2::ProtocolIntent::AxiWriteBurst4RequestComposition->new(debug => $self->{debug})->generate($contract)
        if _is_axi_write_burst4_request_composition_contract($contract);
    ...
    return FSM::IAL2::ProtocolIntent::AxiWriteTransactionComposition->new(debug => $self->{debug})->generate($contract)
        if _is_axi_write_transaction_composition_contract($contract);
...
sub _validate_profile_alias_contract($source_label, $contract) {
    ...
    confess "Error: .axi source '$source_label' (axi-b-response-acceptor ...) remains unsupported for the first profile-alias implementation\n"
        if _is_axi_b_response_acceptor_contract($contract);
    ...
    confess "Error: .axi source '$source_label' (axi-read-burst4-transaction-composition ...) remains unsupported for the first profile-alias implementation\n"
        if _is_axi_read_burst4_transaction_composition_contract($contract);
    ...
    confess "Error: .axi source '$source_label' (axi-write-burst4-request-composition ...) remains unsupported for the first profile-alias implementation\n"
        if _is_axi_write_burst4_request_composition_contract($contract);

    confess "Error: .axi source '$source_label' (axi-write-transaction-composition ...) remains unsupported for the first profile-alias implementation\n"
        if _is_axi_write_transaction_composition_contract($contract);
...
sub _contract_from_root($root, $source_label) {
    ...
    my @axi_write_burst4_request_compositions;
    ...
    for my $clause (@clauses) {
        ...
        if ($head eq 'profile') {
        ...
        } elsif ($head eq 'axi-aw-driver') {
            push @axi_aw_drivers, _parse_axi_aw_driver(\@body, $source_label);
        } elsif ($head eq 'axi-ar-driver') {
        ...
        } elsif ($head eq 'axi-write-burst4-request-composition') {
            push @axi_write_burst4_request_compositions,
                _parse_axi_write_burst4_request_composition(\@body, $source_label);
        ...
        } elsif ($head eq 'axi-read-burst4-transaction-composition') {
            push @axi_read_burst4_transaction_compositions, _parse_axi_read_burst4_transaction_composition(\@body, $source_label);
            ...
            push @axi_write_transaction_compositions, _parse_axi_write_transaction_composition(\@body, $source_label);
    ...
    confess "Error: $surface source '$source_label' is missing required intent object clause, expected (valid-ready-channel ...), (manager-capacity-status ...), (axi-aw-driver ...), (axi-ar-driver ...), (axi-w-driver ...), (axi-w-burst4-driver ...), (axi-write-burst4-request-composition ...), (axi-b-response-acceptor ...), (axi-r-beat-acceptor ...), (axi-read-burst4-transaction-composition ...), (axi-read-transaction-composition ...), (axi-write-request-composition ...), (axi-write-transaction-composition ...), (apb-requester ...), (apb-completer ...), (apb-composition ...), (ahb-requester ...), (ahb-subordinate ...), or (ahb-interconnect ...)\n"
        unless @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_w_burst4_drivers || @axi_write_burst4_request_compositions || @axi_b_response_acceptors || @axi_r_beat_acceptors || @axi_read_burst4_transaction_compositions || @axi_read_transaction_compositions || @axi_write_request_compositions || @axi_write_transaction_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

    if (@axi_write_burst4_request_compositions) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-write-burst4-request-composition ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-write-burst4-request-composition ...) object in this slice\n"
            if @axi_write_burst4_request_compositions > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-write-burst4-request-composition ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_w_burst4_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @axi_read_burst4_transaction_compositions || @axi_read_transaction_compositions || @axi_write_request_compositions || @axi_write_transaction_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_write_burst4_request_compositions[0]},
    ...
    if (@axi_read_burst4_transaction_compositions) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-read-burst4-transaction-composition ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-read-burst4-transaction-composition ...) object in this slice\n"
            if @axi_read_burst4_transaction_compositions > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-read-burst4-transaction-composition ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @axi_read_transaction_compositions || @axi_write_request_compositions || @axi_write_transaction_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_read_burst4_transaction_compositions[0]},
            intent_name => $intent_name,
            protocol    => $profile,
            source      => $source,
        };
    ...
    if (@axi_write_transaction_compositions) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-write-transaction-composition ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-write-transaction-composition ...) object in this slice\n"
            if @axi_write_transaction_compositions > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-write-transaction-composition ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @axi_write_request_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_write_transaction_compositions[0]},
            intent_name => $intent_name,
            protocol    => $profile,
            source      => $source,
        };
    }
    if (@axi_write_request_compositions) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-write-request-composition ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-write-request-composition ...) object in this slice\n"
            if @axi_write_request_compositions > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-write-request-composition ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_write_request_compositions[0]},
            intent_name => $intent_name,
            protocol    => $profile,
            source      => $source,
        };
    }
    if (@axi_b_response_acceptors) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-b-response-acceptor ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-b-response-acceptor ...) object in this slice\n"
            if @axi_b_response_acceptors > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-b-response-acceptor ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_r_beat_acceptors || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_b_response_acceptors[0]},
            intent_name => $intent_name,
            protocol    => $profile,
            source      => $source,
        };
    }
    if (@axi_r_beat_acceptors) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-r-beat-acceptor ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-r-beat-acceptor ...) object in this slice\n"
            if @axi_r_beat_acceptors > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-r-beat-acceptor ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_write_request_compositions || @axi_write_transaction_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;

        return {
            %{$axi_r_beat_acceptors[0]},
            intent_name => $intent_name,
            protocol    => $profile,
            source      => $source,
        };
    ...
    if (@axi_w_drivers) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-w-driver ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-w-driver ...) object in this slice\n"
            if @axi_w_drivers > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-w-driver ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_ar_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;
    ...
    if (@axi_ar_drivers) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-ar-driver ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-ar-driver ...) object in this slice\n"
            if @axi_ar_drivers > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-ar-driver ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_aw_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @axi_write_request_compositions || @axi_write_transaction_compositions || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;
    ...
    if (@axi_aw_drivers) {
        confess "Error: $surface source '$source_label' profile '$profile' does not match (axi-aw-driver ...); expected an AXI-family profile (axi, axi3, axi4, or axi5)\n"
            unless _is_axi_family_profile($profile);
        confess "Error: $surface source '$source_label' supports exactly one (axi-aw-driver ...) object in this slice\n"
            if @axi_aw_drivers > 1;
        confess "Error: $surface source '$source_label' cannot mix (axi-aw-driver ...) with other intent objects in this slice\n"
            if @channels || @managers || @axi_ar_drivers || @axi_w_drivers || @axi_b_response_acceptors || @axi_r_beat_acceptors || @apb_requesters || @apb_completers || @apb_compositions || @ahb_requesters || @ahb_subordinates || @ahb_interconnects;
...
sub _parse_axi_aw_driver($body, $source_label) {
    confess "Error: .ppif (axi-aw-driver ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-aw-driver $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-aw-driver $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_aw_driver_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'channel') {
            $contract{channel} = _parse_axi_aw_driver_channel_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-aw-driver $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command channel)) {
        confess "Error: .ppif (axi-aw-driver $name ...) is missing required ($required ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_aw_driver_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-aw-driver $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
        ...
        [qw(start address id length size burst ready)],
    );
}

sub _parse_axi_aw_driver_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-aw-driver $name channel",
        {
            valid   => 'scalar',
            address => 'width',
            id      => 'width',
        ...
        [qw(valid address id length size burst busy done)],
    );
}

sub _parse_axi_ar_driver($body, $source_label) {
    confess "Error: .ppif (axi-ar-driver ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-ar-driver $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-ar-driver $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_ar_driver_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'channel') {
            $contract{channel} = _parse_axi_ar_driver_channel_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-ar-driver $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command channel)) {
        confess "Error: .ppif (axi-ar-driver $name ...) is missing required ($required ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_ar_driver_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-ar-driver $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
...
sub _parse_axi_ar_driver_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-ar-driver $name channel",
        {
            valid   => 'scalar',
            address => 'width',
            id      => 'width',
...
sub _parse_axi_w_driver($body, $source_label) {
    confess "Error: .ppif (axi-w-driver ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-w-driver $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-w-driver $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_w_driver_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'channel') {
            $contract{channel} = _parse_axi_w_driver_channel_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-w-driver $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command channel)) {
        confess "Error: .ppif (axi-w-driver $name ...) is missing required ($required ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_w_driver_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-w-driver $name command",
        {
            start  => 'scalar',
            data   => 'width',
            strobe => 'width',
        ...
        [qw(start data strobe ready)],
    );
}

sub _parse_axi_w_driver_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-w-driver $name channel",
        {
            valid  => 'scalar',
            data   => 'width',
            strobe => 'width',
...
sub _parse_axi_write_burst4_request_composition($body, $source_label) {
    confess "Error: .ppif (axi-write-burst4-request-composition ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-write-burst4-request-composition $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-write-burst4-request-composition $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_write_burst4_request_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'aw-channel') {
            $contract{aw_channel} = _parse_axi_write_burst4_request_aw_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'w-channel') {
            $contract{w_channel} = _parse_axi_write_burst4_request_w_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'status') {
            $contract{status} = _parse_axi_write_burst4_request_status_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-write-burst4-request-composition $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command aw_channel w_channel status)) {
        my $clause = $required;
        $clause =~ s/_/-/g;
        confess "Error: .ppif (axi-write-burst4-request-composition $name ...) is missing required ($clause ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_write_burst4_request_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-burst4-request-composition $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
        ...
        [qw(start address id data0 data1 data2 data3 strobe0 strobe1 strobe2 strobe3)],
    );
}

sub _parse_axi_write_burst4_request_aw_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-burst4-request-composition $name aw-channel",
        {
            ready   => 'scalar',
            valid   => 'scalar',
            address => 'width',
...
sub _parse_axi_write_burst4_request_w_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-burst4-request-composition $name w-channel",
        {
            ready  => 'scalar',
            valid  => 'scalar',
            data   => 'width',
        ...
        [qw(ready valid data strobe last)],
    );
}

sub _parse_axi_write_burst4_request_status_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-burst4-request-composition $name status",
        {
            busy         => 'scalar',
            'beat-done'  => 'scalar',
            done         => 'scalar',
        ...
        [qw(busy beat-done done beat-index)],
    );
}

sub _parse_axi_write_request_composition($body, $source_label) {
    confess "Error: .ppif (axi-write-request-composition ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-write-request-composition $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-write-request-composition $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_write_request_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'aw-channel') {
            $contract{aw_channel} = _parse_axi_write_request_aw_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'w-channel') {
            $contract{w_channel} = _parse_axi_write_request_w_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'status') {
            $contract{status} = _parse_axi_write_request_status_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-write-request-composition $name ...) has unsupported clause '($head ...)'\n";
        }
    }
    ...
    for my $required (qw(role clock reset command aw_channel w_channel status)) {
        my $clause = $required;
        $clause =~ s/_/-/g;
        confess "Error: .ppif (axi-write-request-composition $name ...) is missing required ($clause ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_write_request_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-request-composition $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
        ...
        [qw(start address id data strobe)],
    );
}

sub _parse_axi_write_request_aw_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-request-composition $name aw-channel",
        {
            ready   => 'scalar',
            valid   => 'scalar',
            address => 'width',
...
sub _parse_axi_write_request_w_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-request-composition $name w-channel",
        {
            ready  => 'scalar',
            valid  => 'scalar',
            data   => 'width',
...
sub _parse_axi_write_request_status_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-request-composition $name status",
        {
            busy => 'scalar',
            done => 'scalar',
        },
...
sub _parse_axi_write_transaction_composition($body, $source_label) {
    confess "Error: .ppif (axi-write-transaction-composition ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-write-transaction-composition $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-write-transaction-composition $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_write_transaction_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'aw-channel') {
            $contract{aw_channel} = _parse_axi_write_transaction_aw_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'w-channel') {
            $contract{w_channel} = _parse_axi_write_transaction_w_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'b-channel') {
            $contract{b_channel} = _parse_axi_write_transaction_b_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'status') {
            $contract{status} = _parse_axi_write_transaction_status_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-write-transaction-composition $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command aw_channel w_channel b_channel status)) {
        my $clause = $required;
        $clause =~ s/_/-/g;
        confess "Error: .ppif (axi-write-transaction-composition $name ...) is missing required ($clause ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_write_transaction_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-transaction-composition $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
...
sub _parse_axi_write_transaction_aw_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-transaction-composition $name aw-channel",
        {
            ready   => 'scalar',
            valid   => 'scalar',
            address => 'width',
...
sub _parse_axi_write_transaction_w_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-transaction-composition $name w-channel",
        {
            ready  => 'scalar',
            valid  => 'scalar',
            data   => 'width',
...
sub _parse_axi_write_transaction_b_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-transaction-composition $name b-channel",
        {
            valid             => 'scalar',
            ready             => 'scalar',
            id                => 'width',
        ...
        [qw(valid ready id response captured-id captured-response)],
    );
}

sub _parse_axi_write_transaction_status_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-write-transaction-composition $name status",
        {
            busy                => 'scalar',
            'request-done'      => 'scalar',
...
sub _parse_axi_read_burst4_transaction_composition($body, $source_label) {
    confess "Error: .ppif (axi-read-burst4-transaction-composition ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-read-burst4-transaction-composition $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-read-burst4-transaction-composition $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_read_burst4_transaction_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'ar-channel') {
            $contract{ar_channel} = _parse_axi_read_burst4_transaction_ar_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'r-channel') {
            $contract{r_channel} = _parse_axi_read_burst4_transaction_r_channel_block(\@items, $source_label, $name);
        } elsif ($head eq 'status') {
            $contract{status} = _parse_axi_read_burst4_transaction_status_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-read-burst4-transaction-composition $name ...) has unsupported clause '($head ...)'\n";
        }
    }
    ...
    for my $required (qw(role clock reset command ar_channel r_channel status)) {
        my $clause = $required;
        $clause =~ s/_/-/g;
        confess "Error: .ppif (axi-read-burst4-transaction-composition $name ...) is missing required ($clause ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_read_burst4_transaction_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-read-burst4-transaction-composition $name command",
        {
            start   => 'scalar',
            address => 'width',
            id      => 'width',
...
sub _parse_axi_read_burst4_transaction_ar_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-read-burst4-transaction-composition $name ar-channel",
        {
            ready   => 'scalar',
            valid   => 'scalar',
            address => 'width',
...
sub _parse_axi_read_burst4_transaction_r_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-read-burst4-transaction-composition $name r-channel",
        {
            valid               => 'scalar',
            ready               => 'scalar',
            id                  => 'width',
...
sub _parse_axi_read_burst4_transaction_status_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-read-burst4-transaction-composition $name status",
        {
            busy                  => 'scalar',
            'request-done'        => 'scalar',
            'beat-done'           => 'scalar',
            'transaction-done'    => 'scalar',
            'response-beat-index' => 'width',
            'response-id-match'   => 'scalar',
            'response-last-match' => 'scalar',
        },
        [qw(busy request-done beat-done transaction-done response-beat-index response-id-match response-last-match)],
    );
}
...
sub _parse_axi_b_response_acceptor($body, $source_label) {
    confess "Error: .ppif (axi-b-response-acceptor ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-b-response-acceptor $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-b-response-acceptor $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_b_response_acceptor_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'channel') {
            $contract{channel} = _parse_axi_b_response_acceptor_channel_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-b-response-acceptor $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command channel)) {
        confess "Error: .ppif (axi-b-response-acceptor $name ...) is missing required ($required ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_b_response_acceptor_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-b-response-acceptor $name command",
        {
            arm => 'scalar',
        },
        [qw(arm)],
...
sub _parse_axi_b_response_acceptor_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-b-response-acceptor $name channel",
        {
            valid               => 'scalar',
            ready               => 'scalar',
            id                  => 'width',
        ...
        [qw(valid ready id response captured-id captured-response busy done)],
    );
}

sub _parse_axi_r_beat_acceptor($body, $source_label) {
    confess "Error: .ppif (axi-r-beat-acceptor ...) requires a scalar object name\n"
        unless @$body >= 1 && !ref($body->[0]) && length($body->[0]);
    ...
    for my $clause (@{$body}[1 .. $#$body]) {
        my ($head, @items) = _clause_parts($clause, $source_label);
        confess "Error: .ppif (axi-r-beat-acceptor $name ...) has duplicate ($head ...) clause\n"
            if $seen{$head}++;

        if ($head =~ /\A(?:role|clock)\z/) {
            confess "Error: .ppif (axi-r-beat-acceptor $name ($head ...)) requires exactly one scalar value\n"
                unless @items == 1 && !ref($items[0]);
            $contract{$head} = $items[0];
        } elsif ($head eq 'reset') {
            $contract{reset} = _parse_reset(\@items, $source_label);
        } elsif ($head eq 'command') {
            $contract{command} = _parse_axi_r_beat_acceptor_command_block(\@items, $source_label, $name);
        } elsif ($head eq 'channel') {
            $contract{channel} = _parse_axi_r_beat_acceptor_channel_block(\@items, $source_label, $name);
        } else {
            confess "Error: .ppif (axi-r-beat-acceptor $name ...) has unsupported clause '($head ...)'\n";
        }
    }

    for my $required (qw(role clock reset command channel)) {
        confess "Error: .ppif (axi-r-beat-acceptor $name ...) is missing required ($required ...) clause\n"
            unless exists $contract{$required};
    }
...
sub _parse_axi_r_beat_acceptor_command_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-r-beat-acceptor $name command",
        {
            arm => 'scalar',
        },
        [qw(arm)],
...
sub _parse_axi_r_beat_acceptor_channel_block($items, $source_label, $name) {
    return _parse_ahb_binding_block(
        $items,
        $source_label,
        "axi-r-beat-acceptor $name channel",
        {
            valid               => 'scalar',
            ready               => 'scalar',
            id                  => 'width',
        ...
        [qw(valid ready id data response last captured-id captured-data captured-response captured-last busy done)],
    );
}
...
sub _is_axi_aw_driver_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_aw_driver';
}

sub _is_axi_ar_driver_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_ar_driver';
}
...
sub _is_axi_b_response_acceptor_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_b_response_acceptor';
}
...
sub _is_axi_read_burst4_transaction_composition_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_read_burst4_transaction_composition';
}

sub _is_axi_write_request_composition_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_write_request_composition';
}

sub _is_axi_write_burst4_request_composition_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_write_burst4_request_composition';
}

sub _is_axi_write_transaction_composition_contract($contract) {
    return ref($contract) eq 'HASH'
        && ($contract->{kind} // '') eq 'axi_write_transaction_composition';
}
