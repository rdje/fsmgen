...
Resolves deferred external-RTL parameter/generic override values after top
...
sub resolve_deferred_overrides ($class, %args) {
    ...
    for my $instance (@{$top->instances || []}) {
        next unless ($instance->kind // '') eq 'rtl';
...
sub _resolve_override ($class, %args) {
    ...
    my $instance_name = $instance->name // $instance->module_name // 'unknown';
    ...
    my $value_info = FSM::ParameterValueSupport->canonical_value(
        value_ast => $value_ast,
        context => "Composition top '$top_name' contains '?rtl' child '$instance_name' parameter override '$name'",
        docs_hint => $docs_hint,
