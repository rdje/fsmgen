...
my $default_revision = '329d7cf1b09319899fb59cc9863cdfffddfa40dc';
my $default_sha256 =
...
die "retired-changes-history: invalid repository root\n"
    if !defined($root) || !-d $root;
die "retired-changes-history: unsafe retired path\n"
...
my @policy_paths = qw(
    ...
    docs/knowledge/project-document-interim-lifecycle-policy.md
...
for my $relative (@policy_paths) {
    ...
    push @problems, "active policy consumer still names retired $path: $relative"
        if index($contents, $path) >= 0 || index($contents, 'change_history') >= 0;
}
...
if ($listed_status != 0) {
    ...
    my %excluded = map { $_ => 1 } (
        'scripts/check_retired_changes_history.pl',
        'scripts/check_doctrine_bootstrap.sh',
        't/1562-retired-changes-history.t',
    );
...
if (@problems) {
    print STDERR "retired-changes-history: $_\n" for @problems;
    exit 1;
}

print "retired-changes-history: exact $revision:$path recovery and zero live consumers verified\n";
exit 0;

sub usage {
    my ($status) = @_;
    print STDERR <<'USAGE';
Usage: check_retired_changes_history.pl [--root DIR] [--revision REV]
