...
die "usage: perl scripts/refresh_vial_vhdl_osvvm_gallery.pl [--check]\n"
...
die 'cannot build OSVVM gallery plan: '
    . JSON::PP->new->canonical(1)->encode($built->{diagnostics}) . "\n"
    unless $built->{ok};

my $emission = FSM::VIAL::Backend::VHDLOSVVM2026_05->emit({
...
for my $relpath (sort keys %expected) {
    ...
    if ($check_only) {
        die "OSVVM gallery snapshot is missing: $gallery_rel/$relpath\n"
            unless -f $path;
        die "OSVVM gallery snapshot has byte drift: $gallery_rel/$relpath\n"
...
if ($check_only) {
    die "OSVVM gallery review guide is missing: $gallery_rel/README.md\n"
    ...
    die "OSVVM gallery has unexpected file: $gallery_rel/$extra[0]\n" if @extra;
}

print(($check_only ? 'OSVVM 2026.05 VHDL gallery current: ' : 'refreshed '),
    scalar(grep { /\.vhd\z/ } keys %expected), ' VHDL sources; ',
    scalar(keys(%expected)) - scalar(grep { /\.vhd\z/ } keys %expected),
    ' evidence artifacts; ', scalar(@{$emission->{mapping_matrix}{mappings}}),
    ' advanced mappings; ', scalar(@{$emission->{source_map}{entries}}),
