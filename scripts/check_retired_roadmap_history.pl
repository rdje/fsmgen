...
die "retired-roadmap-history: invalid repository root\n"
    if !defined($root) || !-d $root;
die "retired-roadmap-history: unsafe retired path\n"
...
my @policy_paths = qw(
    ...
    ROADMAP_V2.md
    TOOLBOX.md
    docs/COMPOSITION_LEGACY_MAPPING.md
    docs/TASK_TREE.md
...
for my $relative (@policy_paths) {
    ...
    push @problems, "active policy consumer still names retired $path: $relative"
        if index($contents, $path) >= 0
            || index($contents, 'frozen_roadmap_status') >= 0;
}
...
if ($listed_status != 0) {
    ...
    my %excluded = map { $_ => 1 } (
        'scripts/check_retired_roadmap_history.pl',
        'scripts/check_doctrine_bootstrap.sh',
        't/1564-retired-roadmap-history.t',
    );
...
if (@problems) {
    print STDERR "retired-roadmap-history: $_\n" for @problems;
    exit 1;
}

print "retired-roadmap-history: exact $revision:$path recovery and zero live consumers verified\n";
exit 0;

sub usage {
    my ($status) = @_;
    print STDERR <<'USAGE';
Usage: check_retired_roadmap_history.pl [--root DIR] [--revision REV]
