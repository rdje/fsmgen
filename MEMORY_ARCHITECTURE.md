...
6. Before re-deriving any durable fact from code or runtime, run
   **`knowledge-map/scripts/query_knowledge_map.sh 'question words'`**. The
   bounded `KNOWLEDGE_MAP.md` landing page routes to deterministic topic shards
   derived from small front-mattered fact cards (`docs/knowledge/`). This is an
   **additive** retrieval layer on top of A–D (it converts/replaces nothing); a
   card is a signpost that points at the canonical home and carries `date` +
   `reverify` so a fact is trusted, not excavated. The full standard, scripts,
   and enforcement ship self-contained in **`knowledge-map/`**.

A resume reads A + one unit of B + a few C records — never a monolith. The Knowledge
Map turns "which layer holds fact X?" into one bounded query, so archaeology is
structurally unnecessary for any fact logged once.
